...
class _LRUCacheWrapper(Generic[T]):
    ...
    def __call__(
        ...
        if iscoroutinefunction(f):
            return AsyncLRUCacheWrapper(f, self._maxsize, self._typed, self._always_checkpoint)

        return functools.lru_cache(maxsize=self._maxsize, typed=self._typed)(f)  # type: ignore[arg-type]


@overload
def cache(func: Callable[P, Coroutine[Any, Any, T]], /) -> AsyncLRUCacheWrapper[P, T]: ...  # type: ignore[overload-overlap]
...
@overload
def lru_cache(*, maxsize: int | None = ..., typed: bool = ..., always_checkpoint: bool = ...) -> _LRUCacheWrapper[Any]: ...
