...
class MelSpectrogram(torch.nn.Module):
    ...
    def __init__(
        ...
        if onesided is not None:
            warnings.warn("Argument 'onesided' has been deprecated and has no influence on the behavior of this module.")
        ...
        self.mel_scale = MelScale(self.n_mels, self.sample_rate, self.f_min, self.f_max, self.n_fft // 2 + 1, norm, mel_scale)
...
class TimeStretch(torch.nn.Module):
    ...
    def forward(self, complex_specgrams: Tensor, overriding_rate: Optional[float] = None) -> Tensor:
        ...
        if not torch.is_complex(complex_specgrams):
            warnings.warn(
                "The input to TimeStretch must be complex type. " "Providing non-complex tensor produces invalid results.",
                stacklevel=4,
            )
