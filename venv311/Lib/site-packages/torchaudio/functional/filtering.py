...
class DifferentiableIIR(torch.autograd.Function):
    ...
    def forward(ctx, waveform, a_coeffs_normalized):
        ...
        padded_output_waveform = torch.zeros(n_batch, n_channel, n_sample_padded, device=waveform.device, dtype=waveform.dtype)
...
def lfilter(waveform: Tensor, a_coeffs: Tensor, b_coeffs: Tensor, clamp: bool = True, batching: bool = True) -> Tensor:
    ...
    if a_coeffs.size() != b_coeffs.size():
        raise ValueError(
            "Expected coeffs to be the same size." f"Found: a_coeffs size: {a_coeffs.size()}, b_coeffs size: {b_coeffs.size()}"
        )
    if a_coeffs.ndim > 2:
