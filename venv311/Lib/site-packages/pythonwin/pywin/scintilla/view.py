...
from . import IDLEenvironment  # nopycln: import # Injects fast_readline into the IDLE auto-indent extension
from . import (
    bindings,
    control,
    scintillacon,
    ...
    def AppendMenu(self, menu, text="", event=None, flags=None, checked=0):
        if event is None:
            ...
            if cmdid is None:
                ...
                print('View.AppendMenu(): Unknown event "{}" specified for menu text "{}" - ignored'.format(event, text))
                return
    ...
    def _AutoComplete(self):
        ...
        if ob is not None:
            try:  # Catch unexpected errors when fetching attribute names from the object
                ...
                win32ui.SetStatusText(f"Error attempting to get object attributes - {sys.exc_info()[0]!r}")
