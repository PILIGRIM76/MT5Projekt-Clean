...
def _info_to_str(model: str, info: dict[str, str], is_repr: bool = False, is_html: bool = False) -> str:
    if is_html:
...
class MCS(MultipleComparison):
    ...
    def _compute_max(self) -> None:
        ...
        while included.sum() > 1:
            ...
            eliminated.extend([(int(idx_val), pval) for idx_val in indices.flat[np.atleast_1d(locs.squeeze())]])
