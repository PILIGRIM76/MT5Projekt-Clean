...
class Distribution(metaclass=ABCMeta):
    ...
    def _check_constraints(self, parameters: Sequence[float] | ArrayLike1D | None) -> Float64Array1D:
...
class Normal(Distribution, metaclass=AbstractDocStringInheritor):
    ...
    def partial_moment(
        ...
        if n < 0:
            ...
            return -(z ** (n - 1)) * stats.norm.pdf(z) + (n - 1) * self.partial_moment(n - 2, z, parameters)
...
class SkewStudent(Distribution, metaclass=AbstractDocStringInheritor):
    ...
    def loglikelihood(
        ...
        llf_resid = ((const_b * resids + const_a) / (1 + sign(resids + const_a / const_b) * lam)) ** 2
    ...
    def moment(self, n: int, parameters: Sequence[float] | ArrayLike1D | None = None) -> float:
        ...
        for k in range(n + 1):  # binomial expansion around loc
            ...
            r_pmom = 0.5 * (gamma(0.5 * (k + 1)) * gamma(0.5 * (eta - k)) * eta ** (0.5 * k)) / (sqrt(pi) * gamma(0.5 * eta))
            l_pmom = ((-1) ** k) * r_pmom
