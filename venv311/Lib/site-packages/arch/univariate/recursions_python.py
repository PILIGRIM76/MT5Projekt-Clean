...
def garch_core_python(
    ...
    for j in range(o):
        if (t - 1 - j) < 0:
            ...
            sigma2[t] += parameters[loc] * (np.abs(resids[t - 1 - j]) ** power) * (resids[t - 1 - j] < 0)
        loc += 1
    for j in range(q):
        if (t - 1 - j) < 0:
            sigma2[t] += parameters[loc] * backcast
...
def figarch_weights_python(parameters: Float64Array1D, p: int, q: int, trunc_lag: int) -> Float64Array:
    r"""
    Parameters
    ----------
    parameters : ndarray
    ...
    def update(
        ...
        for j in range(self.o):
            if (t - 1 - j) < 0:
                ...
                sigma2[t] += parameters[loc] * (np.abs(resids[t - 1 - j]) ** self.power) * (resids[t - 1 - j] < 0)
            loc += 1
...
class EWMAUpdater(VolatilityUpdater, metaclass=AbstractDocStringInheritor):
    ...
    def update(
        ...
        if t == 0:
            ...
            sigma2[t] += self.params[1] * resids[t - 1] * resids[t - 1] + self.params[2] * sigma2[t - 1]
        sigma2[t] = bounds_check(sigma2[t], var_bounds[t])
