...
class HARX(ARCHModel, metaclass=AbstractDocStringInheritor):
    ...
    def __init__(
        ...
        lags: int | Sequence[int] | Sequence[Sequence[int]] | Int32Array | Int64Array | None = None,
        constant: bool = True,
        ...
        self.lags: int | Sequence[int] | Sequence[Sequence[int]] | Int32Array | Int64Array | None = lags
    ...
    def simulate(
        ...
        if params.shape[0] != num_params:
            raise ValueError(
                "params has the wrong number of elements. " "Expected " + str(num_params) + ", got " + str(params.shape[0])
            )
        ...
        sim_data = self.volatility.simulate(vol_params, nobs + burn, simulator, burn, initial_value_vol)
        errors = sim_data[0]
    ...
    def _generate_lag_names(self) -> list[str]:
        ...
        names = [var_name + "[" + str(lags[0, i]) + ":" + str(lags[1, i]) + "]" for i in range(lags.shape[1])]
        return names
    ...
    def _fit_no_arch_normal_errors_params(self) -> Float64Array1D:
        ...
        if nobs < self.num_params:
            raise ValueError(
                "Insufficient data, " + str(self.num_params) + " regressors, " + str(nobs) + " data points available"
            )
        x = self._fit_regressors
        y = self._fit_y
    ...
    def _fit_no_arch_normal_errors(self, cov_type: Literal["robust", "classic"] = "robust") -> ARCHModelResult:
        """
    ...
    def _reformat_forecast_x(
        ...
        if isinstance(x, Mapping):
            if len(x) != nx:
                raise ValueError(
                    "x must have the same number of entries as the number of x " f"variables included in the model ({nx})"
                )
            ...
            if len(base_shape) != 2:
                raise ValueError("The arrays contained in the dictionary must be 1 or 2-dimensional.")
            ...
            if arr.shape[0] != self._x.shape[1]:
                raise ValueError(
                    "The leading dimension of x must match the number of x variables " f"included in the model ({nx})"
                )
        assert isinstance(arr, np.ndarray)
        if arr.shape[2] != horizon:
            raise ValueError(
                f"The number of values passed {arr.shape[2]} does not match the " f"horizon of the forecasts ({horizon})."
            )
            ...
            for i in range(start_index, t):
                ...
                for j in range(horizon):
                    mean_paths[path_loc, :, m + j] = (
                        constant + mean_paths[path_loc, :, j : m + j].dot(dynp_rev) + shocks[path_loc, :, j]
                    )
...
class ConstantMean(HARX):
    ...
    def simulate(
        ...
        sim_values = self.volatility.simulate(vp, nobs + burn, self.distribution.simulate(dp), burn, initial_value_vol)
        errors = sim_values[0]
        y = errors + mp
...
class ARCHInMean(ARX):
    ...
    def __init__(
        ...
        if isinstance(form, str):
            ...
            if float_form == 0:
                raise ValueError("When using a floating point number for form, its value " "must be different from 0")
            self._form_id = 3
