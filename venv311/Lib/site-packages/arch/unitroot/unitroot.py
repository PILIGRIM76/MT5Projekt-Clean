...
def _autolag_ols_low_memory(
    ...
    for i in range(m, m + maxlag + 1):
        ...
        try:
            ...
            raise InfeasibleTestException(singular_array_error.format(max_lags=maxlag, lag=m - i)) from exc
...
def _autolag_ols(
    ...
    if exog_singular:
        ...
        raise InfeasibleTestException(singular_array_error.format(max_lags=maxlag, lag=max(exog_rank - startlag, 0)))
...
def _estimate_df_regression(y: Float64Array, trend: Literal["n", "c", "ct", "ctt"], lags: int) -> RegressionResults:
...
class DFGLS(UnitRootTest, metaclass=AbstractDocStringInheritor):
    ...
    def _compute_statistic(self) -> None:
        ...
        if self._lags is None:
            ...
            _, bestlag = _df_select_lags(y_ols_detrend, "n", max_lags, method, low_memory=self._low_memory)
            self._lags = bestlag
        ...
        assert self._stat is not None
        self._pvalue = mackinnonp(self._stat, regression=cast("Literal['c', 'ct']", trend), dist_type="dfgls")
        critical_values = mackinnoncrit(
...
class PhillipsPerron(UnitRootTest, metaclass=AbstractDocStringInheritor):
    ...
    def _compute_statistic(self) -> None:
        ...
        self._stat_tau = sqrt(gamma0 / lam2) * ((rho - 1) / sigma) - 0.5 * ((lam2 - gamma0) / lam) * (n * sigma / s)
...
class KPSS(UnitRootTest, metaclass=AbstractDocStringInheritor):
    ...
    def __init__(self, y: ArrayLike, lags: int | None = None, trend: Literal["c", "ct"] = "c") -> None:
        valid_trends = ("c", "ct")
        if lags is None:
            warnings.warn(
...
def mackinnonp(
    ...
    if num_unit_roots > 1 and dist_type.lower() != "adf-t":
        raise ValueError("Cointegration results (num_unit_roots > 1) are" "only available for ADF-t values")
    if dist_type == "adf-t":
