...
def estimate_cv_regression(results: pd.DataFrame, statistic: str) -> tuple[dict[int, list[float]], float]:
...
for _final_key in final:
    ...
    cv_params[final_key], cv_tau_min[final_key] = estimate_cv_regression(final[final_key], final_key[0])
...
for key, n_sim_value in nsimulation.items():
    s = key[0].upper()
    t = key[1]
    report.append(f"{s}-type statistics with trend {t} based on {n_sim_value:,} simulations")
