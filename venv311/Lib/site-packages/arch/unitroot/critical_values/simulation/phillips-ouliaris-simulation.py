...
DF_Z_COLUMNS = pd.MultiIndex.from_product([SAMPLE_SIZES, list(range(1, MAX_STOCHASTIC_TRENDS + 1)), Z_STATISTICS])
DF_P_COLUMNS = pd.MultiIndex.from_product([SAMPLE_SIZES, list(range(1, MAX_STOCHASTIC_TRENDS + 1)), P_STATISTICS])
...
def save_partial(gen: np.random.Generator, results: pd.DataFrame, remaining: int, full_path: str) -> None:
...
def worker(
    ...
    print(f"Starting Index: {BLUE}{idx}{RESET}, Statistic: {RED}{statistic}{RESET}," f"Trend: {GREEN}{trend}{RESET}")
    remaining = EX_SIZE
...
if __name__ == "__main__":
    ...
    for statistic, trend, idx in product(STATISTICS, TRENDS, range(EX_NUM)):
        ...
        filename = "phillips-ouliaris-results-statistic-" f"{statistic}-trend-{trend}-{idx:04d}.hdf"
    ...
    if njobs == 1:
        ...
        Parallel(verbose=50, n_jobs=njobs)(
            delayed(worker)(gen, statistic, trend, idx, fullfile) for (gen, statistic, trend, idx, fullfile) in jobs
        )
