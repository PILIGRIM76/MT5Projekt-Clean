...
class Cache:
            ...
            try:
                ...
                if error.errno != errno.EEXIST:
                    raise EnvironmentError(
                        error.errno,
                        'Cache directory "%s" does not exist' " and could not be created" % self._directory,
                    ) from None
    ...
    def add(self, key, value, expire=None, read=False, tag=None, retry=False):
        ...
        with self._transact(retry, filename) as (sql, cleanup):
            rows = sql(
                "SELECT rowid, filename, expire_time FROM Cache" " WHERE key = ? AND raw = ?",
                (db_key, raw),
            ).fetchall()
    ...
    def push(
        ...
        select = ("SELECT key FROM Cache" " WHERE ? < key AND key < ? AND raw = ?" " ORDER BY key %s LIMIT 1") % order[side]
    ...
    def peekitem(self, last=True, expire_time=False, tag=False, retry=False):
        ...
        order = ("ASC", "DESC")
        select = (
            "SELECT rowid, key, raw, expire_time, tag, mode, filename, value" " FROM Cache ORDER BY rowid %s LIMIT 1"
        ) % order[last]
    ...
    def evict(self, tag, retry=False):
        ...
        """
        select = "SELECT rowid, filename FROM Cache" " WHERE tag = ? AND rowid > ?" " ORDER BY rowid LIMIT ?"
        args = [tag, 0, 100]
    ...
    def iterkeys(self, reverse=False):
        ...
        if reverse:
            ...
            iterate = (
                "SELECT key, raw FROM Cache" " WHERE key = ? AND raw < ? OR key < ?" " ORDER BY key DESC, raw DESC LIMIT ?"
            )
        else:
    ...
    def _iter(self, ascending=True):
        ...
        select = ("SELECT rowid, key, raw FROM Cache" " WHERE ? < rowid AND rowid < ?" " ORDER BY rowid %s LIMIT ?") % (
