...
from .utils import EXPOSURE_EVENT, REQUEST_HEADERS, generate_traceparent, normalized_hash, prepare_common_query_params
...
class LocalFeatureFlagsProvider:
    ...
    def get_variant(
        ...
        if test_user_variant := self._get_variant_override_for_test_user(flag_definition, context):
            ...
            selected_variant = self._get_assigned_variant(flag_definition, context_value, flag_key, rollout)
        ...
        logger.debug(f"{flag_definition.context} context {context_value} not eligible for any rollout for flag: {flag_key}")
        return fallback_value
    ...
    def _fetch_flag_definitions(self) -> None:
        try:
            start_time = datetime.now()
            headers = {"traceparent": generate_traceparent()}
            response = self._sync_client.get(self.FLAGS_DEFINITIONS_URL_PATH, params=self._request_params, headers=headers)
            end_time = datetime.now()
            self._handle_response(response, start_time, end_time)
        except Exception:
            logger.exception("Failed to fetch feature flag definitions")
    ...
    def _handle_response(self, response: httpx.Response, start_time: datetime, end_time: datetime) -> None:
        ...
        logger.debug(f"Successfully fetched {len(self._flag_definitions)} flag definitions")
