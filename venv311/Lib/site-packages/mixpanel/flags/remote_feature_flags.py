...
from .utils import EXPOSURE_EVENT, REQUEST_HEADERS, generate_traceparent, prepare_common_query_params
...
class RemoteFeatureFlagsProvider:
    ...
    async def atrack_exposure_event(self, flag_key: str, variant: SelectedVariant, context: Dict[str, Any]):
        """
        ...
        if distinct_id := context.get("distinct_id"):
            ...
            await sync_to_async(self._tracker, thread_sensitive=False)(distinct_id, EXPOSURE_EVENT, properties)
        else:
            logger.error("Cannot track exposure event without a distinct_id in the context")
    ...
    def _prepare_query_params(self, context: Dict[str, Any], flag_key: Optional[str] = None) -> Dict[str, str]:
    ...
    def _lookup_flag_in_response(
        self, flag_key: str, flags: Dict[str, SelectedVariant], fallback_value: SelectedVariant
    ) -> Tuple[SelectedVariant, bool]:
        if flag_key in flags:
            return flags[flag_key], False
        else:
            logging.debug(f"Flag '{flag_key}' not found in remote response. Returning fallback, '{fallback_value}'")
