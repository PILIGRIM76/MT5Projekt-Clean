    ...
    def send(self, endpoint, json_message, api_key=None, api_secret=None):
        ...
        if endpoint not in self._endpoints:
            raise MixpanelException(
                'No such endpoint "{0}". Valid endpoints are one of {1}'.format(endpoint, self._endpoints.keys())
            )
...
class BufferedConsumer(object):
    ...
    def send(self, endpoint, json_message, api_key=None, api_secret=None):
        ...
        if endpoint not in self._buffers:
            raise MixpanelException(
                'No such endpoint "{0}". Valid endpoints are one of {1}'.format(endpoint, self._buffers.keys())
            )
