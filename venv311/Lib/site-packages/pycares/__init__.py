...
def extract_record_data(rr, record_type):
    ...
    if record_type == _lib.ARES_REC_TYPE_A:
        ...
        return SRVRecordData(priority=priority, weight=weight, port=port, target=maybe_str(_ffi.string(target)))
        ...
        return TLSARecordData(
            cert_usage=cert_usage, selector=selector, matching_type=matching_type, cert_association_data=cert_data
        )
        ...
        return HTTPSRecordData(priority=priority, target=maybe_str(_ffi.string(target)), params=params)
        ...
        return URIRecordData(priority=priority, weight=weight, target=maybe_str(_ffi.string(target)))
...
def parse_dnsrec(dnsrec):
    ...
    for i in range(answer_count):
        ...
        if rr != _ffi.NULL:
            ...
            try:
                ...
                answer_records.append(DNSRecord(name=name, type=rec_type, record_class=rec_class, ttl=ttl, data=data))
    ...
    for i in range(authority_count):
        ...
        if rr != _ffi.NULL:
            ...
            try:
                data = extract_record_data(rr, rec_type)
                authority_records.append(DNSRecord(name=name, type=rec_type, record_class=rec_class, ttl=ttl, data=data))
            except (ValueError, Exception):
                # Skip unsupported record types
                pass
    ...
    for i in range(additional_count):
        ...
        if rr != _ffi.NULL:
            ...
            try:
                data = extract_record_data(rr, rec_type)
                additional_records.append(DNSRecord(name=name, type=rec_type, record_class=rec_class, ttl=ttl, data=data))
            except (ValueError, Exception):
                # Skip unsupported record types
                pass

    result = DNSResult(answer=answer_records, authority=authority_records, additional=additional_records)
...
class Channel:
    __qtypes__ = (
        _lib.ARES_REC_TYPE_A,
        _lib.ARES_REC_TYPE_AAAA,
        _lib.ARES_REC_TYPE_ANY,
        _lib.ARES_REC_TYPE_CAA,
        _lib.ARES_REC_TYPE_CNAME,
        _lib.ARES_REC_TYPE_HTTPS,
        _lib.ARES_REC_TYPE_MX,
        _lib.ARES_REC_TYPE_NAPTR,
        _lib.ARES_REC_TYPE_NS,
        _lib.ARES_REC_TYPE_PTR,
        _lib.ARES_REC_TYPE_SOA,
        _lib.ARES_REC_TYPE_SRV,
        _lib.ARES_REC_TYPE_TLSA,
        _lib.ARES_REC_TYPE_TXT,
        _lib.ARES_REC_TYPE_URI,
    )
    __qclasses__ = (
        _lib.ARES_CLASS_IN,
        _lib.ARES_CLASS_CHAOS,
        _lib.ARES_CLASS_HESOID,
        _lib.ARES_CLASS_NONE,
        _lib.ARES_CLASS_ANY,
    )
    ...
    def process_read_fd(self, read_fd: int) -> None:
        _lib.ares_process_fd(
            self._channel[0], _ffi.cast("ares_socket_t", read_fd), _ffi.cast("ares_socket_t", ARES_SOCKET_BAD)
        )

    def process_write_fd(self, write_fd: int) -> None:
        _lib.ares_process_fd(
            self._channel[0], _ffi.cast("ares_socket_t", ARES_SOCKET_BAD), _ffi.cast("ares_socket_t", write_fd)
        )
    ...
    def query(
        self, name: str, query_type: int, *, query_class: int = QUERY_CLASS_IN, callback: Callable[[Any, int], None]
    ) -> None:
        """
        Perform a DNS query.
        ...
        status = _lib.ares_query_dnsrec(
            self._channel[0], parse_name(name), query_class, query_type, _lib._query_dnsrec_cb, userdata, qid
        )
        if status != _lib.ARES_SUCCESS:
    ...
    def search(
        ...
        status = _lib.ares_dns_record_query_add(dnsrec, parse_name(name), query_type, query_class)
        if status != _lib.ARES_SUCCESS:
        ...
        status = _lib.ares_search_dnsrec(self._channel[0], dnsrec, _lib._query_dnsrec_cb, userdata)
        if status != _lib.ARES_SUCCESS:
            _handle_to_channel.pop(userdata, None)
            _lib.ares_dns_record_destroy(dnsrec)
            raise AresError(status, errno.strerror(status))
    ...
    def getnameinfo(self, address: Union[IP4, IP6], flags: int, *, callback: Callable[[Any, int], None]) -> None:
        ...
        _lib.ares_getnameinfo(
            self._channel[0], _ffi.cast("struct sockaddr*", sa), _ffi.sizeof(sa[0]), flags, _lib._nameinfo_cb, userdata
        )
...
def parse_addrinfo_cname(ares_cname) -> AddrInfoCname:
    ...
    return AddrInfoCname(
        ttl=ares_cname.ttl, alias=maybe_str(_ffi.string(ares_cname.alias)), name=maybe_str(_ffi.string(ares_cname.name))
    )
