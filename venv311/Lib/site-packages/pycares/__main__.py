...
def format_record(record):
    ...
    if record.type == pycares.QUERY_TYPE_A:
        ...
        return "%s\t%d %d %d %s" % (prefix, data.priority, data.weight, data.port, data.target)
        ...
        return "%s\t%d %d %d %s" % (
            ...
            data.selector,
            data.matching_type,
            data.cert_association_data.hex(),
        )
...
def cb(result, error):
    if error is not None:
        ...
        parts = [";; QUESTION SECTION:", ";%s\t\t\tIN\t%s" % (hostname, qtype.upper()), "", ";; ANSWER SECTION:"]
