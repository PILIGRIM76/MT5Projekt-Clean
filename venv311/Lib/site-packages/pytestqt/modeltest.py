...
class ModelTester:
    ...
    def check(self, model, force_py=False):
        ...
        if HAS_QT_TESTER and not force_py:
            ...
            self._qt_tester = qt_api.QtTest.QAbstractItemModelTester(model, reporting_mode)
        ...
        # column operations
        self._model.columnsAboutToBeInserted.connect(self._on_columns_about_to_be_inserted)
        self._model.columnsAboutToBeMoved.connect(self._on_columns_about_to_be_moved)
        self._model.columnsAboutToBeRemoved.connect(self._on_columns_about_to_be_removed)
    ...
    def _test_parent(self):
        ...
        if self._model.hasIndex(0, 1):
            ...
            if self._model.rowCount(top_index) > 0 and self._model.rowCount(top_index_1) > 0:
                child_index = self._model.index(0, 0, top_index)
    ...
    def _check_children(self, parent, current_depth=0):
        ...
        self._debug(
            ...
            "({} rows, {} columns)".format(self._modelindex_debug(parent), current_depth, rows, columns)
        )
        ...
        for r in range(rows):
            ...
            for c in range(columns):
                ...
                if not index.isValid():
                    self._debug("Got invalid index at row={} col={} parent={}".format(r, c, self._modelindex_debug(parent)))
                ...
                if self._has_children(index) and current_depth < 10:
                    self._debug("{} has {} children".format(self._modelindex_debug(index), self._model.rowCount(index)))
    ...
    def _test_data(self):
        ...
        alignment = self._model.data(self._model.index(0, 0), qt_api.QtCore.Qt.ItemDataRole.TextAlignmentRole)
        if alignment is not None:
            try:
                alignment = int(alignment)
            except (TypeError, ValueError):
                assert 0, "%r should be a TextAlignmentRole enum" % alignment
            mask = int(qt_api.QtCore.Qt.AlignmentFlag.AlignHorizontal_Mask | qt_api.QtCore.Qt.AlignmentFlag.AlignVertical_Mask)
            assert alignment == alignment & mask

        # Check that the "check state" is one we know about.
        state = self._model.data(self._model.index(0, 0), qt_api.QtCore.Qt.ItemDataRole.CheckStateRole)
        assert state in [
            None,
    ...
    def _on_columns_about_to_be_moved(self, source_parent, source_start, source_end, dest_parent, dest_column):
        assert self._change_in_flight is None
    ...
    def _on_columns_moved(self, source_parent, source_start, source_end, dest_parent, dest_column):
    ...
    def _on_rows_about_to_be_inserted(self, parent, start, end):
        ...
        c = _Changing(parent=parent, old_size=parent_rowcount, last=last_data, next=next_data)
    ...
    def _on_rows_inserted(self, parent, start, end):
        ...
        c = self._insert.pop()
        last_data = self._model.data(self._model.index(start - 1, 0, parent)) if start - 1 >= 0 else None
        next_data = (
            self._model.data(self._model.index(end + 1, 0, c.parent)) if end + 1 < self._model.rowCount(c.parent) else None
        )
    ...
    def _on_rows_about_to_be_moved(self, source_parent, source_start, source_end, dest_parent, dest_row):
        assert self._change_in_flight is None
    ...
    def _on_rows_moved(self, source_parent, source_start, source_end, dest_parent, dest_row):
    ...
    def _on_rows_about_to_be_removed(self, parent, start, end):
        ...
        parent_rowcount = self._model.rowCount(parent)
        last_index = self._model.index(start - 1, 0, parent) if start > 0 and self._column_count(parent) > 0 else None
        next_index = (
            self._model.index(end + 1, 0, parent) if end < parent_rowcount - 1 and self._column_count(parent) > 0 else None
        )
    ...
    def _on_rows_removed(self, parent, start, end):
        ...
        c = self._remove.pop()
        last_data = self._model.data(self._model.index(start - 1, 0, c.parent)) if start > 0 else None
        next_data = self._model.data(self._model.index(start, 0, c.parent)) if end < c.old_size - 1 else None
        current_size = self._model.rowCount(parent)
        expected_size = c.old_size - (end - start + 1)
