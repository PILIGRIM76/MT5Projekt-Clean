...
class _QtApi:
    ...
    def set_qt_api(self, api):
        self.pytest_qt_api = self._get_qt_api_from_env() or api or self._get_already_loaded_backend() or self._guess_qt_api()
        ...
        if not self.pytest_qt_api:  # pragma: no cover
            errors = "\n".join(f"  {module}: {reason}" for module, reason in sorted(self._import_errors.items()))
            msg = "pytest-qt requires either PySide6, PyQt5 or PyQt6 installed.\n" + errors
            raise pytest.UsageError(msg)
    ...
    def _check_qt_api_version(self):
        ...
        if self.QtCore.PYQT_VERSION == 0x060000:  # 6.0.0
            raise pytest.UsageError("PyQt 6.0 is not supported by pytest-qt, use 6.1+ instead.")
        elif self.QtCore.PYQT_VERSION < 0x050B00:  # 5.11.0
            raise pytest.UsageError("PyQt < 5.11 is not supported by pytest-qt, use 5.11+ instead.")
    ...
    def get_versions(self):
        if self.pytest_qt_api == "pyside6":
            ...
            return VersionTuple("PySide6", version, self.QtCore.qVersion(), self.QtCore.__version__)
