    ...
    def pytest_runtest_setup(self, item):
        ...
        if m:
            if not set(m.kwargs).issubset({"extend"}):
                raise ValueError("Invalid keyword arguments in {!r} for " "qt_log_ignore mark.".format(m.kwargs))
        ...
        if call.when == "call":
            ...
            if report.outcome != "failed":
                for rec in item.qt_log_capture.records:
                    is_modeltest_error = (
                        rec.context is not None and rec.context.category == "qt.modeltest" and rec.matches_level("WARNING")
                    )
                    if (rec.matches_level(log_fail_level) and not rec.ignored) or is_modeltest_error:
                        report.outcome = "failed"
                        if report.longrepr is None:
                            report.longrepr = _QtLogLevelErrorRepr(item, log_fail_level, is_modeltest_error)
                        break
                ...
                if hasattr(long_repr, "addsection"):  # pragma: no cover
                    ...
                    for rec in item.qt_log_capture.records:
                        ...
                        if (
                            ...
                            and (rec.context.file is not None or rec.context.function is not None or rec.context.line != 0)
...
class _QtMessageCapture:
    ...
    def _start(self):
        ...
        """
        previous_handler = qt_api.QtCore.qInstallMessageHandler(self._handle_with_context)
...
class _QtLogLevelErrorRepr(TerminalRepr):
    ...
    def __init__(self, item, level, is_modeltest_error):
        ...
        self.fileloc = ReprFileLocation(path, lineno=line_index + 1, message=msg.format(level.upper()))
        self.sections = []
