...
class _AbstractSignalBlocker:
    ...
    def _extract_pyqt_signal_name(self, potential_pyqt_signal):
        ...
        if not isinstance(signal_name, str):
            raise TypeError(
                "Invalid 'signal' attribute in {}. " "Expected str but got {}".format(signal_name, type(signal_name))
            )
    ...
    def _extract_signal_from_signal_tuple(self, potential_signal_tuple):
        if isinstance(potential_signal_tuple, tuple):
            ...
            if not isinstance(signal_name, str):
                raise TypeError("Invalid type for provided signal name, " "expected str but got {}".format(type(signal_name)))
            if not signal_name:
...
class MultiSignalBlocker(_AbstractSignalBlocker):
    ...
    def _record_emitted_signal_if_possible(self, unique_signal, *args):
        if self._are_signal_names_available():
            self.all_signals_and_args.append(SignalAndArgs(signal_name=self._signal_names[unique_signal], args=args))
    ...
    def _get_unemitted_signal_indices(self, signal):
        ...
        return [index for index in self._signals_map[signal] if not self._signals_emitted[index]]
    ...
    def _get_order_violation_message(self):
        ...
        return ("Signal order violated! Expected {expected} as {ordinal} signal, " "but received {actual} instead. ").format(
        ...
        if self.emitted:
            if self.args:
                raise SignalEmittedError(
                    "Signal %r unexpectedly emitted with " "arguments %r" % (self.signal, list(self.args))
                )
            else:
