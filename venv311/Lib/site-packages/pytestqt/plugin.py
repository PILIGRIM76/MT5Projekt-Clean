...
def qapp(qapp_args, qapp_cls, pytestconfig):
    ...
    if app is None:
        ...
        if not isinstance(app, qapp_cls):
            warnings.warn(f"Existing QApplication {app} is not an instance of qapp_cls: " f"{qapp_cls}")
        return app
...
def pytest_addoption(parser):
    ...
    parser.addini("qt_qapp_name", "The Qt application name to use", default="pytest-qt-qapp")

    default_log_fail = QtLoggingPlugin.LOG_FAIL_OPTIONS[0]
    parser.addini(
        "qt_log_level_fail",
        'log level in which tests can fail: {} (default: "{}")'.format(QtLoggingPlugin.LOG_FAIL_OPTIONS, default_log_fail),
...
def pytest_configure(config):
    config.addinivalue_line(
        "markers",
        "qt_no_exception_capture: Disables pytest-qt's automatic exception " "capture for just one test item.",
    )

    config.addinivalue_line("markers", "qt_log_level_fail: overrides qt_log_level_fail ini option.")
    config.addinivalue_line("markers", "qt_log_ignore: overrides qt_log_ignore ini option.")
