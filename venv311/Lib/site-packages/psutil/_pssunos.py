...
def swap_memory():
    ...
    used = total - free
    percent = usage_percent(used, total, round_=1)
    return ntp.sswap(total, used, free, percent, sin * PAGE_SIZE, sout * PAGE_SIZE)
...
class Process:
    ...
    def net_connections(self, kind="inet"):
        ...
        # UNIX sockets
        if kind in {"all", "unix"}:
            ret.extend([ntp.pconn(*conn) for conn in self._get_unix_sockets(self.pid)])
        return ret
    ...
    def num_ctx_switches(self):
        return ntp.pctxsw(*cext.proc_num_ctx_switches(self.pid, self._procfs_path))
