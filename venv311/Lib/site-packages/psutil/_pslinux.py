        ...
        for proto_name, family, type_ in self.tmap[kind]:
            ...
            for fd, family, type_, laddr, raddr, status, bound_pid in ls:
                if pid:
                    ...
                    conn = ntp.sconn(fd, family, type_, laddr, raddr, status, bound_pid)
                ret.add(conn)
        return list(ret)
...
def net_if_stats():
    ...
    for name in names:
        try:
            ...
            ret[name] = ntp.snicstats(isup, duplex_map[duplex], speed, mtu, output_flags)
    return ret
...
class Process:
    ...
    def cpu_times(self):
        ...
        return ntp.pcputimes(utime, stime, children_utime, children_stime, iowait)
            ...
            try:
                ...
                if path.startswith("/") and isfile_strict(path):
                    ...
                    try:
                        ...
                        ntuple = ntp.popenfile(path, int(fd), int(pos), mode, flags)
                        retlist.append(ntuple)
        if hit_enoent:
            self._raise_if_not_alive()
        return retlist
