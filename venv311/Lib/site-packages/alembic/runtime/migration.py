...
from typing import TYPE_CHECKING, Any, Callable, Collection, Dict, Iterable, Iterator, List, Optional, Set, Tuple, Union, cast
...
class MigrationContext:
    ...
    def get_current_revision(self) -> Optional[str]:
        ...
        if len(heads) == 0:
            ...
            raise util.CommandError(
                "Version table '%s' has more than one head present; " "please use get_current_heads()" % self.version_table
            )
        else:
            return heads[0]
    ...
    def get_current_heads(self) -> Tuple[str, ...]:
        ...
        if self.as_sql:
            ...
            if start_from_rev == "base":
                ...
                start_from_rev = [
                    self.script.get_revision(sfr).revision for sfr in util.to_list(start_from_rev) if sfr not in (None, "base")
                ]
...
class HeadMaintainer:
    ...
    def _delete_version(self, version: str) -> None:
        ...
        ret = self.context.impl._exec(
            self.context._version.delete().where(self.context._version.c.version_num == literal_column("'%s'" % version))
        )
    ...
    def _unmerge_to_revisions(self, heads: Set[str]) -> Tuple[str, ...]:
        ...
        if other_heads:
            ...
            ancestors = {
                ...
                for r in self.revision_map._get_ancestor_nodes(self.revision_map.get_revisions(to_revision), check=False)
