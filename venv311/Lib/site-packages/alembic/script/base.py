...
class ScriptDirectory:
    ...
    def _generate_create_date(self) -> datetime.datetime:
        if self.timezone is not None:
            if ZoneInfo is None:
                raise util.CommandError(
                    "Python >= 3.9 is required for timezone support or " "the 'backports.zoneinfo' package must be installed."
                )
        ...
        if depends_on:
            with self._catch_revision_errors():
                resolved_depends_on = [
                    ...
                    for rev, dep in [(not_none(self.revision_map.get_revision(dep)), dep) for dep in util.to_list(depends_on)]
                ]
        else:
        ...
        for root, dirs, files in compat.path_walk(path, top_down=True):
            ...
            if scriptdir.sourceless:
                ...
                if py_cache_path.exists():
                    ...
                    paths.extend(py_cache_path / pyc for pyc in py_cache_path.iterdir() if pyc.name.split(".")[0] not in names)
