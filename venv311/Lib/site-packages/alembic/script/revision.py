...
class RevisionMap:
    ...
    def add_revision(self, revision: Revision, _replace: bool = False) -> None:
        ...
        if revision._is_real_head:
            self._real_heads = tuple(
                head for head in self._real_heads if head not in set(revision._all_down_revisions).union([revision.revision])
            ) + (revision.revision,)
        if revision.is_head:
            self.heads = tuple(
                head for head in self.heads if head not in set(revision._versioned_down_revisions).union([revision.revision])
            ) + (revision.revision,)
            ...
            if not revs:
                raise ResolutionError(
                    ...
                    % (
                        resolved_id,
                        (
                            "; please ensure at least four characters are " "present for partial revision identifier matches"
                ...
                raise ResolutionError(
                    "Multiple revisions start " "with '%s': %s..." % (resolved_id, ", ".join("'%s'" % r for r in revs[0:3])),
                    resolved_id,
                )
            else:
    ...
    def _shares_lineage(
        ...
        resolved_test_against_revs = [
            (self._revision_for_ident(test_against_rev) if not isinstance(test_against_rev, Revision) else test_against_rev)
    ...
    def _collect_upgrade_revisions(
        ...
        current_node_set = set(self._get_ancestor_nodes(current_revisions, check=True, include_dependencies=True)).union(
            current_revisions
        )
...
class Revision:
    ...
    def verify_rev_id(cls, revision: str) -> None:
        ...
        if illegal_chars:
            raise RevisionError(
                "Character(s) '%s' not allowed in revision identifier '%s'" % (", ".join(sorted(illegal_chars)), revision)
            )
