...
class MSSQLImpl(DefaultImpl):
    ...
    def alter_column(
        ...
        *,
        ...
        existing_server_default: Union[_ServerDefaultType, Literal[False], None] = None,
    ...
    def bulk_insert(self, table: Union[TableClause, Table], rows: List[dict], **kw: Any) -> None:  # type: ignore[override]
        if self.as_sql:
    ...
    def adjust_reflected_dialect_options(self, reflected_object: _ReflectedConstraint, kind: str) -> Dict[str, Any]:
...
def _add_column_comment(
    ...
    assert schema_name
    return "exec sp_addextendedproperty 'MS_Description', {}, " "'schema', {}, 'table', {}, 'column', {}".format(
...
def _update_column_comment(
    ...
    schema_name = schema if schema else compiler.dialect.default_schema_name
    assert schema_name
    return "exec sp_updateextendedproperty 'MS_Description', {}, " "'schema', {}, 'table', {}, 'column', {}".format(
        compiler.sql_compiler.render_literal_value(comment, sqltypes.NVARCHAR()),
        compiler.preparer.quote_schema(schema_name),
        compiler.preparer.quote(tname),
        compiler.preparer.quote(cname),
...
def _drop_column_comment(compiler: MSDDLCompiler, schema: Optional[str], tname: str, cname: str) -> str:
    schema_name = schema if schema else compiler.dialect.default_schema_name
    assert schema_name
    return "exec sp_dropextendedproperty 'MS_Description', " "'schema', {}, 'table', {}, 'column', {}".format(
        compiler.preparer.quote_schema(schema_name),
        compiler.preparer.quote(tname),
        compiler.preparer.quote(cname),
    )
...
def visit_column_comment(element: ColumnComment, compiler: MSDDLCompiler, **kw: Any) -> str:
    if element.comment is not None:
        ...
        return _drop_column_comment(compiler, element.schema, element.table_name, element.column_name)
