...
def _add_table(autogen_context: AutogenContext, op: ops.CreateTableOp) -> str:
    ...
    args = [col for col in [_render_column(col, autogen_context) for col in table.columns] if col] + sorted(
        [
            rcons
            for rcons in [_render_constraint(cons, autogen_context, op._namespace_metadata) for cons in table.constraints]
...
def _render_column(column: Column[Any], autogen_context: AutogenContext) -> str:
    ...
    if column.server_default:
        rendered = _render_server_default(column.server_default, autogen_context)  # type: ignore[assignment]
        if rendered:
...
def _should_render_server_default_positionally(server_default: Any) -> bool:
    return sqla_compat._server_default_is_computed(server_default) or sqla_compat._server_default_is_identity(server_default)
...
def _render_primary_key(
    ...
    return "%(prefix)sPrimaryKeyConstraint(%(args)s)" % {
        "prefix": _sqlalchemy_autogenerate_prefix(autogen_context),
        "args": ", ".join([repr(c.name) for c in constraint.columns] + ["%s=%s" % (kwname, val) for kwname, val in opts]),
    }
...
def _render_foreign_key(
    ...
    return "%(prefix)sForeignKeyConstraint([%(cols)s], " "[%(refcols)s], %(args)s)" % {
        ...
        "refcols": ", ".join(repr(_fk_colspec(f, apply_metadata_schema, namespace_metadata)) for f in constraint.elements),
