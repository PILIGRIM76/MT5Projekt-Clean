...
import logging
from typing import TYPE_CHECKING, Any, Collection, Dict, Mapping, Optional, TypeVar, Union, cast
...
if TYPE_CHECKING:
    from sqlalchemy.engine.interfaces import ReflectedForeignKeyConstraint, ReflectedIndex, ReflectedUniqueConstraint
...
def _compare_indexes_and_uniques(
    ...
    if conn_table is not None:
        ...
        try:
            conn_uniques_reflected = _InspectorConv(inspector).get_unique_constraints(tname, schema=schema)
        ...
        if is_drop_table:
            ...
            conn_uniques = {_make_unique_constraint(impl, uq_def, conn_table) for uq_def in conn_uniques_reflected}

        conn_indexes = {
            index for index in (_make_index(impl, ix, conn_table) for ix in conn_indexes_reflected) if index is not None
        }
...
def _make_index(impl: DefaultImpl, params: ReflectedIndex, conn_table: Table) -> Optional[Index]:
...
def _make_unique_constraint(impl: DefaultImpl, params: ReflectedUniqueConstraint, conn_table: Table) -> UniqueConstraint:
...
def _make_foreign_key(params: ReflectedForeignKeyConstraint, conn_table: Table) -> ForeignKeyConstraint:
...
def _compare_foreign_keys(
    ...
    for removed_sig in set(conn_fks_by_sig).difference(metadata_fks_by_sig):
        ...
        if removed_sig not in metadata_fks_by_sig:
            compare_to = metadata_fks_by_name[const.name].const if const.name and const.name in metadata_fks_by_name else None
    ...
    for added_sig in set(metadata_fks_by_sig).difference(conn_fks_by_sig):
        ...
        if added_sig not in conn_fks_by_sig:
            compare_to = conn_fks_by_name[const.name].const if const.name and const.name in conn_fks_by_name else None
