    ...
    def wait_result_broken_or_wakeup(self):
        ...
        if result_reader in ready:
            ...
            mp.util.debug(
                ...
                + str({p.name: p.exitcode for p in list(self.processes.values()) if p is not None and p.sentinel in ready})
            )
...
class ProcessPoolExecutor(Executor):
    ...
    def _adjust_process_count(self):
        ...
        mp.util.debug(
            f"Adjusted process count to {self._max_workers}: " f"{[(p.name, pid) for pid, p in self._processes.items()]}"
        )
