    ...
    def post_exec(self) -> None:
        if self.isdelete and cast(SQLCompiler, self.compiled).effective_returning and not self.cursor.description:
            ...
            self.cursor_fetch_strategy = _cursor.FullyBufferedCursorFetchStrategy(
                self.cursor,
                [(entry.keyname, None) for entry in cast(SQLCompiler, self.compiled)._result_columns],  # type: ignore[misc]
                [],
            )
...
class MySQLCompiler(compiler.SQLCompiler):
    ...
    def visit_json_path_getitem_op_binary(self, binary: elements.BinaryExpression[Any], operator: Any, **kw: Any) -> str:
        return self._render_json_extract_from_binary(binary, operator, **kw)
    ...
    def visit_is_not_distinct_from_binary(self, binary: elements.BinaryExpression[Any], operator: Any, **kw: Any) -> str:
        return "%s <=> %s" % (
            self.process(binary.left),
            self.process(binary.right),
        )
...
class MySQLDialect(default.DefaultDialect):
    ...
    def _correct_for_mysql_bugs_88718_96365(
        ...
        schema_by_table_by_column: DefaultDict[str, DefaultDict[str, List[str]]] = DefaultDict(lambda: DefaultDict(list))
        for rec in fkeys:
