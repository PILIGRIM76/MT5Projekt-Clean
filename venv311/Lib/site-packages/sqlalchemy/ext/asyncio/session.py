...
if TYPE_CHECKING:
    from ...engine import Connection, Engine, Result, Row, RowMapping, ScalarResult
...
class AsyncSessionTransaction(
    ...
    async def start(self, is_ctxmanager: bool = False) -> AsyncSessionTransaction:
        self.sync_transaction = self._assign_proxied(
            await greenlet_spawn(self.session.sync_session.begin_nested if self.nested else self.session.sync_session.begin)
        )
        if is_ctxmanager:
