    ...
    def _adjust_for_extra_criteria(self, context: QueryContext) -> Load:
        ...
        def process(
            ...
            cloned_opt._extra_criteria = tuple(
                replacement_cache_key._apply_params_to_element(orig_cache_key, crit) for crit in cloned_opt._extra_criteria
            )
...
class _LoadElement(cache_key.HasCacheKey, traversals.HasShallowCopy, visitors.Traversible):
    ...
    def _raise_for_no_match(self, parent_loader, mapper_entities):
        ...
        if not found_entities:
            ...
            raise sa_exc.ArgumentError(
                f"Mapped class {path[0]} does not apply to any of the " f"root entities in this query, e.g. " f"""{
                    ", ".join(
                        str(x.entity_zero)
...
class _AttributeStrategyLoad(_LoadElement):
    ...
    def _prepend_path(self, path: PathRegistry) -> Self:
        ...
        if self._path_with_polymorphic_path is not None:
            cloned._path_with_polymorphic_path = PathRegistry.coerce(path[0:-1] + self._path_with_polymorphic_path[:])
