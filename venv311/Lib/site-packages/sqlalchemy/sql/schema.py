    ...
    def _set_parent(  # type: ignore[override]
        ...
        **kw: Any,
        ...
        if existing is not None and existing is not table:
            raise exc.ArgumentError(f"Column object '{self.key}' already " f"assigned to Table '{existing.description}'")
        ...
        if existing_col is not None:
            if existing_col is not self:
                if not allow_replacements:
                    raise exc.DuplicateColumnError(f"A column with {conflicts_on} " f"""'{
    ...
    def reflect(
        ...
        **dialect_kwargs: Any,
        ...
        with inspection.inspect(bind)._inspection_context() as insp:
            ...
            available: util.OrderedSet[str] = util.OrderedSet(insp.get_table_names(schema, **dialect_kwargs))
            if views:
            ...
            if only is None:
                ...
                if missing:
                    ...
                    raise exc.InvalidRequestError(
                        f"Could not reflect: requested table(s) not available " f"in {bind.engine!r}{s}: ({missing_str})"
                    )
