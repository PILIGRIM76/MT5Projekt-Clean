...
class SQLCompiler(Compiled):
    ...
    def visit_bindparam(
        ...
        **kwargs,
        ...
        if is_upsert_set and bindparam.value is None and bindparam.callable is None and self._insertmanyvalues is not None:
            self._insertmanyvalues = self._insertmanyvalues._replace(has_upsert_bound_parameters=True)
    ...
    **kw)`` and returns a select object.   this is used for structural changes
    ...
    def _deliver_insertmanyvalues_batches(
        ...
        if imv.is_default_expr and not self.dialect.supports_default_metavalue:
            ...
            and (imv.sentinel_columns is None or (imv.includes_upsert_behaviors and not imv.embed_values_counter))
        ):
            ...
            use_row_at_a_time = True
            downgraded = True
        elif imv.has_upsert_bound_parameters and not imv.embed_values_counter and self._result_columns:
    ...
    def visit_insert(self, insert_stmt, visited_bindparam=None, visiting_cte=None, **kw):
        ...
        if compile_state._has_multi_parameters:
            if not self.dialect.supports_multivalues_insert:
                ...
                raise exc.CompileError(
                    "RETURNING cannot be deterministically sorted when " "using an INSERT which includes multi-row values()."
                )
            crud_params_single = crud_params_struct.single_params
        else:
            crud_params_single = crud_params_struct.single_params
        ...
        if insert_stmt.select is not None:
            ...
            if (
                ...
                insert_single_values_expr = ", ".join(
                    [value for col, _, value, _ in crud_params_single if col not in add_sentinel_set]
                )

                colnames = ", ".join(f"p{i}" for i, cp in enumerate(crud_params_single) if cp[0] not in add_sentinel_set)
                ...
                insert_crud_params = [elem for elem in crud_params_single if elem[0] not in add_sentinel_set]
