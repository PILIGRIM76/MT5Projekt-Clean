...
from PySide6.QtCore import QCoreApplication, QDateTime, QDeadlineTimer, QEventLoop, QObject, QThread, QTimer, Slot
...
class QAsyncioEventLoop(asyncio.BaseEventLoop, QObject):
        ...
        for result, asyncgen in zip(results, self._asyncgens):
            if isinstance(result, Exception):
                self.call_exception_handler(
                    {
                        "message": f"Closing asynchronous generator {asyncgen}" f"raised an exception",
                        "exception": result,
                        "asyncgen": asyncgen,
                    }
                )
    ...
    async def shutdown_default_executor(self, timeout: int | float | None = None) -> None:  # type: ignore[override]
    ...
    async def create_datagram_endpoint(
        ...
        *,
        ...
        raise NotImplementedError("QAsyncioEventLoop.create_datagram_endpoint() is not implemented yet")
    ...
    async def create_unix_connection(
        ...
        *,
        ...
        raise NotImplementedError("QAsyncioEventLoop.create_unix_connection() is not implemented yet")
    ...
    async def connect_accepted_socket(
        ...
        raise NotImplementedError("QAsyncioEventLoop.connect_accepted_socket() is not implemented yet")
    ...
    def run_in_executor(
        self, executor: concurrent.futures.ThreadPoolExecutor | None, func: Callable[[Unpack[_Ts]], _T], *args: Unpack[_Ts]
    ) -> asyncio.Future[_T]:
        if self.is_closed():
            raise RuntimeError("Event loop is closed")
        if executor is None:
