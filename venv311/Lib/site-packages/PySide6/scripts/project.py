...
class Project:
    ...
    def _qml_module_check(self):
        ...
        if not self.cl_options.quiet:
            ...
            print(f"{self.project.project_file.name}, {count} QML file(s)," f" {self._qml_project_data}")
...
def main(args: Namespace):
    ...
    cl_options = ClOptions(dry_run=args.dry_run, quiet=args.quiet, force=args.force, qml_module=args.qml_module)  # noqa: F841

    if new_project_type := NewProjectTypes.find_by_command(mode):
        if not project_dir:
            print(f"Error creating new project: {mode} requires a directory name or path", file=sys.stderr)
            sys.exit(1)
