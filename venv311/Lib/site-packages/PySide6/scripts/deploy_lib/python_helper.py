...
class PythonExecutable:
    ...
    def __init__(self, python_path: Path = None, dry_run: bool = False, init: bool = False, force: bool = False):
        ...
        if not python_path:
            ...
            if not self.is_venv() and not force and not self.dry_run and not self.init:
                response = input(
                    (
                        "You are not using a virtual environment. pyside6-deploy needs "
        ...
        for package in packages:
            ...
            if (package_name not in installed_packages) and (not self.is_installed(package_name)):
                ...
                if package_version != installed_version:
                    logging.info(f"[DEPLOY] Installing package: {package_name}" f"version: {package_version}")
                    run_command(
