...
from project_lib import DesignStudioProject, ProjectData, resolve_valid_project_file
...
PERMISSION_MAP = {
    "Bluetooth": "NSBluetoothAlwaysUsageDescription:BluetoothAccess",
...
class BaseConfig:
    ...
    def __init__(self, config_file: Path, comment_prefixes: str = "/", existing_config_file: bool = False) -> None:
        self.config_file = config_file
        self.existing_config_file = existing_config_file
        self.parser = ConfigParser(comment_prefixes=comment_prefixes, strict=False, allow_no_value=True)
    ...
    def update_config(self):
        ...
        with open(temp_file_path, "r") as temp_file, open(self.config_file, "w") as config_file:
            ...
            for line in temp_file:
                if line.lstrip().startswith("#") and previous_line is not None and not previous_line.lstrip().startswith("#"):
                    config_file.write("\n")
...
class Config(BaseConfig):
    ...
    def __init__(
        ...
        self.source_file = Path(self.set_or_fetch(property_value=source_file, property_key="input_file")).resolve()
        ...
        if source_file and not DesignStudioProject.is_ds_project(source_file):
            ...
            if config_qml_files and self.project_dir and self.existing_config_file:
                self._qml_files = [Path(self.project_dir) / file for file in config_qml_files.split(",")]
            else:
    ...
    def set_or_fetch(self, property_value, property_key, property_group="app") -> str:
        ...
        raise RuntimeError(f"[DEPLOY] No value for {property_key} specified in config file or as cli option")
    ...
    def qml_files(self, qml_files: list[Path]):
        ...
        qml_files = [
            (
                str(file.absolute().relative_to(self.project_dir.absolute()))
                if file.absolute().is_relative_to(self.project_dir)
                else str(file.absolute())
            )
    ...
    def _find_qml_files(self):
        ...
        if self.project_data:
            qml_files = [(self.project_dir / str(qml_file)) for qml_file in self.project_data.qml_files]
            for sub_project_file in self.project_data.sub_projects_files:
                qml_files.extend(
                    [self.project_dir / str(qml_file) for qml_file in ProjectData(project_file=sub_project_file).qml_files]
                )
        else:
            # Filter out files from DEFAULT_IGNORE_DIRS
            qml_files = [
                file
                for file in self.project_dir.glob("**/*.qml")
    ...
    def _find_project_file(self) -> Path | None:
        ...
        try:
            ...
            logging.warning(
                f"[DEPLOY] Unable to resolve a valid project file. Proceeding without a" f" project file. Details:\n{e}."
            )
        return None
    ...
    def _find_excluded_qml_plugins(self) -> list[str] | None:
        ...
        self.qml_modules = set(run_qmlimportscanner(project_dir=self.project_dir, dry_run=self.dry_run))
    ...
    def _find_pysidemodules(self) -> list[str]:
        modules = find_pyside_modules(
            project_dir=self.project_dir, extra_ignore_dirs=self.extra_ignore_dirs, project_data=self.project_data
        )
        logging.info("The following PySide modules were found from the Python files of " f"the project {modules}")
        return modules
...
class DesktopConfig(Config):
    ...
    def __init__(
        ...
        super().__init__(config_file, source_file, python_exe, dry_run, existing_config_file, extra_ignore_dirs, name=name)
        ...
        if self.get_value("qt", "plugins"):
            ...
            self.qt_plugins = self.dependency_reader.find_plugin_dependencies(self.modules, python_exe)
        ...
        if DesignStudioProject.is_ds_project(self.source_file):
            ...
            if not ds_project.compiled_resources_available():
                raise RuntimeError(
                    f"[DEPLOY] Compiled resources file not found: "
    ...
    def _find_dependent_qt_modules(self, modules: list[str]) -> list[str]:
        ...
        if not self.dependency_reader.lib_reader:
            warnings.warn(
                f"[DEPLOY] Unable to find {self.dependency_reader.lib_reader_name}. This "
                f"tool helps to find the Qt module dependencies of the application. "
                f"Skipping checking for dependencies.",
    ...
    def _find_permissions(self) -> list[str]:
        ...
        """
        permissions = []
        perm_categories = find_permission_categories(
            project_dir=self.project_dir, extra_ignore_dirs=self.extra_ignore_dirs, project_data=self.project_data
        )

        perm_categories_str = ",".join(perm_categories)
        logging.info(
            f"[DEPLOY] Usage descriptions for the {perm_categories_str} will be added to "
