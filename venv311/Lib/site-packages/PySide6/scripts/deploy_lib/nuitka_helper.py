...
from . import DEFAULT_IGNORE_DIRS, MAJOR_VERSION, PLUGINS_TO_REMOVE, run_command
...
class Nuitka:
    ...
    def __init__(self, nuitka):
        ...
        self.qt_plugins_to_ignore = [
            "imageformats",  # being Nuitka `sensible`` plugins
            "iconengines",
            "mediaservice",
            "printsupport",
            "platforms",
    ...
    def _create_windows_command(self, source_file: Path, command: list):
        ...
        new_source_content = nuitka_comment_options_str + Path(source_file).read_text(encoding="utf-8")
    ...
    def create_executable(
        ...
        if qml_files:
            ...
            for subdir in source_file.parent.iterdir():
                if subdir.is_dir() and subdir.name not in DEFAULT_IGNORE_DIRS:
                    extra_args.append(f"--include-data-dir={subdir}=" f"./{subdir.name}")
                    all_relevant_subdirs.append(subdir)

            # find all the qml files that are not included via the data directories
            extra_qml_files = [file for file in qml_files if file.parent not in all_relevant_subdirs]
        ...
        if sys.platform == "win32" and len(" ".join(str(cmd) for cmd in command)) > 7000:
            logging.info(
                "[DEPLOY] Nuitka command too long for Windows. "
