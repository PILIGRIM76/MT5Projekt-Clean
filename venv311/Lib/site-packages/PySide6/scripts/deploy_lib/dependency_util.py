...
from . import DEFAULT_IGNORE_DIRS, IMPORT_WARNING_PYSIDE, run_command
...
def get_py_files(project_dir: Path, extra_ignore_dirs: tuple[Path] = None, project_data=None):
    ...
    if project_data:
        ...
        def add_uic_qrc_candidates(candidates, candidate_type):
            ...
            if missing_files:
                missing_details = "\n".join(
                    f"{candidate_type.upper()} file: {src} -> Missing Python file: {dst}" for src, dst in missing_files
                )
                warnings.warn(
...
def find_permission_categories(project_dir: Path, extra_ignore_dirs: list[Path] = None, project_data=None):
    ...
    def pyside_permission_imports(py_file: Path):
        ...
        try:
            ...
            raise RuntimeError(f"[DEPLOY] Finding permission categories failed on file " f"{str(py_file)} with error {e}")
...
def find_pyside_modules(project_dir: Path, extra_ignore_dirs: list[Path] = None, project_data=None):
    ...
    def pyside_module_imports(py_file: Path):
        ...
        try:
            ...
            raise RuntimeError(f"[DEPLOY] Finding module import failed on file {str(py_file)} with " f"error {e}")
...
class QtDependencyReader:
    ...
    def find_dependencies(self, module: str, used_modules: set[str] = None):
        ...
        if not qt_module_path.exists():
            warnings.warn(
                f"[DEPLOY] {qt_module_path.name} not found in {str(qt_module_path)}." "Skipping finding its dependencies.",
                category=RuntimeWarning,
            )
            return
    ...
    def find_plugin_dependencies(self, used_modules: list[str], python_exe: Path) -> list[str]:
        ...
        _, installed_packages = run_command(command=[str(python_exe), "-m", "pip", "freeze"], dry_run=False, fetch_output=True)
        ...
        for pyside_wheel in pyside_wheels:
            ...
            if not pyside_mod_plugin_json_file.exists():
                warnings.warn(f"[DEPLOY] Unable to find {pyside_mod_plugin_json_file}.", category=RuntimeWarning)
                continue
