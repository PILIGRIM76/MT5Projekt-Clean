...
def scatter(
    ...
    if interaction_index is not None:
        ...
        ax.scatter(xlim[0] * np.ones(xv_nan.sum()), s[xv_nan], marker=MarkerStyle(1), linewidth=2, color=color, alpha=alpha)
    ax.set_xlim(xlim)
...
def dependence_legacy(
    ...
    assert shap_values.shape[1] == features.shape[1], "'shap_values' must have the same number of columns as 'features'!"
