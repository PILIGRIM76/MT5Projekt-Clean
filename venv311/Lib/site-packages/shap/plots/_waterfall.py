...
def waterfall(shap_values, max_display=10, show=True):
    ...
    for i in range(num_individual):
        ...
        if features is None:
            ...
            if np.issubdtype(type(features[order[i]]), np.number):
                yticklabels[rng[i]] = format_value(float(features[order[i]]), "%0.03f") + " = " + str(feature_names[order[i]])
            else:
    ...
    tick_labels[1].set_transform(
        tick_labels[1].get_transform() + matplotlib.transforms.ScaledTranslation(22 / 72.0, -1 / 72.0, fig.dpi_scale_trans)
    )
