...
class TFDeep(Explainer):
    ...
    def shap_values(self, X, ranked_outputs=None, output_rank_order="max", check_additivity=True):
        ...
        if len(self.model_inputs) != len(X):
            raise ValueError(f"Number of model inputs ({len(self.model_inputs)}) does not match the number given ({len(X)})!")
...
def maxpool(explainer, op, *grads):
    ...
    return tf.tile(tf.where(tf.abs(delta_in0) < 1e-7, tf.zeros_like(delta_in0), (xmax_pos + rmax_pos) / delta_in0), dup0)
