...
class PyTorchDeep(Explainer):
    ...
    def gradient(self, idx, inputs):
        ...
        if self.interim:
            ...
            for idx, x in enumerate(X):
                grad = torch.autograd.grad(selected, x, retain_graph=True if idx + 1 < len(X) else None, allow_unused=True)[0]
                if grad is not None:
                    grad = grad.cpu().numpy()
                else:
    ...
    def shap_values(self, X, ranked_outputs=None, output_rank_order="max", check_additivity=True):
        ...
        if ranked_outputs is not None and self.multi_output:
            ...
            model_output_ranks = torch.ones((X[0].shape[0], self.num_outputs)).int() * torch.arange(0, self.num_outputs).int()
