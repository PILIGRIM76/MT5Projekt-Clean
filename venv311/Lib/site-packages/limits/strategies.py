...
class MovingWindowRateLimiter(RateLimiter):
    ...
    def __init__(self, storage: StorageTypes):
        if not (hasattr(storage, "acquire_entry") or hasattr(storage, "get_moving_window")):
            raise NotImplementedError(
                "MovingWindowRateLimiting is not implemented for storage " f"of type {storage.__class__}"
            )
...
class SlidingWindowCounterRateLimiter(RateLimiter):
    ...
    def __init__(self, storage: StorageTypes):
        if not hasattr(storage, "get_sliding_window") or not hasattr(storage, "acquire_sliding_window_entry"):
            raise NotImplementedError(
                "SlidingWindowCounterRateLimiting is not implemented for storage " f"of type {storage.__class__}"
            )
        super().__init__(storage)
    ...
    def test(self, item: RateLimitItem, *identifiers: str, cost: int = 1) -> bool:
        ...
        return self._weighted_count(item, previous_count, previous_expires_in, current_count) < item.amount - cost + 1
    ...
    def get_window_stats(self, item: RateLimitItem, *identifiers: str) -> WindowStats:
        ...
        remaining = max(
            0,
            item.amount - floor(self._weighted_count(item, previous_count, previous_expires_in, current_count)),
        )
...
KnownStrategy = type[SlidingWindowCounterRateLimiter] | type[FixedWindowRateLimiter] | type[MovingWindowRateLimiter]
