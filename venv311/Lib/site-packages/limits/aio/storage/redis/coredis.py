...
class CoredisBridge(RedisBridge):
    ...
    def use_sentinel(
        ...
        **options: str | float | bool,
        ...
        service_name = self.options_from_uri.path.replace("/", "") if self.options_from_uri.path else service_name
        ...
        self.connection_getter = lambda readonly: (self.storage_replica if readonly and use_replicas else self.storage)
    ...
    def use_cluster(self, **options: str | float | bool) -> None:
        ...
        cluster_hosts.extend({"host": host, "port": int(port)} for host, port in self.options_from_uri.locations)
    ...
    def register_scripts(self) -> None:
        ...
        self.lua_acquire_moving_window = self.get_connection().register_script(self.SCRIPT_ACQUIRE_MOVING_WINDOW)
        ...
        self.lua_acquire_sliding_window = self.get_connection().register_script(self.SCRIPT_ACQUIRE_SLIDING_WINDOW)
    ...
    async def get_sliding_window(self, previous_key: str, current_key: str, expiry: int) -> tuple[int, float, int, float]:
    ...
    async def acquire_entry(self, key: str, limit: int, expiry: int, amount: int = 1) -> bool:
        key = self.prefixed_key(key)
        timestamp = time.time()
        acquired = await self.lua_acquire_moving_window.execute([key], [timestamp, limit, expiry, amount])
    ...
    async def acquire_sliding_window_entry(
        ...
        previous_key = self.prefixed_key(previous_key)
        current_key = self.prefixed_key(current_key)
        acquired = await self.lua_acquire_sliding_window.execute([previous_key, current_key], [limit, expiry, amount])
        return bool(acquired)
