...
@versionchanged(
    version="4.2",
    reason=("Added support for using the asyncio redis client from :pypi:`redis`" " through :paramref:`implementation`"),
)
@versionchanged(
    version="4.3",
    reason=(
...
class RedisStorage(Storage, MovingWindowSupport, SlidingWindowCounterSupport):
    ...
    def __init__(
        ...
        **options: float | str | bool,
        ...
        if self.target_server == "valkey" or implementation == "valkey":
            self.bridge = ValkeyBridge(uri, self.dependencies["valkey"].module, key_prefix, **options)
        else:
            if implementation == "redispy":
                self.bridge = RedispyBridge(uri, self.dependencies["redis"].module, key_prefix, **options)
            else:
                self.bridge = CoredisBridge(uri, self.dependencies["coredis"].module, key_prefix, **options)
    ...
    async def acquire_sliding_window_entry(
        ...
        return await self.bridge.acquire_sliding_window_entry(previous_key, current_key, limit, expiry, amount)
...
class RedisSentinelStorage(RedisStorage):
    ...
    def configure_bridge(self) -> None:
        self.bridge.use_sentinel(self.service_name, self.use_replicas, self.sentinel_kwargs, **self.options)
