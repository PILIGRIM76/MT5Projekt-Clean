...
class MongoDBStorage(Storage, MovingWindowSupport, SlidingWindowCounterSupport):
    ...
    async def create_indices(self) -> None:
        if not self.__indices_created:
            await asyncio.gather(
                self.database[self.__collection_mapping["counters"]].create_index("expireAt", expireAfterSeconds=0),
                self.database[self.__collection_mapping["windows"]].create_index("expireAt", expireAfterSeconds=0),
            )
    ...
    async def get_expiry(self, key: str) -> float:
        ...
        return (counter["expireAt"] if counter else datetime.datetime.now()).replace(tzinfo=datetime.timezone.utc).timestamp()
    ...
    async def incr(self, key: str, expiry: int, amount: int = 1) -> int:
        ...
        expiration = datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(seconds=expiry)
    ...
    async def acquire_entry(self, key: str, limit: int, expiry: int, amount: int = 1) -> bool:
        ...
        try:
            updates: dict[
                ...
                "$set": {"expireAt": (datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(seconds=expiry))},
            }
    ...
    async def acquire_sliding_window_entry(self, key: str, limit: int, expiry: int, amount: int = 1) -> bool:
        await self.create_indices()
