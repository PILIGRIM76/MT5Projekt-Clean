...
class MemcachedStorage(Storage, SlidingWindowCounterSupport, TimestampedSlidingWindow):
    ...
    def get_client(self, module: ModuleType, hosts: list[tuple[str, int]], **kwargs: str) -> MemcachedClientP:
        """
        ...
        return cast(
            MemcachedClientP,
            (module.HashClient(hosts, **kwargs) if len(hosts) > 1 else module.PooledClient(*hosts, **kwargs)),
        )
    ...
    def storage(self) -> MemcachedClientP:
        ...
        if not (hasattr(self.local_storage, "storage") and self.local_storage.storage):
            dependency = get_dependency(self.cluster_library if len(self.hosts) > 1 else self.library)[0]
    ...
    def incr(
        ...
        """
        if (value := self.call_memcached_func(self.storage.incr, key, amount, noreply=False)) is not None:
            return value
        else:
            if not self.call_memcached_func(self.storage.add, key, amount, ceil(expiry), noreply=False):
    ...
    def acquire_sliding_window_entry(
        ...
        if floor(weighted_count) + amount > limit:
            ...
            current_count = self.incr(current_key, 2 * expiry, amount=amount, set_expiration_key=False)
