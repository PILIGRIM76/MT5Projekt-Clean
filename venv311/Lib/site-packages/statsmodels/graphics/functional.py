...
def _min_max_band(args):
    ...
    if have_de_optim and not use_brute:
        max_ = differential_evolution(
            _curve_constrained, bounds=bounds, args=(idx, -1, band, pca, ks_gaussian), maxiter=7, seed=seed
        ).x
...
def hdrboxplot(
    data, ncomp=2, alpha=None, threshold=0.95, bw=None, xdata=None, labels=None, ax=None, use_brute=False, seed=None
):
    """
...
def fboxplot(data, xdata=None, labels=None, depth=None, method="MBD", wfactor=1.5, ax=None, plot_opts=None):
    ...
    for ii, ix in enumerate(ix_outliers):
        ...
        ax.plot(
            xdata, data[ix, :], color=cmap(float(ii) / (len(ix_outliers) - 1)), label=label, lw=plot_opts.get("lw_outliers", 1)
        )
