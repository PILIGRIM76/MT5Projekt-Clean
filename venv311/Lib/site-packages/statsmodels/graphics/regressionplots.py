...
@Appender(
    _plot_influence_doc.format(**{"extra_params_doc": "results : Results\n" "        Results for a fitted regression model."})
)
def influence_plot(results, external=True, alpha=0.05, criterion="cooks", size=48, plot_alpha=0.75, ax=None, **kwargs):

    infl = results.get_influence()
    res = _influence_plot(
        results, infl, external=external, alpha=alpha, criterion=criterion, size=size, plot_alpha=plot_alpha, ax=ax, **kwargs
    )
    return res
...
def _plot_leverage_resid2(results, influence, alpha=0.05, ax=None, **kwargs):
    ...
    ax = utils.annotate_axes(
        index, labels, lzip(resid**2, leverage), [(0, 5)] * int(results.nobs), "large", ax=ax, ha="center", va="bottom"
    )
...
@Appender(
    _plot_leverage_resid2_doc.format({"extra_params_doc": "results : object\n" "    Results for a fitted regression model"})
)
...
def plot_added_variable(results, focus_exog, resid_type=None, use_glm_weights=True, fit_kwargs=None, ax=None):
    ...
    endog_resid, focus_exog_resid = added_variable_resids(
        results, focus_exog, resid_type=resid_type, use_glm_weights=use_glm_weights, fit_kwargs=fit_kwargs
    )
