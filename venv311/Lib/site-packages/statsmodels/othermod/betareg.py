...
class BetaModel(GenericLikelihoodModel):
    ...
    **kwds : extra keywords
    ...
    def __init__(
        self, endog, exog, exog_precision=None, link=families.links.Logit(), link_precision=families.links.Log(), **kwds
    ):
        ...
        if exog_precision is None:
            ...
            extra_names = [
                "precision-%s" % zc
                for zc in (
                    exog_precision.columns if hasattr(exog_precision, "columns") else range(1, exog_precision.shape[1] + 1)
                )
            ]
