    ...
    def _extend_index(
        ...
        if forecast_index is not None:
            ...
            if forecast_index.shape[0] != steps:
                raise ValueError(
                    "The number of values in forecast_index " f"({forecast_index.shape[0]}) must match steps ({steps})."
                )
...
class CalendarSeasonality(CalendarDeterministicTerm):
    ...
    def __init__(self, freq: str, period: str) -> None:
        ...
        if freq not in self._supported[period]:
            raise ValueError(f"The combination of freq={freq} and " f"period={period} is not supported.")
        super().__init__(freq)
        self._period = period
