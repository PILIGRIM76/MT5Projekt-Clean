...
class MarkovRegression(markov_switching.MarkovSwitching):
    ...
    def __init__(
        ...
        super().__init__(
            endog, k_regimes, order=order, exog_tvtp=exog_tvtp, exog=exog, dates=dates, freq=freq, missing=missing
        )
        ...
        if self.switching_variance:
            ...
            for i in range(self.k_regimes):
                if k_exog > 0:
                    ...
                    resid = endog
                variance[i] = np.sum(resid**2 * result.smoothed_marginal_probabilities[i]) / np.sum(
