...
from statsmodels.tsa.statespace.tools import _safe_cond, find_best_blas_type, prepare_exog
...
class MarkovSwitching(tsbase.TimeSeriesModel):
    ...
    def regime_transition_matrix(self, params, exog_tvtp=None):
        ...
        if not self.tvtp:
            regime_transition_matrix = np.zeros(
                (self.k_regimes, self.k_regimes, 1), dtype=np.promote_types(np.float64, params.dtype)
            )
    ...
    def _filter(self, params, regime_transition=None):
        ...
        # Apply the filter
        return (regime_transition, initial_probabilities, conditional_loglikelihoods) + cy_hamilton_filter_log(
        ...
        **kwargs
        ...
        if search_reps > 0:
            start_params = self._start_params_search(
                search_reps, start_params=start_params, transformed=transformed, em_iter=search_iter, scale=search_scale
            )
            transformed = True
        ...
        if em_iter and not self.tvtp:
            start_params = self._fit_em(
                start_params, transformed=transformed, maxiter=em_iter, tolerance=0, return_params=True
            )
            transformed = True
            ...
            with warnings.catch_warnings():
                ...
                try:
                    proposed_params = self._fit_em(
                        start_params + variates[i], transformed=False, maxiter=em_iter, return_params=True
                    )
...
class MarkovSwitchingResults(tsbase.TimeSeriesModelResults):
    ...
    def summary(self, alpha=0.05, start=None, title=None, model_name=None, display_params=True):
        ...
        def make_table(self, mask, title, strip_end=True):
            ...
            param_names = [re.sub(r"\[\d+\]$", "", name) for name in np.array(self.data.param_names)[mask].tolist()]
