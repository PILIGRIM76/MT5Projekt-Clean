...
def grangercausalitytests(x, maxlag, addconst=True, verbose=None):
    ...
    if x.shape[0] <= 3 * maxlag + int(addconst):
        raise ValueError(
            "Insufficient observations. Maximum allowable " "lag is {}".format(int((x.shape[0] - int(addconst)) / 3) - 1)
        )
    ...
    for mlg in lags:
        ...
        if (
            ...
            raise InfeasibleTestError(
                "The Granger causality test statistic cannot be computed " "because the VAR has a perfect fit of the data."
            )
