...
def _check_order(order: int | Sequence[int] | None, causal: bool) -> bool:
    ...
    if isinstance(order, (int, np.integer)):
        if int(order) < int(causal):
            raise ValueError(f"integer orders must be at least {int(causal)} when causal " f"is {causal}.")
        return True
    for v in order:
    ...
    def predict(
        ...
        def check_exog(arr, name, orig, exact):
            ...
            if arr.ndim != 2 or arr.shape[1] != orig.shape[1]:
                raise ValueError(f"{name} must have the same number of columns as the " f"original data, {orig.shape[1]}")
            if exact and arr.shape[0] != orig.shape[0]:
                raise ValueError(f"{name} must have the same number of rows as the " f"original data ({n}).")
            return arr
        ...
        if num_oos > max_1step:
            ...
            if self._fixed.shape[1] and fixed_oos is None:
                ...
                raise ValueError(f"fixed_oos must have at least {num_oos} observations " f"to produce {num_oos} forecasts.")
...
class UECM(ARDL):
    ...
    def from_ardl(cls, ardl: ARDL, missing: Literal["none", "drop", "raise"] = "none"):
        ...
        """
        err = (
            "UECM can only be created from ARDL models that include all " "{var_typ} lags up to the maximum lag in the model."
        )
        uecm_lags = {}
