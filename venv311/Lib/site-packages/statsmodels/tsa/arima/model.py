    ...
    def fit(
        ...
        for name in required_kwargs:
            if name in method_kwargs:
                raise ValueError('Cannot override model level value for "%s"' ' when method="%s".' % (name, method))
        ...
        if has_exog or method == "statespace":
            ...
            if has_exog and (gls or (gls is None and method != "statespace")):
                if self._has_fixed_params:
                    raise NotImplementedError("GLS estimation is not yet implemented for the case " "with fixed parameters.")
                ...
                res = super().fit(
                    return_params=return_params, low_memory=low_memory, cov_type=cov_type, cov_kwds=cov_kwds, **method_kwargs
                )
            ...
            if method == "yule_walker":
                p, fit_details = yule_walker(endog, ar_order=order[0], demean=False, **method_kwargs)
            elif method == "burg":
                p, fit_details = burg(endog, ar_order=order[0], demean=False, **method_kwargs)
            elif method == "hannan_rissanen":
                p, fit_details = hannan_rissanen(endog, ar_order=order[0], ma_order=order[2], demean=False, **method_kwargs)
            elif method == "innovations":
                p, fit_details = innovations(endog, ma_order=order[2], demean=False, **method_kwargs)
        ...
        if p is not None:
            ...
            if self.enforce_stationarity and self._spec_arima.max_reduced_ar_order > 0 and not p.is_stationary:
                raise ValueError(
            ...
            if self.enforce_invertibility and self._spec_arima.max_reduced_ma_order > 0 and not p.is_invertible:
                raise ValueError(
            ...
            if return_params:
                ...
                if self.ssm.memory_no_predicted or self.ssm.memory_no_gain or self.ssm.memory_no_smoothing:
                    ...
                    func = self.smooth
                res = func(p.params, transformed=True, includes_fixed=True, cov_type=cov_type, cov_kwds=cov_kwds)
