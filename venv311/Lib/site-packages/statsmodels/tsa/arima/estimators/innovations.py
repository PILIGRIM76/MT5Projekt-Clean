...
def innovations_mle(
    ...
    if spec.is_integrated:
        warnings.warn(
            "Provided `endog` series has been differenced to" " eliminate integration prior to ARMA parameter" " estimation."
        )
    ...
    if start_params is None:
        ...
        if spec.seasonal_periods == 0:
            ...
            _ = SARIMAXSpecification(
                endog, seasonal_order=seasonal_order, enforce_stationarity=True, enforce_invertibility=enforce_invertibility
            )
    ...
    def obj(params):
        ...
        return -arma_innovations.arma_loglike(
            endog, ar_params=-p.reduced_ar_poly.coef[1:], ma_params=p.reduced_ma_poly.coef[1:], sigma2=p.sigma2
        )
    ...
    # Construct other results
    other_results = Bunch(
        {"spec": spec, "minimize_results": minimize_results, "minimize_kwargs": minimize_kwargs, "start_params": start_params}
    )
