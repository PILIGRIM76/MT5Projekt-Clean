...
class SARIMAXSpecification:
    ...
    def k_params(self):
        ...
        k_params = (
            self.k_exog_params + self.k_ar_params + self.k_ma_params + self.k_seasonal_ar_params + self.k_seasonal_ma_params
        )
    ...
    def validate_estimator(self, estimator):
        ...
        if estimator not in ["innovations_mle", "statespace"]:
            if self.max_ar_order > 0 and self.enforce_stationarity:
                raise ValueError(
                    "%s estimator cannot enforce a stationary" " autoregressive lag polynomial." % titles[estimator]
                )
            if self.max_ma_order > 0 and self.enforce_invertibility:
                raise ValueError(
                    "%s estimator cannot enforce an invertible" " moving average lag polynomial." % titles[estimator]
                )
    ...
    def join_params(
        self, exog_params=None, ar_params=None, ma_params=None, seasonal_ar_params=None, seasonal_ma_params=None, sigma2=None
    ):
        """
