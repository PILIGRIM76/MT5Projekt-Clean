...
def load_results_jmulti(dataset, dt_s_list):
    ...
    for dt_s in dt_s_list:
        ...
        section_headers = ["Lagged endogenous term", "Deterministic term"]  # parameter matrices  # c, s, ct
        if dt_string == "nc":
        ...
        for line in params_file:
            if section == -1 and section_headers[section + 1] not in line:
                continue
            if section < len(section_headers) - 1 and section_headers[section + 1] in line:  # new section
                section += 1
                continue
