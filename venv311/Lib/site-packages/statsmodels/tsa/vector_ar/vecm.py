...
def select_order(
    ...
    selected_orders = {ic_name: np.array(ic_value).argmin() - 1 + 1 for ic_name, ic_value in ic.items()}
...
class VECM(tsbase.TimeSeriesModel):
    ...
    def _estimate_vecm_ml(self):
        ...
        gamma_tilde = (
            (delta_y_1_T - alpha_tilde.dot(beta_tilde.T).dot(y_lag1)).dot(delta_x.T).dot(inv(np.dot(delta_x, delta_x.T)))
        )
    ...
    def _lagged_param_names(self):
        ...
        if self.exog is not None:
            param_names += [
                "exog%d.%s" % (exog_no, n) for exog_no in range(1, self.exog.shape[1] + 1) for n in self.endog_names
            ]

        # 2. lagged endogenous terms
        param_names += [
            "L%d.%s.%s" % (i + 1, n1, n2) for n2 in self.endog_names for i in range(self.k_ar_diff) for n1 in self.endog_names
        ]
...
class VECMResults:
    ...
    def predict(self, steps=5, alpha=None, exog_fc=None, exog_coint_fc=None):
        ...
        if self.exog is not None and exog_fc is None:
            raise ValueError(
                "exog_fc is None: Please pass the future values " "of the VECM's exog terms via the exog_fc " "argument!"
            )
