...
from statsmodels.tools.sm_exceptions import IOWarning, X13Error, X13NotFoundError, X13Warning
...
def pandas_to_series_spec(x):
    ...
    if hasattr(x, "name"):
        ...
        name = "Unnamed Series"
    series_spec = SeriesSpec(data=data, name=name, period=period, title=name, start=f"{year}.{stperiod}")
    return series_spec
...
def x13_arima_analysis(
    ...
    if not retspec:
        res = X13ArimaAnalysisResult(
            observed=endog, results=results, seasadj=seasadj, trend=trend, irregular=irregular, stdout=stdout
        )
    else:
        res = X13ArimaAnalysisResult(
            observed=endog, results=results, seasadj=seasadj, trend=trend, irregular=irregular, stdout=stdout, spec=spec
        )
    return res
