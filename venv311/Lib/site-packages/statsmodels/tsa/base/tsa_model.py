...
def get_prediction_index(
    ...
    if index is not None:
        if not len(prediction_index) == len(index):
            raise ValueError(
                "Invalid `index` provided in prediction." " Must have length consistent with `start`" " and `end` arguments."
            )
            ...
            if isinstance(index, (DatetimeIndex, PeriodIndex)):
                ...
                if freq is None and index.freq is None:
                    ...
                    if freq is not None:
                        ...
                        if freq is not None:
                            warnings.warn(
                                "No frequency information was" " provided, so inferred frequency %s" " will be used." % freq,
                                ValueWarning,
                                stacklevel=2,
                            )
                ...
                if freq is None and index.freq is None:
                    ...
                    if dates is not None:
                        raise ValueError(
                            "No frequency information was" " provided with date index and no" " frequency could be inferred."
                        )
