...
class ExponentialSmoothing(TimeSeriesModel):
    ...
    def __init__(
        ...
        *,
        ...
        if (self.trend == "mul" or self.seasonal == "mul") and not np.all(self._data > 0.0):
            raise ValueError("endog must be strictly positive when using" "multiplicative trend or seasonal components.")
    ...
    def _check_bounds(self, bounds):
        ...
        if bounds is None:
            return
        msg = "bounds must be a dictionary of 2-element tuples of the form" " (lb, ub) where lb < ub, lb>=0 and ub<=1"
        variables = self._ordered_names()
        for key in bounds:
            if key not in variables:
                supported = ", ".join(variables[:-1])
                supported += ", and " + variables[-1]
                raise KeyError(f"{key} does not match the list of supported variables " f"names: {supported}.")
            bound = bounds[key]
    ...
    def _initialize(self):
        ...
        msg = f"initialization method is {self._initialization_method} but " "initial_{0} has been set."
    ...
    def _update_for_fixed(self, sel, alpha, beta, gamma, phi, l0, b0, s0):
        if self._fixed_parameters:
            ...
            if (~sel[~not_fixed]).any():
                ...
                raise ValueError("Cannot fix a parameter that is not being " f"estimated: {invalid_names}")
    ...
    def fit(
        ...
        *,
        ...
        if initial_level is not None or initial_trend is not None:
            raise ValueError("Initial values were set during model construction. These " "cannot be changed during fit.")
        ...
        if use_basinhopping is not None:
            raise ValueError("use_basinhopping is deprecated. Set optimization method " "using 'method'.")
        ...
        if optimized:
            ...
            l0, b0, s0 = self.initial_values(initial_level=initial_level, initial_trend=initial_trend)
            res.level = l0
            res.trend = b0
            res.seasonal = s0
            if self._fixed_parameters:
    ...
    def _predict(
        ...
        b[0] = initial_trend
        s[:m] = initial_seasons
        phi_h = np.cumsum(np.repeat(phi, h + 1) ** np.arange(1, h + 1 + 1)) if damped else np.arange(1, h + 1 + 1)
        ...
        if seasonal == "mul":
            for i in range(1, nobs + 1):
                lvls[i] = y_alpha[i - 1] / s[i - 1] + (alphac * trended(lvls[i - 1], dampen(b[i - 1], phi)))
                if has_trend:
                    b[i] = (beta * detrend(lvls[i], lvls[i - 1])) + (betac * dampen(b[i - 1], phi))
                s[i + m - 1] = y_gamma[i - 1] / trended(lvls[i - 1], dampen(b[i - 1], phi)) + (gammac * s[i - 1])
            ...
            for i in range(1, nobs + 1):
                lvls[i] = y_alpha[i - 1] - (alpha * s[i - 1]) + (alphac * trended(lvls[i - 1], dampen(b[i - 1], phi)))
                if has_trend:
                    b[i] = (beta * detrend(lvls[i], lvls[i - 1])) + (betac * dampen(b[i - 1], phi))
                s[i + m - 1] = y_gamma[i - 1] - (gamma * trended(lvls[i - 1], dampen(b[i - 1], phi))) + (gammac * s[i - 1])
            _trend = b[1 : nobs + 1].copy()
            season = s[m : nobs + m].copy()
            lvls[nobs:] = lvls[nobs]
            if has_trend:
