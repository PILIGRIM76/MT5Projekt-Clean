...
class ExponentialSmoothing(MLEModel):
    ...
    **Seasonality**
    ...
    def __init__(
        ...
        super().__init__(endog, k_states=k_states, k_posdef=k_posdef, initialization=init, dates=dates, freq=freq)
    ...
    def state_names(self):
        ...
        if self.trend:
            state_names += ["trend"]
        if self.seasonal:
            state_names += ["seasonal"] + ["seasonal.L%d" % i for i in range(1, self.seasonal_periods)]
    ...
    def param_names(self):
        ...
        if self.initialization_method == "estimated":
            ...
            if self.seasonal:
                param_names += ["initial_seasonal"] + ["initial_seasonal.L%d" % i for i in range(1, self.seasonal_periods - 1)]
    ...
    @property
    def k_params(self):
        k_params = 1 + int(self.trend) + int(self.seasonal) + int(not self.concentrate_scale) + int(self.damped_trend)
        if self.initialization_method == "estimated":
    ...
    def _initialize_constant_statespace(self, initial_level, initial_trend=None, initial_seasonal=None):
    ...
    def update(self, params, transformed=True, includes_fixed=False, complex_step=False):
        ...
        if self.damped_trend and self.initialization_method in methods:
            self._initialize_constant_statespace(self._initial_level, self._initial_trend, self._initial_seasonal)
    ...
    def loglike(self, params, *args, **kwargs):
        if self.initialization_method == "concentrated":
            self._initialize_constant_statespace(*self._compute_concentrated_states(params, *args, **kwargs))
