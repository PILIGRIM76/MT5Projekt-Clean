    ...
    **kwargs
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.prefix_statespace_map = (
            statespace_classes if statespace_classes is not None else tools.prefix_statespace_map.copy()
        )
    ...
    def extend(self, endog, start=None, end=None, **kwargs):
        ...
        **kwargs
        ...
        if start > self.nobs:
            raise ValueError(
                "The `start` argument of the extension within the" " base model cannot be after the end of the" " base model."
            )
        if end > self.nobs:
            raise ValueError(
                "The `end` argument of the extension within the" " base model cannot be after the end of the" " base model."
            )
        if start > end:
            raise ValueError(
                "The `start` argument of the extension within the" " base model cannot be after the `end` argument."
            )
        ...
        error_tv = (
            "Model has time-varying %s matrix, so an updated" " time-varying matrix for the extension period" " is required."
        )
        for name, shape in self.shapes.items():
            if name == "obs":
                continue
    ...
    def diff_endog(self, new_endog, tolerance=1e-10):
        ...
        if len(new_endog) < len(endog):
            raise ValueError(
                "Given data (length %d) is too short to diff" " against model data (length %d)." % (len(new_endog), len(endog))
            )
        if len(new_endog) > len(endog):
    ...
    def initialize(
        ...
        if initialization == "known":
            ...
            initialization = Initialization(
                self.k_states, "approximate_diffuse", approximate_diffuse_variance=approximate_diffuse_variance
            )
    ...
    def initialize_known(self, constant, stationary_cov):
        ...
        if not stationary_cov.shape == (self.k_states, self.k_states):
            raise ValueError(
                ...
                " matrix. Requires shape (%d,%d), got %s" % (self.k_states, self.k_states, str(stationary_cov.shape))
            )
