    ...
    **kwargs
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.prefix_kalman_filter_map = (
            kalman_filter_classes if kalman_filter_classes is not None else tools.prefix_kalman_filter_map.copy()
        )
    ...
    def impulse_responses(self, steps=10, impulse=0, orthogonalized=False, cumulative=False, direct=False):
        ...
        irf, _ = sim_model.simulate(
            steps, measurement_shocks=measurement_shocks, state_shocks=state_shocks, initial_state=initial_state
        )
...
class FilterResults(FrozenRepresentation):
    ...
    def update_filter(self, kalman_filter):
        ...
        if not (self.memory_no_std_forecast or self.invert_lu or self.solve_lu or self.filter_collapsed):
            if has_missing:
                self._standardized_forecasts_error = np.array(
                    reorder_missing_vector(kalman_filter.standardized_forecast_error, self.missing, prefix=self.prefix)
                )
            else:
                self._standardized_forecasts_error = np.array(kalman_filter.standardized_forecast_error, copy=True)
        else:
            self._standardized_forecasts_error = None
        ...
        if has_missing and (not self.memory_no_gain and not self.filter_collapsed):
            self._kalman_gain = np.array(
                reorder_missing_matrix(kalman_filter.kalman_gain, self.missing, reorder_cols=True, prefix=self.prefix)
            )
            self.tmp1 = np.array(
                reorder_missing_matrix(kalman_filter.tmp1, self.missing, reorder_cols=True, prefix=self.prefix)
            )
            self.tmp2 = np.array(reorder_missing_vector(kalman_filter.tmp2, self.missing, prefix=self.prefix))
            self.tmp3 = np.array(
                reorder_missing_matrix(kalman_filter.tmp3, self.missing, reorder_rows=True, prefix=self.prefix)
            )
            self.tmp4 = np.array(
                reorder_missing_matrix(
                    kalman_filter.tmp4, self.missing, reorder_cols=True, reorder_rows=True, prefix=self.prefix
                )
            )
        else:
        ...
        if self.nobs_diffuse > 0:
            self.initial_diffuse_state_cov = np.array(kalman_filter.model.initial_diffuse_state_cov, copy=True)
            self.predicted_diffuse_state_cov = np.array(kalman_filter.predicted_diffuse_state_cov, copy=True)
            if has_missing and not self.filter_collapsed:
                ...
                self.forecasts_error_diffuse_cov = np.array(kalman_filter.forecast_error_diffuse_cov, copy=True)
        ...
        if self.filter_collapsed:
            ...
            self.collapsed_forecasts_error_cov = self.forecasts_error_cov[: self.k_states, : self.k_states, :]
                ...
                if self.nmissing[t] > 0:
                    ...
                    if can_compute_cov:
                        self.forecasts_error_cov[:, :, t] = (
                            np.dot(
                                np.dot(self.design[:, :, design_t], self.predicted_state_cov[:, :, t]),
        ...
        if self.filter_concentrated and self.model._scale is None:
            ...
            if not self.memory_no_likelihood:
                self.llf_obs += -0.5 * ((self.k_endog - nmissing - nsingular) * np.log(self.scale) + scale_obs / self.scale)
            else:
                self.llf_obs[0] += -0.5 * np.squeeze(
                    np.sum((self.k_endog - nmissing - nsingular) * np.log(self.scale)) + scale_obs / self.scale
                )
            ...
            if not (self.memory_no_std_forecast or self.invert_lu or self.solve_lu or self.filter_collapsed):
                self._standardized_forecasts_error = self._standardized_forecasts_error / self.scale**0.5
            ...
            for t in range(self.nobs):
                ...
                if self.nmissing[t] == 0:
                    self._kalman_gain[:, :, t] = np.dot(
                        np.dot(self.transition[:, :, transition_t], self.predicted_state_cov[:, :, t]),
                        np.dot(np.transpose(self.design[:, :, design_t]), np.linalg.inv(self.forecasts_error_cov[:, :, t])),
                    )
                else:
                    mask = ~self.missing[:, t].astype(bool)
                    ...
                    self._kalman_gain[:, mask, t] = np.dot(
                        np.dot(self.transition[:, :, transition_t], self.predicted_state_cov[:, :, t]),
                        np.dot(np.transpose(self.design[mask, :, design_t]), np.linalg.inv(F[:, :, 0])),
                    )
    ...
    def predict(self, start=None, end=None, dynamic=None, **kwargs):
        ...
        **kwargs
        ...
        # re-apply the Kalman filter
            ...
            kwargs["conserve_memory"] = self.conserve_memory & ~MEMORY_NO_FORECAST & ~MEMORY_NO_PREDICTED
            ...
            kwargs.update({"initialization": "known", "constant": constant, "stationary_cov": stationary_cov})
        ...
        return PredictionResults(results, start, end, nstatic, ndynamic, nforecast, oos_results=oos_results)
