    ...
    **kwargs
    ...
    def _initialize_error_transition_var(self):
        ...
        self._idx_error_transition = np.s_[
            "transition", _factor_order : _factor_order + k_endog, _factor_order : _factor_order + _error_order
        ]
    ...
    def start_params(self):
        ...
        if self.k_factors > 1 and self.factor_order > 0:
            ...
            coefficient_matrices = (
                (params[self._params_factor_transition].reshape(self.k_factors * self.factor_order, self.k_factors).T)
        ...
        if self.error_order == 0:
            ...
            # Test for stationarity
            coefficient_matrices = (
                (np.array(res_errors.params.T).ravel().reshape(self.k_endog * self.error_order, self.k_endog).T)
    ...
    def param_names(self):
        ...
        param_names += [f"beta.{self.exog_names[j]}.{endog_names[i]}" for i in range(self.k_endog) for j in range(self.k_exog)]
    ...
    def __init__(self, model, params, filter_results, cov_type=None, **kwargs):
        ...
        if self.model.factor_order > 0:
            ...
            self.coefficient_matrices_var = (
                (ar_params.reshape(k_factors * factor_order, k_factors).T).reshape(k_factors, k_factors, factor_order).T
            )
        ...
        if self.model.error_order > 0:
            ...
            if self.model.error_var:
                self.coefficient_matrices_error = (
                    (ar_params.reshape(k_endog * error_order, k_endog).T).reshape(k_endog, k_endog, error_order).T
                )
            else:
