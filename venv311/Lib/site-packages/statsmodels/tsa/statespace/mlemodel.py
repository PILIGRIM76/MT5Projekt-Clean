...
class MLEModel(tsbase.TimeSeriesModel):
    ...
    **kwargs
    ...
    def clone(self, endog, exog=None, **kwargs):
        ...
        """
        raise NotImplementedError(
            "This method is not implemented in the base" " class and must be set up by each specific" " model."
        )
        ...
        **kwargs,
        ...
        if approx_complex_step:
            ...
            for i in range(n):
                ...
                partials_forecasts_error_cov[:, :, :, i] = (_res1.forecasts_error_cov - _res2.forecasts_error_cov) / (
                    2 * epsilon[i]
                )
    ...
    def observed_information_matrix(
        self, params, transformed=True, includes_fixed=False, approx_complex_step=None, approx_centered=False, **kwargs
    ):
        """
        ...
        **kwargs
        ...
        partials_forecasts_error, partials_forecasts_error_cov = self._forecasts_error_partial_derivatives(
            params,
    ...
    def opg_information_matrix(self, params, transformed=True, includes_fixed=False, approx_complex_step=None, **kwargs):
        ...
        **kwargs
        ...
        score_obs = self.score_obs(
            params, transformed=transformed, includes_fixed=includes_fixed, approx_complex_step=approx_complex_step, **kwargs
        ).transpose()
            ...
            for i in range(n):
                partials[t, i] += np.trace(
                    np.dot(
                        ...
                        (
                            np.eye(k_endog)
                            - np.dot(inv_forecasts_error_cov, np.outer(res.forecasts_error[:, t], res.forecasts_error[:, t]))
                        ),
                    )
                )
                # 2 * dv / di * F^{-1} v_t
                ...
                partials[t, i] += 2 * np.dot(
                    partials_forecasts_error[:, t, i], np.dot(inv_forecasts_error_cov, res.forecasts_error[:, t])
                )
    ...
    def score(self, params, *args, **kwargs):
        ...
        **kwargs
        ...
        """
        transformed, includes_fixed, method, approx_complex_step, approx_centered, kwargs = _handle_args(
        ...
        out = self.handle_params(
            params, transformed=transformed, includes_fixed=includes_fixed, return_jacobian=not transformed
        )
        if transformed:
            params = out
        else:
    ...
    def hessian(self, params, *args, **kwargs):
        ...
        **kwargs
        ...
        if not transformed and approx_complex_step:
            raise ValueError(
                "Cannot use complex-step approximations to" " calculate the hessian with untransformed" " parameters."
            )
    ...
    def _hessian_finite_difference(self, params, approx_centered=False, **kwargs):
        ...
        hessian = approx_fprime(
            params, self._score_finite_difference, epsilon=epsilon, kwargs=kwargs, centered=approx_centered
        )
        ...
        if out_of_sample and k_exog > 0:
            ...
            try:
                ...
                raise ValueError(
                    ...
                    " appropriate shape. Required %s, got %s." % (str(required_exog_shape), str(exog.shape))
                )
        elif k_exog > 0 and exog is not None:
            exog = None
            warnings.warn(
                "Exogenous array provided, but additional data" " is not required. `exog` argument ignored.", ValueWarning
            )

        return exog

    def _get_extension_time_varying_matrices(
        self, params, exog, out_of_sample, extend_kwargs=None, transformed=True, includes_fixed=False, **kwargs
    ):
        """
        ...
        **kwargs,
        ...
        if out_of_sample and extend_model:
            kwargs = self._get_extension_time_varying_matrices(
                params, exog, out_of_sample, extend_kwargs, transformed=transformed, includes_fixed=includes_fixed, **kwargs
            )
...
class MLEResults(tsbase.TimeSeriesModelResults):
    ...
    def __init__(self, model, params, results, cov_type=None, cov_kwds=None, **kwargs):
        ...
        if self._has_fixed_params:
            ...
            self.fixed_params = []
        self.param_names = [
            "%s (fixed)" % name if name in self.fixed_params else name for name in (self.data.param_names or [])
        ]
    ...
    def _cov_params_robust_approx(self, approx_complex_step=True, approx_centered=False):
        ...
        evaluated_hessian = self.nobs_effective * self.model.hessian(
            self.params, transformed=True, includes_fixed=True, method="approx", approx_complex_step=approx_complex_step
        )
    ...
    def info_criteria(self, criteria, method="standard"):
        ...
        if method == "standard":
            ...
            if self.filter_results.state_cov.shape[-1] > 1:
                raise ValueError(
                    "Cannot compute Lütkepohl statistics for" " models with time-varying state covariance" " matrix."
                )
            ...
            if criteria == "aic":
                ...
                out = np.squeeze(
                    np.linalg.slogdet(cov)[1] + 2 * self.df_model * np.log(np.log(self.nobs_effective)) / self.nobs_effective
                )
            else:
    ...
    def states(self):
        if self.model._index_generated and not self.model._index_none:
            warnings.warn(
                "No supported index is available. The `states`" " DataFrame uses a generated integer index", ValueWarning
            )
        return self._states
        ...
        **kwargs
        ...
        if out_of_sample and extend_model:
            kwargs = self.model._get_extension_time_varying_matrices(
                self.params, exog, out_of_sample, extend_kwargs, transformed=True, includes_fixed=True, **kwargs
            )
        ...
        return PredictionResultsWrapper(
            PredictionResults(
                self, prediction_results, information_set=information_set, signal_only=signal_only, row_labels=prediction_index
            )
        )
    ...
    def _get_previous_updated(self, comparison, exog=None, comparison_type=None, **kwargs):
        ...
        if comparison_dataset:
            ...
            if nobs_exog > nobs_endog:
                ...
                if comparison.ndim != 2 or comparison.shape[1] != self.model.k_endog:
                    raise ValueError("Invalid shape for `comparison`. Must" f" contain {self.model.k_endog} columns.")
                extra = np.zeros((nobs_exog - nobs_endog, self.model.k_endog)) * np.nan
                comparison = pd.DataFrame(
                    np.concatenate([comparison, extra], axis=0), index=ix, columns=self.model.endog_names
                )
        ...
        if comparison_type == "updated" or (
            ...
            and (comparison.nobs > self.nobs or (comparison.nobs == self.nobs and nmissing > nmissing_comparison))
        ):
            updated = comparison
            previous = self
    ...
    def _news_updated_results(self, updated, start, end, periods, revisions_details_start=False, state_index=None):
        return updated._news_previous_results(
            self, start, end, periods, revisions_details_start=revisions_details_start, state_index=state_index
        )
    ...
    def _news_previous_data(self, endog, start, end, periods, exog, revisions_details_start=False, state_index=None):
        ...
        return self._news_previous_results(
            previous, start, end, periods, revisions_details_start=revisions_details_start, state_index=state_index
        )
    ...
    def _news_updated_data(self, endog, start, end, periods, exog, revisions_details_start=False, state_index=None):
        ...
        return self._news_updated_results(
            updated, start, end, periods, revisions_details_start=revisions_details_start, state_index=state_index
        )
    ...
    def news(
        ...
        **kwargs,
        ...
        news_results = updated._news_previous_results(
            previous, start, end + 1, periods, revisions_details_start=revisions_details_start, state_index=state_index
        )

        if not return_raw:
            news_results = NewsResults(
                news_results, self, updated, previous, impacted_variable, tolerance, row_labels=prediction_index
            )
        return news_results
    ...
    def get_smoothed_decomposition(self, decomposition_of="smoothed_state", state_index=None):
        ...
        """
        data_contributions, obs_intercept_contributions, state_intercept_contributions, prior_contributions = (
            self.smoother_results.get_smoothed_decomposition(decomposition_of=decomposition_of, state_index=state_index)
        )
        ...
        obs_intercept_contributions = pd.DataFrame(
            obs_intercept_contributions.reshape(shape[0] * shape[1], shape[2] * shape[3], order="F"),
            index=contributions_to,
        ...
        state_intercept_contributions = pd.DataFrame(
            state_intercept_contributions.reshape(shape[0] * shape[1], shape[2] * shape[3], order="F"),
            index=contributions_to,
    ...
    def extend(self, endog, exog=None, fit_kwargs=None, **kwargs):
        ...
        **kwargs
        ...
        mod.ssm.initialization = Initialization(
            mod.k_states, "known", constant=self.predicted_state[..., -1], stationary_cov=self.predicted_state_cov[..., -1]
        )
    ...
    def summary(
        ...
        if display_diagnostics:
            ...
            if self.model.k_endog <= display_max_endog:
                ...
                data = pd.DataFrame(np.c_[lb[:, :2, -1], het[:, :2], jb[:, :4]], index=endog_names, columns=columns)
                try:
