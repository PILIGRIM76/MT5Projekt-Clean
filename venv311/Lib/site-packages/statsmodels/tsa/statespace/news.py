...
class NewsResults:
    ...
    def __init__(self, news_results, model, updated, previous, impacted_variable=None, tolerance=1e-10, row_labels=None):
        ...
        self.update_impacts = pd.DataFrame(news_results.update_impacts, index=self.row_labels, columns=columns).rename_axis(
        ...
        if len(iloc) > 0:
            self.revisions_ix = pd.DataFrame(
                {"revision date": index[iloc["revision date"]], "revised variable": columns[iloc["revised variable"]]}
            )
        else:
        ...
        if len(iloc) > 0:
            self.updates_ix = pd.DataFrame(
                {"update date": index[iloc["update date"]], "updated variable": columns[iloc["updated variable"]]}
            )
        else:
        ...
        r_ix = pd.MultiIndex.from_arrays(
            [self.revisions_ix_detailed["revision date"], self.revisions_ix_detailed["revised variable"]]
        )
    ...
    def data_revisions(self):
        ...
        # Save revisions data
        data = pd.concat(
            [self.revised_all.rename("revised"), self.revised_prev_all.rename("observed (prev)")], axis=1
        ).sort_index()
    ...
    def data_updates(self):
        ...
        """
        data = pd.concat(
            [self.update_realized.rename("observed"), self.update_forecasts.rename("forecast (prev)")], axis=1
        ).sort_index()
        return data
    ...
    def revision_details_by_update(self):
        ...
        details = (
            ...
            .reorder_levels(
                ["revision date", "revised variable", "revised", "observed (prev)", "impact date", "impacted variable"]
            )
            .sort_index()
        )
                ...
                if k_level_values[i] == 1:
                    ...
                    can_drop = (
                        ...
                        or (
                            ...
                            and (impacted_variable is not None or self.impacted_variable is not None)
                        )
                    )
    ...
    def summary_revisions(self, sparsify=True):
        ...
        try:
            data[["revision date", "revised variable"]] = data[["revision date", "revised variable"]].map(str)
            data.iloc[:, 2:-1] = data.iloc[:, 2:-1].map(lambda num: "" if pd.isnull(num) else "%.2f" % num)
        except AttributeError:
    ...
    def summary_news(self, sparsify=True):
        ...
        try:
            data[["update date", "updated variable"]] = data[["update date", "updated variable"]].map(str)
            cols = data.columns[2:]
    ...
    def get_impacts(self, groupby=None, include_revisions=True, include_updates=True):
        ...
        if groupby is not None:
            impacts = (
                impacts.unstack("update date").groupby(groupby).sum(min_count=1).stack("update date").swaplevel().sort_index()
            )
