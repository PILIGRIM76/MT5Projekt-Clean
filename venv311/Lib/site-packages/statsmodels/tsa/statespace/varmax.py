    ...
    **kwargs
    ...
    def start_params(self):
        ...
        if self.k_ar > 0 and self.enforce_stationarity:
            coefficient_matrices = (
                (ar_params.reshape(self.k_endog * self.k_ar, self.k_endog).T).reshape(self.k_endog, self.k_endog, self.k_ar).T
            )
    ...
    def param_names(self):
        ...
        if self.error_cov_type == "diagonal":
            ...
            param_names += [
                ("sqrt.var.%s" % endog_names[i] if i == j else f"sqrt.cov.{endog_names[j]}.{endog_names[i]}")
            ...
            if exog is not None:
                ...
                try:
                    ...
                    raise ValueError(
                        ...
                        " appropriate shape. Required %s, got %s." % (str((self.k_exog,)), str(exog.shape))
                    )
...
class VARMAXResults(MLEResults):
    ...
    def get_prediction(
        self, start=None, end=None, dynamic=False, information_set="predicted", index=None, exog=None, **kwargs
    ):
        if start is None:
            start = 0
