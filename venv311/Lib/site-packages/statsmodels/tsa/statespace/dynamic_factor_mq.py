...
from statsmodels.tsa.statespace.kalman_smoother import SMOOTHER_STATE, SMOOTHER_STATE_AUTOCOV, SMOOTHER_STATE_COV
...
class DynamicFactorMQStates(dict):
    ...
    def __init__(self, k_endog_M, k_endog_Q, endog_names, factors, factor_orders, factor_multiplicities, idiosyncratic_ar1):
        ...
        block_loading_counts = {
            block: np.atleast_1d(self.loading_counts.loc[list(block), "count"]).mean(axis=0) for block in factor_orders.keys()
        }
    ...
    *Impulse responses*
    ...
    def __init__(
        ...
        **kwargs,
        ...
        s = self._s = DynamicFactorMQStates(
            self.k_endog_M, self.k_endog_Q, endog_names, factors, factor_orders, factor_multiplicities, idiosyncratic_ar1
        )
        ...
        if isinstance(standardize, tuple) and len(standardize) == 2:
            ...
            if np.shape(endog_mean) != (n,) or np.shape(endog_std) != (n,):
                raise ValueError("Invalid value passed for `standardize`: each" f" element must be shaped ({n},).")
            standardize = True
        ...
        self.params = OrderedDict(
            [
                ...
                ("factor_ar", np.sum([block.k_factors**2 * block.factor_order for block in s.factor_blocks])),
                ("factor_cov", np.sum([block.k_factors * (block.k_factors + 1) // 2 for block in s.factor_blocks])),
        ...
        for block in self._s.factor_blocks:
            ...
            if block.k_factors == 1:
                ...
                coefficient_matrices = np.transpose(
                    np.reshape(block_factor_ar, (block.k_factors, block.k_factors, block.factor_order)), (2, 0, 1)
                )
    ...
    def transform_params(self, unconstrained):
        ...
        for block in self._s.factor_blocks:
            ...
            tmp_coeff = np.reshape(
                unconstrained_factor_ar[i : i + length], (block.k_factors, block.k_factors * block.factor_order)
            )
    ...
    def untransform_params(self, constrained):
        ...
        for block in self._s.factor_blocks:
            length = block.k_factors**2 * block.factor_order
            tmp_coeff = np.reshape(
                constrained_factor_ar[i : i + length], (block.k_factors, block.k_factors * block.factor_order)
            )
            tmp_cov = np.eye(block.k_factors)
        ...
        if self.idiosyncratic_ar1:
            ...
            unconstrained[self._p["idiosyncratic_ar1"]] = [
                unconstrain_stationary_univariate(idio_ar1[i : i + 1])[0] for i in range(self.k_endog)
            ]
        ...
        while i < maxiter and not terminate and (i < 1 or (delta > tolerance)):
                    ...
                    for j in range(s["idio_ar_M"].start, s["idio_ar_M"].stop):
                        ...
                        if init_type == "stationary":
                            if not np.abs(T[j, j]) < (1 - 1e-10):
                                ...
                                switch_init.append("idiosyncratic AR(1) for monthly" f" variable: {name}")
            ...
            if llf_decrease_action == "revert" and llf_decrease:
                ...
                # Print output every `disp` observations
                    ...
                    print(f"EM iteration {i + 1}, llf={llf[-1]:.5g}," f" convergence criterion={delta:.5g}")
...
class DynamicFactorMQResults(mlemodel.MLEResults):
    ...
    def news(
        ...
        **kwargs,
        ...
        if not return_raw and self.model.standardize and original_scale:
            ...
            news_results.revision_weights = news_results.revision_weights.divide(endog_std, axis=0, level=1).multiply(
    ...
    def get_smoothed_decomposition(self, decomposition_of="smoothed_state", state_index=None, original_scale=True):
        ...
        data_contributions, obs_intercept_contributions, state_intercept_contributions, prior_contributions = (
            super().get_smoothed_decomposition(decomposition_of=decomposition_of, state_index=state_index)
        )
    ...
    def summary(
        ...
        if not display_params_as_list:
            ...
            data = pd.DataFrame(
                self.filter_results.design[:, mod._s["factors_L1"], 0], index=endog_names, columns=mod.factor_names
            )
            try:
