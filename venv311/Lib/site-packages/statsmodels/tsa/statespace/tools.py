...
def concat(series, axis=0, allow_mix=False):
    ...
    if np.all(is_pandas):
        ...
        raise ValueError("Attempted to concatenate Pandas objects with" " non-Pandas objects with `allow_mix=False`.")
...
def _compute_multivariate_pacf_from_autocovariances(autocovariances, order=None, k_endog=None):
    ...
    for s in range(order):  # s = 0, ..., p-1
        ...
        partial_autocorrelations.append(
            linalg.solve_triangular(forward_factors[s], np.dot(forwards[s], backward_factors[s]), lower=True)
        )
...
def validate_matrix_shape(name, shape, nrows, ncols, nobs):
    ...
    if ndim not in [2, 3]:
        raise ValueError(
            "Invalid value for %s matrix. Requires a" " 2- or 3-dimensional array, got %d dimensions" % (name, ndim)
        )
    ...
    if ndim == 3 and nobs is not None and shape[-1] not in [1, nobs]:
        raise ValueError(
            "Invalid dimensions for time-varying %s" " matrix. Requires shape (*,*,%d), got %s" % (name, nobs, str(shape))
        )
...
def validate_vector_shape(name, shape, nrows, nobs):
    ...
    if ndim not in [1, 2]:
        raise ValueError(
            "Invalid value for %s vector. Requires a" " 1- or 2-dimensional array, got %d dimensions" % (name, ndim)
        )
    ...
    if ndim == 2 and shape[1] not in [1, nobs]:
        raise ValueError(
            "Invalid dimensions for time-varying %s" " vector. Requires shape (*,%d), got %s" % (name, nobs, str(shape))
        )


def reorder_missing_matrix(
    matrix, missing, reorder_rows=False, reorder_cols=False, is_diagonal=False, inplace=False, prefix=None
):
    """
