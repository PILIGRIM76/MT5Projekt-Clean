...
from statsmodels.tsa.statespace.tools import copy_index_matrix, reorder_missing_matrix, reorder_missing_vector
    ...
    **kwargs
    ...
    def __init__(self, k_endog, k_states, k_posdef=None, results_class=None, kalman_smoother_classes=None, **kwargs):
        ...
        self.prefix_kalman_smoother_map = (
            kalman_smoother_classes if kalman_smoother_classes is not None else tools.prefix_kalman_smoother_map.copy()
        )
    ...
    def _initialize_smoother(self, smoother_output=None, smooth_method=None, prefix=None, **kwargs):
        ...
        if create_smoother:
            ...
            self._kalman_smoothers[prefix] = cls(
                self._statespaces[prefix], self._kalman_filters[prefix], smoother_output, smooth_method
            )
    ...
    def _smooth(self, smoother_output=None, smooth_method=None, prefix=None, complex_step=False, results=None, **kwargs):
        ...
        if create_filter or create_statespace:
            raise ValueError(
                "Passed settings forced re-creation of the" " Kalman filter. Please run `_filter` before" " running `_smooth`."
            )
...
class SmootherResults(FilterResults):
                ...
                if name in ["smoothing_error", "smoothed_measurement_disturbance"]:
                    ...
                    if matrix is not None and has_missing:
                        matrix = reorder_missing_matrix(
                            matrix, self.missing, reorder_rows=True, reorder_cols=True, prefix=self.prefix
                        )
    ...
    def smoothed_state_autocovariance(self, lag=1, t=None, start=None, end=None, extend_kwargs=None):
        ...
        if lag == 0 and self.smoothed_state_cov is None:
            raise RuntimeError(
                "Cannot return smoothed state covariances" " if those values have not been computed by" " Kalman smoothing."
            )
    ...
    def news(self, previous, t=None, start=None, end=None, revisions_details_start=True, design=None, state_index=None):
        ...
        if not (
            self.k_endog == previous.k_endog and self.k_states == previous.k_states and self.k_posdef == previous.k_posdef
        ):
        ...
        if end > self.nobs and not self.model.time_invariant:
            raise RuntimeError(
                "Cannot compute the impacts of news on periods" " outside of the sample in time-varying" " models."
            )
        ...
        if len(revisions_ix) > 0:
            ...
            if len(revisions_details) > 0:
                ...
                smoothed_state_weights, _, _ = tools._compute_smoothed_state_weights(
                    rev_mod, compute_t=compute_t, compute_j=compute_j, compute_prior_weights=False, scale=previous.scale
                )
                ...
                # instead of m)
                revision_weights = np.nansum(smoothed_state_weights[..., None] * ZT[:, None, :, None, :], axis=2).transpose(
                    0, 1, 3, 2
                )
        ...
        if self._smoothed_forecasts is None:
            ...
            for t in range(self.nobs):
                ...
                self._smoothed_forecasts[:, t] = (
                    np.dot(self.design[:, :, design_t], self.smoothed_state[:, t]) + self.obs_intercept[:, obs_intercept_t]
                )
                ...
                self._smoothed_forecasts_error_cov[:, :, t] = (
                    np.dot(
                        np.dot(self.design[:, :, design_t], self.smoothed_state_cov[:, :, t]), self.design[:, :, design_t].T
                    )
