    ...
    **Mixed**
    ...
    def __init__(
        ...
        self.prefix_initialization_map = (
            initialization_classes if initialization_classes is not None else tools.prefix_initialization_map.copy()
        )
    ...
    def set(self, index, initialization_type, constant=None, stationary_cov=None, approximate_diffuse_variance=None):
        ...
        if k_states == self.k_states:
            ...
            if approximate_diffuse_variance is not None and not initialization_type == "approximate_diffuse":
                raise ValueError(
                    "`approximate_diffuse_variance` can only be" " provided when using approximate diffuse" " initialization."
                )
            ...
            if initialization_type == "known":
                ...
                if not stationary_cov.shape == (k_states, k_states):
                    raise ValueError(
                        ...
                        " given shape %s but require shape %s." % (str(stationary_cov.shape), str((k_states, k_states)))
                    )
