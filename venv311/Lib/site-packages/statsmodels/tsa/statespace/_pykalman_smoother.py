...
class _KalmanSmoother:
    ...
    def __next__(self):
        ...
        if missing_partial_obs:
            design = np.array(_kfilter.selected_design[: k_endog * model.k_states], copy=True).reshape(
        ...
        if smoother_output & (SMOOTHER_STATE | SMOOTHER_DISTURBANCE):
            if missing_entire_obs:
                ...
                smoothing_error[:k_endog, t] = F_inv.dot(forecasts_error) - kalman_gain.transpose().dot(
        ...
        if smoother_output & (SMOOTHER_STATE_COV | SMOOTHER_DISTURBANCE_COV):
            if missing_entire_obs:
                ...
                scaled_smoothed_estimator_cov[:, :, t - 1] = design.transpose().dot(F_inv).dot(design) + L.transpose().dot(
        ...
        if smoother_output & SMOOTHER_DISTURBANCE_COV:
            smoothed_state_disturbance_cov[:, :, t] = state_cov - QR.dot(scaled_smoothed_estimator_cov[:, :, t]).dot(
                QR.transpose()
            )
            ...
            if missing_entire_obs:
                ...
                smoothed_measurement_disturbance_cov[ix] = (
                    obs_cov
                    - obs_cov.dot(
                        F_inv + kalman_gain.transpose().dot(scaled_smoothed_estimator_cov[:, :, t]).dot(kalman_gain)
                    ).dot(obs_cov)
                )[:, :, np.newaxis]
