...
class SARIMAX(MLEModel):
    ...
    **kwargs
    ...
    def initial_design(self):
        ...
        if self.state_regression:
            if self._k_order > 0:
                design = np.c_[np.reshape(np.repeat(design, self.nobs), (design.shape[0], self.nobs)).T, self.exog].T[
                    None, :, :
                ]
            else:
    ...
    def _conditional_sum_squares(
        endog, k_ar, polynomial_ar, k_ma, polynomial_ma, k_trend=0, trend_data=None, warning_description=None
    ):
        k = 2 * k_ma
        r = max(k + k_ma, k_ar)
    ...
    def start_params(self):
        ...
        # Seasonal Parameters
        _, params_seasonal_ar, params_seasonal_ma, params_seasonal_variance = self._conditional_sum_squares(
            endog,
            self.k_seasonal_ar,
        ...
        invalid_seasonal_ar = (
            self.k_seasonal_ar > 0 and self.enforce_stationarity and not is_invertible(np.r_[1, -params_seasonal_ar])
        )
        ...
        # invertibility is on, raise an error
        invalid_seasonal_ma = (
            self.k_seasonal_ma > 0 and self.enforce_invertibility and not is_invertible(np.r_[1, params_seasonal_ma])
        )
    ...
    def endog_names(self, latex=False):
        ...
        if self.k_seasonal_diff > 0:
            if self.k_seasonal_diff == 1:
                ...
                seasonal_diff = (r"\Delta_%d^%d" if latex else "D%dS%d") % (self.k_seasonal_diff, self.seasonal_periods)
    ...
    def model_orders(self):
        ...
        return {
            ...
            "exog_variance": self._k_exog if (self.state_regression and self.time_varying_regression) else 0,
    ...
    def _validate_can_fix_params(self, param_names):
        ...
        items = [
            ...
            ("seasonal_ar", "seasonal autoregressive", self.enforce_stationarity, "`enforce_stationarity=True`"),
            ("ma", "moving average", self.enforce_invertibility, "`enforce_invertibility=True`"),
            ("seasonal_ma", "seasonal moving average", self.enforce_invertibility, "`enforce_invertibility=True`"),
        ]
