...
from .tools import companion_matrix, constrain_stationary_univariate, prepare_exog, unconstrain_stationary_univariate
...
class UnobservedComponents(MLEModel):
    ...
    **Regression effects**
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if stochastic_freq_seasonal is None:
            ...
            if len(stochastic_freq_seasonal) != len(freq_seasonal):
                raise ValueError(
                    ...
                    " of freq_seasonal: {!r} vs {!r}".format(len(stochastic_freq_seasonal), len(freq_seasonal))
                )
        ...
        k_posdef = (
            ...
            + (
                (sum(2 * h if self.stochastic_freq_seasonal[ix] else 0 for ix, h in enumerate(self.freq_seasonal_harmonics)))
                * self.freq_seasonal
            )
    ...
    def _get_init_kwds(self):
        ...
        kwds["freq_seasonal"] = [
            {"period": p, "harmonics": self.freq_seasonal_harmonics[ix]} for ix, p in enumerate(self.freq_seasonal_periods)
        ]
...
class UnobservedComponentsResults(MLEResults):
            ...
            for ix, h in enumerate(spec.freq_seasonal_harmonics):
                ...
                if self.smoothed_state_cov is not None:
                    item.smoothed_cov = np.sum(
                        [self.smoothed_state_cov[offset + j, offset + j] for j in states_in_sum], axis=0
                    )
                out.append(item)
    ...
    def cycle(self):
        ...
        if spec.cycle:
            offset = int(
                spec.trend + spec.level + self._k_states_by_type["seasonal"] + self._k_states_by_type["freq_seasonal"]
            )
            out = Bunch(
                filtered=self.filtered_state[offset],
                filtered_cov=self.filtered_state_cov[offset, offset],
