...
class Table2x2(SquareTable):
    ...
    def summary(self, alpha=0.05, float_format="%.3f", method="normal"):
        ...
        data = [
            ...
            [fmt(x) for x in [self.log_oddsratio, self.log_oddsratio_se, lcb2, ucb2, self.oddsratio_pvalue()]],
            [fmt(x) for x in [self.riskratio, "", lcb3, ucb3, self.riskratio_pvalue()]],
            [fmt(x) for x in [self.log_riskratio, self.log_riskratio_se, lcb4, ucb4, self.riskratio_pvalue()]],
        ]
        tab = iolib.SimpleTable(data, headers, stubs, data_aligns="r", table_dec_above="")
        return tab
