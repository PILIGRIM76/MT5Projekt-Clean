...
def proportion_confint(count, nobs, alpha: float = 0.05, method="normal"):
    ...
    def _check(x: np.ndarray, name: str) -> np.ndarray:
        ...
        if np.any(y != x):
            raise ValueError(f"{name} must have an integral dtype. Found data with " f"dtype {x.dtype}")
        return y
...
def multinomial_proportions_confint(counts, alpha=0.05, method="goodman"):
    ...
    if method == "goodman":
        ...
        def truncated_poisson_factorial_moment(interval, r, p):
            ...
            b, a = interval
            return p**r * (
                1 - ((poisson_interval((a - r + 1, a), p) - poisson_interval((b - r, b - 1), p)) / poisson_interval((b, a), p))
            )
        ...
        def edgeworth(intervals):
            ...
            mu_r1, mu_r2, mu_r3, mu_r4 = (
                np.array([truncated_poisson_factorial_moment(interval, r, p) for (interval, p) in zip(intervals, counts)])
                for r in range(1, 5)
            )
            mu = mu_r1
            mu2 = mu_r2 + mu - mu**2
        ...
        def approximated_multinomial_interval(intervals):
            ...
            """
            return np.exp(
                np.sum(np.log([poisson_interval(interval, p) for (interval, p) in zip(intervals, counts)]))
...
def power_ztost_prop(
    low, upp, nobs, p_alt, alpha=0.05, dist="norm", variance_prop=None, discrete=True, continuity=0, critval_continuity=0
):
    """
...
def score_test_proportions_2indep(
    count1, nobs1, count2, nobs2, value=None, compare="diff", alternative="two-sided", correction=True, return_results=True
):
    """
...
def _score_confint_inversion(count1, nobs1, count2, nobs2, compare="diff", alpha=0.05, correction=True):
    ...
    rt0 = test_proportions_2indep(
        count1, nobs1, count2, nobs2, value=0, compare=compare, method="score", correction=correction, alternative="two-sided"
    )
