...
def anova_generic(means, variances, nobs, use_var="unequal", welch_correction=True, info=None):
    ...
    if use_var == "unequal":
        ...
        df_num = tmp**2 / (
            (variances**2).sum() + (nobs / nobs_t * variances).sum() ** 2 - 2 * (nobs / nobs_t * variances**2).sum()
        )
...
def equivalence_oneway(
    data, equiv_margin, groups=None, use_var="unequal", welch_correction=True, trim_frac=0, margin_type="f2"
):
    ...
    res = equivalence_oneway_generic(
        f_stat, res0.n_groups, res0.nobs_t, equiv_margin, res0.df, alpha=0.05, margin_type=margin_type
    )
...
def simulate_power_equivalence_oneway(
    means, nobs, equiv_margin, vars_=None, k_mc=1000, trim_frac=0, options_var=None, margin_type="f2"
    ...
    for _ in range(k_mc):
        ...
        for uv in options_var:
            ...
            res1 = equivalence_oneway_generic(
                f_stat, n_groups, nobs.sum(), equiv_margin, res0.df, alpha=0.05, margin_type=margin_type
            )
...
def equivalence_scale_oneway(
    data, equiv_margin, method="bf", center="median", transform="abs", trim_frac_mean=0.0, trim_frac_anova=0.0
):
