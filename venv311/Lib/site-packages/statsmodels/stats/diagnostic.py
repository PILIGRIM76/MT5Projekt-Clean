...
def linear_reset(res, power=3, test_type="fitted", use_f=False, cov_type="nonrobust", cov_kwargs=None):
    ...
    * Powers of the first principal component of :math:`X`. If the
    ...
    if bool(res.model.k_constant) and res.model.exog.shape[1] == 1:
        raise ValueError(
            "exog contains only a constant column. The RESET " "test requires exog to have at least 1 " "non-constant column."
        )
...
def spec_white(resid, exog):
    ...
    if x.shape[1] < 2 or not np.any(np.ptp(x, 0) == 0.0):
        raise ValueError("White's specification test requires at least two" "columns where one is a constant.")
...
def recursive_olsresiduals(res, skip=None, lamda=0.0, alpha=0.95, order_by=None):
    ...
    y = res.model.endog
    x = res.model.exog
    order_by = array_like(order_by, "order_by", dtype="int", optional=True, ndim=1, shape=(y.shape[0],))
...
"""
    ...
    # crit = a * np.sqrt(nrr)
    rcusumci = (a * np.sqrt(nrr) + 2 * a * np.arange(0, nobs - skip) / np.sqrt(nrr)) * np.array([[-1.0], [+1.0]])
