...
def _invert_test_confint_2indep(
    count1, exposure1, count2, exposure2, alpha=0.05, method="score", compare="diff", method_start="wald"
):
    """invert hypothesis test to get confidence interval for 2indep"""

    def func(r):
        v = (
            test_poisson_2indep(count1, exposure1, count2, exposure2, value=r, method=method, compare=compare)[1] - alpha
        ) ** 2
        return v
...
method_names_poisson_2indep = {
    "test": {
        "ratio": ["wald", "score", "score-log", "wald-log", "exact-cond", "cond-midp", "sqrt", "etest-score", "etest-wald"],
    ...
    if compare == "ratio":
        ...
        if method in ["score"]:
            ...
            stat, pvalue = etest_poisson_2indep(
                count1, exposure1, count2, exposure2, value=value, method=method_etest, alternative=alternative, **etest_kwds
            )
...
def tost_poisson_2indep(count1, exposure1, count2, exposure2, low, upp, method="score", compare="ratio"):
    ...
    tt1 = test_poisson_2indep(
        count1, exposure1, count2, exposure2, value=low, method=method, compare=compare, alternative="larger"
    )
    tt2 = test_poisson_2indep(
        count1, exposure1, count2, exposure2, value=upp, method=method, compare=compare, alternative="smaller"
    )
...
def nonequivalence_poisson_2indep(count1, exposure1, count2, exposure2, low, upp, method="score", compare="ratio"):
    ...
    """
    tt1 = test_poisson_2indep(
        count1, exposure1, count2, exposure2, value=low, method=method, compare=compare, alternative="smaller"
    )
    tt2 = test_poisson_2indep(
        count1, exposure1, count2, exposure2, value=upp, method=method, compare=compare, alternative="larger"
    )
...
def power_equivalence_poisson_2indep(
    rate1, rate2, nobs1, low, upp, nobs_ratio=1, exposure=1, alpha=0.05, dispersion=1, method_var="alt", return_results=False
):
...
def _power_equivalence_het_v0(es_low, es_upp, nobs, alpha=0.05, std_null_low=None, std_null_upp=None, std_alternative=None):
    ...
    crit = norm.isf(alpha)
    pow_ = (
        norm.cdf((np.sqrt(nobs) * es_low - crit * s0_low) / s1) + norm.cdf((np.sqrt(nobs) * es_upp - crit * s0_upp) / s1) - 1
    )
    return pow_
...
def power_poisson_diff_2indep(
    rate1, rate2, nobs1, nobs_ratio=1, alpha=0.05, value=0, method_var="score", alternative="two-sided", return_results=True
):
...
def power_equivalence_neginb_2indep(
    rate1, rate2, nobs1, low, upp, nobs_ratio=1, exposure=1, alpha=0.05, dispersion=0, method_var="alt", return_results=False
):
    """
