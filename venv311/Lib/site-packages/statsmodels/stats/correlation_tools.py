...
def _spg_optim(
    ...
    for itr in range(int(maxiter)):
        ...
        alpha, params1, fval, gval1 = _nmono_linesearch(
            func, grad, params, d, obj_hist, M=M, sig1=sig1, sig2=sig2, gam=gam, maxiter=maxiter_nmls
        )
...
def corr_nearest_factor(corr, rank, ctol=1e-6, lam_min=1e-30, lam_max=1e30, maxiter=1000):
    ...
    rslt = _spg_optim(
        func, grad, X, _project_correlation_factors, ctol=ctol, lam_min=lam_min, lam_max=lam_max, maxiter=maxiter
    )
    root = rslt.params
    diag = 1 - (root**2).sum(1)
...
def kernel_covariance(exog, loc, groups, kernel=None, bw=None):
    ...
    def cov(x, y):
        ...
        if cw < 1e-10:
            msg = (
                "Effective sample size is 0.  The bandwidth may be too " + "small, or you are outside the range of your data."
            )
            warnings.warn(msg)
