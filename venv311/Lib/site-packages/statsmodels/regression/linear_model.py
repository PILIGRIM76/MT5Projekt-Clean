...
class RegressionModel(base.LikelihoodModel):
    ...
    def fit(
        ...
        **kwargs
        ...
        """
        if method == "pinv":
            if not (hasattr(self, "pinv_wexog") and hasattr(self, "normalized_cov_params") and hasattr(self, "rank")):
        ...
        if isinstance(self, OLS):
            lfit = OLSResults(
                self, beta, normalized_cov_params=self.normalized_cov_params, cov_type=cov_type, cov_kwds=cov_kwds, use_t=use_t
            )
        else:
...
class GLS(RegressionModel):
    ...
    def fit_regularized(
        self, method="elastic_net", alpha=0.0, L1_wt=1.0, start_params=None, profile_scale=False, refit=False, **kwargs
    ):
...
class OLS(WLS):
    ...
    def __init__(self, endog, exog=None, missing="none", hasconst=None, **kwargs):
        if "weights" in kwargs:
            msg = "Weights are not supported in OLS and will be ignored" "An exception will be raised in the next version."
...
class RegressionResults(base.LikelihoodModelResults):
    ...
    **kwargs
    ...
    def __init__(
        self, model, params, normalized_cov_params=None, scale=1.0, cov_type="nonrobust", cov_kwds=None, use_t=None, **kwargs
    ):
    ...
    def compare_f_test(self, restricted):
        ...
        if has_robust1 or has_robust2:
            warnings.warn(
                "F test for comparison is likely invalid with " + "robust covariance, proceeding anyway", InvalidTestWarning
            )
    ...
    def compare_lr_test(self, restricted, large_sample=False):
        ...
        if has_robust1 or has_robust2:
            warnings.warn(
                "Likelihood Ratio test is likely invalid with " + "robust covariance, proceeding anyway", InvalidTestWarning
            )
    ...
    def get_prediction(self, exog=None, transform=True, weights=None, row_labels=None, **kwargs):
        ...
        return pred.get_prediction(self, exog=exog, transform=transform, weights=weights, row_labels=row_labels, **kwargs)
    ...
    def summary(
        ...
        self.diagn = dict(
            jb=jb, jbpv=jbpv, skew=skew, kurtosis=kurtosis, omni=omni, omnipv=omnipv, condno=condno, mineigval=eigvals[-1]
        )
        ...
        if not self.k_constant:
            etext.append("R² is computed without centering (uncentered) since the " "model does not contain a constant.")
        if hasattr(self, "cov_type"):
...
class OLSResults(RegressionResults):
    ...
    **kwargs
    ...
    def el_test(self, b0_vals, param_nums, return_weights=0, ret_params=0, method="nm", stochastic_exog=1):
        ...
        if method == "nm":
            llr = optimize.fmin(
                opt_fun_inst._opt_nuis_regress, x0, maxfun=10000, maxiter=10000, full_output=1, disp=0, args=args
            )[1]
        if method == "powell":
