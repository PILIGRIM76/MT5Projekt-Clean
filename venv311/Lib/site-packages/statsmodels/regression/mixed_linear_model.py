...
def _multi_dot_three(A, B, C):
    ...
    """
    # cost1 = cost((AB)C)
    cost1 = A.shape[0] * A.shape[1] * B.shape[1] + A.shape[0] * B.shape[1] * C.shape[1]  # (AB)  # (--)C
    # cost2 = cost((AB)C)
    cost2 = B.shape[0] * B.shape[1] * C.shape[1] + A.shape[0] * A.shape[1] * C.shape[1]  # (BC)  # A(--)
...
class MixedLM(base.LikelihoodModel):
    ...
    @classmethod
    def from_formula(
        cls, formula, data, re_formula=None, vc_formula=None, subset=None, use_sparse=False, missing="none", *args, **kwargs
    ):
        """
    ...
    def fit_regularized(self, start_params=None, method="l1", alpha=0, ceps=1e-4, ptol=1e-6, maxit=200, **fit_kwargs):
        """
