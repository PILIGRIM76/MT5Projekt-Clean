...
class Optimizer:
    def _fit(
        ...
        methods = ["newton", "nm", "bfgs", "lbfgs", "powell", "cg", "ncg", "basinhopping", "minimize"]
...
def _fit_minimize(
    f, score, start_params, fargs, kwargs, disp=True, maxiter=100, callback=None, retall=False, full_output=True, hess=None
):
    """
...
def _fit_lbfgs(
    ...
    """
    check_kwargs(
        kwargs, ("m", "pgtol", "factr", "maxfun", "epsilon", "approx_grad", "bounds", "loglike_and_score", "iprint"), "lbfgs"
    )
    ...
    if epsilon and not approx_grad:
        raise ValueError("a finite-differences epsilon was provided " "even though we are not using approx_grad")
    if approx_grad and loglike_and_score:
        raise ValueError(
            "gradient approximation was requested " "even though an analytic loglike_and_score function " "was given"
        )
    ...
    retvals = optimize.fmin_l_bfgs_b(
        func, start_params, maxiter=maxiter, callback=callback, args=fargs, bounds=bounds, **extended_kwargs
    )
