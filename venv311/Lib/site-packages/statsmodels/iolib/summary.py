...
def summary(self, yname=None, xname=None, title=0, alpha=0.05, returns="text", model_info=None):
    ...
    gen_title = title
    gen_header = None
    gen_table_left = SimpleTable(gen_data_left, gen_header, gen_stubs_left, title=gen_title, txt_fmt=gen_fmt)

    gen_stubs_right = ("Method:", "Time:", "Number of Obs:", "df resid")
    gen_data_right = ([modeltype], time_of_day, [nobs], [df_resid])  # was dist family need to look at more
    gen_table_right = SimpleTable(gen_data_right, gen_header, gen_stubs_right, title=gen_title, txt_fmt=gen_fmt)
    ...
    tstats = {"OLS": self.t(), "GLS": self.t(), "GLSAR": self.t(), "WLS": self.t(), "RLM": self.t(), "GLM": self.t()}
    prob_stats = {
        "OLS": self.pvalues,
        "GLS": self.pvalues,
        "GLSAR": self.pvalues,
    ...
    parameter_table = SimpleTable(params_data, param_header[modeltype], params_stubs, title=None, txt_fmt=fmt_2)
...
def summary_params(results, yname=None, xname=None, alpha=0.05, use_t=True, skip_header=False, title=None):
    ...
    # summary table. look up by modeltype
    if use_t:
        param_header = ["coef", "std err", "t", "P>|t|", "[" + str(alpha / 2), str(1 - alpha / 2) + "]"]
    else:
    ...
    parameter_table = SimpleTable(params_data, param_header, params_stubs, title=title, txt_fmt=fmt_params)
...
def summary_params_2d(result, extras=None, endog_names=None, exog_names=None, title=None):
    ...
    if extras:
        extras_list = [
            [["%10s" % ("(" + forg(v, prec=3).strip() + ")") for v in col] for col in getattr(result, what)] for what in extras
        ]
...
def summary_params_2dflat(
    result, endog_names=None, exog_names=None, alpha=0.05, use_t=True, keep_headers=True, endog_cols=False
):
