...
class Poisson(CountModel):
    ...
    def fit_constrained(self, constraints, start_params=None, **fit_kwds):
        ...
        **fit_kwds : keyword arguments
        ...
        res = self.fit(maxiter=0, method="nm", disp=0, warn_convergence=False)  # we get a wrapper back
...
class Logit(BinaryModel):
    ...
    exog : ndarray
        A reference to the exogenous design.
    """.format(params=base._model_params_doc, extra_params=base._missing_param_doc + _check_rank_doc)
...
class NegativeBinomial(CountModel):
    ...
    def __init__(
        self, endog, exog, loglike_method="nb2", offset=None, exposure=None, missing="none", check_rank=True, **kwargs
    ):
        super().__init__(endog, exog, offset=offset, exposure=exposure, missing=missing, check_rank=check_rank, **kwargs)
    ...
    def _score_nbin(self, params, Q=0):
        ...
        if Q == 1:  # nb1
            ...
            dalpha = (
                (alpha * (y - mu * np.log(prob) - mu * (dgpart + 1)) - mu * (np.log(prob) + dgpart)) / (alpha**2 * (alpha + 1))
            ).sum()
        ...
        for i in range(dim):
            for j in range(dim):
                ...
                hess_arr[i, j] = np.squeeze(
                    np.sum(
                        dparams[:, i, None] * dmudb[:, j, None] + xmu_alpha[:, i, None] * xmu_alpha[:, j, None] * trigamma,
                        axis=0,
                    )
                )
        tri_idx = np.triu_indices(dim, k=1)
    ...
    def _hessian_nb2(self, params):
        ...
        dada = (
            da2 * dalpha / da1
            + da1**2
            * (special.polygamma(1, a1 + y) - special.polygamma(1, a1) + 1 / a1 - 1 / (a1 + mu) + (y - mu) / (mu + a1) ** 2)
        ).sum()
        hess_arr[-1, -1] = dada
...
class NegativeBinomialP(CountModel):
    ...
    def hessian(self, params):
        ...
        hess_arr[-1, :-1] = (
            ...
            * mu
            * a1
            * ((1 + a4) * (1 - a3 / a2) / a2 - p * (lprob + dgpart + 2) / mu + p / mu * (a3 + p * a1) / a2 - a4 * pgpart)
            / alpha
        ).sum(axis=1)
    ...
    def summary2(self, alpha=0.05, float_format="%.4f"):
        ...
        for i in range(eqn):
            coefs = summary2.summary_params(
                (self, self.params[:, i], self.bse[:, i], self.tvalues[:, i], self.pvalues[:, i], confint[i]), alpha=alpha
            )
