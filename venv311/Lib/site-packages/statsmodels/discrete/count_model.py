...
class GenericZeroInflated(CountModel):
    ...
    def __init__(self, endog, exog, exog_infl=None, offset=None, inflation="logit", exposure=None, missing="none", **kwargs):
        ...
        if len(self.exog) != len(self.exog_infl):
            raise ValueError("exog and exog_infl have different number of" "observation. `missing` handling is not supported")
            ...
            for j in range(i, -1, -1):
                hess_arr[i, j] = (
                    ...
                    * (
                        ...
                        * (1 - w[zero_idx])
                        * (
                            (1 - np.exp(llf_main[zero_idx])) * (1 - 2 * w[zero_idx]) * np.exp(llf[zero_idx])
                            - (w[zero_idx] - w[zero_idx] ** 2) * (1 - np.exp(llf_main[zero_idx])) ** 2
                        )
                        / pmf[zero_idx] ** 2
                    )
                ).sum() - (
                    self.exog_infl[nonzero_idx, i] * self.exog_infl[nonzero_idx, j] * w[nonzero_idx] * (1 - w[nonzero_idx])
                ).sum()
        ...
        for i in range(self.k_inflate):
            for j in range(self.k_exog):
                hess_arr[i, j + self.k_inflate] = -(
                    score_main[zero_idx, j] * w[zero_idx] * (1 - w[zero_idx]) * self.exog_infl[zero_idx, i] / pmf[zero_idx]
                ).sum()
...
class ZeroInflatedPoisson(GenericZeroInflated):
    ...
    def __init__(self, endog, exog, exog_infl=None, offset=None, exposure=None, inflation="logit", missing="none", **kwargs):
        super().__init__(
            endog, exog, offset=offset, inflation=inflation, exog_infl=exog_infl, exposure=exposure, missing=missing, **kwargs
        )
    ...
    def _hessian_main(self, params):
        ...
        for i in range(self.k_exog):
            for j in range(i, -1, -1):
                ...
                ).sum() - (mu[nonzero_idx] * self.exog[nonzero_idx, i] * self.exog[nonzero_idx, j]).sum()
...
class ZeroInflatedGeneralizedPoisson(GenericZeroInflated):
    ...
    def __init__(
        self, endog, exog, exog_infl=None, offset=None, exposure=None, inflation="logit", p=2, missing="none", **kwargs
    ):
        super().__init__(
            endog, exog, offset=offset, inflation=inflation, exog_infl=exog_infl, exposure=exposure, missing=missing, **kwargs
        )
...
class ZeroInflatedResults(CountResults):
    ...
    def get_prediction(
        ...
        res = pred.get_prediction_delta(
            self, exog=exog, which=which, average=average, agg_weights=agg_weights, pred_kwds=pred_kwds
        )
        return res
