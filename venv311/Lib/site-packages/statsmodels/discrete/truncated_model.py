        ...
        if which == "mean":
            ...
            if self.truncation == 0:
                ...
                probs = self.model_main.predict(
                    params, exog=exog, exposure=np.exp(exposure), offset=offset, which="prob", y_values=counts
                )
...
class TruncatedLFNegativeBinomialP(TruncatedLFGeneric):
    ...
    def __init__(self, endog, exog, offset=None, exposure=None, truncation=0, p=2, missing="none", **kwargs):
        ...
        self.model_main = NegativeBinomialP(
            self.endog, self.exog, exposure=getattr(self, "exposure", None), offset=getattr(self, "offset", None), p=p
        )
...
class _RCensored(_RCensoredGeneric):
    ...
    def __init__(
        self, endog, exog, model=Poisson, distribution=truncatedpoisson, offset=None, exposure=None, missing="none", **kwargs
    ):
        super().__init__(endog, exog, offset=offset, exposure=exposure, missing=missing, **kwargs)
