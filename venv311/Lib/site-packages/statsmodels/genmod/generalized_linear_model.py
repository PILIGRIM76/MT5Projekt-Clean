...
class GLM(base.LikelihoodModel):
    ...
    def _derivative_exog(
        self, params, exog=None, transform="dydx", dummy_idx=None, count_idx=None, offset=None, exposure=None
    ):
        """
    ...
    def _derivative_predict(self, params, exog=None, transform="dydx", offset=None, exposure=None):
        ...
        if exog is None:
            exog = self.exog
        if (offset is not None) or (exposure is not None) or (getattr(self, "offset", None) is not None):
            raise NotImplementedError("offset and exposure not supported")
    ...
    def _update_history(self, tmp_result, mu, history):
        ...
        history["deviance"].append(self.family.deviance(self.endog, mu, self.var_weights, self.freq_weights, self.scale))
        return history
    ...
    def estimate_scale(self, mu):
        ...
        if not self.scaletype:
            if isinstance(self.family, (families.Binomial, families.Poisson, families.NegativeBinomial)):
                return 1.0
            else:
        ...
        if isinstance(self.scaletype, str):
            if self.scaletype.lower() == "x2":
                ...
                return self.family.deviance(self.endog, mu, self.var_weights, self.freq_weights, 1.0) / (self.df_resid)
            else:
                raise ValueError("Scale %s with type %s not understood" % (self.scaletype, type(self.scaletype)))
        else:
            raise ValueError("Scale %s with type %s not understood" % (self.scaletype, type(self.scaletype)))
    ...
    def estimate_tweedie_power(self, mu, method="brentq", low=1.01, high=5.0):
        ...
        if method == "brentq":
            ...
            def psi_p(power, mu):
                ...
                return (
                    np.sum(self.iweights * ((self.endog - mu) ** 2 / (scale * (mu**power)) - 1) * np.log(mu))
    ...
    def _fit_gradient(
        ...
        **kwargs,
        ...
        if (max_start_irls > 0) and (start_params is None):
            ...
            del irls_rslt
        rslt = super().fit(
            start_params=start_params, maxiter=maxiter, full_output=full_output, method=method, disp=disp, **kwargs
        )
    ...
    def _fit_irls(
        self, start_params=None, maxiter=100, tol=1e-8, scale=None, cov_type="nonrobust", cov_kwds=None, use_t=None, **kwargs
    ):
        """
    ...
    def fit_regularized(self, method="elastic_net", alpha=0.0, start_params=None, refit=False, opt_method="bfgs", **kwargs):
        ...
        **kwargs
        ...
        result = fit_elasticnet(self, method=method, alpha=alpha, start_params=start_params, refit=refit, **defaults)
...
class GLMResults(base.LikelihoodModelResults):
    ...
    def llnull(self):
        ...
        return self.family.loglike(
            self._endog, self.null, var_weights=self._var_weights, freq_weights=self._freq_weights, scale=self.scale
        )
    ...
    def llf_scaled(self, scale=None):
        ...
        val = _modelfamily.loglike(
            self._endog, self.mu, var_weights=self._var_weights, freq_weights=self._freq_weights, scale=scale
        )
        return val
    ...
    def get_influence(self, observed=True):
        ...
        infl = GLMInfluence(
            self, endog=wendog, exog=wexog, resid=self.resid_pearson / np.sqrt(self.scale), hat_matrix_diag=hat_matrix_diag
        )
        return infl
    ...
    def plot_added_variable(self, focus_exog, resid_type=None, use_glm_weights=True, fit_kwargs=None, ax=None):
        ...
        fig = plot_added_variable(
            self, focus_exog, resid_type=resid_type, use_glm_weights=use_glm_weights, fit_kwargs=fit_kwargs, ax=ax
        )
