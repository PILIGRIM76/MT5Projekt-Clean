...
class GEE(GLM):
    ...
    def fit_regularized(
        self, pen_wt, scad_param=3.7, maxiter=100, ddof_scale=None, update_assoc=5, ctol=1e-5, ztol=1e-3, eps=1e-6, scale=None
    ):
        """
...
class OrdinalGEE(GEE):
    ...
    def fit(self, maxiter=60, ctol=1e-6, start_params=None, params_niter=1, first_dep_update=0, cov_type="robust"):
        ...
        ord_rslt = OrdinalGEEResults(
            self, rslt.params, rslt.cov_params() / rslt.scale, rslt.scale, cov_type=cov_type, attr_kwds=res_kwds
        )
    ...
    def summary(self, alpha=0.05):
        ...
        if J > 1:
            for eq in range(J):
                ...
                tble = summary_params(
                    restup, yname=yname_list[eq], xname=exog_names, alpha=alpha, use_t=False, skip_header=True
                )
    ...
    def get_margeff(self, at="overall", method="dydx", atexog=None, dummy=False, count=False):
        ...
        if at == "all":
            ...
            margeff_cov, margeff_se = margeff_cov_with_se(
                model, params, exog, results.cov_params(), at, model._derivative_exog, dummy_idx, count_idx, method, 1
            )
