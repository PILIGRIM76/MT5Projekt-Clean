...
class NegativeBinomial(Family):
    ...
    def __init__(self, link=None, alpha=1.0, check_link=True):
        ...
        if alpha is self.__init__.__defaults__[1]:  # `is` is intentional
            warnings.warn(
                "Negative binomial dispersion parameter alpha not " f"set. Using default value alpha={alpha}.", ValueWarning
            )
        if link is None:
            link = L.Log()
        super().__init__(link=link, variance=V.NegativeBinomial(alpha=self.alpha), check_link=check_link)
    ...
    def resid_anscombe(self, endog, mu, var_weights=1.0, scale=1.0):
        ...
        resid = (
            3
            / 2.0
            * (endog ** (2 / 3.0) * hyp2f1(-self.alpha * endog) - mu ** (2 / 3.0) * hyp2f1(-self.alpha * mu))
