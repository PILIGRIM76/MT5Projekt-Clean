    ...
    **Experimental**
    ...
    def endog_names(self, value):
        ...
        if value is not None:
            if len(value) != self.corr.shape[0]:
                raise ValueError("The length of `endog_names` must " "equal the number of variables.")
    ...
    def _fit_pa(self, maxiter=50, tol=1e-8):
        ...
        if self.n_factor > self.n_comp:
            raise ValueError(
                "n_factor must be smaller or equal to the rank" " of endog! %d > %d" % (self.n_factor, self.n_comp)
            )
        if maxiter <= 0:
...
class FactorResults:
    ...
    def summary(self):
        ...
        if self.rotation_method is not None:
            loadings = pd.DataFrame(
                self.loadings, columns=["factor %d" % (i) for i in range(self.loadings.shape[1])], index=self.endog_names
            )
    ...
    def get_loadings_frame(
        self, style="display", sort_=True, threshold=0.3, highlight_max=True, color_max="yellow", decimals=None
    ):
    ...
    def plot_loadings(self, loading_pairs=None, plot_prerotated=False):
        ...
        return plot_loadings(
            loadings, loading_pairs=loading_pairs, title=title, row_names=self.endog_names, percent_variance=var_explained
        )
