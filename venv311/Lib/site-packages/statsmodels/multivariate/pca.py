...
estimates are based on only {eff_series} (effective) series."""
    ...
    def _compute_rsquare_and_ic(self):
        ...
        penalties = np.array(
            [sum_to_prod * np.log(1.0 / sum_to_prod), sum_to_prod * np.log(min_dim), np.log(min_dim) / min_dim]
        )
...
def pca(data, ncomp=None, standardize=True, demean=True, normalize=True, gls=False, weights=None, method="svd"):
    ...
    """
    pc = PCA(
        data, ncomp=ncomp, standardize=standardize, demean=demean, normalize=normalize, gls=gls, weights=weights, method=method
    )
