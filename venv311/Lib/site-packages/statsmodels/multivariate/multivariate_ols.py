...
def _multivariate_test(hypotheses, exog_names, endog_names, fn):
    ...
    for hypo in hypotheses:
        ...
        if any(isinstance(j, str) for j in L):
            ...
            if L.shape[1] != k_xvar:
                raise ValueError(
                    "Contrast matrix L should have the same " "number of columns as exog! %d != %d" % (L.shape[1], k_xvar)
                )
        if M is None:
            M = np.eye(k_yvar)
        ...
        if C.shape[0] != L.shape[0]:
            raise ValueError(
                "contrast L and constant C must have the same " "number of rows! %d!=%d" % (L.shape[0], C.shape[0])
            )
        if C.shape[1] != M.shape[1]:
            raise ValueError(
                "transform M and constant C must have the same " "number of columns! %d!=%d" % (M.shape[1], C.shape[1])
            )
