...
def _eval_bspline_basis(x, knots, degree, deriv="all", include_intercept=True):
    ...
    if np.min(x) < np.min(knots) or np.max(x) > np.max(knots):
        raise NotImplementedError(
            "some data points fall outside the " "outermost knots, and I'm not sure how " "to handle them. (Patches accepted!)"
        )
...
def get_knots_bsplines(
    x=None, df=None, knots=None, degree=3, spacing="quantile", lower_bound=None, upper_bound=None, all_knots=None
):
    ...
    if np.any(inner_knots < lower_bound):
        raise ValueError(
            "some knot values (%s) fall below lower bound " "(%r)" % (inner_knots[inner_knots < lower_bound], lower_bound)
        )
    if np.any(inner_knots > upper_bound):
        raise ValueError(
            "some knot values (%s) fall above upper bound " "(%r)" % (inner_knots[inner_knots > upper_bound], upper_bound)
        )
...
class UnivariateBSplines(UnivariateGamSmoother):
    ...
    def __init__(
        self, x, df, degree=3, include_intercept=False, constraints=None, variable_name="x", covder2_kwds=None, **knot_kwds
    ):
        self.degree = degree
        self.df = df
...
class UnivariateCubicCyclicSplines(UnivariateGamSmoother):
    ...
    def _smooth_basis_for_single_variable(self):
        ...
        all_knots = _get_all_sorted_knots(
            self.x, n_inner_knots=n_inner_knots, inner_knots=None, lower_bound=None, upper_bound=None
        )
    ...
    def _make_smoothers_list(self):
        ...
        for v in range(self.k_variables):
            uv_smoother = UnivariateCubicCyclicSplines(
                self.x[:, v], df=self.dfs[v], constraints=self.constraints, variable_name=self.variable_names[v]
            )
            smoothers.append(uv_smoother)
