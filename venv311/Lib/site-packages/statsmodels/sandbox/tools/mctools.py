...
class StatTestMC:
    ...
    def summary_quantiles(self, idx, distppf, frac=[0.01, 0.025, 0.05, 0.1, 0.975], varnames=None, title=None):
        ...
        return SimpleTable(
            mmlar, txt_fmt={"data_fmts": ["%#6.3f"] + ["%#10.4f"] * (mmlar.shape[1] - 1)}, title=title, headers=headers
        )
    ...
    def summary_cdf(self, idx, frac, crit, varnames=None, title=None):
        ...
        return SimpleTable(
            mmlar, txt_fmt={"data_fmts": ["%#6.3f"] + ["%#10.4f"] * (np.array(mml).shape[1] - 1)}, title=title, headers=headers
        )
