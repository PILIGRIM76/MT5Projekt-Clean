...
class IVRegressionResults(RegressionResults):
    ...
    def summary(self, yname=None, xname=None, title=None, alpha=0.05):
        ...
        self.diagn = dict(
            jb=jb, jbpv=jbpv, skew=skew, kurtosis=kurtosis, omni=omni, omnipv=omnipv, condno=condno, mineigval=eigvals[0]
        )
...
class GMM(Model):
    ...
    def fit(
        ...
        if maxiter == 0 or maxiter == "cue":
            ...
            params = self.fitgmm(start, weights=weights, optim_method=optim_method, optim_args=optim_args)
        ...
        results = results_class_dict[self.results_class](
            model=self, params=params, weights=weights, wargs=wargs, options_other=options_other, optim_args=optim_args
        )
    ...
    def fititer(
        self, start, maxiter=2, start_invweights=None, weights_method="cov", wargs=(), optim_method="bfgs", optim_args=None
    ):
    ...
    def gradient_momcond(self, params, epsilon=1e-4, centered=True):
        ...
        if centered:
            gradmoms = (approx_fprime(params, momcond, epsilon=epsilon) + approx_fprime(params, momcond, epsilon=-epsilon)) / 2
        else:
    ...
    def score(self, params, weights, epsilon=None, centered=True):
        """Score"""
        deriv = approx_fprime(params, self.gmmobjective, args=(weights,), centered=centered, epsilon=epsilon)
...
class GMMResults(LikelihoodModelResults):
    ...
    def calc_cov_params(
        self, moms, gradmoms, weights=None, use_weights=False, has_optimal_weights=True, weights_method="cov", wargs=()
    ):
