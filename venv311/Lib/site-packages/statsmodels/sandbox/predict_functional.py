...
def _make_exog_from_formula(result, focus_var, summaries, values, num_points):
    ...
    if len(unmatched) > 0:
        warnings.warn(
            "%s in data frame but not in summaries or values." % ", ".join(["'%s'" % x for x in unmatched]), ValueWarning
        )
...
def _make_exog_from_arrays(result, focus_var, summaries, values, num_points):
    ...
    if len(unmatched) > 0:
        warnings.warn(
            "%s in model but not in `summaries` or `values`." % ", ".join(["'%s'" % x for x in unmatched]), ValueWarning
        )
