...
exogenous variables.  Got length %s" % len(sys))
        ...
        sp_exog = sparse.lil_matrix((int(self.nobs * M), int(np.sum(self.df_model + 1))))  # linked lists to build
    ...
    def whiten(self, X):
        ...
        if X is self.endog:  # definitely not a robust check
            ...
            return (sparse.kron(self.cholsigmainv, sparse.eye(nobs, nobs)) * X).toarray()  # *=dot until cast to array
