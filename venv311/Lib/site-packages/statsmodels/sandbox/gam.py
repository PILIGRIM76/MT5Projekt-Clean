...
class Model(GLM, AdditiveModel):
    ...
    def next(self):
        ...
        m = AdditiveModel(self.exog, smoothers=self.smoothers, weights=self.weights, family=self.family)
    ...
    def estimate_scale(self, Y=None):
        ...
        return (np.power(resid, 2) / self.family.variance(self.results.mu)).sum() / self.df_resid  # TODO check this
