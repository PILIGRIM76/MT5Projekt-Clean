...
class TukeyHSDResults:
    ...
    def _simultaneous_ci(self):
        ...
        self.halfwidths = simultaneous_ci(
            self.q_crit, self.variance, self._multicomp.groupstats.groupnobs, self._multicomp.pairindices
        )
    ...
    def plot_simultaneous(self, comparison_name=None, ax=None, figsize=(10, 6), xlabel=None, ylabel=None):
        ...
        if comparison_name is None:
            ...
            if len(sigidx) > 0:
                ax1.errorbar(
                    means[sigidx], sigidx, xerr=self.halfwidths[sigidx], marker="o", linestyle="None", color="r", ecolor="r"
                )
...
class MultiComparison:
    ...
    def kruskal(self, pairs=None, multimethod="T"):
        ...
        for i, j in zip(*self.pairindices):
            ...
            se = np.sqrt(f * np.sum(1.0 / groupnobs[[i, j]]))  # np.array([8,8]))) #Fixme groupnobs[[i,j]] ))
            Q = pdiff / se
    ...
    def allpairtest(self, testfunc, alpha=0.05, method="bonf", pvalidx=1):
        ...
        if pvals_corrected is None:
            resarr = np.array(
                lzip(self.groupsunique[i1], self.groupsunique[i2], np.round(res[:, 0], 4), np.round(res[:, 1], 4), reject),
                dtype=[("group1", object), ("group2", object), ("stat", float), ("pval", float), ("reject", np.bool_)],
            )
        else:
            resarr = np.array(
                lzip(
...
if __name__ == "__main__":
    ...
    examples = ["tukey", "tukeycrit", "fdr", "fdrmc", "bonf", "randmvn", "multicompdev", "None"]  # [-1]
    ...
    if "multicompdev" in examples:
        ...
        for i, j in zip(v2[diffidx], v1[diffidx]):
            # pdiff = np.abs(mrs[i] - mrs[j])
            pdiff = np.abs(meanranks[i] - meanranks[j])
            se = np.sqrt(f * np.sum(1.0 / xnobs[[i, j]]))  # np.array([8,8]))) #Fixme groupnobs[[i,j]] ))
            print(i, j, pdiff, se, pdiff / se, pdiff / se > 2.6310)
