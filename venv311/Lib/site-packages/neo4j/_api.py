...
SECURITY_TYPE_SELF_SIGNED_CERTIFICATE: t.Final[str] = "SECURITY_TYPE_SELF_SIGNED_CERTIFICATE"
...
def parse_neo4j_uri(uri):
    ...
    if parsed.scheme == api.URI_SCHEME_BOLT_ROUTING:
        raise ConfigurationError(f"Uri scheme {parsed.scheme!r} has been renamed. " f"Use {api.URI_SCHEME_NEO4J!r}")
...
def check_access_mode(access_mode):
    if access_mode not in {api.READ_ACCESS, api.WRITE_ACCESS}:
        raise ValueError(
            f"Unsupported access mode {access_mode}, must be one of " f"'{api.READ_ACCESS}' or '{api.WRITE_ACCESS}'."
        )
...
def parse_routing_context(query):
    ...
    for key in parameters:
        ...
        if len(value_list) != 1:
            raise ConfigurationError(
                f"Duplicated query parameters with key '{key}', value " f"'{value_list}' found in query string '{query}'"
            )
        value = value_list[0]
        if not value:
            raise ConfigurationError(f"Invalid parameters:'{key}={value}' in query string " f"'{query}'.")
        context[key] = value
