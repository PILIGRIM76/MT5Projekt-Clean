...
class AsyncBoltSocketBase(abc.ABC):
    ...
    async def _wait_for_io(self, name, timeout, deadline, io_async_fn, *args, **kwargs):
    ...
    async def _connect_secure(
        ...
        try:
            try:
                ...
                raise ServiceUnavailable("Timed out trying to establish connection to " f"{resolved_address!r}") from None
            except asyncio.CancelledError:
            ...
            if isinstance(error, OSError):
                raise ServiceUnavailable(
                    f"Failed to establish connection to {resolved_address!r} " f"(reason {error})"
                ) from error
            raise
...
class BoltSocketBase(abc.ABC):
    ...
    def _wait_for_io(self, name, timeout, deadline, func, *args, **kwargs):
        if deadline is None:
            ...
            if deadline_timeout <= 0:
                raise SocketDeadlineExceededError(f"{name} timed out")
        if deadline_timeout is not None and (timeout is None or deadline_timeout <= timeout):
    ...
    def _connect_secure(
        ...
        try:
            try:
                ...
                if isinstance(error, OSError):
                    raise ServiceUnavailable(
                        "Failed to establish connection to " f"{resolved_address!r} (reason {error})"
                    ) from error
                raise
