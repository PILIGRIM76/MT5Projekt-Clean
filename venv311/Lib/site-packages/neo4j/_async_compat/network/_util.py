...
class AsyncNetworkUtil:
    ...
    async def _dns_resolver(address, family=0):
        ...
        try:
            ...
            if e.errno in _EAI_NONAME and (address.host is None and address.port is None):
                err_cls = ValueError
            elif e.errno in _RETRYABLE_DNS_ERRNOS or e.errno in _EAI_NONAME:
                err_cls = ServiceUnavailable
            else:
                err_cls = ValueError
            raise err_cls(f"Failed to DNS resolve address {address}: {e}") from e
    ...
    async def resolve_address(address, family=0, resolver=None):
        ...
        if resolver:
            ...
            for address_resolved in addresses_resolved:
                ...
                addresses_dns_resolved = await AsyncNetworkUtil._dns_resolver(address_resolved, family=family)
            ...
            for address_dns_resolved in await AsyncNetworkUtil._dns_resolver(address, family=family):
                log.debug(
                    "[#0000]  _: <RESOLVE> dns resolver out: %s",
                    address_dns_resolved,
                )
    ...
    def resolve_address(address, family=0, resolver=None):
        ...
        if resolver:
            ...
            for address_resolved in addresses_resolved:
                ...
                addresses_dns_resolved = NetworkUtil._dns_resolver(address_resolved, family=family)
                for address_dns_resolved in addresses_dns_resolved:
                    log.debug(
                        "[#0000]  _: <RESOLVE> dns resolver out: %s",
                        address_dns_resolved,
            ...
            for address_dns_resolved in NetworkUtil._dns_resolver(address, family=family):
                log.debug(
                    "[#0000]  _: <RESOLVE> dns resolver out: %s",
                    address_dns_resolved,
                )
