...
if _t.TYPE_CHECKING:
    ...
    from ._optional_deps import np as _np
    from ._optional_deps import pa as _pa
...
class Vector:
    ...
    def raw(self, /, *, byteorder: _T_VectorEndian = "big") -> bytes:
        ...
        match byteorder:
            ...
            case _:
                raise ValueError(f"Invalid byteorder: {byteorder!r}. " "Must be 'big' or 'little'.")
    ...
    def __repr__(self) -> str:
        return f"{self.__class__.__name__}({self.raw()!r}, {self.dtype.value!r})"
    ...
    @classmethod
    @_t.overload
    def from_native(cls, data: _t.Iterable[float], dtype: _T_VectorDTypeFloat, /) -> _t.Self: ...

    @classmethod
    @_t.overload
    def from_native(cls, data: _t.Iterable[int], dtype: _T_VectorDTypeInt, /) -> _t.Self: ...
...
_T_VectorDType = VectorDType | _t.Literal["f32", "f64", "i8", "i16", "i32", "i64"]
...
_T_VectorDTypeFloat = _t.Literal[VectorDType.F32, VectorDType.F64, "f32", "f64"]
...
def _swap_endian(type_size: int, data: bytes, /) -> bytes:
    ...
    if len(data) % type_size != 0:
        raise ValueError(f"Data length {len(data)} is not a multiple of {type_size}")
...
class _InnerVector(_abc.ABC):
    ...
    def __init__(self, data: bytes, /, *, byteorder: _T_VectorEndian = "big") -> None:
        super().__init__()
    ...
    def data(self, data: bytes, /) -> None:
        ...
        if not len(data) % self.size == 0:
            raise ValueError(f"Data length {len(data)} is not a multiple of {self.size}")
        self._data = data
    ...
    def __init_subclass__(cls) -> None:
        ...
        if not isinstance(dtype, VectorDType):
            raise TypeError(f"Class {cls.__name__} must have a VectorDType attribute" "'dtype'")
        if not isinstance(getattr(cls, "size", None), int):
            raise TypeError(f"Class {cls.__name__} must have a str attribute 'size'")
        if cls.size not in {1, 2, 4, 8}:
            ...
            raise ValueError(f"Class {cls.__name__} has an unhandled size {cls.size}")
        if dtype in _TYPES:
            raise ValueError(f"Class {cls.__name__} has a duplicate type '{dtype}'")
        _TYPES[dtype] = cls
    ...
    @classmethod
    def from_numpy(cls, data: _np.ndarray, /) -> _t.Self:
        if data.dtype.byteorder == "<" or (data.dtype.byteorder == "=" and _sys.byteorder == "little"):
            data = data.byteswap()
    ...
    def from_pyarrow(cls, data: _pa.Array, /) -> _t.Self:
        ...
        buffer = buffer[data.offset * width : (data.offset + len(data)) * width]
...
class _VecF64(_InnerVectorFloat):
        ...
        if not all(isinstance(item, float) for item in data):
            for item in data:
                if not isinstance(item, float):
                    raise TypeError(f"Cannot build f64 vector from {type(item).__name__}, " "expected float.")
    ...
    def to_pyarrow(self) -> _pa.Array:
        ...
        return pyarrow.Array.from_buffers(pyarrow.float64(), len(self), [None, buffer], 0)
...
class _VecF32(_InnerVectorFloat):
    ...
    def from_native(cls, data: _t.Iterable[object], /) -> _t.Self:
        ...
        if not all(isinstance(item, float) for item in data):
            for item in data:
                if not isinstance(item, float):
                    raise TypeError(f"Cannot build f32 vector from {type(item).__name__}, " "expected float.")
        try:
            ...
            for item in data:
                try:
                    ...
                    raise OverflowError(f"Value {item} is out of range for f32: " f"[-3.4028234e+38, 3.4028234e+38]") from None
            raise
        return cls(bytes_)
    ...
    def to_pyarrow(self) -> _pa.Array:
        ...
        buffer = pyarrow.py_buffer(self.data_le)
        return pyarrow.Array.from_buffers(pyarrow.float32(), len(self), [None, buffer], 0)
...
class _VecI64(_InnerVectorInt):
        ...
        try:
            ...
            for item in data:
                if not isinstance(item, int):
                    raise TypeError(f"Cannot build i64 vector from {type(item).__name__}, " "expected int.") from None
                if not _I64_MIN <= item <= _I64_MAX:
                    raise OverflowError(f"Value {item} is out of range for i64: " f"[{_I64_MIN}, {_I64_MAX}]") from None
            raise
        return cls(bytes_)
    ...
    def to_pyarrow(self) -> _pa.Array:
        ...
        buffer = pyarrow.py_buffer(self.data_le)
        return pyarrow.Array.from_buffers(pyarrow.int64(), len(self), [None, buffer], 0)
...
class _VecI32(_InnerVectorInt):
    ...
    def from_native(cls, data: _t.Iterable[object], /) -> _t.Self:
        ...
        try:
            ...
            for item in data:
                if not isinstance(item, int):
                    raise TypeError(f"Cannot build i32 vector from {type(item).__name__}, " "expected int.") from None
                if not _I32_MIN <= item <= _I32_MAX:
                    raise OverflowError(f"Value {item} is out of range for i32: " f"[{_I32_MIN}, {_I32_MAX}]") from None
            raise
        return cls(bytes_)
    ...
    def to_pyarrow(self) -> _pa.Array:
        ...
        buffer = pyarrow.py_buffer(self.data_le)
        return pyarrow.Array.from_buffers(pyarrow.int32(), len(self), [None, buffer], 0)
...
class _VecI16(_InnerVectorInt):
    ...
    def from_native(cls, data: _t.Iterable[object], /) -> _t.Self:
        ...
        try:
            ...
            for item in data:
                if not isinstance(item, int):
                    raise TypeError(f"Cannot build i16 vector from {type(item).__name__}, " "expected int.") from None
                if not _I16_MIN <= item <= _I16_MAX:
                    raise OverflowError(f"Value {item} is out of range for i16: " f"[{_I16_MIN}, {_I16_MAX}]") from None
            raise
        return cls(bytes_)
    ...
    def to_pyarrow(self) -> _pa.Array:
        ...
        buffer = pyarrow.py_buffer(self.data_le)
        return pyarrow.Array.from_buffers(pyarrow.int16(), len(self), [None, buffer], 0)
...
class _VecI8(_InnerVectorInt):
    ...
    def from_native(cls, data: _t.Iterable[object], /) -> _t.Self:
        ...
        try:
            ...
            for item in data:
                if not isinstance(item, int):
                    raise TypeError(f"Cannot build i8 vector from {type(item).__name__}, " "expected int.") from None
                if not _I8_MIN <= item <= _I8_MAX:
                    raise OverflowError(f"Value {item} is out of range for i8: " f"[{_I8_MIN}, {_I8_MAX}]") from None
            raise
        return cls(bytes_)
    ...
    def to_pyarrow(self) -> _pa.Array:
        ...
        buffer = pyarrow.py_buffer(self.data_le)
        return pyarrow.Array.from_buffers(pyarrow.int8(), len(self), [None, buffer], 0)
