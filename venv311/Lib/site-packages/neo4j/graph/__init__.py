...
class Graph:
    ...
    def relationship_type(self, name: str) -> type[Relationship]:
        ...
        try:
            ...
            cls = self._relationship_types[name] = _t.cast(type[Relationship], type(str(name), (Relationship,), {}))
        return cls
    ...
    def _restore(relationship_types: tuple[str, ...]) -> Graph:
        ...
        graph.__dict__["_relationship_types"] = {name: type(str(name), (Relationship,), {}) for name in relationship_types}
        return graph
...
class Entity(_t.Mapping[str, _t.Any]):
    ...
    def __eq__(self, other: _t.Any) -> bool:
        try:
            return type(self) is type(other) and self.graph == other.graph and self.element_id == other.element_id
    ...
    def __init__(self, start_node: Node, *relationships: Relationship) -> None:
        ...
        for i, relationship in enumerate(relationships, start=1):
            ...
            if relationship.start_node == nodes[-1]:
                ...
                raise ValueError(f"Relationship {i} does not connect to the last node")
