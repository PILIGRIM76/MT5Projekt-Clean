...
if _t.TYPE_CHECKING:
    from ._async.work import AsyncManagedTransaction as _AsyncManagedTransaction
    from ._async.work import AsyncResult as _AsyncResult
    from ._async.work import AsyncSession as _AsyncSession
    from ._async.work import AsyncTransaction as _AsyncTransaction
    from ._sync.work import ManagedTransaction as _ManagedTransaction
    from ._sync.work import Result as _Result
    from ._sync.work import Session as _Session
    from ._sync.work import Transaction as _Transaction

    _TTransaction: _t.TypeAlias = _AsyncManagedTransaction | _AsyncTransaction | _ManagedTransaction | _Transaction
...
_UNKNOWN_GQL_DESCRIPTION: _t.Final[str] = "error: general processing exception - unexpected error"
_UNKNOWN_GQL_MESSAGE: _t.Final[str] = f"{_UNKNOWN_GQL_STATUS}: " "Unexpected error has occurred. See debug log for details."
...
class GqlError(Exception):
    ...
    def gql_classification(self) -> GqlErrorClassification:
        ...
        if not (isinstance(classification, str) and classification in _t.cast(_t.Iterable[str], iter(GqlErrorClassification))):
    ...
    def diagnostic_record(self) -> _t.Mapping[str, _t.Any]:
        ...
        self._diagnostic_record = _deepcopy(self._get_status_diagnostic_record())
        return self._diagnostic_record
...
class Neo4jError(GqlError):
    ...
    def __init__(self, *args: object) -> None:
        ...
        _, self._classification, self._category, self._title = self._neo4j_code.split(".")
    ...
    def _basic_hydrate(*, neo4j_code: str, message: str) -> Neo4jError:
        try:
            ...
            classification_override, code_override = _ERROR_REWRITE_MAP.get(neo4j_code, (None, None))
            if classification_override is not None:
                classification = classification_override
            if code_override is not None:
                neo4j_code = code_override

        error_class: type[Neo4jError] = Neo4jError._extract_error_class(classification, neo4j_code)
    ...
    def _unauthenticates_all_connections(self) -> bool:
        return self._neo4j_code == "Neo.ClientError.Security.AuthorizationExpired"
    ...
    def _is_fatal_during_discovery(self) -> bool:
        ...
        if code in {
            ...
            return True
        return code.startswith("Neo.ClientError.Security.") and code != "Neo.ClientError.Security.AuthorizationExpired"
...
class _MetaExtractor:
    ...
    def str_value(self, key: str, default: _T | None = None) -> str | _T | None:
    ...
    def map_value(self, key: str, default: _T | None = None) -> dict[str, _t.Any] | _T | None:
        res = self._metadata.pop(key, default)
        if not (isinstance(res, dict) and all(isinstance(k, str) for k in res)):
            res = default
        return res
...
class IncompleteCommit(ServiceUnavailable):
    ...
    def __init__(self, *args: object) -> None:
        ...
        self._init_gql(
            gql_status="08007",
            message=message,
            description=("error: connection exception - transaction resolution unknown"),
        )
