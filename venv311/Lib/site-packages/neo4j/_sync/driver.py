...
from .bookmark_manager import TBmConsumer as _TBmConsumer
from .bookmark_manager import TBmSupplier as _TBmSupplier
...
class GraphDatabase:
    ...
    if t.TYPE_CHECKING:
        ...
        def driver(
            ...
            *,
            ...
            resolver: t.Callable[[Address], t.Iterable[Address]] | t.Callable[[Address], t.Union[t.Iterable[Address]]] = ...,
            encrypted: bool = ...,
            trusted_certificates: TrustStore = ...,
            client_certificate: ClientCertificate | ClientCertificateProvider | None = ...,
            ssl_context: ssl.SSLContext | None = ...,
            user_agent: str = ...,
            keep_alive: bool = ...,
            notifications_min_severity: T_NotificationMinimumSeverity | None = ...,
            # deprecated in favor of notifications_disabled_classifications
            notifications_disabled_categories: t.Iterable[T_NotificationDisabledClassification] | None = ...,
            notifications_disabled_classifications: t.Iterable[T_NotificationDisabledClassification] | None = ...,
            warn_notification_severity: T_NotificationMinimumSeverity | None = ...,
        ...
        def driver(
            ...
            **config,
            ...
            if isinstance(client_certificate, ClientCertificate):
                ...
                config["client_certificate"] = _StaticClientCertificateProvider(client_certificate)

            if "trusted_certificates" in config and not isinstance(config["trusted_certificates"], TrustStore):
            ...
            if security_type in {
                SECURITY_TYPE_SELF_SIGNED_CERTIFICATE,
                SECURITY_TYPE_SECURE,
            } and ("encrypted" in config or "trusted_certificates" in config or "ssl_context" in config):
                raise ConfigurationError(
            ...
            if "notifications_disabled_categories" in config:
                deprecation_warn(
                    "notifications_disabled_categories is deprecated, " "use notifications_disabled_classifications instead.",
                    stack_level=2,
                )
            ...
            return cls._neo4j_driver(parsed.netloc, routing_context=routing_context, **config)
    ...
    def bookmark_manager(
        ...
        if not isinstance(initial_bookmarks, (Bookmarks, NoneType)):
            deprecation_warn(
                ("Passing raw strings as initial_bookmarks is deprecated. " "Use a Bookmarks object instead."),
                stack_level=2,
            )
            cast_initial_bookmarks = Bookmarks.from_raw_values(t.cast(t.Iterable[str], initial_bookmarks))
        else:
    ...
    def _neo4j_driver(cls, target, routing_context=None, **config):
        ...
        try:
            return Neo4jDriver._open(target, routing_context=routing_context, **config)
        except (BoltHandshakeError, BoltSecurityError) as error:
            from ..exceptions import ServiceUnavailable
...
class Driver:
    ...
    def execute_query(
        ...
        bookmark_manager_: BookmarkManager | BookmarkManager | None = ...,
        auth_: _TAuth = None,
    ...
    def execute_query(
        ...
        database_: str | None = None,
        impersonated_user_: str | None = None,
        bookmark_manager_: BookmarkManager | BookmarkManager | t.Literal[_DefaultEnum.default] | None = _default,
        auth_: _TAuth = None,
        ...
        **kwargs: t.Any,
        ...
        with session:
            if routing_ == RoutingControl.WRITE:
                ...
                raise ValueError(f"Invalid routing control value: {routing_!r}")
    ...
    if t.TYPE_CHECKING:
        ...
        def verify_connectivity(self, **config) -> None:
            ...
            self._check_state()
            if config:
                preview_warn("Passing key-word arguments to verify_connectivity() is a " "preview feature.")
            session_config = self._read_session_config(config)
    ...
    if t.TYPE_CHECKING:
        ...
        def get_server_info(self, **config) -> ServerInfo:
            ...
            self._check_state()
            if config:
                preview_warn("Passing key-word arguments to get_server_info() is a " "preview feature.")
            session_config = self._read_session_config(config)
    ...
    if t.TYPE_CHECKING:
        ...
        def verify_authentication(
            ...
            **config,
            ...
            self._check_state()
            if config:
                preview_warn("Passing key-word arguments except 'auth' to " "verify_authentication() is a preview feature.")
...
def _normalize_notifications_config(config_kwargs, *, driver_level=False):
    ...
    disabled_categories = config_kwargs.pop("notifications_disabled_categories", None)
    if disabled_categories is not None:
        disabled_classifications = config_kwargs.get("notifications_disabled_classifications")
        if disabled_classifications is None:
            disabled_classifications = disabled_categories
        else:
            disabled_classifications = list({*disabled_categories, *disabled_classifications})
        config_kwargs["notifications_disabled_classifications"] = disabled_classifications
    ...
    if driver_level:
        if value is None:
            if DEBUG_ENABLED:
                config_kwargs["warn_notification_severity"] = NotificationMinimumSeverity.INFORMATION
