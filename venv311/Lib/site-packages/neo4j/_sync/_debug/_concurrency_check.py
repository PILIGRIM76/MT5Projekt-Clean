        ...
        def _non_concurrent_method(cls, f: _TWrapped) -> _TWrapped:
            if Util.is_async_code:
                if not inspect.iscoroutinefunction(f):
                    raise TypeError(
                        "cannot decorate non-coroutine function with " "NonConcurrentMethodChecked._non_concurrent_method"
                    )
            elif not callable(f):
                raise TypeError(
                    "cannot decorate non-callable object with " "NonConcurrentMethodChecked._non_concurrent_method"
                )
        ...
        def _non_concurrent_iter(cls, f: _TWrappedIter) -> _TWrappedIter:
            if Util.is_async_code:
                if not inspect.isasyncgenfunction(f):
                    raise TypeError(
                        "cannot decorate non-async-generator function with " "NonConcurrentMethodChecked._non_concurrent_iter"
                    )
            elif not inspect.isgeneratorfunction(f):
                raise TypeError(
                    "cannot decorate non-generator function with " "NonConcurrentMethodChecked._non_concurrent_iter"
                )
