...
def dehydrate_time(value):
    ...
    if isinstance(value, Time):
        ...
        nanoseconds = (
            3600000000000 * value.hour + 60000000000 * value.minute + 1000000000 * value.second + 1000 * value.microsecond
        )
    else:
...
if np is not None:
    ...
    def dehydrate_np_datetime(value):
        ...
        if not 0 < year <= 9999:
            ...
            raise ValueError(f"Year out of range ({MIN_YEAR:d}..{MAX_YEAR:d}) found {year}")
        seconds = value.astype(np.dtype("datetime64[s]")).astype(np.int64)
        nanoseconds = value.astype(np.dtype("datetime64[ns]")).astype(np.int64) % _NANO_SECONDS
        return Structure(b"d", seconds, nanoseconds)
...
if np is not None:
    ...
    def dehydrate_np_timedelta(value):
        ...
        try:
            ...
            raise TypeError(f"Unsupported numpy.timedelta64 unit: {unit!r}") from None
