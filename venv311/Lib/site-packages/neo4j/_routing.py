...
class OrderedSet(MutableSet):
    ...
    def __repr__(self):
        return f"{self.__class__.__name__}((" f"{', '.join(map(repr, self._elements))}" f"))"
...
class RoutingTable:
    ...
    def parse_routing_info(cls, *, database, servers, ttl):
        ...
        try:
            for server in servers:
                role = server["role"]
                addresses = [Address.parse(address, default_port=7687) for address in server["addresses"]]
                if role == "ROUTE":
    ...
    def should_be_purged_from_memory(self):
        ...
        perf_time = monotonic()
        valid_until = self.last_updated_time + self.ttl + RoutingConfig.routing_table_purge_delay
