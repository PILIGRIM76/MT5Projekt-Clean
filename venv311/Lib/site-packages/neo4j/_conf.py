...
class Config(t.Mapping, metaclass=ConfigType):
    ...
    def consume_chain(data, *config_classes):
        ...
        if data:
            raise ConfigurationError(f"Unexpected config keys: {', '.join(data.keys())}")
        return values
    ...
    def __update(self, data, warn=True):
        ...
        def set_attr(k, v):
            if k in self.keys():
                ...
                k0 = self._get_new(k)
                if k0 in data_dict:
                    raise ConfigurationError(f"Cannot specify both '{k0}' and '{k}' in config")
                if warn:
                    deprecation_warn(f"The '{k}' config key is deprecated, please use " f"'{k0}' instead")
