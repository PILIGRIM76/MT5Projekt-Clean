...
from ._arithmetic import round_half_to_even as _round_half_to_even
from ._arithmetic import symmetric_divmod as _symmetric_divmod
from ._metaclasses import DateTimeType as _DateTimeType
from ._metaclasses import DateType as _DateType
from ._metaclasses import TimeType as _TimeType
...
_DATE_ISO_PATTERN: _t.Final[_re.Pattern] = _re_compile(r"^(\d{4})-(\d{2})-(\d{2})$")
...
_IS_LEAP_YEAR = {year: _is_leap_year(year) for year in range(MIN_YEAR, MAX_YEAR + 1)}
...
_DAYS_IN_YEAR = {year: _days_in_year(year) for year in range(MIN_YEAR, MAX_YEAR + 1)}
...
def _normalize_day(year, month, day):
    ...
    raise ValueError(f"Day {day} out of range (1..{days_in_month}, -1, -2 ,-3)")
...
class ClockTime(tuple):
    ...
    def __new__(cls, seconds: float = 0, nanoseconds: int = 0) -> ClockTime:
        seconds, nanoseconds = divmod(int(_NANO_SECONDS * seconds) + int(nanoseconds), _NANO_SECONDS)
    ...
    def __new__(cls):
        if cls.__implementations is None:
            ...
            cls.__implementations = sorted(
                (clock for clock in _Clock.__subclasses__() if clock.available()),
...
class Duration(tuple[int, int, int, int], _duration_base_class):  # type: ignore[misc]
    r"""
    ...
    def __new__(
        ...
        tuple_ = (mo, d, s, ns)
        avg_total_seconds = mo * _AVERAGE_SECONDS_IN_MONTH + d * _AVERAGE_SECONDS_IN_DAY + s - (1 if ns < 0 else 0)
    ...
    def __add__(self, other: Duration | _timedelta) -> Duration:  # type: ignore[override]
        ...
        * seconds and all sub-second units go into nanoseconds.
        ...
        if isinstance(other, (int, float)):
            return Duration(
                ...
                nanoseconds=_round_half_to_even(self[2] * _NANO_SECONDS * other + self[3] * other),
            )
        return NotImplemented
    ...
    def __divmod__(self, other: int) -> tuple[Duration, Duration]:  # type: ignore[override]
        """
    ...
    def __repr__(self) -> str:
        mo, day, sec, ns = self
        return "neo4j.time.Duration(" f"months={mo!r}, " f"days={day!r}, " f"seconds={sec!r}, " f"nanoseconds={ns!r}" ")"
...
class Date(_date_base_class, metaclass=_DateType):
    ...
    def today(cls, tz: _tzinfo | None = None) -> Date:
        ...
        if tz is None:
            ...
            return DateTime.utc_now().replace(tzinfo=_timezone.utc).astimezone(tz).date()
    ...
    @classmethod
    def from_timestamp(cls, timestamp: float, tz: _tzinfo | None = None) -> Date:
        """
    ...
    def parse(cls, s: str) -> Date:
        ...
        try:
            ...
            raise ValueError("Date string must be in format YYYY-MM-DD") from None
        else:
            if len(numbers) == 3:
                return cls(*numbers)
    ...
    @classmethod
    @_deprecated("ClockTime is an implementation detail. " "It and its related methods will be removed in a future version.")
    def from_clock_time(
        cls,
    ...
    def _from_clock_time(
        ...
        try:
            ...
            raise ValueError("Clock time must be a 2-tuple of (s, ns)") from None
        else:
    ...
    if _t.TYPE_CHECKING:
        ...
        @classmethod
        def fromtimestamp(cls, timestamp: float, tz: _tzinfo | None = None) -> Date: ...
...
def _dst(tz: _tzinfo | None = None, dt: DateTime | None = None) -> _timedelta | None:
    if tz is None:
        return None
    try:
        value = tz.dst(dt)
...
class Time(_time_base_class, metaclass=_TimeType):
    ...
    def now(cls, tz: _tzinfo | None = None) -> Time:
        ...
        if tz is None:
            ...
            return DateTime.utc_now().replace(tzinfo=_timezone.utc).astimezone(tz).timetz()
    ...
    @_t.overload
    def _get_both_normalized_ticks(self, other: Time | _time, strict: _t.Literal[True] = True) -> tuple[int, int]: ...

    @_t.overload
    def _get_both_normalized_ticks(self, other: Time | _time, strict: _t.Literal[False]) -> tuple[int, int] | None: ...

    def _get_both_normalized_ticks(self, other: Time | _time, strict: bool = True) -> tuple[int, int] | None:
    ...
    def iso_format(self) -> str:
        ...
        if offset is not None:
            offset_hours, offset_minutes = divmod(int(offset.total_seconds() // 60), 60)
    ...
    def __format__(self, format_spec):
        if not format_spec:
            return self.iso_format()
        format_spec = _FORMAT_F_REPLACE.sub(f"{self.__nanosecond:09}", format_spec)
        return self.to_native().__format__(format_spec)
...
class DateTime(_date_time_base_class, metaclass=_DateTimeType):
    ...
    def now(cls, tz: _tzinfo | None = None) -> DateTime:
        ...
        if tz is None:
            ...
            utc_now = cls._from_clock_time(_Clock().utc_time(), UnixEpoch).replace(tzinfo=tz)
            try:
    ...
    @classmethod
    def from_timestamp(cls, timestamp: float, tz: _tzinfo | None = None) -> DateTime:
        """
        ...
        """
        if tz is None:
            return cls._from_clock_time(_ClockTime(timestamp) + _Clock().local_offset(), UnixEpoch)
        else:
            return cls.utc_from_timestamp(timestamp).replace(tzinfo=_timezone.utc).astimezone(tz)
    ...
    if _t.TYPE_CHECKING:
        ...
        @classmethod
        def fromtimestamp(cls, timestamp: float, tz: _tzinfo | None = None) -> DateTime: ...
    ...
    def __lt__(self, other: _datetime | DateTime) -> bool:  # type: ignore[override]
        """
        ``<`` comparison with another datetime.
    ...
    def __le__(self, other: _datetime | DateTime) -> bool:  # type: ignore[override]
        """
        ``<=`` comparison with another datetime.
    ...
    def __ge__(self, other: _datetime | DateTime) -> bool:  # type: ignore[override]
        """
        ``>=`` comparison with another datetime.
    ...
    def __add__(self, other: _timedelta | Duration) -> DateTime:
        ...
        if isinstance(other, Duration):
            if other == (0, 0, 0, 0):
                return self
            t = self.time()._to_clock_time() + _ClockTime(other.seconds, other.nanoseconds)
        ...
        if isinstance(other, _timedelta):
            ...
            time_ = Time.from_ticks(_round_half_to_even(seconds * _NANO_SECONDS + t.nanoseconds))
            return self.combine(date_, time_).replace(tzinfo=self.tzinfo)
        return NotImplemented
    ...
    def __format__(self, format_spec):
        if not format_spec:
            return self.iso_format()
        format_spec = _FORMAT_F_REPLACE.sub(f"{self.__time.nanosecond:09}", format_spec)
        return self.to_native().__format__(format_spec)
    ...
    def __getattr__(self, name):
        ...
        try:
            ...
            raise AttributeError(f"DateTime has no attribute {name!r}") from None
...
def __getattr__(name):
    if name == "ClockTime":
        _deprecation_warn(
            "ClockTime is an implementation detail. It and its related " "methods will be removed in a future version.",
            stack_level=2,
        )
        return _ClockTime
