...
class ResultSummary:
        ...
        if "type" in metadata:
            ...
            if self.query_type not in {"r", "w", "rw", "s"}:
                raise BoltProtocolError(
                    f"Unexpected query type {self.query_type!r} received from " "server. Consider updating the driver.",
                    address,
                )
        ...
        if "diagnostic_record" in status:
            ...
            for notification_key, diag_record_key in (
                ...
                if diag_record_key in diagnostic_record:
                    notification[notification_key] = diagnostic_record[diag_record_key]

        return notification

    @property
    @deprecated("ResultSummary.notifications is deprecated, " "use ResultSummary.gql_status_objects instead.")
    ...
    @property
    @deprecated("ResultSummary.summary_notifications is deprecated, " "use ResultSummary.gql_status_objects instead.")
    def summary_notifications(self) -> Sequence[SummaryNotification]:
        ...
        if not isinstance(raw_notifications, list):
            ...
            return self._summary_notifications
        self._summary_notifications = tuple(SummaryNotification._from_metadata(n) for n in raw_notifications)
        return self._summary_notifications
    ...
    def gql_status_objects(self) -> t.Sequence[GqlStatusObject]:
        ...
        * A "success" (``00xxx``) has precedence over anything informational
        ...
        if isinstance(raw_status_objects, list):
            self._gql_status_objects = tuple(GqlStatusObject._from_status_metadata(s) for s in raw_status_objects)
        ...
        if isinstance(raw_notifications, list):
            notification_status_objects = [GqlStatusObject._from_notification_metadata(n) for n in raw_notifications]
        else:
        ...
        notification_status_objects = itertools.chain(notification_status_objects, (result_status,))
...
_COUNTER_ATTR_NAME_TO_KEY = {v: k for k, v in _COUNTER_KEY_TO_ATTR_NAME.items()}
...
class SummaryCounters:
    ...
    def __repr__(self) -> str:
        statistics = {_COUNTER_ATTR_NAME_TO_KEY[k]: v for k, v in vars(self).items() if k in _COUNTER_ATTR_NAME_TO_KEY}
    ...
    def __str__(self) -> str:
        ...
        attrs.append(f"contains_system_updates: {self.contains_system_updates}")
...
class SummaryInputPosition:
    ...
    def __str__(self) -> str:
        return f"line: {self.line}, column: {self.column}, offset: {self.offset}"

    def __repr__(self) -> str:
        return f"<{self.__class__.__name__} " f"line={self.line!r}, " f"column={self.column!r}, " f"offset={self.offset!r}" ">"
...
class SummaryNotification:
    ...
    def _from_metadata(cls, metadata: object) -> te.Self:
        ...
        kwargs: _SummaryNotificationKwargs = {
            "position": SummaryInputPosition._from_metadata(metadata.get("position")),
        }
...
class GqlStatusObject:
    ...
    def is_notification(self) -> bool:
        ...
        self._is_notification = bool(isinstance(neo4j_code, str) and neo4j_code)
        return self._is_notification
    ...
    def gql_status(self) -> str:
        ...
        self._gql_status = self._extract_str_field(self._status_metadata, "gql_status")
        return self._gql_status
    ...
    def status_description(self) -> str:
        ...
        self._status_description = self._extract_str_field(self._status_metadata, "status_description")
        return self._status_description
    ...
    def _get_status_diagnostic_record(self) -> dict[str, t.Any]:
        ...
        self._status_diagnostic_record = self._status_metadata.get("diagnostic_record", {})
    ...
    def position(self) -> SummaryInputPosition | None:
        ...
        self._position = SummaryInputPosition._from_metadata(diag_record.get("_position"))
        return self._position
    ...
    def raw_classification(self) -> str | None:
        ...
        diag_record = self._get_status_diagnostic_record()
        self._raw_classification = self._extract_str_field(diag_record, "_classification", None)
        return self._raw_classification
    ...
    def classification(self) -> NotificationClassification:
        ...
        self._classification = _CLASSIFICATION_LOOKUP.get(self.raw_classification, NotificationClassification.UNKNOWN)
        return self._classification
    ...
    def raw_severity(self) -> str | None:
        ...
        diag_record = self._get_status_diagnostic_record()
        self._raw_severity = self._extract_str_field(diag_record, "_severity", None)
        return self._raw_severity
    ...
    def severity(self) -> NotificationSeverity:
        ...
        self._severity = _SEVERITY_LOOKUP.get(self.raw_severity, NotificationSeverity.UNKNOWN)
        return self._severity
    ...
    def diagnostic_record(self) -> dict[str, t.Any]:
        ...
        self._diagnostic_record = deepcopy(self._get_status_diagnostic_record())
        return self._diagnostic_record
