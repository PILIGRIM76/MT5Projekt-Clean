...
class BoltProtocolVersion:
    def __init__(self, major: int, minor: int) -> None:
        if major < 0 or minor < 0:
            raise ValueError("Major and minor versions must be non-negative integers. " f"Found {major}, {minor}.")
...
def min_timeout(*timeouts: float | None) -> float | None:
    ...
    return min(
        (to for to in timeouts if to is not None and not math.isnan(to) and to >= 0),
        default=None,
    )
