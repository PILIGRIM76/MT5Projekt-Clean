...
class AsyncWorkspace(AsyncNonConcurrentMethodChecker):
    ...
    def _initialize_bookmarks(self, bookmarks):
        if bookmarks is None:
            ...
            raise TypeError("Bookmarks must be an instance of Bookmarks or None.")
    ...
    async def _connect(self, access_mode, auth=None, **acquire_kwargs) -> None:
        ...
        acquisition_deadline = acquisition_timeout_to_deadline(acquisition_timeout)
        ...
        if target_db.guessed and not self._pinned_database and not self._connection.ssr_enabled:
            # race condition: we now have created a connection which does not
            # support SSR.
            # => we need to fall back to explicit home database resolution
            log.debug(
                "[#0000]  _: <WORKSPACE> detected ssr support race; " "falling back to explicit home database resolution",
            )
            await self._disconnect()
            target_db = await self._get_routing_target_database(
                acquire_auth,
    ...
    async def _get_routing_target_database(
        ...
        if self._pinned_database or self._config.database is not None or self._pool.is_direct_pool:
            log.debug(
        ...
        if ssr_enabled:
            ...
            if cached_db is not None:
                log.debug(
                    ("[#0000]  _: <WORKSPACE> routing towards cached " "database: %s"),
                    cached_db,
                )
    ...
    async def _resolve_session_auth(
        ...
        auth_getter: t.Callable[[], _TAuth | t.Awaitable[_TAuth]] = auth.get_auth
