...
class AsyncResult(AsyncNonConcurrentMethodChecker):
    ...
    def _pull(self):
        def on_records(records):
            ...
            if not self._discarding:
                records = (record.raw_data if isinstance(record, BrokenHydrationObject) else record for record in records)
                self._record_buffer.extend(Record(zip(self._keys, record, strict=True)) for record in records)
    ...
    def _handle_warnings(self) -> None:
        ...
        if self._warn_notification_severity == NotificationMinimumSeverity.WARNING:
            sev_filter = (NotificationMinimumSeverity.WARNING,)
        elif self._warn_notification_severity == NotificationMinimumSeverity.INFORMATION:
            sev_filter = (
        ...
        for notification in (
            gql_status_object for gql_status_object in summary.gql_status_objects if gql_status_object.is_notification
        ):
            ...
            if notification.classification == NotificationClassification.DEPRECATION:
    ...
    def _creation_frame(self) -> t.Literal[False] | inspect.FrameInfo:
        ...
        if self._creation_stack is None:
            ...
            self._creation_frame_cache = next(
                (frame for frame in self._creation_stack if not frame.filename.startswith(str(_driver_dir))),
                False,
            )
    ...
    @t.overload
    async def single(self, strict: t.Literal[False] = False) -> Record | None: ...
    ...
    async def single(self, strict: bool = False) -> Record | None:
        ...
        if not buffer:
            ...
            if not strict:
                ...
                raise ResultNotSingleError(
                    self,
                    "More than one record found. " "Make sure your query returns exactly one record.",
                )
    ...
    @AsyncNonConcurrentMethodChecker._non_concurrent_method
    async def value(self, key: _TResultKey = 0, default: object = None) -> list[t.Any]:
        """
    ...
    async def to_df(self, expand: bool = False, parse_dates: bool = False) -> pandas.DataFrame:
        ...
        def datetime_to_timestamp(x):
            ...
            return pd.Timestamp(x.astimezone(pytz.UTC).iso_format()).astimezone(tzinfo)

        df[dt_columns] = df[dt_columns].apply(lambda col: col.map(datetime_to_timestamp))
        return df
