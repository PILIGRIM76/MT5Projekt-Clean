...
class AsyncHomeDbCache:
    ...
    def __init__(
        ...
        if max_size is not None and max_size <= 0:
            raise ValueError(f"home db cache max_size must be greater 0 or None, " f"got {max_size}")
        self._max_size = max_size
        self._truncate_size = min(max_size, int(0.01 * max_size * math.log(max_size))) if max_size is not None else None
    ...
    def _clean(self, now: float | None = None) -> None:
        ...
        if now - self._oldest_entry > self._ttl:
            self._cache = {k: v for k, v in self._cache.items() if now - v[0] < self._ttl * 0.9}
            self._oldest_entry = min((v[0] for v in self._cache.values()), default=now)
...
def _consolidate_auth_token(auth: dict) -> tuple | str:
    if auth.get("scheme") == "basic" and isinstance(auth.get("principal"), str):
        return auth["principal"]
    return _hashable_dict(auth)


def _hashable_dict(d: dict) -> tuple:
    return tuple((k, _hashable_dict(v) if isinstance(v, dict) else v) for k, v in sorted(d.items()))
