...
class AsyncBolt:
    ...
    HYDRATION_HANDLER_CLS: type[HydrationHandlerABC] = hydration_v1.HydrationHandler
    ...
    def __init__(
        ...
        *,
        ...
        self.notifications_disabled_classifications = notifications_disabled_classifications
    ...
    protocol_handlers: t.ClassVar[dict[BoltProtocolVersion, type[AsyncBolt]]] = {}
    ...
    def __init_subclass__(cls: type[t.Self], **kwargs: t.Any) -> None:
        ...
        if protocol_version is None:
            raise ValueError("AsyncBolt subclasses must define PROTOCOL_VERSION")
        if not isinstance(protocol_version, BoltProtocolVersion):
            raise TypeError(
                "PROTOCOL_VERSION must be a BoltProtocolVersion, found " f"{type(protocol_version)} for {cls.__name__}"
            )
        if protocol_version in AsyncBolt.protocol_handlers:
            cls_conflict = AsyncBolt.protocol_handlers[protocol_version]
            raise TypeError(f"Multiple classes for the same protocol version " f"{protocol_version}: {cls}, {cls_conflict}")
    ...
    def get_handshake(cls) -> bytes:
        ...
        :returns: bytes
        """
        return b"\x00\x00\x01\xff\x00\x08\x08\x05\x00\x02\x04\x04\x00\x00\x00\x03"
    ...
    async def send_all(self):
        ...
        if self.closed():
            raise ServiceUnavailable(
                "Failed to write to closed connection " f"{self.unresolved_address!r} ({self.server_info.address!r})"
            )
        if self.defunct():
            raise ServiceUnavailable(
                "Failed to write to defunct connection " f"{self.unresolved_address!r} ({self.server_info.address!r})"
            )
    ...
    async def fetch_message(self):
        if self._closed:
            raise ServiceUnavailable(
                "Failed to read from closed connection " f"{self.unresolved_address!r} ({self.server_info.address!r})"
            )
        if self._defunct:
            raise ServiceUnavailable(
                "Failed to read from defunct connection " f"{self.unresolved_address!r} ({self.server_info.address!r})"
            )
    ...
    async def _set_defunct_read(self, error=None, silent=False):
        message = "Failed to read from defunct connection " f"{self.unresolved_address!r} ({self.server_info.address!r})"
        await self._set_defunct(message, error=error, silent=silent)

    async def _set_defunct_write(self, error=None, silent=False):
        message = "Failed to write data to connection " f"{self.unresolved_address!r} ({self.server_info.address!r})"
        await self._set_defunct(message, error=error, silent=silent)
    ...
    async def _set_defunct(self, message, error=None, silent=False):
        ...
        if not self._closing:
            ...
            # connection again.
            await self.close()
            if not self._liveness_check and self.pool and not self._get_server_state_manager().failed():
