    ...
    def remove_connection(self, connection):
        if self.feature_check(connection):
            if self.with_feature == 0:
                raise RuntimeError("No connections to be removed from feature tracker")
            self.with_feature -= 1
        else:
...
class AsyncIOPool(abc.ABC):
    ...
    def __init__(self, opener, pool_config, workspace_config):
        ...
        self._ssr_feature_tracker = ConnectionFeatureTracker(feature_check=lambda connection: connection.ssr_enabled)
        ...
        while not deadline.expired():
            ...
            if not await health_check(connection, deadline):
                ...
                if log.isEnabledFor(logging.DEBUG):
                    log.debug(
                        "[#%04X]  _: <POOL> found unhealthy connection %s " "(closed=%s, defunct=%s, stale=%s, in_use=%s)",
                        connection.local_port,
                        connection.connection_id,
                        connection.closed(),
                        connection.defunct(),
    ...
    async def _acquire(
        ...
        async def health_check(connection_, deadline_):
            if connection_.closed() or connection_.defunct() or connection_.stale():
                return False
            if liveness_check_timeout is not None and connection_.is_idle_for(liveness_check_timeout):
        ...
        while True:
            ...
            if connection:
                ...
                try:
                    await self._re_auth_connection(connection, auth, force_auth, unprepared)
                except ConfigurationError:
                    if auth:
            ...
            with self.lock:
                ...
                if timeout == 0 or not await self.cond.wait(timeout):  # deadline expired
                    ...
                    raise ConnectionAcquisitionTimeoutError(
                        "failed to obtain a connection from the pool within " f"{deadline.original_timeout!r}s (timeout)"
                    )
        ...
        for connection in connections:
            if not (connection.defunct() or connection.closed() or connection.is_reset):
                ...
                try:
                    ...
                    log.debug(
                        "[#%04X]  _: <POOL> failed to reset connection " "on release: %r",
                        connection.local_port,
                        exc,
                    )
                except asyncio.CancelledError as exc:
                    log.debug(
                        "[#%04X]  _: <POOL> cancelled reset connection " "on release: %r",
                        connection.local_port,
                        exc,
                    )
                    cancelled = exc
    ...
    async def on_write_failure(self, address, database):
        raise WriteServiceUnavailable(f"No write service available for pool {self}")
    ...
    async def close(self):
        ...
        try:
            with self.lock:
                connections = [
                    connection for address in list(self.connections) for connection in self.connections.pop(address, ())
                ]
                for connection in connections:
                    self._ssr_feature_tracker.remove_connection(connection)
...
class AsyncBoltPool(AsyncIOPool):
    ...
    async def acquire(
        ...
        log.debug(
            "[#0000]  _: <POOL> acquire direct connection, " "access_mode=%r, database=%r",
            access_mode,
            database,
        )
        return await self._acquire(self.address, auth, deadline, liveness_check_timeout, unprepared)
...
class AsyncNeo4jPool(AsyncIOPool):
    ...
    async def fetch_routing_table(
        ...
        *,
        ...
        if num_routers == 0:
            ...
            if ignored_errors is not None:
                ignored_errors.append(RoutingServiceUnavailable("Rejected routing table: no routers"))
            return None
        ...
        if num_readers == 0:
            ...
            if ignored_errors is not None:
                ignored_errors.append(ReadServiceUnavailable("Rejected routing table: no readers"))
            return None
    ...
    async def _update_routing_table_from(
        ...
        *routers,
        ...
        if routers:
            log.debug(
                ("[#0000]  _: <POOL> " "attempting to update routing table from %s"),
    ...
    async def update_routing_table(
        ...
        *,
        ...
        """
        acquisition_timeout = acquisition_timeout_to_deadline(acquisition_timeout)
        errors = []
        async with self.refresh_lock:
            ...
            if routing_table is not None:
                ...
                prefer_initial_routing_address = routing_table.initialized_without_writers
            else:
            ...
            if not prefer_initial_routing_address and await self._update_routing_table_from(
                self.address,
                database=database,
                imp_user=imp_user,
                bookmarks=bookmarks,
            ...
            if sys.version_info >= (3, 11):
                e = ExceptionGroup("All routing table requests failed", errors)  # noqa: F821 # version guard in place
            else:
                e = None
                for error in errors:
                    if e is None:
                    ...
                    cause.__cause__ = e
                    e = error
            raise ServiceUnavailable("Unable to retrieve routing information") from e
    ...
    async def ensure_routing_table_is_fresh(
        ...
        *,
        ...
        async with self.refresh_lock:
            ...
            if routing_table is not None and routing_table.is_fresh(readonly=(access_mode == READ_ACCESS)):
    ...
    async def acquire(
        ...
        log.debug(
            "[#0000]  _: <POOL> acquire routing connection, " "access_mode=%r, database=%r",
            access_mode,
            database,
        )
        await self.ensure_routing_table_is_fresh(
            ...
            database_callback=(wrapped_database_callback if database.guessed else database_callback),
        )
        ...
        while True:
            try:
                ...
                # connections.
                address = await self._select_address(access_mode=access_mode, database=target_database)
            except (ReadServiceUnavailable, WriteServiceUnavailable) as err:
                raise SessionExpired(f"Failed to obtain connection towards '{access_mode}' " "server.") from err
            try:
                log.debug(
...
def _check_acquisition_timeout(timeout: object) -> None:
    if not isinstance(timeout, (int, float)):
        raise TypeError("Connection acquisition timeout must be a number, " f"got {type(timeout)}")
    if timeout <= 0:
        raise ValueError(f"Connection acquisition timeout must be > 0, got {timeout}")
