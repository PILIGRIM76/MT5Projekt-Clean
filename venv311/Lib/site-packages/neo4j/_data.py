...
class Record(tuple, t.Mapping):
    ...
    def __repr__(self) -> str:
        fields = " ".join(f"{field}={value!r}" for field, value in zip(self.__keys, super().__iter__(), strict=True))
    ...
    def __getitem__(self, key: _K | slice) -> t.Any:  # type: ignore[override]
    ...
    def items(self, *keys):
        ...
        if keys:
            ...
            return d
        return [(self.__keys[i], self._super_getitem_single(i)) for i in range(len(self))]
...
class RecordTableRowExporter(DataTransformer):
    ...
    def transform(self, x):
        ...
        typ = type(x)
        return typ(item for k, v in x.items() for item in self._transform(v, prefix=self._escape_map_key(k)).items())
    ...
    def _transform(self, x, prefix):
        if isinstance(x, Node):
            ...
            return dict(item for i, v in enumerate(x) for item in self._transform(v, prefix=f"{prefix}[].{i}").items())
        elif isinstance(x, t.Mapping):
            typ = type(x)
            return typ(
                item
                for k, v in x.items()
                for item in self._transform(v, prefix=f"{prefix}{{}}.{self._escape_map_key(k)}").items()
            )
        else:
            return {prefix: x}
