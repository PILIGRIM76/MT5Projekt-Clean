...
class HumanRendering(VectorWrapper, gym.utils.RecordConstructorArgs):
    ...
    def _render_frame(self):
        ...
        if self.scaled_subenv_size is None:
            ...
            assert (num_cols * subenv_size[0] * scaling_factor == self.screen_size[0]) or (
                num_rows * subenv_size[1] * scaling_factor == self.screen_size[1]
            )
...
class RecordVideo(
    ...
    def __init__(
        ...
        if autoreset_mode is not None:
            ...
            warn(f"{env} metadata doesn't specify its autoreset mode ({env.metadata!r}), therefore, defaulting to next step.")
    ...
    def _concat_frames(self, frames):
        ...
        grid = np.zeros((self.frame_rows * h, self.frame_cols * w, c), dtype=frames.dtype)
    ...
    def step(self, actions: ActType) -> tuple[ObsType, SupportsFloat, bool, bool, dict[str, Any]]:
        ...
        elif self.autoreset_mode == gym.vector.AutoresetMode.SAME_STEP and (terminations[0] or truncations[0]):
            self.episode_id += 1
            if self.recording and self.video_length == float("inf"):
                self.stop_recording()
