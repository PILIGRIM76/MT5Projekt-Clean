...
class TransformObservation(VectorObservationWrapper):
    ...
    def __init__(
        ...
        if self.observation_space != batch_space(self.single_observation_space, self.num_envs):
            warn(
...
class VectorizeTransformObservation(VectorObservationWrapper):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.wrapper = wrapper(self._SingleEnv(self.env.single_observation_space), **kwargs)
    ...
    def step(self, actions: ActType) -> tuple[ObsType, ArrayType, ArrayType, ArrayType, dict[str, Any]]:
        ...
        if self.autoreset_mode == AutoresetMode.SAME_STEP and "final_obs" in infos:
            ...
            for i, (sub_obs, has_final_obs) in enumerate(zip(final_obs, infos["_final_obs"])):
                if has_final_obs:
    ...
    def observations(self, observations: ObsType) -> ObsType:
        ...
        if self.same_out:
            return concatenate(
                ...
                tuple(self.wrapper.func(obs) for obs in iterate(self.observation_space, observations)),
                observations,
            )
        else:
            return deepcopy(
                concatenate(
                    self.single_observation_space,
                    tuple(self.wrapper.func(obs) for obs in iterate(self.env.observation_space, observations)),
                    self.out,
                )
            )
...
class GrayscaleObservation(VectorizeTransformObservation):
    ...
    def __init__(self, env: VectorEnv, keep_dim: bool = False):
        ...
        """
        super().__init__(env, transform_observation.GrayscaleObservation, keep_dim=keep_dim)
