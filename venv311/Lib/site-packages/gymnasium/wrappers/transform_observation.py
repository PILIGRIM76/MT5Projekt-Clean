...
class FilterObservation(
    ...
    def __init__(self, env: gym.Env[ObsType, ActType], filter_keys: Sequence[str | int]):
        ...
        if not isinstance(filter_keys, Sequence):
            raise TypeError(f"Expects `filter_keys` to be a Sequence, actual type: {type(filter_keys)}")
        ...
        if isinstance(env.observation_space, spaces.Dict):
            ...
            if any(key not in env.observation_space.spaces.keys() for key in filter_keys):
                missing_keys = [key for key in filter_keys if key not in env.observation_space.spaces.keys()]
                raise ValueError(
            ...
            new_observation_space = spaces.Dict({key: env.observation_space[key] for key in filter_keys})
            if len(new_observation_space) == 0:
                raise ValueError("The observation space is empty due to filtering all of the keys.")
            ...
            assert len(set(filter_keys)) == len(filter_keys), f"Duplicate keys exist, filter_keys: {filter_keys}"

            if any(0 < key and key >= len(env.observation_space) for key in filter_keys):
                missing_index = [key for key in filter_keys if 0 < key and key >= len(env.observation_space)]
                raise ValueError(
        ...
        if keep_dim:
            ...
            TransformObservation.__init__(
                ...
                func=lambda obs: np.expand_dims(
                    np.sum(np.multiply(obs, np.array([0.2125, 0.7154, 0.0721])), axis=-1).astype(np.uint8),
                    axis=-1,
                ),
                observation_space=new_observation_space,
            )
        else:
            new_observation_space = spaces.Box(low=0, high=255, shape=env.observation_space.shape[:2], dtype=np.uint8)
            TransformObservation.__init__(
                self,
                env=env,
                func=lambda obs: np.sum(np.multiply(obs, np.array([0.2125, 0.7154, 0.0721])), axis=-1).astype(np.uint8),
                observation_space=new_observation_space,
            )
...
class ResizeObservation(
    ...
    def __init__(self, env: gym.Env[ObsType, ActType], shape: tuple[int, int]):
        ...
        try:
            ...
            raise DependencyNotInstalled('opencv (cv2) is not installed, run `pip install "gymnasium[other]"`') from e
        ...
        TransformObservation.__init__(
            self,
            env=env,
            func=lambda obs: cv2.resize(obs, self.cv2_shape, interpolation=cv2.INTER_AREA),
            observation_space=new_observation_space,
        )
...
class AddRenderObservation(
    ...
    def __init__(
        ...
        if render_only:
            ...
            TransformObservation.__init__(self, env=env, func=lambda _: self.render(), observation_space=obs_space)
        elif isinstance(env.observation_space, spaces.Dict):
            assert render_key not in env.observation_space.spaces.keys()

            obs_space = spaces.Dict({render_key: pixel_space, **env.observation_space.spaces})
            TransformObservation.__init__(
                self,
                env=env,
            ...
            obs_space = spaces.Dict({obs_key: env.observation_space, render_key: pixel_space})
            TransformObservation.__init__(
                self,
                env=env,
...
class DiscretizeObservation(
    ...
    def __init__(
        ...
        if not isinstance(env.observation_space, spaces.Box):
            raise TypeError("DiscretizeObservation is only compatible with Box continuous observations.")
        ...
        if np.any(np.isinf(self.low)) or np.any(np.isinf(self.high)):
            raise ValueError(
                "Discretization requires observation space to be finite. " f"Found: low={self.low}, high={self.high}"
            )
        ...
        if isinstance(bins, int):
            ...
            assert len(bins) == self.n_dims, f"bins must match action dimensions: expected {self.n_dims}, got {len(bins)}"
            self.bins = np.array(bins)

        self.bin_edges = [np.linspace(self.low[i], self.high[i], self.bins[i] + 1)[1:-1] for i in range(self.n_dims)]
    ...
    def observation(self, observation):
        ...
        indices = [int(np.digitize(clipped[i], self.bin_edges[i])) for i in range(self.n_dims)]
        if self.multidiscrete:
    ...
    def revert_observation(self, obs):
        ...
        return np.array(lows, dtype=self.env.observation_space.dtype), np.array(highs, dtype=self.env.observation_space.dtype)
