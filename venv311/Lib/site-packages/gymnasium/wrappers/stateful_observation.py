...
class TimeAwareObservation(
    ...
    def __init__(
        ...
        *,
        ...
        if self.normalize_time:
            self._time_preprocess_func = lambda time: np.array([time / self.max_timesteps], dtype=np.float32)
        ...
        if self.flatten:
            self.observation_space: gym.Space[WrapperObsType] = spaces.flatten_space(observation_space)
    ...
    def observation(self, observation: ObsType) -> WrapperObsType:
        ...
        """
        return self._obs_postprocess_func(self._append_data_func(observation, self._time_preprocess_func(self.timesteps)))
...
class FrameStackObservation(
    ...
    def step(self, action: WrapperActType) -> tuple[WrapperObsType, SupportsFloat, bool, bool, dict[str, Any]]:
        ...
        updated_obs = deepcopy(concatenate(self.env.observation_space, self.obs_queue, self.stacked_obs))
...
class NormalizeObservation(
    ...
    def __init__(self, env: gym.Env[ObsType, ActType], epsilon: float = 1e-8):
        ...
        self.obs_rms = RunningMeanStd(shape=self.observation_space.shape, dtype=self.observation_space.dtype)
    ...
    def observation(self, observation: ObsType) -> WrapperObsType:
        ...
        return np.float32((observation - self.obs_rms.mean) / np.sqrt(self.obs_rms.var + self.epsilon))
