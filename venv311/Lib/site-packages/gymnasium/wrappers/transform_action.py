...
class DiscretizeAction(
    ...
    def __init__(
        ...
        if not isinstance(env.action_space, Box):
            raise TypeError("DiscretizeAction is only compatible with Box continuous actions.")
        ...
        if np.any(np.isinf(self.low)) or np.any(np.isinf(self.high)):
            raise ValueError("Discretization requires action space to be finite. " f"Found: low={self.low}, high={self.high}")
    ...
    def action(self, act):
        ...
        centers = [self.bin_centers[i][min(max(idx, 0), self.bins[i] - 1)] for i, idx in enumerate(indices)]
    ...
    def revert_action(self, action):
        ...
        indices = [np.argmin(np.abs(self.bin_centers[i] - action[i])) for i in range(self.n_dims)]
        if self.multidiscrete:
