...
class RenderCollection(
    ...
    def __init__(
        ...
        """
        gym.utils.RecordConstructorArgs.__init__(self, pop_frames=pop_frames, reset_clean=reset_clean)
...
class HumanRendering(gym.Wrapper[ObsType, ActType, ObsType, ActType], gym.utils.RecordConstructorArgs):
    ...
    def _render_frame(self):
        ...
        try:
            ...
            raise DependencyNotInstalled('pygame is not installed, run `pip install "gymnasium[classic-control]"`')
...
class AddWhiteNoise(gym.Wrapper[ObsType, ActType, ObsType, ActType], gym.utils.RecordConstructorArgs):
    ...
    def render(self) -> RenderFrame:
        ...
        mask = self.np_random.random(render_out.shape[0:2]) < self.probability_of_noise_per_pixel

        return np.where(mask[..., None], noise, render_out)


class ObstructView(gym.Wrapper[ObsType, ActType, ObsType, ActType], gym.utils.RecordConstructorArgs):
    ...
    def __init__(
        ...
        if not 0 <= obstructed_pixels_ratio < 1:
            raise ValueError(f"obstructed_pixels_ratio should be in the interval [0,1). Received {obstructed_pixels_ratio}")

        if obstruction_width < 1:
            raise ValueError(f"obstruction_width should be larger or equal than 1. Received {obstruction_width}")
