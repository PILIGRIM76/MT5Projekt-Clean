...
class CartPoleFunctional(FuncEnv[StateType, jax.Array, int, float, bool, RenderStateType, CartPoleParams]):
    """Cartpole but in jax and functional."""

    observation_space = gym.spaces.Box(-np.inf, np.inf, shape=(4,), dtype=np.float32)
    action_space = gym.spaces.Discrete(2)

    def initial(self, rng: PRNGKeyType, params: CartPoleParams = CartPoleParams) -> StateType:
        """Initial state generation."""
        return jax.random.uniform(key=rng, minval=-params.x_init, maxval=params.x_init, shape=(4,))
    ...
    def observation(self, state: StateType, rng: Any, params: CartPoleParams = CartPoleParams) -> jax.Array:
        """Cartpole observation."""
        return state

    def terminal(self, state: StateType, rng: Any, params: CartPoleParams = CartPoleParams) -> jax.Array:
    ...
    def render_image(
        ...
        try:
            ...
            raise DependencyNotInstalled('pygame is not installed, run `pip install "gymnasium[classic_control]"`') from e
        ...
        return (screen, clock), np.transpose(np.array(pygame.surfarray.pixels3d(screen)), axes=(1, 0, 2))
    ...
    def render_close(self, render_state: RenderStateType, params: CartPoleParams = CartPoleParams) -> None:
