...
ENV_ID_RE = re.compile(r"^(?:(?P<namespace>[\w:-]+)\/)?(?:(?P<name>[\w:.-]+?))(?:-v(?P<version>\d+))?$")
    ...
    def from_json(json_env_spec: str) -> EnvSpec:
        ...
        for wrapper_spec_json in parsed_env_spec.pop("additional_wrappers"):
            try:
                ...
                raise ValueError(f"An issue occurred when trying to make {wrapper_spec_json} a WrapperSpec") from e
...
def _check_namespace_exists(ns: str | None):
    ...
    namespaces: set[str] = {env_spec.namespace for env_spec in registry.values() if env_spec.namespace is not None}
    if ns in namespaces:
        return
...
def _check_version_exists(ns: str | None, name: str, version: int | None):
    ...
    env_specs = [env_spec for env_spec in registry.values() if env_spec.namespace == ns and env_spec.name == name]
...
def _check_spec_register(testing_spec: EnvSpec):
    ...
    unversioned_spec = next(
        (
            env_spec
            for env_spec in registry.values()
            if env_spec.namespace == testing_spec.namespace and env_spec.name == testing_spec.name and env_spec.version is None
        ),
        None,
    )
...
def _check_metadata(testing_metadata: dict[str, Any]):
    ...
    if render_modes is None:
        ...
        logger.warn(f"Expects the environment metadata render_modes to be a Iterable, actual type: {type(render_modes)}")
...
def _find_spec(env_id: str) -> EnvSpec:
    ...
    if version is not None and latest_version is not None and latest_version > version:
        logger.deprecation(
            f"The environment {env_name} is out of date. You should consider " f"upgrading to version `v{latest_version}`."
        )
    if version is None and latest_version is not None:
        ...
        logger.warn(
            f"Using the latest versioned environment `{new_env_id}` " f"instead of the unversioned environment `{env_name}`."
        )
...
def make(
    ...
    **kwargs: Any,
    ...
    try:
        ...
        if str(e).find("got an unexpected keyword argument 'render_mode'") >= 0 and apply_human_rendering:
            raise error.Error(
    ...
    if env_spec.additional_wrappers[:num_prior_wrappers] != env.spec.additional_wrappers:
        for env_spec_wrapper_spec, recreated_wrapper_spec in zip(env_spec.additional_wrappers, env.spec.additional_wrappers):
            raise ValueError(
    ...
    if disable_env_checker is False or (disable_env_checker is None and env_spec.disable_env_checker is False):
    ...
    **kwargs,
    ...
    if vectorization_mode == VectorizeMode.SYNC:
        ...
        if len(vector_kwargs) > 0:
            ...
            raise error.Error(f"Cannot use `vector_entry_point` vectorization mode with the wrappers argument ({wrappers}).")
        ...
        if env_spec.max_episode_steps is not None and "max_episode_steps" not in env_spec_kwargs:
    ...
    if "autoreset_mode" not in env.metadata:
        warn(f"The VectorEnv ({env}) is missing AutoresetMode metadata, metadata={env.metadata}")
