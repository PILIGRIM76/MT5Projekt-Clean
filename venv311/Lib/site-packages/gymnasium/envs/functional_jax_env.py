...
class FunctionalJaxVectorEnv(gym.vector.VectorEnv[ObsType, ActType, Any], Generic[ObsType, ActType, StateType]):
    ...
    def step(self, action: ActType):
        ...
        terminated = self.func_env.terminal(next_state, rng)
        truncated = self.steps >= self.time_limit if self.time_limit > 0 else jnp.zeros_like(terminated)
