...
class OneOf(Space[Any]):
    ...
    def seed(self, seed: int | tuple[int, ...] | None = None) -> tuple[int, ...]:
        ...
        * ``Tuple[int, ...]`` - Values used to seed the subspaces, first value seeds the OneOf and subsequent seed the subspaces. This allows the seeding of multiple composite subspaces ``[42, 54, ...]``.
        ...
        if seed is None:
            ...
            super().seed(seed)
            return (super_seed,) + tuple(space.seed(int(subseed)) for space, subseed in zip(self.spaces, subseeds))
            ...
            return (super().seed(seed[0]),) + tuple(space.seed(subseed) for space, subseed in zip(self.spaces, seed[1:]))
        else:
            raise TypeError(f"Expected None, int, or tuple of ints, actual type: {type(seed)}")
    ...
    def sample(
        ...
        if mask is not None and probability is not None:
            ...
            assert isinstance(mask, tuple), f"Expected type of `mask` is tuple, actual type: {type(mask)}"
            assert len(mask) == len(
                self.spaces
            ...
            assert isinstance(probability, tuple), f"Expected type of `probability` is tuple, actual type: {type(probability)}"
    ...
    def to_jsonable(self, sample_n: typing.Sequence[tuple[int, Any]]) -> list[list[Any]]:
        """Convert a batch of samples from this space to a JSONable data type."""
        return [[int(i), self.spaces[i].to_jsonable([subsample])[0]] for (i, subsample) in sample_n]
