...
class Graph(Space[GraphInstance]):
    ...
    def _generate_sample_space(self, base_space: None | Box | Discrete, num: int) -> Box | MultiDiscrete | None:
    ...
    def seed(self, seed: int | tuple[int, int] | tuple[int, int, int] | None = None) -> tuple[int, int] | tuple[int, int, int]:
        ...
        * ``Tuple[int, int, int]`` - Seeds the :class:`Graph`, node and edge subspaces with a particular value.
        ...
        if seed is None:
            ...
            if self.edge_space is None:
                ...
                super_seed = super().seed(seed)
                node_seed, edge_seed = self.np_random.integers(np.iinfo(np.int32).max, size=(2,))
                # this is necessary such that after int or list/tuple seeding, the Graph PRNG are equivalent
                super().seed(seed)
                return (
                    super_seed,
            ...
            if self.edge_space is None:
                if len(seed) != 2:
                    raise ValueError(f"Expects a tuple of two values for Graph and node space, actual length: {len(seed)}")
            ...
            raise TypeError(f"Expects `None`, int or tuple of ints, actual type: {type(seed)}")
    ...
    def sample(
        ...
        if num_edges is None:
            ...
            if self.edge_space is None:
                gym.logger.warn(f"The number of edges is set ({num_edges}) but the edge space is None.")
            assert num_edges >= 0, f"Expects the number of edges to be greater than 0, actual value: {num_edges}"
        ...
        if sampled_edges is not None and num_edges > 0:
            sampled_edge_links = self.np_random.integers(low=0, high=num_nodes, size=(num_edges, 2), dtype=np.int32)
    ...
    def __eq__(self, other: Any) -> bool:
        ...
        return isinstance(other, Graph) and (self.node_space == other.node_space) and (self.edge_space == other.edge_space)
    ...
    def from_jsonable(self, sample_n: Sequence[dict[str, list[list[int] | list[float]]]]) -> list[GraphInstance]:
