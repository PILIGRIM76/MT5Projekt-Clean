...
class Box(Space[NDArray[Any]]):
    ...
    * Independent bound for each dimension::
    ...
    def __init__(
        ...
        if not (np.issubdtype(self.dtype, np.integer) or np.issubdtype(self.dtype, np.floating) or self.dtype == np.bool_):
            raise ValueError(f"Invalid Box dtype ({self.dtype}), must be an integer, floating, or bool dtype")

        # determine shape
        if shape is not None:
            if not isinstance(shape, Iterable):
                raise TypeError(f"Expected Box shape to be an iterable, actual type={type(shape)}")
            ...
            if low.shape != high.shape:
                raise ValueError(f"Box low.shape and high.shape don't match, low.shape={low.shape}, high.shape={high.shape}")
            shape = low.shape
        ...
        if self.low.shape != shape:
            raise ValueError(f"Box low.shape doesn't match provided shape, low.shape={self.low.shape}, shape={self.shape}")
        if self.high.shape != shape:
            raise ValueError(f"Box high.shape doesn't match provided shape, high.shape={self.high.shape}, shape={self.shape}")
        ...
        if is_float_integer(low):
            ...
            if np.isnan(low):
                ...
                if self.dtype.kind == "i":  # signed int
                    ...
                    raise ValueError(f"Box unsigned int dtype don't support `-np.inf`, low={low}")
            elif low < dtype_min:
                raise ValueError(f"Box low is out of bounds of the dtype range, low={low}, min dtype={dtype_min}")

            low = np.full(self.shape, low, dtype=self.dtype)
            return low, bounded_below
        else:  # cast for low - array
            if not isinstance(low, np.ndarray):
                raise ValueError(f"Box low must be a np.ndarray, integer, or float, actual type={type(low)}")
            elif not (np.issubdtype(low.dtype, np.floating) or np.issubdtype(low.dtype, np.integer) or low.dtype == np.bool_):
                raise ValueError(f"Box low must be a floating, integer, or bool dtype, actual dtype={low.dtype}")
            ...
            if (
                ...
                gym.logger.warn(f"Box low's precision lowered by casting to {self.dtype}, current low.dtype={low.dtype}")
        ...
        if is_float_integer(high):
            ...
            if np.isnan(high):
                ...
                if self.dtype.kind == "i":  # signed int
                    ...
                    raise ValueError(f"Box unsigned int dtype don't support `np.inf`, high={high}")
            elif high > dtype_max:
                raise ValueError(f"Box high is out of bounds of the dtype range, high={high}, max dtype={dtype_max}")

            high = np.full(self.shape, high, dtype=self.dtype)
            return high, bounded_above
        else:
            if not isinstance(high, np.ndarray):
                raise ValueError(f"Box high must be a np.ndarray, integer, or float, actual type={type(high)}")
            elif not (
                np.issubdtype(high.dtype, np.floating) or np.issubdtype(high.dtype, np.integer) or high.dtype == np.bool_
            ):
                raise ValueError(f"Box high must be a floating or integer dtype, actual dtype={high.dtype}")
            ...
            if (
                ...
                gym.logger.warn(f"Box high's precision lowered by casting to {self.dtype}, current high.dtype={high.dtype}")
    ...
    def sample(self, mask: None = None, probability: None = None) -> NDArray[Any]:
        ...
        * :math:`(-\infty, \infty)` : normal distribution
        ...
        if mask is not None:
            ...
            raise gym.error.Error(f"Box.sample cannot be provided a probability mask, actual value: {probability}")
        ...
        sample[upp_bounded] = -self.np_random.exponential(size=upp_bounded[upp_bounded].shape) + high[upp_bounded]
    ...
    def contains(self, x: Any) -> bool:
        ...
        return bool(
            np.can_cast(x.dtype, self.dtype) and x.shape == self.shape and np.all(x >= self.low) and np.all(x <= self.high)
        )
