...
class Text(Space[str]):
    ...
    def __init__(
        ...
        *,
        ...
        assert 0 <= min_length, f"Minimum text length must be non-negative, actual value: {min_length}"
        assert (
        ...
        self._char_index: dict[str, np.int32] = {val: np.int32(i) for i, val in enumerate(tuple(charset))}
    ...
    def sample(
        ...
        if mask is not None and probability is not None:
            ...
            length, charlist_mask = self._validate_mask(probability, np.float64, "probability")
        ...
        string = self.np_random.choice(self.character_list, size=length, p=charlist_mask)
        return "".join(string)
    ...
    def _validate_mask(
        ...
        assert isinstance(mask, tuple), f"Expects the `{mask_type}` type to be a tuple, actual type: {type(mask)}"
        assert len(mask) == 2, f"Expects the `{mask_type}` length to be two, actual length: {len(mask)}"
