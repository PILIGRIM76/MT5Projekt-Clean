...
class Dict(Space[dict[str, Any]], typing.Mapping[str, Space[Any]]):
        ...
        **spaces_kwargs: Space,
        ...
        for key, space in spaces_kwargs.items():
            if key not in spaces:
                ...
                raise ValueError(f"Dict space keyword '{key}' already exists in the spaces dictionary.")
    ...
    def seed(self, seed: int | dict[str, Any] | None = None) -> dict[str, int]:
        ...
        * ``Dict`` - A dictionary of seeds for each subspace, requires a seed key for every subspace. This supports seeding of multiple composite subspaces (``Dict["space": Dict[...], ...]`` with ``{"space": {...}, ...}``).
        ...
        if seed is None:
            ...
            return {key: subspace.seed(int(subseed)) for (key, subspace), subseed in zip(self.spaces.items(), subseeds)}
        elif isinstance(seed, dict):
            if seed.keys() != self.spaces.keys():
                raise ValueError(f"The seed keys: {seed.keys()} are not identical to space keys: {self.spaces.keys()}")
    ...
    def sample(
        ...
        if mask is not None and probability is not None:
            ...
            assert isinstance(mask, dict), f"Expected sample mask to be a dict, actual type: {type(mask)}"
            assert (
            ...
            return {k: space.sample(probability=probability[k]) for k, space in self.spaces.items()}
        else:
    ...
    def from_jsonable(self, sample_n: dict[str, list[Any]]) -> list[dict[str, Any]]:
        ...
        dict_of_list: dict[str, list[Any]] = {key: space.from_jsonable(sample_n[key]) for key, space in self.spaces.items()}

        n_elements = len(next(iter(dict_of_list.values())))
        result = [{key: value[n] for key, value in dict_of_list.items()} for n in range(n_elements)]
        return result
