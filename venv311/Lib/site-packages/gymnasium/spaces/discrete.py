...
class Discrete(Space[IntType]):
    ...
    def __init__(
        ...
        """
        assert np.issubdtype(type(n), np.integer), f"Expects `n` to be an integer, actual dtype: {type(n)}"
        assert n > 0, "n (counts) have to be positive"
        assert np.issubdtype(type(start), np.integer), f"Expects `start` to be an integer, actual type: {type(start)}"
        ...
        if not (np.issubdtype(self.dtype, np.integer)):
            raise TypeError(f"Invalid Discrete dtype ({self.dtype}), must be an integer dtype")
    ...
    def sample(self, mask: MaskNDArray | None = None, probability: MaskNDArray | None = None) -> IntType:
        ...
        # binary mask sampling
            ...
            assert mask.dtype == np.int8, f"The expected dtype of the sample mask is np.int8, actual dtype: {mask.dtype}"
            assert mask.shape == (
                self.n,
            ...
            if np.any(valid_action_mask):
                return self.start + self.dtype.type(self.np_random.choice(np.where(valid_action_mask)[0]))
            else:
                return self.start
        # probability mask sampling
            ...
            return self.start + self.np_random.choice(np.arange(self.n, dtype=self.dtype), p=probability)
        # uniform sampling
        else:
    ...
    def __eq__(self, other: Any) -> bool:
        ...
        return isinstance(other, Discrete) and self.n == other.n and self.start == other.start and self.dtype == other.dtype
