...
class Tuple(Space[tuple[Any, ...]], typing.Sequence[Any]):
    ...
    def seed(self, seed: int | typing.Sequence[int] | None = None) -> tuple[int, ...]:
        ...
        * ``List`` / ``Tuple`` - Values used to seed the subspaces. This allows the seeding of multiple composite subspaces ``[42, 54, ...]``.
        ...
        if seed is None:
            ...
            return tuple(subspace.seed(int(subseed)) for subspace, subseed in zip(self.spaces, subseeds))
            ...
            return tuple(space.seed(subseed) for subseed, space in zip(seed, self.spaces))
        else:
            raise TypeError(f"Expected seed type: list, tuple, int or None, actual type: {type(seed)}")
    ...
    def sample(
        ...
        if mask is not None and probability is not None:
            ...
            assert isinstance(mask, tuple), f"Expected type of `mask` to be tuple, actual type: {type(mask)}"
            assert len(mask) == len(
                self.spaces
            ), f"Expected length of `mask` to be {len(self.spaces)}, actual length: {len(mask)}"

            return tuple(space.sample(mask=space_mask) for space, space_mask in zip(self.spaces, mask))
            ...
            return tuple(
                space.sample(probability=space_probability) for space, space_probability in zip(self.spaces, probability)
            )
        else:
