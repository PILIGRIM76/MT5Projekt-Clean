...
class MultiDiscrete(Space[NDArray[np.integer]]):
    ...
    def __init__(
        ...
        # determine dtype
        if dtype is None:
            raise ValueError("MultiDiscrete dtype must be explicitly provided, cannot be None.")
        self.dtype = np.dtype(dtype)

        #  * check that dtype is an accepted dtype
        if not (np.issubdtype(self.dtype, np.integer)):
            raise ValueError(f"Invalid MultiDiscrete dtype ({self.dtype}), must be an integer dtype")
    ...
    def _apply_mask(
        ...
        assert isinstance(sub_mask, np.ndarray), f"Expects the sub mask to be np.ndarray, actual type: {type(sub_mask)}"
        assert (
        ...
        if mask_type == "mask":
            assert sub_mask.dtype == np.int8, f"Expects the mask dtype to be np.int8, actual dtype: {sub_mask.dtype}"
            ...
            assert sub_mask.dtype == np.float64, f"Expects the mask dtype to be np.float64, actual dtype: {sub_mask.dtype}"
