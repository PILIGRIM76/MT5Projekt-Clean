...
class Sequence(Space[Union[tuple[Any, ...], Any]]):
    ...
    def __init__(
        ...
        """
        assert isinstance(space, Space), f"Expects the feature space to be instance of a gym Space, actual type: {type(space)}"
    ...
    def seed(self, seed: int | tuple[int, int] | None = None) -> tuple[int, int]:
        ...
        * ``Tuple of ints`` - A tuple for the :class:`Sequence` and feature space.
        ...
        if seed is None:
            ...
            raise TypeError(f"Expected None, int, tuple of ints, actual type: {type(seed)}")
    ...
    def sample(
        ...
        if mask is not None and probability is not None:
            ...
            sampled_values = tuple(self.feature_space.sample(mask=mask[1]) for _ in range(sample_length))
        elif probability is not None:
            sample_length = self.generate_sample_length(probability[0], "probability")
            sampled_values = tuple(self.feature_space.sample(probability=probability[1]) for _ in range(sample_length))
        else:
            sample_length = self.np_random.geometric(0.25)
            sampled_values = tuple(self.feature_space.sample() for _ in range(sample_length))
    ...
    def contains(self, x: Any) -> bool:
        ...
        if self.stack:
            return all(item in self.feature_space for item in gym.vector.utils.iterate(self.stacked_feature_space, x))
        else:
    ...
    def __eq__(self, other: Any) -> bool:
        ...
        return isinstance(other, Sequence) and self.feature_space == other.feature_space and self.stack == other.stack
