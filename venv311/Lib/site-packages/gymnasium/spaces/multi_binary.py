...
class MultiBinary(Space[NDArray[np.int8]]):
    ...
    def __init__(
        ...
        if isinstance(n, int):
            ...
            raise ValueError(f"Expected n to be an int or a sequence of ints, actual type: {type(n)}")
    ...
    def sample(self, mask: MaskNDArray | None = None, probability: MaskNDArray | None = None) -> NDArray[np.int8]:
        ...
        if mask is not None:
            ...
            return (self.np_random.random(size=self.shape) <= probability).astype(self.dtype)
        else:
    ...
    def contains(self, x: Any) -> bool:
        ...
        return bool(isinstance(x, np.ndarray) and self.shape == x.shape and np.all(np.logical_or(x == 0, x == 1)))
