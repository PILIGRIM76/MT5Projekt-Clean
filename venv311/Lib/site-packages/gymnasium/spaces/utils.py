...
def _flatten_oneof(space: OneOf, x: tuple[int, Any]) -> NDArray[Any]:
    ...
    if flat_sample.size < max_flatdim:
        padding = np.full(max_flatdim - flat_sample.size, flat_sample[0], dtype=flat_sample.dtype)
...
def _unflatten_multidiscrete(space: MultiDiscrete, x: NDArray[np.integer[Any]]) -> NDArray[np.integer[Any]]:
...
def _flatten_space_dict(space: Dict) -> Box | Dict:
    ...
    return Dict(spaces={key: flatten_space(space) for key, space in space.spaces.items()})
...
def is_space_dtype_shape_equiv(space_1: Space, space_2: Space) -> bool:
    ...
    if isinstance(space_1, Space) and isinstance(space_2, Space):
        raise NotImplementedError("`check_dtype_shape_equivalence` doesn't support Generic Gymnasium Spaces, ")
    else:
        raise TypeError()
...
def _is_space_dict_dtype_shape_equiv(space_1: Dict, space_2):
    return (
        ...
        and all(is_space_dtype_shape_equiv(space_1[key], space_2[key]) for key in space_1.keys())
    )


@is_space_dtype_shape_equiv.register(Tuple)
def _is_space_tuple_dtype_shape_equiv(space_1, space_2):
    return isinstance(space_2, Tuple) and all(is_space_dtype_shape_equiv(space_1[i], space_2[i]) for i in range(len(space_1)))
...
def _is_space_oneof_dtype_shape_equiv(space_1: OneOf, space_2):
    return (
        ...
        and all(is_space_dtype_shape_equiv(space_1[i], space_2[i]) for i in range(len(space_1)))
    )
