...
def check_space(space: Space, space_type: str, check_box_space_fn: Callable[[spaces.Box], None]):
    ...
    if not isinstance(space, spaces.Space):
        if str(space.__class__.__base__) == "<class 'gym.spaces.space.Space'>":
            ...
            raise TypeError(f"{space_type} space does not inherit from `gymnasium.spaces.Space`, actual type: {type(space)}")
...
def _check_render_return(render_mode, render_return):
    ...
    if render_mode == "human":
        ...
        if not isinstance(render_return, np.ndarray):
            logger.warn(f"RGB-array rendering should return a numpy array, got {type(render_return)}")
        else:
            if render_return.dtype != np.uint8:
                logger.warn(f"RGB-array rendering should return a numpy array with dtype uint8, got {render_return.dtype}")
            if render_return.ndim != 3:
                logger.warn(f"RGB-array rendering should return a numpy array with three axes, got {render_return.ndim}")
        ...
        if not isinstance(render_return, np.ndarray):
            logger.warn(f"Depth-array rendering should return a numpy array, got {type(render_return)}")
        elif render_return.ndim != 2:
            logger.warn(f"Depth-array rendering should return a numpy array with two axes, got {render_return.ndim}")
        ...
        if not isinstance(render_return, list):
            logger.warn(f"Render mode `{render_mode}` should produce a list, got {type(render_return)}")
        else:
            base_render_mode = render_mode[: -len("_list")]
            for item in render_return:
                _check_render_return(base_render_mode, item)  # Check that each item of the list matches the base render mode
    ...
    if render_modes is None:
        ...
        if len(render_modes) > 0:
            if render_fps is None:
                ...
                if not (np.issubdtype(type(render_fps), np.integer) or np.issubdtype(type(render_fps), np.floating)):
                    logger.warn(
