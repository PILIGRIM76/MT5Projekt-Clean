...
def check_environments_match(
    ...
    """
    skip_render = skip_render or env_a.unwrapped.render_mode in [None, "human"] or env_b.unwrapped.render in [None, "human"]
    ...
    if not skip_render:
        assert (env_a.render() == env_b.render()).all(), "resetting render is not equivalent"
    ...
    for step in range(num_steps):
        ...
        if not skip_render:
            assert (env_a.render() == env_b.render()).all(), "stepping render is not equivalent in step = {step}"
        ...
        if terminal_a or truncated_a or terminal_b or truncated_b:
            ...
            if not skip_render:
                assert (env_a.render() == env_b.render()).all(), "resetting render is not equivalent in step = {step}"
