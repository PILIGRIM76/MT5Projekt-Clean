...
try:
    ...
    raise gym.error.DependencyNotInstalled('pygame is not installed, run `pip install "gymnasium[classic_control]"`') from e
...
class PlayableGame:
    ...
    def _get_relevant_keys(self, keys_to_action: dict[tuple[int], int] | None = None) -> set:
...
def play(
    ...
    if keys_to_action is None:
        if env.has_wrapper_attr("get_keys_to_action"):
            ...
            raise MissingKeysToAction(
                f"{env.spec.id} does not have explicit key to action mapping, " "please specify one manually"
            )
...
class PlayPlot:
    ...
    def __init__(self, callback: Callable, horizon_timesteps: int, plot_names: list[str]):
        ...
        if plt is None:
            raise DependencyNotInstalled('matplotlib is not installed, run `pip install "gymnasium[other]"`')
