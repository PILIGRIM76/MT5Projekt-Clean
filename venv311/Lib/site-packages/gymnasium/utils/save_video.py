...
try:
    ...
    raise gym.error.DependencyNotInstalled('moviepy is not installed, run `pip install "gymnasium[other]"`') from e
...
def save_video(
    ...
    **kwargs,
    ...
    if episode_trigger is not None and episode_trigger(episode_index):
        ...
        clip.write_videofile(f"{path_prefix}-episode-{episode_index}.mp4", logger=save_logger)
    ...
    if step_trigger is not None:
        ...
        for step_index, frame_index in enumerate(range(1, len(frames)), start=step_starting_index):
            if step_trigger(step_index):
                ...
                clip.write_videofile(f"{path_prefix}-step-{step_index}.mp4", logger=save_logger)
