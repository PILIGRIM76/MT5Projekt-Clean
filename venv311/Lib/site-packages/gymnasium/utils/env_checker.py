...
def data_equivalence(data_1, data_2, exact: bool = False) -> bool:
    ...
    if type(data_1) is not type(data_2):
        ...
        return data_1.keys() == data_2.keys() and all(data_equivalence(data_1[k], data_2[k], exact) for k in data_1.keys())
    elif isinstance(data_1, (tuple, list)):
        return len(data_1) == len(data_2) and all(data_equivalence(o_1, o_2, exact) for o_1, o_2 in zip(data_1, data_2))
...
def check_reset_seed_determinism(env: gym.Env):
    ...
    if "seed" in signature.parameters or (
        "kwargs" in signature.parameters and signature.parameters["kwargs"].kind is inspect.Parameter.VAR_KEYWORD
    ):
        try:
            ...
            if env.spec is not None and env.spec.nondeterministic is False:
                ...
                if not data_equivalence(obs_1, obs_3, exact=True):
                    logger.warn("Using `env.reset(seed=123)` observations are not equal although similar.")
                if not data_equivalence(obs_2, obs_4, exact=True):
                    logger.warn("Using `env.reset(seed=123)` then `env.reset()` observations are not equal although similar.")
            ...
            assert (
                env.unwrapped._np_random.bit_generator.state != seed_123_rng_1.bit_generator.state
...
def check_step_determinism(env: gym.Env, seed=123):
    ...
    assert data_equivalence(obs_0, obs_1), "Deterministic step observations are not equivalent for the same seed and action"
    if not data_equivalence(obs_0, obs_1, exact=True):
        logger.warn("Step observations are not equal although similar given the same seed and action")

    assert data_equivalence(rew_0, rew_1), "Deterministic step rewards are not equivalent for the same seed and action"
    if not data_equivalence(rew_0, rew_1, exact=True):
        logger.warn("Step rewards are not equal although similar given the same seed and action")

    assert data_equivalence(
        term_0, term_1, exact=True
    ), "Deterministic step termination are not equivalent for the same seed and action"
    assert trunc_0 is False and trunc_1 is False, "Environment truncates after 1 step, something has gone very wrong."
    ...
    if not data_equivalence(info_0, info_1, exact=True):
        logger.warn("Step info are not equal although similar given the same seed and action")
...
def check_reset_return_type(env: gym.Env):
    ...
    assert len(result) == 2, f"Calling the reset method did not return a 2-tuple, actual length: {len(result)}"

    obs, info = result
    assert obs in env.observation_space, "The first element returned by `env.reset()` is not within the observation space."
    assert isinstance(
        info, dict
