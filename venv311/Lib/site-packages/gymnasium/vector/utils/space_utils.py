...
def batch_space(space: Space[Any], n: int = 1) -> Space[Any]:
    ...
    """
    raise TypeError(f"The space provided to `batch_space` is not a gymnasium Space instance, type: {type(space)}, {space}")
...
def _batch_differing_spaces_tuple(spaces: list[Tuple]):
    return Tuple(
        tuple(batch_differing_spaces(subspaces) for subspaces in zip(*[space.spaces for space in spaces])),
        seed=deepcopy(spaces[0].np_random),
    )
...
def _batch_differing_spaces_dict(spaces: list[Dict]):
    ...
    return Dict(
        {key: batch_differing_spaces([space[key] for space in spaces]) for key in spaces[0].keys()},
        seed=deepcopy(spaces[0].np_random),
    )
...
def _batch_spaces_undefined(spaces: list[Graph | Text | Sequence | OneOf]):
    return Tuple([deepcopy(space) for space in spaces], seed=deepcopy(spaces[0].np_random))
...
def iterate(space: Space[T_cov], items: T_cov) -> Iterator:
    ...
    if isinstance(space, Space):
        ...
        raise TypeError(f"The space provided to `iterate` is not a gymnasium Space instance, type: {type(space)}, {space}")
...
def _iterate_tuple(space: Tuple, items: tuple[Any, ...]):
    ...
    try:
        ...
        unregistered_spaces = [type(subspace) for subspace in space if type(subspace) not in iterate.registry]
        raise CustomSpaceError(
...
def concatenate(
    ...
    """
    raise TypeError(f"The space provided to `concatenate` is not a gymnasium Space instance, type: {type(space)}, {space}")
...
def _concatenate_dict(space: Dict, items: Iterable, out: dict[str, Any]) -> dict[str, Any]:
    return {key: concatenate(subspace, [item[key] for item in items], out[key]) for key, subspace in space.items()}
...
@singledispatch
def create_empty_array(space: Space, n: int = 1, fn: Callable = np.zeros) -> tuple[Any, ...] | dict[str, Any] | np.ndarray:
...
def _create_empty_array_dict(space: Dict, n: int = 1, fn=np.zeros) -> dict[str, Any]:
    return {key: create_empty_array(subspace, n=n, fn=fn) for key, subspace in space.items()}
