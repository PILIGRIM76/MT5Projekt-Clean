...
@singledispatch
def create_shared_memory(space: Space[Any], n: int = 1, ctx=mp) -> dict[str, Any] | tuple[Any, ...] | SynchronizedArray:
...
def _create_base_shared_memory(space: Box | Discrete | MultiDiscrete | MultiBinary, n: int = 1, ctx=mp):
...
def _create_dict_shared_memory(space: Dict, n: int = 1, ctx=mp):
    return {key: create_shared_memory(subspace, n=n, ctx=ctx) for (key, subspace) in space.spaces.items()}
...
def _read_base_from_shared_memory(space: Box | Discrete | MultiDiscrete | MultiBinary, shared_memory, n: int = 1):
...
def _read_dict_from_shared_memory(space: Dict, shared_memory, n: int = 1):
    return {key: read_from_shared_memory(subspace, shared_memory[key], n=n) for (key, subspace) in space.spaces.items()}
...
def _read_text_from_shared_memory(space: Text, shared_memory, n: int = 1) -> tuple[str, ...]:
    ...
    return tuple("".join([space.character_list[val] for val in values if val < len(space.character_set)]) for values in data)


@read_from_shared_memory.register(OneOf)
def _read_one_of_from_shared_memory(space: OneOf, shared_memory, n: int = 1) -> tuple[Any, ...]:
    sample_indexes = np.frombuffer(shared_memory[0].get_obj(), dtype=np.int64)

    subspace_samples = tuple(
        read_from_shared_memory(subspace, memory, n=n) for (memory, subspace) in zip(shared_memory[1:], space.spaces)
    )
    return tuple((sample_index, subspace_samples[sample_index][index]) for index, sample_index in enumerate(sample_indexes))
...
def _write_oneof_to_shared_memory(space: OneOf, index: int, values: tuple[int, Any], shared_memory):
    ...
    write_to_shared_memory(space.spaces[subspace_idx], index, space_value, shared_memory[1 + subspace_idx])
