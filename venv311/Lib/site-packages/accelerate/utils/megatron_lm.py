...
class MegatronLMDummyDataLoader:
    ...
    def set_megatron_data_args(self):
        ...
        for key, value in self.dataset_args.items():
            ...
            if old_value != value:
                print(f"WARNING: MegatronLMDummyDataLoader overriding arguments for {key}:{old_value} with {key}:{value}")
...
class MegatronEngine(torch.nn.Module):
    ...
    def get_batch_data_iterator(self, batch_data):
        ...
        if len(self.module) > 1:
            batch_data_iterator = (
                [iter(data_chunks) for _ in range(len(self.module))] if len(batch_data) > 0 else [None] * len(self.module)
            )
        else:
