...
def get_balanced_memory(
    ...
    gpus_idx_list = list(
        sorted(device_id for device_id, device_mem in max_memory.items() if isinstance(device_id, int) and device_mem > 0)
    )
...
def calculate_maximum_sizes(model: torch.nn.Module):
    ...
    modules_to_treat = (
        list(model.named_parameters(recurse=False)) + list(model.named_children()) + list(model.named_buffers(recurse=False))
    )
...
def fallback_allocate(
    ...
    while modules_to_search:
        ...
        modules_children = (
            [] if isinstance(module, nn.Parameter) or isinstance(module, torch.Tensor) else list(module.named_children())
        )
    ...
    for dot_index in dot_idx:
        ...
        if parent_name in current_names:
            ...
            module_children = list(parent_module.named_parameters(recurse=False)) + list(parent_module.named_children())
            modules = (
...
def check_device_map(model: nn.Module, device_map: dict[str, Union[int, str, torch.device]]):
    ...
    if invalid_keys:
        warnings.warn(f"The following device_map keys do not match any submodules in the model: {invalid_keys}", UserWarning)
