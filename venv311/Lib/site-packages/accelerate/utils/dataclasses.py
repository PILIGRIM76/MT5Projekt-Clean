...
class TorchDynamoPlugin(KwargsHandler):
    ...
    def __post_init__(self):
        ...
        if self.use_regional_compilation is None:
            self.use_regional_compilation = str_to_bool(os.environ.get(prefix + "USE_REGIONAL_COMPILATION", "False")) == 1
            ...
            config = {
                ...
                "zero_optimization": {
                    ...
                    "offload_optimizer": {
                        ...
                        "nvme_path": (self.offload_optimizer_nvme_path if self.offload_optimizer_device == "nvme" else None),
                    },
                    "offload_param": {
...
class FullyShardedDataParallelPlugin:
    ...
    auto_wrap_policy: Optional[Union[Callable, Literal["transformer_based_wrap", "size_based_wrap", "no_wrap"]]] = field(
        default=None,
        metadata={
    ...
    optim_state_dict_config: Optional[
        ...
        default=None,
        metadata={"help": "Optim state dict config to use. Is determined based on the `state_dict_type` if not passed in."},
    )
    ...
    def __post_init__(self):
        ...
        if isinstance(self.auto_wrap_policy, str):
            if self.auto_wrap_policy.upper() not in FSDP_AUTO_WRAP_POLICY:
                raise ValueError(f"Invalid auto wrap policy: {self.auto_wrap_policy}. Must be one of {FSDP_AUTO_WRAP_POLICY}")
    ...
    def set_mixed_precision(self, mixed_precision, buffer_autocast=False, override=False):
        ...
        if override or self.mixed_precision_policy is None:
            ...
            valid_keys = ["param_dtype", "reduce_dtype"] + (["buffer_dtype"] if self.fsdp_version == 1 else ["output_dtype"])
...
class TorchContextParallelConfig:
    ...
    def __post_init__(self):
        ...
        if self.cp_comm_strategy not in ["allgather", "alltoall"]:
            raise ValueError(f"Invalid cp_comm_strategy: {self.cp_comm_strategy}. Must be one of 'allgather' or 'alltoall'.")
...
class MegatronLMPlugin:
    ...
    consumed_samples: list[int] = field(
        default=None,
        metadata={"help": "Number of samples consumed in the same order as the dataloaders to `accelerator.prepare` call."},
    )
...
class BnbQuantizationConfig:
    ...
    bnb_4bit_quant_type: str = field(
        default="fp4",
        metadata={"help": "set the quantization data type in the `bnb.nn.Linear4Bit` layers. Options are {'fp4','nf4'}."},
    )
