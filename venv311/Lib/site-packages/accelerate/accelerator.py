...
class Accelerator:
    ...
    **Available attributes:**
    ...
    def accumulate(self, *models):
        ...
        allow_gradient_sync = self.sync_gradients or (  # must sync if sync gradients need to complete an optimizer step
    ...
    def _prepare_ao(self, *args):
        ...
        if self.is_fsdp2 and len(optimizers) > 0 and self.ao_recipe_handler.config.enable_fsdp_float8_all_gather:
            ...
            optimizers[0].register_step_post_hook(lambda *args, **kwargs: precompute_float8_dynamic_scale_for_fsdp(models[0]))
    ...
    def _prepare_deepspeed(self, *args):
        ...
        result = [
            self._prepare_one(obj, first_pass=True) if isinstance(obj, torch.utils.data.DataLoader) else obj for obj in args
        ]
        ...
        for obj in args:
            if isinstance(obj, torch.nn.Module):
                ...
                named_parameters.update(
                    {
                        n: getattr(p, accessor_mapping[type(p)]).data_ptr() if type(p) in accessor_mapping else p.data_ptr()
        ...
        if self.distributed_type == DistributedType.DEEPSPEED:
            ...
            if zero3_sharding or tp_sharding:
                if model.zero_gather_16bit_weights_on_model_save():
                    ...
                    state_dict = (
                        model._consolidated_16bit_state_dict() if tp_sharding else model._zero3_consolidated_16bit_state_dict()
                    )
                else:
                    raise ValueError(
