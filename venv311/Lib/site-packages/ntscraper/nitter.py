...
class Nitter:
    ...
    def _is_instance_encrypted(self):
        ...
        if (
            ...
            and "/enc/" in soup.find("a", class_="profile-card-avatar").find("img")["src"]
        ):
            return True
        else:
            return False
    ...
    def _check_error_page(self, soup):
        ...
        if not soup.find(
            ...
            if soup.find("div", class_="error-panel"):
                message = f"Fetching error: " + soup.find("div", class_="error-panel").find("span").text.strip()
            else:
                ...
                if self.retry_count == max_retries // 2:
                    if not self.skip_instance_check:
                        ...
                        if not self.working_instances:
                            ...
                            return None
                if not self.skip_instance_check:
                    self._initialize_session(instance=self._get_new_instance(f"{self.instance} unreachable"))
                self.retry_count += 1
                self.cooldown_count = 0
                    ...
                    if self.retry_count == max_retries // 2:
                        ...
                        if "cursor" in endpoint:
                            ...
                            if self.cooldown_count >= 5 and not self.session_reset:
                                ...
                                if not self.skip_instance_check:
                                    self._initialize_session(self._get_new_instance(f"Error fetching {self.instance}"))
                        else:
                            self.cooldown_count = 0
                            if not self.skip_instance_check:
                                self._initialize_session(self._get_new_instance(f"Error fetching {self.instance}"))
        ...
        if quoted_tweet.find("div", class_="attachments"):
            if is_encrypted:
                quoted_pictures = [
                    ...
                    for img in quoted_tweet.find("div", class_="attachments").find_all("img")
                ]
                quoted_videos = [
                    (
                    ...
                    for video in quoted_tweet.find("div", class_="attachments").find_all("video", class_="")
                ]
                quoted_gifs = [
                    "https://" + b64decode(gif.source["src"].split("/")[-1].encode("utf-8")).decode("utf-8")
                    for gif in quoted_tweet.find("div", class_="attachments").find_all("video", class_="gif")
                ]
            else:
                quoted_pictures = [
                    "https://pbs.twimg.com" + unquote(img["src"].split("/pic")[1]).split("?")[0]
                    for img in quoted_tweet.find("div", class_="attachments").find_all("img")
                ]
                quoted_videos = [
                    (
    ...
    def _get_tweet_stats(self, tweet):
        ...
        """
        return {
            "comments": int(tweet.find_all("span", class_="tweet-stat")[0].find("div").text.strip().replace(",", "") or 0),
            "retweets": int(tweet.find_all("span", class_="tweet-stat")[1].find("div").text.strip().replace(",", "") or 0),
            "quotes": int(tweet.find_all("span", class_="tweet-stat")[2].find("div").text.strip().replace(",", "") or 0),
            "likes": int(tweet.find_all("span", class_="tweet-stat")[3].find("div").text.strip().replace(",", "") or 0),
        }
    ...
    def _get_tweet_date(self, tweet):
        ...
        """
        return (
            tweet.find("span", class_="tweet-date").find("a")["title"].split("/")[-1].split("#")[0]
    ...
    def _get_tweet_text(self, tweet):
        ...
        """
        return (
            tweet.find("div", class_="tweet-content media-body").text.strip().replace("\n", " ")
            if tweet.find("div", class_="tweet-content media-body")
            else (
                tweet.find("div", class_="quote-text").text.strip().replace("\n", " ")
    ...
    def _get_external_link(self, tweet):
        ...
        """
        return tweet.find("a", class_="card-container")["href"] if tweet.find("a", class_="card-container") else ""
    ...
    def _get_replied_to(self, tweet):
        ...
        """
        return (
            [user.text.strip() for user in tweet.find("div", class_="replying-to").find_all("a")]
    ...
    def _extract_tweet(self, tweet, is_encrypted):
        ...
        quoted_tweet = tweet.find("div", class_="quote") if tweet.find("div", class_="quote") else None
            ...
            "quoted-post": (
                {
                    ...
                    "user": self._get_user(quoted_tweet, is_encrypted) if not deleted else {},
    ...
    def _search(
        ...
        if filters:
            for f in filters:
                if f not in valid_filters:
                    raise ValueError(f"Invalid filter '{f}'. Valid filters are: {', '.join(valid_filters)}")
                endpoint += f"&f-{f}=on"

        if exclude:
            for e in exclude:
                if e not in valid_filters:
                    raise ValueError(f"Invalid exclusion filter '{e}'. Valid filters are: {', '.join(valid_filters)}")
                endpoint += f"&e-{e}=on"
        ...
        while keep_scraping:
            ...
            logging.info(f"Current stats for {term}: {len(tweets['tweets'])} tweets, {len(tweets['threads'])} threads...")
            if not (since and until) and not (since) and len(tweets["tweets"]) + len(tweets["threads"]) >= number:
                ...
                if soup.find_all("div", class_="show-more"):
                    if mode == "user":
                        if since or until:
                            next_page = f"/{term}/search?" + show_more_buttons[-1].find("a")["href"].split("?")[-1]
                        else:
                            next_page = f"/{term}?" + show_more_buttons[-1].find("a")["href"].split("?")[-1]
                    else:
    ...
    def get_tweets(
        ...
        if type(terms) == str:
            ...
            if len(terms) > cpu_count():
                raise ValueError(f"Too many terms. You can search at most {cpu_count()} terms.")
    ...
    def _profile_info(self, username, max_retries, instance):
        ...
        if soup.find("div", class_="profile-banner").find("img") and is_encrypted:
            profile_id = (
                b64decode(soup.find("div", class_="profile-banner").find("img")["src"].split("/enc/")[1].encode("utf-8"))
                .decode("utf-8")
        ...
        if soup.find("a", class_="profile-card-avatar").find("img") and is_encrypted:
            profile_image = "https://" + b64decode(
                soup.find("a", class_="profile-card-avatar").find("img")["src"].split("/enc/")[1].encode("utf-8")
            ).decode("utf-8")
        elif soup.find("a", class_="profile-card-avatar").find("img"):
            profile_image = (
                "https://" + unquote(soup.find("a", class_="profile-card-avatar").find("img")["src"]).split("/pic/")[1]
            )
        else:
            profile_image = ""

        icon_container = (
            soup.find("div", class_="photo-rail-header").find("div", class_="icon-container")
        ...
        return {
            ...
            "id": profile_id,
            "bio": soup.find("div", class_="profile-bio").p.text.strip() if soup.find("div", class_="profile-bio") else "",
            "location": (
                soup.find("div", class_="profile-location").find_all("span")[-1].text.strip()
                if soup.find("div", class_="profile-location")
                else ""
            ),
            "website": (
                soup.find("div", class_="profile-website").find("a")["href"]
            ...
            "stats": {
                ...
                "media": int(icon_container.text.strip().replace(",", "").split(" ")[0] if icon_container else 0),
            },
        }
