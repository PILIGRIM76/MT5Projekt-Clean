...
class _HiredisParser(BaseParser, PushNotificationsParser):
    ...
    def read_response(self, disable_decoding=False, push_request=False):
        ...
        if self._next_response is not NOT_ENOUGH_DATA:
            ...
            self._next_response = NOT_ENOUGH_DATA
            if self._hiredis_PushNotificationType is not None and isinstance(response, self._hiredis_PushNotificationType):
        ...
        if isinstance(response, ConnectionError):
            raise response
        elif self._hiredis_PushNotificationType is not None and isinstance(response, self._hiredis_PushNotificationType):
            response = self.handle_push_response(response)
            if push_request:
                return response
            return self.read_response(
