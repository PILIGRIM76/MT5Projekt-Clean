...
MSG_TYPE_TO_MAINT_NOTIFICATION_PARSER_MAPPING: dict[str, tuple[type[MaintenanceNotification], Callable]] = {
    _MIGRATING_MESSAGE: (
        NodeMigratingNotification,
...
class PushNotificationsParser(Protocol):
    ...
    def handle_push_response(self, response, **kwargs):
        ...
        try:
            if msg_type == _INVALIDATION_MESSAGE and self.invalidation_push_handler_func:
                return self.invalidation_push_handler_func(response)

            if msg_type == _MOVING_MESSAGE and self.node_moving_push_handler_func:
                parser_function = MSG_TYPE_TO_MAINT_NOTIFICATION_PARSER_MAPPING[msg_type][1]
            ...
            if msg_type in _MAINTENANCE_MESSAGES and self.maintenance_push_handler_func:
                ...
                if msg_type == _SMIGRATING_MESSAGE:
                    ...
                    notification_type = MSG_TYPE_TO_MAINT_NOTIFICATION_PARSER_MAPPING[msg_type][0]
            ...
            if msg_type == _SMIGRATED_MESSAGE and (
                self.oss_cluster_maint_push_handler_func or self.maintenance_push_handler_func
            ):
                parser_function = MSG_TYPE_TO_MAINT_NOTIFICATION_PARSER_MAPPING[msg_type][1]
                notification = parser_function(response)
...
class AsyncPushNotificationsParser(Protocol):
    ...
    async def handle_push_response(self, response, **kwargs):
        ...
        try:
            ...
            if msg_type == _SMIGRATED_MESSAGE and self.oss_cluster_maint_push_handler_func:
                parser_function = MSG_TYPE_TO_MAINT_NOTIFICATION_PARSER_MAPPING[msg_type][1]
                notification = parser_function(response)
                if notification is not None:
