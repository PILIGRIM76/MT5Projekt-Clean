...
class MaintNotificationsAbstractConnection:
    ...
    def __init__(
        ...
        maint_notifications_pool_handler: Optional[MaintNotificationsPoolHandler] = None,
        ...
        oss_cluster_maint_notifications_handler: Optional[OSSMaintNotificationsHandler] = None,
    ...
    def _configure_maintenance_notifications(
        ...
        """
        if not self.maint_notifications_config or not self.maint_notifications_config.enabled:
        ...
        if not parser:
            raise RedisError("To configure maintenance notifications, a parser must be provided!")

        if not isinstance(parser, _HiredisParser) and not isinstance(parser, _RESP3Parser):
            raise RedisError("Maintenance notifications are only supported with hiredis and RESP3 parsers!")
        ...
        if maint_notifications_pool_handler:
            ...
            self._maint_notifications_pool_handler = maint_notifications_pool_handler.get_handler_for_connection()
            self._maint_notifications_pool_handler.set_connection(self)
        else:
            self._maint_notifications_pool_handler = None

        self._maint_notifications_connection_handler = MaintNotificationsConnectionHandler(
            self, self.maint_notifications_config
        )

        if oss_cluster_maint_notifications_handler:
            self._oss_cluster_maint_notifications_handler = oss_cluster_maint_notifications_handler
        else:
            self._oss_cluster_maint_notifications_handler = None

        # Set up OSS cluster handler to parser if available
        if self._oss_cluster_maint_notifications_handler:
            parser.set_oss_cluster_maint_push_handler(self._oss_cluster_maint_notifications_handler.handle_notification)

        # Set up pool handler to parser if available
        if self._maint_notifications_pool_handler:
            parser.set_node_moving_push_handler(self._maint_notifications_pool_handler.handle_notification)

        # Set up connection handler
        parser.set_maintenance_push_handler(self._maint_notifications_connection_handler.handle_notification)

        # Store original connection parameters
        self.orig_host_address = orig_host_address if orig_host_address else self.host
        self.orig_socket_timeout = orig_socket_timeout if orig_socket_timeout else self.socket_timeout
        self.orig_socket_connect_timeout = (
            orig_socket_connect_timeout if orig_socket_connect_timeout else self.socket_connect_timeout
        )
    ...
    def set_maint_notifications_pool_handler_for_connection(
        ...
        maint_notifications_pool_handler_copy = maint_notifications_pool_handler.get_handler_for_connection()

        maint_notifications_pool_handler_copy.set_connection(self)
        self._get_parser().set_node_moving_push_handler(maint_notifications_pool_handler_copy.handle_notification)
        ...
        if not self._maint_notifications_connection_handler:
            ...
            self._get_parser().set_maintenance_push_handler(self._maint_notifications_connection_handler.handle_notification)
        else:
            self._maint_notifications_connection_handler.config = maint_notifications_pool_handler.config
    ...
    def set_maint_notifications_cluster_handler_for_connection(
        ...
        self._get_parser().set_oss_cluster_maint_push_handler(oss_cluster_maint_notifications_handler.handle_notification)
        ...
        if not self._maint_notifications_connection_handler:
            ...
            self._maint_notifications_connection_handler.config = oss_cluster_maint_notifications_handler.config
    ...
    def _enable_maintenance_notifications(self, maint_notifications_config: MaintNotificationsConfig, check_health=True):
        try:
            ...
            if host is None:
                ...
                if not response or str_if_bytes(response) != "OK":
                    raise ResponseError("The server doesn't support maintenance notifications")
        except Exception as e:
            if isinstance(e, ResponseError) and maint_notifications_config.enabled == "auto":
...
class AbstractConnection(MaintNotificationsAbstractConnection, ConnectionInterface):
    ...
    @deprecated_args(
        ...
        reason="Use 'driver_info' parameter instead. " "lib_name and lib_version will be removed in a future version.",
    )
    def __init__(
        self,
        db: int = 0,
...
class CacheProxyConnection(MaintNotificationsAbstractConnection, ConnectionInterface):
    ...
    def set_maint_notifications_pool_handler_for_connection(self, maint_notifications_pool_handler):
        if isinstance(self._conn, MaintNotificationsAbstractConnection):
            self._conn.set_maint_notifications_pool_handler_for_connection(maint_notifications_pool_handler)

    def set_maint_notifications_cluster_handler_for_connection(self, oss_cluster_maint_notifications_handler):
        if isinstance(self._conn, MaintNotificationsAbstractConnection):
            self._conn.set_maint_notifications_cluster_handler_for_connection(oss_cluster_maint_notifications_handler)
    ...
    def send_command(self, *args, **kwargs):
        ...
        # Creates cache key.
        self._current_command_cache_key = CacheKey(command=args[0], redis_keys=tuple(kwargs.get("keys")), redis_args=args)
    ...
    def _maint_notifications_connection_handler(self, value: Optional[MaintNotificationsConnectionHandler]):
    ...
    def _get_socket(self) -> Optional[socket.socket]:
        if isinstance(self._conn, MaintNotificationsAbstractConnection):
            ...
            raise NotImplementedError("Maintenance notifications are not supported by this connection type")

    def _get_maint_notifications_connection_instance(self, connection) -> MaintNotificationsAbstractConnection:
        """
...
class ConnectionPoolInterface(ABC):
    ...
    def get_connection(self, command_name: Optional[str], *keys, **options) -> ConnectionInterface:
        pass
...
class MaintNotificationsAbstractConnectionPool:
        ...
        **kwargs,
        ...
        if maint_notifications_config and maint_notifications_config.enabled:
            if not is_protocol_supported:
                raise RedisError("Maintenance notifications handlers on connection are only supported with RESP version 3")

            self._event_dispatcher = kwargs.get("event_dispatcher", None)
            if self._event_dispatcher is None:
                self._event_dispatcher = EventDispatcher()

            self._maint_notifications_pool_handler = MaintNotificationsPoolHandler(self, maint_notifications_config)
            if oss_cluster_maint_notifications_handler:
                self._oss_cluster_maint_notifications_handler = oss_cluster_maint_notifications_handler
    ...
    def maint_notifications_enabled(self):
        ...
        """
        if self._oss_cluster_maint_notifications_handler:
            maint_notifications_config = self._oss_cluster_maint_notifications_handler.config
        else:
            maint_notifications_config = (
                self._maint_notifications_pool_handler.config if self._maint_notifications_pool_handler else None
            )
    ...
    def update_maint_notifications_config(
        ...
        """
        if self.maint_notifications_enabled() and not maint_notifications_config.enabled:
            raise ValueError("Cannot disable maintenance notifications after enabling them")
        if oss_cluster_maint_notifications_handler:
            self._oss_cluster_maint_notifications_handler = oss_cluster_maint_notifications_handler
        else:
            ...
            self.connection_kwargs.update(
                {
                    ...
                    "orig_socket_timeout": self.connection_kwargs.get("socket_timeout", None),
                    "orig_socket_connect_timeout": self.connection_kwargs.get("socket_connect_timeout", None),
                }
            )
        ...
        with self._get_pool_lock():
            for conn in self._get_free_connections():
                if oss_cluster_maint_notifications_handler:
                    ...
                    conn.set_maint_notifications_cluster_handler_for_connection(oss_cluster_maint_notifications_handler)
                    conn.maint_notifications_config = oss_cluster_maint_notifications_handler.config
                    ...
                    conn.maint_notifications_config = maint_notifications_pool_handler.config
                else:
                    raise ValueError(
    ...
    def _should_update_connection(
        ...
        matching_pattern: Literal["connected_address", "configured_address", "notification_hash"] = "connected_address",
        ...
        if matching_pattern == "connected_address":
            ...
            if matching_notification_hash and conn.maintenance_notification_hash != matching_notification_hash:
                return False
        return True
    ...
    def update_active_connections_for_reconnect(
        ...
        with self._get_pool_lock():
            for conn in self._get_in_use_connections():
                if self._should_update_connection(conn, "connected_address", moving_address_src):
                    conn.mark_for_reconnect()
...
class ConnectionPool(MaintNotificationsAbstractConnectionPool, ConnectionPoolInterface):
        ...
        if connection_kwargs.get("cache_config") or connection_kwargs.get("cache"):
            ...
            if cache is not None:
                ...
                if self._cache_factory is not None:
                    ...
                    self.cache = CacheFactory(self._connection_kwargs.get("cache_config")).get_cache()
    ...
    def __repr__(self) -> str:
        conn_kwargs = ",".join(
            [f"{k}={'<REDACTED>' if k in self.SENSITIVE_REPR_KEYS else v}" for k, v in self.connection_kwargs.items()]
        )
        return (
    ...
    def reset(self) -> None:
        ...
        if hasattr(self, "_available_connections") and hasattr(self, "_in_use_connections"):
            with self._lock:
    ...
    def __del__(self) -> None:
        ...
        try:
            if not hasattr(self, "_available_connections") or not hasattr(self, "_in_use_connections"):
                return
    ...
    def get_connection(self, command_name=None, *keys, **options) -> "Connection":
        ...
        try:
            ...
            try:
                if connection.can_read() and self.cache is None and not self.maint_notifications_enabled():
...
class BlockingConnectionPool(ConnectionPool):
    ...
    def reset(self):
        ...
        try:
            ...
            if hasattr(self, "_connections") and self._connections and hasattr(self, "pool"):
                with self._lock:
    ...
    def _get_in_use_connections(self):
        with self._lock:
            ...
            return {conn for conn in self._connections if conn not in connections_in_queue}
