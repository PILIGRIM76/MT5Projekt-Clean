...
from redis.asyncio.observability.recorder import record_streaming_lag_from_response as async_record_streaming_lag
...
class ManagementCommands(CommandsProtocol):
    ...
    def config_get(self, pattern: PatternT = "*", *args: PatternT, **kwargs) -> ResponseT:
        """
    ...
    def hotkeys_stop(self, **kwargs) -> Union[Awaitable[Union[str, bytes]], Union[str, bytes]]:
        """
    ...
    def hotkeys_reset(self, **kwargs) -> Union[Awaitable[Union[str, bytes]], Union[str, bytes]]:
        """
    ...
    def set(
        ...
        **Experimental** since 7.1.
        ...
        if not at_most_one_value_set((nx, xx, ifeq, ifne, ifdeq, ifdne)):
            raise DataError("``nx``, ``xx``, ``ifeq``, ``ifne``, ``ifdeq``, ``ifdne`` are mutually exclusive.")
...
class ListCommands(CommandsProtocol):
    ...
    def blpop(self, keys: KeysT, timeout: Optional[Number] = 0) -> Union[Awaitable[list], list]:
        """
    ...
    def brpop(self, keys: KeysT, timeout: Optional[Number] = 0) -> Union[Awaitable[list], list]:
        """
    ...
    def lindex(self, name: KeyT, index: int) -> Union[Awaitable[Optional[str]], Optional[str]]:
        """
    ...
    def linsert(self, name: KeyT, where: str, refvalue: str, value: str) -> Union[Awaitable[int], int]:
        """
...
class SetCommands(CommandsProtocol):
    ...
    def sintercard(self, numkeys: int, keys: List[KeyT], limit: int = 0) -> Union[Awaitable[int], int]:
        """
    ...
    def spop(self, name: KeyT, count: Optional[int] = None) -> Union[Awaitable[Union[str, List, None]], str, List, None]:
        """
...
class StreamCommands(CommandsProtocol):
    ...
    def xcfgset(
        ...
        if idmp_duration is None and idmp_maxsize is None:
            raise DataError("XCFGSET requires at least one of idmp_duration or idmp_maxsize")

        pieces: list[EncodableT] = []

        if idmp_duration is not None:
            if not isinstance(idmp_duration, int) or idmp_duration < 1 or idmp_duration > 300:
                raise DataError("XCFGSET idmp_duration must be an integer between 1 and 300")
        ...
        if idmp_maxsize is not None:
            if not isinstance(idmp_maxsize, int) or idmp_maxsize < 1 or idmp_maxsize > 1000000:
                raise DataError("XCFGSET idmp_maxsize must be an integer between 1 and 1,000,000")
    ...
    def xreadgroup(
        ...
        if claim_min_idle_time is not None:
            if not isinstance(claim_min_idle_time, int) or claim_min_idle_time < 0:
                raise DataError("XREADGROUP claim_min_idle_time must be a non-negative integer")
...
class ScriptCommands(CommandsProtocol):
    ...
    def eval(self, script: str, numkeys: int, *keys_and_args: Union[KeyT, EncodableT]) -> Union[Awaitable[str], str]:
        """
    ...
    def eval_ro(self, script: str, numkeys: int, *keys_and_args: Union[KeyT, EncodableT]) -> Union[Awaitable[str], str]:
        """
    ...
    def evalsha(self, sha: str, numkeys: int, *keys_and_args: Union[KeyT, EncodableT]) -> Union[Awaitable[str], str]:
        """
    ...
    def evalsha_ro(self, sha: str, numkeys: int, *keys_and_args: Union[KeyT, EncodableT]) -> Union[Awaitable[str], str]:
        """
