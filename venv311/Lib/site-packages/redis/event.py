            ...
            for event_type in mappings:
                if event_type in self._event_listeners_mapping:
                    self._event_listeners_mapping[event_type] = list(
                        set(self._event_listeners_mapping[event_type] + mappings[event_type])
                    )
                else:
...
class RegisterReAuthForPubSub(EventListenerInterface):
    ...
    def listen(self, event: AfterPubSubConnectionInstantiationEvent):
        if isinstance(event.pubsub_connection.credential_provider, StreamingCredentialProvider) and check_protocol_version(
            event.pubsub_connection.get_protocol(), 3
        ):
            self._event = event
