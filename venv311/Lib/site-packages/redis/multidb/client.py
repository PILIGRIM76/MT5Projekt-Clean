...
class MultiDBClient(RedisModuleCommands, CoreCommands):
    ...
    def add_database(self, config: DatabaseConfig, skip_initial_health_check: bool = True):
        ...
        if "maint_notifications_config" not in config.client_kwargs:
            config.client_kwargs["maint_notifications_config"] = MaintNotificationsConfig(enabled=False)
    ...
    def _on_circuit_state_change_callback(self, circuit: CircuitBreaker, old_state: CBState, new_state: CBState):
        if new_state == CBState.HALF_OPEN:
            self._bg_scheduler.run_coro_fire_and_forget(self._check_db_health, circuit.database)
            return
...
class PubSub:
    ...
    def get_sharded_message(self, ignore_subscribe_messages: bool = False, timeout: float = 0.0):
        """
