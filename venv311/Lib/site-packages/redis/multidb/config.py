...
class MultiDbConfig:
    ...
    command_retry: Retry = Retry(backoff=ExponentialWithJitterBackoff(base=1, cap=10), retries=3)
    ...
    def databases(self) -> Databases:
        ...
        for database_config in self.databases_config:
            ...
            if "maint_notifications_config" not in database_config.client_kwargs:
                database_config.client_kwargs["maint_notifications_config"] = MaintNotificationsConfig(enabled=False)
            ...
            circuit = database_config.default_circuit_breaker() if database_config.circuit is None else database_config.circuit
            databases.add(
                Database(
                    client=client,
                    circuit=circuit,
