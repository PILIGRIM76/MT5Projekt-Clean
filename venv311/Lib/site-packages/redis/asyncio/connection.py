...
class AbstractConnection:
    ...
    async def connect_check_health(self, check_health: bool = True, retry_socket_connect: bool = True):
    ...
    async def _ping_failed(self, error, failure_count):
        ...
        await self.disconnect(error=error, failure_count=failure_count, health_check_failed=True)

    async def check_health(self):
        """Check the health of the connection with a PING/PONG"""
        if self.health_check_interval and asyncio.get_running_loop().time() > self.next_health_check:
            await self.retry.call_with_retry(self._send_ping, self._ping_failed, with_failure_count=True)
...
class ConnectionPool:
    ...
    def reset(self):
        ...
        if hasattr(self, "_available_connections") and hasattr(self, "_in_use_connections"):
            ...
            if idle_count > 0 or in_use_count > 0:
                ...
                from redis.observability.recorder import record_connection_count as sync_record_connection_count
    ...
    async def get_connection(self, command_name=None, *keys, **options):
        ...
        async with self._lock:
            connections_before = len(self._available_connections) + len(self._in_use_connections)
            start_time_created = time.monotonic()
            connection = self.get_available_connection()
            connections_after = len(self._available_connections) + len(self._in_use_connections)
