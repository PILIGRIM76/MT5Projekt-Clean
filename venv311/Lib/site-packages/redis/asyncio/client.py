...
class Redis(AbstractRedis, AsyncRedisModuleCommands, AsyncCoreCommands, AsyncSentinelCommands):
    ...
    async def execute_command(self, *args, **options):
        ...
        def failure_callback(error, failure_count):
            ...
            return self._close_connection(conn, error, failure_count, start_time, command_name)
    ...
    async def on_connect(self, connection: Connection):
        ...
        if self.channels:
            ...
            if channels_with_handlers or channels_without_handlers:
                await self.subscribe(*channels_without_handlers, **channels_with_handlers)
        if self.patterns:
            ...
            if patterns_with_handlers or patterns_without_handlers:
                await self.psubscribe(*patterns_without_handlers, **patterns_with_handlers)
        ...
        while True:
            try:
                if pubsub is None:
                    ...
                    await pubsub.get_message(ignore_subscribe_messages=True, timeout=poll_timeout)
...
class Pipeline(Redis):  # lgtm [py/init-calls-subclass]
    ...
    async def immediate_execute_command(self, *args, **options):
        ...
        def failure_callback(error, failure_count):
            nonlocal actual_retry_attempts
            actual_retry_attempts = failure_count
            return self._disconnect_reset_raise_on_watching(conn, error, failure_count, start_time, command_name)
    ...
    def annotate_exception(self, exception: Exception, number: int, command: Iterable[object]) -> None:
        ...
        msg = f"Command # {number} ({truncate_text(cmd)}) " f"of pipeline caused error: {exception.args}"
    ...
    async def execute(self, raise_on_error: bool = True) -> List[Any]:
        ...
        def failure_callback(error, failure_count):
            nonlocal actual_retry_attempts
            actual_retry_attempts = failure_count
            return self._disconnect_raise_on_watching(conn, error, failure_count, start_time, operation_name)
