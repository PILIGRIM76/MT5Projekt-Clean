            ...
            try:
                ...
                if len(target_nodes) == 1:
                    ...
                    if command in self.result_callbacks:
                        ret = self.result_callbacks[command](command, {target_nodes[0].name: ret}, **kwargs)
                ...
                if not command_policies:
                    ...
                    if not command_flag:
                        ...
                        if command_flag in client._command_flags_mapping:
                            command_policies = CommandPolicies(request_policy=client._command_flags_mapping[command_flag])
                        else:
                            command_policies = CommandPolicies()
...
class TransactionStrategy(AbstractStrategy):
    ...
    def _get_client_and_connection_for_transaction(
        ...
        node: ClusterNode = self._pipe.cluster_client.nodes_manager.get_node_from_slot(list(self._pipeline_slots)[0], False)
    ...
    async def _get_connection_and_send_command(self, *args, **options):
        ...
        try:
            response = await self._send_command_parse_response(connection, redis_node, args[0], *args, **options)
