...
class AsyncCommandExecutor(CommandExecutor):
    ...
    @abstractmethod
    async def set_active_database(self, database: AsyncDatabase, reason: GeoFailoverReason) -> None:
...
class DefaultCommandExecutor(BaseCommandExecutor, AsyncCommandExecutor):
    ...
    async def execute_pubsub_run(self, sleep_time: float, exception_handler=None, pubsub=None) -> Any:
        async def callback():
    ...
    async def _check_active_database(self):
        ...
        if (
            ...
            or (self._auto_fallback_interval > 0 and self._next_fallback_attempt <= datetime.now())
        ):
