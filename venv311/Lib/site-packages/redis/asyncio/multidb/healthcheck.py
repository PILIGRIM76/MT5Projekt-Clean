...
class AbstractHealthCheckPolicy(HealthCheckPolicy):
        ...
        if client is None:
            ...
            if isinstance(database.client, (AsyncRedis, SyncRedis)):
                ...
                if startup_nodes:
                    ...
                    raise ValueError("Cluster client has no nodes - cannot create health check client")
            else:
    ...
    async def close(self) -> None:
        """Close all health check clients."""
        close_tasks = [asyncio.create_task(client.aclose()) for client in self._clients.values()]
...
class LagAwareHealthCheck(AbstractHealthCheck):
    ...
    async def check_health(self, database, hc_client: AsyncRedisClientT) -> bool:
        ...
        if database.health_check_url is None:
            raise ValueError("Database health check url is not set. Please check DatabaseConfig for the current database.")
