...
class MultiDBClient(AsyncRedisModuleCommands, AsyncCoreCommands):
    ...
    def __init__(self, config: MultiDbConfig):
        ...
        self._failure_detectors = (
            config.default_failure_detectors() if not config.failure_detectors else config.failure_detectors
        )

        self._failover_strategy = (
            config.default_failover_strategy() if config.failover_strategy is None else config.failover_strategy
        )
    ...
    async def set_active_database(self, database: AsyncDatabase) -> None:
        ...
        if database.circuit.state == CBState.CLOSED:
            ...
            await self.command_executor.set_active_database(database, GeoFailoverReason.MANUAL)
            return
    ...
    async def add_database(self, config: DatabaseConfig, skip_initial_health_check: bool = True):
        ...
        if config.from_url:
            client = self._config.client_class.from_url(config.from_url, **config.client_kwargs)
        elif config.from_pool:
            config.from_pool.set_retry(Retry(retries=0, backoff=NoBackoff()))
            client = self._config.client_class.from_pool(connection_pool=config.from_pool)
        else:
            client = self._config.client_class(**config.client_kwargs)

        circuit = config.default_circuit_breaker() if config.circuit is None else config.circuit
    ...
    async def _change_active_database(self, new_database: AsyncDatabase, highest_weight_database: AsyncDatabase):
        if new_database.weight > highest_weight_database.weight and new_database.circuit.state == CBState.CLOSED:
            await self.command_executor.set_active_database(new_database, GeoFailoverReason.AUTOMATIC)
    ...
    async def remove_database(self, database: AsyncDatabase):
        ...
        if highest_weight <= weight and highest_weighted_db.circuit.state == CBState.CLOSED:
            await self.command_executor.set_active_database(highest_weighted_db, GeoFailoverReason.MANUAL)
    ...
    async def _check_databases_health(self) -> dict[Database, bool]:
        ...
        db_results = {task_to_db[task]: result for task, result in zip(self._hc_tasks, results)}
    ...
    async def _perform_initial_health_check(self):
        ...
        elif self._config.initial_health_check_policy == InitialHealthCheck.MAJORITY_AVAILABLE:
            is_healthy = sum(results.values()) > len(results) / 2
        elif self._config.initial_health_check_policy == InitialHealthCheck.ONE_AVAILABLE:
    ...
    def _on_circuit_state_change_callback(self, circuit: CircuitBreaker, old_state: CBState, new_state: CBState):
        ...
        if old_state == CBState.CLOSED and new_state == CBState.OPEN:
            logger.warning(f"Database {circuit.database} is unreachable. Failover has been initiated.")
...
class PubSub:
    ...
    async def psubscribe(self, *args: ChannelT, **kwargs: PubSubHandler):
        ...
        ``listen()``.
        """
        return await self._client.command_executor.execute_pubsub_method("psubscribe", *args, **kwargs)
    ...
    async def punsubscribe(self, *args: ChannelT):
        ...
        all patterns.
        """
        return await self._client.command_executor.execute_pubsub_method("punsubscribe", *args)
    ...
    async def subscribe(self, *args: ChannelT, **kwargs: Callable):
        ...
        ``get_message()``.
        """
        return await self._client.command_executor.execute_pubsub_method("subscribe", *args, **kwargs)
    ...
    async def unsubscribe(self, *args):
        ...
        all channels
        """
        return await self._client.command_executor.execute_pubsub_method("unsubscribe", *args)
