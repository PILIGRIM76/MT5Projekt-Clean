...
class MaintNotificationsAbstractRedisCluster:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if maint_notifications_config and maint_notifications_config.enabled and not is_protocol_supported:
        ...
        if self.maint_notifications_config and self.maint_notifications_config.enabled:
            self._oss_cluster_maint_notifications_handler = OSSMaintNotificationsHandler(self, self.maint_notifications_config)
            # Update connection kwargs for all future nodes connections
            self._update_connection_kwargs_for_maint_notifications(self._oss_cluster_maint_notifications_handler)
...
class RedisCluster(AbstractRedisCluster, MaintNotificationsAbstractRedisCluster, RedisClusterCommands):
    @classmethod
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if maint_notifications_config and not check_protocol_version(protocol, 3):
            raise RedisError("Maintenance notifications are only supported with RESP version 3")
        ...
        MaintNotificationsAbstractRedisCluster.__init__(self, maint_notifications_config, **kwargs)
        ...
        while ttl > 0:
            ...
            try:
                ...
                if self._should_reinitialized():
                    ...
                    self.nodes_manager.initialize(additional_startup_nodes_info=[(e.host, e.port)])
                    # Reset the counter
                    self.reinitialize_counter = 0
                else:
    ...
    def _extracts_socket_address(self, connection: Optional[Connection]) -> Optional[int]:
...
class NodesManager:
    ...
    def initialize(
        ...
        with self._initialization_lock:
            ...
            additional_startup_nodes = [ClusterNode(host, port) for host, port in additional_startup_nodes_info]
            if is_debug_log_enabled():
                logger.debug(
                ...
                try:
                    if startup_node.redis_connection:
                        ...
                        if is_debug_log_enabled():
                            ...
                            socket_connect_timeout = kwargs.get("socket_connect_timeout", "not set")
                            maint_enabled = (
                                self.maint_notifications_config.enabled if self.maint_notifications_config else False
                            )
                            logger.debug(
                        ...
                        if i not in tmp_slots:
                            ...
                            if tmp_slot.name != target_node.name:
                                ...
                                if len(disagreements) > 5:
                                    raise RedisClusterException(
                                        f"startup_nodes could not agree on a valid " f"slots cache: {', '.join(disagreements)}"
                                    )
...
class ClusterPubSub(PubSub):
    ...
    def execute_command(self, *args):
        ...
        if self.connection is None:
            ...
            self._event_dispatcher.dispatch(
                AfterPubSubConnectionInstantiationEvent(self.connection, self.connection_pool, ClientType.SYNC, self._lock)
            )
...
class ClusterPipeline(RedisCluster):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._execution_strategy: ExecutionStrategy = PipelineStrategy(self) if not transaction else TransactionStrategy(self)
...
class NodeCommands:
    ...
    def __init__(self, parse_response, connection_pool: ConnectionPool, connection: Connection):
        """ """
        ...
        for c in sorted(stack, key=lambda x: x.position):
            if c.args[0] in self._pipe.cluster_response_callbacks:
                ...
                c.result = self._pipe._policies_callback_mapping[c.command_policies.response_policy](
                    self._pipe.cluster_response_callbacks[c.args[0]](c.result, **c.options)
                )
...
class TransactionStrategy(AbstractStrategy):
    ...
    def _get_connection_and_send_command(self, *args, **options):
        ...
        try:
            response = self._send_command_parse_response(connection, redis_node, args[0], *args, **options)
