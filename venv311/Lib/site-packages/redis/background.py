...
class BackgroundScheduler:
    ...
    def run_recurring_coro(self, interval: float, coro: Callable[..., Coroutine[Any, Any, Any]], *args):
        """
        ...
        loop.call_soon_threadsafe(self._call_later_recurring_coro, loop, interval, coro, *args)
    ...
    def run_coro_fire_and_forget(self, coro: Callable[..., Coroutine[Any, Any, Any]], *args) -> None:
        """
    ...
    def _ensure_health_check_loop(self, timeout: float = 5.0):
        ...
        if self._health_check_loop_ready.is_set():
            with self._lock:
                if self._health_check_loop is not None and self._health_check_loop.is_running():
                    return
        ...
        with self._lock:
            ...
            if not self._health_check_loop_ready.wait(timeout=timeout):
                ...
                try:
                    ...
                    pass
                raise RuntimeError(f"Health check event loop failed to start within {timeout} seconds")
    ...
    def _run_health_check_loop(self):
        ...
        try:
            ...
            try:
                ...
                for task in pending:
                    task.cancel()
                self._health_check_loop.run_until_complete(asyncio.gather(*pending, return_exceptions=True))
            except Exception:
                pass
            finally:
    ...
    def _call_later_recurring_coro(
        ...
        *args,
        ...
        with self._lock:
            if self._stopped:
                return
        self._call_later(loop, interval, self._execute_recurring_coro, loop, interval, coro, *args)
    ...
    async def run_recurring_async(self, interval: float, coro: Callable[..., Coroutine[Any, Any, Any]], *args):
        """
        ...
        def execute_and_reschedule():
            ...
            try:
                ...
                logging.getLogger(__name__).debug("Failed to schedule recurring async coroutine", exc_info=True)
                schedule_next()
    ...
    def _call_later_recurring(
        ...
        *args,
        ...
        with self._lock:
            if self._stopped:
                return
        self._call_later(loop, interval, self._execute_recurring, loop, interval, callback, *args)
