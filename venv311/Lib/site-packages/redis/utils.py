...
def _get_filterable_args(func: Callable, args: tuple, kwargs: dict, allowed_args: Optional[List[str]] = None) -> dict:
    """
    ...
    def _check_deprecated_args(func, filterable_args):
        ...
        for arg in args_to_warn:
            if arg == "*" and len(filterable_args) > 0:
                ...
                warn_deprecated_arg_usage(arg, func.__name__, reason, version, stacklevel=5)
...
def check_protocol_version(protocol: Optional[Union[str, int]], expected_version: int = 3) -> bool:
    if protocol is None:
        return False
...
def warn_experimental_arg_usage(
    ...
    msg = f"Call to '{function_name}' method with experimental" f" usage of input argument/s '{arg_name}'."
    warnings.warn(msg, category=UserWarning, stacklevel=stacklevel)
...
def experimental_args(
    ...
    def _check_experimental_args(func, filterable_args):
        ...
        for arg in args_to_warn:
            if arg == "*" and len(filterable_args) > 0:
                warn_experimental_arg_usage(list(filterable_args.keys()), func.__name__, stacklevel=4)
            elif arg in filterable_args:
