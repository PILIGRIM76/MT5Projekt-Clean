    ...
    def __init__(
        ...
        if not connection_pool:
            ...
            if unix_socket_path is not None:
                ...
                maint_notifications_enabled = maint_notifications_config and maint_notifications_config.enabled
        ...
        while conn and self.health_check_response_counter > 0 and ttl > 0:
            if self._execute(conn, conn.can_read, timeout=conn.socket_timeout):
                ...
                if self.is_health_check_response(response):
                    ...
                    raise PubSubError("A non health check response was cleaned by " "execute_command: {}".format(response))
            ttl -= 1
...
class Pipeline(Redis):
    ...
    def immediate_execute_command(self, *args, **options):
        ...
        def failure_callback(error, failure_count):
            actual_retry_attempts[0] = failure_count
            self._disconnect_reset_raise_on_watching(conn, error, failure_count, start_time, command_name)
    ...
    def execute(self, raise_on_error: bool = True) -> List[Any]:
        ...
        def failure_callback(error, failure_count):
            actual_retry_attempts[0] = failure_count
            self._disconnect_raise_on_watching(conn, error, failure_count, start_time, operation_name)
