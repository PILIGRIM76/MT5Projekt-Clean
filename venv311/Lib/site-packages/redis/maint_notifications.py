...
class NodeMovingNotification(MaintenanceNotification):
    ...
    def __eq__(self, other) -> bool:
        ...
        if not isinstance(other, NodeMovingNotification):
            return False
        return self.id == other.id and self.new_node_host == other.new_node_host and self.new_node_port == other.new_node_port
...
class MaintNotificationsConfig:
    ...
    def get_endpoint_type(self, host: str, connection: "MaintNotificationsAbstractConnection") -> EndpointType:
        """
...
class MaintNotificationsPoolHandler:
    ...
    def handle_node_moving_notification(self, notification: NodeMovingNotification):
        if not self.config.proactive_reconnect and not self.config.is_relaxed_timeouts_enabled():
            return
        with self._lock:
            ...
            with self.pool._lock:
                ...
                if self.config.proactive_reconnect or self.config.is_relaxed_timeouts_enabled():
                    ...
                    moving_address_src = self.connection.getpeername() if self.connection else None
    ...
    def handle_node_moved_notification(self, notification: NodeMovingNotification):
        ...
        with self._lock:
            ...
            if self.pool.connection_kwargs.get("maintenance_notification_hash") == notification_hash:
...
class MaintNotificationsConnectionHandler:
    ...
    def _get_pool_name(self) -> str:
        ...
        """
        pool_handler = getattr(self.connection, "_maint_notifications_pool_handler", None)
    ...
    def handle_notification(self, notification: MaintenanceNotification):
        ...
        if notification_type:
            self.handle_maintenance_start_notification(MaintenanceState.MAINTENANCE, notification)
        else:
    ...
    def handle_maintenance_start_notification(
        ...
        if self.connection.maintenance_state == MaintenanceState.MOVING or not self.config.is_relaxed_timeouts_enabled():
            return

        self.connection.maintenance_state = maintenance_state
        self.connection.set_tmp_settings(tmp_relaxed_timeout=self.config.relaxed_timeout)
...
class OSSMaintNotificationsHandler:
    ...
    def handle_oss_maintenance_completed_notification(self, notification: OSSNodeMigratedNotification):
        self.remove_expired_notifications()

        with self._lock:
            if notification in self._in_progress or notification in self._processed_notifications:
            ...
            for (
                ...
                src_node = self.cluster_client.nodes_manager.get_node(host=src_host, port=src_port)
                ...
                for dest_mapping in dest_mappings:
                    for dest_address in dest_mapping.keys():
                        ...
                        additional_startup_nodes_info.append((dest_host, int(dest_port)))
            ...
            all_nodes = all_nodes.union(self.cluster_client.nodes_manager.nodes_cache.values())
            ...
            for current_node in all_nodes:
                ...
                with current_node.redis_connection.connection_pool._lock:
                    ...
                    if current_node in affected_nodes:
                        ...
                        record_connection_handoff(pool_name=get_pool_name(current_node.redis_connection.connection_pool))
                    ...
                    if current_node not in self.cluster_client.nodes_manager.nodes_cache.values():
