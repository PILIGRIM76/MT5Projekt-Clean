...
class CrossEncoderNanoBEIREvaluator(BaseEvaluator):
    ...
    def __call__(
        ...
        logger.info(f"MAP:{' ' * len(str(self.at_k))}   {agg_results['base_map'] * 100:.2f} -> {agg_results['map'] * 100:.2f}")
        logger.info(
    ...
    def _load_dataset(self, dataset_name: DatasetNameType | str, **ir_evaluator_kwargs) -> CrossEncoderRerankingEvaluator:
    ...
    def _load_dataset_subset_split(self, subset: str, split: str, required_columns: list[str]):
        ...
        try:
            ...
            raise ValueError(f"Could not load subset '{subset}' split '{split}' from dataset '{self.dataset_id}'.") from e
    ...
    def _validate_dataset_names(self):
        ...
        missing_datasets = [
            dataset_name for dataset_name in self.dataset_names if dataset_name.lower() not in DATASET_NAME_TO_HUMAN_READABLE
        ]
        if missing_datasets:
            raise ValueError(
