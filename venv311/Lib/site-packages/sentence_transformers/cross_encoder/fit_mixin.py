...
class EvaluatorCallback(TrainerCallback):
    ...
    def on_epoch_end(
        ...
        **kwargs,
        ...
        evaluator_metrics = self.evaluator(model, output_path=self.output_path, epoch=state.epoch, steps=state.global_step)
...
class FitMixin:
    ...
    def smart_batching_collate(self, batch: list[InputExample]) -> tuple[BatchEncoding, Tensor]:
        ...
        labels = torch.tensor(labels, dtype=torch.float if self.config.num_labels == 1 else torch.long).to(self.model.device)
