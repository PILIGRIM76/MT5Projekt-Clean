...
class NDCGLoss2Scheme(BaseWeightingScheme):
    ...
    def forward(self, gain: Tensor, discount: Tensor, true_sorted: Tensor) -> Tensor:
        ...
        deltas = torch.abs(
            torch.pow(torch.abs(discount[0, delta_idxs - 1]), -1.0) - torch.pow(torch.abs(discount[0, delta_idxs]), -1.0)
        )
...
class LambdaLoss(nn.Module):
    ...
    def forward(
        ...
        if isinstance(labels, Tensor):
            raise ValueError("LambdaLoss expects a list of labels for each sample, but got a single value for each sample.")
        if len(inputs) != 2:
