...
class MultipleNegativesRankingLoss(nn.Module):
    ...
    def call_model_with_pairs(self, pairs: list[list[str]], prompt: str | None = None, task: str | None = None) -> Tensor:
        inputs = self.model.preprocess(pairs, prompt=prompt, task=task)
        inputs = inputs.to(self.model.device)
        logits = self.model(inputs)["scores"]
        return logits.squeeze(1)

    def get_in_batch_negatives(self, anchors: list[str], candidates: list[list[str]]) -> Generator[list[str], None, None]:
    ...
    def forward(self, inputs: list[list[str]], labels: Tensor, prompt: str | None = None, task: str | None = None) -> Tensor:
        anchors = inputs[0]
        positives = inputs[1]
        batch_size = len(anchors)
