...
class ListNetLoss(nn.Module):
    ...
    def forward(
        ...
        if isinstance(labels, Tensor):
            raise ValueError("ListNetLoss expects a list of labels for each sample, but got a single value for each sample.")

        if len(inputs) != 2:
            raise ValueError(f"ListNetLoss expects two inputs (queries, documents_list), but got {len(inputs)} inputs.")
