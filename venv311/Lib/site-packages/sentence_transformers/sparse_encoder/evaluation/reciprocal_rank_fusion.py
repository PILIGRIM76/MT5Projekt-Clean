...
class ReciprocalRankFusionEvaluator(BaseEvaluator):
    ...
    def __init__(
        ...
        for i, (dense_sample, sparse_sample) in enumerate(zip(dense_samples, sparse_samples)):
            ...
            if dense_sample["query_id"] != sparse_sample["query_id"]:
                raise ValueError(f"Query ID mismatch at index {i}: {dense_sample['query_id']} != {sparse_sample['query_id']}")
        ...
        if self.write_predictions:
            self.predictions_file = "ReciprocalRankFusion_evaluation" + ("_" + name if name else "") + "_predictions.jsonl"
    ...
    def __call__(self, output_path: str | None = None, epoch: int = -1, steps: int = -1) -> dict[str, float]:
        ...
        for i, (dense_sample, sparse_sample) in enumerate(
            ...
            assert query_id == sparse_sample["query_id"], f"Query ID mismatch: {query_id} != {sparse_sample['query_id']}"
