...
def semantic_search_elasticsearch(
    ...
    **kwargs: Any,
    ...
    try:
        ...
        raise ImportError("Please install the Elasticsearch client with `pip install elasticsearch` to use this function.")

    # Validate input sparse tensors
    if not isinstance(query_embeddings_decoded, list) or not all(
        isinstance(item, list) and all(isinstance(t, tuple) and len(t) == 2 for t in item) for item in query_embeddings_decoded
    ):
    ...
    if corpus_index is None:
        ...
        es.indices.create(
            ...
            body={
                "mappings": {
                    "properties": {
                        "tokens": {"type": "rank_features"},  # This is the key - use rank_features for sparse vectors
                        "id": {"type": "keyword"},
                    }
                }
            },
...
def semantic_search_opensearch(
    ...
    **kwargs: Any,
    ...
    tuple[list[list[dict[str, int | float]]], float] | tuple[list[list[dict[str, int | float]]], float, tuple[OpenSearch, str]]
):
    """
    ...
    try:
        ...
        raise ImportError("Please install the OpenSearch client with `pip install opensearch-py` to use this function.")
