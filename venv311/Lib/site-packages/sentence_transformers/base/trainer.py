...
class BaseTrainer(Trainer, ABC):
    ...
    def __init__(
        ...
        if args is None:
            ...
            raise ValueError(f"Please pass an instance of `{fullname(self.training_args_class)}` as the `args` argument.")
        ...
        for dataset_name, dataset in zip(["train", "eval"], [train_dataset, eval_dataset]):
            if isinstance(dataset, IterableDataset) and dataset.column_names is None:
                ...
                example_features = {key: Value(naive_type_mapping.get(type(value), "null")) for key, value in sample.items()}
                raise ValueError(
    ...
    def log(self, logs: dict[str, float], start_time: float | None = None) -> None:
        ...
        if training_type:
            ...
            if hasattr(self, "_nested_gather"):
                ...
                accum_losses = nested_gather(self.accum_loss_components[training_type], parallel_mode=self.args.parallel_mode)
            if "steps" in accum_losses:
                ...
                for key, value in accum_losses.items():
                    ...
                    self.accum_loss_components[training_type][key] = torch.tensor(0.0, dtype=value.dtype, device=value.device)
    ...
    def get_eval_dataloader(self, eval_dataset: Dataset | DatasetDict | IterableDataset | None = None) -> DataLoader:
        ...
        return self.accelerator.prepare(self._build_dataloader(eval_dataset, self.args.eval_batch_size, dataset_kind="eval"))
    ...
    def get_test_dataloader(self, test_dataset: Dataset | DatasetDict | IterableDataset) -> DataLoader:
        ...
        # For evaluation, setting 'even_batches' to False results in hanging, so we keep it as True here.
        self.accelerator.even_batches = True
        return self.accelerator.prepare(self._build_dataloader(test_dataset, self.args.eval_batch_size, dataset_kind="test"))
    ...
    def get_optimizer_cls_and_kwargs(self, args: BaseTrainingArguments, model: BaseModel | None = None) -> tuple[Any, Any]:
        """
        ...
        if not {"params", "model", "optimizer_dict"} & set(optimizer_kwargs.keys()):
            optimizer_kwargs["optimizer_dict"] = [
                {
                    "params": [p for n, p in loss_model.named_parameters() if (n in decay_parameters and p.requires_grad)],
                    "weight_decay": self.args.weight_decay,
                },
                {
                    "params": [p for n, p in loss_model.named_parameters() if (n not in decay_parameters and p.requires_grad)],
                    "weight_decay": 0.0,
                },
            ]
