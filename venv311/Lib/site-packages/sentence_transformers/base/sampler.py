...
class GroupByLabelBatchSampler(DefaultBatchSampler):
    ...
    def __init__(
        ...
        self.groups = {label: indices[: len(indices) // 2 * 2] for label, indices in groups.items() if len(indices) >= 2}
    ...
    def __iter__(self) -> Iterator[list[int]]:
        ...
        while len(remaining_labels) >= 2:
            remaining_labels = [remaining_labels[i] for i in torch.randperm(len(remaining_labels), generator=self.generator)]
...
def _hash_batch(batch: dict[str, list[Any]], columns: list[str], exclude_columns: set[str]) -> dict[str, list[list[int]]]:
...
class NoDuplicatesBatchSampler(DefaultBatchSampler):
    ...
    def __iter__(self) -> Iterator[list[int]]:
        ...
        if self.precompute_hashes:
            ...
            def get_sample_values(index: int) -> set[str] | np.ndarray:
                return {str(value) for key, value in self.dataset[index].items() if key not in _EXCLUDE_DATASET_COLUMNS}
