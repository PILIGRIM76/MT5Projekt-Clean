...
TransformerTask = Literal["feature-extraction", "sequence-classification", "text-generation", "any-to-any", "fill-mask"]
...
class Transformer(InputModule):
    ...
    def __init__(
        ...
        *,
        ...
        self.model = self._load_model(model_name_or_path, transformer_task, config, backend, is_peft_model, **model_kwargs)
    ...
    def preprocess(
        ...
        **kwargs,
        ...
        if prompt and modality == "message":
            processor_inputs["message"] = self.input_formatter.prepend_prompt_to_messages(processor_inputs["message"], prompt)
    ...
    def _call_multimodal_processor(
        ...
        if self.config.model_type in {"clipseg", "whisper", "sam3"} or not _TRANSFORMERS_PROCESSOR_SUPPORTS_MODALITY_KWARGS:
    ...
    def _load_encoder_only_model(
        ...
        **model_kwargs,
        ...
        def _load_encoder(model_cls, load_config=None, **extra_class_attrs):
            with set_temporary_class_attrs(model_cls, _keys_to_ignore_on_load_unexpected=["decoder.*"], **extra_class_attrs):
    ...
    def infer_modalities(
        ...
        processor: (
            ProcessorMixin | PreTrainedTokenizerBase | FeatureExtractionMixin | BaseVideoProcessor | ImageProcessingMixin
        ),
            ...
            if hasattr(processor, "chat_template") and processor.chat_template is not None:
                if "message" not in modality_config:
                    modality_config["message"] = {
                        **modality_config.get("text", {"method": "forward", "method_output_name": default_method_output_name}),
        ...
        return {
            modality: {"method": "forward", "method_output_name": default_method_output_name} for modality in modalities
    ...
    def infer_modalities_edge_cases(
        ...
        for modality, params in raw_config.items():
            if not hasattr(model, params["method"]):
                logger.warning_once(f"Model does not have method {params['method']!r} for modality {modality!r}. Skipping.")
                continue
