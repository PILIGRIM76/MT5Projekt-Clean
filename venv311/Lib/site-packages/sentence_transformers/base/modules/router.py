...
class Router(InputModule):
    ...
    def __init__(
        ...
        self.sub_modules = nn.ModuleDict({route_name: nn.Sequential(*modules) for route_name, modules in sub_modules.items()})
    ...
    def _resolve_route_name(self, task: str | None = None, modality: str | tuple[str, ...] | None = None) -> str | None:
        """
    ...
    def get_embedding_dimension(self) -> int | None:
        ...
        if not dims:
            return None
        if len(set(dims)) > 1:
            logger.warning_once(f"Different embedding dimensions detected across routes: {set(dims)}. Using the first value.")
        return dims[0]
