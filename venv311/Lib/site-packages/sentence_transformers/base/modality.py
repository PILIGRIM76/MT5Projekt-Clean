...
class InputFormatter:
    ...
    def to_message(self, typed_input: dict[Modality, Any], role: str = "user") -> list[dict[str, Any]]:
        ...
        if self.message_format == "flat":
            if len(typed_input) == 1:
                ...
                logger.warning(
                    "Flat message format requested but multiple modalities detected. " "Falling back to structured format."
                )
    ...
    def batch_to_message(self, modality: Modality, processor_inputs: dict) -> tuple[Literal["message"], dict[str, list]]:
    ...
    def prepend_prompt_to_messages(self, messages: list[list[dict[str, Any]]], prompt: str) -> list[list[dict[str, Any]]]:
        ...
        return [
            [{"role": "system", "content": [{"type": "text", "text": prompt}]}] + message_list for message_list in messages
        ]

    def prepend_prompt_to_texts(self, texts: list[str | tuple[str, str] | list[str]], prompt: str) -> list[str | list[str]]:
