...
class BaseModelCardCallback(TrainerCallback):
    ...
    def on_init_end(
        ...
        **kwargs,
        ...
        callbacks = [callback for callback in trainer.callback_handler.callbacks if isinstance(callback, CodeCarbonCallback)]
        if callbacks:
    ...
    def on_train_begin(
        ...
        **kwargs,
        ...
        model.model_card_data.all_hyperparameters = {key: value for key, value in args_dict.items() if key not in ignore_keys}
    ...
    def on_evaluate(
        ...
        **kwargs,
        ...
        loss_dict = {
            " ".join(key.split("_")[1:]): metrics[key] for key in metrics if key.startswith("eval_") and key.endswith("_loss")
        }
        ...
        if model.model_card_data.training_logs and model.model_card_data.training_logs[-1]["Step"] == state.global_step:
...
class BaseModelCardData(CardData):
        ...
        for dataset in dataset_list:
            ...
            if "id" in dataset and not self.local_files_only:
                ...
                try:
                    ...
                    logger.warning(f"The dataset `id` {dataset['id']!r} does not exist on the Hub. Setting the `id` to None.")
                    del dataset["id"]
                else:
    ...
    def set_losses(self, losses: list[nn.Module]) -> None:
        citations = {"Sentence Transformers": """
...
"""}
        ...
        for dataset_name, num_samples in tqdm(
            ...
            for idx in target_indices:
                ...
                while len(sentences) < 4 and backup_indices:
                    ...
                    backup_sample = [sentence for key, sentence in str_dataset[backup_idx].items() if key != "dataset_name"]
                ...
                sentences = [list(sentence.values())[0] if isinstance(sentence, dict) else sentence for sentence in sentences]
    ...
    def set_evaluation_metrics(self, evaluator: BaseEvaluator, metrics: dict[str, Any], epoch: int = 0, step: int = 0) -> None:
            ...
            for column in dataset_columns:
                ...
                if isinstance(first, str):
                    ...
                    if PILImage and isinstance(first, PILImage):
                        ...
                        widths = [
                            v.metadata.width for v in subsection if VideoDecoder is not None and isinstance(v, VideoDecoder)
                        ]
                        heights = [
                            v.metadata.height for v in subsection if VideoDecoder is not None and isinstance(v, VideoDecoder)
                        ]
                        if durations:
                            dataset_info["stats"][column] = {
                                "dtype": "video",
        ...
        if dataset:
            ...
            if isinstance(dataset, DatasetDict):
                dataset_metadata = [
                    ...
                    for dataset_name, dataset_value, dataset_info in zip(dataset.keys(), dataset.values(), dataset_metadata)
                ]
            else:
            ...
            table_lines = [
                {
                    ...
                    "Value": f"**{format_log(metric_value)}**" if metric_key == primary_metric else format_log(metric_value),
                }
