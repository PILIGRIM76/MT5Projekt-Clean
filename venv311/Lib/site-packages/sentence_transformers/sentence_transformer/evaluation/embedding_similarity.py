...
class EmbeddingSimilarityEvaluator(BaseEvaluator):
    ...
    def __init__(
        ...
        self.csv_file = (
            "similarity_evaluation" + ("_" + name if name else "") + ("_" + precision if precision else "") + "_results.csv"
        )
        self.csv_headers = [
            "epoch",
            "steps",
    ...
    def __call__(
        ...
        for fn_name in self.similarity_fn_names:
            if fn_name in similarity_functions:
                ...
                logger.info(f"{fn_name.capitalize()}-Similarity:\tPearson: {eval_pearson:.4f}\tSpearman: {eval_spearman:.4f}")
