    ...
    def forward(self, features: dict[str, Tensor | Any], **kwargs) -> dict[str, Tensor | Any]:
        ...
        if "cu_seq_lens_q" in features:
            ...
            if "attention_mask" in features and features["attention_mask"].size(-1) == token_embeddings.size(1):
                ...
                attention_mask = torch.ones(token_embeddings.shape[:-1], device=token_embeddings.device, dtype=torch.int64)
        ...
        for mode in modes:
            if mode == "cls":
                ...
                if mode == "mean":
                    ...
                    output_vectors.append(mean_sum / torch.sqrt(torch.clamp(effective_lengths.unsqueeze(1).float(), min=1e-9)))
