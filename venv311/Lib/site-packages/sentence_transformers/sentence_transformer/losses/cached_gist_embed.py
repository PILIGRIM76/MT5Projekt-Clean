...
def _backward_hook(grad_output: Tensor, sentence_features: Iterable[dict[str, Tensor]], loss_obj: CachedGISTEmbedLoss) -> None:
...
class CachedGISTEmbedLoss(nn.Module):
    ...
    def __init__(
        ...
        if not isinstance(model.tokenizer, PreTrainedTokenizerBase) or not isinstance(
            ...
            raise ValueError("Both the training model and the guiding model must use a PreTrainedTokenizer from transformers.")
        self.mini_batch_size = mini_batch_size
        self.cache: list[list[Tensor]] | None = None
        self.random_states: list[list[RandContext]] | None = None
        self.show_progress_bar = show_progress_bar
        self.must_retokenize = model.tokenizer.vocab != guide.tokenizer.vocab or guide.max_seq_length < model.max_seq_length
    ...
    def embed_minibatch(
        ...
        with random_state_context:
            ...
            with torch.no_grad():
                if self.must_retokenize:
                    decoded = self.tokenizer.batch_decode(sentence_feature_minibatch["input_ids"], skip_special_tokens=True)
    ...
    def calculate_loss(self, reps: list[list[Tensor]], reps_guided: list[list[Tensor]], with_backward: bool = False) -> Tensor:
        ...
        for begin in tqdm.trange(
            ...
            if self.contrast_positives:
                pp_sim = self.sim_matrix(candidates[0][offset + begin : min(offset + end, offset + batch_size)], candidates[0])
            ...
            loss_mbatch: torch.Tensor = self.cross_entropy_loss(scores, range_labels[begin:end]) * len(scores) / batch_size
            if with_backward:
