...
class SentenceTransformer(BaseModel, FitMixin):
        ...
        *,
        ...
        if use_auth_token is not None:
            ...
            if token is not None:
                raise ValueError("Both `token` and `use_auth_token` are specified. Please only specify the `token` argument.")
        ...
        elif model_name_or_path and "/" in model_name_or_path and "instructor" in model_name_or_path.split("/")[1].lower():
        ...
        **kwargs,
        ...
        for start_index in trange(0, len(inputs_sorted), batch_size, desc="Batches", disable=not show_progress_bar):
            ...
            if truncate_dim is not None:
                out_features["sentence_embedding"] = truncate_embeddings(out_features["sentence_embedding"], truncate_dim)
    ...
    def _pad_features_for_hpu(features: dict[str, Tensor]) -> dict[str, Tensor]:
        ...
        features["input_ids"] = torch.cat((features["input_ids"], torch.ones((batch_size, pad_len), dtype=torch.int8)), -1)
    ...
    @overload
    def similarity_pairwise(self, embeddings1: npt.NDArray[np.float32], embeddings2: npt.NDArray[np.float32]) -> Tensor: ...
