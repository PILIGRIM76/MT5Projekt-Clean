...
def _get_intermediate_plot_info(study: Study) -> _IntermediatePlotInfo:
    trials = study.get_trials(deepcopy=False, states=(TrialState.PRUNED, TrialState.COMPLETE, TrialState.RUNNING))
    ...
    trial_infos = [
        _TrialInfo(trial.number, sorted(trial.intermediate_values.items()), _satisfies_constraints(trial))
        for trial in trials
    ...
    if len(trials) == 0:
        ...
        _logger.warning("You need to set up the pruning feature to utilize `plot_intermediate_values()`")
...
def _get_intermediate_plot(info: _IntermediatePlotInfo) -> "go.Figure":
    ...
    traces = [
        go.Scatter(
            ...
            mode="lines+markers",
            marker=(default_marker if tinfo.feasible else {**default_marker, "color": "#CCCCCC"}),  # type: ignore[dict-item]
