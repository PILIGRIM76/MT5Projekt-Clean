...
from optuna.visualization._utils import _check_plot_args, _is_log_scale, _is_numerical
...
if plotly_is_available:
    from optuna.visualization._plotly_imports import Scatter, go, make_subplots, plotly
...
def _get_rank_info(
    ...
    if len(params) == 2:
        ...
        sub_plot_infos = [
            [_get_rank_subplot_info(trials, target_values, colors, x_param, y_param) for x_param in params]
...
def _get_rank_subplot(info: _RankSubplotInfo, target_name: str, print_raw_objectives: bool) -> "Scatter":
    ...
    scatter = go.Scatter(
        ...
        hovertext=[get_hover_text(trial, target_value) for trial, target_value in zip(info.trials, info.zs)],
    )
    return scatter
    ...
    if len(params) == 2:
        ...
        for x_i, x_param in enumerate(params):
            for y_i, y_param in enumerate(params):
                scatter = _get_rank_subplot(sub_plot_infos[y_i][x_i], info.target_name, info.has_custom_target)
