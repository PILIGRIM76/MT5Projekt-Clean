...
from optuna.visualization._utils import _check_plot_args, _filter_nonfinite, _is_log_scale, _is_numerical, _is_reverse_scale

if _imports.is_successful():
    from optuna.visualization._plotly_imports import Contour, Scatter, go, make_subplots
...
def _get_contour_plot(info: _ContourInfo) -> "go.Figure":
    ...
    if len(sorted_params) == 2:
        ...
        figure = make_subplots(rows=len(sorted_params), cols=len(sorted_params), shared_xaxes=True, shared_yaxes=True)
        ...
        for x_i, x_param in enumerate(sorted_params):
            for y_i, y_param in enumerate(sorted_params):
                if x_param == y_param:
                    ...
                    sub_plots = _get_contour_subplot(sub_plot_infos[y_i][x_i], reverse_scale, target_name)
                    contour = sub_plots[0]
                    scatter = sub_plots[1]
...
def _get_contour_subplot_info(
    ...
    for i, trial in enumerate(trials):
        ...
        if existing is None or target is not None:
            ...
            z_values[(x_i, y_i)] = min(existing, value) if study.direction is StudyDirection.MINIMIZE else max(existing, value)
