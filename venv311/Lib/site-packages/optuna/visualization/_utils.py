...
def _check_plot_args(
    ...
    if target is None and any(study._is_multi_objective() for study in studies):
        raise ValueError("If the `study` is being used for multi-objective optimization, " "please specify the `target`.")

    if target is not None and target_name == "Objective Value":
        optuna_warn("`target` is specified, but `target_name` is the default value, 'Objective Value'.")
...
def _get_skipped_trial_numbers(trials: list[FrozenTrial], used_param_names: Sequence[str]) -> set[int]:
...
def _filter_nonfinite(
    ...
    for trial in trials:
        ...
        try:
            ...
            optuna_warn(f"Trial{trial.number}'s target value {repr(value)} could not be cast to float.")
            raise
        ...
        if not np.isfinite(value):
            if with_message:
                _logger.warning(
                    f"Trial {trial.number} is omitted in visualization " "because its objective value is inf or nan."
                )
        else:
