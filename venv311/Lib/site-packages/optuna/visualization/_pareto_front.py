...
def plot_pareto_front(
    ...
    *,
    ...
    info = _get_pareto_front_info(study, target_names, include_dominated_trials, axis_order, constraints_func, targets)
...
def _get_pareto_front_info(
    ...
    if axis_order is not None:
        msg = _deprecated._DEPRECATION_WARNING_TEMPLATE.format(name="`axis_order`", d_ver="3.0.0", r_ver="5.0.0")
        optuna_warn(msg, FutureWarning)

    if constraints_func is not None:
        msg = _deprecated._DEPRECATION_WARNING_TEMPLATE.format(name="`constraints_func`", d_ver="4.0.0", r_ver="6.0.0")
        optuna_warn(msg, FutureWarning)

    if targets is not None and axis_order is not None:
        raise ValueError("Using both `targets` and `axis_order` is not supported. " "Use either `targets` or `axis_order`.")
    ...
    def _make_trials_with_values(
        ...
        for v in target_values:
            if not isinstance(v, Sequence):
                raise ValueError("`targets` should return a sequence of target values." f" your `targets` returns {type(v)}")
    ...
    n_targets = _infer_n_targets(best_trials_with_values) or _infer_n_targets(infeasible_trials_with_values)
    if n_targets is None:
        if target_names is not None:
            ...
            raise ValueError("If `targets` is specified for empty studies, `target_names` must be specified.")

    if n_targets not in (2, 3):
        raise ValueError("`plot_pareto_front` function only supports 2 or 3 targets." f" you used {n_targets} targets now.")
    ...
    if axis_order is None:
        ...
        if len(axis_order) != n_targets:
            raise ValueError(f"Size of `axis_order` {axis_order}. Expect: {n_targets}, " f"Actual: {len(axis_order)}.")
        ...
        if max(axis_order) > n_targets - 1:
            raise ValueError(
                f"Given `axis_order` {axis_order} contains invalid index {max(axis_order)} " f"higher than {n_targets - 1}."
            )
        if min(axis_order) < 0:
...
def _get_non_pareto_front_trials(trials: list[FrozenTrial], pareto_trials: list[FrozenTrial]) -> list[FrozenTrial]:
    non_pareto_trials = []
    for trial in trials:
