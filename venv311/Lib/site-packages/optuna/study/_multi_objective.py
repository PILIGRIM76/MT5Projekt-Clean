...
def _get_pareto_front_trials_by_trials(
    ...
    if any(len(t.values) != len(directions) for t in trials):
        raise ValueError("The number of the values and the number of the objectives must be identical.")

    loss_values = np.asarray([[_normalize_value(v, d) for v, d in zip(t.values, directions)] for t in trials])
...
def _get_pareto_front_trials(study: "optuna.study.Study", consider_constraint: bool = False) -> list[FrozenTrial]:
...
def _fast_non_domination_rank(
    ...
    if len(penalty) != len(loss_values):
        raise ValueError(
            "The length of penalty and loss_values must be same, but got " f"{len(penalty)=} and {len(loss_values)=}."
        )
    ...
    ranks[is_penalty_nan] = top_rank_penalty_nan + _calculate_nondomination_rank(loss_values[is_penalty_nan], n_below=n_below)
...
def _is_pareto_front_nd(unique_lexsorted_loss_values: np.ndarray) -> np.ndarray:
    ...
    while len(remaining_indices):
        ...
        nondominated_and_not_top = np.any(loss_values[remaining_indices] < loss_values[new_nondominated_index], axis=1)
...
def _calculate_nondomination_rank(loss_values: np.ndarray, *, n_below: int | None = None) -> np.ndarray:
