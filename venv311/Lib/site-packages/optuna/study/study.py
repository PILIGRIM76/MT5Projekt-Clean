...
from optuna.distributions import BaseDistribution, _convert_old_distribution_to_new_distribution
from optuna.storages._heartbeat import is_heartbeat_enabled
from optuna.study._constrained_optimization import _CONSTRAINTS_KEY, _get_feasible_trials
...
class Study:
    ...
    def ask(self, fixed_distributions: dict[str, BaseDistribution] | None = None) -> Trial:
        ...
        fixed_distributions = {
            key: _convert_old_distribution_to_new_distribution(dist) for key, dist in fixed_distributions.items()
        }
    ...
    def stop(self) -> None:
        ...
        if not self._thread_local.in_optimize_loop:
            raise RuntimeError("`Study.stop` is supposed to be invoked inside an objective function or a " "callback.")
    ...
    def set_metric_names(self, metric_names: list[str]) -> None:
        ...
        self._storage.set_study_system_attr(self._study_id, _SYSTEM_ATTR_METRIC_NAMES, metric_names)
    ...
    def _pop_waiting_trial_id(self) -> int | None:
        for trial in self._storage.get_all_trials(self._study_id, deepcopy=False, states=(TrialState.WAITING,)):
    ...
    def _should_skip_enqueue(self, params: Mapping[str, JSONSerializable]) -> bool:
        for trial in self.get_trials(deepcopy=False):
            ...
            for param_name, param_value in params.items():
                ...
                is_repeated = (
                    np.isnan(float(param_value)) or np.isclose(float(param_value), float(existing_param), atol=0.0)
    ...
    def _log_completed_trial(self, values: list[float], number: int, params: dict[str, Any]) -> None:
        ...
        if len(values) > 1:
            ...
            _logger.info(f"Trial {number} finished with values: {trial_values} and parameters: {params}.")
        elif len(values) == 1:
            ...
            message = f"Trial {number} finished with value: {trial_value} and parameters: {params}."
            try:
    ...
    *,
    ...
    if direction is None and directions is None:
        ...
        if isinstance(direction, Sequence) and not isinstance(direction, str):
            raise ValueError("Use `directions` instead of `direction` for multi-objective optimization.")
    ...
    elif any(d not in ["minimize", "maximize", StudyDirection.MINIMIZE, StudyDirection.MAXIMIZE] for d in directions):
        raise ValueError(
    ...
    direction_objects = [d if isinstance(d, StudyDirection) else StudyDirection[d.upper()] for d in directions]
    ...
    try:
        ...
        if load_if_exists:
            ...
            _logger.info(f"Using an existing study with name '{study_name}' instead of creating a new one.")
            study_id = storage.get_study_id_from_name(study_name)
        else:
            raise
...
def get_all_study_summaries(storage: str | storages.BaseStorage, include_best_trial: bool = True) -> list[StudySummary]:
    ...
    for s in frozen_studies:
        ...
        datetime_start = min([t.datetime_start for t in all_trials if t.datetime_start is not None], default=None)
