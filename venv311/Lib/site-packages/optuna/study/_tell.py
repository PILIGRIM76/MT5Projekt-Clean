...
import optuna
from optuna import logging, pruners
...
def _check_state_and_values(state: TrialState | None, values: float | Sequence[float] | None) -> None:
    if state == TrialState.COMPLETE:
        ...
        if values is not None:
            raise ValueError(
                "Values were told. Values cannot be specified when state is " "TrialState.PRUNED or TrialState.FAIL."
            )
...
def _check_values_are_feasible(study: Study, values: Sequence[float]) -> str | None:
    ...
    if len(study.directions) != len(values):
        return f"The number of the values {len(values)} did not match the number of the objectives " f"{len(study.directions)}"
