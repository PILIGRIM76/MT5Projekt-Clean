...
import optuna
from optuna import exceptions, logging
...
def _optimize(
    ...
    if not isinstance(catch, tuple):
        raise TypeError(f"The catch argument is of type '{type(catch).__name__}' but must be a tuple.")
        ...
        if n_jobs == 1:
            ...
            with ThreadPoolExecutor(max_workers=n_jobs) as executor:
                for n_submitted_trials in itertools.count():
                    ...
                    if timeout is not None and (datetime.datetime.now() - time_start).total_seconds() > timeout:
                        break
...
def _run_trial(
    ...
    if updated_state == TrialState.FAIL and func_err is not None and not isinstance(func_err, catch):
        raise func_err
...
def _log_failed_trial(
    ...
    _logger.warning(
        (f"Trial {trial_number} failed with parameters: {trial_params} " f"because of the following error: {message}."),
        exc_info=exc_info,
    )
