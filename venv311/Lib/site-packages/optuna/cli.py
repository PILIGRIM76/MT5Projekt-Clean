...
from optuna.storages import BaseStorage, JournalFileStorage, JournalRedisStorage, JournalStorage, RDBStorage
from optuna.storages.journal import JournalFileBackend, JournalRedisBackend
...
class _CreateStudy(_BaseCommand):
    ...
    def add_arguments(self, parser: ArgumentParser) -> None:
        ...
        parser.add_argument(
            ...
            default=False,
            action="store_true",
            help="If specified, the creation of the study is skipped " "without any error when the study name is duplicated.",
        )
        parser.add_argument(
            "--directions",
            type=str,
...
class _Studies(_BaseCommand):
    ...
    def take_action(self, parsed_args: Namespace) -> int:
        ...
        records = []
        for s in summaries:
            start = s.datetime_start.strftime(_DATETIME_FORMAT) if s.datetime_start is not None else None
        ...
        print(_format_output(records, self._study_list_header, parsed_args.format, parsed_args.flatten))
        return 0
...
class _BestTrial(_BaseCommand):
    ...
    def take_action(self, parsed_args: Namespace) -> int:
        ...
        records, columns = _dataframe._create_records_and_aggregate_column(study, attrs)
        print(_format_output(records[study.best_trial.number], columns, parsed_args.format, parsed_args.flatten))
        return 0
...
class _BestTrials(_BaseCommand):
    ...
    def take_action(self, parsed_args: Namespace) -> int:
        optuna_warn(
            "'best-trials' is an experimental CLI command. The interface can change in the " "future.",
            ExperimentalWarning,
        )
...
class _Tell(_BaseCommand):
    ...
    def add_arguments(self, parser: ArgumentParser) -> None:
        ...
        parser.add_argument(
            ...
            default=False,
            action="store_true",
            help="If specified, tell is skipped without any error when the trial is already " "finished.",
        )
...
def _parse_storage_class_without_suggesting_deprecated_choices(value: str) -> str:
    ...
    if value in choices + deprecated_choices:
        return value
    raise argparse.ArgumentTypeError(f"Invalid choice: {value}  (choose from {str(choices)[1:-1]})")
...
def _add_common_arguments(parser: ArgumentParser) -> ArgumentParser:
    parser.add_argument(
        "--storage",
        default=None,
        help=("DB URL. (e.g. sqlite:///example.db) " "Also can be specified via OPTUNA_STORAGE environment variable."),
    )
    parser.add_argument(
        "--storage-class",
...
def _add_commands(main_parser: ArgumentParser, parent_parser: ArgumentParser) -> dict[str, ArgumentParser]:
    subparsers = main_parser.add_subparsers()
    command_name_to_subparser = {}

    for command_name, command_type in _COMMANDS.items():
        command = command_type()
        subparser = subparsers.add_parser(command_name, parents=[parent_parser], help=inspect.getdoc(command_type))
    ...
    subparsers.add_parser("help", help="Show help message and exit.").set_defaults(handler=_print_help)
