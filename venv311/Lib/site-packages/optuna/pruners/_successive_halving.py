...
class SuccessiveHalvingPruner(BasePruner):
    ...
    def __init__(
        ...
        if isinstance(min_resource, str) and min_resource != "auto":
            raise ValueError(
                f"The value of `min_resource` is {min_resource}, " "but must be either `min_resource` >= 1 or 'auto'"
            )
        ...
        if reduction_factor < 2:
            raise ValueError(f"The value of `reduction_factor` is {reduction_factor}, " "but must be `reduction_factor >= 2`")
        ...
        if bootstrap_count < 0:
            raise ValueError("The value of `bootstrap_count` is " f"{bootstrap_count}, but must be `bootstrap_count >= 0`")
