...
class HyperbandPruner(BasePruner):
    ...
    def __init__(
        ...
        if not isinstance(self._max_resource, int) and self._max_resource != "auto":
            raise ValueError("The 'max_resource' should be integer or 'auto'. " f"But max_resource = {self._max_resource}")
    ...
    def _get_bracket_id(self, study: "optuna.study.Study", trial: "optuna.trial.FrozenTrial") -> int:
        ...
        assert self._n_brackets is not None
        n = binascii.crc32(f"{study.study_name}_{trial.number}".encode()) % self._total_trial_allocation_budget
        for bracket_id in range(self._n_brackets):
