...
from optuna.distributions import BaseDistribution, check_distribution_compatibility, distribution_to_json, json_to_distribution
from optuna.exceptions import DuplicatedStudyError, UpdateFinishedTrialError
from optuna.storages import BaseStorage
from optuna.storages._base import DEFAULT_STUDY_NAME_PREFIX
from optuna.storages.journal._base import BaseJournalBackend, BaseJournalSnapshot
...
class JournalStorage(BaseStorage):
    ...
    def create_new_study(self, directions: Sequence[StudyDirection], study_name: str | None = None) -> int:
        ...
        with self._thread_lock:
            self._write_log(JournalOperation.CREATE_STUDY, {"study_name": study_name, "directions": directions})
            self._sync_with_backend()
            ...
            for frozen_study in self._replay_result.get_all_studies():
                ...
                # Dump snapshot here.
                if isinstance(self._backend, BaseJournalSnapshot) and study_id != 0 and study_id % SNAPSHOT_INTERVAL == 0:
    ...
    def create_new_trial(self, study_id: int, template_trial: FrozenTrial | None = None) -> int:
        ...
        if template_trial:
            ...
            if template_trial.datetime_start:
                log["datetime_start"] = template_trial.datetime_start.isoformat(timespec="microseconds")
            else:
                log["datetime_start"] = None
            if template_trial.datetime_complete:
                log["datetime_complete"] = template_trial.datetime_complete.isoformat(timespec="microseconds")

            log["distributions"] = {k: distribution_to_json(dist) for k, dist in template_trial.distributions.items()}
            log["params"] = {
                k: template_trial.distributions[k].to_internal_repr(param) for k, param in template_trial.params.items()
            }
        ...
        with self._thread_lock:
            ...
            # Dump snapshot here.
            if isinstance(self._backend, BaseJournalSnapshot) and trial_id != 0 and trial_id % SNAPSHOT_INTERVAL == 0:
                self._backend.save_snapshot(pickle.dumps(self._replay_result))
        return trial_id
    ...
    def set_trial_state_values(self, trial_id: int, state: TrialState, values: Sequence[float] | None = None) -> bool:
        ...
        with self._thread_lock:
            if state == TrialState.RUNNING:
                ...
                assert existing_trial is not None, "Please report your bug on GitHub if this line fails your script."
                if existing_trial.state.is_finished():
                    raise UpdateFinishedTrialError(UNUPDATABLE_MSG.format(trial_number=existing_trial.number))
...
class JournalStorageReplayResult:
    ...
    def _apply_set_trial_param(self, log: dict[str, Any]) -> None:
        ...
        for prev_trial_id in self._study_id_to_trial_ids[study_id]:
            ...
            if param_name in prev_trial.params.keys():
                try:
                    check_distribution_compatibility(prev_trial.distributions[param_name], distribution)
                except Exception:
                    if self._is_issued_by_this_worker(log):
                        raise
                    return
    ...
    def _trial_exists_and_updatable(self, trial_id: int, log: dict[str, Any]) -> bool:
        if trial_id not in self._trials:
            ...
            if self._is_issued_by_this_worker(log):
                raise UpdateFinishedTrialError(UNUPDATABLE_MSG.format(trial_number=self._trials[trial_id].number))
            return False
        else:
            return True
