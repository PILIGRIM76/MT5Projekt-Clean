...
import optuna
from optuna import distributions, version
...
class RDBStorage(BaseStorage, BaseHeartbeat):
    ...
    def __init__(
        ...
        *,
        ...
        try:
            ...
            raise ImportError(
                "Failed to import DB access module for the specified storage URL. " "Please install appropriate one."
            ) from e

        self.scoped_session = sqlalchemy_orm.scoped_session(sqlalchemy_orm.sessionmaker(bind=self.engine))
    ...
    def create_new_study(self, directions: Sequence[StudyDirection], study_name: str | None = None) -> int:
        try:
    ...
    def set_study_user_attr(self, study_id: int, key: str, value: Any) -> None:
        with _create_scoped_session(self.scoped_session, True) as session:
            ...
            if attribute is None:
                attribute = models.StudyUserAttributeModel(study_id=study_id, key=key, value_json=json.dumps(value))
    ...
    def set_study_system_attr(self, study_id: int, key: str, value: JSONSerializable) -> None:
        with _create_scoped_session(self.scoped_session, True) as session:
            ...
            if attribute is None:
                attribute = models.StudySystemAttributeModel(study_id=study_id, key=key, value_json=json.dumps(value))
                session.add(attribute)
            else:
                attribute.value_json = json.dumps(value)
    ...
    def _create_new_trial(self, study_id: int, template_trial: FrozenTrial | None = None) -> FrozenTrial:
        ...
        def _create_frozen_trial(trial: "models.TrialModel", template_trial: FrozenTrial | None) -> FrozenTrial:
            if template_trial:
    ...
    def _get_prepared_new_trial(
        ...
        if template_trial is not None:
            ...
            for key, value in template_trial.user_attrs.items():
                self._set_trial_attr_without_commit(session, models.TrialUserAttributeModel, trial.trial_id, key, value)

            for key, value in template_trial.system_attrs.items():
                self._set_trial_attr_without_commit(session, models.TrialSystemAttributeModel, trial.trial_id, key, value)

            for step, intermediate_value in template_trial.intermediate_values.items():
                self._set_trial_intermediate_value_without_commit(session, trial.trial_id, step, intermediate_value)
    ...
    def _set_trial_param_without_commit(
        ...
        if previous_distribution is None:
            ...
            if previous_record is not None:
                previous_distribution = distributions.json_to_distribution(previous_record.distribution_json)
    ...
    def get_trial_param(self, trial_id: int, param_name: str) -> float:
        with _create_scoped_session(self.scoped_session) as session:
            trial = models.TrialModel.find_or_raise_by_id(trial_id, session)
            trial_param = models.TrialParamModel.find_or_raise_by_trial_and_param_name(trial, param_name, session)
            param_value = trial_param.param_value

        return param_value

    def set_trial_state_values(self, trial_id: int, state: TrialState, values: Sequence[float] | None = None) -> bool:
        try:
            with _create_scoped_session(self.scoped_session) as session:
    ...
    def _set_trial_intermediate_value_without_commit(
        ...
        (
            stored_value,
            value_type,
        ) = models.TrialIntermediateValueModel.intermediate_value_to_stored_repr(intermediate_value)
        trial_intermediate_value = models.TrialIntermediateValueModel.find_by_trial_and_step(trial, step, session)
    ...
    def _set_trial_attr_without_commit(
        ...
        if self.engine.name == "mysql":
            ...
            mysql_upsert_stmt = mysql_insert_stmt.on_duplicate_key_update(value_json=mysql_insert_stmt.inserted.value_json)
    ...
    def get_trial_id_from_study_id_trial_number(self, study_id: int, trial_number: int) -> int:
        with _create_scoped_session(self.scoped_session) as session:
            ...
            if trial_id is None:
                raise KeyError(f"No trial with trial number {trial_number} " f"exists in study with study_id {study_id}.")
            return trial_id[0]
    ...
    def _get_trials(
        ...
        included_trial_ids = set(trial_id for trial_id in included_trial_ids if trial_id <= trial_id_greater_than)
        ...
        with _create_scoped_session(self.scoped_session) as session:
            ...
            try:
                ...
                _logger.warning(f"Caught an error from sqlalchemy: {e!s}. " "Falling back to a slower alternative.")

                trial_models = query.order_by(models.TrialModel.trial_id).all()
                trial_models = [
                    t for t in trial_models if t.trial_id in included_trial_ids or t.trial_id > trial_id_greater_than
                ]
    ...
    def _build_frozen_trial_from_trial_model(self, trial: "models.TrialModel") -> FrozenTrial:
        ...
        return FrozenTrial(
            ...
            params={
                p.param_name: distributions.json_to_distribution(p.distribution_json).to_external_repr(p.param_value)
                for p in params
            },
            distributions={p.param_name: distributions.json_to_distribution(p.distribution_json) for p in params},
...
class _VersionManager:
    ...
    def check_table_schema_compatibility(self) -> None:
        ...
        if current_version in known_versions:
            message += "Please execute `$ optuna storage upgrade --storage $STORAGE_URL` " "for upgrading the storage."
        else:
            message += "Please try updating optuna to the latest version by `$ pip install -U optuna`."
