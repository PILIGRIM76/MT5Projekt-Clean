...
from optuna.exceptions import DuplicatedStudyError, UpdateFinishedTrialError
...
if TYPE_CHECKING:
    import grpc
    from optuna.storages._grpc.auto_generated import api_pb2, api_pb2_grpc
else:
...
class OptunaStorageProxyService(api_pb2_grpc.StorageServiceServicer):
    ...
    def CreateNewStudy(
        ...
        directions = [
            StudyDirection.MINIMIZE if d == api_pb2.MINIMIZE else StudyDirection.MAXIMIZE for d in request.directions
        ]
    ...
    def SetStudyUserAttribute(
        ...
        try:
            self._backend.set_study_user_attr(request.study_id, request.key, json.loads(request.value))
        except KeyError as e:
            context.abort(code=grpc.StatusCode.NOT_FOUND, details=str(e))
    ...
    def SetStudySystemAttribute(
        ...
        try:
            self._backend.set_study_system_attr(request.study_id, request.key, json.loads(request.value))
        except KeyError as e:
            context.abort(code=grpc.StatusCode.NOT_FOUND, details=str(e))
    ...
    def GetStudyDirections(
        ...
        return api_pb2.GetStudyDirectionsReply(
            directions=[api_pb2.MINIMIZE if d == StudyDirection.MINIMIZE else api_pb2.MAXIMIZE for d in directions]
        )
        ...
        return api_pb2.GetAllStudiesReply(
            studies=[
                api_pb2.Study(
                    ...
                    directions=[
                        api_pb2.MINIMIZE if d == StudyDirection.MINIMIZE else api_pb2.MAXIMIZE for d in study.directions
                    ],
                    user_attributes={key: json.dumps(value) for key, value in study.user_attrs.items()},
                    system_attributes={key: json.dumps(value) for key, value in study.system_attrs.items()},
                )
    ...
    def SetTrialStateValues(
        ...
        try:
            trial_updated = self._backend.set_trial_state_values(trial_id, _from_proto_trial_state(state), values)
        except KeyError as e:
            context.abort(code=grpc.StatusCode.NOT_FOUND, details=str(e))
        except UpdateFinishedTrialError as e:
            context.abort(code=grpc.StatusCode.FAILED_PRECONDITION, details=str(e))
    ...
    def GetTrials(
        ...
        filtered_trials = [
            _to_proto_trial(t) for t in trials if t._trial_id > trial_id_greater_than or t._trial_id in included_trial_ids
        ]
...
def _to_proto_trial(trial: FrozenTrial) -> api_pb2.Trial:
    ...
    return api_pb2.Trial(
        ...
        values=trial.values,
        datetime_start=(trial.datetime_start.strftime(DATETIME_FORMAT) if trial.datetime_start else ""),
        datetime_complete=(trial.datetime_complete.strftime(DATETIME_FORMAT) if trial.datetime_complete else ""),
        distributions={key: distribution_to_json(distribution) for key, distribution in trial.distributions.items()},
        params=params,
...
def _from_proto_trial(trial: api_pb2.Trial) -> FrozenTrial:
    datetime_start = datetime.strptime(trial.datetime_start, DATETIME_FORMAT) if trial.datetime_start else None
    datetime_complete = datetime.strptime(trial.datetime_complete, DATETIME_FORMAT) if trial.datetime_complete else None
    distributions = {key: json_to_distribution(value) for key, value in trial.distributions.items()}
    params = {}
    for key, value in trial.params.items():
