...
class BasicSamplerTestCase(_BaseSamplerTestCase):
    ...
    def test_float(
        ...
        points = np.array(
            [study.sampler.sample_independent(study, _create_new_trial(study), "x", distribution) for _ in range(100)]
        )
    ...
    def test_categorical(self, sampler: Callable[[], BaseSampler], choices: Sequence[CategoricalChoiceType]) -> None:
    ...
    def test_conditional_sample_independent(self, sampler: Callable[[], BaseSampler]) -> None:
        ...
        category = study.sampler.sample_independent(study, _trial, "category", categorical_distribution)
    ...
    def test_dynamic_range_objective(self, sampler: Callable[[], BaseSampler], second_low: int, second_high: int) -> None:
...
class RelativeSamplerTestCase(_BaseSamplerTestCase):
    ...
    def test_sample_relative_mixed(self, sampler: Callable[[], BaseSampler], x_distribution: BaseDistribution) -> None:
        search_space: dict[str, BaseDistribution] = dict(x=x_distribution, y=CategoricalDistribution([-1, -10, -100]))
        study = optuna.study.create_study(sampler=sampler())
        trial = study.ask(search_space)
        study.tell(trial, sum(trial.params.values()))
...
class SingleOnlySamplerTestCase(_BaseSamplerTestCase):
    def test_raise_error_for_samplers_during_multi_objectives(
        ...
        with pytest.raises(ValueError):
            trial = _create_new_trial(study)
            study.sampler.sample_relative(study, trial, study.sampler.infer_relative_search_space(study, trial))
