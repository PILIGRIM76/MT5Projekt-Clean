...
class StorageTestCase:
    ...
    def test_set_and_get_study_directions(self, storage: BaseStorage) -> None:
        for target in [
            ...
            def check_get() -> None:
                ...
                assert got_directions == list(target), "Direction of a study should be a tuple of `StudyDirection` objects."
    ...
    def test_set_trial_state_values_for_state(self, storage: BaseStorage) -> None:
        ...
        for trial_id, state in zip(trial_ids, ALL_STATES):
            ...
            storage.set_trial_state_values(trial_id, state=state, values=(0.0,) if state.is_finished() else None)
        ...
        for _, trial_id_to_trial in study_to_trials.items():
            for trial_id, expected_trial in trial_id_to_trial.items():
                ...
                for key in expected_trial.params.keys():
                    assert storage.get_trial_param(trial_id, key) == expected_trial.distributions[key].to_internal_repr(
                        expected_trial.params[key]
                    )

        non_existent_trial_id = max(tid for ts in study_to_trials.values() for tid in ts.keys()) + 1
        with pytest.raises(KeyError):
    ...
    def test_set_trial_param(self, storage: BaseStorage) -> None:
        ...
        trial_id_3 = storage.create_new_trial(storage.create_new_study(directions=[StudyDirection.MINIMIZE]))
        ...
        with pytest.raises(ValueError):
            storage.set_trial_param(trial_id_2, "y", 2, CategoricalDistribution(choices=("Meguro", "Shibuya", "Ebisu")))
    ...
    def test_set_trial_state_values_for_values(self, storage: BaseStorage) -> None:
        ...
        with pytest.raises(KeyError):
            storage.set_trial_state_values(non_existent_trial_id, state=TrialState.COMPLETE, values=(1,))
    ...
    def test_set_trial_user_attr(self, storage: BaseStorage) -> None:
        trial_id_1 = storage.create_new_trial(storage.create_new_study(directions=[StudyDirection.MINIMIZE]))
        ...
        # Test another trial.
        trial_id_2 = storage.create_new_trial(storage.create_new_study(directions=[StudyDirection.MINIMIZE]))
    ...
    def test_get_all_trials_params_order(self, storage: BaseStorage, param_names: list[str]) -> None:
        study_id = storage.create_new_study(directions=[StudyDirection.MINIMIZE])
    ...
    def test_get_all_trials_state_option(self, storage: BaseStorage) -> None:
        ...
        other_states = [s for s in ALL_STATES if s != TrialState.COMPLETE and s != TrialState.PRUNED]
    ...
    def test_get_best_trial(self, storage: BaseStorage, direction: StudyDirection, values: list[float]) -> None:
    ...
    def test_get_best_trial_for_multi_objective_optimization(self, storage: BaseStorage) -> None:
        study_id = storage.create_new_study(directions=(StudyDirection.MAXIMIZE, StudyDirection.MINIMIZE))
    ...
    def test_get_trial_id_from_study_id_trial_number(self, storage: BaseStorage) -> None:
        ...
        assert trial_id == storage.get_trial_id_from_study_id_trial_number(study_id, trial_number=0)
...
def _test_set_and_get_trial_user_attr_for_floats(storage: BaseStorage) -> None:
    trial_id = storage.create_new_trial(storage.create_new_study(directions=[StudyDirection.MINIMIZE]))
