...
import fakeredis
import optuna
from optuna.storages import BaseStorage, GrpcStorageProxy
...
class StorageSupplier(AbstractContextManager):
    ...
    def __enter__(
        ...
        if self.storage_specifier == "inmemory":
            ...
            self.storage = optuna.storages._CachedStorage(rdb_storage) if "cached" in self.storage_specifier else rdb_storage
        elif self.storage_specifier == "journal_redis":
            journal_redis_storage = optuna.storages.journal.JournalRedisBackend("redis://localhost")
            journal_redis_storage._redis = self.extra_args.get("redis", fakeredis.FakeStrictRedis())
            ...
            storage = optuna.storages.JournalStorage(optuna.storages.journal.JournalFileBackend(self.tempfile.name))
            self.storage = self._create_proxy(storage, thread_pool=self.extra_args.get("thread_pool"))
    ...
    def _create_proxy(self, storage: BaseStorage, thread_pool: ThreadPoolExecutor | None = None) -> GrpcStorageProxy:
        with _lock_to_search_for_free_port():
            port = _find_free_port()
            self.server = optuna.storages._grpc.server.make_server(storage, "localhost", port, thread_pool=thread_pool)
