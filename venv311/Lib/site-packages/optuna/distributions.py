...
class FloatDistribution(BaseDistribution):
    ...
    def __init__(self, low: float, high: float, log: bool = False, step: None | float = None) -> None:
    ...
    def to_internal_repr(self, param_value_in_external_repr: float) -> float:
        try:
            ...
            raise ValueError(
                f"'{param_value_in_external_repr}' is not a valid type. " "float-castable value is expected."
            ) from e
        ...
        if self.log and internal_repr <= 0.0:
            raise ValueError(f"`{param_value_in_external_repr}` is invalid value for the case log=True.")
...
class IntDistribution(BaseDistribution):
    ...
    def __init__(self, low: int, high: int, log: bool = False, step: int = 1) -> None:
        if log and step != 1:
            raise ValueError("Samplers and other components in Optuna only accept step is 1 " "when `log` argument is True.")
...
def check_distribution_compatibility(dist_old: BaseDistribution, dist_new: BaseDistribution) -> None:
    ...
    if dist_old != dist_new:
        raise ValueError(CategoricalDistribution.__name__ + " does not support dynamic value space.")
