...
def warn_experimental_argument(option_name: str) -> None:
    optuna_warn(
        f"Argument ``{option_name}`` is an experimental feature." " The interface can change in the future.",
        ExperimentalWarning,
    )
...
def _validate_version(version: str) -> None:
    if not isinstance(version, str) or len(version.split(".")) != 3:
        raise ValueError(f"Invalid version specification. Must follow `x.y.z` format but `{version}` is given")
...
def experimental_func(
    ...
    def decorator(func: Callable[FP, FT]) -> Callable[FP, FT]:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> FT:
            warnings.warn(
                f"{_name} is experimental (supported from v{version}). " "The interface can change in the future.",
                ExperimentalWarning,
                stacklevel=2,
            )
