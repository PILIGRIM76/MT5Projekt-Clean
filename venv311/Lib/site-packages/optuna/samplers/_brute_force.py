...
class _TreeNode:
    ...
    def add_path(self, params_and_search_spaces: Iterable[tuple[str, Iterable[float], float]]) -> _TreeNode | None:
    ...
    def sample_child(self, rng: np.random.RandomState, exclude_running: bool) -> float:
        ...
        if any(not value.is_running and weights[i] > 0 for i, value in enumerate(self.children.values())):
...
class BruteForceSampler(BaseSampler):
    ...
    def sample_independent(
        ...
        if tree.count_unexpanded(exclude_running) == 0:
            ...
            return param_distribution.to_external_repr(tree.sample_child(self._rng.rng, exclude_running))
...
def _enumerate_candidates(param_distribution: BaseDistribution) -> Sequence[float]:
    if isinstance(param_distribution, FloatDistribution):
        if param_distribution.step is None:
            raise ValueError(
                "FloatDistribution.step must be given for BruteForceSampler" " (otherwise, the search space will be infinite)."
            )
