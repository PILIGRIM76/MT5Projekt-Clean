...
def _constrained_dominates(trial0: FrozenTrial, trial1: FrozenTrial, directions: Sequence[StudyDirection]) -> bool:
    ...
    if constraints0 is None:
        optuna_warn(f"Trial {trial0.number} does not have constraint values." " It will be dominated by the other trials.")

    if constraints1 is None:
        optuna_warn(f"Trial {trial1.number} does not have constraint values." " It will be dominated by the other trials.")
...
def _validate_constraints(
    ...
    *,
    ...
    num_constraints = max([len(t.system_attrs.get(_CONSTRAINTS_KEY, [])) for t in population], default=0)
    for _trial in population:
        _constraints = _trial.system_attrs.get(_CONSTRAINTS_KEY)
        if _constraints is None:
            optuna_warn(f"Trial {_trial.number} does not have constraint values." " It will be dominated by the other trials.")
            continue
