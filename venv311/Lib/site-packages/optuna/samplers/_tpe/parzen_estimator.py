...
class _ParzenEstimatorParameters(NamedTuple):
    ...
    multivariate: bool
    categorical_distance_func: dict[str, Callable[[CategoricalChoiceType, CategoricalChoiceType], float]]
...
class _ParzenEstimator:
    def __init__(
        ...
        if parameters.prior_weight < 0:
            raise ValueError("A non-negative value must be specified for prior_weight," f" but got {parameters.prior_weight}.")
        ...
        self._mixture_distribution = _MixtureOfProductDistribution(
            weights=weights,
            distributions=[
                self._calculate_distributions(transformed_observations[:, i], param, search_space[param], parameters)
    ...
    def _calculate_distributions(
        ...
        if isinstance(search_space, CategoricalDistribution):
            return self._calculate_categorical_distributions(observations, param_name, search_space, parameters)
        else:
    ...
    def _calculate_categorical_distributions(
        ...
        if len(observations) == 0:
            return _BatchedCategoricalDistributions(weights=np.full((1, n_choices), fill_value=1.0 / n_choices))
    ...
    def _calculate_numerical_distributions(
        ...
        def compute_sigmas() -> np.ndarray:
            if parameters.multivariate:
                ...
                sigma = SIGMA0_MAGNITUDE * max(len(observations), 1) ** (-1.0 / (len(self._search_space) + 4)) * (high - low)
        ...
        if search_space.step is None:
            if not search_space.log:
                return _BatchedTruncNormDistributions(mus, sigmas, search_space.low, search_space.high)
            else:
                return _BatchedTruncLogNormDistributions(mus, sigmas, search_space.low, search_space.high)
        else:
            if not search_space.log:
