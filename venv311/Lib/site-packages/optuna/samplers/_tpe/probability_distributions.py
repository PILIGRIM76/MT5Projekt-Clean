...
class _MixtureOfProductDistribution(NamedTuple):
    ...
    def sample(self, rng: np.random.RandomState, batch_size: int) -> np.ndarray:
        ...
        if len(numerical_dists):
            ...
            ret[:, disc_inds] = np.clip(low_d + np.round((ret[:, disc_inds] - low_d) / step_d) * step_d, low_d, high_d)
    ...
    def log_pdf(self, x: np.ndarray) -> np.ndarray:
        ...
        cont_dists: list[
            _BatchedTruncNormDistributions | _BatchedTruncLogNormDistributions | _BatchedDiscreteTruncLogNormDistributions
        ] = []
        x_cont = []
        lows_cont = []
        highs_cont = []
        for i, d in enumerate(self.distributions):
            if isinstance(d, _BatchedCategoricalDistributions):
                ...
                weighted_log_pdf += np.log(np.take_along_axis(d.weights[np.newaxis], xi, axis=-1))[..., 0]
            elif isinstance(d, _BatchedTruncNormDistributions):
                cont_dists.append(d)
                x_cont.append(x[:, i])
