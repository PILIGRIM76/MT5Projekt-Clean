...
from optuna.search_space.group_decomposed import _GroupDecomposedSearchSpace, _SearchSpaceGroup
from optuna.study._multi_objective import _fast_non_domination_rank, _is_pareto_front
...
class TPESampler(BaseSampler):
    ...
    def __init__(
        ...
        *,
        ...
        categorical_distance_func: dict[str, Callable[[CategoricalChoiceType, CategoricalChoiceType], float]] | None = None,
    ) -> None:
        if not consider_prior:
            msg = _deprecated._DEPRECATION_WARNING_TEMPLATE.format(name="`consider_prior`", d_ver="4.3.0", r_ver="6.0.0")
            optuna_warn(
        ...
        if group:
            if not multivariate:
                raise ValueError("``group`` option can only be enabled when ``multivariate`` is enabled.")
    ...
    def infer_relative_search_space(self, study: Study, trial: FrozenTrial) -> dict[str, BaseDistribution]:
        ...
        if self._group:
            ...
            self._search_space_group = self._group_decomposed_search_space.calculate(study, use_trial_cache)
    ...
    def _sample_relative(self, study: Study, trial: FrozenTrial, search_space: dict[str, BaseDistribution]) -> dict[str, Any]:
        if search_space == {}:
            return {}
    ...
    def _sample(self, study: Study, trial: FrozenTrial, search_space: dict[str, BaseDistribution]) -> dict[str, Any]:
    ...
    def _build_parzen_estimator(
        ...
        if handle_below and study._is_multi_objective():
            ...
            weights_below = _calculate_weights_below_for_multi_objective(study, trials, self._constraints_func)[
                param_mask_below
            ]
            assert np.isfinite(weights_below).all()
            mpe = self._parzen_estimator_cls(observations, search_space, self._parzen_estimator_parameters, weights_below)
        else:
            mpe = self._parzen_estimator_cls(observations, search_space, self._parzen_estimator_parameters)
    ...
    @classmethod
    def _compare(cls, samples: dict[str, np.ndarray], acquisition_func_vals: np.ndarray) -> dict[str, int | float]:
...
def _split_infeasible_trials(trials: Sequence[FrozenTrial], n_below: int) -> tuple[list[FrozenTrial], list[FrozenTrial]]:
    n_below = min(n_below, len(trials))
...
def _calculate_weights_below_for_multi_objective(
    ...
    if len(study.directions) <= 3:
        contribs[on_front] = [hv - compute_hypervolume(pareto_sols[loo], ref_point, assume_pareto=True) for loo in loo_mat]
    else:
        contribs[on_front] = np.prod(ref_point - pareto_sols, axis=-1)
        limited_sols = np.maximum(pareto_sols, pareto_sols[:, np.newaxis])
        contribs[on_front] -= [compute_hypervolume(limited_sols[i, loo], ref_point) for i, loo in enumerate(loo_mat)]
