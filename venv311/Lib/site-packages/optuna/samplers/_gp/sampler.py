...
class GPSampler(BaseSampler):
    ...
    def _optimize_acqf(self, acqf: acqf_module.BaseAcquisitionFunc, best_params: np.ndarray | None) -> np.ndarray:
    ...
    def _get_constraints_acqf_args(
        ...
        if (
            ...
            and len(self._gprs_cache_list[0].inverse_squared_lengthscales) != internal_search_space.dim
        ):
        ...
        for i, vals in enumerate(standardized_constraint_vals.T):
            cache = self._constraints_gprs_cache_list[i] if self._constraints_gprs_cache_list is not None else None
    ...
    def _get_best_params_for_multi_objective(
        ...
        pareto_params = normalized_params[_is_pareto_front(-standardized_score_vals, assume_unique_lexsorted=False)]
    ...
    def _get_normalized_params_of_running_trials(
        ...
        running_trials = [
            t
            for t in trials
            if internal_search_space._optuna_search_space.keys() <= _get_params(t).keys() and t.state == TrialState.RUNNING
        ]
        if len(running_trials) == 0:
            return None
        return internal_search_space.get_normalized_params(running_trials, [_get_params(t) for t in running_trials])
    ...
    def _sample_relative_impl(
        ...
        standardized_score_vals, _, _ = _standardize_values(_sign * np.array([trial.values for trial in completed_trials]))
            ...
            if n_objectives == 1:
                ...
                acqf = acqf_module.LogEI(
                    ...
                    normalized_params_of_running_trials=(
                        self._get_normalized_params_of_running_trials(trials, internal_search_space)
                    ),
                )
                ...
                best_params = self._get_best_params_for_multi_objective(normalized_params, standardized_score_vals)
        else:
            if n_objectives == 1:
                assert len(gprs_list) == 1
                constraint_vals, is_feasible = _get_constraint_vals_and_feasibility(study, completed_trials)
                ...
                best_params = None if np.isneginf(best_feasible_y) else normalized_params[i_opt, np.newaxis]
            else:
                constraint_vals, is_feasible = _get_constraint_vals_and_feasibility(study, completed_trials)
                constr_gpr_list, constr_threshold_list = self._get_constraints_acqf_args(
                    constraint_vals, internal_search_space, normalized_params
...
def _get_constraint_vals_and_feasibility(study: Study, trials: list[FrozenTrial]) -> tuple[np.ndarray, np.ndarray]:
    _constraint_vals = [study._storage.get_trial_system_attrs(trial._trial_id).get(_CONSTRAINTS_KEY, ()) for trial in trials]
