...
class NSGAIIIElitePopulationSelectionStrategy:
    ...
    def __call__(self, study: Study, population: list[FrozenTrial]) -> list[FrozenTrial]:
        ...
        population_per_rank = _rank_population(population, study.directions, is_constrained=self._constraints_func is not None)
        ...
        for population in population_per_rank:
            if len(elite_population) + len(population) < self._population_size:
                ...
                if self._reference_points is None:
                    self._reference_points = _generate_default_reference_point(n_objectives, self._dividing_parameter)
                ...
                objective_matrix = _normalize_objective_values(_filter_inf(elite_population + population))
                (
                    closest_reference_points,
                    distance_reference_points,
                ) = _associate_individuals_with_reference_points(objective_matrix, self._reference_points)
...
def _generate_default_reference_point(n_objectives: int, dividing_parameter: int = 3) -> np.ndarray:
    ...
    indices = np.array(list(combinations_with_replacement(range(n_objectives), dividing_parameter)))
...
def _preserve_niche_individuals(
    ...
    if len(population) < target_population_size:
        raise ValueError("The population size must be greater than or equal to the target population size.")
    ...
    for i, reference_point_idx in enumerate(closest_reference_points[elite_population_num:]):
        ...
        reference_point_to_borderline_population[reference_point_idx].append((distance_reference_points[population_idx], i))
    ...
    for reference_point_idx in reference_point_to_borderline_population:
        ...
        nearest_points_count_to_reference_points[elite_population_count].append(reference_point_idx)
    ...
    while len(additional_elite_population) < target_population_size:
        ...
        _, selected_individual_id = reference_point_to_borderline_population[reference_point_idx].pop()
