    ...
    def transform(self, params: dict[str, Any]) -> np.ndarray:
        ...
        for name, distribution in self._search_space.items():
            ...
            if isinstance(distribution, CategoricalDistribution):
                ...
                trans_params[bound_idx] = _transform_numerical_param(param, distribution, self._transform_log)
                bound_idx += 1

        if self._transform_0_1:
            single_mask = self._raw_bounds[:, 0] == self._raw_bounds[:, 1]
            trans_params[single_mask] = 0.5
            trans_params[~single_mask] = (trans_params[~single_mask] - self._raw_bounds[~single_mask, 0]) / (
    ...
    def untransform(self, trans_params: np.ndarray) -> dict[str, Any]:
        ...
        for (name, distribution), encoded_columns in zip(self._search_space.items(), self.column_to_encoded_columns):
            ...
            if isinstance(distribution, CategoricalDistribution):
                ...
                param = _untransform_numerical_param(trans_param.item(), distribution, self._transform_log)
...
def _transform_search_space(
    ...
    n_bounds = sum(len(d.choices) if isinstance(d, CategoricalDistribution) else 1 for d in search_space.values())
...
def _transform_numerical_param(param: int | float, distribution: BaseDistribution, transform_log: bool) -> float:
    d = distribution
...
def _untransform_numerical_param(trans_param: float, distribution: BaseDistribution, transform_log: bool) -> int | float:
    d = distribution
    ...
    if isinstance(d, CategoricalDistribution):
        ...
        if d.log:
            ...
            param = float(np.clip(np.round((trans_param - d.low) / d.step) * d.step + d.low, d.low, d.high))
        else:
            if d.single():
                param = trans_param
            else:
        ...
        if d.log:
            ...
            param = int(np.clip(np.round((trans_param - d.low) / d.step) * d.step + d.low, d.low, d.high))
    else:
        assert False, "Should not reach. Unexpected distribution."
