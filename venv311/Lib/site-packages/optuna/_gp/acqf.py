...
def standard_logei(z: torch.Tensor) -> torch.Tensor:
    ...
    out = (
        (z_half := 0.5 * z) * torch.special.erfc(-_SQRT_HALF * z) + (-z_half * z).exp() * _INV_SQRT_2PI  # z * cdf(z)  # pdf(z)
    ).log()
...
class LogEI(BaseAcquisitionFunc):
    def __init__(
        ...
        if normalized_params_of_running_trials is not None:
            normalized_params_of_running_trials_tensor = torch.from_numpy(normalized_params_of_running_trials)
            ...
            constant_liar_y = constant_liar_value.expand(normalized_params_of_running_trials_tensor.shape[0])
...
class LogEHVI(BaseAcquisitionFunc):
    def __init__(
        ...
        self._non_dominated_box_intervals = (non_dominated_box_upper_bounds - self._non_dominated_box_lower_bounds).clamp_min_(
            _EPS
        )
