...
class GPRegressor:
    def __init__(
        ...
        if self._is_categorical.any():
            self._squared_X_diff[..., self._is_categorical] = (self._squared_X_diff[..., self._is_categorical] > 0.0).type(
                torch.float64
            )
    ...
    def _cache_matrix(self) -> None:
        assert self._cov_Y_Y_chol is None and self._cov_Y_Y_inv_Y is None, "Cannot call cache_matrix more than once."
        with torch.no_grad():
    ...
    def append_running_data(self, X_running: torch.Tensor, y_running: torch.Tensor) -> None:
        ...
        cov_Y_Y_chol[n_train:, n_train:] = np.linalg.cholesky(
            kernel_running_running - cov_Y_Y_chol[n_train:, :n_train] @ cov_Y_Y_chol[n_train:, :n_train].T
        )
    ...
    def kernel(self, X1: torch.Tensor | None = None, X2: torch.Tensor | None = None) -> torch.Tensor:
        """
        ...
        if X1 is None:
            ...
            if self._is_categorical.any():
                sqd[..., self._is_categorical] = (sqd[..., self._is_categorical] > 0.0).type(torch.float64)
