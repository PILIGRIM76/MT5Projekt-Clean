...
class SearchSpace:
    ...
    def get_normalized_params(
        ...
        for i, (param, distribution) in enumerate(self._optuna_search_space.items()):
            if isinstance(distribution, CategoricalDistribution):
                values[:, i] = [distribution.to_internal_repr(trial_param[param]) for trial_param in trial_params]
            else:
...
def _sample_normalized_params(n: int, search_space: SearchSpace, rng: np.random.RandomState | None) -> np.ndarray:
