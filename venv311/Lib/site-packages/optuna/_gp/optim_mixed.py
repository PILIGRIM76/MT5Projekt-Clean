...
def _gradient_ascent_batched(
    ...
    def negative_acqf_with_grad(scaled_x: np.ndarray, fixed_params: list[np.ndarray]) -> tuple[np.ndarray, np.ndarray]:
...
def local_search_mixed_batched(
    ...
    for _ in range(max_iter):
        ...
        for i, choices, xtol in zip(discrete_indices, choices_of_discrete_params, discrete_xtols):
            ...
            best_xs[remaining_inds], best_fvals[remaining_inds], updated = _local_search_discrete_batched(
...
def optimize_acqf_mixed(
    ...
    *,
    ...
    n_additional_warmstart = min(n_local_search - len(warmstart_normalized_params_array) - 1, n_non_zero_probs_improvement)
    ...
    if n_additional_warmstart > 0:
        additional_idxs = rng.choice(len(sampled_xs), size=n_additional_warmstart, replace=False, p=probs)
