...
from optuna.terminator.improvement.evaluator import BaseImprovementEvaluator, _compute_standardized_regret_bound
...
if TYPE_CHECKING:
    ...
    from optuna._gp import gp, prior
...
class EMMREvaluator(BaseImprovementEvaluator):
    ...
    def evaluate(self, trials: list[FrozenTrial], study_direction: StudyDirection) -> float:
        ...
        if not search_space.dim:
            optuna_warn(
                f"{self.__class__.__name__} cannot consider any search space." "Termination will never occur in this study."
            )
            return sys.float_info.max * MARGIN_FOR_NUMARICAL_STABILITY  # Do not terminate.
        ...
        standarized_score_vals = (score_vals - score_vals.mean()) / max(sys.float_info.min, score_vals.std())
        ...
        mu_t1_theta_t_with_nu_t, variance_t1_theta_t_with_nu_t = _compute_gp_posterior(normalized_params[-1, :], gpr_t)
        ...
        theorem1_v = math.sqrt(
            max(
                1e-10,
                variance_t_theta_t_star - 2.0 * cov_t_between_theta_t_star_and_theta_t1_star + variance_t_theta_t1_star,
            )
        )
        ...
        eq4_rhs_term2 = -0.5 * variance_t1_theta_t_with_nu_t / (variance_t1_theta_t_with_nu_t + _lambda**-1)
        eq4_rhs_term3 = (
            0.5
        ...
        alg1_delta_r_tilde_t_term4 = kappa_t1 * math.sqrt(0.5 * (eq4_rhs_term1 + eq4_rhs_term2 + eq4_rhs_term3))

        return min(
            sys.float_info.max * 0.5,
            alg1_delta_r_tilde_t_term1 + alg1_delta_r_tilde_t_term2 + alg1_delta_r_tilde_t_term3 + alg1_delta_r_tilde_t_term4,
        )
...
def _compute_gp_posterior_cov_two_thetas(
    ...
    _, covar = gpr.posterior(torch.from_numpy(normalized_params[[theta1_index, theta2_index]]), joint=True)
