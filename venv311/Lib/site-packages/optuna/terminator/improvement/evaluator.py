...
if TYPE_CHECKING:
    ...
    from optuna._gp import gp, optim_sample, prior
...
class RegretBoundEvaluator(BaseImprovementEvaluator):
    ...
    def _get_top_n(self, normalized_params: np.ndarray, values: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    def _validate_input(cls, trials: list[FrozenTrial], search_space: dict[str, BaseDistribution]) -> None:
        if len([t for t in trials if t.state == TrialState.COMPLETE]) == 0:
            raise ValueError("Because no trial has been completed yet, the regret bound cannot be evaluated.")

        if len(search_space) == 0:
            raise ValueError("The intersection search space is empty. This condition is not supported by " f"{cls.__name__}.")
...
class BestValueStagnationEvaluator(BaseImprovementEvaluator):
    ...
    def _validate_input(cls, trials: list[FrozenTrial]) -> None:
        if len([t for t in trials if t.state == TrialState.COMPLETE]) == 0:
            raise ValueError("Because no trial has been completed yet, the improvement cannot be evaluated.")
