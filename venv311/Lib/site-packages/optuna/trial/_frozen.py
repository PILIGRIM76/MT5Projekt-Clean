...
from optuna import distributions, logging
...
from optuna.trial._base import _SUGGEST_INT_POSITIONAL_ARGS, BaseTrial
...
class FrozenTrial(BaseTrial):
    ...
    def __repr__(self) -> str:
        ...
        kwargs = (
            ", ".join(
                f"{field if not field.startswith('_') else field[1:]}={repr(getattr(self, field))}" for field in self.__dict__
            )
            + ", value=None"
        )
        return f"{cls}({kwargs})"
    ...
    def _validate(self) -> None:
        ...
        if set(self.params.keys()) != set(self.distributions.keys()):
            raise ValueError(
                f"Inconsistent parameters {set(self.params.keys())} and " f"distributions {set(self.distributions.keys())}."
            )
    ...
    def _suggest(self, name: str, distribution: BaseDistribution) -> Any:
        ...
        if not distribution._contains(param_value_in_internal_repr):
            optuna_warn(
                f"The value {value} of the parameter '{name}' is out of " f"the range of the distribution {distribution}."
            )
