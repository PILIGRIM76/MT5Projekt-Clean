...
import optuna
from optuna import distributions, logging, pruners
...
class Trial(BaseTrial):
    ...
    def report(self, value: float, step: int) -> None:
        ...
        try:
            ...
            message = f"The `value` argument is of type '{type(value)}' but supposed to be a float."
        ...
        if step in self._cached_frozen_trial.intermediate_values:
            ...
            optuna_warn(f"The reported value is ignored because this `step` {step} is already reported.")
            return
    ...
    def _is_fixed_param(self, name: str, distribution: BaseDistribution) -> bool:
        ...
        if not contained:
            optuna_warn(
                f"Fixed parameter {name} with value {param_value} is out of range " f"for distribution {distribution}."
            )
        return True
