...
from optuna.importance._base import BaseImportanceEvaluator, _get_distributions, _get_filtered_trials, _sort_dict_by_importance
...
class PedAnovaImportanceEvaluator(BaseImportanceEvaluator):
    ...
    def __init__(
        ...
        *,
        ...
        assert (
            0.0 < target_quantile < region_quantile <= 1.0
        ), "condition 0.0 < `target_quantile` < `region_quantile` <= 1.0 must be satisfied"
        if baseline_quantile is not None:
            msg = _DEPRECATION_WARNING_TEMPLATE.format(name="`baseline_quantile`", d_ver="4.7.0", r_ver="5.0.0")
            optuna_warn(
    ...
    def _get_top_quantile_trials(
        ...
        top_trials = _QuantileFilter(quantile, is_lower_better, self._min_n_top_trials, target).filter(trials)
    ...
    def _compute_pearson_divergence(
        ...
        if self._evaluate_on_local:  # The importance of param during the study.
            pe_local = _build_parzen_estimator(param_name, dist, region_trials, self._n_steps, prior_weight)
    ...
    def evaluate(
        ...
        *,
        ...
        if len(target_trials) == len(region_trials):
            optuna_warn("Target and region quantiles select the same set of trials. " "Parameter importances will be equal.")
