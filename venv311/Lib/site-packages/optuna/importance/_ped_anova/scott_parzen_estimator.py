...
from optuna.samplers._tpe.probability_distributions import _BatchedDiscreteTruncNormDistributions, _BatchedDistributions
...
class _ScottParzenEstimator(_ParzenEstimator):
    ...
    def _calculate_numerical_distributions(
        ...
        return _BatchedDiscreteTruncNormDistributions(mu=mus, sigma=sigmas, low=0, high=self.n_steps - 1, step=1)
...
def _count_numerical_param_in_grid(
    ...
    n_grids, grid_indices_of_trials = _get_grids_and_grid_indices_of_trials(param_name, dist, trials, n_steps)
...
def _count_categorical_param_in_grid(param_name: str, dist: CategoricalDistribution, trials: list[FrozenTrial]) -> np.ndarray:
