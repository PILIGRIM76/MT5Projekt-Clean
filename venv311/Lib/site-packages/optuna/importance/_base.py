...
def _get_distributions(study: Study, params: list[str] | None) -> dict[str, BaseDistribution]:
    ...
    for trial in completed_trials:
        ...
        if any(trial_distributions[name] != distribution for name, distribution in distributions.items()):
            raise ValueError(
    ...
    distributions = dict(sorted(distributions.items(), key=lambda name_and_distribution: name_and_distribution[0]))
...
def _check_evaluate_args(completed_trials: list[FrozenTrial], params: list[str] | None) -> None:
    ...
    if params is not None:
        if not isinstance(params, (list, tuple)):
            raise TypeError(f"Parameters must be specified as a list. Actual parameters: {params}.")
        if any(not isinstance(p, str) for p in params):
            raise TypeError(f"Parameters must be specified by their names with strings. " f"Actual parameters: {params}.")
        ...
        if len(params) > 0:
            ...
            if not at_least_one_trial:
                raise ValueError(
                    "Study must contain completed trials with all specified parameters. " f"Specified parameters: {params}."
                )
...
def _get_filtered_trials(
    ...
    return [
        ...
        and np.isfinite(target(trial) if target is not None else cast("float", trial.value))  # TC006
    ]


def _param_importances_to_dict(params: Collection[str], param_importances: np.ndarray | float) -> dict[str, float]:
    return {name: value for name, value in zip(params, np.broadcast_to(param_importances, (len(params),)))}
...
def _get_target_values(trials: list[FrozenTrial], target: Callable[[FrozenTrial], float] | None) -> np.ndarray:
...
def _sort_dict_by_importance(param_importances: dict[str, float]) -> dict[str, float]:
    return dict(reversed(sorted(param_importances.items(), key=lambda name_and_importance: name_and_importance[1])))
