...
class _FanovaTree:
            ...
            if feature >= 0:  # Not leaf. Avoid unnecessary call to `_is_node_leaf`.
                ...
                if not np.isnan(response):
                    if response <= self._get_node_split_threshold(node_index):
                        ...
                        next_subspace = self._get_node_left_child_subspaces(node_index, search_spaces)
                    else:
                        next_node_index = self._get_node_right_child(node_index)
                        next_subspace = self._get_node_right_child_subspaces(node_index, search_spaces)
    ...
    def _get_node_left_child_subspaces(self, node_index: int, search_spaces: np.ndarray) -> np.ndarray:
    ...
    def _get_node_right_child_subspaces(self, node_index: int, search_spaces: np.ndarray) -> np.ndarray:
        return _get_subspaces(
            search_spaces,
    ...
    def _get_node_children_subspaces(self, node_index: int, search_spaces: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        return (
...
def _get_subspaces(search_spaces: np.ndarray, *, search_spaces_column: int, feature: int, threshold: float) -> np.ndarray:
