...
def swig_ptr_from_UInt8Tensor(x):
    ...
    return faiss.cast_integer_to_uint8_ptr(x.untyped_storage().data_ptr() + x.storage_offset())
...
def swig_ptr_from_HalfTensor(x):
    ...
    return faiss.cast_integer_to_void_ptr(x.untyped_storage().data_ptr() + x.storage_offset() * 2)
...
def swig_ptr_from_IntTensor(x):
    ...
    return faiss.cast_integer_to_int_ptr(x.untyped_storage().data_ptr() + x.storage_offset() * 4)
...
def handle_torch_Index(the_class):
    ...
    torch_replace_method(the_class, "update_vectors", torch_replacement_update_vectors, ignore_missing=True)
    torch_replace_method(the_class, "search_and_reconstruct", torch_replacement_search_and_reconstruct, ignore_missing=True)
    torch_replace_method(the_class, "search_preassigned", torch_replacement_search_preassigned, ignore_missing=True)
...
def torch_replacement_knn(xq, xb, k, metric=faiss.METRIC_L2, metric_arg=0):
    ...
    if metric == faiss.METRIC_L2:
        ...
        faiss.knn_extra_metrics(xq_ptr, xb_ptr, d, nq, nb, metric, metric_arg, k, D_ptr, I_ptr)
