...
def range_search_gpu(xq, r2, index_gpu, index_cpu, gpu_k=1024):
    ...
    CombinerRangeKNN = faiss.CombinerRangeKNNint16 if is_binary_index else faiss.CombinerRangeKNNfloat
...
def range_ground_truth(xq, db_iterator, threshold, metric_type=faiss.METRIC_L2, shard=False, ngpu=-1):
...
def range_search_max_results(
    index, query_iterator, radius, max_results=None, min_results=None, shard=False, ngpu=0, clip_to_min=False
):
    ...
    for xqi in query_iterator:
        ...
        if max_results is not None and totres > max_results:
            ...
            radius, totres = apply_maxres(res_batches, min_results, keep_max=index.metric_type == faiss.METRIC_INNER_PRODUCT)
        t2 = time.time()
        t_search += t1 - t0
        t_post_process += t2 - t1
        LOG.debug("   [%.3f s] %d queries done, %d results" % (time.time() - t_start, qtot, totres))

    LOG.info("search done in %.3f s + %.3f s, total %d results, end threshold %g" % (t_search, t_post_process, totres, radius))
