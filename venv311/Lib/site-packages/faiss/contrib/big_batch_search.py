...
class BigBatchSearcher:
    ...
    def report(self, l):
        if self.verbose == 1 or (self.verbose == 2 and (l > 1000 and time.time() < self.t_display + 1.0)):
            return
        t = time.time() - self.t0
        print(
    ...
    def reorder_assign(self):
        ...
        q_assign += 1  # move -1 -> 0
        self.bucket_lims = faiss.matrix_bucket_sort_inplace(self.q_assign, nbucket=self.index.nlist + 1, nt=16)
...
class BlockComputer:
    ...
    def __init__(self, index, method="knn_function", pairwise_distances=faiss.pairwise_distances, knn=faiss.knn):
        ...
        def compute_task(task_id, output_queue, input_queue):
            try:
                ...
                while True:
                    ...
                    t0 = time.time()
                    output_queue.put((centroid, t_wait_in, t_wait_out, t_compute, q_subset, D, list_ids, I))
