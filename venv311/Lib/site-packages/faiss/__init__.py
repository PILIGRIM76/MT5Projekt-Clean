...
for symbol in dir(this_module):
    ...
    if inspect.isclass(obj):
        ...
        if issubclass(the_class, IndexRowwiseMinMax) or issubclass(the_class, IndexRowwiseMinMaxFP16):
...
def search_with_parameters(index, x, k, params=None, output_stats=False):
    ...
    search_with_parameters_c(
        index, n, swig_ptr(x), k, swig_ptr(distances), swig_ptr(labels), params, swig_ptr(nb_dis), swig_ptr(ms_per_stage)
    )
    if not output_stats:
...
def range_search_with_parameters(index, x, radius, params=None, output_stats=False):
    ...
    range_search_with_parameters_c(index, n, swig_ptr(x), radius, res, params, swig_ptr(nb_dis), swig_ptr(ms_per_stage))
