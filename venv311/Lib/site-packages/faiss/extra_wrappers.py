...
def pairwise_distances(xq, xb, metric=METRIC_L2, metric_arg=0):
    ...
    if metric == METRIC_L2:
        ...
        pairwise_extra_distances(d, nq, swig_ptr(xq), nb, swig_ptr(xb), metric, metric_arg, swig_ptr(dis))
    return dis
...
def bucket_sort(tab, nbucket=None, nt=0):
    ...
    bucket_sort_c(tab.size, faiss.swig_ptr(tab.view("uint64")), nbucket, faiss.swig_ptr(lims), faiss.swig_ptr(perm), nt)
    return lims, perm
...
class ResultHeap:
    ...
    def add_result_subset(self, subset, D, I):
        ...
        self.heaps.addn_query_subset_with_ids(nsubset, swig_ptr(subset), kd, swig_ptr(D), swig_ptr(I), id_stride)
...
class MapInt64ToInt64:
    ...
    def add(self, keys, vals):
        (n,) = keys.shape
        assert vals.shape == (n,)
        faiss.hashtable_int64_to_int64_add(self.log2_capacity, swig_ptr(self.tab), n, swig_ptr(keys), swig_ptr(vals))

    def lookup(self, keys):
        (n,) = keys.shape
        vals = np.empty((n,), dtype="int64")
        faiss.hashtable_int64_to_int64_lookup(self.log2_capacity, swig_ptr(self.tab), n, swig_ptr(keys), swig_ptr(vals))
        return vals
...
def knn(xq, xb, k, metric=METRIC_L2, metric_arg=0.0):
    ...
    if metric == METRIC_L2:
        ...
        knn_extra_metrics(swig_ptr(xq), swig_ptr(xb), d, nq, nb, metric, metric_arg, k, swig_ptr(D), swig_ptr(I))
...
def knn_hamming(xq, xb, k, variant="hc"):
    ...
    if variant == "hc":
        ...
        faiss.hammings_knn_mc(faiss.swig_ptr(xq), faiss.swig_ptr(xb), nq, nb, k, d, faiss.swig_ptr(D), faiss.swig_ptr(I))
    else:
...
class Kmeans:
    ...
    def train(self, x, weights=None, init_centroids=None):
        ...
        self.iteration_stats = [{field: getattr(st, field) for field in stat_fields} for st in stats]
