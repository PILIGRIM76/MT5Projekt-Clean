...
class ScriptItem:
    ...
    def GetDefaultSourceTypeInfo(self, typeinfo):
        ...
        for i in range(cImplType):
            ...
            flags = typeinfo.GetImplTypeFlags(i)
            if (
                flags & (pythoncom.IMPLTYPEFLAG_FDEFAULT | pythoncom.IMPLTYPEFLAG_FSOURCE | pythoncom.IMPLTYPEFLAG_FRESTRICTED)
...
class COMScript:
    ...
    _public_methods_ = (
        IActiveScriptMethods + IActiveScriptParseMethods + IObjectSafetyMethods + IActiveScriptParseProcedureMethods
    )
    _com_interfaces_ = [
    ...
    def SetScriptSite(self, site):
        ...
        try:
            ...
            traceback.print_exc()
            trace("*** Debugger Manager could not initialize - {}: {}".format(sys.exc_info()[0], sys.exc_info()[1]))
            self.debugManager = None
    ...
    def HandleException(self, codeBlock: AXScriptCodeBlock | None) -> NoReturn:
        ...
        if isinstance(exc_value, pythoncom.com_error) and exc_value.hresult == axscript.SCRIPT_E_REPORTED:
            # Ensure the traceback doesn't cause a cycle.
