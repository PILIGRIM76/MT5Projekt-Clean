...
class CGenerator:
    ...
    def _generate_struct_union_enum(self, n: c_ast.Struct | c_ast.Union | c_ast.Enum, name: str) -> str:
                ...
                for i, modifier in enumerate(modifiers):
                    match modifier:
                        ...
                        case c_ast.FuncDecl():
                            if i != 0 and isinstance(modifiers[i - 1], c_ast.PtrDecl):
                                nstr = "(" + nstr + ")"
                            args = self.visit(modifier.args) if modifier.args is not None else ""
                            nstr += "(" + args + ")"
                        case c_ast.PtrDecl():
                            if modifier.quals:
