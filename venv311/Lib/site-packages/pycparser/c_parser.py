...
class CParser:
    ...
    def _fix_decl_name_type(
        ...
        if not typename:
            ...
            typ.type = c_ast.IdentifierType([name for id in typename for name in id.names], coord=typename[0].coord)
        return decl
    ...
    def _parse_any_declarator(
        ...
        if name_type is None or (typeid_paren_as_abstract and name_type == "TYPEID" and saw_paren):
    ...
    def _parse_decl_body_with_spec(self, spec: "_DeclSpec", saw_type: bool) -> List[c_ast.Node]:
        decls = None
        if saw_type:
    ...
    #                               | function_specifier
    #                               | alignment_specifier)+
    def _parse_declaration_specifiers(self, allow_no_type: bool = False) -> Tuple["_DeclSpec", bool, Optional[Coord]]:
        ...
        while True:
            ...
            if tok.type == "_ALIGNAS":
                ...
                spec = self._add_declaration_specifier(spec, self._parse_alignment_specifier(), "alignment", append=True)
                continue

            if tok.type == "_ATOMIC" and self._peek_type(2) == "LPAREN":
                if first_coord is None:
                    first_coord = self._tok_coord(tok)
                spec = self._add_declaration_specifier(spec, self._parse_atomic_specifier(), "type", append=True)
                saw_type = True
                continue
            ...
            if tok.type in _TYPE_QUALIFIER:
                if first_coord is None:
                    first_coord = self._tok_coord(tok)
                spec = self._add_declaration_specifier(spec, self._advance().value, "qual", append=True)
                continue

            if tok.type in _STORAGE_CLASS:
                if first_coord is None:
                    first_coord = self._tok_coord(tok)
                spec = self._add_declaration_specifier(spec, self._advance().value, "storage", append=True)
                continue

            if tok.type in _FUNCTION_SPEC:
                if first_coord is None:
                    first_coord = self._tok_coord(tok)
                spec = self._add_declaration_specifier(spec, self._advance().value, "function", append=True)
                continue
            ...
            if tok.type in {"STRUCT", "UNION"}:
                if first_coord is None:
                    first_coord = self._tok_coord(tok)
                spec = self._add_declaration_specifier(spec, self._parse_struct_or_union_specifier(), "type", append=True)
                saw_type = True
                continue
            ...
            if tok.type == "ENUM":
                if first_coord is None:
                    first_coord = self._tok_coord(tok)
                spec = self._add_declaration_specifier(spec, self._parse_enum_specifier(), "type", append=True)
                saw_type = True
                continue
    ...
    def _parse_init_declarator_list(self, first: Optional["_DeclInfo"] = None, id_only: bool = False) -> List["_DeclInfo"]:
        decls = [first] if first is not None else [self._parse_init_declarator(id_only=id_only)]
        ...
        if self._peek_type() in {"ID", "TYPEID"}:
            ...
            if self._peek_type() == "LBRACE":
                ...
                if self._accept("RBRACE"):
                    return klass(name=name_tok.value, decls=[], coord=self._tok_coord(name_tok))
    ...
    def _parse_struct_declaration(self) -> Optional[List[c_ast.Node]]:
        ...
        if len(spec["type"]) == 1:
            ...
            self._expect("SEMI")
            return self._build_declarations(spec=spec, decls=[dict(decl=decl_type, init=None, bitsize=None)])

        self._expect("SEMI")
        return self._build_declarations(spec=spec, decls=[dict(decl=None, init=None, bitsize=None)])
    ...
    def _parse_declarator(self) -> c_ast.Node:
        decl, _ = self._parse_any_declarator(allow_abstract=False, typeid_paren_as_abstract=False)
    ...
    def _parse_array_decl_common(self, base_type: Optional[c_ast.Node], coord: Optional[Coord] = None) -> c_ast.Node:
    ...
    def _parse_function_decl(self, base_decl: c_ast.Node) -> c_ast.Node:
        ...
        if self._accept("RPAREN"):
            ...
            args = self._parse_parameter_type_list() if self._starts_declaration() else self._parse_identifier_list_opt()
            self._expect("RPAREN")
    ...
    def _parse_parameter_declaration(self) -> c_ast.Node:
        ...
        if self._starts_declarator():
            decl, is_named = self._parse_any_declarator(allow_abstract=True, typeid_paren_as_abstract=True)
            if is_named:
                return self._build_declarations(spec=spec, decls=[dict(decl=decl, init=None, bitsize=None)])[0]
    ...
    def _parse_abstract_array_base(self) -> c_ast.Node:
        return self._parse_array_decl_common(base_type=c_ast.TypeDecl(None, None, None, None), coord=None)
    ...
    def _parse_binary_expression(self, min_prec: int = 0, lhs: Optional[c_ast.Node] = None) -> c_ast.Node:
        if lhs is None:
...
_STARTS_EXPRESSION = _EXPR_START | _INT_CONST | _FLOAT_CONST | _CHAR_CONST | _STRING_LITERAL | _WSTR_LITERAL
