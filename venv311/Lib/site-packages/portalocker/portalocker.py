    ...
    class Win32Locker(BaseLocker):
        ...
        def __init__(self) -> None:
            try:
                ...
                raise ImportError("pywintypes is required for Win32Locker but not " "found. Please install pywin32.") from e
        ...
        def _get_os_handle(self, fd: int) -> int:
            try:
                ...
                raise ImportError("msvcrt is required for _get_os_handle on Windows " "but not found.") from e
        ...
        def lock(self, file_obj: types.FileArgument, flags: LockFlags) -> None:
            ...
            try:
                win32file.LockFileEx(os_fh, mode, 0, self._lock_bytes_low, self._overlapped)
    ...
    class MsvcrtLocker(BaseLocker):
        ...
        def __init__(self) -> None:
            ...
            try:
                ...
                raise ImportError("msvcrt is required for MsvcrtLocker but not found.") from e
        ...
        def unlock(self, file_obj: types.FileArgument) -> None:
            ...
            try:
                ...
                if exc.errno == 13:  # EACCES (Permission denied)
                    ...
                    _restore_windows_file_pos(io_obj_ctx, pos_ctx)
                    try:
                        self._win32_locker.unlock(file_obj)  # win32_locker handles its own seeking
                    except exceptions.LockException as win32_exc:
                        raise exceptions.LockException(
                            exceptions.LockException.LOCK_FAILED,
                            f"msvcrt unlock failed ({exc.strerror}), and " f"win32 fallback failed ({win32_exc.strerror})",
                            fh=file_obj,
                        ) from win32_exc
    ...
    class PosixLocker(BaseLocker):
        ...
        def _get_fd(self, file_obj: PosixFileArgument) -> int:
            ...
            # Check for fileno() method; covers typing.IO and HasFileno
                ...
                raise TypeError("Argument 'file_obj' must be an int, an IO object " "with fileno(), or implement HasFileno.")
        ...
        def lock(self, file_obj: PosixFileArgument, flags: LockFlags) -> None:
            if (flags & LockFlags.NON_BLOCKING) and not flags & (LockFlags.SHARED | LockFlags.EXCLUSIVE):
                raise RuntimeError(
                    "When locking in non-blocking mode on POSIX, " "the SHARED or EXCLUSIVE flag must be specified as well."
                )
