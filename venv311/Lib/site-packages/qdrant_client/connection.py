...
class _GenericAsyncClientInterceptor(
    ...
    async def intercept_unary_unary(self, continuation: Any, client_call_details: Any, request: Any) -> Any:
        new_details, new_request_iterator, postprocess = await self._fn(client_call_details, iter((request,)), False, False)
    ...
    async def intercept_unary_stream(self, continuation: Any, client_call_details: Any, request: Any) -> Any:
        new_details, new_request_iterator, postprocess = await self._fn(client_call_details, iter((request,)), False, True)
    ...
    async def intercept_stream_unary(self, continuation: Any, client_call_details: Any, request_iterator: Any) -> Any:
        new_details, new_request_iterator, postprocess = await self._fn(client_call_details, request_iterator, True, False)
    ...
    async def intercept_stream_stream(self, continuation: Any, client_call_details: Any, request_iterator: Any) -> Any:
        new_details, new_request_iterator, postprocess = await self._fn(client_call_details, request_iterator, True, True)
    ...
    async def process_response(call: Any) -> Any:
        try:
            ...
            if er.code() == grpc.StatusCode.RESOURCE_EXHAUSTED:
                ...
                if retry_after:
                    raise ResourceExhaustedResponse(message=reason_phrase, retry_after_s=retry_after) from er
            raise
...
def parse_ssl_credentials(options: dict[str, Any] | None = None) -> dict[str, bytes | None]:
    ...
    """
    ssl_options: dict[str, bytes | None] = dict(root_certificates=None, private_key=None, certificate_chain=None)
...
def get_channel(
    ...
    _copied_options = options.copy() if options is not None else None  # we're changing options inplace
    _ssl_cred_options = parse_ssl_credentials(_copied_options)
    _options = parse_channel_options(_copied_options)
    metadata_interceptor = header_adder_interceptor(new_metadata=metadata or [], auth_token_provider=auth_token_provider)
...
def get_async_channel(
    ...
    metadata_interceptor = header_adder_async_interceptor(new_metadata=metadata or [], auth_token_provider=auth_token_provider)
    ...
    if ssl:
        ...
        return grpc.aio.insecure_channel(f"{host}:{port}", _options, compression, interceptors=[metadata_interceptor])
