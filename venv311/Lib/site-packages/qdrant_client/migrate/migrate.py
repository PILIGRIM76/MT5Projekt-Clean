...
from qdrant_client._pydantic_compat import model_fields, to_dict
...
def migrate(
    ...
    if any(_has_custom_shards(source_client, collection_name) for collection_name in collection_names):
...
def _has_custom_shards(source_client: QdrantBase, collection_name: str) -> bool:
    ...
    return getattr(collection_info.config.params, "sharding_method", None) == models.ShardingMethod.CUSTOM


def _select_source_collections(source_client: QdrantBase, collection_names: list[str] | None = None) -> list[str]:
...
def _recreate_collection(
    ...
    if src_config.strict_mode_config is not None:
        strict_mode_config = models.StrictModeConfig(
            **{k: v for k, v in to_dict(src_config.strict_mode_config).items() if k in model_fields(models.StrictModeConfig)}
        )
...
def _migrate_collection(
    ...
    while next_offset is not None:
        records, next_offset = source_client.scroll(collection_name, offset=next_offset, limit=batch_size, with_vectors=True)
        upload_with_retry(client=dest_client, collection_name=collection_name, points=records)  # type: ignore
