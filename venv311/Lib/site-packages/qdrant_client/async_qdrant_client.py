...
class AsyncQdrantClient(AsyncQdrantFastembedMixin):
    ...
    async def query_points(
        ...
        **kwargs: Any,
        ...
        if not self.cloud_inference:
            if self._inference_inspector.inspect(query) or self._inference_inspector.inspect(prefetch):
                query = (
        ...
        if not self.cloud_inference:
            if self._inference_inspector.inspect(query) or self._inference_inspector.inspect(prefetch):
                ...
                if isinstance(prefetch, list):
                    ...
                    prefetch = next(
                        iter(self._embed_models(prefetch, is_query=True, batch_size=self.local_inference_batch_size))
                    )
    ...
    async def upsert(
        ...
        **kwargs: Any,
        ...
        """
        assert len(kwargs) == 0, f"Unknown arguments: {list(kwargs.keys())}"
        if not isinstance(points, types.Batch) and len(points) > 0 and isinstance(points[0], grpc.PointStruct):
            show_warning_once(
    ...
    async def create_full_snapshot(self, wait: bool = True, **kwargs: Any) -> types.SnapshotDescription | None:
    ...
    async def get_optimizations(
        ...
        """
        return await self._client.get_optimizations(collection_name=collection_name, completed_limit=completed_limit, **kwargs)
    ...
    async def cluster_telemetry(
        ...
        """
        return await self._client.cluster_telemetry(details_level=details_level, timeout=timeout, **kwargs)
