...
def evaluate_expression(
    ...
    if isinstance(expression, (float, int)):  # Constant
        ...
        return sum(evaluate_expression(expr, point_id, scores, payload, has_vector, defaults) for expr in expression.sum)

    elif isinstance(expression, models.NegExpression):
        value = evaluate_expression(expression.neg, point_id, scores, payload, has_vector, defaults)
        return -value

    elif isinstance(expression, models.AbsExpression):
        return abs(evaluate_expression(expression.abs, point_id, scores, payload, has_vector, defaults))

    elif isinstance(expression, models.DivExpression):
        left = evaluate_expression(expression.div.left, point_id, scores, payload, has_vector, defaults)

        if left == 0.0:
            return left

        right = evaluate_expression(expression.div.right, point_id, scores, payload, has_vector, defaults)
        ...
        value = evaluate_expression(expression.sqrt, point_id, scores, payload, has_vector, defaults)
        ...
        base = evaluate_expression(expression.pow.base, point_id, scores, payload, has_vector, defaults)
        exponent = evaluate_expression(expression.pow.exponent, point_id, scores, payload, has_vector, defaults)
        ...
        value = evaluate_expression(expression.exp, point_id, scores, payload, has_vector, defaults)
        ...
        value = evaluate_expression(expression.log10, point_id, scores, payload, has_vector, defaults)
        ...
        raise ValueError(f"Expected geo point for {to} in the payload and/or in the formula defaults.")
        ...
        if dt is None:
            raise ValueError(f"Expected datetime for {expression.datetime_key} in the payload and/or in the formula defaults.")
...
def evaluate_decay_params(
    ...
    if params.target is None:
        ...
        target = evaluate_expression(params.target, point_id, scores, payload, has_vector, defaults)
