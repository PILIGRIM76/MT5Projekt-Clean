...
class AsyncQdrantLocal(AsyncQdrantBase):
    ...
    def search(
        ...
        collection_name: str,
        query_vector: (
            types.NumpyArray | Sequence[float] | tuple[str, list[float]] | types.NamedVector | types.NamedSparseVector
        ),
    ...
    def _resolve_query_input(
        ...
        vector_name = (
            lookup_from.vector if lookup_from is not None and lookup_from.vector is not None else search_in_vector_name
        )
    ...
    def _resolve_prefetch_input(self, prefetch: types.Prefetch, collection_name: str) -> types.Prefetch:
        ...
        query, mentioned_ids = self._resolve_query_input(collection_name, prefetch.query, prefetch.using, prefetch.lookup_from)
    ...
    def _default_update_result(cls, operation_id: int = 0) -> types.UpdateResult:
        return types.UpdateResult(operation_id=operation_id, status=rest_models.UpdateStatus.COMPLETED)
    ...
    async def recreate_collection(
        ...
        **kwargs: Any,
        ...
        return await self.create_collection(collection_name, vectors_config, sparse_vectors_config, metadata=metadata)
    ...
    def upload_points(
        ...
        **kwargs: Any,
        ...
        self._upload_points(collection_name, points, update_filter=update_filter, update_mode=update_mode)
    ...
    def _upload_points(
        ...
        collection = self._get_collection(collection_name)
        collection.upsert(
            [rest_models.PointStruct(id=point.id, vector=point.vector or {}, payload=point.payload or {}) for point in points],
    ...
    async def delete_snapshot(self, collection_name: str, snapshot_name: str, **kwargs: Any) -> bool:
        raise NotImplementedError(
            "Snapshots are not supported in the local Qdrant. Please use server Qdrant if you need full snapshots."
        )
    ...
    async def delete_shard_snapshot(self, collection_name: str, shard_id: int, snapshot_name: str, **kwargs: Any) -> bool:
        raise NotImplementedError(
            "Snapshots are not supported in the local Qdrant. Please use server Qdrant if you need snapshots."
        )

    async def recover_shard_snapshot(self, collection_name: str, shard_id: int, location: str, **kwargs: Any) -> bool:
        raise NotImplementedError(
            "Snapshots are not supported in the local Qdrant. Please use server Qdrant if you need snapshots."
        )
    ...
    async def get_optimizations(self, collection_name: str, **kwargs: Any) -> types.OptimizationsResponse:
        raise NotImplementedError("Get optimizations is not supported in the local Qdrant. Please use server Qdrant.")
