...
from qdrant_client.http.models.models import Distance, ExtendedPointId, OrderValue, SparseVector
from qdrant_client.hybrid.formula import evaluate_expression, raise_non_finite_error
from qdrant_client.hybrid.fusion import distribution_based_score_fusion, reciprocal_rank_fusion
...
from qdrant_client.local.distances import FeedbackItem as DistFeedbackItem
from qdrant_client.local.distances import (
...
class LocalCollection:
    ...
    def _get_vectors(self, idx: int, with_vectors: bool | Sequence[str] | None = False) -> models.VectorStruct | None:
    ...
    def query_groups(
        ...
        if prefetch is not None:
            prefetch = deepcopy(prefetch)  # we're modifying Prefetch inplace, but we don't want to modify
    ...
    def search_groups(
        ...
        query_vector: (
            Sequence[float]
            | list[list[float]]
            | tuple[str, models.Vector | RecoQuery | SparseRecoQuery | MultiRecoQuery | types.NumpyArray]
            | types.NamedVector
            | types.NamedSparseVector
            | RecoQuery
            | SparseRecoQuery
    ...
    def _mmr(
        ...
        candidate_distance_matrix: dict[tuple[models.ExtendedPointId, models.ExtendedPointId], float] = {}
        ...
        distance = self.get_vector_params(using).distance if using not in self.sparse_vectors else None
        query_vector = (
            np.array(query_vector) if not isinstance(query_vector, SparseVector) else sort_sparse_vector(query_vector)
        )
        ...
        for candidate_id, candidate_vector in zip(candidate_ids, candidate_vectors):
            if isinstance(candidate_vector, list) and isinstance(candidate_vector[0], float):
                ...
                nearest_candidates = calculate_distance_core(candidate_vector_np, candidate_vectors, distance).tolist()
        ...
        while len(selected) < limit and len(pending) > 0:
            ...
            for pending_id in pending:
                ...
                for selected_id in selected:
                    similarities_to_selected.append(candidate_distance_matrix[(pending_id, selected_id)])
                max_similarity_to_selected = max(similarities_to_selected)
                mmr_score = lambda_ * relevance_score - (1.0 - lambda_) * max_similarity_to_selected
                mmr_scores.append(mmr_score)

            if all([np.isneginf(sim) for sim in mmr_scores]):  # no points left passing score threshold
                break
    ...
    def _relevance_feedback(
        ...
        target_vector, target_id = self._preprocess_vector_input(relevance_feedback.target, self, vector_name)
        ...
        for item in relevance_feedback.feedback:
            example_vector, example_id = self._preprocess_vector_input(item.example, self, vector_name)
        ...
        if point.id in self.ids:
            ...
            if not self.deleted[idx] and update_filter is not None:
                ...
                for vector_name, deleted in self.deleted_per_vector.items():
                    if not deleted[idx]:
                        has_vector[vector_name] = True
                if not check_filter(update_filter, self.payload[idx], self.ids_inv[idx], has_vector):
                    return None
    ...
    def _update_named_vectors(self, idx: int, vectors: dict[str, list[float] | SparseVector | list[list[float]]]) -> None:
    ...
    def update_vectors(self, points: Sequence[types.PointVectors], update_filter: types.Filter | None = None) -> None:
        for point in points:
    ...
    def delete_vectors(
        ...
        selector: models.Filter | list[models.ExtendedPointId] | models.FilterSelector | models.PointIdsList,
    ) -> None:
    ...
    def batch_update_points(
        ...
        for update_op in update_operations:
            if isinstance(update_op, models.UpsertOperation):
                ...
                self.set_payload(update_op.set_payload.payload, points_selector, update_op.set_payload.key)
                ...
                self.update_vectors(update_vectors.points, update_filter=update_vectors.update_filter)
...
def ignore_mentioned_ids_filter(query_filter: types.Filter | None, mentioned_ids: list[types.PointId]) -> types.Filter:
...
def _include_ids_in_filter(query_filter: types.Filter | None, ids: list[types.PointId]) -> types.Filter:
    if len(ids) == 0:
        return query_filter
...
def set_prefetch_limit_iteratively(prefetch: types.Prefetch | list[types.Prefetch], limit: int) -> None:
