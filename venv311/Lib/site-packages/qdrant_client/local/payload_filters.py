...
def check_geo_bounding_box(condition: models.GeoBoundingBox, values: Any) -> bool:
    if isinstance(values, dict) and "lat" in values and "lon" in values:
        ...
        if condition.top_left.lon > condition.bottom_right.lon:
            longitude_condition = condition.top_left.lon <= lon <= 180 or -180 <= lon <= condition.bottom_right.lon
        else:
...
def check_geo_polygon(condition: models.GeoPolygon, values: Any) -> bool:
    if isinstance(values, dict) and "lat" in values and "lon" in values:
        ...
        if condition.interiors is not None:
            interiors = [[(point.lat, point.lon) for point in interior.points] for interior in condition.interiors]
...
def check_datetime_range(condition: models.DatetimeRange, value: Any) -> bool:
    ...
    return (lt is None or dt < lt) and (lte is None or dt <= lte) and (gt is None or dt > gt) and (gte is None or dt >= gte)
...
def check_condition(
    ...
    if isinstance(condition, models.IsNullCondition):
        ...
        if values is None or len(values) == 0 or all((v is None or (isinstance(v, list) and len(v) == 0)) for v in values):
            return True
...
def check_must(
    ...
    return all(check_condition(condition, payload, point_id, has_vector) for condition in conditions)
...
def check_must_not(
    ...
    return all(not check_condition(condition, payload, point_id, has_vector) for condition in conditions)
...
def check_should(
    ...
    return any(check_condition(condition, payload, point_id, has_vector) for condition in conditions)
...
def check_min_should(
    ...
    return sum(check_condition(condition, payload, point_id, vectors) for condition in conditions) >= min_count
...
def check_filter(
    ...
    if payload_filter.must is not None:
        if not check_must(ensure_condition_list(payload_filter.must), payload, point_id, has_vector):
            return False
    if payload_filter.must_not is not None:
        if not check_must_not(ensure_condition_list(payload_filter.must_not), payload, point_id, has_vector):
            return False
    if payload_filter.should is not None:
        if not check_should(ensure_condition_list(payload_filter.should), payload, point_id, has_vector):
            return False
