...
class JsonPathItem(BaseModel):
    ...
    index: int | None = None  # split into index and key instead of using Union, because pydantic coerces
...
def match_quote(path: str) -> tuple[JsonPathItem | None, str]:
    ...
    return (
        JsonPathItem(item_type=JsonPathItemType.KEY, key=path[left_quote_pos + 1 : right_quote_pos]),
