...
class SparseRecoQuery:
    ...
    def transform_sparse(self, foo: Callable[["SparseVector"], "SparseVector"]) -> "SparseRecoQuery":
...
class SparseDiscoveryQuery:
    ...
    def transform_sparse(self, foo: Callable[["SparseVector"], "SparseVector"]) -> "SparseDiscoveryQuery":
        return SparseDiscoveryQuery(
            target=foo(self.target),
            context=[SparseContextPair(foo(pair.positive), foo(pair.negative)) for pair in self.context],
        )
...
class SparseContextQuery:
    ...
    def transform_sparse(self, foo: Callable[["SparseVector"], "SparseVector"]) -> "SparseContextQuery":
        return SparseContextQuery(
            context_pairs=[SparseContextPair(foo(pair.positive), foo(pair.negative)) for pair in self.context_pairs]
        )


SparseQueryVector: TypeAlias = SparseVector | SparseDiscoveryQuery | SparseContextQuery | SparseRecoQuery
...
def calculate_sparse_discovery_ranks(
    ...
    for pair in context:
        ...
        pair_ranks = np.array(
            [1 if is_bigger else 0 if is_equal else -1 for is_bigger, is_equal in zip(pos > neg, pos == neg)]
        )
...
def calculate_sparse_discovery_scores(query: SparseDiscoveryQuery, vectors: list[SparseVector]) -> types.NumpyArray:
    ...
    sigmoided_distances = np.fromiter((scaled_fast_sigmoid(xi) for xi in distances_to_target), np.float32)

    return ranks + sigmoided_distances


def calculate_sparse_context_scores(query: SparseContextQuery, vectors: list[SparseVector]) -> types.NumpyArray:
    ...
    for pair in query.context_pairs:
        ...
        pair_scores = np.fromiter((fast_sigmoid(xi) for xi in np.minimum(difference, 0.0)), np.float32)
        overall_scores += pair_scores

    return overall_scores


def calculate_sparse_recommend_best_scores(query: SparseRecoQuery, vectors: list[SparseVector]) -> types.NumpyArray:
...
def calculate_sparse_recommend_sum_scores(query: SparseRecoQuery, vectors: list[SparseVector]) -> types.NumpyArray:
...
# Expects sorted indices
def merge_positive_and_negative_avg(positive: SparseVector, negative: SparseVector) -> SparseVector:
