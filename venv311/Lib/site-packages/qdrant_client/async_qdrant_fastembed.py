...
class AsyncQdrantFastembedMixin(AsyncQdrantBase):
    ...
    def get_fastembed_sparse_vector_params(
        ...
        if vector_field_name is None:
            return None
        return {
            vector_field_name: models.SparseVectorParams(index=models.SparseIndexParams(on_disk=on_disk), modifier=modifier)
        }
    ...
    async def query(
        ...
        **kwargs: Any,
        ...
        return self._scored_points_to_query_responses(
            reciprocal_rank_fusion([dense_request_response.points, sparse_request_response.points], limit=limit)
        )
    ...
    async def query_batch(
        ...
        **kwargs: Any,
        ...
        if self.sparse_embedding_model_name is None:
            responses = await self.query_batch_points(collection_name=collection_name, requests=requests)
    ...
    def _resolve_query(
        ...
        if isinstance(query, get_args(types.PointId)):
            query = GrpcToRest.convert_point_id(query) if isinstance(query, grpc.PointId) else query
            return models.NearestQuery(nearest=query)
