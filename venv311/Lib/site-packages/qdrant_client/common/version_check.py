...
def get_server_version(
    ...
    if response.status_code == 200:
        ...
        if not version_info:
            logging.debug(f"Unable to parse response from server: {response}, server version defaults to None")
        return version_info
    else:
        logging.debug(f"Unexpected response from server: {response}, server version defaults to None")
    return None
...
def parse_version(version: str) -> Version:
    ...
    try:
        ...
        raise ValueError(f"Unable to parse version, expected format: x.y.z, found: {version}") from er
