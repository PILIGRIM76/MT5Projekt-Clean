...
class FastEmbedMisc:
    ...
    def list_text_models(cls) -> dict[str, tuple[int, models.Distance]]:
        ...
        """
        return (
            {model["model"]: (model["dim"], models.Distance.COSINE) for model in TextEmbedding.list_supported_models()}
            if TextEmbedding
            else {}
        )
    ...
    def list_image_models(cls) -> dict[str, tuple[int, models.Distance]]:
        ...
        """
        return (
            {model["model"]: (model["dim"], models.Distance.COSINE) for model in ImageEmbedding.list_supported_models()}
            if ImageEmbedding
            else {}
        )
    ...
    def is_supported_late_interaction_text_model(cls, model_name: str) -> bool:
        ...
        if model_name.lower() in cls._LATE_INTERACTION_TEXT_MODELS:
            return True
        # update cached list in case custom models were added
        cls._LATE_INTERACTION_TEXT_MODELS = {model.lower() for model in cls.list_late_interaction_text_models()}
        if model_name.lower() in cls._LATE_INTERACTION_TEXT_MODELS:
            return True
        return False
    ...
    def is_supported_late_interaction_multimodal_model(cls, model_name: str) -> bool:
        ...
        if model_name.lower() in cls._LATE_INTERACTION_MULTIMODAL_MODELS:
            return True
        # update cached list in case custom models were added
        cls._LATE_INTERACTION_MULTIMODAL_MODELS = {model.lower() for model in cls.list_late_interaction_multimodal_models()}
        if model_name.lower() in cls._LATE_INTERACTION_MULTIMODAL_MODELS:
            return True
        return False
...
SUPPORTED_SPARSE_EMBEDDING_MODELS: dict[str, dict[str, Any]] = (
    {model["model"]: model for model in SparseTextEmbedding.list_supported_models()} if SparseTextEmbedding else {}
)
...
_LATE_INTERACTION_EMBEDDING_MODELS: dict[str, tuple[int, models.Distance]] = (
    {model["model"]: (model["dim"], models.Distance.COSINE) for model in LateInteractionTextEmbedding.list_supported_models()}
    if LateInteractionTextEmbedding
    else {}
)
