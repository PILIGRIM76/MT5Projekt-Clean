...
class BaseUploader(Worker, ABC):
    ...
    def iterate_batches(
        ...
        elif isinstance(vectors, dict) and any(isinstance(value, np.ndarray) for value in vectors.values()):
    ...
    def _vector_batches_from_numpy_named_vectors(
        ...
        vector_batches = ({name: vectors[name][i].tolist() for name in vectors.keys()} for i in range(num_vectors))
