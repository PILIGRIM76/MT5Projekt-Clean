...
class GrpcToRest:
    ...
    def convert_filter(cls, model: grpc.Filter) -> rest.Filter:
        return rest.Filter(
            ...
            min_should=(
                rest.MinShould(
                    conditions=[cls.convert_condition(condition) for condition in model.min_should.conditions],
    ...
    def convert_geo_polygon(cls, model: grpc.GeoPolygon) -> rest.GeoPolygon:
        return rest.GeoPolygon(
            ...
            interiors=([cls.convert_geo_line_string(interior) for interior in model.interiors] if model.interiors else None),
        )
    ...
    def convert_collection_info(cls, model: grpc.CollectionInfo) -> rest.CollectionInfo:
        return rest.CollectionInfo(
            ...
            update_queue=(cls.convert_update_queue_info(model.update_queue) if model.HasField("update_queue") else None),
        )
    ...
    def convert_collection_config(cls, model: grpc.CollectionConfig) -> rest.CollectionConfig:
        return rest.CollectionConfig(
            ...
            quantization_config=(
                cls.convert_quantization_config(model.quantization_config) if model.HasField("quantization_config") else None
            ),
            strict_mode_config=(
    ...
    @classmethod
    def convert_max_optimization_threads(cls, model: grpc.MaxOptimizationThreads) -> rest.MaxOptimizationThreads:
    ...
    def convert_optimizer_config(cls, model: grpc.OptimizersConfigDiff) -> rest.OptimizersConfig:
        ...
        if model.HasField("deprecated_max_optimization_threads"):
            ...
            max_optimization_threads = cls.convert_max_optimization_threads(model.max_optimization_threads)
        ...
        return rest.OptimizersConfig(
            ...
            prevent_unoptimized=(model.prevent_unoptimized if model.HasField("prevent_unoptimized") else None),
        )
    ...
    def convert_payload_schema_info(cls, model: grpc.PayloadSchemaInfo) -> rest.PayloadIndexInfo:
        return rest.PayloadIndexInfo(
            ...
            params=(cls.convert_payload_schema_params(model.params) if model.HasField("params") else None),
            points=model.points,
        )

    @classmethod
    def convert_payload_schema_params(cls, model: grpc.PayloadIndexParams) -> rest.PayloadSchemaParams:
    ...
    @classmethod
    def convert_has_vector_condition(cls, model: grpc.HasVectorCondition) -> rest.HasVectorCondition:
    ...
    def convert_search_params(cls, model: grpc.SearchParams) -> rest.SearchParams:
        return rest.SearchParams(
            ...
            quantization=(
                cls.convert_quantization_search_params(model.quantization) if model.HasField("quantization") else None
            ),
            indexed_only=model.indexed_only if model.HasField("indexed_only") else None,
            acorn=cls.convert_acorn_search_params(model.acorn) if model.HasField("acorn") else None,
        )
    ...
    def convert_scored_point(cls, model: grpc.ScoredPoint) -> rest.ScoredPoint:
        return construct(
            ...
            score=model.score,
            vector=(cls.convert_vectors_output(model.vectors) if model.HasField("vectors") else None),
            version=model.version,
            shard_key=(cls.convert_shard_key(model.shard_key) if model.HasField("shard_key") else None),
            order_value=(cls.convert_order_value(model.order_value) if model.HasField("order_value") else None),
        )
    ...
    def convert_field_condition(cls, model: grpc.FieldCondition) -> rest.FieldCondition:
        ...
        geo_polygon = cls.convert_geo_polygon(model.geo_polygon) if model.HasField("geo_polygon") else None
    ...
    def convert_collection_params(cls, model: grpc.CollectionParams) -> rest.CollectionParams:
        return rest.CollectionParams(
            ...
            read_fan_out_factor=(model.read_fan_out_factor if model.HasField("read_fan_out_factor") else None),
            read_fan_out_delay_ms=(model.read_fan_out_delay_ms if model.HasField("read_fan_out_delay_ms") else None),
            write_consistency_factor=(model.write_consistency_factor if model.HasField("write_consistency_factor") else None),
            sparse_vectors=(
            ...
            sharding_method=(
                cls.convert_sharding_method(model.sharding_method) if model.HasField("sharding_method") else None
            ),
        )
    ...
    def convert_optimizers_config_diff(cls, model: grpc.OptimizersConfigDiff) -> rest.OptimizersConfigDiff:
        ...
        return rest.OptimizersConfigDiff(
            default_segment_number=(model.default_segment_number if model.HasField("default_segment_number") else None),
            deleted_threshold=(model.deleted_threshold if model.HasField("deleted_threshold") else None),
            flush_interval_sec=(int(model.flush_interval_sec) if model.HasField("flush_interval_sec") else None),
            indexing_threshold=(model.indexing_threshold if model.HasField("indexing_threshold") else None),
            max_optimization_threads=max_optimization_threads,
            max_segment_size=(model.max_segment_size if model.HasField("max_segment_size") else None),
            memmap_threshold=(model.memmap_threshold if model.HasField("memmap_threshold") else None),
    ...
    def convert_update_collection(cls, model: grpc.UpdateCollection) -> rest.UpdateCollection:
        return rest.UpdateCollection(
            vectors=(cls.convert_vectors_config_diff(model.vectors_config) if model.HasField("vectors_config") else None),
            optimizers_config=(
                cls.convert_optimizers_config_diff(model.optimizers_config) if model.HasField("optimizers_config") else None
            ),
            params=(cls.convert_collection_params_diff(model.params) if model.HasField("params") else None),
            hnsw_config=(cls.convert_hnsw_config_diff(model.hnsw_config) if model.HasField("hnsw_config") else None),
            quantization_config=(
    ...
    def convert_vector_params(cls, model: grpc.VectorParams) -> rest.VectorParams:
        return rest.VectorParams(
            ...
            multivector_config=(
                cls.convert_multivector_config(model.multivector_config) if model.HasField("multivector_config") else None
            ),
        )

    @classmethod
    def convert_multivector_config(cls, model: grpc.MultiVectorConfig) -> rest.MultiVectorConfig:
        return rest.MultiVectorConfig(comparator=cls.convert_multivector_comparator(model.comparator))

    @classmethod
    def convert_multivector_comparator(cls, model: grpc.MultiVectorComparator) -> rest.MultiVectorComparator:
    ...
    def convert_vectors_config(cls, model: grpc.VectorsConfig) -> rest.VectorsConfig:
        ...
        if name == "params_map":
            return dict((key, cls.convert_vector_params(vec_params)) for key, vec_params in val.map.items())
        raise ValueError(f"invalid VectorsConfig model: {model}")  # pragma: no cover

    @classmethod
    def _convert_vector(cls, model: grpc.Vector | grpc.VectorOutput) -> tuple[
        str | None,
        (list[float] | list[list[float]] | rest.SparseVector | grpc.Document | grpc.Image | grpc.InferenceObject),
    ]:
    ...
    ) -> list[float] | list[list[float]] | rest.SparseVector | rest.Document | rest.Image | rest.InferenceObject:
    ...
    @classmethod
    def convert_vector_output(cls, model: grpc.VectorOutput) -> list[float] | list[list[float]] | rest.SparseVector:
        name, val = cls._convert_vector(model)
        if name is None:
            return val
        raise ValueError(f"invalid Vector model: {model}")  # pragma: no cover
    ...
    @classmethod
    def convert_named_vectors_output(cls, model: grpc.NamedVectorsOutput) -> dict[str, rest.VectorOutput]:
        vectors = {}
        for name, vector in model.vectors.items():
    ...
    def convert_recommend_input(cls, model: grpc.RecommendInput) -> rest.RecommendInput:
        return rest.RecommendInput(
            ...
            strategy=(cls.convert_recommend_strategy(model.strategy) if model.HasField("strategy") else None),
        )
    ...
    def convert_formula_query(cls, model: grpc.Formula) -> rest.FormulaQuery:
        ...
        return rest.FormulaQuery(formula=cls.convert_expression(model.expression), defaults=defaults)
    ...
    def convert_expression(cls, model: grpc.Expression) -> rest.Expression:
        ...
        if name == "lin_decay":
            return rest.LinDecayExpression(lin_decay=cls.convert_decay_params_expression(model.lin_decay))
        if name == "exp_decay":
            return rest.ExpDecayExpression(exp_decay=cls.convert_decay_params_expression(model.exp_decay))
        if name == "gauss_decay":
            return rest.GaussDecayExpression(gauss_decay=cls.convert_decay_params_expression(model.gauss_decay))
    ...
    @classmethod
    def convert_decay_params_expression(cls, model: grpc.DecayParamsExpression) -> rest.DecayParamsExpression:
    ...
    def convert_mmr(cls, model: grpc.Mmr) -> rest.Mmr:
        return rest.Mmr(
            ...
            candidates_limit=model.candidates_limit if model.HasField("candidates_limit") else None,
        )
    ...
    def convert_query(cls, model: grpc.Query) -> rest.Query:
        ...
        if name == "nearest_with_mmr":
            ...
            return rest.NearestQuery(nearest=cls.convert_vector_input(val.nearest), mmr=cls.convert_mmr(val.mmr))
        ...
        if name == "relevance_feedback":
            return rest.RelevanceFeedbackQuery(relevance_feedback=cls.convert_relevance_feedback_input(val))

        raise ValueError(f"invalid Query model: {model}")  # pragma: no cover

    @classmethod
    def convert_relevance_feedback_input(cls, model: grpc.RelevanceFeedbackInput) -> rest.RelevanceFeedbackInput:
    ...
    def convert_feedback_strategy(cls, model: grpc.FeedbackStrategy) -> rest.FeedbackStrategy:
        ...
        if name == "naive":
            return rest.NaiveFeedbackStrategy(naive=cls.convert_naive_feedback_strategy_params(model.naive))

        raise ValueError(f"invalid FeedbackStrategy model: {model}")  # pragma: no cover

    @classmethod
    def convert_naive_feedback_strategy_params(cls, model: grpc.NaiveFeedbackStrategy) -> rest.NaiveFeedbackStrategyParams:
    ...
    def convert_prefetch_query(cls, model: grpc.PrefetchQuery) -> rest.Prefetch:
        return rest.Prefetch(
            prefetch=(
                [cls.convert_prefetch_query(prefetch) for prefetch in model.prefetch] if len(model.prefetch) != 0 else None
            ),
    ...
    def convert_query_points(cls, model: grpc.QueryPoints) -> rest.QueryRequest:
        return rest.QueryRequest(
            shard_key=(
                cls.convert_shard_key_selector(model.shard_key_selector) if model.HasField("shard_key_selector") else None
            ),
            prefetch=(
                [cls.convert_prefetch_query(prefetch) for prefetch in model.prefetch] if len(model.prefetch) != 0 else None
            ),
            ...
            with_vector=(cls.convert_with_vectors_selector(model.with_vectors) if model.HasField("with_vectors") else None),
    ...
    def convert_text_index_params(cls, model: grpc.TextIndexParams) -> rest.TextIndexParams:
        return rest.TextIndexParams(
            ...
            stopwords=cls.convert_stopwords(model.stopwords) if model.HasField("stopwords") else None,
            on_disk=model.on_disk if model.HasField("on_disk") else None,
    ...
    def convert_snowball_parameters(cls, model: grpc.SnowballParams) -> rest.SnowballParams:
        return rest.SnowballParams(type=rest.Snowball.SNOWBALL, language=rest.SnowballLanguage(model.language))

    @classmethod
    def convert_integer_index_params(cls, model: grpc.IntegerIndexParams) -> rest.IntegerIndexParams:
    ...
    @classmethod
    def convert_keyword_index_params(cls, model: grpc.KeywordIndexParams) -> rest.KeywordIndexParams:
    ...
    @classmethod
    def convert_datetime_index_params(cls, model: grpc.DatetimeIndexParams) -> rest.DatetimeIndexParams:
    ...
    @classmethod
    def convert_scalar_quantization_config(cls, model: grpc.ScalarQuantization) -> rest.ScalarQuantizationConfig:
    ...
    @classmethod
    def convert_product_quantization_config(cls, model: grpc.ProductQuantization) -> rest.ProductQuantizationConfig:
    ...
    @classmethod
    def convert_binary_quantization_config(cls, model: grpc.BinaryQuantization) -> rest.BinaryQuantizationConfig:
        return rest.BinaryQuantizationConfig(
            always_ram=model.always_ram if model.HasField("always_ram") else None,
            encoding=cls.convert_binary_quantization_encoding(model.encoding) if model.HasField("encoding") else None,
            query_encoding=(
    ...
    @classmethod
    def convert_binary_quantization_encoding(cls, model: grpc.BinaryQuantizationEncoding) -> rest.BinaryQuantizationEncoding:
    ...
    def convert_binary_quantization_query_encoding(
        ...
        name = model.WhichOneof("variant")
        if name is None:
            raise ValueError(f"invalid BinaryQuantizationQueryEncoding model: {model}")  # pragma: no cover
        ...
        if name == "setting":
            ...
            raise ValueError(f"invalid BinaryQuantizationQueryEncoding setting: {val}")  # pragma: no cover
        raise ValueError(f"invalid BinaryQuantizationQueryEncoding model: {model}")  # pragma: no cover
    ...
    @classmethod
    def convert_quantization_config(cls, model: grpc.QuantizationConfig) -> rest.QuantizationConfig:
    ...
    @classmethod
    def convert_quantization_search_params(cls, model: grpc.QuantizationSearchParams) -> rest.QuantizationSearchParams:
    ...
    def convert_with_lookup(cls, model: grpc.WithLookup) -> rest.WithLookup:
        return rest.WithLookup(
            ...
            with_payload=(cls.convert_with_payload_selector(model.with_payload) if model.HasField("with_payload") else None),
            with_vectors=(cls.convert_with_vectors_selector(model.with_vectors) if model.HasField("with_vectors") else None),
        )

    @classmethod
    def convert_quantization_config_diff(cls, model: grpc.QuantizationConfigDiff) -> rest.QuantizationConfigDiff:
        name = model.WhichOneof("quantization")
        if name is None:
    ...
    def convert_vectors_config_diff(cls, model: grpc.VectorsConfigDiff) -> rest.VectorsConfigDiff:
        ...
        if name == "params_map":
            return dict((key, cls.convert_vector_params_diff(vec_params)) for key, vec_params in val.map.items())
        raise ValueError(f"invalid VectorsConfigDiff model: {model}")  # pragma: no cover

    @classmethod
    def convert_points_update_operation(cls, model: grpc.PointsUpdateOperation) -> rest.UpdateOperation:
        ...
        if name == "upsert":
            shard_key_selector = cls.convert_shard_key(val.shard_key_selector) if val.HasField("shard_key_selector") else None
            update_filter = cls.convert_filter(val.update_filter) if val.HasField("update_filter") else None
            update_mode = cls.convert_update_mode(val.update_mode) if val.HasField("update_mode") else None
            ...
            shard_key_selector = val.shard_key_selector if val.HasField("shard_key_selector") else None
            points_selector = cls.convert_points_selector(val.points, shard_key_selector=shard_key_selector)
            return rest.DeleteOperation(delete=points_selector)
        elif name == "set_payload":
            shard_key_selector = val.shard_key_selector if val.HasField("shard_key_selector") else None
            points_selector = cls.convert_points_selector(val.points_selector, shard_key_selector=shard_key_selector)
            points = None
            filter_ = None
            if isinstance(points_selector, rest.PointIdsList):
                ...
                raise ValueError(f"invalid PointsSelector model: {points_selector}")  # pragma: no cover
    ...
    @classmethod
    def convert_sparse_vector_params(cls, model: grpc.SparseVectorParams) -> rest.SparseVectorParams:
        return rest.SparseVectorParams(
            index=(cls.convert_sparse_index_config(model.index) if model.HasField("index") is not None else None),
            modifier=(cls.convert_modifier(model.modifier) if model.HasField("modifier") else None),
        )

    @classmethod
    def convert_sparse_vector_config(cls, model: grpc.SparseVectorConfig) -> dict[str, rest.SparseVectorParams]:
    ...
    def convert_cluster_operations(
        ...
        if isinstance(model, grpc.AbortShardTransfer):
            return rest.AbortTransferOperation(abort_transfer=cls.convert_abort_shard_transfer(model))
        ...
        if isinstance(model, grpc.CreateShardKey):
            return rest.CreateShardingKeyOperation(create_sharding_key=cls.convert_create_shard_key(model))

        if isinstance(model, grpc.DeleteShardKey):
            return rest.DropShardingKeyOperation(drop_sharding_key=cls.convert_delete_shard_key(model))

        if isinstance(model, grpc.RestartTransfer):
            return rest.RestartTransferOperation(restart_transfer=cls.convert_restart_transfer(model))

        if isinstance(model, grpc.ReplicatePoints):
            return rest.ReplicatePointsOperation(replicate_points=cls.convert_replicate_points(model))
    ...
    def convert_replicate_shard(cls, model: grpc.ReplicateShard) -> rest.ReplicateShard:
        if model.HasField("to_shard_id"):
            raise ValueError("to_shard_id is a field for internal purposes, can't be converted to rest")  # pragma: no cover
    ...
    @classmethod
    def convert_abort_shard_transfer(cls, model: grpc.AbortShardTransfer) -> rest.AbortShardTransfer:
        if model.HasField("to_shard_id"):
            raise ValueError("to_shard_id is a field for internal purposes, can't be converted to rest")  # pragma: no cover
        return rest.AbortShardTransfer(shard_id=model.shard_id, to_peer_id=model.to_peer_id, from_peer_id=model.from_peer_id)
    ...
    def convert_create_shard_key(cls, model: grpc.CreateShardKey) -> rest.CreateShardingKey:
        return rest.CreateShardingKey(
            ...
            initial_state=cls.convert_replica_state(model.initial_state) if model.HasField("initial_state") else None,
        )
    ...
    @classmethod
    def convert_shard_transfer_method(cls, model: grpc.ShardTransferMethod) -> rest.ShardTransferMethod:
    ...
    def convert_order_by(cls, model: grpc.OrderBy) -> rest.OrderBy:
        return rest.OrderBy(
            key=model.key,
            direction=(cls.convert_direction(model.direction) if model.HasField("direction") else None),
            start_from=(cls.convert_start_from(model.start_from) if model.HasField("start_from") else None),
        )
    ...
    def convert_strict_mode_multivector(cls, model: grpc.StrictModeMultivector) -> rest.StrictModeMultivector:
        return rest.StrictModeMultivector(max_vectors=model.max_vectors if model.HasField("max_vectors") else None)
    ...
    def convert_strict_mode_sparse(cls, model: grpc.StrictModeSparse) -> rest.StrictModeSparse:
        return rest.StrictModeSparse(max_length=model.max_length if model.HasField("max_length") else None)
    ...
    def convert_strict_mode_config(cls, model: grpc.StrictModeConfig) -> rest.StrictModeConfig:
        return rest.StrictModeConfig(
            ...
            unindexed_filtering_retrieve=(
                model.unindexed_filtering_retrieve if model.HasField("unindexed_filtering_retrieve") else None
            ),
            unindexed_filtering_update=(
                model.unindexed_filtering_update if model.HasField("unindexed_filtering_update") else None
            ),
            search_max_hnsw_ef=(model.search_max_hnsw_ef if model.HasField("search_max_hnsw_ef") else None),
            search_allow_exact=(model.search_allow_exact if model.HasField("search_allow_exact") else None),
            search_max_oversampling=(model.search_max_oversampling if model.HasField("search_max_oversampling") else None),
            upsert_max_batchsize=(model.upsert_max_batchsize if model.HasField("upsert_max_batchsize") else None),
            max_collection_vector_size_bytes=(
                model.max_collection_vector_size_bytes if model.HasField("max_collection_vector_size_bytes") else None
            ),
            read_rate_limit=model.read_rate_limit if model.HasField("read_rate_limit") else None,
            write_rate_limit=(model.write_rate_limit if model.HasField("write_rate_limit") else None),
            max_collection_payload_size_bytes=(
                model.max_collection_payload_size_bytes if model.HasField("max_collection_payload_size_bytes") else None
            ),
            max_points_count=(model.max_points_count if model.HasField("max_points_count") else None),
            filter_max_conditions=(model.filter_max_conditions if model.HasField("filter_max_conditions") else None),
            condition_max_size=(model.condition_max_size if model.HasField("condition_max_size") else None),
            multivector_config=(
            ...
            sparse_config=(
                cls.convert_strict_mode_sparse_config(model.sparse_config) if model.HasField("sparse_config") else None
            ),
            max_payload_index_count=model.max_payload_index_count if model.HasField("max_payload_index_count") else None,
        )

    @classmethod
    def convert_strict_mode_config_output(cls, model: grpc.StrictModeConfig) -> rest.StrictModeConfigOutput:
        return rest.StrictModeConfigOutput(
            ...
            sparse_config=(
                cls.convert_strict_mode_sparse_config_output(model.sparse_config) if model.HasField("sparse_config") else None
            ),
            max_payload_index_count=model.max_payload_index_count if model.HasField("max_payload_index_count") else None,
        )
    ...
    def convert_strict_mode_multivector_config_output(
        ...
        return dict((key, cls.convert_strict_mode_multivector_output(val)) for key, val in model.multivector_config.items())

    @classmethod
    def convert_strict_mode_sparse_config_output(cls, model: grpc.StrictModeSparseConfig) -> rest.StrictModeSparseConfigOutput:
        return dict((key, cls.convert_strict_mode_sparse_output(val)) for key, val in model.sparse_config.items())

    @classmethod
    def convert_strict_mode_sparse_output(cls, model: grpc.StrictModeSparse) -> rest.StrictModeSparseOutput:
        return rest.StrictModeSparseOutput(max_length=model.max_length if model.HasField("max_length") else None)

    @classmethod
    def convert_strict_mode_multivector_output(cls, model: grpc.StrictModeMultivector) -> rest.StrictModeMultivectorOutput:
        return rest.StrictModeMultivectorOutput(max_vectors=model.max_vectors if model.HasField("max_vectors") else None)

    @classmethod
    def convert_collection_cluster_info(cls, model: grpc.CollectionClusterInfoResponse) -> rest.CollectionClusterInfo:
        return rest.CollectionClusterInfo(
            peer_id=model.peer_id,
            shard_count=model.shard_count,
            local_shards=[cls.convert_local_shard_info(local_shard) for local_shard in model.local_shards],
            remote_shards=[cls.convert_remote_shard_info(remote_shard) for remote_shard in model.remote_shards],
            shard_transfers=[
                cls.convert_shard_transfer_info(shard_transfer_info) for shard_transfer_info in model.shard_transfers
            ],
            resharding_operations=[
                cls.convert_resharding_info(resharding_operation) for resharding_operation in model.resharding_operations
            ],
        )
    ...
    @classmethod
    def convert_resharding_direction(cls, model: grpc.ReshardingDirection) -> rest.ReshardingDirection:
...
class RestToGrpc:
    ...
    def convert_collection_info(cls, model: rest.CollectionInfo) -> grpc.CollectionInfo:
        return grpc.CollectionInfo(
            ...
            optimizer_status=cls.convert_optimizer_status(model.optimizer_status),
            payload_schema=(cls.convert_payload_schema(model.payload_schema) if model.payload_schema is not None else None),
            segments_count=model.segments_count,
            status=cls.convert_collection_status(model.status),
            points_count=model.points_count,
            update_queue=(cls.convert_update_queue_info(model.update_queue) if model.update_queue is not None else None),
        )
    ...
    @classmethod
    def convert_payload_schema_params(cls, model: rest.PayloadSchemaParams) -> grpc.PayloadIndexParams:
        ...
        if isinstance(model, rest.IntegerIndexParams):
            return grpc.PayloadIndexParams(integer_index_params=cls.convert_integer_index_params(model))

        if isinstance(model, rest.KeywordIndexParams):
            return grpc.PayloadIndexParams(keyword_index_params=cls.convert_keyword_index_params(model))

        if isinstance(model, rest.FloatIndexParams):
            return grpc.PayloadIndexParams(float_index_params=cls.convert_float_index_params(model))
        ...
        if isinstance(model, rest.DatetimeIndexParams):
            return grpc.PayloadIndexParams(datetime_index_params=cls.convert_datetime_index_params(model))
    ...
    def convert_has_id_condition(cls, model: rest.HasIdCondition) -> grpc.HasIdCondition:
        return grpc.HasIdCondition(has_id=[cls.convert_extended_point_id(idx) for idx in model.has_id])

    @classmethod
    def convert_has_vector_condition(cls, model: rest.HasVectorCondition) -> grpc.HasVectorCondition:
    ...
    def convert_rename_alias(cls, model: rest.RenameAlias) -> grpc.RenameAlias:
        return grpc.RenameAlias(old_alias_name=model.old_alias_name, new_alias_name=model.new_alias_name)
    ...
    def convert_search_params(cls, model: rest.SearchParams) -> grpc.SearchParams:
        return grpc.SearchParams(
            ...
            exact=model.exact,
            quantization=(
                cls.convert_quantization_search_params(model.quantization) if model.quantization is not None else None
            ),
            indexed_only=model.indexed_only,
            acorn=(cls.convert_acorn_search_params(model.acorn) if model.acorn is not None else None),
        )
    ...
    def convert_scored_point(cls, model: rest.ScoredPoint) -> grpc.ScoredPoint:
        return grpc.ScoredPoint(
            ...
            score=model.score,
            vectors=(cls.convert_vector_struct_output(model.vector) if model.vector is not None else None),
            version=model.version,
    ...
    def convert_field_condition(cls, model: rest.FieldCondition) -> grpc.FieldCondition:
        ...
        if model.geo_radius is not None:
            return grpc.FieldCondition(key=model.key, geo_radius=cls.convert_geo_radius(model.geo_radius))
        if model.geo_polygon is not None:
            return grpc.FieldCondition(key=model.key, geo_polygon=cls.convert_geo_polygon(model.geo_polygon))
        if model.values_count is not None:
            return grpc.FieldCondition(key=model.key, values_count=cls.convert_values_count(model.values_count))
    ...
    def convert_collection_config(cls, model: rest.CollectionConfig) -> grpc.CollectionConfig:
        return grpc.CollectionConfig(
            ...
            wal_config=cls.convert_wal_config(model.wal_config),
            quantization_config=(
                cls.convert_quantization_config(model.quantization_config) if model.quantization_config is not None else None
            ),
            strict_mode_config=(
                cls.convert_strict_mode_config_output(model.strict_mode_config)
    ...
    def convert_collection_params(cls, model: rest.CollectionParams) -> grpc.CollectionParams:
        return grpc.CollectionParams(
            ...
            sparse_vectors_config=(
                cls.convert_sparse_vector_config(model.sparse_vectors) if model.sparse_vectors is not None else None
            ),
            sharding_method=(
                cls.convert_sharding_method(model.sharding_method) if model.sharding_method is not None else None
            ),
        )

    @classmethod
    def convert_max_optimization_threads(cls, model: rest.MaxOptimizationThreads) -> grpc.MaxOptimizationThreads:
    ...
    def convert_update_collection(cls, model: rest.UpdateCollection, collection_name: str) -> grpc.UpdateCollection:
        return grpc.UpdateCollection(
            ...
            optimizers_config=(
                cls.convert_optimizers_config_diff(model.optimizers_config) if model.optimizers_config is not None else None
            ),
            vectors_config=(cls.convert_vectors_config_diff(model.vectors) if model.vectors is not None else None),
            params=(cls.convert_collection_params_diff(model.params) if model.params is not None else None),
    ...
    def convert_payload_selector(cls, model: rest.PayloadSelector) -> grpc.WithPayloadSelector:
        if isinstance(model, rest.PayloadSelectorInclude):
            return grpc.WithPayloadSelector(include=grpc.PayloadIncludeSelector(fields=model.include))
        if isinstance(model, rest.PayloadSelectorExclude):
            return grpc.WithPayloadSelector(exclude=grpc.PayloadExcludeSelector(fields=model.exclude))
    ...
    def convert_order_by(cls, model: rest.OrderBy) -> grpc.OrderBy:
        return grpc.OrderBy(
            key=model.key,
            direction=(cls.convert_direction(model.direction) if model.direction is not None else None),
            start_from=(cls.convert_start_from(model.start_from) if model.start_from is not None else None),
        )
    ...
    def convert_vector_params(cls, model: rest.VectorParams) -> grpc.VectorParams:
        return grpc.VectorParams(
            ...
            multivector_config=(
                cls.convert_multivector_config(model.multivector_config) if model.multivector_config is not None else None
            ),
        )

    @classmethod
    def convert_multivector_config(cls, model: rest.MultiVectorConfig) -> grpc.MultiVectorConfig:
        return grpc.MultiVectorConfig(comparator=cls.convert_multivector_comparator(model.comparator))

    @classmethod
    def convert_multivector_comparator(cls, model: rest.MultiVectorComparator) -> grpc.MultiVectorComparator:
    ...
    def convert_vectors_config(cls, model: rest.VectorsConfig) -> grpc.VectorsConfig:
        if isinstance(model, rest.VectorParams):
            ...
            return grpc.VectorsConfig(
                params_map=grpc.VectorParamsMap(map=dict((key, cls.convert_vector_params(val)) for key, val in model.items()))
            )
        else:
            raise ValueError(f"invalid VectorsConfig model: {model}")  # pragma: no cover
            ...
            if len(vector) != 0 and isinstance(
                ...
                return grpc.Vector(
                    multi_dense=grpc.MultiDenseVector(
                        vectors=[grpc.DenseVector(data=inner_vector) for inner_vector in vector]  # type: ignore[union-attr]
                    )
                )
    ...
    def convert_multi_dense_vector(cls, model: list[list[float]]) -> grpc.MultiDenseVector:
        return grpc.MultiDenseVector(vectors=[cls.convert_dense_vector(vector) for vector in model])
    ...
    def convert_recommend_input(cls, model: rest.RecommendInput) -> grpc.RecommendInput:
        return grpc.RecommendInput(
            ...
            strategy=(cls.convert_recommend_strategy(model.strategy) if model.strategy is not None else None),
        )
    ...
    def convert_context_input(cls, model: rest.ContextInput) -> grpc.ContextInput:
        if isinstance(model, list):
            return grpc.ContextInput(pairs=[cls.convert_context_input_pair(pair) for pair in model])
    ...
    def convert_query(cls, model: rest.Query) -> grpc.Query:
        ...
        if isinstance(model, rest.RelevanceFeedbackQuery):
            return grpc.Query(relevance_feedback=cls.convert_relevance_feedback_input(model.relevance_feedback))

        raise ValueError(f"invalid Query model: {model}")  # pragma: no cover

    @classmethod
    def convert_relevance_feedback_input(cls, model: rest.RelevanceFeedbackInput) -> grpc.RelevanceFeedbackInput:
    ...
    def convert_feedback_strategy(cls, model: rest.FeedbackStrategy) -> grpc.FeedbackStrategy:
        if isinstance(model, rest.NaiveFeedbackStrategy):
            return grpc.FeedbackStrategy(naive=cls.convert_naive_feedback_strategy_params(model.naive))

        raise ValueError(f"invalid FeedbackStrategy model: {model}")  # pragma: no cover

    @classmethod
    def convert_naive_feedback_strategy_params(cls, model: rest.NaiveFeedbackStrategyParams) -> grpc.NaiveFeedbackStrategy:
    ...
    def convert_expression(cls, model: rest.Expression) -> grpc.Expression:
        ...
        if isinstance(model, rest.GaussDecayExpression):
            return grpc.Expression(gauss_decay=cls.convert_decay_params_expression(model.gauss_decay))
    ...
    @classmethod
    def convert_decay_params_expression(cls, model: rest.DecayParamsExpression) -> grpc.DecayParamsExpression:
    ...
    @classmethod
    def convert_query_request(cls, model: rest.QueryRequest, collection_name: str) -> grpc.QueryPoints:
        prefetch = [model.prefetch] if isinstance(model.prefetch, rest.Prefetch) else model.prefetch
        return grpc.QueryPoints(
            collection_name=collection_name,
            prefetch=([cls.convert_prefetch_query(p) for p in prefetch] if model.prefetch is not None else None),
            query=cls.convert_query_interface(model.query) if model.query is not None else None,
            using=model.using,
            filter=cls.convert_filter(model.filter) if model.filter is not None else None,
            params=cls.convert_search_params(model.params) if model.params is not None else None,
            ...
            with_payload=(cls.convert_with_payload_interface(model.with_payload) if model.with_payload is not None else None),
            shard_key_selector=(cls.convert_shard_key_selector(model.shard_key) if model.shard_key is not None else None),
            lookup_from=(cls.convert_lookup_location(model.lookup_from) if model.lookup_from is not None else None),
        )

    @classmethod
    def convert_query_points(cls, model: rest.QueryRequest, collection_name: str) -> grpc.QueryPoints:
    ...
    def convert_text_index_params(cls, model: rest.TextIndexParams) -> grpc.TextIndexParams:
        return grpc.TextIndexParams(
            ...
            on_disk=model.on_disk,
            stopwords=cls.convert_stopwords(model.stopwords) if model.stopwords is not None else None,
    ...
    @classmethod
    def convert_integer_index_params(cls, model: rest.IntegerIndexParams) -> grpc.IntegerIndexParams:
    ...
    @classmethod
    def convert_keyword_index_params(cls, model: rest.KeywordIndexParams) -> grpc.KeywordIndexParams:
        return grpc.KeywordIndexParams(is_tenant=model.is_tenant, on_disk=model.on_disk, enable_hnsw=model.enable_hnsw)

    @classmethod
    def convert_float_index_params(cls, model: rest.FloatIndexParams) -> grpc.FloatIndexParams:
        return grpc.FloatIndexParams(is_principal=model.is_principal, on_disk=model.on_disk, enable_hnsw=model.enable_hnsw)
    ...
    @classmethod
    def convert_datetime_index_params(cls, model: rest.DatetimeIndexParams) -> grpc.DatetimeIndexParams:
        return grpc.DatetimeIndexParams(is_principal=model.is_principal, on_disk=model.on_disk, enable_hnsw=model.enable_hnsw)

    @classmethod
    def convert_uuid_index_params(cls, model: rest.UuidIndexParams) -> grpc.UuidIndexParams:
        return grpc.UuidIndexParams(is_tenant=model.is_tenant, on_disk=model.on_disk, enable_hnsw=model.enable_hnsw)
    ...
    @classmethod
    def convert_scalar_quantization_config(cls, model: rest.ScalarQuantizationConfig) -> grpc.ScalarQuantization:
    ...
    @classmethod
    def convert_product_quantization_config(cls, model: rest.ProductQuantizationConfig) -> grpc.ProductQuantization:
    ...
    @classmethod
    def convert_binary_quantization_config(cls, model: rest.BinaryQuantizationConfig) -> grpc.BinaryQuantization:
        return grpc.BinaryQuantization(
            always_ram=model.always_ram,
            encoding=cls.convert_binary_quantization_encoding(model.encoding) if model.encoding is not None else None,
            query_encoding=(
                cls.convert_binary_quantization_query_encoding(model.query_encoding)
    ...
    def convert_binary_quantization_query_encoding(
        ...
        if model == rest.BinaryQuantizationQueryEncoding.DEFAULT:
            return grpc.BinaryQuantizationQueryEncoding(setting=grpc.BinaryQuantizationQueryEncoding.Setting.Default)

        if model == rest.BinaryQuantizationQueryEncoding.BINARY:
            return grpc.BinaryQuantizationQueryEncoding(setting=grpc.BinaryQuantizationQueryEncoding.Setting.Binary)

        if model == rest.BinaryQuantizationQueryEncoding.SCALAR4BITS:
            return grpc.BinaryQuantizationQueryEncoding(setting=grpc.BinaryQuantizationQueryEncoding.Setting.Scalar4Bits)

        if model == rest.BinaryQuantizationQueryEncoding.SCALAR8BITS:
            return grpc.BinaryQuantizationQueryEncoding(setting=grpc.BinaryQuantizationQueryEncoding.Setting.Scalar8Bits)
    ...
    @classmethod
    def convert_quantization_config(cls, model: rest.QuantizationConfig) -> grpc.QuantizationConfig:
        if isinstance(model, rest.ScalarQuantization):
            return grpc.QuantizationConfig(scalar=cls.convert_scalar_quantization_config(model.scalar))
        if isinstance(model, rest.ProductQuantization):
            return grpc.QuantizationConfig(product=cls.convert_product_quantization_config(model.product))
        if isinstance(model, rest.BinaryQuantization):
            return grpc.QuantizationConfig(binary=cls.convert_binary_quantization_config(model.binary))
        else:
            raise ValueError(f"invalid QuantizationConfig model: {model}")  # pragma: no cover

    @classmethod
    def convert_quantization_search_params(cls, model: rest.QuantizationSearchParams) -> grpc.QuantizationSearchParams:
    ...
    def convert_with_lookup(cls, model: rest.WithLookup) -> grpc.WithLookup:
        return grpc.WithLookup(
            collection=model.collection,
            with_vectors=(cls.convert_with_vectors(model.with_vectors) if model.with_vectors is not None else None),
            with_payload=(cls.convert_with_payload_interface(model.with_payload) if model.with_payload is not None else None),
        )

    @classmethod
    def convert_quantization_config_diff(cls, model: rest.QuantizationConfigDiff) -> grpc.QuantizationConfigDiff:
        if isinstance(model, rest.ScalarQuantization):
            return grpc.QuantizationConfigDiff(scalar=cls.convert_scalar_quantization_config(model.scalar))
        if isinstance(model, rest.ProductQuantization):
            return grpc.QuantizationConfigDiff(product=cls.convert_product_quantization_config(model.product))
        if isinstance(model, rest.BinaryQuantization):
            return grpc.QuantizationConfigDiff(binary=cls.convert_binary_quantization_config(model.binary))
        ...
        if isinstance(model, dict) and len(model) == 1 and "" in model:
            ...
            return grpc.VectorsConfigDiff(
                params_map=grpc.VectorParamsDiffMap(
                    map=dict((key, cls.convert_vector_params_diff(val)) for key, val in model.items())
                )
            )
        else:
            raise ValueError(f"invalid VectorsConfigDiff model: {model}")  # pragma: no cover
    ...
    @classmethod
    def convert_point_insert_operation(cls, model: rest.PointInsertOperations) -> list[grpc.PointStruct]:
        # shard key and update_filter are converted in the parent function
        if isinstance(model, rest.PointsBatch):
            vectors_batch: list[grpc.Vectors] = cls.convert_batch_vector_struct(model.batch.vectors, len(model.batch.ids))
            return [
                grpc.PointStruct(
                    id=RestToGrpc.convert_extended_point_id(model.batch.ids[idx]),
                    vectors=vectors_batch[idx],
                    payload=(
                        RestToGrpc.convert_payload(model.batch.payloads[idx]) if model.batch.payloads is not None else None
                    ),
                )
    ...
    @classmethod
    def convert_points_update_operation(cls, model: rest.UpdateOperation) -> grpc.PointsUpdateOperation:
        if isinstance(model, rest.UpsertOperation):
            shard_key_selector = cls.convert_shard_key_selector(model.upsert.shard_key) if model.upsert.shard_key else None
            update_filter = cls.convert_filter(model.upsert.update_filter) if model.upsert.update_filter else None
            update_mode = cls.convert_update_mode(model.upsert.update_mode) if model.upsert.update_mode is not None else None
            ...
            shard_key_selector = cls.convert_shard_key_selector(model.delete.shard_key) if model.delete.shard_key else None
            ...
            shard_key_selector = (
                cls.convert_shard_key_selector(model.set_payload.shard_key) if model.set_payload.shard_key else None
            )
            ...
            if model.overwrite_payload.points:
                ...
                raise ValueError(f"invalid OverwritePayloadOperation model: {model}")  # pragma: no cover
            ...
            if model.delete_payload.points:
                ...
                raise ValueError(f"invalid DeletePayloadOperation model: {model}")  # pragma: no cover

            shard_key_selector = (
                cls.convert_shard_key_selector(model.delete_payload.shard_key) if model.delete_payload.shard_key else None
            )
            ...
            shard_key_selector = (
                cls.convert_shard_key_selector(model.clear_payload.shard_key) if model.clear_payload.shard_key else None
            )
            ...
            shard_key_selector = (
                cls.convert_shard_key_selector(model.update_vectors.shard_key) if model.update_vectors.shard_key else None
            )
            update_filter = (
                cls.convert_filter(model.update_vectors.update_filter) if model.update_vectors.update_filter else None
            )
            return grpc.PointsUpdateOperation(
                update_vectors=grpc.PointsUpdateOperation.UpdateVectors(
                    points=[cls.convert_point_vectors(point) for point in model.update_vectors.points],
                    shard_key_selector=shard_key_selector,
                    update_filter=update_filter,
                )
            )
            ...
            shard_key_selector = (
                cls.convert_shard_key_selector(model.delete_vectors.shard_key) if model.delete_vectors.shard_key else None
            )
    ...
    def convert_sparse_index_params(cls, model: rest.SparseIndexParams) -> grpc.SparseIndexConfig:
        return grpc.SparseIndexConfig(
            full_scan_threshold=(model.full_scan_threshold if model.full_scan_threshold is not None else None),
    ...
    @classmethod
    def convert_sparse_vector_params(cls, model: rest.SparseVectorParams) -> grpc.SparseVectorParams:
        return grpc.SparseVectorParams(
            index=(cls.convert_sparse_index_params(model.index) if model.index is not None else None),
            modifier=(cls.convert_modifier(model.modifier) if model.modifier is not None else None),
        )

    @classmethod
    def convert_sparse_vector_config(cls, model: Mapping[str, rest.SparseVectorParams]) -> grpc.SparseVectorConfig:
        return grpc.SparseVectorConfig(map=dict((key, cls.convert_sparse_vector_params(val)) for key, val in model.items()))
    ...
    def convert_shard_key_selector(cls, model: rest.ShardKeySelector) -> grpc.ShardKeySelector:
        if isinstance(model, rest.ShardKeyWithFallback):  # have to be the first, since it's a part of the union
    ...
    @classmethod
    def convert_abort_shard_transfer(cls, model: rest.AbortShardTransfer) -> grpc.AbortShardTransfer:
    ...
    def convert_create_shard_key(cls, model: rest.CreateShardingKey) -> grpc.CreateShardKey:
        return grpc.CreateShardKey(
            ...
            replication_factor=model.replication_factor if model.replication_factor is not None else None,
            placement=model.placement if model.placement is not None else None,
            initial_state=cls.convert_replica_state(model.initial_state) if model.initial_state is not None else None,
        )
    ...
    @classmethod
    def convert_shard_transfer_method(cls, model: rest.ShardTransferMethod) -> grpc.ShardTransferMethod:
    ...
    def convert_strict_mode_config(cls, model: rest.StrictModeConfig) -> grpc.StrictModeConfig:
        return grpc.StrictModeConfig(
            ...
            multivector_config=(
                cls.convert_strict_mode_multivector_config(model.multivector_config) if model.multivector_config else None
            ),
            sparse_config=(cls.convert_strict_mode_sparse_config(model.sparse_config) if model.sparse_config else None),
            max_payload_index_count=model.max_payload_index_count,
        )

    @classmethod
    def convert_strict_mode_config_output(cls, model: rest.StrictModeConfigOutput) -> grpc.StrictModeConfig:
        return grpc.StrictModeConfig(
            enabled=model.enabled,
            max_query_limit=model.max_query_limit,
            max_timeout=model.max_timeout,
            ...
            sparse_config=(cls.convert_strict_mode_sparse_config_output(model.sparse_config) if model.sparse_config else None),
            max_payload_index_count=model.max_payload_index_count,
        )
    ...
    def convert_strict_mode_multivector_config_output(
        ...
        return grpc.StrictModeMultivectorConfig(
            multivector_config=dict((key, cls.convert_strict_mode_multivector_output(val)) for key, val in model.items())
        )

    @classmethod
    def convert_strict_mode_sparse_config_output(cls, model: rest.StrictModeSparseConfigOutput) -> grpc.StrictModeSparseConfig:
        return grpc.StrictModeSparseConfig(
            sparse_config=dict((key, cls.convert_strict_mode_sparse_output(val)) for key, val in model.items())
        )

    @classmethod
    def convert_strict_mode_multivector_output(cls, model: rest.StrictModeMultivectorOutput) -> grpc.StrictModeMultivector:
        return grpc.StrictModeMultivector(
            max_vectors=model.max_vectors,
        )

    @classmethod
    def convert_strict_mode_sparse_output(cls, model: rest.StrictModeSparseOutput) -> grpc.StrictModeSparse:
        return grpc.StrictModeSparse(
            max_length=model.max_length,
        )

    @classmethod
    def convert_collection_cluster_info(cls, model: rest.CollectionClusterInfo) -> grpc.CollectionClusterInfoResponse:
        return grpc.CollectionClusterInfoResponse(
            ...
            resharding_operations=[
                cls.convert_resharding_info(resharding_operation) for resharding_operation in model.resharding_operations or []
            ],
        )
    ...
    def convert_local_shard_info(cls, model: rest.LocalShardInfo) -> grpc.LocalShardInfo:
        return grpc.LocalShardInfo(
            shard_id=model.shard_id,
            shard_key=cls.convert_shard_key(model.shard_key) if model.shard_key is not None else None,
            points_count=model.points_count,
            state=cls.convert_replica_state(model.state),
        )
    ...
    @classmethod
    def convert_resharding_direction(cls, model: rest.ReshardingDirection) -> grpc.ReshardingDirection:
