...
class ModelSchemaParser:
    ...
    def __init__(self) -> None:
        ...
        self.name_recursive_ref_mapping: dict[str, str] = {k: v for k, v in NAME_RECURSIVE_REF_MAPPING.items()}
        self.path_cache: dict[str, list[FieldPath]] = {model: convert_paths(paths) for model, paths in self._cache.items()}
    ...
    def _replace_refs(
        ...
        if isinstance(schema, dict):
            if "$ref" in schema:
                ...
                return self._replace_refs(self._defs[def_key], parent=def_key, seen_refs=copy(seen_refs))

            schemes = {}
            if "properties" in schema:
                for k, v in schema.items():
                    if k == "properties":
                        schemes[k] = self._replace_refs(schema=v, parent=parent, seen_refs=copy(seen_refs))
                    else:
                        schemes[k] = v
            else:
                for k, v in schema.items():
                    parent_key = k if isinstance(v, dict) and "properties" in v else parent
                    schemes[k] = self._replace_refs(schema=v, parent=parent_key, seen_refs=copy(seen_refs))

            return schemes
        elif isinstance(schema, list):
            return [
                self._replace_refs(schema=item, parent=parent, seen_refs=copy(seen_refs)) for item in schema  # type: ignore
            ]
        else:
            return schema
    ...
    def _find_document_paths(
        ...
        for key, value in schema.items():
            ...
            if key == "$ref":
                ...
                if model_name in self._excluded_recursive_refs:  # on the first run it might be empty
                    continue
            ...
            if isinstance(value, dict):
                document_paths.extend(
                    self._find_document_paths(value, new_path, key == "properties", seen_refs=seen_recursive_refs)
                )
    ...
    def parse_model(self, model: Type[BaseModel]) -> None:
        ...
        if "$defs" in schema:
            raw_refs = {"$ref": schema["$ref"]} if "$ref" in schema else {"properties": schema["properties"]}
        ...
        self.name_recursive_ref_mapping = {
            k: v for k, v in self.name_recursive_ref_mapping.items() if v not in self._excluded_recursive_refs
        }
