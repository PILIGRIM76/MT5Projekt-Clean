...
class Embedder:
    def __init__(self, threads: int | None = None, use_core_bm25: bool = True, **kwargs: Any) -> None:
        self.embedding_models: dict[str, list[ModelInstance[TextEmbedding]]] = defaultdict(list)
        self.sparse_embedding_models: dict[str, list[ModelInstance[SparseTextEmbedding]]] = defaultdict(list)
        self.late_interaction_embedding_models: dict[str, list[ModelInstance[LateInteractionTextEmbedding]]] = defaultdict(
            list
        )
        self.image_embedding_models: dict[str, list[ModelInstance[ImageEmbedding]]] = defaultdict(list)
    ...
    def get_or_init_model(
        ...
        **kwargs: Any,
        ...
        for instance in self.embedding_models[model_name]:
            if (deprecated and instance.deprecated) or (not deprecated and instance.options == options):
                return instance.model

        model = TextEmbedding(model_name=model_name, **options)
        model_instance: ModelInstance[TextEmbedding] = ModelInstance(model=model, options=options, deprecated=deprecated)
    ...
    def get_or_init_sparse_model(
        ...
        **kwargs: Any,
        ...
        model_instance: ModelInstance[SparseTextEmbedding] = ModelInstance(model=model, options=options, deprecated=deprecated)
    ...
    def get_or_init_late_interaction_model(
        ...
        **kwargs: Any,
        ...
        model_instance: ModelInstance[LateInteractionTextEmbedding] = ModelInstance(model=model, options=options)
    ...
    def get_or_init_late_interaction_multimodal_model(
        ...
        **kwargs: Any,
        ...
        model_instance: ModelInstance[LateInteractionMultimodalEmbedding] = ModelInstance(model=model, options=options)
    ...
    def embed(
        ...
        if texts is not None:
            if FastEmbedMisc.is_supported_text_model(model_name):
                ...
                embeddings = self._embed_late_interaction_text(texts, model_name, options, is_query, batch_size)
            elif FastEmbedMisc.is_supported_late_interaction_multimodal_model(model_name):
                embeddings = self._embed_late_interaction_multimodal_text(texts, model_name, options, batch_size)
            else:
                raise ValueError(f"Unsupported embedding model: {model_name}")
        else:
            assert images is not None  # just to satisfy mypy which can't infer it from the previous conditions
            if FastEmbedMisc.is_supported_image_model(model_name):
                ...
                embeddings = self._embed_late_interaction_multimodal_image(images, model_name, options, batch_size)
            else:
                raise ValueError(f"Unsupported embedding model: {model_name}")
    ...
    def _embed_dense_text(
        ...
        if not is_query:
            embeddings = [
                embedding.tolist() for embedding in embedding_model_inst.embed(documents=texts, batch_size=batch_size)
            ]
        else:
            embeddings = [embedding.tolist() for embedding in embedding_model_inst.query_embed(query=texts)]
        return embeddings
    ...
    def _embed_sparse_text(
        ...
        if self._use_core_bm25 and model_name.lower() == "Qdrant/bm25".lower():
            return [models.Document(text=text, model=model_name, options=options) for text in texts]

        embedding_model_inst = self.get_or_init_sparse_model(model_name=model_name, **options or {})
        if not is_query:
            embeddings = [
                models.SparseVector(
                ...
                for sparse_embedding in embedding_model_inst.embed(documents=texts, batch_size=batch_size)
            ]
        else:
            embeddings = [
                models.SparseVector(
    ...
    def _embed_late_interaction_text(
        ...
        embedding_model_inst = self.get_or_init_late_interaction_model(model_name=model_name, **options or {})
        if not is_query:
            embeddings = [
                embedding.tolist() for embedding in embedding_model_inst.embed(documents=texts, batch_size=batch_size)
            ]
    ...
    def _embed_late_interaction_multimodal_text(
        ...
        embedding_model_inst = self.get_or_init_late_interaction_multimodal_model(model_name=model_name, **options or {})
        return [embedding.tolist() for embedding in embedding_model_inst.embed_text(documents=texts, batch_size=batch_size)]
    ...
    def _embed_late_interaction_multimodal_image(
        ...
        embedding_model_inst = self.get_or_init_late_interaction_multimodal_model(model_name=model_name, **options or {})
        return [embedding.tolist() for embedding in embedding_model_inst.embed_image(images=images, batch_size=batch_size)]
    ...
    def _embed_dense_image(
        ...
        embeddings = [embedding.tolist() for embedding in embedding_model_inst.embed(images=images, batch_size=batch_size)]
        return embeddings
