...
class Inspector:
    ...
    def _inspect_model(self, model: BaseModel, paths: list[FieldPath] | None = None) -> bool:
        ...
        paths = self.parser.path_cache.get(model.__class__.__name__, []) if paths is None else paths

        for path in paths:
            type_found = self._inspect_inner_models(model, path.current, path.tail if path.tail else [])
            if type_found:
                return True
        return False

    def _inspect_inner_models(self, original_model: BaseModel, current_path: str, tail: list[FieldPath]) -> bool:
        ...
        if isinstance(model, BaseModel):
            ...
            for next_path in tail:
                type_found = self._inspect_inner_models(model, next_path.current, next_path.tail if next_path.tail else [])
                if type_found:
                    return True
            return False
