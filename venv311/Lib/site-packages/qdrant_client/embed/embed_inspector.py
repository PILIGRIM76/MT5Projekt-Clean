...
class InspectorEmbed:
    ...
    def _inspect_model(self, mod: BaseModel, paths: list[FieldPath] | None = None, accum: str | None = None) -> list[str]:
        ...
        found_paths = []
        for path in paths:
            found_paths.extend(self._inspect_inner_models(mod, path.current, path.tail if path.tail else [], accum))
        return found_paths
    ...
    def _inspect_inner_models(
        ...
        if isinstance(model, BaseModel):
            ...
            for next_path in tail:
                found_paths.extend(
                    self._inspect_inner_models(model, next_path.current, next_path.tail if next_path.tail else [], accum)
                )
