...
class ModelEmbedderWorker(Worker):
    ...
    def process(self, items: Iterable[tuple[int, Any]]) -> Iterable[tuple[int, Any]]:
        for idx, batch in items:
            yield (
                idx,
                list(self.model_embedder.embed_models_batch(batch, inference_batch_size=self.batch_size)),
            )
...
class ModelEmbedder:
    ...
    def embed_models(
        ...
        for raw_models_batch in iter_batch(raw_models, batch_size):
            yield from self.embed_models_batch(raw_models_batch, is_query, inference_batch_size=batch_size)
    ...
    def embed_models_strict(
        ...
        if parallel is None or parallel == 1 or is_small:
            ...
            for batch in pool.ordered_map(raw_models_batches, batch_size=multiprocessing_batch_size):
                yield from batch
    ...
    def _process_model(
    ...
    ) -> dict[str, BaseModel] | dict[str, NumericVector] | BaseModel | NumericVector | models.Document:
        ...
        if isinstance(model, get_args(INFERENCE_OBJECT_TYPES)):
            if accumulating:
                ...
                assert inference_batch_size is not None, "inference_batch_size should be passed for inference"
            ...
            for item in list_model:
                ...
                if path.tail:
                    ...
                    if not accumulating:
                        assert inference_batch_size is not None, "inference_batch_size should be passed for inference"
                        embeddings = [
                            self._drain_accumulator(data, is_query=is_query, inference_batch_size=inference_batch_size)
    ...
    def _drain_accumulator(
        ...
        if isinstance(data, dict):
            for key, value in data.items():
                data[key] = self._drain_accumulator(value, is_query=is_query, inference_batch_size=inference_batch_size)
            return data
        ...
        if isinstance(data, list):
            for i, value in enumerate(data):
                ...
                data[i] = self._drain_accumulator(value, is_query=is_query, inference_batch_size=inference_batch_size)
            return data

        if not isinstance(data, get_args(INFERENCE_OBJECT_TYPES)):  # ide type checker ignores `not` and scolds
    ...
    def _embed_accumulator(self, is_query: bool = False, inference_batch_size: int = 8) -> None:
        ...
        def embed(
            ...
            for i, obj in enumerate(objects):
                ...
                for j, (options, options_is_text) in enumerate(zip(unique_options, unique_options_is_text)):
        ...
        for model, data in self._batch_accumulator.items():
            self._embed_storage[model] = embed(objects=data, model_name=model, batch_size=inference_batch_size)
