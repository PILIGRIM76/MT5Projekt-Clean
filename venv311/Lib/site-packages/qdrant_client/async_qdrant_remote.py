...
class AsyncQdrantRemote(AsyncQdrantBase):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if limits is None:
            ...
            raise ValueError(f"`pool_size` and `limits` are mutually exclusive. `pool_size`: {pool_size}, `limit`: {limits}")
        ...
        if api_key is not None:
            ...
            if any([header_name == "api-key" for (header_name, _) in self._grpc_headers]) or "api-key" in self._rest_headers:
                show_warning_once(
    ...
    @staticmethod
    def _check_compatibility(rest_uri: str, rest_headers: dict[str, Any], auth: Any, timeout: int) -> None:
        try:
            client_version = importlib.metadata.version("qdrant-client")
    ...
    def _init_grpc_points_client(self) -> None:
        ...
        self._grpc_points_client_pool = [grpc.PointsStub(channel) for channel in self._grpc_channel_pool]

    def _init_grpc_collections_client(self) -> None:
        self._init_grpc_channel()
        self._grpc_collections_client_pool = [grpc.CollectionsStub(channel) for channel in self._grpc_channel_pool]

    def _init_grpc_snapshots_client(self) -> None:
        self._init_grpc_channel()
        self._grpc_snapshots_client_pool = [grpc.SnapshotsStub(channel) for channel in self._grpc_channel_pool]

    def _init_grpc_root_client(self) -> None:
        self._init_grpc_channel()
        self._grpc_root_client_pool = [grpc.QdrantStub(channel) for channel in self._grpc_channel_pool]

    def _next_grpc_client(self) -> int:
        current_index = self._grpc_client_next_index
        self._grpc_client_next_index = (self._grpc_client_next_index + 1) % self._get_grpc_pool_size()
        ...
        **kwargs: Any,
        ...
        if self._prefer_grpc:
            ...
            if isinstance(prefetch, list):
                prefetch = [RestToGrpc.convert_prefetch_query(p) if isinstance(p, models.Prefetch) else p for p in prefetch]
    ...
    async def query_batch_points(
        ...
        **kwargs: Any,
        ...
        if self._prefer_grpc:
            requests = [
                RestToGrpc.convert_query_request(r, collection_name) if isinstance(r, models.QueryRequest) else r
                for r in requests
            ]
            if isinstance(consistency, get_args_subscribed(models.ReadConsistency)):
                consistency = RestToGrpc.convert_read_consistency(consistency)
            ...
            return [
                models.QueryResponse(points=[GrpcToRest.convert_scored_point(hit) for hit in r.result])
    ...
    async def facet(
        ...
        **kwargs: Any,
        ...
        if self._prefer_grpc:
            ...
            return types.FacetResponse(hits=[GrpcToRest.convert_facet_value_hit(hit) for hit in response.hits])
        ...
        **kwargs: Any,
        ...
        if self._prefer_grpc:
            if isinstance(points, models.Batch):
                vectors_batch: list[grpc.Vectors] = RestToGrpc.convert_batch_vector_struct(points.vectors, len(points.ids))
                points = [
                    grpc.PointStruct(
                        id=RestToGrpc.convert_extended_point_id(points.ids[idx]),
                        vectors=vectors_batch[idx],
                        payload=RestToGrpc.convert_payload(points.payloads[idx]) if points.payloads is not None else None,
                    )
                    for idx in range(len(points.ids))
                ]
            if isinstance(points, list):
                points = [
                    RestToGrpc.convert_point_struct(point) if isinstance(point, models.PointStruct) else point
                    for point in points
                ]
            ...
            if isinstance(points, list):
                points = [
                    GrpcToRest.convert_point_struct(point) if isinstance(point, grpc.PointStruct) else point
                    for point in points
                ]
    ...
    async def retrieve(
        ...
        **kwargs: Any,
        ...
        if self._prefer_grpc:
            ...
            if isinstance(with_payload, grpc.WithPayloadSelector):
                with_payload = GrpcToRest.convert_with_payload_selector(with_payload)
            ids = [GrpcToRest.convert_point_id(idx) if isinstance(idx, grpc.PointId) else idx for idx in ids]
            http_result = (
    ...
    def _try_argument_to_grpc_selector(
        ...
        if isinstance(points, list):
            ...
            points_selector = RestToGrpc.convert_points_selector(construct(models.FilterSelector, filter=points))
    ...
    def _try_argument_to_rest_selector(
        ...
        if isinstance(points, list):
            _points = [GrpcToRest.convert_point_id(idx) if isinstance(idx, grpc.PointId) else idx for idx in points]
    ...
    async def delete(
        ...
        **kwargs: Any,
        ...
        if self._prefer_grpc:
            points_selector, opt_shard_key_selector = self._try_argument_to_grpc_selector(points_selector)
            shard_key_selector = shard_key_selector or opt_shard_key_selector
            if isinstance(ordering, models.WriteOrdering):
                ordering = RestToGrpc.convert_write_ordering(ordering)
            if isinstance(shard_key_selector, get_args_subscribed(models.ShardKeySelector)):
    ...
    async def batch_update_points(
        ...
        **kwargs: Any,
        ...
        if self._prefer_grpc:
            update_operations = [RestToGrpc.convert_update_operation(operation) for operation in update_operations]
            if isinstance(ordering, models.WriteOrdering):
                ordering = RestToGrpc.convert_write_ordering(ordering)
            return [
    ...
    async def update_collection_aliases(
        ...
        **kwargs: Any,
        ...
        change_aliases_operation = [
            GrpcToRest.convert_alias_operations(operation) if isinstance(operation, grpc.AliasOperations) else operation
            for operation in change_aliases_operations
        ]
        result: bool | None = (
    ...
    async def get_collection_aliases(self, collection_name: str, **kwargs: Any) -> types.CollectionsAliasesResponse:
        if self._prefer_grpc:
            ...
            return types.CollectionsAliasesResponse(
                aliases=[GrpcToRest.convert_alias_description(description) for description in response]
            )
    ...
    async def get_aliases(self, **kwargs: Any) -> types.CollectionsAliasesResponse:
        if self._prefer_grpc:
            response = (await self.grpc_collections.ListAliases(grpc.ListAliasesRequest(), timeout=self._timeout)).aliases
            return types.CollectionsAliasesResponse(
                aliases=[GrpcToRest.convert_alias_description(description) for description in response]
            )
        result: types.CollectionsAliasesResponse | None = (await self.http.aliases_api.get_collections_aliases()).result
        assert result is not None, "Get aliases returned None"
        return result

    async def get_collections(self, **kwargs: Any) -> types.CollectionsResponse:
        if self._prefer_grpc:
            response = (await self.grpc_collections.List(grpc.ListCollectionsRequest(), timeout=self._timeout)).collections
        ...
        result: types.CollectionsResponse | None = (await self.http.collections_api.get_collections()).result
        ...
        **kwargs: Any,
        ...
        if self._prefer_grpc:
            ...
            if isinstance(quantization_config, get_args(models.QuantizationConfigDiff)):
                quantization_config = RestToGrpc.convert_quantization_config_diff(quantization_config)
            if isinstance(sparse_vectors_config, dict):
                sparse_vectors_config = RestToGrpc.convert_sparse_vector_config(sparse_vectors_config)
    ...
    async def delete_collection(self, collection_name: str, timeout: int | None = None, **kwargs: Any) -> bool:
        if self._prefer_grpc:
            return (
        ...
        result: bool | None = (await self.http.collections_api.delete_collection(collection_name, timeout=timeout)).result
    ...
    async def create_collection(
        ...
        **kwargs: Any,
        ...
        if self._prefer_grpc:
            ...
            return (await self.grpc_collections.Create(create_collection, timeout=self._timeout)).result
        if isinstance(hnsw_config, grpc.HnswConfigDiff):
            hnsw_config = GrpcToRest.convert_hnsw_config_diff(hnsw_config)
        if isinstance(optimizers_config, grpc.OptimizersConfigDiff):
            optimizers_config = GrpcToRest.convert_optimizers_config_diff(optimizers_config)
    ...
    async def delete_snapshot(self, collection_name: str, snapshot_name: str, wait: bool = True, **kwargs: Any) -> bool | None:
        if self._prefer_grpc:
            await self.grpc_snapshots.Delete(
                grpc.DeleteSnapshotRequest(collection_name=collection_name, snapshot_name=snapshot_name),
                timeout=self._timeout,
            )
            return True
        return (
    ...
    async def list_full_snapshots(self, **kwargs: Any) -> list[types.SnapshotDescription]:
        if self._prefer_grpc:
            snapshots = (
                await self.grpc_snapshots.ListFull(grpc.ListFullSnapshotsRequest(), timeout=self._timeout)
            ).snapshot_descriptions
            return [GrpcToRest.convert_snapshot_description(snapshot) for snapshot in snapshots]
    ...
    async def create_full_snapshot(self, wait: bool = True, **kwargs: Any) -> types.SnapshotDescription:
        if self._prefer_grpc:
            snapshot_description = (
                await self.grpc_snapshots.CreateFull(grpc.CreateFullSnapshotRequest(), timeout=self._timeout)
            ).snapshot_description
    ...
    async def delete_full_snapshot(self, snapshot_name: str, wait: bool = True, **kwargs: Any) -> bool | None:
        if self._prefer_grpc:
    ...
    async def info(self) -> types.VersionInfo:
        if self._prefer_grpc:
            version_info = await self.grpc_root.HealthCheck(grpc.HealthCheckRequest(), timeout=self._timeout)
    ...
    async def remove_peer(self, peer_id: int, force: bool | None = None, timeout: int | None = None, **kwargs: Any) -> bool:
        update_result = (await self.rest.distributed_api.remove_peer(peer_id=peer_id, force=force, timeout=timeout)).result
    ...
    async def collection_cluster_info(self, collection_name: str) -> types.CollectionClusterInfo:
        ...
        collection_info = (await self.rest.distributed_api.collection_cluster_info(collection_name=collection_name)).result
    ...
    async def get_optimizations(
        ...
        result = (
            await self.http.collections_api.get_optimizations(collection_name=collection_name, completed_limit=completed_limit)
        ).result
        assert result is not None, "Get optimizations returned None"
        return result

    async def list_shard_keys(self, collection_name: str, **kwargs: Any) -> types.ShardKeysResponse:
        if self._prefer_grpc:
            ...
            return models.ShardKeysResponse(
                shard_keys=[models.ShardKeyDescription(key=GrpcToRest.convert_shard_key(sk.key)) for sk in response.shard_keys]
            )
        result = (await self.rest.distributed_api.list_shard_keys(collection_name=collection_name)).result
    ...
    async def cluster_telemetry(
        ...
        result = (await self.rest.distributed_api.cluster_telemetry(details_level=details_level, timeout=timeout)).result
