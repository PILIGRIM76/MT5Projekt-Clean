...
class Inform6Lexer(RegexLexer):
    ...
    tokens = {
        ...
        "string": [
            ...
            (
                rf"@(\\\s*[{_newline}]\s*)*[({{]((\\\s*[{_newline}]\s*)*[0-9a-zA-Z_])*" rf"(\\\s*[{_newline}]\s*)*[)}}]",
                String.Escape,
            ),
        ...
        "property-keyword*": [
            include("_whitespace"),
            (
                words(("additive", "individual", "long"), suffix=rf"\b(?=(\s*|(![^{_newline}]*[{_newline}]))*[_a-zA-Z])"),
                Keyword,
            ),
            default("#pop"),
        ],
        ...
        "miscellaneous-keyword?": [
            ...
            (
                r"(a|A|an|address|char|name|number|object|property|string|the|" rf"The)\b(?=(\s+|(![^{_newline}]*))*\))",
                Keyword.Pseudo,
                "#pop",
            ),
...
class Tads3Lexer(RegexLexer):
    ...
    def _make_string_state(triple, double, verbatim=None, _escape=_escape):
        ...
        if verbatim:
            ...
            state.append(
                (rf"\\?<(/|\\\\|(?!{_escape})\\){verbatim}(?=[\s=>])", Name.Tag, ("#pop", f"{prefix}qs", tag_state_name))
            )
        else:
            state += [
