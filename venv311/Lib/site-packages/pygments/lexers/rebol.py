...
class RedLexer(RegexLexer):
    ...
    def word_callback(lexer, match):
        ...
        if re.match(".*:$", word):
            ...
            r"(yes|on|no|off|true|false|tab|cr|lf|newline|escape|slash|sp|space|null|" r"none|crlf|dot|null-byte)$", word
        ):
            yield match.start(), Name.Builtin.Pseudo, word
        elif re.match(
            r"(#system-global|#include|#enum|#define|#either|#if|#import|#export|" r"#switch|#default|#get-definition)$", word
        ):
            yield match.start(), Keyword.Namespace, word
        elif re.match(
