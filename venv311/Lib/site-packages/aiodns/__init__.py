...
WINDOWS_SELECTOR_ERR_MSG = (
    "aiodns needs a SelectorEventLoop on Windows. See more: " "https://github.com/aio-libs/aiodns#note-for-windows-users"
)
...
class DNSResolver:
    ...
    def _make_channel(self, **kwargs: Any) -> tuple[bool, pycares.Channel]:
        ...
        def sock_state_cb_wrapper(fd: int, readable: bool, writable: bool) -> None:
            this = weak_self()
            if this is not None:
    ...
    def _callback(self, fut: asyncio.Future[_T], result: _T, errorno: int | None) -> None:
        if fut.cancelled():
            return
    ...
    def _get_query_future_callback(self, qtype: int) -> tuple[asyncio.Future[QueryResult], Callable[..., None]]:
    ...
    @overload
    def query(self, host: str, qtype: Literal["A"], qclass: str | None = ...) -> asyncio.Future[list[AresQueryAResult]]: ...
    @overload
    def query(
    ...
    @overload
    def query(self, host: str, qtype: Literal["CNAME"], qclass: str | None = ...) -> asyncio.Future[AresQueryCNAMEResult]: ...
    @overload
    def query(self, host: str, qtype: Literal["MX"], qclass: str | None = ...) -> asyncio.Future[list[AresQueryMXResult]]: ...
    @overload
    def query(
        self, host: str, qtype: Literal["NAPTR"], qclass: str | None = ...
    ) -> asyncio.Future[list[AresQueryNAPTRResult]]: ...
    @overload
    def query(self, host: str, qtype: Literal["NS"], qclass: str | None = ...) -> asyncio.Future[list[AresQueryNSResult]]: ...
    @overload
    def query(self, host: str, qtype: Literal["PTR"], qclass: str | None = ...) -> asyncio.Future[AresQueryPTRResult]: ...
    @overload
    def query(self, host: str, qtype: Literal["SOA"], qclass: str | None = ...) -> asyncio.Future[AresQuerySOAResult]: ...
    @overload
    def query(
    ...
    def query(self, host: str, qtype: str, qclass: str | None = None) -> asyncio.Future[list[Any]] | asyncio.Future[Any]:
        ...
        if qclass_int is not None:
            self._channel.query(host, qtype_int, query_class=qclass_int, callback=cb)
        else:
            self._channel.query(host, qtype_int, callback=cb)
        return fut

    def query_dns(self, host: str, qtype: str, qclass: str | None = None) -> asyncio.Future[pycares.DNSResult]:
    ...
    def _gethostbyname_callback(
        ...
        if errorno is not None:
            ...
            fut.set_result(AresHostResult(name=name, aliases=[], addresses=addresses))

    def gethostbyname(self, host: str, family: socket.AddressFamily) -> asyncio.Future[AresHostResult]:
        """
    ...
    def _sock_state_cb(self, fd: int, readable: bool, writable: bool) -> None:
        if readable or writable:
            if readable:
                self.loop.add_reader(fd, self._channel.process_fd, fd, pycares.ARES_SOCKET_BAD)
                self._read_fds.add(fd)
            if writable:
                self.loop.add_writer(fd, self._channel.process_fd, pycares.ARES_SOCKET_BAD, fd)
