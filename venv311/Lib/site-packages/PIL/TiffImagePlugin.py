...
class ImageFileDirectory_v2(_IFDv2Base):
    ...
    def load(self, fp: IO[bytes]) -> None:
        ...
        try:
            tag_count = (
                self._unpack("Q", self._ensure_read(fp, 8)) if self._bigtiff else self._unpack("H", self._ensure_read(fp, 2))
            )[0]
            ...
            (self.next,) = (
                self._unpack("Q", self._ensure_read(fp, 8)) if self._bigtiff else self._unpack("L", self._ensure_read(fp, 4))
            )
