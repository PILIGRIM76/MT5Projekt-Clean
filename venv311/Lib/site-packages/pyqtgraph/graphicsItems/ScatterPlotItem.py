...
coords = {
    ...
    "arrow_up": [(-0.125, 0.125), (0, 0), (0.125, 0.125), (0.05, 0.125), (0.05, 0.5), (-0.05, 0.5), (-0.05, 0.125)],
}
    ...
    def setPointsVisible(self, visible, update=True, dataSet=None, mask=None):
        ...
        if isinstance(visible, np.ndarray) or isinstance(visible, list):
            ...
            if len(visibilities) != len(dataSet):
                raise Exception(
                    "Number of visibilities does not match number of points (%d != %d)" % (len(visibilities), len(dataSet))
                )
    ...
    def paint(self, p, option, widget):
        ...
        if self.opts["pxMode"] is True:
            ...
            if self.opts["useCache"] and self._exportOpts is False:
                ...
                for pt, style in zip(
                    pts[:, viewMask].T, zip(*(self._style(["symbol", "size", "pen", "brush"], idx=viewMask, scale=scale)))
                ):
                    p.resetTransform()
                    p.translate(*pt)
                    drawSymbol(p, *style)
            ...
            if self.picture is None:
                ...
                for x, y, style in zip(
                    self.data["x"], self.data["y"], zip(*self._style(["symbol", "size", "pen", "brush"], scale=scale))
                ):
                    p2.resetTransform()
                    p2.translate(x, y)
                    drawSymbol(p2, *style)
