    ...
    **Bases:** :class:`GraphicsWidget <pyqtgraph.GraphicsWidget>`
    ...
    def childrenBounds(self, frac=None, orthoRange=(None, None), items=None):
        ...
        for item in items:
            ...
            if hasattr(item, "dataBounds") and item.dataBounds is not None:
                ...
                if xr is None or (xr[0] is None and xr[1] is None) or not math.isfinite(xr[0]) or not math.isfinite(xr[1]):
                    useX = False
                    xr = (0, 0)
                if yr is None or (yr[0] is None and yr[1] is None) or not math.isfinite(yr[0]) or not math.isfinite(yr[1]):
                    useY = False
                    yr = (0, 0)
        ...
        if aspect is not False and 0 not in [aspect, tr.height(), bounds.height(), bounds.width()]:
            ...
            if forceX:
                ...
                if maxRng is not None or minRng is not None:
                    ...
                    if (
                        maxRng[target] is not None
                        and diff > maxRng[target]
                        or minRng[target] is not None
                        and diff < minRng[target]
                    ):
    ...
    def quit():
        ...
        for k in ViewBox.AllViews:
            ...
            if QT_LIB == "PySide6" and isQObjectAlive(k) and k.receivers(QtCore.SIGNAL("destroyed()")) == 0:
                continue
