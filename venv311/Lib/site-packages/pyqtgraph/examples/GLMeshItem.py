...
colors = np.array([[1, 0, 0, 0.3], [0, 1, 0, 0.3], [0, 0, 1, 0.3], [1, 1, 0, 0.3]])
...
m2 = gl.GLMeshItem(vertexes=verts, vertexColors=colors, smooth=False, shader="balloon", drawEdges=True, edgeColor=(1, 1, 0, 1))
m2.translate(-5, 5, 0)
w.addItem(m2)
...
# Example 5:
# cylinder
md = gl.MeshData.cylinder(rows=10, cols=20, radius=[1.0, 2.0], length=5.0)
md2 = gl.MeshData.cylinder(rows=10, cols=20, radius=[2.0, 0.5], length=10.0)
colors = np.ones((md.faceCount(), 4), dtype=float)
colors[::2, 0] = 0
colors[:, 1] = np.linspace(0, 1, colors.shape[0])
md.setFaceColors(colors)
