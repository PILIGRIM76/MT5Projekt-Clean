...
def _getFromFile(name):
    ...
    with open(filename, "r") as fh:
        ...
        for line in fh:
            ...
            if csv_mode:
                comp = parts[0].split(",")
                if len(comp) < 3:
                    continue  # not enough components given
    ...
    # end of open
    cmap = ColorMap(name=name, pos=np.linspace(0.0, 1.0, len(color_list)), color=color_list)  # , names=color_names)
    if cmap is not None:
        cmap.name = name
        _mapCache[name] = cmap
    return cmap
...
class ColorMap(object):
    ...
    def getSubset(self, start, span):
        ...
        cmap = ColorMap(pos=new_pos, color=255.0 * new_col)
    ...
    def getBrush(self, span=(0.0, 1.0), orientation="vertical"):
        """
        ...
        if orientation == "vertical":
            grad = self.getGradient(p1=QtCore.QPointF(0.0, span[0]), p2=QtCore.QPointF(0.0, span[1]))
        elif orientation == "horizontal":
            grad = self.getGradient(p1=QtCore.QPointF(span[0], 0.0), p2=QtCore.QPointF(span[1], 0.0))
        else:
            raise ValueError("Orientation must be 'vertical' or 'horizontal'")
        return QtGui.QBrush(grad)

    def getPen(self, span=(0.0, 1.0), orientation="vertical", width=1.0):
        """
