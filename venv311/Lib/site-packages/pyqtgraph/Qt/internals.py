...
class PrimitiveArray:
    ...
    def __init__(self, Klass, nfields, *, use_array=None):
        ...
        if QT_LIB.startswith("PyQt"):
            if use_array is None:
                use_array = hasattr(sip, "array") and (
                    (0x60301 <= QtCore.PYQT_VERSION) or (0x50F07 <= QtCore.PYQT_VERSION < 0x60000)
                )
    ...
    def ndarray(self):
        ...
        if self.use_sip_array:
            if (
                sip.SIP_VERSION >= 0x60708 and np.__version__ != "1.22.4"  # TODO: remove me after numpy 1.23+
