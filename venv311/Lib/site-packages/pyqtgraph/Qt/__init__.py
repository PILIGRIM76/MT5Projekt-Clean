    ...
    if QT_LIB == PYSIDE2:
        ...
        if pyside2uic is None:
            ...
            if (5, 14) <= pyside2version < (5, 14, 2, 2):
                warnings.warn(
                    "For UI compilation, it is recommended to upgrade to PySide >= 5.15", RuntimeWarning, stacklevel=2
                )
...
if m is not None and list(map(int, m.groups())) < versionReq:
    warnings.warn(
        f"PyQtGraph supports Qt version >= {versionReq[0]}.{versionReq[1]}," f" but {QtVersion} detected.",
        RuntimeWarning,
        stacklevel=2,
    )
    ...
    **Arguments:**
    ...
    try:
        ...
        with contextlib.suppress(TypeError):
            ...
            QAPP.styleHints().colorSchemeChanged.connect(_onColorSchemeChange, type=QtCore.Qt.ConnectionType.UniqueConnection)
        ...
        with contextlib.suppress(TypeError):
            # some qt bindings raise a TypeError when using a UniqueConnection
            # to an already connected signal/slot
            QAPP.paletteChanged.connect(_onPaletteChange, type=QtCore.Qt.ConnectionType.UniqueConnection)
