...
def _convert_lut_to_rgba(xp, lut):
    ...
    if not (lut is None or lut.ndim == 1 or (lut.ndim == 2 and lut.shape[1] in (1, 3, 4))):
...
def _rescale_and_lookup_float(xp, image, levels, lut, *, forceApplyLut):
    ...
    if xp == numpy and (fn_numba := getNumbaFunctions()) is not None:
        ...
        image = functions.rescaleData(image, scale, offset, dtype=dtype, clip=(0, num_colors - 1))
        if apply_lut:
...
def try_make_qimage(image, *, levels, lut, transparentLocations=None):
    ...
    alpha_channel_required = (  # image itself has alpha channel
        image.ndim == 3 and image.shape[2] == 4
    ) or (  # lut has alpha channel
        lut is not None and lut.ndim == 2 and lut.shape[1] == 4
    )
    ...
    if image.dtype.kind == "f":
        if image.ndim == 2:
            ...
            if transparentLocations is None:
                image, lut = _rescale_and_lookup_float(xp, image, levels, lut, forceApplyLut=False)
                levels = None
                ...
                image, lut = _rescale_and_lookup_float(xp, image, levels, lut, forceApplyLut=True)
                levels = None
                assert lut is None
