...
def _register_kernel_internal(functional, input_type, *, tv_tensor_wrapper=True):
    ...
    def decorator(kernel):
        registry[input_type] = (
            _kernel_tv_tensor_wrapper(kernel) if issubclass(input_type, tv_tensors.TVTensor) and tv_tensor_wrapper else kernel
        )
        return kernel
...
def register_kernel(functional, tv_tensor_cls):
    ...
    if isinstance(functional, str):
        ...
        callable(functional) and getattr(functional, "__module__", "").startswith("torchvision.transforms.v2.functional")
    ):
        raise ValueError(
    ...
    if not (isinstance(tv_tensor_cls, type) and issubclass(tv_tensor_cls, tv_tensors.TVTensor)):
        raise ValueError(
            f"Kernels can only be registered for subclasses of torchvision.tv_tensors.TVTensor, " f"but got {tv_tensor_cls}."
        )
...
def _get_kernel(functional, input_type, *, allow_passthrough=False):
    ...
    raise TypeError(
        f"Functional F.{functional.__name__} supports inputs of type {registry.keys()}, " f"but got {input_type} instead."
    )
