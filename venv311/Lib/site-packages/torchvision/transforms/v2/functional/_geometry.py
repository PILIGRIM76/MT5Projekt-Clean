...
def _apply_grid_transform(img: torch.Tensor, grid: torch.Tensor, mode: str, fill: _FillTypeJIT) -> torch.Tensor:
    ...
    if fill is not None:
        mask = torch.ones((squashed_batch_size, 1, input_height, input_width), dtype=float_img.dtype, device=float_img.device)
...
def _rotate_keypoints_dispatch(
    ...
    output, canvas_size = rotate_keypoints(inpt, canvas_size=inpt.canvas_size, angle=angle, center=center, expand=expand)
    return tv_tensors.wrap(output, like=inpt, canvas_size=canvas_size)
...
def pad_keypoints(keypoints: torch.Tensor, canvas_size: tuple[int, int], padding: list[int], padding_mode: str = "constant"):
...
def _crop_keypoints_dispatch(inpt: tv_tensors.KeyPoints, top: int, left: int, height: int, width: int) -> tv_tensors.KeyPoints:
...
def perspective_bounding_boxes(
    ...
    bounding_boxes = (convert_bounding_box_format(bounding_boxes, old_format=format, new_format=intermediate_format)).reshape(
        -1, 8 if is_rotated else 4
    )
...
def elastic_image(
    ...
    grid = _create_identity_grid((height, width), device=device, dtype=dtype).add_(displacement.to(dtype=dtype, device=device))
...
def elastic_bounding_boxes(
    ...
    return convert_bounding_box_format(out_bboxes, old_format=intermediate_format, new_format=format, inplace=False).reshape(
        original_shape
    )
