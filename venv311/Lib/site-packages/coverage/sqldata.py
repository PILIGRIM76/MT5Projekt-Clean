        ...
        with self._dbs[threading.get_ident()] as db:
            try:
                ...
                if "no such table: coverage_schema" in str(exc):
                    ...
                    raise DataError(f"Data file {self._filename!r} isn't a coverage data file: {exc}") from exc
            else:
            ...
            for filename, plugin_name in file_tracers.items():
                ...
                if existing_plugin:
                    if existing_plugin != plugin_name:
                        raise DataError(
                            f"Conflicting file tracer name for {filename!r}: " + f"{existing_plugin!r} vs {plugin_name!r}"
                        )
                elif plugin_name:
                    con.execute_void(
        ...
        with self._connect() as con:
            ...
            """) as cur:
                ...
                if conflicts:
                    ...
                    raise DataError(f"Conflicting file tracer name for {path!r}: " + f"{this_tracer!r} vs {other_tracer!r}")
