...
class InOrOut:
    ...
    def __init__(
        ...
        if self.source_dirs or self.source_pkgs:
            if self.source_dirs:
                self.source_match = TreeMatcher(self.source_dirs, "source", "Source directory", self._debug)
            if self.source_pkgs:
                self.source_pkgs_match = ModuleMatcher(self.source_pkgs, "source_pkgs", "Source imports", self._debug)
        else:
            if self.pylib_paths:
                self.pylib_match = TreeMatcher(self.pylib_paths, "pylib", "Python stdlib", self._debug)
        if self.include:
        ...
        self.coverage_match = TreeMatcher(self.cover_paths, "coverage", "Coverage code", self._debug)
