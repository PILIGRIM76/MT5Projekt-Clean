...
class token(str):
    ...
    def __repr__(self):
        return "token(%s, %r, %r, %r)" % (str.__repr__(self), self.pre_tags, self.post_tags, self.trailing_whitespace)
...
class tag_token(token):
    ...
    def __new__(cls, tag, data, html_repr, pre_tags=None, post_tags=None, trailing_whitespace=""):
        obj = token.__new__(
            cls, f"{type}: {data}", pre_tags=pre_tags, post_tags=post_tags, trailing_whitespace=trailing_whitespace
        )
        obj.tag = tag
        obj.data = data
