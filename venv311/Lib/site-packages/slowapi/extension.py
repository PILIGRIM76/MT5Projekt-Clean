...
class Limiter:
    ...
    * **key_style**: set to "url" to use the url, "endpoint" to use the view_func
    ...
    def __init__(
        ...
        self.app_config = Config(".env" if dotenv_file_exists and config_filename is None else config_filename)
        ...
        for limit in set(default_limits):
            self._default_limits.extend([LimitGroup(limit, self._key_func, None, False, None, None, None, 1, False)])
        ...
        for limit in in_memory_fallback:
            self._in_memory_fallback.extend([LimitGroup(limit, self._key_func, None, False, None, None, None, 1, False)])
        ...
        self._swallow_errors = self.get_app_config(C.SWALLOW_ERRORS, self._swallow_errors)
        self._headers_enabled = self._headers_enabled or self.get_app_config(C.HEADERS_ENABLED, False)
        ...
        self._retry_after = self._retry_after or self.get_app_config(C.HEADER_RETRY_AFTER_VALUE)
        self._key_prefix = self._key_prefix or self.get_app_config(C.KEY_PREFIX)
        app_limits: Optional[StrOrCallableStr] = self.get_app_config(C.APPLICATION_LIMITS, None)
        ...
        conf_limits: Optional[StrOrCallableStr] = self.get_app_config(C.DEFAULT_LIMITS, None)
        if not self._default_limits and conf_limits:
            self._default_limits = [LimitGroup(conf_limits, self._key_func, None, False, None, None, None, 1, False)]
        fallback_enabled = self.get_app_config(C.IN_MEMORY_FALLBACK_ENABLED, False)
        fallback_limits: Optional[StrOrCallableStr] = self.get_app_config(C.IN_MEMORY_FALLBACK, None)
    ...
    def limiter(self) -> RateLimiter:
        ...
        if self._storage_dead and self._in_memory_fallback_enabled:
            assert self._fallback_limiter, "Fallback limiter is needed when in memory fallback is enabled"
            return self._fallback_limiter
        else:
            return self._limiter

    def _inject_headers(self, response: Response, current_limit: Tuple[RateLimitItem, List[str]]) -> Response:
    ...
    def _inject_asgi_headers(self, headers: MutableHeaders, current_limit: Tuple[RateLimitItem, List[str]]) -> MutableHeaders:
        """
        ...
        if self.enabled and self._headers_enabled and current_limit is not None:
            try:
                window_stats: Tuple[int, int] = self.limiter.get_window_stats(current_limit[0], *current_limit[1])
                reset_in = 1 + window_stats[0]
                headers[self._header_mapping[HEADERS.LIMIT]] = str(current_limit[0].amount)
    ...
    def _check_request_limit(
        ...
        endpoint_func_name = f"{view_func.__module__}.{view_func.__name__}" if view_func else ""
        ...
        if not in_middleware:
            limits = self._route_limits[endpoint_func_name] if endpoint_func_name in self._route_limits else []
            dynamic_limits = []
                ...
                async def async_wrapper(*args: Any, **kwargs: Any) -> Response:
                    ...
                    if self.enabled:
                        if not isinstance(response, Response):
                            ...
                            self._inject_headers(kwargs.get("response"), request.state.view_rate_limit)  # type: ignore
                        else:
