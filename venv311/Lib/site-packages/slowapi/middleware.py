...
class _ASGIMiddlewareResponder:
    ...
    async def send_wrapper(self, message: Message) -> None:
        if message["type"] == "http.response.start":
            ...
            if self.inject_headers:
                ...
                headers = self.limiter._inject_asgi_headers(headers, self.request.state.view_rate_limit)
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        error_response, should_inject_headers = await async_check_limits(limiter, request, handler, _app)
        if error_response is not None:
