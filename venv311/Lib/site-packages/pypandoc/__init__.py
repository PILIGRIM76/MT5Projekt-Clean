...
def _validate_formats(format, to, outputfile):
    ...
    if (
        ...
        raise RuntimeError(
            "Invalid output format! Got {} but expected one of these: {}".format(base_to_format, ", ".join(to_formats))
        )
...
def _convert_input(
    ...
    if p.returncode != 0:
        raise RuntimeError(f'Pandoc died with exitcode "{p.returncode}" during conversion: {stderr}')
...
def _get_pandoc_version(pandoc_path: str) -> str:
    ...
    if p.returncode != 0 or len(out_lines) == 0:
        raise RuntimeError("Couldn't call pandoc to get version information. Output from " "pandoc:\n%s" % str(comm))
        ...
        for path in search_paths:
            ...
            try:
                ...
                if os.path.exists(path):
                    ...
                    log_msg = "Found {}, but not using it because of an " "error:".format(path)
