...
def write(
    file, data, samplerate, subtype=None, endian=None, format=None, closefd=True, compression_level=None, bitrate_mode=None
):
    ...
    with SoundFile(file, "w", samplerate, channels, subtype, endian, format, closefd, compression_level, bitrate_mode) as f:
        f.write(data)
...
class SoundFile(object):
    ...
    def _array_io(self, action, array, frames):
        ...
        if array_channels != self.channels:
            raise ValueError(
                "Invalid shape: {0!r} (Expected {1} channels, got {2})".format(array.shape, self.channels, array_channels)
            )
    ...
    def _set_compression_level(self, compression_level):
        ...
        err = _snd.sf_command(
            self._file, _snd.SFC_SET_COMPRESSION_LEVEL, pointer_compression_level, _ffi.sizeof(pointer_compression_level)
        )
        if err != _snd.SF_TRUE:
            err = _snd.sf_error(self._file)
