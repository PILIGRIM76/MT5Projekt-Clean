...
def ast_names(code):
    ...
    for node in ast.walk(ast.parse(code)):
        if isinstance(node, disallowed_ast_nodes):
            raise PatsyError(
                "Lambda, list/dict/set comprehension, generator " "expression in patsy formula not currently supported."
            )
...
def test_EvalEnvironment_eq():
    ...
    if sys.version_info >= (3, 13):
        pytest.skip("`frame.f_locals` may return write-through proxies in Python 3.13+, " "breaking direct comparison by ids.")
...
class EvalFactor(object):
    ...
    def memorize_passes_needed(self, state, eval_env):
        ...
        for obj_name in state["transforms"]:
            ...
            memorize_code = obj_name + ".memorize_chunk" + transform_call_code[len(transform_call_name) :]
...
def test_annotated_tokens():
    tokens_without_origins = [
        (token_type, token, props) for (token_type, token, origin, props) in (annotated_tokens("a(b) + c.d"))
    ]
