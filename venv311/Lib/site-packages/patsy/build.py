...
from patsy.contrasts import (
    Treatment,
    code_contrast_matrix,
)
from patsy.design_info import (
    DesignInfo,
    DesignMatrix,
    FactorInfo,
    SubtermInfo,
...
def _max_allowed_dim(dim, arr, factor):
    if arr.ndim > dim:
        msg = "factor '%s' evaluates to an %s-dimensional array; I only " "handle arrays with dimension <= %s" % (
            factor.name(),
            arr.ndim,
            dim,
        )
...
def _eval_factor(factor_info, data, NA_action):
    ...
    if factor_info.type == "numerical":
        ...
        if result.shape[1] != factor_info.num_columns:
            raise PatsyError(
                ...
                "instead of the %s I was expecting" % (factor.name(), factor_info.num_columns, result.shape[1]),
                factor,
            )
        if not safe_issubdtype(np.asarray(result).dtype, np.number):
            raise PatsyError(
                "when evaluating numeric factor %s, " "I got non-numeric data of type '%s'" % (factor.name(), result.dtype),
                factor,
            )
    ...
    # returns either a 1d ndarray or a pandas.Series, plus is_NA mask
        ...
        result = categorical_to_int(result, factor_info.categories, NA_action, origin=factor_info.factor)
...
def test__factors_memorize():
    ...
    expected = {
        ...
        f2a: {
            "calls": mem_chunks0 + [("memorize_finish", 0)] + mem_chunks1 + [("memorize_finish", 1)],
            "token": "f2a",
        },
        f2b: {
            "calls": mem_chunks0 + [("memorize_finish", 0)] + mem_chunks1 + [("memorize_finish", 1)],
