...
def _get_all_sorted_knots(x, n_inner_knots=None, inner_knots=None, lower_bound=None, upper_bound=None):
    ...
    if all_knots.size != n_inner_knots + 2:
        raise ValueError(
            ...
            "lower_bound(=%r) and upper_bound(=%r)." % (n_inner_knots, x.size, lower_bound, upper_bound)
        )
    ...
    def memorize_finish(self):
        ...
        if constraints is not None:
            ...
            if constraints.shape[1] != df_before_constraints:
                raise ValueError(
                    "Constraints array should have %r columns but" " %r found." % (df_before_constraints, constraints.shape[1])
                )
...
def test_te_3smooths():
    ...
    builder = incr_dbuilder("te(cr(x1, df=3), cr(x2, df=3), cc(x3, df=3)) - 1", lambda: iter(data_chunked))
