...
class DesignInfo(object):
    ...
    def __init__(self, column_names, factor_infos=None, term_codings=None):
        ...
        if term_codings is None:
            ...
            self.term_name_slices = OrderedDict([(term.name(), slice_) for (term, slice_) in self.term_slices.items()])
...
class DesignMatrix(np.ndarray):
    ...
    def _repr_pretty_(self, p, cycle):
        ...
        if min_total_width <= MAX_TOTAL_WIDTH:
            ...
            column_widths = [
                max(name_width, num_width) for (name_width, num_width) in zip(column_name_widths, column_num_widths)
            ]
