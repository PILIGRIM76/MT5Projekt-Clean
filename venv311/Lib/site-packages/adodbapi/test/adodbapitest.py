...
class CommonDBTests(unittest.TestCase):
    ...
    def testAutomaticParamstyle(self):
        ...
        for inParam in inputs:
            ...
            try:
                crsr.execute(
                    "INSERT INTO xx_" + config.tmp + " (fldId,fldConst,fldData) VALUES (?,'thi%s :may cause? troub:1e', ?)",
                    (fldId, inParam),
                )
            except:
                conn.printADOerrors()
    ...
    def testAutoRollback(self):
        ...
        assert row is None, f"cursor.fetchone should return None if a query retrieves no rows. Got {row!r}"
        self.helpRollbackTblTemp()
