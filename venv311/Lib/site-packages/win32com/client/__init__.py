...
def CastTo(ob, target, typelib=None):
    ...
    if typelib is not None:  # caller specified target typelib (TypelibSpec). See e.g. selecttlb.EnumTlbs().
        ...
        if not hasattr(mod, target):
            raise ValueError(
                f"The interface name '{target}' does not appear in the " f"specified library {typelib.ver_desc!r}"
            )
        ...
        if target_clsid is None:
            raise ValueError(f"The interface name '{target}' does not appear in the " f"same library as object '{ob!r}'")
