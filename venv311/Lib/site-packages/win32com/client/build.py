...
class VTableItem(DispatchItem):
    def Build(self, typeinfo, attr, bForUser=1):
        ...
        self.vtableFuncs = [(entry.names, entry.dispid, entry.desc) for entry in meth_list]
...
def MakeDefaultArgRepr(defArgVal):
    ...
    if inOut & pythoncom.PARAMFLAG_FHASDEFAULT:
        ...
        if isinstance(val, TimeType):
            ...
            msec = val.msec
            return (
                "pywintypes.Time((%(year)d, %(month)d, %(day)d, %(hour)d, %(minute)d, %(second)d,0,0,0,%(msec)d))" % locals()
            )
        return repr(val)
    return None
