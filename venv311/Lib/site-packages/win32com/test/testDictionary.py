...
def TestDictAgainst(dict, check):
    for key, value in check.items():
        assert dict(key) == value, f"Indexing for '{key!r}' gave the incorrect value - {dict[key]!r}/{check[key]!r}"
...
def Register(quiet):
    ...
    win32com.test.util.RegisterPythonServer(win32com.servers.dictionary.__file__, "Python.Dictionary")
...
def TestDict(quiet=None):
    ...
    try:
        ...
        assert hr == winerror.DISP_E_BADPARAMCOUNT, f"Expected DISP_E_BADPARAMCOUNT - got {hr} ({desc})"
    ...
    try:
        ...
        assert hr == winerror.DISP_E_TYPEMISMATCH, f"Expected DISP_E_TYPEMISMATCH - got {hr} ({desc})"
