...
class AXScript(win32com.test.util.TestCase):
    def setUp(self):
        file = win32api.GetFullPathName(os.path.join(next(iter(win32com.axscript.client.__path__)), "pyscript.py"))

        self.verbose = verbose
        win32com.test.util.RegisterPythonServer(file, "python", verbose=self.verbose)

    def testHost(self):
        file = win32api.GetFullPathName(os.path.join(next(iter(win32com.axscript.__path__)), "test\\testHost.py"))
    ...
    def testCScript(self):
        file = win32api.GetFullPathName(os.path.join(next(iter(win32com.axscript.__path__)), "Demos\\Client\\wsh\\test.pys"))
