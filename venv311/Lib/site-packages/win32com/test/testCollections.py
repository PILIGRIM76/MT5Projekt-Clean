...
def TestEnumAgainst(o, check):
    for i in range(len(check)):
        assert o(i) == check[i], f"Using default method gave the incorrect value - {o(i)!r}/{check[i]!r}"

    for i in range(len(check)):
        assert o.Item(i) == check[i], f"Using Item method gave the incorrect value - {o(i)!r}/{check[i]!r}"
    ...
    assert cmp[: len(check)] == check, f"Result after looping isn't correct - {cmp[: len(check)]!r}/{check!r}"
...
def TestEnum(quiet=None):
    ...
    try:
        o()
        raise AssertionError("default method with no args worked when it shouldn't have!")
    except pythoncom.com_error as exc:
        assert exc.hresult == winerror.DISP_E_BADPARAMCOUNT, f"Expected DISP_E_BADPARAMCOUNT - got {exc}"

    try:
        o.Insert("foo", 2)
        raise AssertionError("Insert worked when it shouldn't have!")
    except pythoncom.com_error as exc:
        assert exc.hresult == winerror.DISP_E_TYPEMISMATCH, f"Expected DISP_E_TYPEMISMATCH - got {exc}"
    ...
    try:
        ...
        assert exc.hresult == winerror.DISP_E_BADINDEX, f"Expected DISP_E_BADINDEX - got {exc}"
