...
def CreateTestAccessDatabase(dbname=None):
    ...
    tab1.MoveLast()
    assert tab1.Fields("First Name").Value == "Second", "Unexpected record is last - makes bookmark test pointless!"

    tab1.Bookmark = bk
    assert tab1.Bookmark == bk, "The bookmark data is not the same"
    assert tab1.Fields("First Name").Value == "Mark", "The bookmark did not reset the record pointer correctly"
