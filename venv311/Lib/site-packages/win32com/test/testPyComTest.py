...
def check_get_set_raises(exc, func, arg):
    try:
        ...
        raise AssertionError(f"{func} with arg {arg!r} didn't raise {exc} - returned {got!r}")
...
def TestConstant(constName, pyConst):
    ...
    assert comConst == pyConst, f"Constant value wrong for {constName} - got {comConst}, wanted {pyConst}"
...
def TestCommon(o, is_generated):
    ...
    assert isinstance(o.GetSetDispatch(o), expected_class), f"GetSetDispatch failed: {o.GetSetDispatch(o)!r}"
    ...
    o.LongProp = 3
    assert o.LongProp == o.IntProp == 3, f"Property value wrong - got {o.LongProp}/{o.IntProp}"
    ...
    check = 3 * 10**9
    o.ULongProp = check
    assert o.ULongProp == check, f"Property value wrong - got {o.ULongProp} (expected {check})"
...
def TestCounter(counter, bIsGenerated):
    ...
    for i in range(50):
        ...
        try:
            ...
            if bIsGenerated:
                ...
                ret = counter[num]
            assert ret == num + 1, f"Random access into element {num} failed - return was {ret!r}"
        except IndexError:
    ...
    if bIsGenerated:
        ...
        assert bounds[0] == 1 and bounds[1] == 10, "** Error - counter did not give the same properties back"
        counter.SetBounds(bounds[0], bounds[1])

    for item in counter:
        num += 1
    assert num == len(counter), "*** Length of counter and loop iterations don't match ***"
