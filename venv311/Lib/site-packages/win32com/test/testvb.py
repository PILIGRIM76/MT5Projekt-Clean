...
def TestVB(vbtest, bUseGenerated):
    ...
    assert vbtest.VariantProperty == 10, "Could not set the variant integer property correctly."
    vbtest.VariantProperty = memoryview(b"raw\0data")
    assert vbtest.VariantProperty == memoryview(b"raw\0data"), "Could not set the variant buffer property correctly."
    vbtest.StringProperty = "Hello from Python"
    assert vbtest.StringProperty == "Hello from Python", "Could not set the string property correctly."
    vbtest.VariantProperty = "Hello from Python"
    assert vbtest.VariantProperty == "Hello from Python", "Could not set the variant string property correctly."
    ...
    if bUseGenerated:
        ...
        assert vbtest.VariantPutref._oleobj_ == vbtest._oleobj_, "Could not set the VariantPutref property correctly."
        ...
        assert vbtest.IncrementVariantParam(1) == 2, f"Could not pass an int VARIANT byref: {vbtest.IncrementVariantParam(1)}"
        assert vbtest.IncrementVariantParam(1.5) == 2.5, "Could not pass a float VARIANT byref"
...
def _DoTestCollection(vbtest, col_name, expected):
    ...
    for item in c:
        check.append(item)
    assert check == list(expected), f"Collection {col_name} didn't have {expected!r} (had {check!r})"
    # Just looping over the collection again works (ie, is restartable)
    check = []
    for item in c:
        check.append(item)
    assert check == list(expected), f"Collection 2nd time around {col_name} didn't have {expected!r} (had {check!r})"
    ...
    for item in i:
        check.append(item)
    assert check == list(expected), f"Collection iterator {col_name} didn't have {expected!r} 2nd time around (had {check!r})"
    # but an iterator is not restartable
    check = []
    for item in i:
        check.append(item)
    assert check == [], "2nd time around Collection iterator {col_name} wasn't empty (had {check!r})"
    # Check len()==Count()
    c = getattr(vbtest, col_name)
    assert len(c) == _getcount(c), f"Collection {col_name} __len__({len(c)!r}) wasn't==Count({_getcount(c)!r})"
...
def TestCollections(vbtest):
    ...
    assert vbtest.CollectionProperty[0] == 1, "The CollectionProperty[0] element was not the default value"
...
def _DoTestArray(vbtest, data, expected_exception=None):
    ...
    assert got == data, f"Could not set the array data correctly - got {got!r}, expected {data!r}"
...
def TestArrays(vbtest, bUseGenerated):
    ...
    try:
        ...
        assert exc.excepinfo[1] == "Python COM Server Internal Error", f"Didn't get the correct exception - '{exc}'"
    ...
    if bUseGenerated:
        ...
        assert testData == list(resultData), f"The safe array data was not what we expected - got {resultData}"
        assert testData == list(byRefParam), f"The safe array data was not what we expected - got {byRefParam}"
...
def TestStructs(vbtest):
    try:
        ...
        assert exc.hresult == winerror.DISP_E_TYPEMISMATCH, "Expected DISP_E_TYPEMISMATCH"

    s = vbtest.StructProperty
    assert s.int_val == 99 and str(s.str_val) == "hello", "The struct value was not correct"
    s.str_val = "Hi from Python"
    s.int_val = 11
    assert s.int_val == 11 and str(s.str_val) == "Hi from Python", "The struct value didn't persist!"
    assert s.sub_val.int_val == 66 and str(s.sub_val.str_val) == "sub hello", "The sub-struct value was not correct"
    sub = s.sub_val
    sub.int_val = 22
    ...
    assert s.sub_val.array_val[0].int_val == 0 and str(s.sub_val.array_val[0].str_val) == "zero", (
        "The array element wasn't correct",
        str(s.sub_val.array_val[0].int_val),
    )
    s.sub_val.array_val[0].int_val = 99
    s.sub_val.array_val[1].int_val = 66
    assert s.sub_val.array_val[0].int_val == 99 and s.sub_val.array_val[1].int_val == 66, (
    ...
    s = vbtest.StructProperty
    assert s.int_val == 11 and str(s.str_val) == "Hi from Python", "After sending to VB, the struct value didn't persist!"
    assert s.sub_val.array_val[0].int_val == 99, "After sending to VB, the struct array value didn't persist!"
    ...
    assert vbtest.GetStructFunc().int_val == -1, "new struct didn't make the round trip!"
