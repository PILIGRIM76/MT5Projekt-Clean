    ...
    if mod is None:
        ...
        for name in interface_names or mod.VTablesToClassMap.values():
            try:
                ...
                raise ValueError(f"Interface '{name}' does not exist in this cached typelib")
    ...
    def dispatch(
        ...
        if isinstance(retVal, tuple):  # Like pythoncom, we special case a tuple.
            ...
            if len(retVal) == len(meth._gw_out_args) + 1:
                ...
                raise TypeError("Expected {} return values, got: {}".format(len(meth._gw_out_args) + 1, len(retVal)))
        else:
            retVal = [retVal]
