...
def make_framework_support(
    ...
    fout = open(
        os.path.join(directory if directory else os.getcwd(), f"Py{interface.name}.cpp"),
        "w",
    )
    try:
        fout.write(f"""\
...
def parse_arguments():
    ...
    parser.add_argument("--header_file", "-f", required=True, help="header file (system) to parse")
    parser.add_argument("--interface_name", "-n", required=True, help="interface name to search for")
    parser.add_argument(
        "--no_create_interface",
        "-i",
        action="store_false",
    ...
    parser.add_argument("--output_directory", "-o", help="directory where to generate files")
...
if __name__ == "__main__":
    header_file, interface_name, create_interface, create_gateway, directory = parse_arguments()
    if directory:
