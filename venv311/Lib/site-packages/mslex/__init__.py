...
import re
import sys
from typing import Iterator, List, Match, Optional, TextIO  # noqa: F401
...
def split(s: str, like_cmd: bool = True, check: bool = True, ucrt: Optional[bool] = None) -> List[str]:
    """
    ...
    if ucrt is None:
        v = split_ucrt(s)
        if v != split_msvcrt(s):
            raise MSLexError("String is ambiguous, legacy and modern runtimes disagree: " + repr(s))
        return v
    elif ucrt:
        return split_ucrt(s)
    else:
...
def split_cli() -> None:
    ...
    parser = argparse.ArgumentParser(description="split a file into strings using windows-style quoting ")
