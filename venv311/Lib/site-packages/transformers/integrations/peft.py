...
class PeftConcatenate(Concatenate):
    ...
    def convert(
        ...
        **kwargs,
        ...
        ordered_tensors = [input_dict[source_pattern] for source_pattern in source_patterns if source_pattern in input_dict]
    ...
    def load_adapter(
        ...
        **kwargs,
        ...
        if self._hf_peft_config_loaded and (not hotswap) and (adapter_name in self.peft_config):
            ...
            raise ValueError("To hotswap an adapter, there must already be an existing adapter with the same adapter name.")
            ...
            for adapter_filename in adapter_filenames:
                try:
                    checkpoint_files, sharded_metadata = _get_resolved_checkpoint_files(
                        ...
                        variant=None,
                        gguf_file=None,
                        use_safetensors=(load_config.use_safetensors if adapter_filename.endswith(".safetensors") else False),
                        user_agent=None,
...
def _convert_peft_config_moe(peft_config, model_type: str):
    ...
    for new_name, required_old_targets in fused_targets.items():
        ...
        if len(present_targets) == len(required_old_targets) and len(required_old_targets) > 1:
            ...
            peft_config.alpha_pattern[rf".*\.{re.escape(new_name)}"] = peft_config.lora_alpha * len(required_old_targets)
