...
def _load_triton_kernel():
    ...
    """
    global _triton_available, triton_fp8_act_quant, triton_fp8_matmul, triton_batched_fp8_matmul, triton_grouped_fp8_matmul
...
class FP8Linear(nn.Linear):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        if self.activation_scheme == "dynamic":
            ...
            qinput, scale = triton_fp8_act_quant(input, self.block_size[1] if self.block_size is not None else input.shape[-1])
...
def _unpad_from_deepgemm_contiguous_layout(hidden_states_padded: torch.Tensor, sorted_to_padded: torch.Tensor) -> torch.Tensor:
...
class FP8Experts(nn.Module):
    def __init__(
        ...
        assert has_bias is False, "FP8Experts does not support bias for now, please open an issue if you want this feature"
        ...
        if self.has_gate:
            ...
            self.up_proj_scale_inv = nn.Parameter(torch.empty(self.num_experts, u_scale_out, u_scale_in, dtype=torch.float32))
        ...
        self.down_proj_scale_inv = nn.Parameter(torch.empty(self.num_experts, d_scale_out, d_scale_in, dtype=torch.float32))
    ...
    def forward(self, hidden_states: torch.Tensor, top_k_index: torch.Tensor, top_k_weights: torch.Tensor) -> torch.Tensor:
        ...
        for expert_idx in expert_hit:
            ...
            gate_up_act_scale = self.gate_up_proj_activation_scale[expert_idx] if self.activation_scheme == "static" else None
            ...
            down_act_scale = self.down_proj_activation_scale[expert_idx] if self.activation_scheme == "static" else None
            proj_out = self.linear(
                proj_out,
