...
def _grouped_mm_fallback_fake(input: torch.Tensor, weight: torch.Tensor, offs: torch.Tensor) -> torch.Tensor:
    ...
    assert weight.dim() == 3, f"weight must be 3D (num_experts, input_dim, output_dim), got shape {tuple(weight.shape)}"
    ...
    assert input.size(1) == weight.size(1), f"input_dim mismatch: input has {input.size(1)}, weight has {weight.size(1)}"
