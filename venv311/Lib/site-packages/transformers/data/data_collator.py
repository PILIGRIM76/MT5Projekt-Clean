...
class DataCollatorForLanguageModeling(DataCollatorMixin):
    ...
    def __post_init__(self):
        ...
        if self.whole_word_mask:
            ...
            if self.mask_replace_prob < 1:
                warnings.warn(
                    "Random token replacement is not supported with whole word masking. " "Setting mask_replace_prob to 1.",
                )
    ...
    def torch_mask_tokens(
        ...
        if self.whole_word_mask:
            word_ids, no_mask_mask = self._calc_word_ids_and_prob_mask(to_numpy(offset_mapping), to_numpy(special_tokens_mask))
        ...
        indices_replaced = (
            torch.bernoulli(torch.full(labels.shape, self.mask_replace_prob), generator=self.generator).bool() & masked_indices
        )
