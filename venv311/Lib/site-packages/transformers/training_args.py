...
class TrainingArguments:
    ...
    adam_beta1: float = field(
        default=0.9,
        metadata={"help": "The exponential decay rate for the first moment estimates (momentum) in Adam-based optimizers."},
    )
    adam_beta2: float = field(
        default=0.999,
        metadata={"help": "The exponential decay rate for the second moment estimates (variance) in Adam-based optimizers."},
    )
    ...
    bf16_full_eval: bool = field(
        default=False,
        metadata={"help": "Use full BF16 precision for evaluation (not just mixed precision). Faster and saves memory."},
    )
    ...
    run_name: str | None = field(
        ...
        metadata={
            "help": (
                "An optional descriptor for the run. Notably used for trackio, wandb, mlflow comet and swanlab " "logging."
            )
        },
    )
    project: str = field(
    ...
    metric_for_best_model: str | None = field(
        default=None,
        metadata={"help": "Metric to use for comparing models when `load_best_model_at_end=True`. Defaults to 'loss'."},
    )
    ...
    use_cpu: bool = field(
        default=False,
        metadata={"help": "Whether or not to use cpu. If set to False, we will use the available torch device/backend."},
    )
    ...
    do_train: bool = field(
        default=False,
        metadata={"help": "Whether to run training. Not directly used by Trainer; intended for training/evaluation scripts."},
    )
    do_eval: bool = field(
        default=False,
        metadata={
    ...
    def get_warmup_steps(self, num_training_steps: int):
        ...
        """
        warmup_steps = int(self.warmup_steps) if self.warmup_steps >= 1 else math.ceil(num_training_steps * self.warmup_steps)
        return warmup_steps
