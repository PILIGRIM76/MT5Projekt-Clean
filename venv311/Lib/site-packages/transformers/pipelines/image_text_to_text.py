...
class ImageTextToTextPipeline(Pipeline):
    ...
    def __call__(
        self,
        images: (
            Union[str, list[str], list[list[str]], "Image.Image", list["Image.Image"], list[list["Image.Image"]], list[dict]]
            | None
        ) = None,
    ...
    def preprocess(self, inputs=None, timeout=None, continue_final_message=None, **processing_kwargs):
        ...
        model_inputs = self.processor(images=images, text=text, return_tensors="pt", **processor_kwargs).to(dtype=self.dtype)
    ...
    def postprocess(
        ...
        **postprocess_kwargs,
        ...
        if return_type == ReturnType.TENSORS:
            return [
                {"input_text": input_texts[i], "generated_token_ids": generated_sequence[i]} for i in range(len(input_texts))
            ]
