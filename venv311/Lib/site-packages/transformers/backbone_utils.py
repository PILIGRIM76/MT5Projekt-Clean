    ...
    def verify_out_features_out_indices(self):
        ...
        if self._out_features is not None:
            ...
            if any(feat not in self.stage_names for feat in self._out_features):
                raise ValueError(f"out_features must be a subset of stage_names: {self.stage_names} got {self._out_features}")
            ...
            if self._out_features != (sorted_feats := [feat for feat in self.stage_names if feat in self._out_features]):
                raise ValueError(
        ...
        if self._out_indices is not None:
            ...
            if positive_indices != tuple(sorted(positive_indices)):
                sorted_negative = [idx for _, idx in sorted(zip(positive_indices, self._out_indices), key=lambda x: x[0])]
                raise ValueError(
