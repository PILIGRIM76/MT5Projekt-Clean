...
class PreTrainedModel(nn.Module, EmbeddingAccessMixin, ModuleUtilsMixin, PushToHubMixin, PeftAdapterMixin):
    ...
    def _flash_attn_import_error(
        ...
        if not general_availability_check():
            preface = (
                f"FlashAttention{flash_attn_version} has been toggled on, but it cannot be used due to the following error:"
            )
    ...
    def _check_and_adjust_attn_implementation(
        ...
        if is_kernel(applicable_attn_implementation):
            try:
                ...
                if is_paged:
                    lazy_import_paged_flash_attention(applicable_attn_implementation, allow_all_kernels=allow_all_kernels)
                else:
    ...
    def _initialize_weights(self, module, is_remote_code: bool = False):
        ...
        if (
            ...
            and all(
                getattr(buffer, "_is_hf_initialized", False) for buffer in module.buffers(recurse=False) if buffer is not None
            )
        ):
    ...
    def _init_added_embeddings_weights_with_mean(self, old_embeddings, new_embeddings, old_num_tokens, added_num_tokens):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        config, dtype = _get_dtype(dtype, checkpoint_files, config, sharded_metadata, state_dict, weights_only, hf_quantizer)
    ...
    @staticmethod
    def _finalize_model_loading(model, load_config: LoadStateDictConfig, loading_info: LoadStateDictInfo) -> LoadStateDictInfo:
    ...
    def _adjust_missing_and_unexpected_keys(self, loading_info: LoadStateDictInfo) -> None:
        ...
        if ignore_missing_regex is not None:
            loading_info.missing_keys = {key for key in loading_info.missing_keys if ignore_missing_regex.search(key) is None}
...
def get_total_byte_count(model: PreTrainedModel, accelerator_device_map: dict, hf_quantizer: HfQuantizer | None = None):
    """
