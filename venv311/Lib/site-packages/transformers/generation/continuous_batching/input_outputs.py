...
class ContinuousBatchingIOs:
    ...
    def _setup_static_tensors(self) -> None:
        ...
        self._bulk_input_tensor = torch.empty((7, bulk_size), dtype=torch.int32, device=self.device, pin_memory=pin_memory)
    ...
    def _transfer_inputs(self, other: "ContinuousBatchingIOs", stream: torch.cuda.Stream, non_blocking: bool = False) -> None:
    ...
    def carry_over_tokens(self, input_ids: torch.Tensor, carry_over_ids: torch.Tensor, prev_output_ids: torch.Tensor) -> None:
        pass
...
class HostDeviceIOPair:
    def __init__(
        ...
        self.host_io = ContinuousBatchingIOs(cache, config, torch.device("cpu"), model_dtype, max_graphs, return_logprobs)
