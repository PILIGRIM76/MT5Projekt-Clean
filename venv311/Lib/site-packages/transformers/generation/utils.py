...
class GenerationMixin(ContinuousMixin):
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if not self.config.is_encoder_decoder and inputs_embeds is not None and is_first_iteration:
            ...
            prompt_embeds = inputs_embeds[:, -next_sequence_length:, :] if next_sequence_length is not None else inputs_embeds
    ...
    def _get_logits_processor(
        ...
        if generation_config.watermarking_config is not None:
            processors.append(
                generation_config.watermarking_config.construct_processor(self.config.get_text_config().vocab_size, device)
            )
    ...
    def compute_transition_scores(
        ...
        if normalize_logits:
            stacked_scores = stacked_scores.reshape(-1, self.config.get_text_config().vocab_size, stacked_scores.shape[-1])
    ...
    def _prepare_static_cache(
        ...
        if isinstance(encoder_decoder_cache, EncoderDecoderCache):
            need_new_cache = (
                need_new_cache
                or encoder_decoder_cache.cross_attention_cache.max_cache_len != model_kwargs["encoder_outputs"][0].shape[1]
            )
    ...
    def generate(
        ...
        **kwargs,
        ...
        if kwargs.get("cache_implementation") == "paged":
            ...
            sequences = [outputs[f"req_{i}"].prompt_ids + outputs[f"req_{i}"].generated_tokens for i in range(len(outputs))]
