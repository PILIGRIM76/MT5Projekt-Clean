...
class SeedOssAttention(nn.Module):
    def __init__(self, config: SeedOssConfig, layer_idx: int):
        ...
        self.o_proj = nn.Linear(self.num_attention_heads * self.head_dim, config.hidden_size, bias=config.attention_out_bias)
...
class SeedOssModel(SeedOssPreTrainedModel):
    def __init__(self, config: SeedOssConfig):
        ...
        self.layers = nn.ModuleList([SeedOssDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
