...
class PPDocLayoutV2PositionRelationEmbedding(nn.Module):
    ...
    def __init__(self, config, device=None):
        ...
        self.pos_proj = nn.Conv2d(in_channels=self.embed_dim * 4, out_channels=config.num_attention_heads, kernel_size=1)
    ...
    def box_relative_encoding(self, source_boxes: torch.Tensor, target_boxes: torch.Tensor = None, epsilon: float = 1e-5):
...
class PPDocLayoutV2ReadingOrderSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, rel_pos=None, rel_2d_pos=None, **kwargs: Unpack[TransformersKwargs]):
...
class PPDocLayoutV2ReadingOrder(PPDocLayoutV2PreTrainedModel):
    ...
    def forward(self, boxes, labels=None, mask=None, **kwargs: Unpack[TransformersKwargs]):
        ...
        input_ids = torch.full((batch_size, seq_len + 2), self.config.pad_token_id, dtype=torch.long, device=boxes.device)
...
class PPDocLayoutV2MLP(nn.Module):
    def __init__(self, config: PPDocLayoutV2Config, hidden_size: int, intermediate_size: int, activation_function: str):
        super().__init__()
...
class PPDocLayoutV2EncoderLayer(nn.Module):
    def __init__(self, config: PPDocLayoutV2Config):
        ...
        self.dropout = config.dropout
        self.mlp = PPDocLayoutV2MLP(config, self.hidden_size, config.encoder_ffn_dim, config.encoder_activation_function)
...
class PPDocLayoutV2DecoderLayer(nn.Module):
    def __init__(self, config: PPDocLayoutV2Config):
        ...
        self.mlp = PPDocLayoutV2MLP(config, self.hidden_size, config.decoder_ffn_dim, config.decoder_activation_function)
        self.final_layer_norm = nn.LayerNorm(self.hidden_size, eps=config.layer_norm_eps)
...
class PPDocLayoutV2AIFILayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        hidden_states = hidden_states.permute(0, 2, 1).reshape(batch_size, self.encoder_hidden_dim, height, width).contiguous()
