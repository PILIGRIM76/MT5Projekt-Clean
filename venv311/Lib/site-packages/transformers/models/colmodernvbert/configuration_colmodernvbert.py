...
class ColModernVBertConfig(PreTrainedConfig):
    ...
    def __post_init__(self, **kwargs):
        if self.vlm_config is None:
            ...
            logger.info("`vlm_config` is `None`. Initializing `vlm_config` with the `ModernVBertConfig` with default values.")
        ...
        if self.vlm_config is None:
            ...
            logger.info("`vlm_config` is `None`. Initializing `vlm_config` with the `Qwen2VLConfig` with default values.")
        elif isinstance(self.vlm_config, dict):
            self.vlm_config = CONFIG_MAPPING[self.vlm_config["model_type"]](**self.vlm_config)
