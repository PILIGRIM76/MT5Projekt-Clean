...
class PPDocLayoutV3ImageProcessor(TorchvisionBackend):
    ...
    def extract_custom_vertices(self, polygon, sharp_angle_thresh=45):
        ...
        while i < n:
            ...
            if cross_product_value < 0:
                angle_cos = np.clip((vector_1 @ vector_2) / (np.linalg.norm(vector_1) * np.linalg.norm(vector_2)), -1.0, 1.0)
    ...
    def post_process_object_detection(
        ...
        results = []
        for score, label, box, order_seq, target_size, mask in zip(scores, labels, boxes, order_seqs, target_sizes, masks):
