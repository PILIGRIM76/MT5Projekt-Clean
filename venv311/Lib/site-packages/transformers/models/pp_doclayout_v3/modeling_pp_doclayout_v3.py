...
class PPDocLayoutV3MLP(nn.Module):
    def __init__(self, config: PPDocLayoutV3Config, hidden_size: int, intermediate_size: int, activation_function: str):
        super().__init__()
...
class PPDocLayoutV3EncoderLayer(nn.Module):
    def __init__(self, config: PPDocLayoutV3Config):
        ...
        self.dropout = config.dropout
        self.mlp = PPDocLayoutV3MLP(config, self.hidden_size, config.encoder_ffn_dim, config.encoder_activation_function)
...
class PPDocLayoutV3DecoderLayer(nn.Module):
    def __init__(self, config: PPDocLayoutV3Config):
        ...
        self.mlp = PPDocLayoutV3MLP(config, self.hidden_size, config.decoder_ffn_dim, config.decoder_activation_function)
        self.final_layer_norm = nn.LayerNorm(self.hidden_size, eps=config.layer_norm_eps)
...
def mask_to_box_coordinate(mask, dtype):
    ...
    x_min = torch.where(mask, x_coords_masked, torch.tensor(torch.finfo(dtype).max)).flatten(start_dim=-2).min(dim=-1).values

    y_coords_masked = y_coords * mask
    y_max = y_coords_masked.flatten(start_dim=-2).max(dim=-1).values + 1
    y_min = torch.where(mask, y_coords_masked, torch.tensor(torch.finfo(dtype).max)).flatten(start_dim=-2).min(dim=-1).values
