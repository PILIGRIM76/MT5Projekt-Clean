        ...
        **kwargs: Unpack[Florence2ProcessorKwargs],
        ...
        if image_inputs.get("pixel_values") is not None:
            ...
            for sample in prompt_strings:
                sample = (
                    self.image_token * self.num_image_tokens + self.tokenizer.bos_token + sample + self.tokenizer.eos_token
                )
...
class Florence2PostProcessor:
    ...
    def parse_phrase_grounding_from_text_and_spans(self, text: str, image_size: tuple[int, int]) -> list[dict[str, Any]]:
        """
        ...
        for phrase_text in phrases:
            ...
            for poly_inst in poly_instances:
                ...
                for poly_match in poly_matches:
                    ...
                    poly_coords = self.dequantize(torch.tensor(poly_bins).reshape(-1, 2), size=image_size).flatten().tolist()
    ...
    def __call__(self, text=None, sequence=None, image_size=None, parse_tasks=None) -> dict[str, Any]:
        ...
        for task in tasks_to_parse:
            ...
            if task == "ocr":
                ...
                parsed_dict["phrase_grounding"] = self.parse_phrase_grounding_from_text_and_spans(text, image_size=image_size)
            elif task == "pure_text":
...
class Florence2VisionBackbone(Florence2VisionPreTrainedModel):
    ...
    @capture_outputs
    def forward(self, hidden_states: torch.Tensor, **kwargs: Unpack[TransformersKwargs]) -> tuple | BaseModelOutputWithPooling:
...
class Florence2ForConditionalGeneration(LlavaForConditionalGeneration):
    ...
    def get_placeholder_mask(
        ...
        return self.model.get_placeholder_mask(input_ids=input_ids, inputs_embeds=inputs_embeds, image_features=image_features)
