...
if is_xlstm_available():
    ...
    def mlstm_chunkwise_recurrent_fw_C(
        ...
        matC_k = (
            torch.zeros((batch_size, nh, dhqk, dhhv), dtype=_dtype, device=_device) if matC_initial is None else matC_initial
        )
        vecN_k = torch.zeros((batch_size, nh, dhqk), dtype=_dtype, device=_device) if vecN_initial is None else vecN_initial
        scaM_inter_k = (
            torch.zeros((batch_size, nh, 1), dtype=_dtype, device=_device) if scaMinter_initial is None else scaMinter_initial
        )
    ...
    def wrap_chunkwise_arbitrary_sequence_length(
        ...
        n_state = (
            n_initial if n_initial is not None else torch.zeros(batch_size, nh, dhqk, device=key.device, dtype=torch.float32)
        )
        m_state = (
            m_initial if m_initial is not None else torch.zeros(batch_size, nh, 1, device=key.device, dtype=torch.float32)
        )
...
class xLSTMModel(xLSTMPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if not self.training and self.config.max_inference_chunksize < hidden_states.shape[1] and not output_hidden_states:
            offset = 0
            with torch.no_grad():
                ...
                while offset < hidden_states.shape[1]:
                    ...
                    final_state[:, offset : min(offset + self.config.max_inference_chunksize, hidden_states.shape[1])] = (
                        hidden_states_chunk
                    )
