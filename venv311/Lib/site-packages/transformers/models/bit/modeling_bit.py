...
class BitDownsampleConv(nn.Module):
    def __init__(
        ...
        self.norm = (
            nn.Identity() if preact else BitGroupNormActivation(config, num_channels=out_channels, apply_activation=False)
        )
...
class BitEncoder(nn.Module):
    def __init__(self, config: BitConfig):
        ...
        layer_dropouts = [
            x.tolist() for x in torch.Tensor(np.linspace(0, config.drop_path_rate, sum(config.depths))).split(config.depths)
        ]
