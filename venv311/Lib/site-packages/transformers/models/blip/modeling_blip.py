...
class BlipModel(BlipPreTrainedModel):
    ...
    def __init__(self, config: BlipConfig):
        ...
        if not isinstance(config.text_config, BlipTextConfig):
            raise TypeError(
                "config.text_config is expected to be of type BlipTextConfig but is of type" f" {type(config.text_config)}."
            )
...
class BlipForImageTextRetrieval(BlipPreTrainedModel):
    ...
    def __init__(self, config: BlipConfig):
        ...
        self.decoder_pad_token_id = (
            config.text_config.pad_token_id if not hasattr(config, "decoder_pad_token_id") else config.decoder_pad_token_id
        )
        self.decoder_start_token_id = (
            config.text_config.bos_token_id if not hasattr(config, "decoder_start_token_id") else config.decoder_start_token_id
        )
