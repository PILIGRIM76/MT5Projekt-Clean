...
class MoonshineStreamingEncoderEmbedder(nn.Module):
    def __init__(self, config):
        ...
        self.conv1 = MoonshineStreamingCausalConv1d(config.hidden_size, config.hidden_size * 2, kernel_size=5, stride=2)
        self.conv2 = MoonshineStreamingCausalConv1d(config.hidden_size * 2, config.hidden_size, kernel_size=5, stride=2)
    ...
    def forward(self, input_values, padding_mask=None):
        ...
        if padding_mask is not None:
            ...
            padding_mask = torch.arange(hidden_states.shape[1], device=padding_mask.device)[None, :] < num_frames[:, None]
...
class MoonshineStreamingEncoder(MoonshineStreamingPreTrainedModel):
    ...
    def __init__(self, config: MoonshineStreamingEncoderConfig):
        ...
        self.layers = nn.ModuleList([MoonshineStreamingEncoderLayer(config, idx) for idx in range(config.num_hidden_layers)])
...
class MoonshineStreamingDecoder(MoonshineStreamingPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList([MoonshineStreamingDecoderLayer(config, idx) for idx in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        encoder_attention_mask (`torch.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
            ...
            [What are attention masks?](../glossary#attention-mask)
        """
        position_embeddings = self.pos_emb(torch.arange(encoder_hidden_states.shape[1], device=encoder_hidden_states.device))
