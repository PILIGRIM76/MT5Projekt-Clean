...
def byte_group_hash_function(token_ids: torch.Tensor, group_size: int = 2, prime: int = 1000000007, max_hash: int = 30000):
...
def _prepare_patch_cross_attention_mask(
    ...
    if patches_as_queries:
        ...
        q_patch_ids = (
            torch.arange(num_patches, device=device).unsqueeze(0).unsqueeze(-1).expand(batch_size, num_patches, seq_len)
        )
...
class BltGlobalTransformer(BltPreTrainedModel):
    ...
    def __init__(self, config: BltGlobalTransformerConfig):
        ...
        if getattr(config, "encoder_cross_output_size", None) is not None:
            self.token_embedding_projection = nn.Linear(config.encoder_cross_output_size, config.hidden_size, bias=False)
        else:
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if patch_size is not None:
            ...
            patch_lengths = torch.ones((batch_size, sequence_length), dtype=inputs_embeds.dtype, device=inputs_embeds.device)
