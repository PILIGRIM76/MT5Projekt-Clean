...
class EfficientLoFTRRepVGGBlock(GradientCheckpointingLayer):
    ...
    def __init__(self, config: EfficientLoFTRConfig, stage_idx: int, block_idx: int):
        ...
        self.conv1 = EfficientLoFTRConvNormLayer(config, in_channels, out_channels, kernel_size=3, stride=stride, padding=1)
        self.conv2 = EfficientLoFTRConvNormLayer(config, in_channels, out_channels, kernel_size=1, stride=stride, padding=0)
...
class EfficientLoFTRAggregatedAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # Aggregate features
        aggregated_hidden_states, aggregated_encoder_hidden_states = self.aggregation(hidden_states, encoder_hidden_states)
...
class EfficientLoFTROutConvBlock(nn.Module):
    def __init__(self, config: EfficientLoFTRConfig, hidden_size: int, intermediate_size: int):
        ...
        self.out_conv2 = nn.Conv2d(intermediate_size, intermediate_size, kernel_size=3, stride=1, padding=1, bias=False)
    ...
    def forward(self, hidden_states: torch.Tensor, residual_states: torch.Tensor) -> torch.Tensor:
        ...
        residual_states = nn.functional.interpolate(residual_states, scale_factor=2.0, mode="bilinear", align_corners=False)
...
class EfficientLoFTRFineFusionLayer(nn.Module):
    def __init__(self, config: EfficientLoFTRConfig):
        ...
        self.out_conv = nn.Conv2d(fine_fusion_dims[0], fine_fusion_dims[0], kernel_size=1, stride=1, padding=0, bias=False)
    ...
    def forward_pyramid(
        ...
        hidden_states = nn.functional.interpolate(hidden_states, scale_factor=2.0, mode="bilinear", align_corners=False)
    ...
    def forward(
        ...
        fine_features_0 = nn.functional.unfold(fine_features_0, kernel_size=self.fine_kernel_size, stride=stride, padding=0)
...
class EfficientLoFTRModel(EfficientLoFTRPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        coarse_features = self.local_feature_transformer(coarse_features, position_embeddings=position_embeddings, **kwargs)
...
class EfficientLoFTRForKeypointMatching(EfficientLoFTRPreTrainedModel):
    ...
    def _get_second_stage_fine_matching(
        ...
        fine_confidence = nn.functional.softmax(fine_confidence / self.config.fine_matching_regress_temperature, dim=-1)
    ...
    def _fine_matching(
        ...
        first_stage_fine_confidence = fine_confidence.reshape(batch_size, num_keypoints, fine_window_size, fine_window_size)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        coarse_keypoints, coarse_matching_scores, coarse_matched_indices = self._coarse_matching(coarse_features, coarse_scale)
