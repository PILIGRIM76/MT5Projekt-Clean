...
class BigBirdPegasusLearnedPositionalEmbedding(nn.Embedding):
    ...
    def forward(self, input_ids_shape: torch.Size, past_key_values_length: int = 0, position_ids: torch.Tensor | None = None):
...
class BigBirdPegasusBlockSparseAttention(nn.Module):
    ...
    def _bigbird_block_rand_mask_with_head(
        ...
        rand_attn = [
            np.zeros((num_blocks, np.sum(plan_num_rand_blocks[: max_plan_idx + 1])), dtype=np.int32) for i in range(num_heads)
        ]
        # During inference (eval) no randomness
        if not self.training:
