...
class T5GemmaEncoder(T5GemmaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList([T5GemmaEncoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
        self.dropout = nn.Dropout(config.dropout_rate)
...
class T5GemmaDecoder(T5GemmaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList([T5GemmaDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
        self.dropout = nn.Dropout(config.dropout_rate)
        self.rotary_emb = T5GemmaRotaryEmbedding(config=config)
