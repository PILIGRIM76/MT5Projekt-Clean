...
class TimesFm2_5ModelForPrediction(TimesFm2_5PreTrainedModel):
    ...
    def _postprocess_output(self, model_output: torch.Tensor, stats: tuple[torch.Tensor, torch.Tensor]) -> torch.Tensor:
    ...
    def _decode_and_project(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        point_output = self.model._revin(self.output_projection_point(hidden_states), context_mu, context_sigma, reverse=True)
        ...
        point_forecast = point_output.view(batch_size, num_patches, self.config.horizon_length, num_quantiles)[:, -1, :, :]
        quantile_spreads = quantile_output.view(batch_size, num_patches, self.config.output_quantile_len, num_quantiles)[
            :, -1, :, :
        ]
