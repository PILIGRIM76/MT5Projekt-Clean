...
class Mask2FormerImageProcessor(TorchvisionBackend):
    ...
    def resize(
        ...
        **kwargs,
        ...
        image = super().resize(image, size=SizeDict(height=new_size[0], width=new_size[1]), resample=resample, **kwargs)
        return image
    ...
    def _preprocess(
        ...
        **kwargs,
        ...
        for shape, stacked_images in grouped_images.items():
            if do_resize:
                stacked_images = self.resize(image=stacked_images, size=size, size_divisor=size_divisor, resample=resample)
            if segmentation_maps is not None:
        ...
        if segmentation_maps is not None:
            resized_segmentation_maps = reorder_images(resized_segmentation_maps_grouped, grouped_segmentation_maps_index)
    ...
    def post_process_semantic_segmentation(self, outputs, target_sizes: list[tuple[int, int]] | None = None) -> "torch.Tensor":
        """
