...
class Mask2FormerPixelDecoder(nn.Module):
    ...
    def forward(
        ...
        masks = [torch.zeros((x.size(0), x.size(2), x.size(3)), device=x.device, dtype=torch.bool) for x in inputs_embeds]
...
class Mask2FormerMaskPredictor(nn.Module):
    ...
    def forward(self, outputs: torch.Tensor, pixel_embeddings: torch.Tensor, attention_mask_target_size: int | None = None):
    ...
    def forward(
        ...
        for i in range(self.num_feature_levels):
            ...
            multi_stage_features.append(
                self.input_projections[i](multi_scale_features[i]).flatten(2) + self.level_embed.weight[i][None, :, None]
            )
