...
class SegGptAttention(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, output_attentions=False) -> torch.Tensor:
        ...
        qkv = (
            self.qkv(hidden_states).reshape(batch_size, height * width, 3, self.num_attention_heads, -1).permute(2, 0, 3, 1, 4)
        )
...
class SegGptEncoder(nn.Module):
    ...
    def forward(
        ...
        if not return_dict:
            return tuple(
                v for v in [hidden_states, all_hidden_states, all_self_attentions, intermediate_hidden_states] if v is not None
            )
...
class SegGptModel(SegGptPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        prompt_pixel_values = (
            torch.cat((prompt_masks, prompt_masks), dim=2) if labels is None else torch.cat((prompt_masks, labels), dim=2)
        )
