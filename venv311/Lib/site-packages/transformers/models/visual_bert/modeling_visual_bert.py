...
class VisualBertForMultipleChoice(VisualBertPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        visual_embeds = (
            visual_embeds.view(-1, visual_embeds.size(-2), visual_embeds.size(-1)) if visual_embeds is not None else None
        )
        visual_attention_mask = (
            visual_attention_mask.view(-1, visual_attention_mask.size(-1)) if visual_attention_mask is not None else None
        )
        visual_token_type_ids = (
            visual_token_type_ids.view(-1, visual_token_type_ids.size(-1)) if visual_token_type_ids is not None else None
        )
