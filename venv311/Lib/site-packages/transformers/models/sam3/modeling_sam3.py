...
class Sam3Attention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if is_flash_attention_requested(self.config) and attention_mask is not None and attention_mask.dtype != torch.bool:
...
class Sam3DetrDecoder(Sam3PreTrainedModel):
    ...
    def _get_rpb_matrix(self, reference_boxes: torch.Tensor, spatial_shape: tuple[torch.Tensor, torch.Tensor]) -> torch.Tensor:
        """
        ...
        coords_h, coords_w = self._get_coords(height, width, dtype=reference_boxes.dtype, device=reference_boxes.device)
        ...
        rpb_matrix = deltas_y.unsqueeze(3) + deltas_x.unsqueeze(2)  # [batch_size, num_queries, height, width, num_heads]
...
class Sam3PixelDecoder(nn.Module):
    ...
    def __init__(self, config: Sam3MaskDecoderConfig):
        ...
        self.conv_layers = nn.ModuleList(
            [nn.Conv2d(hidden_size, hidden_size, kernel_size=3, stride=1, padding=1) for _ in range(num_upsampling_stages)]
        )
...
class Sam3Model(Sam3PreTrainedModel):
    ...
    def get_text_features(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        ```
        """
        text_outputs = self.text_encoder(input_ids=input_ids, attention_mask=attention_mask, return_dict=True, **kwargs)
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if geometry_prompt_features is not None:
            ...
            if text_mask is not None and geometry_prompt_mask is not None:
                ...
                geo_valid_mask = torch.ones(batch_size, geometry_prompt_features.shape[1], dtype=torch.bool, device=device)
