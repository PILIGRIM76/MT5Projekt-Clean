...
class MarkupLMFeatureExtractor(FeatureExtractionMixin):
    ...
    def xpath_soup(self, element):
        ...
        for parent in child.parents:  # type: bs4.element.Tag
            ...
            xpath_subscripts.append(0 if len(siblings) == 1 else next(i for i, s in enumerate(siblings, 1) if s is child))
            child = parent
        xpath_tags.reverse()
    ...
    def __call__(self, html_strings) -> BatchFeature:
        ...
        if not valid_strings:
            raise ValueError(
                "HTML strings must of type `str`, `list[str]` (batch of examples), " f"but is of type {type(html_strings)}."
            )
