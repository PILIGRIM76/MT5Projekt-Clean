...
class DepthProConfig(PreTrainedConfig):
    ...
    def validate_architecture(self):
        ...
        if list(self.scaled_images_ratios) != sorted(self.scaled_images_ratios):
            raise ValueError(f"Values in scaled_images_ratios={self.scaled_images_ratios} should be sorted from low to high")

        # scaled_images_ratios, scaled_images_overlap_ratios, scaled_images_feature_dims should be consistent
        if not (
            len(self.scaled_images_ratios) == len(self.scaled_images_overlap_ratios) == len(self.scaled_images_feature_dims)
        ):
            raise ValueError(
