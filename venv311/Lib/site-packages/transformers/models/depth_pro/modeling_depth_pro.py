...
class DepthProPatchEncoder(nn.Module):
    ...
    def forward(
        ...
        scaled_images_last_hidden_state = torch.split_with_sizes(encodings.last_hidden_state, n_patches_per_scaled_image[::-1])
...
class DepthProPreActResidualLayer(nn.Module):
    ...
    def __init__(self, config: DepthProConfig):
        ...
        use_bias_in_fusion_residual = (
            config.use_bias_in_fusion_residual if config.use_bias_in_fusion_residual is not None else not self.use_batch_norm
        )
...
class DepthProFovHead(nn.Module):
    def __init__(self, config: DepthProConfig):
        ...
        self.layers.append(
            nn.Conv2d(in_channels=final_in_channels, out_channels=1, kernel_size=final_kernel_size, stride=1, padding=0)
        )
