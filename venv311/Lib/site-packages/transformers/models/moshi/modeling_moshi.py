...
class MoshiDecoderLayer(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states = self.mlp(hidden_states) if not self.use_flexible_linear else self.mlp(hidden_states, codebook_idx)
        hidden_states = residual + hidden_states
...
class MoshiDepthDecoder(MoshiPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if not return_dict:
            return tuple(v for v in [loss, logits, past_key_values, all_hidden_states, all_self_attns] if v is not None)
...
class MoshiModel(MoshiPreTrainedModel):
    def __init__(self, config: MoshiConfig):
        ...
        self.layers = nn.ModuleList(
            [MoshiDecoderLayer(config, layer_idx, use_flexible_linear=False) for layer_idx in range(config.num_hidden_layers)]
        )
...
class MoshiForConditionalGeneration(MoshiPreTrainedModel, GenerationMixin):
    ...
    def generate(
        ...
        **kwargs,
        ...
        input_ids, user_audio_codes, moshi_audio_codes, concat_unconditional_inputs = self._check_and_maybe_initialize_inputs(
            input_ids=input_ids,
        ...
        **kwargs,
        ...
        if model_inputs["input_ids"] is not None:
            ...
            user_audio_codes = self.apply_delay_pattern_mask(
                torch.cat([self.generated_audio_codes, blank_user_audio_codes.to(self.generated_audio_codes.device)], dim=2),
                user_delay_pattern_mask,
            )[:, :, -1:]
