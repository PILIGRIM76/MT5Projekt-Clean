...
class NemotronHConfig(PreTrainedConfig):
    ...
    def __post_init__(self, **kwargs):
        ...
        self.time_step_floor = kwargs.pop("mamba_dt_init_floor") if "mamba_dt_init_floor" in kwargs else self.time_step_floor
    ...
    def validate_layers_block_type(self):
        ...
        if not isinstance(self.layers_block_type, list):
            raise ValueError(f"`layers_block_type` must be a list of strings. Got type: {type(self.layers_block_type)}")
        ...
        if self.num_nextn_predict_layers > 0:
            ...
            if not all(block_type in valid_types for block_type in self.mtp_layers_block_type):
                ...
                raise ValueError(f"`mtp_layers_block_type` contains invalid types: {invalid}. Must be one of: {valid_types}")
