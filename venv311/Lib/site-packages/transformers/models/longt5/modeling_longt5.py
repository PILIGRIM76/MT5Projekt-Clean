...
class LongT5LocalAttention(nn.Module):
    ...
    def compute_bias(self, block_length: int):
        """Compute binned relative position bias"""
        target_device = (
            self.relative_attention_bias.weight.device if self.relative_attention_bias.weight.device.type != "meta" else None
        )
        ...
        if isinstance(module, LongT5LayerNorm):
            ...
            if module.has_relative_attention_bias:
                ...
                if isinstance(module, LongT5TransientGlobalAttention):
                    init.normal_(module.global_relative_attention_bias.weight, mean=0.0, std=factor * ((d_model) ** -0.5))
...
class LongT5Stack(LongT5PreTrainedModel):
    def __init__(self, config):
        ...
        self.block = nn.ModuleList(
            [LongT5Block(config, has_relative_attention_bias=bool(i == 0), layer_idx=i) for i in range(config.num_layers)]
        )
