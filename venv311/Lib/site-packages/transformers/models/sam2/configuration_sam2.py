...
class Sam2HieraDetConfig(PreTrainedConfig):
    ...
    def __post_init__(self, **kwargs):
        ...
        self.embed_dim_per_stage = self.embed_dim_per_stage if self.embed_dim_per_stage is not None else [96, 192, 384, 768]
        ...
        self.window_size_per_stage = self.window_size_per_stage if self.window_size_per_stage is not None else [8, 4, 14, 7]
        self.global_attention_blocks = self.global_attention_blocks if self.global_attention_blocks is not None else [5, 7, 9]
...
class Sam2VisionConfig(PreTrainedConfig):
    ...
    def __post_init__(self, **kwargs):
        self.backbone_channel_list = [768, 384, 192, 96] if self.backbone_channel_list is None else self.backbone_channel_list
