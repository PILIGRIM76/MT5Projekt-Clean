...
class Sam2VisionNeck(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> tuple[tuple[torch.Tensor, ...], tuple[torch.Tensor, ...]]:
        ...
        for i in range(n, -1, -1):
            ...
            prev_position_encoding = self.position_encoding(prev_features.shape, prev_features.device, prev_features.dtype).to(
                prev_features.dtype
            )
...
class Sam2MultiScaleBlock(GradientCheckpointingLayer):
    def __init__(
        ...
        self.query_stride = config.query_stride if 0 < stage_idx <= config.num_query_pool_stages and block_idx == 0 else None
...
class Sam2HieraDetModel(Sam2PreTrainedModel):
    ...
    def __init__(self, config: Sam2HieraDetConfig):
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, config.hidden_size, *config.window_positional_embedding_background_size))
...
class Sam2MaskEmbedding(nn.Module):
    def __init__(self, config: Sam2PromptEncoderConfig):
        ...
        self.layer_norm1 = Sam2LayerNorm(self.mask_input_channels, eps=config.layer_norm_eps, data_format="channels_first")
        self.layer_norm2 = Sam2LayerNorm(self.mask_input_channels * 4, eps=config.layer_norm_eps, data_format="channels_first")
...
class Sam2TwoWayAttentionBlock(GradientCheckpointingLayer):
    def __init__(self, config: Sam2MaskDecoderConfig, skip_first_layer_pe: bool = False):
        ...
        self.mlp = Sam2FeedForward(config.hidden_size, config.mlp_dim, config.hidden_size, num_layers=config.num_hidden_layers)
