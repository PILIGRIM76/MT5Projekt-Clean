...
class LwDetrViTBackbone(BackboneMixin, LwDetrViTPreTrainedModel):
    ...
    def forward(self, pixel_values: torch.Tensor, **kwargs: Unpack[TransformersKwargs]) -> BackboneOutput:
        ...
        return BackboneOutput(feature_maps=feature_maps, hidden_states=output.hidden_states, attentions=output.attentions)
...
class LwDetrRepVggBlock(nn.Module):
    def __init__(self, config: LwDetrConfig):
        ...
        self.conv1 = LwDetrConvNormLayer(config, hidden_channels, hidden_channels, 3, 1, activation=config.activation_function)
        self.conv2 = LwDetrConvNormLayer(config, hidden_channels, hidden_channels, 3, 1, activation=config.activation_function)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if self.training:
            ...
            hidden_states_original = torch.cat(hidden_states_original.split(seq_len // self.config.group_detr, dim=1), dim=0)
