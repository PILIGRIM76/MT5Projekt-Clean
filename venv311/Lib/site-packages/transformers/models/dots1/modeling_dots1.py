...
class Dots1MoE(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.shared_experts = Dots1MLP(config=config, intermediate_size=config.moe_intermediate_size * config.n_shared_experts)
        self.n_routed_experts = config.n_routed_experts
...
class Dots1Model(Dots1PreTrainedModel):
    def __init__(self, config: Dots1Config):
        ...
        self.layers = nn.ModuleList([Dots1DecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
