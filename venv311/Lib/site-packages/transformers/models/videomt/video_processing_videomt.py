...
class VideomtVideoProcessor(BaseVideoProcessor):
    ...
    def post_process_instance_segmentation(
        ...
        for frame_idx in range(num_frames):
            ...
            mask_scores = (mask_probs.flatten(1) * pred_masks.flatten(1)).sum(1) / (pred_masks.flatten(1).sum(1) + 1e-6)
    ...
    def post_process_panoptic_segmentation(
        ...
        for frame_idx in range(num_frames):
            ...
            if mask_probs.shape[0] <= 0:
                ...
                segmentation = torch.full((height, width), fill_value=-1, dtype=torch.long, device=masks_queries_logits.device)
