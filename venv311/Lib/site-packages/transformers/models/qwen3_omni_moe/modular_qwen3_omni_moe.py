            ...
            for i, input_ids in enumerate(total_input_ids):
                ...
                for _ in range(multimodal_nums):
                    ...
                    # Audio in Video
                        ...
                        while (
                            video_data_index < video_llm_pos_ids.shape[-1] and audio_data_index < audio_llm_pos_ids.shape[-1]
                        ):
...
class Qwen3OmniMoeTalkerCodePredictorModel(Qwen3Model):
    ...
    def __init__(self, config: Qwen3OmniMoeTalkerCodePredictorConfig):
        ...
        self.layers = nn.ModuleList(
            [Qwen3OmniMoeTalkerCodePredictorDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if not is_first_iteration and kwargs.get("use_cache", True):
            ...
            last_residual_hidden = self.code_predictor.get_input_embeddings()[-1](predictor_result.sequences[..., -1:]).to(
                last_id_hidden.device
            )
            ...
            if generation_step < trailing_text_hidden.shape[1]:
                inputs_embeds = inputs_embeds + trailing_text_hidden[:, generation_step].unsqueeze(1).to(inputs_embeds.device)
            else:
    ...
    def generate(
        ...
        **kwargs,
        ...
        thinker_hidden = torch.cat(
            [hidden_states[self.config.talker_config.accept_hidden_layer] for hidden_states in thinker_result.hidden_states],
            dim=1,
        ).to(
            input_ids.device
        )  # [1 t d]
                ...
                if special_token == self.audio_token:
                    ...
                    if not use_audio_in_video:
                        ...
                        if video_data_index < len(video_token_indices):
                            placeholder_string += "<|video_placeholder|>" * (len(video_token_indices) - video_data_index)
                        if audio_data_index < len(audio_token_indices):
                            placeholder_string += "<|audio_placeholder|>" * (len(audio_token_indices) - audio_data_index)
