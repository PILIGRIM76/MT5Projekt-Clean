...
class ConditionalDetrDecoderCrossAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        query_states = query_states.view(*query_input_shape, self.num_attention_heads, expanded_head_dim).transpose(1, 2)
...
class ConditionalDetrMaskHeadSmallConv(nn.Module):
    ...
    def __init__(
        ...
        self.fpn_stages = nn.ModuleList(
            [
                ConditionalDetrFPNFusionStage(fpn_channels[0], hidden_size // 2, hidden_size // 4, activation_function),
                ConditionalDetrFPNFusionStage(fpn_channels[1], hidden_size // 4, hidden_size // 8, activation_function),
                ConditionalDetrFPNFusionStage(fpn_channels[2], hidden_size // 8, hidden_size // 16, activation_function),
            ]
        )
    ...
    def forward(
        ...
        fpn_features = [fpn_feat.unsqueeze(1).expand(-1, num_queries, -1, -1, -1).flatten(0, 1) for fpn_feat in fpn_features]
...
class ConditionalDetrMHAttentionMap(nn.Module):
    ...
    def forward(self, query_states: torch.Tensor, key_states: torch.Tensor, attention_mask: torch.Tensor | None = None):
        ...
        key_states = nn.functional.conv2d(key_states, self.k_proj.weight.unsqueeze(-1).unsqueeze(-1), self.k_proj.bias).view(
            key_hidden_shape
        )
        ...
        attn_weights = (torch.matmul(query * self.scaling, key.transpose(1, 2))).view(attn_weights_shape).transpose(1, 2)
...
class ConditionalDetrPreTrainedModel(PreTrainedModel):
    ...
    _keys_to_ignore_on_load_unexpected = [r"detr\.model\.backbone\.model\.layer\d+\.0\.downsample\.1\.num_batches_tracked"]
...
class ConditionalDetrModel(ConditionalDetrPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        object_queries_position_embeddings = self.query_position_embeddings.weight.unsqueeze(0).repeat(batch_size, 1, 1)
...
class ConditionalDetrForSegmentation(ConditionalDetrPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        object_queries_position_embeddings = self.conditional_detr.model.query_position_embeddings.weight.unsqueeze(0).repeat(
            batch_size, 1, 1
        )
        ...
        memory = encoder_outputs.last_hidden_state.permute(0, 2, 1).view(batch_size, self.config.d_model, height, width)
