    ...
    def _get_image_nums_and_video_nums(
        ...
        for num_image_tokens, num_video_tokens in zip(image_mask.sum(dim=1), video_mask.sum(dim=1)):
            ...
            while image_idx < len(num_image_features) and cu_num_features < num_image_tokens:
                ...
                image_idx += 1
            assert (
                cu_num_features == num_image_tokens
            ...
            while video_idx < len(num_video_features) and cu_num_features < num_video_tokens:
                ...
                video_idx += 1
            assert (
                cu_num_features == num_video_tokens
        ...
        def _expand_dict_for_generation_visual(dict_to_expand):
            ...
            for key in dict_to_expand:
                if key == "pixel_values":
                    ...
                    lengths = [
                        (torch.prod(sample, dim=1) // merge_size**2).sum() for sample, merge_size in zip(samples, merge_sizes)
                    ]
                    dict_to_expand[key] = _repeat_interleave_samples(
                        dict_to_expand[key], lengths=lengths, repeat_times=expand_size
                    )
        ...
        if videos is not None:
            ...
            for i in range(len(text)):
                while self.video_token in text[i]:
                    ...
                    frame_prompts = [
                        f"Time {t:.1f}s:" + "<|placeholder|>" * n for n, t in zip(num_frame_tokens, timestamps[video_index])
                    ]
        ...
        **kwargs,
        ...
        for image in images:
            ...
            if patches.shape[1] % temporal_patch_size != 0:
                repeats = np.repeat(patches[:, -1:], temporal_patch_size - (patches.shape[1] % temporal_patch_size), axis=1)
    ...
    def _preprocess(
        ...
        **kwargs,
        ...
        if use_token_compression:
            ...
            video_compression_mask = torch.ones((num_video_tokens,), dtype=torch.bool, device=pixel_values_videos.device)
