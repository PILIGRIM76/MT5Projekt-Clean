...
class EdgeTamVideoMemoryFuser(nn.Module):
    def __init__(self, config: EdgeTamVideoConfig):
        super().__init__()
        self.layers = nn.ModuleList([EdgeTamVideoMemoryFuserCXBlock(config) for _ in range(config.memory_fuser_num_layers)])
    ...
    def forward(
        ...
        **kwargs,
        ...
        if positional_encoding is not None:
            pos_encoding = positional_encoding.view(batch_size, seq_len_kv, self.num_attention_heads, self.head_dim).transpose(
                1, 2
            )
...
class EdgeTamVideoPerceiverResampler(nn.Module):
    def __init__(self, config: EdgeTamVideoConfig):
        ...
        self.positional_encoding = EdgeTamVideoPositionEmbeddingSine(num_pos_feats=self.hidden_size // 2, normalize=True)
    ...
    def _forward_2d(self, hidden_states: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        latents_2d = latents_2d.view(batch_size, num_windows_per_dim, num_windows_per_dim, channels).permute(0, 3, 1, 2)
