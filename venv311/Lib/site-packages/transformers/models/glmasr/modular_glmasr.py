        ...
        **kwargs: Unpack[GlmAsrProcessorKwargs],
        ...
        if prompt is None:
            ...
            if len(prompt) != batch_size:
                raise ValueError(f"Received {len(prompt)} prompt(s) for {batch_size} audio sample(s); counts must match.")
            prompts = []
            for item in prompt:
                if item is None:
...
class GlmAsrEncoder(GlmAsrPreTrainedModel):
    ...
    def __init__(self, config: GlmAsrEncoderConfig):
        ...
        self.layers = nn.ModuleList([GlmAsrEncoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
