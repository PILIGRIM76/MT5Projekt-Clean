    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if self.attn_temperature_tuning and not self.use_rope:
            ...
            attn_scales = torch.log1p(torch.floor((positions.float() + 1.0) / self.floor_scale)) * self.attn_scale + 1.0
...
class Llama4VisionRotaryEmbedding(nn.Module):
    ...
    def _compute_freqs_ci(config):
        ...
        rope_freq = 1.0 / (
            config.rope_parameters["rope_theta"] ** (torch.arange(0, freq_dim, 2)[: (freq_dim // 2)].float() / freq_dim)
        )
