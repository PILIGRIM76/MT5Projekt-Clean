...
class OneFormerImageProcessorPil(PilBackend):
    ...
    def encode_inputs(
        ...
        pixel_values_list = self._prepare_image_like_inputs(pixel_values_list, input_data_format=ChannelDimension.FIRST)
        if segmentation_maps is not None:
            segmentation_maps = self._prepare_image_like_inputs(
                images=segmentation_maps,
                expected_ndims=2,
        ...
        if annotations is not None:
            ...
            for i, label in enumerate(annotations):
                ...
                padded_masks = np.stack(padded_masks, axis=0) if padded_masks else np.zeros((0, *pad_size), dtype=np.float32)
            ...
            encoded_inputs["class_labels"] = [
                torch.from_numpy(class_label) if return_tensors == "pt" else class_label for class_label in class_labels
            ]
