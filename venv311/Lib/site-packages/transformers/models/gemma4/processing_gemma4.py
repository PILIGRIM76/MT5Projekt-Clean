...
class Gemma4Processor(ProcessorMixin):
        ...
        if videos is not None:
            ...
            for metadata, n_tokens in zip(video_metadata, num_video_tokens):
                ...
                video_replacements.append(
                    " ".join([f"{t} {self.boi_token}{self.video_token * n_tokens}{self.eoi_token}" for t in timestamp_str])
                )
    ...
    def _get_num_multimodal_tokens(self, image_sizes=None, audio_lengths=None, **kwargs):
        ...
        pooling_kernel_size = images_kwargs.get("pooling_kernel_size", None) or self.image_processor.pooling_kernel_size
        ...
        if audio_lengths is not None:
            ...
            num_audio_tokens = [self._compute_audio_num_tokens(np.zeros(length), sampling_rate) for length in audio_lengths]
    ...
    def model_input_names(self):
        ...
        model_input_names = [
            name for name in model_input_names if name not in ["num_soft_tokens_per_image", "num_soft_tokens_per_video"]
        ]
