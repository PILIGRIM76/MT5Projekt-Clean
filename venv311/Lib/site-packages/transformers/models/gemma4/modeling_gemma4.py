...
class Gemma4VisionPooler(nn.Module):
    ...
    def _avg_pool_by_positions(
        ...
        if k_squared * length != input_seq_len:
            raise ValueError(f"Cannot pool {hidden_states.shape} to {length}: {k=}^2 times {length=} must be {input_seq_len}.")
...
class Gemma4VisionRotaryEmbedding(nn.Module):
    ...
    def compute_default_rope_parameters(
        ...
        inv_freq = 1.0 / (
            base ** (torch.arange(0, spatial_dim, 2, dtype=torch.int64).to(device=device, dtype=torch.float) / spatial_dim)
        )
...
class Gemma4VisionEncoder(nn.Module):
    def __init__(self, config: Gemma4VisionConfig):
        ...
        self.layers = nn.ModuleList([Gemma4VisionEncoderLayer(config=config, layer_idx=i) for i in range(self.num_layers)])
...
class Gemma4ForCausalLM(Gemma4PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: Gemma4TextConfig):
        ...
        self._keys_to_ignore_on_load_unexpected = [f"model.{name}" for name in self.model._keys_to_ignore_on_load_unexpected]
...
class Gemma4AudioModel(Gemma4PreTrainedModel):
    ...
    def __init__(self, config: Gemma4AudioConfig):
        ...
        self.layers = nn.ModuleList([Gemma4AudioLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
...
class Gemma4Model(Gemma4PreTrainedModel):
    ...
    def __init__(self, config: Gemma4Config):
        ...
        self.embed_vision = (
            Gemma4MultimodalEmbedder(config.vision_config, config.text_config) if config.vision_config is not None else None
        )
    ...
    def create_masks_for_generate(
        ...
        **kwargs,
        ...
        if getattr(config.get_text_config(), "use_bidirectional_attention", None) == "vision":
            ...
            # Smaller Gemma models use a conventional casual attention mask
            return create_masks_for_generate(config, inputs_embeds, attention_mask, past_key_values, position_ids, **kwargs)
