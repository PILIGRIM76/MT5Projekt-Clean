...
class ModernBertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids: torch.LongTensor | None = None, inputs_embeds: torch.Tensor | None = None) -> torch.Tensor:
...
class ModernBertAttention(nn.Module):
    ...
    def __init__(self, config: ModernBertConfig, layer_idx: int | None = None):
        ...
        self.Wqkv = nn.Linear(config.hidden_size, 3 * self.head_dim * config.num_attention_heads, bias=config.attention_bias)
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if self.config.classifier_pooling == "cls":
            ...
            if attention_mask is None:
                attention_mask = torch.ones(last_hidden_state.shape[:2], device=last_hidden_state.device, dtype=torch.bool)
