...
def multi_scale_deformable_attention_v2(
    ...
    value_list = (
        value.permute(0, 2, 3, 1).flatten(0, 1).split([height * width for height, width in value_spatial_shapes], dim=-1)
    )
    ...
    for level_id, (height, width) in enumerate(value_spatial_shapes):
        ...
        if method == "default":
            ...
            sampling_coord = (sampling_grid_l_ * torch.tensor([[width, height]], device=value.device) + 0.5).to(torch.int64)
            ...
            sampling_idx = (
                torch.arange(sampling_coord.shape[0], device=value.device).unsqueeze(-1).repeat(1, sampling_coord.shape[1])
            )
...
class DFineCSPRepLayer(nn.Module):
    ...
    def __init__(self, config: DFineConfig, in_channels: int, out_channels: int, num_blocks: int, expansion: float = 1.0):
        super().__init__()
        activation = config.activation_function
...
class DFineEncoderLayer(nn.Module):
    def __init__(self, config: DFineConfig):
        ...
        self.mlp = DFineMLP(self.hidden_size, config.encoder_ffn_dim, self.hidden_size, 2, config.encoder_activation_function)
...
class DFineDecoderLayer(nn.Module):
    def __init__(self, config: DFineConfig):
        ...
        self.mlp = DFineMLP(self.hidden_size, config.decoder_ffn_dim, self.hidden_size, 2, config.decoder_activation_function)
        self.final_layer_norm = nn.LayerNorm(self.hidden_size, eps=config.layer_norm_eps)
        # gate
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        for i, decoder_layer in enumerate(self.layers):
            ...
            if i == 0:
                ...
                new_reference_points = F.sigmoid(self.pre_bbox_head(hidden_states) + inverse_sigmoid(ref_points_detach))
