...
class Gemma3nTextConfig(Gemma3TextConfig):
    ...
    def convert_rope_params_to_dict(self, **kwargs):
        ...
        self.rope_parameters["full_attention"].setdefault("rope_theta", kwargs.pop("rope_theta", self.default_theta["global"]))
...
class Gemma3nAudioRelativePositionEmbedding(nn.Module):
    ...
    def forward(self, queries: torch.Tensor, keys: torch.Tensor) -> torch.Tensor:
        ...
        sin_emb = projected_sin_emb.reshape(1, max_span_plus_1, self.num_heads, self.head_dim).squeeze(0)  # Shape [F, N, H]
...
class Gemma3nAudioAttention(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, mask: torch.BoolTensor) -> torch.Tensor:
        ...
        if (
            ...
            extracted_valid_mask_blocks = extracted_valid_mask_blocks.reshape(batch_size, num_query_blocks, self.context_size)
...
class Gemma3nAudioConformerBlock(nn.Module):
    ...
    def forward(self, audio_encodings: torch.Tensor, audio_mel_mask: torch.BoolTensor) -> torch.Tensor:
        ...
        audio_encodings_for_lconv_input = audio_encodings * validity_mask_for_lconv.unsqueeze(-1).to(audio_encodings.dtype)
...
class Gemma3nAudioEncoder(Gemma3nPreTrainedModel):
    ...
    def __init__(self, config: Gemma3nAudioConfig):
        ...
        self.conformer = nn.ModuleList([Gemma3nAudioConformerBlock(config) for _ in range(config.conf_num_hidden_layers)])
        self.post_init()
