...
class PeVideoEncoderAttention(nn.Module):
    ...
    def __init__(self, config, layer_idx):
        ...
        self.q_norm = PeVideoEncoderRMSNorm(self.head_dim, eps=config.rms_norm_eps)  # unlike olmo, only on the head dim!
        self.k_norm = PeVideoEncoderRMSNorm(self.head_dim, eps=config.rms_norm_eps)  # thus post q_norm does not need reshape
...
class PeVideoEncoder(PeVideoPreTrainedModel):
    ...
    def __init__(self, config: PeVideoEncoderConfig):
        ...
        self.layers = nn.ModuleList([PeVideoEncoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
