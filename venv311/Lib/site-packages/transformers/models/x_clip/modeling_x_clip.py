...
class XCLIPOutput(ModelOutput):
    ...
    def to_tuple(self) -> tuple[Any]:
        return tuple(
            self[k] if k not in ["text_model_output", "vision_model_output", "mit_output"] else getattr(self, k).to_tuple()
            for k in self.keys()
        )
...
class XCLIPModel(XCLIPPreTrainedModel):
    ...
    def __init__(self, config: XCLIPConfig):
        ...
        if not isinstance(config.text_config, XCLIPTextConfig):
            raise TypeError(
                "config.text_config is expected to be of type XCLIPTextConfig but is of type" f" {type(config.text_config)}."
            )
