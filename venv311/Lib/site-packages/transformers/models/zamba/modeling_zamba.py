...
class ZambaMambaMixer(nn.Module):
    ...
    def __init__(self, config: ZambaConfig, layer_idx):
        ...
        self.dt_proj_weight = nn.Parameter(
            (torch.zeros(self.n_mamba_heads, self.mamba_head_dim, self.time_step_rank) - 0.5) * 2 / self.time_step_rank**0.5
        )
    ...
    def cuda_kernels_forward(self, hidden_states: torch.Tensor, cache_params: Cache | None = None, attention_mask=None):
