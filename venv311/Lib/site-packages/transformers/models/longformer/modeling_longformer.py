...
class LongformerSelfAttention(nn.Module):
    ...
    def _mask_invalid_locations(input_tensor, affected_seq_len) -> torch.Tensor:
        ...
        input_tensor[:, :affected_seq_len, :, : affected_seq_len + 1] = torch.full_like(beginning_input, -float("inf")).where(
            beginning_mask.bool(), beginning_input
        )
        ending_input = input_tensor[:, -affected_seq_len:, :, -(affected_seq_len + 1) :]
        ending_mask = ending_mask.expand(ending_input.size())
        input_tensor[:, -affected_seq_len:, :, -(affected_seq_len + 1) :] = torch.full_like(ending_input, -float("inf")).where(
...
class LongformerModel(LongformerPreTrainedModel):
    ...
    def _pad_to_window_size(
        ...
        if padding_len > 0:
            ...
            attention_mask = nn.functional.pad(attention_mask, (0, padding_len), value=0)  # no attention on the padding tokens
    ...
    def forward(
        ...
        **kwargs,
        ...
        flat_global_attention_mask = (
            global_attention_mask.view(-1, global_attention_mask.size(-1)) if global_attention_mask is not None else None
        )
        flat_inputs_embeds = (
