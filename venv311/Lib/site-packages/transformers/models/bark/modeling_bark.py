...
class BarkBlock(GradientCheckpointingLayer):
    def __init__(self, config, is_causal=False, layer_idx=None):
        ...
        self.attn = BARK_ATTENTION_CLASSES[config._attn_implementation](config, is_causal=is_causal, layer_idx=layer_idx)
...
class BarkCausalModel(BarkPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if not return_dict:
            return tuple(v for v in [None, logits, past_key_values, all_hidden_states, all_self_attentions] if v is not None)
...
class BarkSemanticModel(BarkCausalModel):
    ...
    def generate(
        ...
        **kwargs,
        ...
        infer_array = torch.tensor([[semantic_generation_config.semantic_infer_token]] * batch_size, dtype=torch.int).to(
            self.device
        )
...
class BarkFineModel(BarkPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList([BarkBlock(config, is_causal=False, layer_idx=i) for i in range(config.num_layers)])
    ...
    def forward(
        ...
        **kwargs,
        ...
        if position_ids is None:
            position_ids = torch.arange(0, seq_length, dtype=torch.long, device=self.position_embeds_layer.weight.device)
            position_ids = position_ids.unsqueeze(0)  # shape (1, seq_length)
    ...
    def generate(
        ...
        **kwargs,
        ...
        for n_outer in range(n_loops):
            ...
            for n_inner in range(n_coarse, fine_generation_config.n_fine_codebooks):
                fine_input[:, start_fill_idx : start_fill_idx + (max_fine_input_length - rel_start_fill_idx), n_inner] = (
