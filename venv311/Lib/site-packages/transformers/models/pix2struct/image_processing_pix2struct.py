...
class Pix2StructImageProcessor(TorchvisionBackend):
    ...
    def extract_flattened_patches(
        ...
        row_ids = torch.arange(rows, device=images.device).reshape(rows, 1).repeat(1, columns).reshape(1, rows * columns, 1)
        col_ids = torch.arange(columns, device=images.device).reshape(1, columns).repeat(rows, 1).reshape(1, rows * columns, 1)
    ...
    def _preprocess(
        ...
        **kwargs,
        ...
        for shape, stacked_images in grouped_images.items():
            ...
            patches = self.extract_flattened_patches(images=stacked_images, max_patches=max_patches, patch_size=patch_size)
