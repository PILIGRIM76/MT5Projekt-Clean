...
class AutoformerFeatureEmbedder(nn.Module):
    ...
    def forward(self, features: torch.Tensor) -> torch.Tensor:
        ...
        return torch.cat(
            [embed(cat_feature_slice.squeeze(-1)) for embed, cat_feature_slice in zip(self.embedders, cat_feature_slices)],
            dim=-1,
        )
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if not self.training:
            ...
            init_index = (
                torch.arange(time_length).view(1, -1, 1).repeat(bsz * self.num_heads, 1, channel).to(value_states.device)
            )
