...
is_fast_path_available = all((ShortConvolution, chunk_gated_delta_rule, fused_recurrent_gated_delta_rule, FusedRMSNormGated))
...
class OlmoHybridGatedDeltaNet(nn.Module):
    ...
    def __init__(self, config: OlmoHybridConfig, layer_idx: int):
        ...
        A = torch.empty(self.num_v_heads, dtype=torch.float32).uniform_(config.linear_a_log_min, config.linear_a_log_max)
...
class OlmoHybridModel(Qwen3NextModel):
    def __init__(self, config: OlmoHybridConfig):
        ...
        self.rotary_emb = (
            ...
            if getattr(config, "rope_parameters", None) is not None and config.rope_parameters.get("rope_theta") is not None
            else None
        )
        self.post_init()
