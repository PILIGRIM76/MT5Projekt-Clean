...
class WhisperGenerationMixin(GenerationMixin):
        ...
        if "beam_indices" in generate_outputs:
            ...
            if num_input_ids is not None and num_input_ids > 1:
                ...
                beam_indices_first_step_unrolled = torch.ones(
                    beam_indices.shape[0], num_input_ids - 1, device=beam_indices.device, dtype=torch.long
                ) * (beam_indices[:, 0:1])
            ...
            if num_return_sequences > 1:
                if hasattr(outputs, "encoder_attentions") and outputs.encoder_attentions is not None:
                    outputs.encoder_attentions = tuple(
                        outputs.encoder_attentions[i][::num_return_sequences] for i in range(len(outputs.encoder_attentions))
                    )
            ...
            if key in ["sequences", "beam_indices", "token_timestamps"]:
                ...
                if seek_outputs[0][key] is not None:
                    ...
                    for layer_idx in range(len(seek_outputs[0][key])):
                        ...
                        all_past_key_values.append((self_attention_k, self_attention_v, cross_attention_k, cross_attention_v))
    ...
    def _set_thresholds_and_condition(
        ...
        generation_config.logprob_threshold = (
            logprob_threshold if logprob_threshold is not None else getattr(generation_config, "logprob_threshold", None)
        )
        ...
        generation_config.no_speech_threshold = (
            no_speech_threshold if no_speech_threshold is not None else getattr(generation_config, "no_speech_threshold", None)
        )
                ...
                segments.append(
                    {
                        ...
                        + start_timestamp_pos.to(torch.float32 if device.type == "mps" else torch.float64) * time_precision,
                        "end": time_offset[prev_idx]
                        + end_timestamp_pos.to(torch.float32 if device.type == "mps" else torch.float64) * time_precision,
