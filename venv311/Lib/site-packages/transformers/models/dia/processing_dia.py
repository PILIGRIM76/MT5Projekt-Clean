...
class DiaProcessor(ProcessorMixin):
    ...
    def __call__(
        ...
        **kwargs: Unpack[DiaProcessorKwargs],
        ...
        if audio_bos_token_id is None or audio_eos_token_id is None or audio_pad_token_id is None or delay_pattern is None:
            raise ValueError(
    ...
    def batch_decode(
        ...
        **kwargs: Unpack[DiaProcessorKwargs],
        ...
        # -1 for the eos token
        end_of_generation_idx = decoder_input_ids.shape[1] - (decoder_input_ids[:, :, 0] == audio_pad_token_id).sum(dim=-1) - 1
