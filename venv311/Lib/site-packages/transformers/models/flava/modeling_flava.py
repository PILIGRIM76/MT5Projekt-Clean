    ...
    def forward(self, pixel_values: torch.Tensor, interpolate_pos_encoding: bool = False) -> torch.Tensor:
        ...
        if not interpolate_pos_encoding:
            if height != self.image_size[0] or width != self.image_size[1]:
                raise ValueError(
                    f"Input image size ({height}*{width}) doesn't match model" f" ({self.image_size[0]}*{self.image_size[1]})."
                )
...
class FlavaAttention(nn.Module):
    ...
    def forward(
        ...
        self_outputs = self.attention(hidden_states, attention_mask=attention_mask, output_attentions=output_attentions)
...
class FlavaModel(FlavaPreTrainedModel):
    ...
    def __init__(self, config: FlavaConfig):
        ...
        if not isinstance(config.text_config, FlavaTextConfig):
            raise TypeError(
                "config.text_config is expected to be of type FlavaTextConfig but is of type" f" {type(config.text_config)}."
            )
