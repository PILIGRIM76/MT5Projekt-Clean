...
class Pop2PianoTokenizer(PreTrainedTokenizer):
    ...
    def batch_decode(
        ...
        if attention_masks_present:
            ...
            if (
                sum(feature_extractor_output["attention_mask"][:, 0] == 0) != feature_extractor_output["beatsteps"].shape[0]
                or feature_extractor_output["beatsteps"].shape[0] != feature_extractor_output["extrapolated_beatstep"].shape[0]
            ):
                raise ValueError(
