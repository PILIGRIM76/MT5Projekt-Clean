...
class MusicFlamingoMultiModalProjector(nn.Module):
    ...
    def __init__(self, config: MusicFlamingoConfig):
        super().__init__()
        self.linear_1 = nn.Linear(config.audio_config.hidden_size, config.text_config.hidden_size, bias=config.projector_bias)
...
class MusicFlamingoForConditionalGeneration(MusicFlamingoPreTrainedModel, GenerationMixin):
    ...
    def _build_audio_timestamps(
        ...
        frame_offsets = torch.arange(max_post_length, device=post_lengths.device, dtype=torch.float32) * audio_embed_frame_step
        ...
        window_indices = torch.arange(post_lengths.shape[0], device=post_lengths.device) - sample_start_rows[sample_indices]
