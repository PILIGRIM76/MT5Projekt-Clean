...
class Sam3VideoModel(Sam3VideoPreTrainedModel):
    ...
    def _tracker_update_memories(
        ...
        prompt_ids_for_objects = [inference_session.obj_id_to_prompt_id[obj_id] for obj_id in inference_session.obj_ids]
    ...
    def run_tracker_update_planning_phase(
        ...
        trk_prompt_ids = (
            torch.tensor(
                ...
                device=(
                    tracker_low_res_masks_global.device if tracker_low_res_masks_global.numel() > 0 else det_mask_preds.device
                ),
                dtype=torch.long,
            )
        ...
        if len(new_det_obj_ids) > 0:
            ...
            new_det_scores = det_scores[torch.tensor(new_det_out_inds, dtype=torch.long, device=det_scores.device)].tolist()
            ...
            tracker_metadata_new["obj_id_to_tracker_score_frame_wise"][frame_idx].update(zip(new_det_obj_ids, new_det_scores))
    ...
    def forward(
        ...
        **kwargs,
        ...
        inference_session.obj_id_to_tracker_score_frame_wise = tracker_metadata_new["obj_id_to_tracker_score_frame_wise"]
