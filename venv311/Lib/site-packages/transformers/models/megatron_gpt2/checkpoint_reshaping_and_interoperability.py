...
def convert_checkpoint_from_megatron_to_transformers(args):
    ...
    word_embeddings = torch.cat(
        [
            get_element_from_dict_by_path(tp_state_dicts[tp_rank], "model.language_model.embedding.word_embeddings.weight")
        ...
        for key, val in get_element_from_dict_by_path(tp_state_dicts[0], path).items():
            ...
            if op_name + "." + weight_or_bias not in tensor_parallel_params:
                ...
                params = torch.cat(
                    [val]
                    + [
                        get_element_from_dict_by_path(tp_state_dicts[tp_rank], f"{path}")[key] for tp_rank in range(1, tp_size)
                    ],
                    dim=dim,
                ).to(dtype)
...
def convert_checkpoint_from_transformers_to_megatron(args):
    ...
    for pp_rank in range(args.target_pipeline_model_parallel_size):
        ...
        for tp_rank in range(args.target_tensor_model_parallel_size):
            ...
            if args.print_checkpoint_structure:
                print(
                    f"Checkpoint structure of model state dict shard belonging to TP rank {tp_rank} and PP rank" f" {pp_rank}:"
                )
