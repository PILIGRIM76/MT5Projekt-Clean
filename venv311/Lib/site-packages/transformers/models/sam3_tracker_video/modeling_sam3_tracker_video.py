...
class Sam3TrackerVideoInferenceSession:
    ...
    def __init__(
        ...
        self.processed_frames = dict(enumerate(video.to(video_storage_device, dtype=dtype))) if video is not None else None
    ...
    def add_mask_inputs(self, obj_idx: int, frame_idx: int, inputs: torch.Tensor):
        ...
        self.mask_inputs_per_obj[obj_idx][frame_idx] = inputs.to(self.inference_device, dtype=self.dtype, non_blocking=True)
...
class Sam3TrackerVideoMemoryFuserCXBlock(GradientCheckpointingLayer):
    def __init__(self, config: Sam3TrackerVideoConfig):
        ...
        self.layer_norm = Sam3TrackerVideoLayerNorm(config.memory_fuser_embed_dim, eps=1e-6, data_format="channels_first")
...
class Sam3TrackerVideoMemoryEncoder(nn.Module):
    def __init__(self, config: Sam3TrackerVideoConfig):
        ...
        self.position_encoding = Sam3TrackerVideoPositionEmbeddingSine(num_pos_feats=output_channels // 2, normalize=True)
...
class Sam3TrackerVideoMaskDecoder(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        image_embeddings = image_embeddings.transpose(2, 3).view(batch_size * point_batch_size, num_channels, height, width)
...
class Sam3TrackerVideoModel(Sam3TrackerVideoPreTrainedModel):
    ...
    def __init__(self, config: Sam3TrackerVideoConfig, remove_vision_encoder: bool = False):
        ...
        self.no_memory_positional_encoding = torch.nn.Parameter(torch.zeros(1, 1, config.vision_config.fpn_hidden_size))
        ...
        self.memory_temporal_positional_encoding = torch.nn.Parameter(torch.zeros(self.num_maskmem, 1, 1, self.mem_dim))
    ...
    def get_image_features(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        """
        vision_outputs: Sam3TrackerVideoVisionEncoderOutput = self.vision_encoder(pixel_values, return_dict=True, **kwargs)
    ...
    def _gather_memory_frame_outputs(
        ...
        if not conditioning_outputs:
            raise ValueError("maskmem_features in conditioning outputs cannot be empty when not is_initial_conditioning_frame")
    ...
    def _get_object_pointers(
        ...
        for t_diff_offset in range(1, max_object_pointers_to_use):
            ...
            if ref_frame_idx < 0 or (not streaming and num_total_frames is not None and ref_frame_idx >= num_total_frames):
    ...
    def _process_object_pointers(
        ...
        if self.config.enable_temporal_pos_encoding_for_object_pointers:
            ...
            normalized_temporal_diffs = torch.tensor(temporal_offsets, device=device, dtype=torch.float32) / max_temporal_diff
        ...
        if self.mem_dim < num_channels:
            ...
            object_pointers = object_pointers.permute(0, 2, 1, 3).flatten(0, 1)  # (SeqLen_ptr*num_splits, Batch, MemDim)
    ...
    def _batch_encode_memories(
        ...
        for i, obj_idx in enumerate(objects_needing_memory_encoding):
            ...
            storage_key = "cond_frame_outputs" if frame_idx in output_dict["cond_frame_outputs"] else "non_cond_frame_outputs"
