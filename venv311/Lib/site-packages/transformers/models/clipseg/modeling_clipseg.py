    ...
    def __init__(self, config: CLIPSegConfig):
        ...
        if config.use_complex_transposed_convolution:
            ...
            self.transposed_convolution = nn.Sequential(
                ...
                nn.ReLU(),
                nn.ConvTranspose2d(config.reduce_dim // 2, 1, kernel_size=transposed_kernels[1], stride=transposed_kernels[1]),
            )
        else:
...
class CLIPSegModel(CLIPSegPreTrainedModel):
    ...
    def __init__(self, config: CLIPSegConfig):
        ...
        if not isinstance(config.text_config, CLIPSegTextConfig):
            raise TypeError(
                "config.text_config is expected to be of type CLIPSegTextConfig but is of type" f" {type(config.text_config)}."
            )
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if conditional_embeddings is None:
            ...
            if conditional_embeddings.shape[1] != self.config.projection_dim:
                raise ValueError(
                    "Make sure that the feature dimension of the conditional embeddings matches" " `config.projection_dim`."
                )
