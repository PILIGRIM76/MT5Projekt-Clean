...
class Phi4MultimodalVisionPreTrainedModel(SiglipPreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, Phi4MultimodalVisionEmbeddings):
            width = self.config.hidden_size if isinstance(self.config, Phi4MultimodalVisionConfig) else self.config.hidden_size
...
class Phi4MultimodalVisionMultiheadAttentionPoolingHead(SiglipMultiheadAttentionPoolingHead):
    ...
    def forward(self, hidden_state, attention_mask):
        ...
        hidden_state = self.attention(query=probe, key=hidden_state, value=hidden_state, key_padding_mask=~attention_mask)[0]
...
class Phi4MultimodalImageEmbedding(nn.Module):
    ...
    def get_img_features(self, img_embeds: torch.FloatTensor, attention_mask=None) -> torch.FloatTensor:
        img_processor_output = self.img_processor(img_embeds, patch_attention_mask=attention_mask, output_hidden_states=True)
...
class Phi4MultimodalAudioModel(Phi4MultimodalAudioPreTrainedModel):
    ...
    def _streaming_mask(self, seq_len, batch_size, chunk_size, left_chunk):
        ...
        enc_streaming_mask = (
            adaptive_enc_mask(seq_len, chunk_start_idx, left_window=left_chunk).unsqueeze(0).expand([batch_size, -1, -1])
        )
