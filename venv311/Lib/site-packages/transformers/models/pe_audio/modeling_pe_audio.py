...
class PeAudioEncoderAttention(nn.Module):
    ...
    def __init__(self, config, layer_idx):
        ...
        self.q_norm = PeAudioEncoderRMSNorm(self.head_dim, eps=config.rms_norm_eps)  # unlike olmo, only on the head dim!
        self.k_norm = PeAudioEncoderRMSNorm(self.head_dim, eps=config.rms_norm_eps)  # thus post q_norm does not need reshape
...
class PeAudioEncoder(PeAudioPreTrainedModel):
    ...
    def __init__(self, config: PeAudioEncoderConfig):
        ...
        self.layers = nn.ModuleList([PeAudioEncoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
