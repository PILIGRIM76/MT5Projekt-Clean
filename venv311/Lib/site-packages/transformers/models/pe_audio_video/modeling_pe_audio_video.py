...
class PeAudioVideoEncoderEmbedder(nn.Module):
    ...
    def _align_video_hidden_state(
        ...
        if (audio_lengths == video_hidden_state.shape[1]).all() or (video_lengths == audio_hidden_state.shape[1]).all():
        ...
        for i, (hidden_state, video_length, audio_length) in enumerate(zip(video_hidden_state, video_lengths, audio_lengths)):
...
class PeAudioVideoEncoderAttention(nn.Module):
    ...
    def __init__(self, config, layer_idx):
        ...
        self.q_norm = PeAudioVideoEncoderRMSNorm(self.head_dim, eps=config.rms_norm_eps)  # unlike olmo, only on the head dim!
