...
class AttentiveStatisticsPooling(nn.Module):
    ...
    def _length_to_mask(self, length, max_len=None, dtype=None, device=None):
        ...
        mask = torch.arange(max_len, device=length.device, dtype=length.dtype).expand(len(length), max_len) < length.unsqueeze(
            1
        )
...
class Qwen2_5OmniToken2WavBigVGANModel(Qwen2_5OmniPreTrainedModel):
    ...
    def amplitude_to_db(self, amplitude, min_db_level):
        min_level = torch.exp(torch.tensor(min_db_level / 20.0 * np.log(10), device=amplitude.device, dtype=amplitude.dtype))
...
class RungeKutta4ODESolver:
    ...
    def _compute_step(self, function, time_start, time_step, time_end, value_start):
        ...
        return (
            self._rk4_step(function, time_start, time_step, time_end, value_start, function_value_start=function_value_start),
...
class Qwen2_5OmniForConditionalGeneration(Qwen2_5OmniPreTrainedModel, GenerationMixin):
    ...
    def generate(
        ...
        **kwargs,
        ...
        processed_thinker_hidden = ((embeds_to_talker,) + thinker_result.hidden_states[0][1:],) + thinker_result.hidden_states[
            1:
        ]
        ...
        if "attention_mask" in kwargs:
            talker_attention_mask = torch.cat([kwargs["attention_mask"], kwargs["attention_mask"].new_ones((1, 2))], dim=1).to(
                input_ids.device
            )
