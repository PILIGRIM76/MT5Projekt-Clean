...
class T5Gemma2MultiModalProjector(nn.Module):
    def __init__(self, config: T5Gemma2EncoderConfig):
        ...
        self.mm_soft_emb_norm = T5Gemma2RMSNorm(config.vision_config.hidden_size, eps=config.vision_config.layer_norm_eps)
...
class T5Gemma2TextEncoder(T5Gemma2PreTrainedModel):
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList([T5Gemma2EncoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
        self.dropout = nn.Dropout(config.dropout_rate)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if pixel_values is not None:
            ...
            image_mask = self.get_image_placeholder_mask(input_ids, inputs_embeds=inputs_embeds, image_features=image_features)
...
class T5Gemma2Decoder(T5Gemma2PreTrainedModel):
    ...
    def __init__(self, config: T5Gemma2DecoderConfig, eoi_token_index: int = 256_000):
        ...
        self.layers = nn.ModuleList([T5Gemma2DecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
        self.dropout = nn.Dropout(config.dropout_rate)
        self.rotary_emb = T5Gemma2RotaryEmbedding(config)
        self.post_init()
