...
class LSHSelfAttention(nn.Module, EfficientAttentionMixin):
    ...
    def _get_sorted_bucket_idx_and_undo_sorted_bucket_idx(self, sequence_length, buckets, num_hashes):
        ...
        with torch.no_grad():
            ...
            indices = (
                torch.arange(sorted_bucket_idx.shape[-1], device=buckets.device).view(1, 1, -1).expand(sorted_bucket_idx.shape)
            )
    ...
    def _attend(
        ...
        if not do_standard_self_attention:
            ...
            key_value_bucket_idx = torch.arange(query_key_dots.shape[-1], dtype=torch.long, device=query_key_dots.device)[
                None, None, :
    ...
    def _get_relevant_hid_states_and_buckets(
        ...
        assert (
            relevant_bucket_idx_chunk.shape[-1] == (self.num_chunks_before + self.num_chunks_after + 1) * self.chunk_length
        ), (
            "There should be"
