...
class Idefics3ImageProcessorPil(PilBackend):
    ...
    def split_images(
        ...
        if height > max_height or width > max_width:
            ...
            image = self.resize(image, SizeDict(height=global_image_height, width=global_image_width), resample=resample)
            frames.append(image)
        else:
    ...
    def _preprocess(
        ...
        **kwargs,
        ...
        if do_resize:
            images = [
                [self.resize(image=img, size=size, resample=resample) for img in batch_images] for batch_images in images
            ]
        ...
        if do_image_splitting:
            ...
            for batch_images in images:
                ...
                image_cols = []
                for image in batch_images:
                    split_image_array, rows, cols = self.split_images(image, max_image_size=max_image_size, resample=resample)
        ...
        if do_pad:
            ...
            pixel_attention_masks = [
                [np.zeros((max_height, max_width), dtype=np.int64) for _ in range(max_num_images)] for _ in range(len(images))
            ]
