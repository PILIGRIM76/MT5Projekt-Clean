        ...
        **kwargs,
        ...
        if all_decode_position_ids:
            ...
            padded_decode_pos_ids = [
                F.pad(pos_ids, (0, max_decode_len - pos_ids.shape[1]), mode="replicate") for pos_ids in all_decode_position_ids
            ]
            ...
            if images_per_sample is not None:
                ...
                if total_grids % batch_size == 0:
                    ...
                    if dict_to_expand.get("pixel_values") is not None:
                        dict_to_expand["pixel_values"] = dict_to_expand["pixel_values"].repeat_interleave(expand_size, dim=0)
                    return dict_to_expand
            else:
...
def smart_resize(
    ...
    if height < factor or width < factor:
        ...
        raise ValueError(f"absolute aspect ratio must be smaller than 4, got {max(height, width) / min(height, width)}")
...
class GlmImageProcessor(ProcessorMixin):
    ...
    def __call__(
        ...
        **kwargs: Unpack[GlmImageProcessorKwargs],
        ...
        image_inputs["images_per_sample"] = torch.tensor([num_source_images + num_target_grids] * batch_size, dtype=torch.long)
