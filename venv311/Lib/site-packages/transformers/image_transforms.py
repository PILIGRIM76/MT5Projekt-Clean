...
def safe_squeeze(tensor: Union[np.ndarray, "torch.Tensor"], axis: int | None = None) -> Union[np.ndarray, "torch.Tensor"]:
    """
...
def reorder_images(
    ...
    if not is_nested:
        return [
            processed_images[grouped_images_index[i][0]][grouped_images_index[i][1]] for i in range(len(grouped_images_index))
        ]
