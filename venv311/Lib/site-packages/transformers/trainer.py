...
class Trainer:
    ...
    def _remove_unused_columns(self, dataset: "datasets.Dataset", description: str | None = None) -> "datasets.Dataset":
    ...
    def _prepare_context_parallel_inputs(
        ...
        if (
            ...
            if "position_ids" not in inputs:
                ...
                inputs["position_ids"] = torch.arange(inputs["input_ids"].size(1), device=inputs["input_ids"].device).expand(
    ...
    def _track_num_input_tokens(self, inputs):
        ...
        if self.args.include_num_input_tokens_seen == "non_padding":
            if "attention_mask" in inputs:
                ...
                logger.warning("Could not determine method to count non-padding tokens, falling back to counting all tokens.")
    ...
    def _evaluate(
        ...
        if isinstance(self.lr_scheduler, (torch.optim.lr_scheduler.ReduceLROnPlateau, GreedyLR)) and not skip_scheduler:
    ...
    def _save_optimizer_and_scheduler(self, output_dir: str) -> None:
        ...
        if is_torch_xla_available():
            ...
            if self.is_fsdp_xla_v1_enabled:
                ...
                xm.save(
                    optm,
                    os.path.join(output_dir, f"rank{self.args.process_index}-of-{self.args.world_size}-{OPTIMIZER_NAME}"),
                    master_only=False,
                )
            else:
            ...
            if is_torch_xla_available():
                ...
                if self.is_fsdp_xla_v1_enabled:
                    ...
                    optimizer_state = torch.load(
                        os.path.join(checkpoint, f"rank{self.args.process_index}-of-{self.args.world_size}-{OPTIMIZER_NAME}"),
                        map_location="cpu",
                        weights_only=True,
                    )
    ...
    def _load_scaler(self, checkpoint: str | None) -> None:
        ...
        if checkpoint_file_exists:
            ...
            if is_torch_xla_available():
                with warnings.catch_warnings(record=True) as caught_warnings:
                    check_torch_load_is_safe()
                    scaler_state = torch.load(os.path.join(checkpoint, SCALER_NAME), map_location="cpu", weights_only=True)
                reissue_pt_warnings(caught_warnings)
    ...
    def _move_model_to_device(self, model: nn.Module, device: torch.device) -> None:
        ...
        if getattr(model, "hf_device_map", None) is not None:
            logger.warning("The model is already on multiple devices. Skipping the move to device specified in `args`.")
            return
