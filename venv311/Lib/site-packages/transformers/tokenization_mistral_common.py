...
class MistralCommonBackend(PreTrainedTokenizerBase):
    ...
    def encode(
        ...
        **kwargs,
        ...
        if return_offsets_mapping or split_special_tokens:
            raise ValueError("`MistralCommonBackend` does not support `return_offsets_mapping` and `split_special_tokens`.")

        if truncation in [TruncationStrategy.ONLY_FIRST, TruncationStrategy.ONLY_SECOND, "only_first", "only_second"]:
            raise ValueError("Truncation strategy `only_first` and `only_second` are not supported by `MistralCommonBackend`.")
    ...
    def prepare_for_model(
        ...
        **kwargs,
        ...
        if kwargs:
            raise ValueError(f"Kwargs {list(kwargs.keys())} are not supported by `MistralCommonBackend.prepare_for_model`.")
    ...
    def apply_chat_template(  # type: ignore[override]
        ...
        **kwargs,
        ...
        """
        if kwargs:
            raise ValueError(f"Kwargs {list(kwargs.keys())} are not supported by `MistralCommonBackend.apply_chat_template`.")
    ...
    def num_special_tokens_to_add(self, pair: Literal[False] = False) -> int:
        ...
        """
        if pair:
            raise ValueError("`MistralCommonBackend` does not implement `pair = True` for `num_special_tokens_to_add`.")
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if text_pair or text_target or text_pair_target:
            raise ValueError("`text_pair`, `text_target` and `text_pair_target` are not supported by `MistralCommonBackend`.")
    ...
    def save_pretrained(  # type: ignore[override]
        ...
        **kwargs,
        ...
        """
        if kwargs:
            raise ValueError(f"Kwargs {list(kwargs.keys())} are not supported by `MistralCommonBackend.save_pretrained`.")
