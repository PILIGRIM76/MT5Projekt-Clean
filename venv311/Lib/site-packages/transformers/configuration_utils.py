...
def wrap_init_to_accept_kwargs(cls: dataclass):
    ...
    def __init__(self, *args, **kwargs: Any) -> None:
        ...
        if len(args) > 0:
            raise ValueError(f"{cls.__name__} accepts only keyword arguments, but found `{len(args)}` positional args.")
...
class PreTrainedConfig(PushToHubMixin, RotaryEmbeddingConfigMixin):
    ...
    def _experts_implementation(self, value: str | dict | None):
        ...
        for subconfig_key in self.sub_configs:
            ...
            if subconfig is not None:
                ...
                sub_implementation = value if not isinstance(value, dict) else value.get(subconfig_key, current_subconfig_moe)
    ...
    def validate_architecture(self):
        ...
        if (
            ...
            raise ValueError(
                f"The embed_dim ({self.embed_dim}) is not a multiple of the number of attention " f"heads ({self.num_heads})."
            )
    ...
    @classmethod
    def from_json_file(cls: type[SpecificPreTrainedConfigType], json_file: str | os.PathLike) -> SpecificPreTrainedConfigType:
        """
...
def layer_type_validation(layer_types: list[str], num_hidden_layers: int | None = None, attention: bool = True):
    ...
    if num_hidden_layers is not None and num_hidden_layers != len(layer_types):
        raise ValueError(
            f"`num_hidden_layers` ({num_hidden_layers}) must be equal to the number of layer types " f"({len(layer_types)})"
        )
