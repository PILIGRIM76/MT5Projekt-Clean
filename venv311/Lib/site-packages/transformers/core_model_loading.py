...
class SplitModulelist(ConversionOps):
    ...
    def get_target_patterns(self, input_dict: dict, source_pattern: str, target_patterns: list[str], sizes: int) -> list[str]:
        # Here it's a single operation, so we use the target
        if len(input_dict) == 1:
            if len(target_patterns) == 1:
...
def rename_source_key(
    ...
    if prefix is not None and meta_state_dict is not None:
        if renamed_key.startswith(prefix) and meta_state_dict.get(re.sub(f"^{prefix}.", "", renamed_key, count=1)) is not None:
...
def convert_and_load_state_dict_in_model(
    ...
    for original_key, tensor in state_dict:
        ...
        if renamed_key in meta_model_state_dict:
            ...
            needs_quantization = (
                hf_quantizer and not hf_quantizer.pre_quantized and hf_quantizer.param_needs_quantization(model, renamed_key)
            )
