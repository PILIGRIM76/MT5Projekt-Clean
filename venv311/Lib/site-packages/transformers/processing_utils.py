...
class ProcessorMixin(PushToHubMixin):
        ...
        **kwargs,
        ...
        for modality in default_kwargs:
            ...
            if modality in map_preprocessor_kwargs:
                ...
                preprocessor_valid_kwargs = getattr(preprocessor, "valid_kwargs", None) if preprocessor is not None else None
    ...
    @classmethod
    def _load_tokenizer_from_pretrained(cls, sub_processor_type, pretrained_model_name_or_path, subfolder="", **kwargs):
        ...
        if is_primary:
            ...
            tokenizer = auto_processor_class.from_pretrained(pretrained_model_name_or_path, subfolder=subfolder, **kwargs)
        else:
    ...
    def apply_chat_template(
        ...
        **kwargs,
        ...
        if processor_kwargs_from_kwargs:
            logger.warning("Kwargs passed to `processor.__call__` have to be in `processor_kwargs` dict, not in `**kwargs`")
        ...
        if return_assistant_tokens_mask:
            if not is_tokenizers_fast:
                ...
                processor_kwargs["return_offsets_mapping"] = True  # force offset mapping so we can infer token boundaries
    ...
    def post_process_multimodal_output(self, generated_outputs, skip_special_tokens=True, generation_mode=None, **kwargs):
        """
        ...
        return self.post_process_image_text_to_text(generated_outputs, skip_special_tokens=skip_special_tokens, **kwargs)
