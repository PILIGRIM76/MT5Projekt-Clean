...
def _format_type_annotation_recursive(type_hint):
    ...
    if origin is not None and args:
        ...
        if module_name in ("typing", "types", "builtins"):
            ...
            module_name = (
                module_name.replace("transformers.", "~").replace("typing.", "").replace("types.", "").replace("builtins.", "")
            )
...
def _process_kwargs_parameters(sig, func, parent_class, documented_kwargs, indent_level, undocumented_parameters):
    ...
    kwargs_parameters = [
        kwargs_param for _, kwargs_param in sig.parameters.items() if kwargs_param.kind == inspect.Parameter.VAR_KEYWORD
    ]
    for kwarg_param in kwargs_parameters:
        ...
        if not hasattr(kwarg_param.annotation, "__args__") or not hasattr(kwarg_param.annotation.__args__[0], "__name__"):
            continue
...
def _add_return_tensors_to_docstring(func, parent_class, docstring, indent_level):
    ...
    if (is_processor_call or is_image_processor_preprocess) and "return_tensors" not in docstring:
        ...
        source_args_dict = (
            get_args_doc_from_source(ProcessorArgs) if is_processor_call else get_args_doc_from_source(ImageProcessorArgs)
        )
...
def auto_method_docstring(
    ...
    return_docstring, func_documentation = _process_returns_section(func_documentation, sig, config_class, indent_level)
