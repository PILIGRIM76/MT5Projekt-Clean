...
def generate_attention_matrix_from_mask(
    ...
    row_dummy = " ".join(
        (
            ...
            if mask[0, j]
            else f"{GREEN}{BLACK_SQUARE}{RESET}" if j == 0 else BLACK_SQUARE if mask[0, j] else WHITE_SQUARE
        )
        for j in range(n)
    )
    ...
    if token_type_ids is not None:
        ...
        token_type_buckets = torch.where((token_type_ids.cumsum(-1) % 5 + is_special).bool(), token_type_ids.cumsum(-1), 0)
    ...
    for row in vertical_header:
        output.append(
            (max_word_length + 5) * " " + " ".join(row) + "    |    " + " ".join(row) if sliding_window is not None else ""
        )
    for i, word in enumerate(words):
        ...
        row_display = " ".join(
            (
                ...
                else f"{GREEN}{BLACK_SQUARE}{RESET}" if i == j else BLACK_SQUARE if mask[i, j] else WHITE_SQUARE
            )
            for j in range(n)
        )
        sliding_window_row = ""
