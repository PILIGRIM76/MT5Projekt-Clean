...
def device_validator(value: str | int | None = None):
    ...
    if value is None:
        ...
        if device_str.split(":")[0] not in possible_names:
            raise ValueError(f"If device is a torch.device, the value must be one of {possible_names} but got device={value}")
    elif isinstance(value, int) and value < 0:
        raise ValueError(f"If device is an integer, the value must be a strictly positive integer but got device={value}")
...
def video_metadata_validator(value: VideoMetadataType | None = None):
    ...
    if isinstance(value, Sequence) and isinstance(value[0], Sequence) and isinstance(value[0][0], dict):
        ...
        if not check_dict_keys(value):
            raise ValueError(f"Invalid keys found in video metadata. Valid keys: {valid_keys}, got: {list(value.keys())}")
