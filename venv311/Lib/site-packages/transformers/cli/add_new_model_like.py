...
def add_model_to_auto_mappings(
    ...
    filenames_to_add = [(filename.replace(old_lowercase_name, "auto"), to_add) for filename, to_add in filenames_to_add[1:]]
    ...
    for filename, to_add in corrected_filenames_to_add:
        if to_add:
            ...
            matching_lines = re.findall(rf'( {{8,12}}\(\s*"{old_lowercase_name}",.*?\),\n)(?: {{4,12}}\(|\])', file, re.DOTALL)
            for match in matching_lines:
                add_content_to_file(
                    repo_path / "src" / "transformers" / "models" / "auto" / filename,
                    new_content=match.replace(old_lowercase_name, new_lowercase_name).replace(old_cased_name, new_cased_name),
                    add_after=match,
                )
...
def insert_model_in_doc_toc(repo_path: Path, old_lowercase_name: str, new_lowercase_name: str, new_model_paper_name: str):
    """
    ...
    add_content_to_file(repo_path / "docs" / "source" / "en" / "_toctree.yml", new_content=new_toc, add_after=old_model_toc)
...
def create_init_file(old_lowercase_name: str, new_lowercase_name: str, filenames_to_add: list[tuple[str, bool]]):
    ...
    init_file = COPYRIGHT + textwrap.dedent(f"""
...
def create_modular_file(
    ...
    all_statement = textwrap.dedent(f"""
...
def _add_new_model_like_internal(
    ...
    modular_file, public_classes = create_modular_file(repo_path, old_model_infos, new_lowercase_name, filenames_to_add)
    ...
    subprocess.run(["python", "utils/check_doc_toc.py", "--fix_and_overwrite"], cwd=repo_path, stdout=subprocess.DEVNULL)
...
def get_user_input():
    ...
    while not valid_model_type:
        old_model_type = input("What model would you like to duplicate? Please provide it as lowercase, e.g. `llama`): ")
    ...
    new_lowercase_name = get_user_field("What is the new model name? Please provide it as snake lowercase, e.g. `new_model`?")
