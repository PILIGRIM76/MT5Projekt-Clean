...
class GenerateManager(BaseGenerateManager):
    ...
    async def generate_non_streaming(
        ...
        sequences = await self.async_submit(model.generate, **inputs, generation_config=gen_config, tokenizer=processor)
...
class CBGenerateManager(BaseGenerateManager):
    ...
    def init_cb(self, model: "PreTrainedModel", gen_config: "GenerationConfig") -> None:
        ...
        self._cb = model.init_continuous_batching(generation_config=gen_config, continuous_batching_config=self._cb_config)
        self._cb.start()
...
class GenerationState:
    ...
    def use_continuous_batching(self, model: "PreTrainedModel", modality: Modality) -> bool:
        ...
        if not can:
            logger.warning_once(
                f"{model.__class__.__name__} does not support continuous batching. " "Falling back to sequential generation."
            )
        return can
