...
class ModelManager:
    ...
    def _validate_args(self):
        if self.quantization is not None and self.quantization not in ("bnb-4bit", "bnb-8bit"):
            raise ValueError(f"Unsupported quantization method: '{self.quantization}'. Must be 'bnb-4bit' or 'bnb-8bit'.")
    ...
    def load_model_and_processor(
        ...
        with lock:
            if model_id_and_revision not in self.loaded_models:
                ...
                model = self._load_model(model_id_and_revision, tqdm_class=tqdm_class, progress_callback=progress_callback)
