        ...
        ctx.optimizer_kwargs.update(
            {
                ...
                "compensation_buffer_dtype": getattr(torch, ctx.optim_args.get("compensation_buffer_dtype", "bfloat16")),
            }
        )
...
def _get_lomo_optimizer(ctx: OptimizerContext) -> tuple[Any, dict[str, Any]]:
    ...
    if not is_lomo_available():
        raise ImportError(
            "You need to install `lomo_optim` in order to use LOMO optimizers. " "Install it with `pip install lomo-optim`"
        )
