...
class Cache:
    ...
    def has_previous_state(self, layer_idx: int | None = None) -> bool:
        ...
        if layer_idx is None:
            try:
                layer_idx = next(
                    idx for idx in range(len(self) - 1, -1, -1) if isinstance(self.layers[idx], LinearAttentionCacheLayerMixin)
                )
            except StopIteration:
                raise ValueError(
...
class QuantizedCache(Cache):
    ...
    def __init__(
        ...
        config = config.get_text_config(decoder=True)
        layers = [
            layer_class(nbits, axis_key, axis_value, q_group_size, residual_length) for _ in range(config.num_hidden_layers)
        ]
