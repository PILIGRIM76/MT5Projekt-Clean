...
class TensorProcessor:
    ...
    def perform_fallback_tensor_mapping(self, gguf_to_hf_name_map: dict[str, str], suffix: str, qual_name: str, hf_name: str):
        """
...
class LlamaTensorProcessor(TensorProcessor):
    ...
    def _reverse_permute_weights(self, weights: np.ndarray, n_head: int, num_kv_heads: int | None = None) -> np.ndarray:
...
def get_gguf_hf_weights_map(
    ...
    if named_children := hf_model.named_children():
        for name, child in named_children:
            sub_map = get_gguf_hf_weights_map(child, processor, model_type, num_layers, qual_name=f"{qual_name}{name}.")
