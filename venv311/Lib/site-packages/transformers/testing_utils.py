...
def require_accelerate(test_case, min_version: str = ACCELERATE_MIN_VERSION):
    ...
    """
    return unittest.skipUnless(is_accelerate_available(min_version), f"test requires accelerate version >= {min_version}")(
        test_case
    )
...
def require_torch_fp16(test_case):
    ...
    return unittest.skipUnless(is_torch_fp16_available_on_device(torch_device), "test requires device with fp16 support")(
        test_case
    )
...
def require_torch_bf16(test_case):
    ...
    return unittest.skipUnless(is_torch_bf16_available_on_device(torch_device), "test requires device with bf16 support")(
        test_case
    )
...
def require_torch_tf32(test_case):
    ...
    return unittest.skipUnless(is_torch_tf32_available(), "test requires Ampere or a newer GPU arch, cuda>=11 and torch>=1.7")(
        test_case
    )
        ...
        class MockAwareDocTestFinder(doctest.DocTestFinder):
            ...
            def _find(self, tests, obj, name, module, source_lines, globs, seen) -> None:
                ...
                with _patch_unwrap_mock_aware():
                    # Type ignored because this is a private function.
                    super()._find(tests, obj, name, module, source_lines, globs, seen)  # type: ignore[misc]
...
if is_torch_available():
    ...
    if "TRANSFORMERS_TEST_DEVICE_SPEC" in os.environ:
        ...
        if not Path(device_spec_path).is_file():
            raise ValueError(f"Specified path to device spec file is not a file or not found. Received '{device_spec_path}")
    ...
    if str(obj.__class__) == "<class 'torch.Tensor'>":
        ...
        if isinstance(obj, dict):
            ...
            for idx, x in enumerate(obj):
                last_element = idx == len(obj) - 1
                o = _format_py_obj(x, indent=indent + 1, mode=mode, cache=cache, prefix="," if not last_element else "")
                # Each element could be multiple-line, but the indent of its first line is removed
        ...
        for idx, x in enumerate(elements_without_indent):
            ...
            should_finalize_x = x_expanded or len(f"{' ' * (4 * (indent + 1))}") + len(", ".join(buf[-1:])) > 120 - int(
                not_last_element
            )
