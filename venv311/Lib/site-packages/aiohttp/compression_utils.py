...
class DecompressionBaseHandler(ABC):
    ...
    @abstractmethod
    def decompress_sync(self, data: bytes, max_length: int = ZLIB_MAX_LENGTH_UNLIMITED) -> bytes:
        """Decompress the given data."""

    async def decompress(self, data: bytes, max_length: int = ZLIB_MAX_LENGTH_UNLIMITED) -> bytes:
        """Decompress the given data."""
...
class ZLibCompressor:
    def __init__(
        ...
        self._executor = executor
        self._max_sync_chunk_size = max_sync_chunk_size
        self._mode = encoding_to_mode(encoding, suppress_deflate_header) if wbits is None else wbits
    ...
    async def compress(self, data: bytes) -> bytes:
        ...
        **WARNING: This method is NOT cancellation-safe when used with flush().**
        ...
        should_use_executor = self._max_sync_chunk_size is not None and len(data) > self._max_sync_chunk_size
...
class ZLibDecompressor(DecompressionBaseHandler):
    ...
    def decompress_sync(self, data: Buffer, max_length: int = ZLIB_MAX_LENGTH_UNLIMITED) -> bytes:
...
class ZSTDDecompressor(DecompressionBaseHandler):
    ...
    def decompress_sync(self, data: bytes, max_length: int = ZLIB_MAX_LENGTH_UNLIMITED) -> bytes:
        ...
        zstd_max_length = ZSTD_MAX_LENGTH_UNLIMITED if max_length == ZLIB_MAX_LENGTH_UNLIMITED else max_length
