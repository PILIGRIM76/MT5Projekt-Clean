...
class PipelineTuningOptions(ffi.ObjectRef):
    ...
    def size_level(self, value):
        ...
        if value != 0 and self.speed_level != 2:
            raise ValueError("Optimization for size should be encoded with speed level == 2")
...
class PassBuilder(ffi.ObjectRef):
    ...
    def getModulePassManager(self):
        return ModulePassManager(
            ffi.lib.LLVMPY_buildPerModuleDefaultPipeline(self, self._pto.speed_level, self._pto.size_level)
        )

    def getFunctionPassManager(self):
        return FunctionPassManager(
            ffi.lib.LLVMPY_buildFunctionSimplificationPipeline(self, self._pto.speed_level, self._pto.size_level)
        )
    ...
    def finish_pass_timing(self):
        ...
        with ffi.OutputString() as buf:
            ffi.lib.LLVMPY_ReportAndDisableTimePasses(self._time_passes_handler, buf)
            return str(buf)
...
ffi.lib.LLVMPY_PTOSetInlinerThreshold.argtypes = [ffi.LLVMPipelineTuningOptionsRef, c_int]
...
ffi.lib.LLVMPY_buildPerModuleDefaultPipeline.argtypes = [ffi.LLVMPassBuilderRef, c_int, c_int]

ffi.lib.LLVMPY_buildFunctionSimplificationPipeline.restype = ffi.LLVMFunctionPassManagerRef

ffi.lib.LLVMPY_buildFunctionSimplificationPipeline.argtypes = [ffi.LLVMPassBuilderRef, c_int, c_int]
