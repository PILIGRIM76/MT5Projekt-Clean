...
class Checker:
    ...
    # same for operators
    AND = OR = ADD = SUB = MULT = DIV = MOD = POW = LSHIFT = RSHIFT = BITOR = BITXOR = BITAND = FLOORDIV = INVERT = NOT = (
        UADD
    ) = USUB = EQ = NOTEQ = LT = LTE = GT = GTE = IS = ISNOT = IN = NOTIN = MATMULT = ignore
    ...
    def GLOBAL(self, node):
        ...
        if self.scope is not global_scope:
            ...
            for node_name in node.names:
                ...
                self.messages = [
                    m for m in self.messages if not isinstance(m, messages.UndefinedName) or m.message_args[0] != node_name
                ]
