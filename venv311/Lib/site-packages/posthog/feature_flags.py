...
def evaluate_flag_dependency(property, flags_by_key, evaluation_cache, distinct_id, properties, cohort_properties):
    """
    ...
    if len(dependency_chain) == 0:
        ...
        raise InconclusiveMatchError(f"Circular dependency detected for flag '{property.get('key', 'unknown')}'")
            ...
            if not dep_flag:
                ...
                if not dep_flag.get("active"):
                    ...
                    try:
                        ...
                        evaluation_cache[dep_flag_key] = None
                        raise InconclusiveMatchError(f"Cannot evaluate flag dependency '{dep_flag_key}': {e}") from e

        # Check the cached result
        cached_result = evaluation_cache[dep_flag_key]
        if cached_result is None:
            # Previously inconclusive - raise error again
            raise InconclusiveMatchError(f"Flag dependency '{dep_flag_key}' was previously inconclusive")
    ...
    if flag_key and expected_value is not None:
        ...
        if actual_value is None:
            ...
            raise InconclusiveMatchError(f"Flag '{flag_key}' was not evaluated despite being in dependency chain")
