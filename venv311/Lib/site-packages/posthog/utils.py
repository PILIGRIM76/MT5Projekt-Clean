...
class FlagCache:
    ...
    def set_cached_flag(self, distinct_id, flag_key, flag_result, flag_definition_version):
        ...
        self.cache[distinct_id][flag_key] = FlagCacheEntry(flag_result, flag_definition_version, current_time)
        self.access_times[distinct_id] = current_time
...
class RedisFlagCache:
    def __init__(self, redis_client, default_ttl=300, stale_ttl=3600, key_prefix="posthog:flags:"):
    ...
    def get_cached_flag(self, distinct_id, flag_key, current_flag_version):
        try:
            ...
            if data:
                ...
                if entry and entry.is_valid(time.time(), self.default_ttl, current_flag_version):
                    return entry.flag_result
    ...
    def set_cached_flag(self, distinct_id, flag_key, flag_result, flag_definition_version):
        try:
            cache_key = self._get_cache_key(distinct_id, flag_key)
            serialized_entry = self._serialize_entry(flag_result, flag_definition_version)
