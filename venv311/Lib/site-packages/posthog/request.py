...
KEEP_ALIVE_SOCKET_OPTIONS: SocketOptions = list(HTTPConnection.default_socket_options) + [
...
def post(
    ...
    **kwargs,
    ...
    res = (session or _get_session()).post(url, data=data, headers=headers, timeout=timeout)
    ...
    if retry_after_header:
        try:
            ...
            try:
                ...
                retry_after = max(
                    0.0,
                    (parsedate_to_datetime(retry_after_header) - datetime.now(timezone.utc)).total_seconds(),
                )
            except (ValueError, TypeError):
                pass
...
def get(
    ...
    data = _process_response(res, success_message=f"GET {masked_url} completed successfully")
    response_etag = res.headers.get("ETag")
