...
class Client(object):
    ...
    def __init__(
        ...
        self.code_variables_mask_patterns = (
            code_variables_mask_patterns if code_variables_mask_patterns is not None else DEFAULT_CODE_VARIABLES_MASK_PATTERNS
        )
    ...
    def new_context(self, fresh=False, capture_exceptions=True):
        ...
        Category:
            Contexts
        """
        return new_context(fresh=fresh, capture_exceptions=capture_exceptions, client=self)
    ...
    @no_throw()
    def capture(self, event: str, **kwargs: Unpack[OptionalCaptureArgs]) -> Optional[str]:
        """
    ...
    def _parse_send_feature_flags(self, send_feature_flags) -> SendFeatureFlagsOptions:
        ...
        if isinstance(send_feature_flags, dict):
            ...
            raise TypeError(f"Invalid type for send_feature_flags: {type(send_feature_flags)}. " f"Expected bool or dict.")
    ...
    def capture_exception(
        ...
        **kwargs: Unpack[OptionalCaptureArgs],
        ...
        try:
            ...
            enabled = context_enabled if context_enabled is not None else self.capture_exception_code_variables
    ...
    def _update_flag_state(self, data: FlagDefinitionCacheData, old_flags_by_key: Optional[dict] = None) -> None:
        ...
        if self.flag_cache and old_flags_by_key is not None and old_flags_by_key != (self.feature_flags_by_key or {}):
    ...
    def _load_feature_flags(self):
        ...
        if self._flag_definition_cache_provider:
            try:
                should_fetch = self._flag_definition_cache_provider.should_fetch_flag_definitions()
            except Exception as e:
                self.log.error(f"[FEATURE FLAGS] Cache provider should_fetch error: {e}")
        ...
        if not should_fetch and self._flag_definition_cache_provider:
            try:
                cached_data = self._flag_definition_cache_provider.get_flag_definitions()
                if cached_data:
                    self.log.debug("[FEATURE FLAGS] Using cached flag definitions from external cache")
                    self._update_flag_state(cached_data, old_flags_by_key=self.feature_flags_by_key or {})
                    ...
                    if not self.feature_flags:
                        self.log.debug("[FEATURE FLAGS] Cache empty and no flags loaded, falling back to API fetch")
                        should_fetch = True
            except Exception as e:
    ...
    def _fetch_feature_flags_from_api(self):
        ...
        try:
            ...
            if response.not_modified:
                self.log.debug("[FEATURE FLAGS] Flags not modified (304), using cached data")
                self._last_feature_flag_poll = datetime.now(tz=tzutc())
                return

            if response.data is None:
                self.log.error("[FEATURE FLAGS] Unexpected empty response data in non-304 response")
                return
    ...
    def load_feature_flags(self):
        ...
        if self.enable_local_evaluation and not (self.poller and self.poller.is_alive()):
            self.poller = Poller(
    ...
    def _get_stale_flag_fallback(self, distinct_id: ID_TYPES, key: str) -> Optional[FeatureFlagResult]:
        ...
        if self.flag_cache:
            ...
            if stale_result:
                self.log.info(f"[FEATURE FLAGS] Using stale cached value for flag {key}")
                return stale_result
        return None
    ...
    def _get_feature_flag_result(
        ...
        *,
        ...
        if flag_was_locally_evaluated:
            ...
            payload = self._compute_payload_locally(key, lookup_match_value) if lookup_match_value is not None else None
            ...
            if self.flag_cache and flag_result:
                self.flag_cache.set_cached_flag(distinct_id, key, flag_result, self.flag_definition_version)
        elif not only_evaluate_locally:
            try:
                flag_details, request_id, evaluated_at, errors_while_computing = self._get_feature_flag_details_from_server(
                    key,
                    distinct_id,
                    groups,
                    person_properties,
        ...
        *,
        ...
        if self.feature_flags:
            ...
            if flag_keys_to_evaluate:
                ...
                flags_to_process = [flag for flag in self.feature_flags if flag["key"] in flag_keys_set]
        ...
        try:
            ...
            if scheme == "memory":
                ...
                try:
                    ...
                    redis_url += f"{parsed.hostname or 'localhost'}:{parsed.port or 6379}"
                    if parsed.path:
                    ...
                    self.log.warning("[FEATURE FLAGS] Redis not available, flag caching disabled")
                    return None
                except Exception as e:
                    self.log.warning(f"[FEATURE FLAGS] Redis connection failed: {e}, flag caching disabled")
                    return None
            else:
                raise ValueError(f"Unknown cache URL scheme: {scheme}. Supported schemes: memory, redis")

        except Exception as e:
            self.log.warning(f"[FEATURE FLAGS] Failed to parse cache URL '{cache_url}': {e}")
            return None
