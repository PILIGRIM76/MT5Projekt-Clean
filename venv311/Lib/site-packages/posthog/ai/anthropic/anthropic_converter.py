...
def format_anthropic_response(response: Any) -> List[FormattedMessage]:
    ...
    if hasattr(response, "content"):
        for choice in response.content:
            ...
            elif hasattr(choice, "type") and choice.type == "tool_use" and hasattr(choice, "name") and hasattr(choice, "id"):
...
def extract_anthropic_usage_from_event(event: Any) -> TokenUsage:
    ...
    if hasattr(event, "type") and event.type == "message_start":
        if hasattr(event, "message") and hasattr(event.message, "usage"):
            ...
            usage["cache_creation_input_tokens"] = getattr(event.message.usage, "cache_creation_input_tokens", 0)
            usage["cache_read_input_tokens"] = getattr(event.message.usage, "cache_read_input_tokens", 0)
            # Capture raw usage metadata for backend processing
            # Serialize to dict here in the converter (not in utils)
    ...
    if hasattr(event, "usage") and event.usage:
        ...
        if hasattr(event.usage, "server_tool_use"):
            ...
            if hasattr(server_tool_use, "web_search_requests"):
                web_search_count = int(getattr(server_tool_use, "web_search_requests", 0))
                if web_search_count > 0:
...
def handle_anthropic_text_delta(event: Any, current_block: Optional[StreamingContentBlock]) -> Optional[str]:
    """
...
def handle_anthropic_tool_delta(
    ...
    if not (hasattr(event, "delta") and hasattr(event.delta, "type") and event.delta.type == "input_json_delta"):
        return
