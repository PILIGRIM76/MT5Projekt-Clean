...
def _is_prompt_api_response(data: Any) -> bool:
    ...
    return isinstance(data, dict) and "prompt" in data and isinstance(data.get("prompt"), str)
...
class Prompts:
    ...
    def __init__(
        ...
        *,
        ...
        """
        self._default_cache_ttl_seconds = default_cache_ttl_seconds or DEFAULT_CACHE_TTL_SECONDS
        self._cache: Dict[str, CachedPrompt] = {}

        if posthog is not None:
            self._personal_api_key = getattr(posthog, "personal_api_key", None) or ""
            self._host = remove_trailing_slash(getattr(posthog, "raw_host", None) or APP_ENDPOINT)
        else:
    ...
    def get(
        ...
        *,
        ...
        """
        ttl = cache_ttl_seconds if cache_ttl_seconds is not None else self._default_cache_ttl_seconds
    ...
    def _fetch_prompt_from_api(self, name: str) -> str:
        ...
        if not response.ok:
            ...
            raise Exception(f'[PostHog Prompts] Failed to fetch prompt "{name}": HTTP {response.status_code}')

        try:
            data = response.json()
        except Exception:
            raise Exception(f'[PostHog Prompts] Invalid response format for prompt "{name}"')
