...
def merge_usage_stats(target: TokenUsage, source: TokenUsage, mode: str = "incremental") -> None:
    """
    ...
    if mode == "incremental":
        ...
        if source_raw_usage is not None and isinstance(source_raw_usage, dict):
            ...
            current_raw: Dict[str, Any] = current_raw_value if isinstance(current_raw_value, dict) else {}
    ...
    **kwargs: Any,
    ...
    with new_context(client=ph_client, capture_exceptions=False):
        ...
        try:
            ...
            tag(
                "$ai_output_choices",
                with_privacy_mode(ph_client, posthog_privacy_mode, format_response(response, provider)),
            )
            ...
            if provider == "openai" and kwargs.get("instructions") is not None:
                tag(
                    "$ai_instructions",
                    with_privacy_mode(ph_client, posthog_privacy_mode, kwargs.get("instructions")),
                )
...
def capture_streaming_event(
    ...
    if web_search_count is not None and isinstance(web_search_count, int) and web_search_count > 0:
    ...
    if event_data["provider"] == "openai" and event_data["kwargs"].get("instructions") is not None:
