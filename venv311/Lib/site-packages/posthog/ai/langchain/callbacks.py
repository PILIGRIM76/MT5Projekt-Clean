try:
    ...
    raise ModuleNotFoundError("Please install LangChain to use this feature: 'pip install langchain-core'")
...
class CallbackHandler(BaseCallbackHandler):
    ...
    def _capture_trace_or_span(
        ...
        event_properties = {
            ...
            "$ai_input_state": with_privacy_mode(self._ph_client, self._privacy_mode, sanitize_langchain(run.input)),
        ...
        if isinstance(outputs, BaseException):
            ...
            event_properties["$ai_output_state"] = with_privacy_mode(self._ph_client, self._privacy_mode, outputs)
    ...
    def _capture_generation(
        ...
        event_properties = {
            ...
            "$ai_input": with_privacy_mode(self._ph_client, self._privacy_mode, sanitize_langchain(run.input)),
            "$ai_http_status": 200,
            "$ai_latency": run.latency,
        ...
        if isinstance(output, BaseException):
            ...
            if isinstance(generation_result[-1], ChatGeneration):
                ...
                completions = [_extract_raw_response(generation) for generation in generation_result]
            event_properties["$ai_output_choices"] = with_privacy_mode(self._ph_client, self._privacy_mode, completions)
    ...
    def _log_debug_event(
        ...
        **kwargs,
        ...
        log.debug(f"Event: {event_name}, run_id: {str(run_id)[:5]}, parent_run_id: {str(parent_run_id)[:5]}, kwargs: {kwargs}")
...
def _convert_message_to_dict(message: BaseMessage) -> dict[str, Any]:
    ...
    if isinstance(message, HumanMessage):
        ...
        if message.tool_calls:
            message_dict["tool_calls"] = _convert_lc_tool_calls_to_oai(message.tool_calls)
...
def _parse_usage_model(
    ...
    if is_anthropic and normalized_usage.input_tokens:
        cache_tokens = (normalized_usage.cache_read_tokens or 0) + (normalized_usage.cache_write_tokens or 0)
        if cache_tokens > 0:
            normalized_usage.input_tokens = max(normalized_usage.input_tokens - cache_tokens, 0)
    return normalized_usage


def _parse_usage(response: LLMResult, provider: Optional[str] = None, model: Optional[str] = None) -> ModelUsage:
    ...
    if response.llm_output is not None:
        for key in llm_usage_keys:
            if response.llm_output.get(key):
                llm_usage = _parse_usage_model(response.llm_output[key], provider, model)
                break
