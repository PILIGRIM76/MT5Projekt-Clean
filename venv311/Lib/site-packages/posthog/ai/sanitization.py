...
def sanitize_openai_image(item: Any) -> Any:
    ...
    if item.get("type") == "image_url" and isinstance(item.get("image_url"), dict) and "url" in item["image_url"]:
        return {
            **item,
            "image_url": {
                **item["image_url"],
...
def sanitize_langchain_image(item: Any) -> Any:
    ...
    if item.get("type") == "image" and isinstance(item.get("source"), dict) and "data" in item["source"]:
        if _is_multimodal_enabled():
            return item
