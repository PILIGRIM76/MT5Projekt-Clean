...
def get_error_message(exc_value):
    ...
    message = getattr(exc_value, "message", "") or getattr(exc_value, "detail", "") or exc_value
...
def exceptions_from_error_tuple(
    ...
    if is_exception_group:
        ...
        for exc_type, exc_value, tb in walk_exception_chain(exc_info):
            exceptions.append(single_exception_from_error_tuple(exc_type, exc_value, tb, mechanism))
...
def exception_is_already_captured(error):
    ...
    # the second item is the exception value (the first is the exception type)
        ...
        return error[1] is not None and hasattr(error[1], "__posthog_exception_captured")
    else:
...
def serialize_code_variables(frame, limiter, mask_patterns=None, ignore_patterns=None, max_length=1024):
...
def try_attach_code_variables_to_frames(all_exceptions, exc_info, mask_patterns, ignore_patterns):
    try:
...
def attach_code_variables_to_frames(all_exceptions, exc_info, mask_patterns, ignore_patterns):
