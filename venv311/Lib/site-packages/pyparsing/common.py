...
class pyparsing_common:
    ...
    integer = (
        Word(nums)
        .set_name("integer")
        .set_parse_action(convert_to_integer if PY_310_OR_LATER else lambda t: [int(tt) for tt in t])  # type: ignore[misc]
    )
    ...
    fraction = (
        signed_integer().set_parse_action(
            convert_to_float if PY_310_OR_LATER else lambda t: [float(tt) for tt in t]  # type: ignore[misc]
        )
        + "/"
        + signed_integer().set_parse_action(
            convert_to_float if PY_310_OR_LATER else lambda t: [float(tt) for tt in t]  # type: ignore[misc]
    ...
    real = (
        ...
        .set_parse_action(convert_to_float if PY_310_OR_LATER else lambda t: [float(tt) for tt in t])  # type: ignore[misc]
    )
    ...
    def as_datetime(s, l, t):
        ...
        try:
            return datetime(year, month, day, hour, minute, int(second), int((second % 1) * 1000))
        except ValueError as ve:
            raise ParseException(t, l, f"Invalid date/time: {ve}").with_traceback(ve.__traceback__) from None
