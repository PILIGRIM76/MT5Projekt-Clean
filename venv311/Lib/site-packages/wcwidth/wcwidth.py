...
from .escape_sequences import CURSOR_LEFT_SEQUENCE, CURSOR_RIGHT_SEQUENCE, INDETERMINATE_EFFECT_SEQUENCE, ZERO_WIDTH_PATTERN
from .grapheme import iter_graphemes
from .sgr_state import _SGR_PATTERN, _SGR_STATE_DEFAULT, _sgr_state_is_active, _sgr_state_to_sequence, _sgr_state_update
from .table_ambiguous import AMBIGUOUS_EASTASIAN
from .table_grapheme import EXTENDED_PICTOGRAPHIC, GRAPHEME_REGIONAL_INDICATOR, ISC_CONSONANT
...
_REGIONAL_INDICATOR_SET = frozenset(range(GRAPHEME_REGIONAL_INDICATOR[0][0], GRAPHEME_REGIONAL_INDICATOR[0][1] + 1))
_EMOJI_ZWJ_SET = frozenset(cp for lo, hi in EXTENDED_PICTOGRAPHIC for cp in range(lo, hi + 1)) | _REGIONAL_INDICATOR_SET
...
def wcswidth(
    ...
    while idx < end:
        ...
        if ucs == 0xFE0F and last_measured_idx >= 0:
            ...
            total_width += _bisearch(ord(pwcs[last_measured_idx]), VS16_NARROW_TO_WIDE["9.0.0"])
        ...
        if ucs > 0xFFFF:
            ...
            elif _FITZPATRICK_RANGE[0] <= ucs <= _FITZPATRICK_RANGE[1] and last_measured_ucs in _EMOJI_ZWJ_SET:
                idx += 1
                continue
...
def _width_ignored_codes(text: str, ambiguous_width: int = 1) -> int:
    ...
    """
    return wcswidth(strip_sequences(text).translate(_CONTROL_CHAR_TABLE), ambiguous_width=ambiguous_width)
    ...
    *,
    ...
    if control_codes == "parse" and len(text) > _WIDTH_FAST_PATH_MIN_LEN:
        ...
        if "\b" not in text and "\t" not in text and "\r" not in text:
            ...
            if "\x1b" not in text or (not CURSOR_RIGHT_SEQUENCE.search(text) and not CURSOR_LEFT_SEQUENCE.search(text)):
