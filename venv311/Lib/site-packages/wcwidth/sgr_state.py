...
def _parse_extended_color(params: Iterator[int | tuple[int, ...]], base: int) -> tuple[int, ...] | None:
    """
...
def _sgr_state_update(state: _SGRState, sequence: str) -> _SGRState:
    ...
    for p in params:
        ...
        elif _SGR.FG_BLACK <= p <= _SGR.FG_WHITE or _SGR.FG_BRIGHT_BLACK <= p <= _SGR.FG_BRIGHT_WHITE:
            state = state._replace(foreground=(p,))
        elif _SGR.BG_BLACK <= p <= _SGR.BG_WHITE or _SGR.BG_BRIGHT_BLACK <= p <= _SGR.BG_BRIGHT_WHITE:
            state = state._replace(background=(p,))
