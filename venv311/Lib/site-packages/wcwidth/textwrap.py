...
class SequenceTextWrapper(textwrap.TextWrapper):
    ...
    def _width(self, text: str) -> int:
        ...
        return _width(text, control_codes=self.control_codes, tabsize=self.tabsize, ambiguous_width=self.ambiguous_width)
    ...
    def _wrap_chunks(self, chunks: list[str]) -> list[str]:  # pylint: disable=too-many-branches
        ...
        while chunks:
            ...
            if hyperlink_state is not None:
                open_seq = _make_hyperlink_open(hyperlink_state.url, hyperlink_state.params, hyperlink_state.terminator)
            ...
            if self.drop_whitespace and current_line and stripped_last and not stripped_last.strip():
            ...
            if current_line:
                ...
                no_more_content = (
                    not chunks or self.drop_whitespace and len(chunks) == 1 and not self._strip_sequences(chunks[0]).strip()
                )
                if (
                    ...
                    if new_state is not None:
                        ...
                        if current_hyperlink_id is None:
                            if "id=" in new_state.params:
                                ...
                                current_hyperlink_id = f"id={self._next_hyperlink_id()}:{new_state.params}"
                            else:
                        ...
                        if "id=" not in new_state.params:
                            ...
                            old_open = _make_hyperlink_open(new_state.url, new_state.params, new_state.terminator)
                            new_open = _make_hyperlink_open(new_state.url, current_hyperlink_id, new_state.terminator)
                            line_content = line_content.replace(old_open, new_open, 1)

                        # Update state for next line, using computed id
                        hyperlink_state = _HyperlinkState(new_state.url, current_hyperlink_id, new_state.terminator)
                    else:
                    ...
                    while current_line:
                        ...
                        if last_text.strip() and current_width + placeholder_w <= line_width:
                            line_content = "".join(current_line)
                            new_state = self._track_hyperlink_state(line_content, hyperlink_state)
                            if new_state is not None:
                                line_content += _make_hyperlink_close(new_state.terminator)
    ...
    def _track_hyperlink_state(self, text: str, state: _HyperlinkState | None) -> _HyperlinkState | None:
        """
