...
class okx(Exchange, ImplicitAPI):
            ...
            if hasStopLoss:
                ...
                if stopLossTriggerPriceType is not None:
                    if (
                        ...
                        raise InvalidOrder(
                            self.id + ' createOrder() stop loss trigger price type must be one of "last", "index" or "mark"'
                        )
            ...
            if hasTakeProfit:
                ...
                if takeProfitTriggerPriceType is not None:
                    if (
                        ...
                        raise InvalidOrder(
                            self.id + ' createOrder() take profit trigger price type must be one of "last", "index" or "mark"'
                        )
    ...
    def create_order(self, symbol: str, type: OrderType, side: OrderSide, amount: float, price: Num = None, params={}):
        ...
        if (
            ...
            or (requestOrdType == "move_order_stop")
    ...
    def edit_order_request(self, id: str, symbol, type, side, amount=None, price=None, params={}):
        ...
        if isAlgoOrder:
            if (stopLossTriggerPrice is None) and (takeProfitTriggerPrice is None):
                raise BadRequest(
                    self.id + " editOrder() requires a stopLossPrice or takeProfitPrice parameter for editing an algo order"
                )
        ...
        params = self.omit(
            params, ["clOrdId", "clientOrderId", "takeProfitPrice", "stopLossPrice", "stopLoss", "takeProfit", "postOnly"]
        )
        return self.extend(request, params)
        ...
        if initialMarginPercentage is None:
            ...
            if market["linear"]:
                ...
                initialMarginString = Precise.string_div(
                    Precise.string_div(Precise.string_mul(contractsAbs, contractSizeString), entryPriceString), leverageString
                )
