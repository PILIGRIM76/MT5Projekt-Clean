...
from ccxt.static_dependencies.lighter_client.signer import CreateOrderTxReq, decode_auth, decode_tx_info, load_lighter_library
...
class Exchange(object):
    ...
    def ecdsa(request, secret, algorithm="p256", hash=None, fixed_length=False):
        ...
        if secret.find(b"-----BEGIN EC PRIVATE KEY-----") > -1:
            ...
            key = ecdsa.SigningKey.from_string(base64.b16decode(secret, casefold=True), curve=curve_info[0])
        r_binary, s_binary, v = key.sign_digest_deterministic(
            digest, hashfunc=hash_function, sigencode=ecdsa.util.sigencode_strings_canonize
        )
    ...
    def eddsa(request, secret, curve="ed25519", url_encode=False):
        if isinstance(secret, str):
            secret = Exchange.encode(secret)
        private_key = (
            ed25519.Ed25519PrivateKey.from_private_bytes(secret) if len(secret) == 32 else load_pem_private_key(secret, None)
        )
    ...
    def encode_dydx_tx_for_signing(self, message, memo, chainId, account, authenticators, fee=None):
        if not encode_as_any:
            raise NotSupported(
                self.id + ' requires protobuf to encode messages, please install it with `pip install "protobuf==5.29.5"`'
            )
        if fee is None:
            fee = {
                "amount": [],
        ...
        if authenticators is not None:
            non_critical_extension_options.append(
                encode_as_any(
                    {"typeUrl": "/dydxprotocol.accountplus.TxExtension", "value": {"selected_authenticators": authenticators}}
                )
            )
    ...
    def lighter_sign_create_grouped_orders(self, signer, request):
        ...
        tx_type, tx_info, tx_hash, error = decode_tx_info(
    ...
    def handle_deltas_with_keys(
        self, bookSide: Any, deltas, priceKey: IndexType = 0, amountKey: IndexType = 1, countOrIdKey: IndexType = 2
    ):
        for i in range(0, len(deltas)):
    ...
    def filter_by_since_limit(
        self, array: List[object], since: Int = None, limit: Int = None, key: IndexType = "timestamp", tail=False
    ):
    ...
    def fetch_long_short_ratio_history(
        self, symbol: Str = None, timeframe: Str = None, since: Int = None, limit: Int = None, params={}
    ):
        raise NotSupported(self.id + " fetchLongShortRatioHistory() is not supported yet")

    def fetch_margin_adjustment_history(
        self, symbol: Str = None, type: Str = None, since: Num = None, limit: Num = None, params={}
    ):
        """
    ...
    def feature_value_by_type(
        self, marketType: str, subType: Str, methodName: Str = None, paramName: Str = None, defaultValue: Any = None
    ):
        """
        self method is a very deterministic to help users to know what feature is supported by the exchange
    ...
    def safe_order(self, order: dict, market: Market = None):
        ...
        isTriggerOrSLTpOrder = (
            self.safe_string(order, "triggerPrice") is not None or (self.safe_string(order, "stopLossPrice") is not None)
        ...
        for i in range(0, len(symbols)):
            ...
            if sameTypeOnly and (marketType is not None):
                if market["type"] != marketType:
                    raise BadRequest(
                        self.id + " symbols must be of the same type, either " + marketType + " or " + market["type"] + "."
                    )
    ...
    def select_network_key_from_networks(
        self, currencyCode, networkCode, indexedNetworkEntries, isIndexedByUnifiedNetworkCode=False
    ):
    ...
    def parse_trades_helper(
        self, isWs: bool, trades: List[Any], market: Market = None, since: Int = None, limit: Int = None, params={}
    ):
    ...
    def parse_transfers(
        self, transfers: List[Any], currency: Currency = None, since: Int = None, limit: Int = None, params={}
    ):
    ...
    def create_market_order_with_cost(self, symbol: str, side: OrderSide, cost: float, params={}):
        ...
        if self.has["createMarketOrderWithCost"] or (
            self.has["createMarketBuyOrderWithCost"] and self.has["createMarketSellOrderWithCost"]
        ):
    ...
    def create_market_order_with_cost_ws(self, symbol: str, side: OrderSide, cost: float, params={}):
        ...
        if self.has["createMarketOrderWithCostWs"] or (
            self.has["createMarketBuyOrderWithCostWs"] and self.has["createMarketSellOrderWithCostWs"]
        ):
    ...
    def create_stop_limit_order(
        self, symbol: str, side: OrderSide, amount: float, price: float, triggerPrice: float, params={}
    ):
    ...
    def filter_by_symbols_since_limit(
        self, array, symbols: List[str] = None, since: Int = None, limit: Int = None, tail=False
    ):
    ...
    def fetch_paginated_call_incremental(
        self, method: str, symbol: Str = None, since=None, limit=None, params={}, pageKey=None, maxEntriesPerRequest=None
    ):
        maxCalls = None
        maxCalls, params = self.handle_option_and_params(params, method, "paginationCalls", 10)
        maxRetries = None
    ...
    def remove_repeated_elements_from_array(self, input, fallbackToTimestamp: bool = True):
        ...
        for i in range(0, len(input)):
            entry = input[i]
            uniqValue = (
                self.safe_string_n(entry, ["id", "timestamp", 0]) if fallbackToTimestamp else self.safe_string(entry, "id")
            )
    ...
    def parse_margin_modes(
        self, response: List[object], symbols: List[str] = None, symbolKey: Str = None, marketType: MarketType = None
    ):
    ...
    def parse_margin_modifications(
        self, response: List[object], symbols: Strings = None, symbolKey: Str = None, marketType: MarketType = None
    ):
        marginModifications = []
        for i in range(0, len(response)):
            info = response[i]
