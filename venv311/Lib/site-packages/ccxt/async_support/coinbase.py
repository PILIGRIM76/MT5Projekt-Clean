...
class coinbase(Exchange, ImplicitAPI):
    ...
    async def fetch_closed_orders(self, symbol: Str = None, since: Int = None, limit: Int = None, params={}) -> List[Order]:
        ...
        if paginate:
            return await self.fetch_paginated_call_cursor(
                "fetchClosedOrders", symbol, since, limit, params, "cursor", "cursor", None, 100
            )
    ...
    async def fetch_deposit(self, id: str, code: Str = None, params={}):
        ...
        if accountId is None:
            if code is None:
                raise ArgumentsRequired(
                    self.id + " fetchDeposit() requires an account_id(or accountId) parameter OR a currency code argument"
                )
            accountId = await self.find_account_id(code, params)
            if accountId is None:
    ...
    def sign(self, path, api=[], method="GET", params={}, headers=None, body=None):
        ...
        if signed:
            ...
            if authorization is not None:
                ...
                isV2CloudAPiKey = (
                    len(self.secret) == 88 or self.safe_bool(self.options, "v2CloudAPiKey", False) or self.secret.endswith("=")
                )
