...
class lighter(Exchange, ImplicitAPI):
    ...
    async def pre_load_lighter_library(self, params={}):
        ...
        if privateKeyIsSet and (libraryPath is not None) and (apiKeyIndex is not None) and (accountIndex is not None):
            signer = await self.load_lighter_library(
                libraryPath, self.options["chainId"], self.privateKey, apiKeyIndex, accountIndex
            )
            self.options["signer"] = signer
            return True
        return False
        ...
        if accountIndex is None:
            ...
            if walletAddress is None or walletAddress == "":
                raise ArgumentsRequired(
                    ...
                    + "/"
                    + optionName2
                    + " parameter or walletAddress to fetch accountIndex"
                )
            ...
            if isinstance(subAccounts, list):
                ...
                if account is None:
                    raise ArgumentsRequired(
                        self.id + " " + methodName1 + "() requires an " + optionName1 + " or " + optionName2 + " parameter"
                    )
    ...
    def create_order_request(
        ...
        if hasStopLoss or hasTakeProfit:
            ...
            takeProfitOrderLimitPrice = self.safe_number_n(
    ...
    async def create_order(self, symbol: str, type: OrderType, side: OrderSide, amount: float, price: Num = None, params={}):
        ...
        groupingType, params = self.handle_option_and_params(
            params, "createOrder", "groupingType", 3
