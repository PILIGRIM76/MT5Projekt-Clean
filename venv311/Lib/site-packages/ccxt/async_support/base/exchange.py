...
class Exchange(BaseExchange):
    ...
    async def fetch(self, url, method="GET", headers=None, body=None):
        ...
        if httpProxy:
            ...
            if SocksProxyConnector is None:
                raise NotSupported(
                    self.id
                    + ' - to use SOCKS proxy with ccxt, you need "aiohttp_socks" module that can be installed by "pip install aiohttp_socks"'
                )
            # override session
        ...
        try:
            async with session_method(
                ...
                http_response = self.on_rest_response(
                    http_status_code, http_status_text, url, method, headers, http_response, request_headers, request_body
                )
    ...
    def watch_multiple(self, url, message_hashes, message=None, subscribe_hashes=None, subscription=None):
        ...
        connected = (
            client.connected if client.connected.done() else asyncio.ensure_future(client.connect(self.session, backoff_delay))
        )
    ...
    async def ws_close(self):
        if self.clients:
            await asyncio.wait(
                [asyncio.create_task(client.close()) for client in self.clients.values()], return_when=asyncio.ALL_COMPLETED
            )
    ...
    def decode_proto_msg(self, data):
        if not MessageToDict:
            raise NotSupported(
                self.id + ' requires protobuf to decode messages, please install it with `pip install "protobuf==5.29.5"`'
            )
    ...
    async def watch_ohlcv_for_symbols(
        self, symbolsAndTimeframes: List[List[str]], since: Int = None, limit: Int = None, params={}
    ):
    ...
    async def fetch_open_interest_history(
        self, symbol: str, timeframe: str = "1h", since: Int = None, limit: Int = None, params={}
    ):
