...
from asyncio import TimeoutError, ensure_future, sleep, wait_for
...
from .functions import deep_extend, is_json_encoded_object, iso8601, milliseconds
...
class Client(object):
    ...
    def create_connection(self, session):
        ...
        return session.ws_connect(
            self.url, autoping=False, autoclose=False, headers=self.options.get("headers"), max_msg_size=10485760
        ).__aenter__()
    ...
    async def ping_loop(self):
        ...
        while self.keepAlive and not self.closed():
            ...
            if (self.lastPong + self.keepAlive * self.maxPingPongMisses) < now:
                self.on_error(
                    RequestTimeout("Connection to " + self.url + " timed out due to a ping-pong keepalive missing on time")
                )
