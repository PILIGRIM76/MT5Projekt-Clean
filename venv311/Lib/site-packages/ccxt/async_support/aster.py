...
class aster(Exchange, ImplicitAPI):
    ...
    def hash_message(self, binaryMessage):
        ...
        prefix = self.binary_concat(
            x19, self.encode("Ethereum Signed Message:"), newline, self.encode(self.number_to_string(binaryMessageLength))
        )
                ...
                signingHash = self.hash_message(
                    self.hash(
                        self.eth_abi_encode(
                            ...
                            [self.json(signingPayload), self.walletAddress, signerAddress, nonce],
                        ),
                        "keccak",
                        "binary",
                    )
