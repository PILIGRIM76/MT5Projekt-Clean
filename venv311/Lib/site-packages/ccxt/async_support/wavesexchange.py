...
class wavesexchange(Exchange, ImplicitAPI):
    ...
    def describe(self) -> Any:
        return self.deep_extend(
            super(wavesexchange, self).describe(),
            {
                "id": "wavesexchange",
                "name": "Waves.Exchange",
                ...
                "currencies": {
                    "WX": self.safe_currency_structure(
                        {
    ...
    async def fetch_deposit_address(self, code: str, params={}) -> DepositAddress:
        ...
        if not (code in currencies):
            ...
            raise ExchangeError(
                self.id + " fetchDepositAddress() " + code + " not supported. Currency code must be one of " + ", ".join(codes)
            )
        response = None
        if network is None:
            request: dict = {
            ...
            if not (network in supportedNetworks):
                ...
                raise ExchangeError(
                    ...
                    + " network "
                    + code
                    + " deposit address not supported. Network must be one of "
                    + ", ".join(supportedNetworkKeys)
                )
            if network == "WAVES":
                request: dict = {
    ...
    async def create_order(self, symbol: str, type: OrderType, side: OrderSide, amount: float, price: Num = None, params={}):
        ...
        if (isMarketOrder) and (price is None):
            raise InvalidOrder(
                ...
                + type
                + " orders to determine the max price for buy and the min price for sell"
            )
        timestamp = self.milliseconds()
        defaultExpiryDelta = None
        defaultExpiryDelta, params = self.handle_option_and_params(
            params, "createOrder", "defaultExpiry", self.safe_integer(self.options, "createOrderDefaultExpiry", 2419200000)
        )
        ...
        if matcherFeeAssetId is None:
            raise InsufficientFunds(
                self.id
                + " not enough funds on none of the eligible asset fees: "
                + baseFeeAsset
                + " "
                + floatBaseMatcherFee
                + " or "
        ...
        signature = self.axolotl(
            self.binary_to_base16(self.base58_to_binary(serializedOrder)),
            self.binary_to_base16(self.base58_to_binary(self.secret)),
            "ed25519",
        )
    ...
    async def withdraw(self, code: str, amount: float, address: str, tag: Str = None, params={}) -> Transaction:
        ...
        if code != "WAVES":
            ...
            if not (code in currencies):
                codes = list(currencies.keys())
                raise ExchangeError(
                    self.id + " withdraw() " + code + " not supported. Currency code must be one of " + str(codes)
                )
        await self.load_markets()
        ...
        if code == "WAVES" and not isErc20:
            ...
            if amount <= minimum:
                raise BadRequest(
                    ...
                    + str(amount)
                    + " must be greater than the minimum allowed amount of "
                    + str(minimum)
                )
            # {
