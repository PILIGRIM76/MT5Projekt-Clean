...
class phemex(Exchange, ImplicitAPI):
    ...
    def custom_parse_order_book(
        self, orderbook, symbol, timestamp=None, bidsKey="bids", asksKey="asks", priceKey=0, amountKey=1, market: Market = None
    ):
        result: dict = {
            "symbol": symbol,
            "timestamp": timestamp,
    ...
    async def fetch_order(self, id: str, symbol: Str = None, params={}):
        ...
        if isinstance(data, list):
            ...
            if numOrders < 1:
                if clientOrderId is not None:
                    raise OrderNotFound(
                        self.id + " fetchOrder() " + symbol + " order with clientOrderId " + clientOrderId + " not found"
                    )
                else:
    ...
    async def set_margin_mode(self, marginMode: str, symbol: Str = None, params={}):
        ...
        if self.in_array(market["settle"], ["USDT", "USDC"]):
            ...
            request["leverageRr"] = (
                Precise.string_neg(Precise.string_abs(currentLeverage)) if isCross else Precise.string_abs(currentLeverage)
            )
