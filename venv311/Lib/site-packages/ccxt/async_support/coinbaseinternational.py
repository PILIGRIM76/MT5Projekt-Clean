...
class coinbaseinternational(Exchange, ImplicitAPI):
    ...
    def describe(self) -> Any:
        return self.deep_extend(
            super(coinbaseinternational, self).describe(),
            {
    ...
    async def fetch_ohlcv(
        self, symbol: str, timeframe: str = "1m", since: Int = None, limit: Int = 100, params={}
    ) -> List[list]:
        """
    ...
    async def fetch_funding_rate_history(self, symbol: Str = None, since: Int = None, limit: Int = None, params={}):
        ...
        maxEntriesPerRequest, params = self.handle_option_and_params(
            params, "fetchFundingRateHistory", "maxEntriesPerRequest", 100
        )
        pageKey = "ccxtPageKey"
        if paginate:
    ...
    async def fetch_deposits_withdrawals(
        ...
        maxEntriesPerRequest = None
        maxEntriesPerRequest, params = self.handle_option_and_params(
            params, "fetchDepositsWithdrawals", "maxEntriesPerRequest", 100
        )
        pageKey = "ccxtPageKey"
        if paginate:
            return await self.fetch_paginated_call_incremental(
    ...
    def parse_transaction(self, transaction: dict, currency: Currency = None) -> Transaction:
        ...
        addressTo = self.safe_string_n(
            transaction,
            ["to_address", "to_cb_account", self.safe_string_n(toPorfolio, ["id", "uuid", "name"]), "to_counterparty_id"],
        )
        return {
            "info": transaction,
    ...
    async def edit_order(
        self, id: str, symbol: str, type: OrderType, side: OrderSide, amount: float = None, price: float = None, params={}
    ):
        """
        edit a trade order
    ...
    async def fetch_my_trades(self, symbol: Str = None, since: Int = None, limit: Int = None, params={}):
        ...
        if limit is not None:
            if limit > 100:
                raise BadRequest(
                    self.id + " fetchMyTrades() maximum limit is 100. Consider setting paginate to True to fetch more trades."
                )
            request["result_limit"] = limit
        if since is not None:
            request["time_from"] = self.iso8601(since)
