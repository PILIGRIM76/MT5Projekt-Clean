...
class dydx(Exchange, ImplicitAPI):
    ...
    def sign_dydx_tx(
        self, privateKey: str, message: Any, memo: str, chainId: str, account: Any, authenticators: Any, fee=None
    ) -> str:
    ...
    async def fetch_dydx_account(self):
        ...
        if not self.walletAddress.startswith("dydx"):
            raise ArgumentsRequired(
                self.id + " fetchDydxAccount() requires a valid dydx chain address, starting with dydx, not the l1 address."
            )
        request = {
    ...
    def create_order_request(self, symbol: str, type: OrderType, side: OrderSide, amount: float, price: Num = None, params={}):
        ...
        orderId = self.create_order_id_from_parts(
            self.get_wallet_address(), subaccountId, clientOrderId, orderFlag, marketInfo["clobPairId"]
        )
    ...
    async def cancel_order(self, id: str, symbol: Str = None, params={}) -> Order:
        ...
        if clientOrderId is None:
            raise ArgumentsRequired(
                self.id + " cancelOrder() requires a clientOrderId parameter, cancelling using id is not currently supported."
            )
        idString = str(id)
