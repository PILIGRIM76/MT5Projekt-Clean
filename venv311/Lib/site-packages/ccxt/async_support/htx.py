...
class htx(Exchange, ImplicitAPI):
    ...
    async def fetch_status(self, params={}):
        ...
        enabledForContracts = self.handle_option(
            "fetchStatus", "enableForContracts", False
        )  # temp fix for: https://status-linear-swap.huobigroup.com/api/v2/summary.json
        response = None
    ...
    async def fetch_tickers(self, symbols: Strings = None, params={}) -> Tickers:
        ...
        if not isSpot or isSubTypeRequested:
            if linear:
                ...
                raise NotSupported(
                    self.id + ' fetchTickers() you have to set params["subType"] to either "linear" or "inverse" for contracts'
                )
        else:
        ...
        if marketType == "spot":
            ...
            if market["linear"]:
                ...
                if marginMode == "isolated":
                    ...
                    response = await self.contractPrivatePostLinearSwapApiV3SwapCrossMatchresultsExact(
    ...
    async def fetch_contract_orders(self, symbol: Str = None, since: Int = None, limit: Int = None, params={}):
        ...
        if market["linear"]:
            ...
            if marginMode == "isolated":
                ...
                if trigger:
                    response = await self.contractPrivatePostLinearSwapApiV1SwapCrossTriggerHisorders(
                        self.extend(request, params)
                    )
                elif stopLossTakeProfit:
                    response = await self.contractPrivatePostLinearSwapApiV1SwapCrossTpslHisorders(
                        self.extend(request, params)
                    )
                elif trailing:
                    response = await self.contractPrivatePostLinearSwapApiV1SwapCrossTrackHisorders(
                        self.extend(request, params)
                    )
                else:
    ...
    async def fetch_open_orders(self, symbol: Str = None, since: Int = None, limit: Int = None, params={}) -> List[Order]:
        ...
        if marketType == "spot":
            ...
            if subType == "linear":
                ...
                if marginMode == "isolated":
                    if trigger:
                        response = await self.contractPrivatePostLinearSwapApiV1SwapTriggerOpenorders(
                            self.extend(request, params)
                        )
                    elif stopLossTakeProfit:
                        response = await self.contractPrivatePostLinearSwapApiV1SwapTpslOpenorders(
                            self.extend(request, params)
                        )
                    elif trailing:
                        response = await self.contractPrivatePostLinearSwapApiV1SwapTrackOpenorders(
                            self.extend(request, params)
                        )
                    else:
                    ...
                    if trigger:
                        response = await self.contractPrivatePostLinearSwapApiV1SwapCrossTriggerOpenorders(
                            self.extend(request, params)
                        )
                    elif stopLossTakeProfit:
                        response = await self.contractPrivatePostLinearSwapApiV1SwapCrossTpslOpenorders(
                            self.extend(request, params)
                        )
                    elif trailing:
                        response = await self.contractPrivatePostLinearSwapApiV1SwapCrossTrackOpenorders(
                            self.extend(request, params)
                        )
                    else:
                        response = await self.contractPrivatePostLinearSwapApiV1SwapCrossOpenorders(
                            self.extend(request, params)
                        )
            elif subType == "inverse":
                if marketType == "swap":
    ...
    def parse_order(self, order: dict, market: Market = None) -> Order:
        ...
        clientOrderId = self.safe_string_2(
            order, "client_order_id", "client-or" + "der-id"
    ...
    async def create_spot_order_request(
        ...
        params = self.omit(
            params, ["triggerPrice", "stopPrice", "stop-price", "clientOrderId", "client-order-id", "operator", "timeInForce"]
        )
        ...
        if marketType == "spot":
            ...
            if market["linear"]:
                ...
                if marginMode == "isolated":
                    ...
                    if trigger:
                        response = await self.contractPrivatePostLinearSwapApiV1SwapCrossTriggerCancel(
                            self.extend(request, params)
                        )
                    elif stopLossTakeProfit:
                        response = await self.contractPrivatePostLinearSwapApiV1SwapCrossTpslCancel(
                            self.extend(request, params)
                        )
                    elif trailing:
                        response = await self.contractPrivatePostLinearSwapApiV1SwapCrossTrackCancel(
                            self.extend(request, params)
                        )
                    else:
    ...
    async def cancel_all_orders(self, symbol: Str = None, params={}):
        ...
        if marketType == "spot":
            ...
            if market["linear"]:
                ...
                if marginMode == "isolated":
                    if trigger:
                        response = await self.contractPrivatePostLinearSwapApiV1SwapTriggerCancelall(
                            self.extend(request, params)
                        )
                    elif stopLossTakeProfit:
                        response = await self.contractPrivatePostLinearSwapApiV1SwapTpslCancelall(self.extend(request, params))
                    elif trailing:
                        response = await self.contractPrivatePostLinearSwapApiV1SwapTrackCancelall(
                            self.extend(request, params)
                        )
                    else:
                    ...
                    if trigger:
                        response = await self.contractPrivatePostLinearSwapApiV1SwapCrossTriggerCancelall(
                            self.extend(request, params)
                        )
                    elif stopLossTakeProfit:
                        response = await self.contractPrivatePostLinearSwapApiV1SwapCrossTpslCancelall(
                            self.extend(request, params)
                        )
                    elif trailing:
                        response = await self.contractPrivatePostLinearSwapApiV1SwapCrossTrackCancelall(
                            self.extend(request, params)
                        )
                    else:
                        response = await self.contractPrivatePostLinearSwapApiV1SwapCrossCancelall(
                            self.extend(request, params)
                        )
            elif market["inverse"]:
                if market["swap"]:
    ...
    async def fetch_withdraw_addresses(self, code: str, note=None, networkCode=None, params={}):
        ...
        #
        data = self.safe_value(response, "data", [])
        allAddresses = self.parse_deposit_addresses(
            data, [currency["code"]], False
        )  # cjg: to do remove self weird object or array ambiguity
        addresses = []
    ...
    async def fetch_borrow_interest(
        self, code: Str = None, symbol: Str = None, since: Int = None, limit: Int = None, params={}
    ) -> List[BorrowInterest]:
        """
    ...
    async def fetch_position(self, symbol: str, params={}):
        ...
        if market["linear"]:
            if marginMode == "isolated":
                ...
                response = await self.contractPrivatePostLinearSwapApiV1SwapCrossAccountPositionInfo(
    ...
    async def close_position(self, symbol: str, side: OrderSide = None, params={}) -> Order:
        ...
        if market["inverse"]:  # Coin-M
            ...
            if marginMode == "cross":
                response = await self.contractPrivatePostLinearSwapApiV1SwapCrossLightningClosePosition(
                    self.extend(request, params)
                )
            else:  # isolated
                response = await self.contractPrivatePostLinearSwapApiV1SwapLightningClosePosition(
                    self.extend(request, params)
                )
