...
class kucoin(Exchange, ImplicitAPI):
        ...
        if uta:
            if limit is None:
                raise ArgumentsRequired(
                    self.id + " fetchOrderBook() requires a limit argument for uta, either 20, 50, 100 or FULL"
                )
            request["limit"] = limit
 ...
 ----------------- Exchange Specific Parameters -----------------
    ...
    def create_contract_order_request(
        ...
        params = self.omit(
            params, ["timeInForce", "stopPrice", "triggerPrice", "stopLossPrice", "takeProfitPrice", "reduceOnly", "hedged"]
    ...
    async def cancel_orders(self, ids: List[str], symbol: Str = None, params={}):
        ...
        market = None
        isContractMarket = (
            True  # default to contract market orders if symbol is not provided, uta endpoint requires a symbol to be provided
        )
        if symbol is not None:
            market = self.market(symbol)
    ...
    def sign(self, path, api="public", method="GET", params={}, headers=None, body=None):
        ...
        if isPrivate or isFuturePrivate or isBroker or isEarn or isUtaPrivate:
            ...
            partner = (
                self.safe_value(partner, "future", partner) if isFuturePartner else self.safe_value(partner, "spot", partner)
            )
