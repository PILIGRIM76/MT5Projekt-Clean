...
class bybit(Exchange, ImplicitAPI):
        ...
        if isTrailingOrder:
            ...
            if market["spot"]:
                if triggerDirection is not None:
                    raise NotSupported(
                        self.id + " createOrder() : trigger order does not support triggerDirection for spot markets yet"
                    )
            else:
    ...
    def edit_order_request(
        ...
        params = self.omit(
            params,
            ["stopPrice", "stopLossPrice", "takeProfitPrice", "triggerPrice", "clientOrderId", "stopLoss", "takeProfit"],
        )
        return request
    ...
    async def cancel_orders_for_symbols(self, orders: List[CancellationRequest], params={}):
        ...
        for i in range(0, len(orders)):
            ...
            if (category is not None) and (category != currentCategory):
                raise ExchangeError(
                    self.id + " cancelOrdersForSymbols requires all orders to be of the same category(linear, spot or option))"
                )
    ...
    async def fetch_orders(self, symbol: Str = None, since: Int = None, limit: Int = None, params={}) -> List[Order]:
        ...
        *classic accounts only/ spot not supported* fetches information on multiple orders made by the user *classic accounts only/ spot not supported*
        ...
        if enableUnifiedAccount:
            raise NotSupported(
                self.id
                + " fetchOrders() is not supported after the 5/02 update for UTA accounts, please use fetchOpenOrders, fetchClosedOrders or fetchCanceledOrders"
            )
    ...
    async def fetch_derivatives_open_interest_history(
        self, symbol: str, timeframe="1h", since: Int = None, limit: Int = None, params={}
    ):
        await self.load_markets()
        market = self.market(symbol)
