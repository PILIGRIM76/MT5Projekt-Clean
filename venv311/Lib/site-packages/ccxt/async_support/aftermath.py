...
class aftermath(Exchange, ImplicitAPI):
    ...
    async def withdraw(self, code: str, amount: float, address: str, tag: Str = None, params={}) -> Transaction:
        ...
        #
        return self.extend(
            self.parse_transaction(response, currency),
            {
                "addressFrom": account,
                "amount": amount,
            },
    ...
    def sign_tx_ed25519(self, tx: dict) -> dict:
        ...
        if self.privateKey.find("suiprivkey") >= 0:
            raise NotSupported(
                self.id + " only support hex encoding private key, please transform bech32 encoding private key"
            )
        ...
        suiSignature = self.binary_concat(
            self.base16_to_binary("00"), self.binary_concat(self.base64_to_binary(signature), publicKey)
        )
