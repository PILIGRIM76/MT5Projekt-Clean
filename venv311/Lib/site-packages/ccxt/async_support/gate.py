...
class gate(Exchange, ImplicitAPI):
            ...
            if contract:
                ...
                if trigger is None:
                    ...
                    if priceType < 0 or priceType > 2:
                        raise BadRequest(
                            self.id + " createOrder() price_type should be 0 latest deal price, 1 mark price, 2 index price"
                        )
    ...
    def fetch_order_request(self, id: str, symbol: Str = None, params={}):
        ...
        request, requestParams = (
            self.prepare_request(market, type, query) if contract else self.spot_order_prepare_request(market, trigger, query)
        )
    ...
    def sign(self, path, api=[], method="GET", params={}, headers=None, body=None):
        ...
        if containsSettle and path.endswith(
            "batch_cancel_orders"
        ...
        if isFuture:
            ...
            if market is None:
                ...
                if underlying is None:
                    raise ArgumentsRequired(
                        self.id + " fetchMySettlementHistory() requires a symbol argument or an underlying parameter in params"
                    )
            else:
                marketId = market["id"]
                optionParts = marketId.split("-")
