...
class arkham(Exchange, ImplicitAPI):
    ...
    async def cancel_all_orders(self, symbol: Str = None, params={}):
        ...
        if symbol is not None:
            raise BadRequest(
                self.id
                + " cancelAllOrders() does not support a symbol argument, use cancelOrder() or fetchOpenOrders() instead"
            )
        isTriggerOrder = self.safe_bool(params, "trigger")
        params = self.omit(params, "trigger")
        response = None
    ...
    def create_order_request(self, symbol: str, type: OrderType, side: OrderSide, amount: float, price: Num = None, params={}):
        ...
        params = self.omit(
            params, ["postOnly", "timeInForce", "triggerPrice", "stopLossPrice", "takeProfitPrice", "triggerDirection"]
        )
        if postOnly:
    ...
    async def withdraw(self, code: str, amount: float, address: str, tag: Str = None, params={}) -> Transaction:
        ...
        if not (address in indexedList):
            raise InvalidAddress(
                self.id + " withdraw() requires an address that has been previously added to the whitelisted addresses"
            )
        ...
        if foundWithdrawalObject is None:
            raise InvalidAddress(
                self.id
                + " withdraw() can not find whitelisted withdrawal address for "
                + address
                + " with network "
                + networkCode
            )
    ...
    def sign(self, path, api="public", method="GET", params={}, headers=None, body=None):
        ...
        if access == "private":
            ...
            expires = (
                self.milliseconds() + self.safe_integer(self.options, "requestExpiration", 5000)
            ) * 1000  # need macroseconds
            if method == "POST":
