...
class coinmetro(Exchange, ImplicitAPI):
    ...
    async def fetch_ohlcv(
        ...
        if since is not None:
            ...
            request["from"] = (
                ":from"  # self endpoint doesn't accept empty from and to params(setting them into the value described in the documentation)
            )
    ...
    async def create_order(self, symbol: str, type: OrderType, side: OrderSide, amount: float, price: Num = None, params={}):
        ...
        if type == "limit":
            if (price is None) and (cost is None):
                raise ArgumentsRequired(
                    self.id + " createOrder() requires a price or params.cost argument for a " + type + " order"
                )
        ...
        if side == "sell":
            request = self.handle_create_order_side(
                market["baseId"], market["quoteId"], formattedAmount, precisedCost, request
            )
        elif side == "buy":
            request = self.handle_create_order_side(
                market["quoteId"], market["baseId"], precisedCost, formattedAmount, request
            )
