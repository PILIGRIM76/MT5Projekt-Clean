...
from ccxt.base.errors import ArgumentsRequired, AuthenticationError, BadRequest, ExchangeNotAvailable, InsufficientFunds
...
class p2b(Exchange, ImplicitAPI):
    ...
    async def fetch_ticker(self, symbol: str, params={}) -> Ticker:
        ...
        return self.extend({"timestamp": timestamp, "datetime": self.iso8601(timestamp)}, self.parse_ticker(result, market))
    ...
    async def fetch_closed_orders(self, symbol: Str = None, since: Int = None, limit: Int = None, params={}) -> List[Order]:
        ...
        if (until - since) > 86400000:
            raise BadRequest(
                self.id + ' fetchClosedOrders() the time between since and params["until"] cannot be greater than 24 hours'
            )
        request: dict = {
            "startTime": self.parse_to_int(since / 1000),
            "endTime": self.parse_to_int(until / 1000),
