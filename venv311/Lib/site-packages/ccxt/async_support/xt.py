...
class xt(Exchange, ImplicitAPI):
    ...
    async def fetch_swap_and_future_markets(self, params={}):
        ...
        swapAndFutureMarkets = self.array_concat(
            self.safe_value(markets[0], "result", []), self.safe_value(markets[1], "result", [])
        )
    ...
    def parse_market(self, market: dict) -> Market:
        ...
        if contract:
            ...
            if (
                (state == "ONLINE")
                and (self.safe_value(market, "tradingEnabled"))
                and (self.safe_value(market, "openapiEnabled"))
            ):
                isActive = True
    ...
    async def fetch_bids_asks(self, symbols: List[str] = None, params={}):
        ...
        if subType is not None:
            raise NotSupported(
                self.id + " fetchBidsAsks() is not available for swap and future markets, only spot markets are supported"
            )
    ...
    def parse_order(self, order, market=None):
        ...
        filled = (
            filledQuantity
            if (marketType == "spot")
            else Precise.string_mul(self.number_to_string(filledQuantity), self.number_to_string(market["contractSize"]))
        )
