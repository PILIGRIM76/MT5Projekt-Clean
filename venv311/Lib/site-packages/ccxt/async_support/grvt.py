...
class grvt(Exchange, ImplicitAPI):
    ...
    async def load_account_infos(self):
        ...
        if accountIsUndefined:
            ...
            if length < 1:
                raise ArgumentsRequired(
                    self.id + " loadAccountInfos(): no sub accounts found, you might need to create an api-key in GRVT website"
                )
            if length > 1:
                raise ArgumentsRequired(
                    self.id
    ...
    def create_signed_request(self, request: Any, structureType: str, currencyObj=None, signerAddress: Str = None) -> dict:
        ...
        request["signature"]["signer"] = (
            self.eth_get_address_from_private_key("0x" + privateKeyWithoutZero) if (signerAddress is None) else signerAddress
        )
        return request
