...
class hyperliquid(Exchange, ImplicitAPI):
    ...
    def describe(self) -> Any:
        return self.deep_extend(
            super(hyperliquid, self).describe(),
            {
                "id": "hyperliquid",
                "name": "Hyperliquid",
                "countries": [],
    ...
    async def initialize_client(self):
        try:
            await asyncio.gather(
                *[self.handle_builder_fee_approval(), self.set_ref(), self.is_unified_enabled("fetchBalance", None, False, {})]
    ...
    def create_orders_request(self, orders, params={}) -> dict:
        ...
        if hasClientOrderId:
            for i in range(0, len(orders)):
                ...
                if clientOrderId is None:
                    raise ArgumentsRequired(
                        self.id + " createOrders() all orders must have clientOrderId if at least one has a clientOrderId"
                    )
        params = self.omit(
            params,
            [
        ...
        for i in range(0, len(orders)):
            ...
            if hasStopLoss or hasTakeProfit:
                ...
                takeProfitOrderLimitPrice = self.safe_string_n(
                ...
                if hasTakeProfit:
                    orderObj: dict = self.create_order_request(
                        symbol,
                        takeProfitOrderType,
                        triggerOrderSide,
                        amount,
        ...
        for i in range(0, symbolsLength):
            if dexName is None:
                ...
                if currentDexName != dexName:
                    raise NotSupported(
                        self.id + " " + methodName + " only supports fetching positions for one DEX at a time for HIP3 markets"
                    )
        return dexName
