...
class onetrading(Exchange, ImplicitAPI):
    ...
    def describe(self) -> Any:
        return self.deep_extend(
            super(onetrading, self).describe(),
            {
                "id": "onetrading",
                "name": "One Trading",
        ...
        if since is not None:
            ...
            if to is None:
                raise ArgumentsRequired(
                    self.id
                    + ' fetchOpenOrders() requires a "to" iso8601 string param with the since argument is specified, max range is 100 days'
                )
            request["from"] = self.iso8601(since)
        if limit is not None:
    ...
    async def fetch_my_trades(self, symbol: Str = None, since: Int = None, limit: Int = None, params={}):
        ...
        if since is not None:
            ...
            if to is None:
                raise ArgumentsRequired(
                    self.id
                    + ' fetchMyTrades() requires a "to" iso8601 string param with the since argument is specified, max range is 100 days'
                )
            request["from"] = self.iso8601(since)
        if limit is not None:
            request["max_page_size"] = limit
