...
class ImplicitAPI:
    ...
    public_get_provisioning_currencies = publicGetProvisioningCurrencies = Entry(
        "provisioning/currencies", "public", "GET", {"cost": 1}
    )
    public_get_provisioning_trading_pairs = publicGetProvisioningTradingPairs = Entry(
        "provisioning/trading-pairs", "public", "GET", {"cost": 1}
    )
    public_get_provisioning_limitations_and_fees = publicGetProvisioningLimitationsAndFees = Entry(
        "provisioning/limitations-and-fees", "public", "GET", {"cost": 1}
    )
    public_get_trading_history_pair = publicGetTradingHistoryPair = Entry(
    ...
    private_get_orders_pair_orderid = privateGetOrdersPairOrderId = Entry(
        "orders/{pair}/{orderId}", "private", "GET", {"cost": 1}
    )
    private_get_wallet_withdraw_currency_serial = privateGetWalletWithdrawCurrencySerial = Entry(
        "wallet/withdraw/{currency}/{serial}", "private", "GET", {"cost": 1}
    )
    private_get_wallet_withdraw_currency_id_id = privateGetWalletWithdrawCurrencyIdId = Entry(
        "wallet/withdraw/{currency}/id/{id}", "private", "GET", {"cost": 1}
    )
    private_get_wallet_deposithistory_currency = privateGetWalletDepositHistoryCurrency = Entry(
        "wallet/depositHistory/{currency}", "private", "GET", {"cost": 1}
    )
    private_get_wallet_withdrawhistory_currency = privateGetWalletWithdrawHistoryCurrency = Entry(
    ...
    private_post_wallet_withdraw_currency = privatePostWalletWithdrawCurrency = Entry(
        "wallet/withdraw/{currency}", "private", "POST", {"cost": 10}
    )
    private_put_orders = privatePutOrders = Entry("orders", "private", "PUT", {"cost": 5})
    private_delete_orders_pair_id = privateDeleteOrdersPairId = Entry(
