...
from .exceptions import UndefinedLabel, VisitationError
    ...
    def __repr__(self, top_level=True):
        ...
        ret.append(
            ...
            % (
                ...
                self.start,
                self.end,
                (", children=[%s]" % ", ".join([c.__repr__(top_level=False) for c in self.children])) if self.children else "",
            )
        )
        return "\n".join(ret)
...
class RuleDecoratorMeta(type):
    def __new__(metaclass, name, bases, namespace):
        ...
        if methods:
            ...
            methods.sort(
                key=(lambda x: x.func_code.co_firstlineno) if version_info[0] < 3 else (lambda x: x.__code__.co_firstlineno)
            )
...
class NodeVisitor(object, metaclass=RuleDecoratorMeta):
    ...
    * It makes it impossible to report errors: you'd end up with the "error"
    ...
    def generic_visit(self, node, visited_children):
        ...
        """
        raise NotImplementedError("No visitor method was defined for this expression: %s" % node.expr.as_rule())
    ...
    def _parse_or_match(self, text, pos, method_name):
        ...
        if not self.grammar:
            raise RuntimeError(
                ...
                "`grammar` attribute, and try again.".format(cls=self.__class__.__name__, method=method_name)
            )
