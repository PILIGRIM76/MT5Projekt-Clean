...
from .exceptions import IncompleteParseError, ParseError
...
class Compound(Expression):
    ...
    def __eq__(self, other):
        return isinstance(other, self.__class__) and self.name == other.name and self.members == other.members
...
class Optional(Compound):
    ...
    def _uncached_match(self, text, pos, cache, error):
        node = self.members[0].match_core(text, pos, cache, error)
        return Node(self, text, pos, pos) if node is None else Node(self, text, pos, node.end, children=[node])
