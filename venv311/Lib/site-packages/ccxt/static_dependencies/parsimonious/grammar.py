...
class BootstrappingGrammar(Grammar):
    ...
    def _expressions_from_rules(self, rule_syntax, custom_rules):
        ...
        spaceless_literal = Regex(r'u?r?"[^"\\]*(?:\\.[^"\\]*)*"', ignore_case=True, dot_all=True, name="spaceless_literal")
        literal = Sequence(spaceless_literal, _, name="literal")
        regex = Sequence(Literal("~"), literal, Regex("[ilmsuxa]*", ignore_case=True), _, name="regex")
...
class RuleVisitor(NodeVisitor):
    ...
    def _resolve_refs(self, rule_map, expr, done):
        ...
        if isinstance(expr, LazyReference):
            ...
            if getattr(expr, "members", ()) and expr not in done:
                ...
                expr.members = tuple(self._resolve_refs(rule_map, member, done) for member in expr.members)
            return expr
    ...
    def visit_rules(self, node, rules_list):
        ...
        rule_map = OrderedDict((expr.name, self._resolve_refs(rule_map, expr, done)) for expr in rule_map.values())
        ...
        return rule_map, (rule_map[rules[0].name] if isinstance(rules, list) and rules else None)
