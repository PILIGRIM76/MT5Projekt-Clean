...
def serializer_for_type(cairo_type: CairoType) -> CairoDataSerializer:
    ...
    if isinstance(cairo_type, StructType):
        ...
        return StructSerializer(
            OrderedDict((name, serializer_for_type(member_type)) for name, member_type in cairo_type.types.items())
        )
    ...
    if isinstance(cairo_type, TupleType):
        return TupleSerializer([serializer_for_type(member) for member in cairo_type.types])
    ...
    if isinstance(cairo_type, EnumType):
        return EnumSerializer(
            OrderedDict((name, serializer_for_type(variant_type)) for name, variant_type in cairo_type.variants.items())
        )
...
def serializer_for_payload(payload: Dict[str, CairoType]) -> PayloadSerializer:
    ...
    """
    return PayloadSerializer(OrderedDict((name, serializer_for_type(cairo_type)) for name, cairo_type in payload.items()))
...
def serializer_for_outputs(payload: List[CairoType]) -> OutputSerializer:
    ...
    """
    return OutputSerializer(serializers=[serializer_for_type(cairo_type) for cairo_type in payload])
