...
class UintSerializer(CairoDataSerializer[Union[int, Uint256Dict], int]):
    ...
    @staticmethod
    def _serialize_from_int(value: int, context: SerializationContext, bits: int) -> Generator[int, None, None]:
        if bits < 256:
    ...
    def _serialize_from_dict(self, context: SerializationContext, value: Uint256Dict) -> Generator[int, None, None]:
        with context.push_entity("low"):
    ...
    def _ensure_valid_uint(value: int, context: Context, bits: int):
        ...
        """
        context.ensure_valid_value(0 <= value < 2**bits, "expected value in range [0;2**" + str(bits) + ")")
