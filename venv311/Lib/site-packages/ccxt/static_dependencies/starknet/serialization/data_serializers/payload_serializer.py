...
class PayloadSerializer(CairoDataSerializer[Dict, TupleDataclass]):
    ...
    def __post_init__(self, input_serializers):
        ...
        self.serializers = _OrderedDict(
            (key, serializer) for key, serializer in input_serializers.items() if not self._is_len_arg(key, input_serializers)
        )

    def deserialize_with_context(self, context: DeserializationContext) -> TupleDataclass:
        as_dictionary = deserialize_to_dict(self.serializers, context)
        return TupleDataclass.from_dict(as_dictionary)

    def serialize_with_context(self, context: SerializationContext, value: Dict) -> Generator[int, None, None]:
    ...
    def _is_len_arg(arg_name: str, serializers: Dict[str, CairoDataSerializer]) -> bool:
        return (
            ...
            and isinstance(serializers.get(arg_name[:-SIZE_SUFFIX_LEN]), ArraySerializer)
        )
