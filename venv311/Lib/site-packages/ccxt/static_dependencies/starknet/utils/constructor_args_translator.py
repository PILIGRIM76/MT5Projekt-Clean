...
def translate_constructor_args(
    ...
    serializer = _get_constructor_serializer_v1(abi) if cairo_version == 1 else _get_constructor_serializer_v0(abi)

    if serializer is None or len(serializer.inputs_serializer.serializers) == 0:
        return []

    if not constructor_args:
        raise ValueError("Provided contract has a constructor and no arguments were provided.")

    args, kwargs = ([], constructor_args) if isinstance(constructor_args, dict) else (constructor_args, {})
