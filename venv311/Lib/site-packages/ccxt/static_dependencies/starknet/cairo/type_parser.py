...
class TypeParser:
    ...
    def __init__(self, defined_types: Dict[str, StructType]):
        ...
        for name, struct in defined_types.items():
            if name != struct.name:
                raise ValueError(f"Keys must match name of type, '{name}' != '{struct.name}'.")
    ...
    def _transform_cairo_lang_type(self, cairo_type: cairo_lang_types.CairoType) -> CairoType:
        """
        ...
        if isinstance(cairo_type, cairo_lang_types.TypeTuple):
            ...
            return TupleType([self._transform_cairo_lang_type(member.typ) for member in cairo_type.members])
        ...
        raise RuntimeError(f"Received unknown type '{cairo_type}' from parser.")  # pragma: no cover
