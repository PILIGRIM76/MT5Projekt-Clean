...
def _maybe_get_callers_frame(cls: type, stacklevel: int = 1) -> Optional[types.FrameType]:
...
def dataclass(
    ...
    *,
    ...
    Point(x=0.0, y=0.0)
    """
    dc = dataclasses.dataclass(repr=repr, eq=eq, order=order, unsafe_hash=unsafe_hash, frozen=frozen)

    def decorator(cls: Type[_U], stacklevel: int = 1) -> Type[_U]:
        return add_schema(dc(cls), base_schema, cls_frame=cls_frame, stacklevel=stacklevel + 1)
...
def _internal_class_schema(
    ...
    attributes = {k: v for k, v in inspect.getmembers(clazz) if hasattr(v, "__marshmallow_hook__") or k in MEMBERS_WHITELIST}
    ...
    if sys.version_info >= (3, 9):
        ...
        type_hints = get_type_hints(clazz, globalns=schema_ctx.globalns, localns=schema_ctx.localns)
    attributes.update(
        (
            field.name,
            _field_for_schema(
...
def _field_by_type(
    ...
    return (base_schema and base_schema.TYPE_MAPPING.get(typ)) or marshmallow.Schema.TYPE_MAPPING.get(typ)
    ...
    **metadata: Any,
    ...
    if origin:
        ...
        if origin in (collections.abc.Sequence, Sequence) or (
            origin in (tuple, Tuple) and len(arguments) == 2 and arguments[1] is Ellipsis
        ):
        ...
        if origin in (set, Set):
            ...
            child_type = _field_for_schema(arguments[0], base_schema=base_schema)
            return collection_field.Set(cls_or_instance=child_type, frozen=False, **metadata)
        if origin in (frozenset, FrozenSet):
            from . import collection_field

            child_type = _field_for_schema(arguments[0], base_schema=base_schema)
            return collection_field.Set(cls_or_instance=child_type, frozen=True, **metadata)
        if origin in (tuple, Tuple):
            children = tuple(_field_for_schema(arg, base_schema=base_schema) for arg in arguments)
            tuple_type = cast(
                Type[marshmallow.fields.Tuple],
                type_mapping.get(Tuple, marshmallow.fields.Tuple),  # type: ignore[call-overload]
            )
    ...
    if origin and origin is Annotated:
        ...
        if marshmallow_annotations:
            if len(marshmallow_annotations) > 1:
                warnings.warn("Multiple marshmallow Field annotations found. Using the last one.")
...
def _field_for_schema(
    ...
    if typing_inspect.is_literal_type(typ):
        ...
        return marshmallow.fields.Raw(
            validate=(
                marshmallow.validate.Equal(arguments[0]) if len(arguments) == 1 else marshmallow.validate.OneOf(arguments)
            ),
            **metadata,
        )
...
def _base_schema(clazz: type, base_schema: Optional[Type[marshmallow.Schema]] = None) -> Type[marshmallow.Schema]:
    """
