...
class StrategyRegistry(BaseRegistry):
    ...
    def register_strategy(self, lookup: Lookup, registration: StrategyRegistration, label: str = None) -> None:
...
def get_tuple_strategy(abi_type: ABIType, registry: StrategyRegistry) -> st.SearchStrategy:
    component_strategies = [registry.get_strategy(comp_abi_type.to_type_str()) for comp_abi_type in abi_type.components]
