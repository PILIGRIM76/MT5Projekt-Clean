...
def to_normalized_address(value: Union[AnyAddress, str, bytes]) -> HexAddress:
    ...
    if is_address(hex_address):
        ...
        raise ValueError(f"Unknown format {repr(value)}, attempted to normalize to " f"{repr(hex_address)}")
...
def to_checksum_address(value: Union[AnyAddress, str, bytes]) -> ChecksumAddress:
    ...
    checksum_address = add_0x_prefix(
        HexStr("".join((norm_address[i].upper() if int(address_hash[i], 16) > 7 else norm_address[i]) for i in range(2, 42)))
    )
...
def _is_checksum_formatted(value: Any) -> bool:
    ...
    return not unprefixed_value.islower() and not unprefixed_value.isupper() and not unprefixed_value.isnumeric()
