...
def to_hex(
    ...
    if isinstance(primitive, (bytes, bytearray)):
        ...
        raise TypeError("Unsupported type: The primitive argument must be one of: bytes," "bytearray, int or bool and not str")

    if is_integer(primitive):
        return HexStr(hex(cast(int, primitive)))

    raise TypeError(f"Unsupported type: '{repr(type(primitive))}'. Must be one of: bool, str, " "bytes, bytearray or int.")
...
def to_int(
    ...
    if hexstr is not None:
        ...
        raise TypeError("Invalid type. Expected one of int/bool/str/bytes/bytearray. Got " f"{type(primitive)}")
...
def to_bytes(
    ...
    raise TypeError("expected a bool, int, byte or bytearray in first arg, " "or keyword of hexstr or text")
...
def hexstr_if_str(to_type: Callable[..., T], hexstr_or_primitive: Union[bytes, int, str]) -> T:
    ...
    if isinstance(hexstr_or_primitive, str):
        if remove_0x_prefix(HexStr(hexstr_or_primitive)) and not is_hexstr(hexstr_or_primitive):
            raise ValueError("when sending a str, it must be a hex string. " f"Got: {repr(hexstr_or_primitive)}")
