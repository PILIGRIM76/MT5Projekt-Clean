...
def apply_formatter_at_index(
    ...
    if at_index + 1 > len(value):
        raise IndexError(f"Not enough values in iterable to apply formatter. Got: {len(value)}. " f"Need: {at_index + 1}")
...
def combine_argument_formatters(*formatters: List[Callable[..., Any]]) -> Formatters:
    ...
    return compose(*(_formatter_at_index(formatter, index) for index, formatter in enumerate(formatters)))  # type: ignore
...
def apply_formatters_to_sequence(formatters: List[Any], sequence: List[Any]) -> Generator[List[Any], None, None]:
    if len(formatters) > len(sequence):
        raise IndexError(f"Too many formatters for sequence: {len(formatters)} formatters for " f"{repr(sequence)}")
    elif len(formatters) < len(sequence):
        raise IndexError(f"Too few formatters for sequence: {len(formatters)} formatters for " f"{repr(sequence)}")
    else:
    ...
    for key, item in value.items():
        if key in formatters:
            try:
                ...
                new_error_message = f"Could not format invalid value {repr(item)} as field {repr(key)}"
                raise ValueError(new_error_message) from exc
            except TypeError as exc:
                new_error_message = f"Could not format invalid type {repr(item)} as field {repr(key)}"
...
def apply_key_map(key_mappings: Dict[Any, Any], value: Dict[Any, Any]) -> Generator[Tuple[Any, Any], None, None]:
    key_conflicts = (
        set(value.keys()).difference(key_mappings.keys()).intersection(v for k, v in key_mappings.items() if v in value)
    )
    if key_conflicts:
        raise KeyError(f"Could not apply key map due to conflicting key(s): {key_conflicts}")
