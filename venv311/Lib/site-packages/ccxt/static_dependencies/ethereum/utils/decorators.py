...
class combomethod:
    ...
    def __get__(self, obj: Optional[T] = None, objtype: Optional[Type[T]] = None) -> Callable[..., Any]:
...
def _assert_one_val(*args: T, **kwargs: T) -> None:
    if not _has_one_val(*args, **kwargs):
        raise TypeError(
            "Exactly one of the passed values can be specified. " f"Instead, values were: {repr(args)}, {repr(kwargs)}"
        )
...
def _validate_supported_kwarg(kwargs: Any) -> None:
    if next(iter(kwargs)) not in ["primitive", "hexstr", "text"]:
        raise TypeError("Kwarg must be 'primitive', 'hexstr', or 'text'. " f"Instead, kwarg was: {repr((next(iter(kwargs))))}")
        ...
        def wrapped(*args: Any, **kwargs: Any) -> T:
            try:
                ...
                try:
                    ...
                    raise TypeError(f"could not look up new exception to use for {repr(err)}") from err
