...
from ccxt.base.errors import BadRequest, ExchangeError, OrderNotFound
...
class hibachi(Exchange, ImplicitAPI):
    ...
    def order_message(
        self, market, nonce: float, feeRate: float, type: OrderType, side: OrderSide, amount: float, price: Num = None
    ):
        sideInternal = 0
        if side == "sell":
            sideInternal = 0
        ...
        if type == "limit":
            ...
            priceInternal = Precise.string_div(
                Precise.string_div(Precise.string_mul(Precise.string_mul(priceStr, priceFactor), settlement), underlying),
                one,
                0,
            )
    ...
    def create_order_request(
        self, nonce: float, symbol: str, type: OrderType, side: OrderSide, amount: float, price: Num = None, params={}
    ):
        market = self.market(symbol)
        feeRate = max(
            self.safe_number(market, "taker", self.safe_number(self.options, "defaultTakerFee", 0.00045)),
            self.safe_number(market, "maker", self.safe_number(self.options, "defaultMakerFee", 0.00015)),
        )
        sideInternal = ""
        if side == "sell":
            sideInternal = "ASK"
