...
class backpack(ccxt.async_support.backpack):
    ...
    async def watch_ohlcv_for_symbols(
        ...
        if symbolsLength == 0 or not isinstance(symbolsAndTimeframes[0], list):
            raise ArgumentsRequired(
                self.id + " watchOHLCVForSymbols() requires a an array of symbols and timeframes, like  ['ETH/USDC', '1m']"
            )
        await self.load_markets()
        topics = []
        messageHashes = []
    ...
    async def un_watch_ohlcv_for_symbols(self, symbolsAndTimeframes: List[List[str]], params={}) -> Any:
        ...
        if symbolsLength == 0 or not isinstance(symbolsAndTimeframes[0], list):
            raise ArgumentsRequired(
                self.id + " unWatchOHLCVForSymbols() requires a an array of symbols and timeframes, like  ['ETH/USDC', '1m']"
            )
        await self.load_markets()
        topics = []
        messageHashes = []
    ...
    def handle_message(self, client: Client, message):
        ...
        if event == "ticker":
            ...
            event == "positionAdjusted" or event == "positionOpened" or event == "positionClosed" or event == "positionUpdated"
        ):
