...
class krakenfutures(ccxt.async_support.krakenfutures):
            ...
            if (previousOrder is None) or (reason == "edited_by_user"):
                ...
                if previousOrder["filled"] is not None:
                    ...
                    if previousOrder["amount"] is not None:
                        previousOrder["remaining"] = Precise.string_sub(
                            self.number_to_string(previousOrder["amount"]), stringOrderFilled
                        )
    ...
    async def watch_multi_helper(
        self, unifiedName: str, channelName: str, symbols: Strings = None, subscriptionArgs=None, params={}
    ):
        await self.load_markets()
