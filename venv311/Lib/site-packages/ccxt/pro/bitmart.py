...
class bitmart(ccxt.async_support.bitmart):
    ...
    async def un_watch_orders(self, symbol: Str = None, params={}) -> Any:
        ...
        if symbol is not None:
            ...
            if market["swap"]:
                raise NotSupported(
                    self.id + " unWatchOrders() does not support a symbol for swap markets, unWatch from all markets only"
                )
            messageHash += "::" + symbol
        type = "spot"
        type, params = self.handle_market_type_and_params("watchOrders", market, params)
    ...
    async def un_watch_positions(self, symbols: Strings = None, params={}) -> Any:
        ...
        if symbols is not None:
            ...
            if length > 0:
                raise NotSupported(
                    self.id + " unWatchPositions() does not support a list of symbols, unWatch from all markets only"
                )
        await self.load_markets()
        request: dict = {
            "action": "unsubscribe",
