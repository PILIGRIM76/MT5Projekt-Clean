...
class xt(ccxt.async_support.xt):
    ...
    async def subscribe(
        self, name: str, access: str, methodName: str, market: Market = None, symbols: List[str] = None, params={}
    ):
        """
        @ignore
    ...
    async def un_watch_tickers(self, symbols: Strings = None, params={}) -> Tickers:
        ...
        if symbols is not None:
            raise NotSupported(
                self.id + " unWatchTickers() does not support symbols argument, unsubscribtion is for all tickers at once only"
            )
        messageHash = "unsubscribe::" + name
