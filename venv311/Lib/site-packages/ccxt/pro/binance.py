...
from ccxt.base.errors import ArgumentsRequired, BadRequest, ChecksumError, NotSupported
...
class binance(ccxt.async_support.binance):
    ...
    async def watch_order_book_for_symbols(self, symbols: List[str], limit: Int = None, params={}) -> OrderBook:
        ...
        url = (
            self.get_ws_url(type, self.get_future_ws_category(name)) + "/" + self.stream(type, streamHash, messageHashesLength)
        )
        requestId = self.request_id(url)
        request: dict = {
            "method": "SUBSCRIBE",
    ...
    async def watch_multi_ticker_helper(
        self, methodName, channelName: str, symbols: Strings = None, params={}, isUnsubscribe: bool = False
    ):
        await self.load_markets()
        symbols = self.market_symbols(symbols, None, True, False, True)
    ...
    async def fetch_my_trades_ws(self, symbol: Str = None, since: Int = None, limit: Int = None, params={}) -> List[Trade]:
        ...
        if fromId is not None and since is not None:
            raise BadRequest(
                self.id + " fetchMyTradesWs does not support fetching by both fromId and since parameters at the same time"
            )
        message: dict = {
            "id": messageHash,
            "method": "myTrades",
