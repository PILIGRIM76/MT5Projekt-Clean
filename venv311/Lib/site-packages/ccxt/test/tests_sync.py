...
from tests_helpers import (  # noqa: F401
    AuthenticationError,
...
class testMainClass:
    ...
    def test_method(self, method_name, exchange, args, is_public):
        ...
        if not is_load_markets and (
            len(self.only_specific_tests) > 0 and not exchange.in_array(method_name, self.only_specific_tests)
        ):
    ...
    def check_constructor(self, exchange):
        ...
        if exchange.id == "binance":
            ...
            assert (
                "lending/union/account" in exchange.api["sapi"]["get"]
    ...
    def assert_new_and_stored_output_inner(
        ...
        if (isinstance(stored_output, dict)) and (isinstance(new_output, dict)):
            ...
            if strict_type_check and (self.lang != "C#"):
                ...
                self.assert_static_error(
                    sanitized_new_output == sanitized_stored_output, message_error, stored_output, new_output, asserting_key
                )
            else:
    ...
    def assert_new_and_stored_output(
        ...
        try:
            ...
            if self.info:
                error_message = (
                    self.var_to_string(new_output) + "(calculated)" + " != " + self.var_to_string(stored_output) + "(stored)"
                )
    ...
    def test_binance(self):
        ...
        assert client_order_id_inverse.startswith(inverse_swap_id), (
            "binance - swap clientOrderIdInverse: " + client_order_id_inverse + " does not start with swapId" + inverse_swap_id
        )
        ...
        try:
            ...
            assert client_algo_id_swap.startswith(swap_algo_id_string), (
                "binance - swap clientOrderId: " + client_algo_id_swap + " does not start with swapId" + swap_algo_id_string
            )
        except Exception as e:
