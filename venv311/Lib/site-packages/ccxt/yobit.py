...
class yobit(Exchange, ImplicitAPI):
    ...
    def fetch_tickers(self, symbols: Strings = None, params={}) -> Tickers:
        ...
        if symbols is None and not allSymbols:
            raise ArgumentsRequired(
                self.id
                + ' fetchTickers() requires "symbols" argument or use `params["all"] = True` to send multiple requests for all markets'
            )
        self.load_markets()
        promises = []
        ...
        if allSymbols:
            ...
            if actualLength > maxLength:
                raise ArgumentsRequired(
                    self.id
                    + " fetchTickers() is being requested for "
                    + str(idsLength)
                    + " markets(which has an URL length of "
                    + str(actualLength)
                    + " characters), but it exceedes max URL length("
                    + str(maxLength)
                    + "), please pass limisted symbols array to fetchTickers to fit in one request"
                )
    ...
    def withdraw(self, code: str, amount: float, address: str, tag: Str = None, params={}) -> Transaction:
        ...
        if tag is not None:
            raise ExchangeError(
                self.id
                + " withdraw() does not support the tag argument yet due to a lack of docs on withdrawing with tag/memo on behalf of the exchange."
            )
