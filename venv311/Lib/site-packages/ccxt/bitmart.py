...
class bitmart(Exchange, ImplicitAPI):
    ...
    def fetch_ticker(self, symbol: str, params={}) -> Ticker:
        ...
        if market["swap"]:
            ...
            raise NotSupported(
                ...
                + market["type"]
                + " markets, only spot and swap markets are accepted"
            )
    ...
    def fetch_tickers(self, symbols: Strings = None, params={}) -> Tickers:
        ...
        if type == "spot":
            ...
            raise NotSupported(
                self.id + " fetchTickers() does not support " + type + " markets, only spot and swap markets are accepted"
            )
        tickers = []
        if type == "spot":
    ...
    def fetch_trades(self, symbol: str, since: Int = None, limit: Int = None, params={}) -> List[Trade]:
        ...
        if not market["spot"]:
            raise NotSupported(
                self.id + " fetchTrades() does not support " + market["type"] + " orders, only spot orders are accepted"
            )
        request: dict = {
            "symbol": market["id"],
        }
    ...
    def fetch_my_trades(self, symbol: Str = None, since: Int = None, limit: Int = None, params={}):
        ...
        if type == "spot":
            ...
            raise NotSupported(
                self.id + " fetchMyTrades() does not support " + type + " orders, only spot and swap orders are accepted"
            )
        #
        # spot
        #
    ...
    def fetch_balance(self, params={}) -> Balances:
        ...
        if marketType == "spot":
            ...
            raise NotSupported(
                ...
                + marketType
                + " markets, only spot, swap and account and margin markets are accepted"
            )
        #
        # spot
        #
    ...
    def fetch_trading_fee(self, symbol: str, params={}) -> TradingFeeInterface:
        ...
        if not market["spot"]:
            raise NotSupported(
                self.id + " fetchTradingFee() does not support " + market["type"] + " orders, only spot orders are accepted"
            )
        request: dict = {
            "symbol": market["id"],
        }
    ...
    def create_swap_order_request(
        ...
        trailingTriggerPrice = self.safe_string_2(
            params, "trailingTriggerPrice", "activation_price", self.number_to_string(price)
        )
    ...
    def cancel_orders(self, ids: List[str], symbol: Str = None, params={}) -> List[Order]:
        ...
        if not market["spot"]:
            raise NotSupported(
                self.id + " cancelOrders() does not support " + market["type"] + " orders, only spot orders are accepted"
            )
    ...
    def fetch_orders_by_status(self, status, symbol: Str = None, since: Int = None, limit: Int = None, params={}):
        ...
        if not market["spot"]:
            raise NotSupported(
                self.id
                + " fetchOrdersByStatus() does not support "
                + market["type"]
    ...
    def fetch_open_orders(self, symbol: Str = None, since: Int = None, limit: Int = None, params={}) -> List[Order]:
        ...
        if type == "spot":
            ...
            raise NotSupported(
                self.id + " fetchOpenOrders() does not support " + type + " orders, only spot and swap orders are accepted"
            )
        #
        # spot
        #
    ...
    def edit_order(
        ...
        if not market["swap"]:
            raise NotSupported(
                self.id + " editOrder() does not support " + market["type"] + " markets, only swap markets are supported"
            )
        stopLossPrice = self.safe_string(params, "stopLossPrice")
        takeProfitPrice = self.safe_string(params, "takeProfitPrice")
        triggerPrice = self.safe_string_n(params, ["triggerPrice", "stopPrice", "trigger_price"])
    ...
    def fetch_transactions_request(
        self, flowType: Int = None, symbol: Str = None, since: Int = None, limit: Int = None, params={}
    ):
        request: dict = {}
