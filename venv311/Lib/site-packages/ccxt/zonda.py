...
class zonda(Exchange, ImplicitAPI):
    ...
    def fetch_ticker(self, symbol: str, params={}):
        ...
        if fetchTickerMethod == method:
            ...
            raise BadRequest(
                self.id
                + ' fetchTicker params["method"] must be "v1_01PublicGetTradingTickerSymbol" or "v1_01PublicGetTradingStatsSymbol"'
            )
    ...
    def fetch_tickers(self, symbols: Strings = None, params={}) -> Tickers:
        ...
        if fetchTickersMethod == method:
            ...
            raise BadRequest(
                self.id
                + ' fetchTickers params["method"] must be "v1_01PublicGetTradingTicker" or "v1_01PublicGetTradingStats"'
            )
    ...
    def create_order(self, symbol: str, type: OrderType, side: OrderSide, amount: float, price: Num = None, params={}):
        ...
        if isStopOrder:
            if not isStopLossPrice:
                raise ExchangeError(
                    self.id + " createOrder() zonda requires `triggerPrice` parameter for stop-limit or stop-market orders"
                )
