...
class deepcoin(Exchange, ImplicitAPI):
    ...
    def edit_order(
        ...
        if isTPSL:
            ...
            if stopLossPrice is not None:
                request["slTriggerPx"] = (
                    self.price_to_precision(symbol, stopLossPrice) if symbol else self.number_to_string(stopLossPrice)
                )
            if takeProfitPrice is not None:
                request["tpTriggerPx"] = (
                    self.price_to_precision(symbol, takeProfitPrice) if symbol else self.number_to_string(takeProfitPrice)
                )
