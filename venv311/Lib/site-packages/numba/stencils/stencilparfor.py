...
class StencilPass(object):
            ...
            for i, stmt in reversed(list(enumerate(block.body))):
                ...
                if (
                    ...
                    stencil_ir, rt, arg_to_arr_dict = get_stencil_ir(
                        sf, self.typingctx, arg_typemap, block.scope, block.loc, input_dict, self.typemap, self.calltypes
                    )
    ...
    def _mk_stencil_parfor(
        ...
        if out_arr is None:
            ...
            def handle_border(
                slice_fn_ty, dim, scope, loc, slice_func_var, stmts, border_inds, border_tuple_items, other_arg, other_first
            ):
                ...
                slice_callexpr = ir.Expr.call(
                    func=slice_func_var, args=(other_arg, si_var) if other_first else (si_var, other_arg), kws=(), loc=loc
                )
                self.calltypes[slice_callexpr] = sig
                # ---- Generate slice var
                ...
                self.calltypes[setitem_call] = signature(
                    types.none, self.typemap[out_arr.name], self.typemap[border_ind_var.name], self.typemap[out_arr.name].dtype
                )
            ...
            for dim in range(in_arr_typ.ndim):
                ...
                handle_border(
                    slice_fn_ty, dim, scope, loc, slice_func_var, stmts, start_inds, start_tuple_items, zero_index_var, True
                )
                handle_border(
                    slice_fn_ty,
                    dim,
            ...
            if "cval" in stencil_func.options:  # do out[:] = cval
                ...
                sig = signature(
                    types.none, self.typemap[out_arr.name], self.typemap[slice_inst_var.name], self.typemap[out_arr.name].dtype
                )
        ...
        for label, block in stencil_blocks.items():
            ...
            for stmt in block.body:
                ...
                if (
                    ...
                    (isinstance(stmt, ir.SetItem) or isinstance(stmt, ir.StaticSetItem)) and stmt.target.name in in_arg_names
                ):
                    raise NumbaValueError("Assignments to arrays passed to " "stencil kernels is not allowed.")
                ...
                if (
                    ...
                    if need_to_calc_kernel:
                        ...
                        if isinstance(index_list, ir.Var) or any([not isinstance(v, int) for v in index_list]):
                            raise NumbaValueError("Variable stencil index " "only possible with known " "neighborhood")
