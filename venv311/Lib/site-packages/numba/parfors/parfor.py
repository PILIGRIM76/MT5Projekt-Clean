...
from numba.core import analysis, config, errors, ir, ir_utils, postproc, rewrites, typeinfer, types, typing, utils
from numba.core.analysis import compute_cfg_from_blocks, compute_dead_maps, compute_live_map, compute_use_defs
...
class PreParforPass(object):
    ...
    def __init__(self, func_ir, typemap, calltypes, typingctx, targetctx, options, swapped=None, replace_functions_map=None):
        if swapped is None:
            swapped = {}
        self.func_ir = func_ir
    ...
    def run(self):
        ...
        if self.options.stencil:
            stencil_pass = StencilPass(
                self.func_ir, self.typemap, self.calltypes, self.array_analysis, self.typingctx, self.targetctx, self.flags
            )
            stencil_pass.run()
        ...
        if op in npydecl.supported_array_operators:
            ...
            if len(arg_vars) == 2:
                ...
                if op == operator.truediv:
                    func_typ, ir_expr = _gen_np_divide(arg_vars[0], arg_vars[1], out_ir, typemap, typingctx)
            else:
...
def _gen_np_divide(arg1, arg2, out_ir, typemap, typingctx):
    ...
    func_typ = func_var_typ.get_call_type(typingctx, [typemap[arg1.name], typemap[arg2.name]], {})
    out_ir.extend([g_np_assign, attr_assign])
    return func_typ, div_call


def _gen_arrayexpr_getitem(
    equiv_set, var, parfor_index_tuple_var, all_parfor_indices, el_typ, calltypes, typingctx, typemap, init_block, out_ir
):
        ...
        for stmt in reversed(parfor.loop_body[label].body):
            ...
            if isinstance(stmt, Parfor):
                ...
                get_parfor_reductions(
                    func_ir, stmt, parfor_params, calltypes, reductions, reduce_varnames, None, param_nodes, var_to_param
                )
        ...
        if isinstance(rhs, ir.Expr):
            ...
            if name in in_vars:
                ...
                if i + 1 < len(nodes) and (
                    (not isinstance(nodes[i + 1], ir.Assign)) or nodes[i + 1].target.unversioned_name != unversioned_name
                ):
                    foundj = None
...
def has_cross_iter_dep(parfor, func_ir, typemap, index_positions=None, indexed_arrays=None, non_indexed_arrays=None):
    ...
    def check_index(stmt_index, array_accessed, index_positions, indexed_arrays, non_indexed_arrays, derived_from_indices):
        ...
        raise errors.InternalError(
            "Some code path in the parfor fusion " "cross-iteration dependency checker " "check_index didn't return a result."
        )
        ...
        for stmt in b.body:
            ...
            if isinstance(stmt, (ir.SetItem, ir.StaticSetItem)):
                if isinstance(typemap[stmt.target.name], types.npytypes.Array):
                    ...
                    if check_index(
                        stmt.index, stmt.target.name, index_positions, indexed_arrays, non_indexed_arrays, derived_from_indices
                    ):
