...
class Propagate(object):
    ...
    def __call__(self, typeinfer):
        with new_error_context("typing of assignment at {loc}", loc=self.loc):
...
class _BuildContainerConstraint(object):
    ...
    def __call__(self, typeinfer):
        with new_error_context("typing of {container_type} at {loc}", container_type=self.container_type, loc=self.loc):
            typevars = typeinfer.typevars
            tsets = [typevars[i.name].get() for i in self.items]
            if not tsets:
...
class BuildListConstraint(_BuildContainerConstraint):
    ...
    def __call__(self, typeinfer):
        with new_error_context("typing of {container_type} at {loc}", container_type=types.List, loc=self.loc):
            typevars = typeinfer.typevars
            tsets = [typevars[i.name].get() for i in self.items]
            if not tsets:
...
class ExhaustIterConstraint(object):
    ...
    def __call__(self, typeinfer):
        with new_error_context("typing of exhaust iter at {loc}", loc=self.loc):
            typevars = typeinfer.typevars
...
class PairFirstConstraint(object):
    ...
    def __call__(self, typeinfer):
        with new_error_context("typing of pair-first at {loc}", loc=self.loc):
            typevars = typeinfer.typevars
...
class PairSecondConstraint(object):
    ...
    def __call__(self, typeinfer):
        with new_error_context("typing of pair-second at {loc}", loc=self.loc):
            typevars = typeinfer.typevars
            for tp in typevars[self.pair.name].get():
                if not isinstance(tp, types.Pair):
                    # XXX is this an error?
...
class StaticGetItemConstraint(object):
    ...
    def __call__(self, typeinfer):
        with new_error_context("typing of static-get-item at {loc}", loc=self.loc):
            typevars = typeinfer.typevars
...
class TypedGetItemConstraint(object):
    ...
    def __call__(self, typeinfer):
        with new_error_context("typing of typed-get-item at {loc}", loc=self.loc):
            typevars = typeinfer.typevars
...
class CallConstraint(object):
    ...
    def __call__(self, typeinfer):
        with new_error_context("typing of call at {loc}", loc=self.loc):
            typevars = typeinfer.typevars
            with new_error_context("resolving caller type: {func}", func=self.func):
                fnty = typevars[self.func].getone()
            with new_error_context("resolving callee type: {fnty}", fnty=fnty):
...
class IntrinsicCallConstraint(CallConstraint):
    def __call__(self, typeinfer):
        with new_error_context("typing of intrinsic-call at {loc}", loc=self.loc):
            fnty = self.func
...
class GetAttrConstraint(object):
    ...
    def __call__(self, typeinfer):
        with new_error_context("typing of get attribute at {loc}", loc=self.loc):
            typevars = typeinfer.typevars
...
class StaticSetItemConstraint(SetItemRefinement):
    ...
    def __call__(self, typeinfer):
        with new_error_context("typing of staticsetitem at {loc}", loc=self.loc):
            typevars = typeinfer.typevars
...
class SetAttrConstraint(object):
    ...
    def __call__(self, typeinfer):
        with new_error_context("typing of set attribute {attr!r} at {loc}", attr=self.attr, loc=self.loc):
            typevars = typeinfer.typevars
    ...
    def typeof_expr(self, inst, target, expr):
        if expr.op == "call":
            ...
            constraint = BuildMapConstraint(
                target.name, items=expr.items, special_value=expr.literal_value, value_indexes=expr.value_indexes, loc=inst.loc
            )
            self.constraints.append(constraint)
        elif expr.op == "cast":
