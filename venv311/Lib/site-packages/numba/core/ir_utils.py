...
from numba.core.errors import CompilerError, NumbaPendingDeprecationWarning, TypingError, UnsupportedError
...
def get_ir_of_code(glbls, fcode):
    ...
    inline_pass = numba.core.inline_closurecall.InlineClosureCallPass(ir, numba.core.cpu.ParallelOptions(False), swapped)
    inline_pass.run()
...
def replace_returns(blocks, target, return_label):
    ...
    for block in blocks.values():
        ...
        if isinstance(stmt, ir.Return):
            ...
            assert (
                isinstance(cast_stmt, ir.Assign) and isinstance(cast_stmt.value, ir.Expr) and cast_stmt.value.op == "cast"
            ), "invalid return cast"
