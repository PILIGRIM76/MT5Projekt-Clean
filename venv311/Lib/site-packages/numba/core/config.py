...
def _parse_cc(text):
    ...
    if not text:
        ...
        if not m:
            raise ValueError(
                "Compute capability must be specified as a " 'string of "major.minor" where major ' "and minor are decimals"
            )
        grp = m.groups()
...
class _EnvReloader(object):
    ...
    def process_environ(self, environ):
        ...
        DISABLE_TYPEINFER_FAIL_CACHE = _readenv("NUMBA_DISABLE_TYPEINFER_FAIL_CACHE", int, 0)
