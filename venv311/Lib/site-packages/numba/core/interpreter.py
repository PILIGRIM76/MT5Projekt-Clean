...
class Interpreter(object):
    ...
    def interpret(self, bytecode):
        ...
        if PYVERSION in ((3, 12), (3, 13), (3, 14)):
            self.active_exception_entries = tuple(
                [entry for entry in self.bytecode.exception_entries if entry.start < self.last_active_offset]
            )
        elif PYVERSION in ((3, 10), (3, 11)):
            pass
        else:
        ...
        func_ir = ir.FunctionIR(
            self.blocks, self.is_generator, self.func_id, self.first_loc, self.definitions, self.arg_count, self.arg_names
        )
    ...
    def _insert_outgoing_phis(self):
        ...
        for phiname, varname in self.dfainfo.outgoing_phis.items():
            ...
            try:
                ...
                assert PYVERSION in ((3, 11), (3, 12), (3, 13), (3, 14)), "unexpected missing definition"
    ...
    def op_LOAD_CONST(self, inst, res):
        ...
        def process_slice(value):
            start = self.store(ir.Const(value.start, loc=self.loc), name=f"$const_{value.start}", redefine=True)
            stop = self.store(ir.Const(value.stop, loc=self.loc), name=f"$const_{value.stop}", redefine=True)

            slicevar = self.store(value=ir.Global("slice", slice, loc=self.loc), name="$const_slice", redefine=True)

            if value.step is None:
                params = (start, stop)
            else:
                step = self.store(ir.Const(value.step, loc=self.loc), name=f"$const_{value.step}", redefine=True)
        ...
        def process_args(value):
            ...
            for x in value:
                if isinstance(x, slice):
                    st.append(self.store(process_slice(x), name="$const_my_slice", redefine=True))
                else:
                    st.append(self.store(ir.Const(x, loc=self.loc), name=f"$const_{x}", redefine=True))
            return st
    ...
    def op_BUILD_CONST_KEY_MAP(self, inst, keys, keytmps, values, res):
        ...
        expr = ir.Expr.build_map(items=items, size=2, literal_value=literal_dict, value_indexes=value_indexes, loc=self.loc)
    ...
    def op_MAKE_FUNCTION(self, inst, name, code, closure, annotations, annotate, kwdefaults, defaults, res):
        ...
        if not isinstance(assume_code_const, ir.Const):
            msg = "Unsupported use of closure. " "Probably caused by complex control-flow constructs; " "e.g. try-except"
    ...
    if PYVERSION in ((3, 14),):
        ...
        def op_LOAD_SPECIAL(self, inst, contextmanager, exit_method, block_end):
            ...
            if block_end > self.last_active_offset:
                # Use exception entries to figure out end of syntax block
                block_end = max([ex.end for ex in self.active_exception_entries if ex.target == block_end])
