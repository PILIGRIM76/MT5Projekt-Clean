    ...
    def _dbg():
        ...
        if axis is not None:
            if isinstance(axis, int):
                ...
                printf(
                    builder, "debug: IndexError: index %d is out of bounds " "for axis %d with size %d\n", ind, axis, dimlen
                )
        else:
...
def _raw_memcpy(builder, func_name, dst, src, count, itemsize, align):
    ...
    builder.call(
        memcpy, [builder.bitcast(dst, voidptr_t), builder.bitcast(src, voidptr_t), builder.mul(count, itemsize), is_volatile]
    )
