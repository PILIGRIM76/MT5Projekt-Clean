...
class BitwiseShiftOperation(ConcreteTemplate):
    ...
    cases = [
        signature(max(op, types.intp), op, op2) for op in sorted(types.signed_domain) for op2 in [types.uint64, types.int64]
    ]
    cases += [
        signature(max(op, types.uintp), op, op2) for op in sorted(types.unsigned_domain) for op2 in [types.uint64, types.int64]
    ]
    ...
    def generic(self, args, kws):
        ...
        if len(args) == 1:
            ...
            if isinstance(args[0], types.BaseTuple):
                tys = list(args[0])
                if not tys:
                    raise errors.TypingError("%s() argument is an empty tuple" % (self.key.__name__,))
            else:
                return
        else:
            # max(*args)
