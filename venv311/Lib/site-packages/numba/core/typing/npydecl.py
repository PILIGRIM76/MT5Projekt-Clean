...
from numba.core.errors import NumbaAssertionError, NumbaPerformanceWarning, NumbaTypeError, TypingError
from numba.core.typing.templates import AbstractTemplate, AttributeTemplate, CallableTemplate, Registry, signature
...
def _sequence_of_arrays(context, func_name, arrays, dim_chooser=_homogeneous_dims):
    if not isinstance(arrays, types.BaseTuple) or not len(arrays) or not all(isinstance(a, types.Array) for a in arrays):
        raise TypingError("%s(): expecting a non-empty tuple of arrays, " "got %s" % (func_name, arrays))
