...
from numba.core import config, errors, funcdesc, ir, lowering, postproc, rewrites, typeinfer, types, typing
    ...
    try:
        ...
        if not state.status.can_fallback:
            ...
            warnings.warn_explicit(
                "%s due to: %s" % (msg_rewrite, e), errors.NumbaWarning, state.func_id.filename, state.func_id.firstlineno
            )
            raise


def type_inference_stage(typingctx, targetctx, interp, args, return_type, locals=None, raise_errors=True):
    if locals is None:
        locals = {}
