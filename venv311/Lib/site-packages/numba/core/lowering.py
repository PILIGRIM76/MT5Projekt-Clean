...
class Lower(BaseLower):
    ...
    def __call_first_class_function_pointer(self, ftype, fname, sig, argvals):
        ...
        struct = cgutils.create_struct_proxy(self.typeof(fname))(context, builder, value=fstruct)
        ...
        if_jit_addr_is_null = builder.if_else(cgutils.is_null(builder, jit_addr), likely=False)
        with if_jit_addr_is_null as (then, orelse):
            ...
            with orelse:
                llty = ctx.call_conv.get_function_type(sig.return_type, sig.args).as_pointer()
                func_ptr = builder.bitcast(jit_addr, llty)
                # call
                status, res = ctx.call_conv.call_function(builder, func_ptr, sig.return_type, sig.args, argvals)
    ...
    def lower_expr(self, resty, expr):
        if expr.op == "binop":
            ...
            castvals = [
                self.context.cast(self.builder, val, fromty, toty) for val, toty, fromty in zip(itemvals, resty, itemtys)
            ]
        ...
        if name in self._singly_assigned_vars and not self._disable_sroa_like_opt:
            ...
            if argidx is not None:
                ...
                self.debuginfo.mark_variable(
                    self.builder, ptr, name=name, lltype=lltype, size=sizeof, line=loc.line, datamodel=datamodel, argidx=argidx
                )
            else:
                self.builder.store(value, ptr)
