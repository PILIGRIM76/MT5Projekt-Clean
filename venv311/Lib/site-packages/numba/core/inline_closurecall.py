...
def _inline_arraycall(func_ir, cfg, visited, loop, swapped, enable_prange=False, typed=False):
    ...
    if range_def:
        ...
        if typed:
            ...
            stmts.append(
                _new_definition(func_ir, len_func_var, ir.Global("length_of_iterator", length_of_iterator, loc=loc), loc)
            )
    ...
    if dtype_def and dtype_mod_def:
        ...
        if typed:
            ...
            stmts.append(
                _new_definition(func_ir, empty_func, ir.Global("unsafe_empty_inferred", unsafe_empty_inferred, loc=loc), loc)
            )
            array_kws = []
        else:
            raise GuardException
    ...
    if range_def:
        if range_def[0] != 0:
            ...
            blk.body.insert(
                0,
                _new_definition(
                    func_ir, index_var, ir.Expr.binop(fn=operator.sub, lhs=iter_first_var, rhs=range_def[0], loc=loc), loc
                ),
            )
    else:
        ...
        stmts.append(
            _new_definition(func_ir, next_index_var, ir.Expr.binop(fn=operator.add, lhs=index_var, rhs=one, loc=loc), loc)
        )
