...
from numba.core.compiler import DEFAULT_FLAGS, CompilerBase, DefaultPassBuilder, Flags, compile_extra
...
skip_if_py313plus_on_windows = unittest.skipIf(
    utils.PYVERSION >= (3, 13) and sys.platform.startswith("win"), "Not supported on Python 3.13+ on Windows"
)

skip_if_py314 = unittest.skipIf(utils.PYVERSION == (3, 14), "Test unstable on 3.14")

skip_if_linux_aarch64 = unittest.skipIf(
    sys.platform.startswith("linux") and platform.machine() == "aarch64", "Not supported on Linux aarch64"
)
...
skip_if_freethreading = unittest.skipIf(
    _free_threading, ("Skipped [NOT APPLICABLE] if using a " "free-threading build and " "free-threading is enabled.")
)
...
class TestCase(unittest.TestCase):
    ...
    def subprocess_test_runner(
        self, test_module, test_class=None, test_name=None, envvars=None, timeout=60, flags=None, _subproc_test_env="1"
    ):
        """
        ...
        cmd = [sys.executable, *flags_args, "-m", "numba.runtests", fully_qualified_test]
