...
def _get_wrapper_address(func, sig):
    ...
    if addr <= 0 and addr != -1:
        raise ValueError(f"wrapper address of {type(func)} instance must be" f" a positive integer but got {addr} [sig={sig}]")
...
def unbox_function_type(typ, obj, c):
    ...
    addr = lower_get_jit_address(c.context, c.builder, obj, typ.signature, failure_mode="return_null")
