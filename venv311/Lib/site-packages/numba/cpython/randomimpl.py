...
def np_gauss_impl3(loc, scale, size):
    ...
    if (
        isinstance(loc, (types.Float, types.Integer))
        and isinstance(scale, (types.Float, types.Integer))
        and (isinstance(size, types.Integer) or (isinstance(size, types.UniTuple) and isinstance(size.dtype, types.Integer)))
    ):
...
def np_randint_impl_3(low, high, size):
    ...
    if isinstance(low, types.Integer) and isinstance(high, types.Integer) and is_empty_tuple(size):
        # Handle size = ()
...
def triangular_impl(left, mode, right, size=None):
    ...
    if is_empty_tuple(size):
        # Handle size = ()
        return lambda left, mode, right, size=None: np.array(np.random.triangular(left, mode, right))
    if isinstance(size, types.Integer) or (isinstance(size, types.UniTuple) and isinstance(size.dtype, types.Integer)):
...
def hypergeometric_impl(ngood, nbad, nsample, size):
    ...
    if is_empty_tuple(size):
        # Handle size = ()
        return lambda ngood, nbad, nsample, size: np.array(np.random.hypergeometric(ngood, nbad, nsample))
    if isinstance(size, types.Integer) or (isinstance(size, types.UniTuple) and isinstance(size.dtype, types.Integer)):
...
def negative_binomial_impl(n, p):
    if isinstance(n, (types.Float, types.Integer)) and isinstance(p, (types.Float, types.Integer)):
...
def choice(a, size=None, replace=True):
    ...
    if isinstance(a, types.Array):
        ...
        raise NumbaTypeError("np.random.choice() first argument should be " "int or array, got %s" % (a,))
...
def multinomial(n, pvals, size=None):
    ...
    if not isinstance(n, types.Integer):
        raise NumbaTypeError("np.random.multinomial(): n should be an " "integer, got %s" % (n,))

    if not isinstance(pvals, (types.Sequence, types.Array)):
        raise NumbaTypeError("np.random.multinomial(): pvals should be an " "array or sequence, got %s" % (pvals,))
    ...
    if size in (None, types.none):
        ...
        raise NumbaTypeError("np.random.multinomial(): size should be int or " "tuple or None, got %s" % (size,))
