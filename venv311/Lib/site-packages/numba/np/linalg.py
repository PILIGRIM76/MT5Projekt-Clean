...
from numba.core.imputils import impl_ret_borrowed, impl_ret_new_ref, impl_ret_untracked, lower_builtin
                ...
                with r_mat:
                    with builder.if_else(is_left_vec) as (v_m, m_m):
                        ...
                        with m_m:
                            # M * M
                            call_xxgemm(
                                context, builder, xty, x_shapes, x_data, yty, y_shapes, y_data, outty, out_shapes, out_data
                            )
...
def dot_3(a, b, out):
    ...
    if isinstance(a, types.Array) and isinstance(b, types.Array) and isinstance(out, types.Array):
        ...
        if a.layout not in "CF" or b.layout not in "CF" or out.layout not in "CF":
            warnings.warn("np.vdot() is faster on contiguous arrays, called on %s" % ((a, b),), NumbaPerformanceWarning)
...
def solve_impl(a, b):
    ...
    def solve_impl(a, b):
        ...
        r = numba_xgesv(
            kind, n, nrhs, acpy.ctypes, n, ipiv.ctypes, bcpy.ctypes, n  # kind  # n  # nhrs  # a  # lda  # ipiv  # b  # ldb
        )
        _inv_err_handler(r)
