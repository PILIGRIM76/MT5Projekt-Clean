...
from numba.np.numpy_support import _ufunc_loop_sig, from_dtype, select_array_wrapper, ufunc_find_matching_loop
...
class _ArrayGUHelper(
    namedtuple(
        "_ArrayHelper",
        ("context", "builder", "shape", "strides", "data", "layout", "base_type", "ndim", "inner_arr_ty", "is_input_arg"),
    )
):
    """Helper class to handle array arguments/result.
    It provides methods to generate code loading/storing specific
    ...
    def _load_effective_address(self, indices):
        ...
        return cgutils.get_item_pointer2(
            context, builder, data=arr.data, shape=self.shape, strides=self.strides, layout=self.layout, inds=indices
        )
    ...
    def guard_shape(self, loopshape):
        ...
        tup = (context.make_tuple(builder, sig.args[0], loopshape), context.make_tuple(builder, sig.args[1], self.shape))
...
def numpy_gufunc_kernel(context, builder, sig, args, ufunc, kernel_class):
    ...
    for (idx_a, sig_a), (idx_b, sig_b) in itertools.combinations(zip(range(len(arguments)), _sig[0] + _sig[1]), r=2):
