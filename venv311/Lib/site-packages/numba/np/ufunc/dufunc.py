...
from numba.core.extending import intrinsic, is_jitted, overload_attribute, overload_method, register_jitable
...
class DUFunc(serialize.ReduceMixin, _internal._DUFunc, UfuncBase):
    ...
    def _install_ufunc_attributes(self, template) -> None:
        ...
        attributes = ("nin", "nout", "nargs", "identity", "signature")  # 'ntypes', # 'types',
    ...
    def _install_ufunc_at(self, template) -> None:
        ...
        def ol_at(ufunc, a, indices, b=None):
            warnings.warn("ufunc.at feature is experimental", category=errors.NumbaExperimentalFeatureWarning)
    ...
    def _install_ufunc_reduce(self, template) -> None:
        ...
        def ol_reduce(ufunc, array, axis=0, dtype=None, initial=None):
            ...
            if not self.reorderable and axis_int_tuple and len(axis) > 1:
                msg = f"reduction operation '{ufunc_name}' is not " "reorderable, so at most one axis may be specified"
                raise errors.NumbaTypeError(msg)
            ...
            def impl_1d(ufunc, array, axis=0, dtype=None, initial=None):
                if identity_none and initial is None and len(array) == 0:
                    msg = "zero-size array to reduction operation " f"{ufunc_name} which has no identity"
    ...
    def _install_ufunc_reduceat(self, template) -> None:
        ...
        def ol_reduceat(ufunc, array, indices, axis=0, dtype=None, out=None):
            ...
            warnings.warn("ufunc.reduceat feature is experimental", category=errors.NumbaExperimentalFeatureWarning)
            ...
            if not isinstance(axis, (types.Integer, types.Omitted, types.IntegerLiteral, int)):
            ...
            if indices_arr and indices.ndim != 1:
                msg = 'Expect "indices" array to have at most 1 dimension. ' f"Got {indices.ndim}"
                raise errors.NumbaValueError(msg)

            if need_cast:

                def impl_cast(ufunc, array, indices, axis=0, dtype=None, out=None):  # noqa: E501
                    return ufunc.reduceat(np.asarray(array), np.asarray(indices), axis, dtype=dtype, out=out)
            ...
            def impl(ufunc, array, indices, axis=0, dtype=None, out=None):
                ...
                if axis < 0 or axis >= array_ndim:
                    raise ValueError(f"axis {ax} is out of bounds for array " f"of dimension {array_ndim}")
                ...
                if not out_none and out.shape != shape:
                    ...
                    msg = "operands could not be broadcast together with " "remmaped shapes"
                    raise ValueError(msg)
    ...
    def at(self, a, indices, b=None):
        ...
        ewise_types = tuple(arg.dtype if isinstance(arg, types.Array) else arg for arg in argtys)
    ...
    def _type_me(self, argtys, kwtys):
        ...
        if sig is None:
            ...
            # compile.
            if self._frozen:
                raise errors.NumbaTypeError("cannot call %s with types %s" % (self, argtys))
