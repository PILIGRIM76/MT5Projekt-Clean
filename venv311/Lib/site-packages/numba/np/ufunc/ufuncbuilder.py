            ...
            with targetconfig.ConfigStack().enter(flags.copy()):
                with store_overloads_on_success():
                    ...
                    cres = compiler.compile_extra(
                        typingctx, targetctx, self.py_func, args=args, return_type=return_type, flags=flags, locals=locals
                    )
...
class GUFuncBuilder(_BaseUFuncBuilder):
    ...
    # TODO handle scalar
    def __init__(self, py_func, signature, identity=None, cache=False, targetoptions=None, writable_args=()):
        if targetoptions is None:
            targetoptions = {}
        self.py_func = py_func
        self.identity = parse_identity(identity)
