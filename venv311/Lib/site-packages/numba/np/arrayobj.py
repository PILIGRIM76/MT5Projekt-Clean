...
from numba.core import cgutils, errors, extending, types, typing
...
def populate_array(array, data, shape, strides, itemsize, meminfo, parent=None):
    ...
    *shape* and *strides* can be Python tuples or LLVM arrays.
    ...
    if (required_fields & datamodel_fields) != required_fields:
        ...
        msg = f"The datamodel for type {array._fe_type} is missing " f"field{'s' if len(missing) > 1 else ''} {missing}."
...
def normalize_axis_tuple_overloads(func_name, arg_name, ndim, axis):
    ...
    invalid_axis_msg = (
        f"{func_name.literal_value}: Argument {arg_name.literal_value} " "out of bounds for dimensions of the array"
    )

    repeated_axis_msg = f"{func_name.literal_value}: repeated axis in " f"{arg_name.literal_value} argument"
    ...
    def src_getitem(source_indices):
        ...
        with builder.if_else(use_copy, likely=False) as (if_copy, otherwise):
            ...
            with otherwise:
                builder.store(
                    cgutils.get_item_pointer2(
                        context, builder, src_data, src_shapes, src_strides, srcty.layout, source_indices, wraparound=False
                    ),
                    src_ptr,
                )
...
def ol_raise_with_shape_context_cpu(src_shapes, index_shape):
    if (
        ...
        def impl(src_shapes, index_shape):
            ...
            msg = f"cannot assign slice of shape {shape_str} from input of " f"shape {index_str}"
            raise ValueError(msg)
...
def fancy_setslice(context, builder, sig, args, index_types, indices):
    ...
    def raise_shape_mismatch_error(context, builder, src_shapes, index_shape):
        ...
        argtys = (types.UniTuple(types.int64, len(src_shapes)), types.UniTuple(types.int64, len(index_shape)))
...
def _change_dtype(context, builder, oldty, newty, ary):
    ...
    populate_array(
        strides_ary, data=strides_data, shape=shape_constant, strides=strides_constant, itemsize=sizeof_intp, meminfo=None
    )
...
def np_unique(ar):
    def np_unique_impl(ar):
        ...
        b = np.sort(ar.ravel())
        head = list(b[:1])
        tail = [x for i, x in enumerate(b[1:]) if b[i] != x and not (isnan(b[i]) and isnan(x))]
...
def ol_np_empty(shape, dtype=float):
    ...
    if dtype is float or (isinstance(dtype, types.Function) and dtype.typing_key is float) or is_nonelike(dtype):  # default
...
def numpy_take(a, indices, axis=None):
    ...
    if cgutils.is_nonelike(axis):
        ...
        if isinstance(a, types.Array) and isinstance(indices, (types.Array, types.List, types.BaseTuple)):
            ...
            def take_impl(a, indices, axis=None):
                ...
                if axis < 0 or axis >= a.ndim:
                    msg = f"axis {ax} is out of bounds for array " f"of dimension {a.ndim}"
                    raise ValueError(msg)
...
def _as_layout_array_intrinsic(typingctx, a, output_layout):
    ...
    # copies are writable
    ret = a.copy(layout=output_layout.literal_value, ndim=max(a.ndim, 1), readonly=False)
    ...
    def codegen(context, builder, sig, args):
        ...
        if ty.is_c_contig and ty.layout == "C":
            ...
            with cgutils.loop_nest(builder, shape, intp_t) as indices:
                ptr = cgutils.get_item_pointer2(context, builder, arr.data, shape, strides, layout, indices)
...
def np_frombuffer(typingctx, buffer, dtype, count, offset, retty):
    ...
    def codegen(context, builder, sig, args):
        ...
        data = builder.bitcast(data, context.get_value_type(out_datamodel.get_type("data")))

        populate_array(
            out_ary, data=data, shape=shape, strides=strides, itemsize=ll_itemsize, meminfo=buf.meminfo, parent=buf.parent
        )
...
def numpy_moveaxis(a, source, destination):
    if not isinstance(a, types.Array):
        raise errors.TypingError('The first argument "a" must be an array')
    if not (
        isinstance(source, types.Integer) or (isinstance(source, types.Sequence) and isinstance(source.dtype, types.Integer))
    ):
        raise errors.TypingError('The second argument "source" must be an integer ' "or sequence of integers")
    if not (
        isinstance(destination, types.Integer)
        or (isinstance(destination, types.Sequence) and isinstance(destination.dtype, types.Integer))
    ):
        raise errors.TypingError('The third argument "destination" must be an integer ' "or sequence of integers")
    ...
    def impl(a, source, destination):
        ...
        destination = normalize_axis_tuple("np.moveaxis", "destination", ndim, destination)
        if len(source) != len(destination):
            raise ValueError("`source` and `destination` arguments must have " "the same number of parameters")
...
def nan_to_num_impl(x, copy=True, nan=0.0, posinf=None, neginf=None):
    if isinstance(x, types.Number):
        if isinstance(x, types.Integer):
            ...
            def impl(x, copy=True, nan=0.0, posinf=None, neginf=None):
                min_inf = neginf if neginf is not None else np.finfo(type(x)).min
                max_inf = posinf if posinf is not None else np.finfo(type(x)).max
        ...
        if isinstance(x.dtype, types.Integer):
            ...
            def impl(x, copy=True, nan=0.0, posinf=None, neginf=None):
                min_inf = neginf if neginf is not None else np.finfo(x.dtype).min
                max_inf = posinf if posinf is not None else np.finfo(x.dtype).max
            ...
            def impl(x, copy=True, nan=0.0, posinf=None, neginf=None):
                ...
                np.nan_to_num(output.real, copy=False, nan=nan, posinf=posinf, neginf=neginf)
