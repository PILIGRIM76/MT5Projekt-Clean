...
def complete_zsh(parser, root_prefix=None, preamble="", choice_functions=None):
    ...
    def format_optional(opt, parser):
        ...
        return (
            ('{nargs}{options}"[{help}]"' if isinstance(opt, FLAG_OPTION) else '{nargs}{options}"[{help}]:{dest}:{pattern}"')
            .format(
                ...
                dest=opt.dest,
                pattern=(
                    complete2pattern(opt.complete, "zsh", choice_type2fn)
    ...
    all_commands = {
        root_prefix: {
            "cmd": prog,
            "arguments": [format_optional(opt, parser) for opt in parser._get_optional_actions() if opt.help != SUPPRESS]
            + [
        ...
        for sub in parser._get_positional_actions():
            ...
            if not sub.choices or not isinstance(sub.choices, dict):
                ...
                for cmd, subparser in sub.choices.items():
                    ...
                    # optionals
                    arguments = [
                        format_optional(opt, parser) for opt in subparser._get_optional_actions() if opt.help != SUPPRESS
                    ]
                    ...
                    options["commands"] = {
                        all_commands[pref]["cmd"]: all_commands[pref] for pref in new_subcmds if pref in all_commands
                    }
...
"""
    ...
    def command_list(prefix, options):
        ...
        commands = "\n    ".join(
            f'"{escape_zsh(cmd)}:{escape_zsh(opt["help"])}"' for cmd, opt in sorted(options["commands"].items())
        )
        return f"""
{prefix}_commands() {{
  local _commands=(
