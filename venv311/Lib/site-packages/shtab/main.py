...
def get_main_parser():
    ...
    parser.add_argument("-o", "--output", default="-", help="output file (- for stdout)", type=Path)
...
def main(argv=None):
    ...
    with _open(args.output) as fd:
        print(
            complete(
                other_parser, shell=args.shell, root_prefix=args.prefix or args.parser.split(".", 1)[0], preamble=args.preamble
            ),
            file=fd,
        )
