...
def _send_response(rpc_event: cygrpc.BaseEvent, state: _RPCState, serialized_response: bytes) -> bool:
    with state.condition:
        if not _is_rpc_state_active(state):
            return False
        if state.initial_metadata_allowed:
...
def _add_registered_method_handlers(state: _ServerState, method_handlers: Dict[str, grpc.RpcMethodHandler]) -> None:
    with state.lock:
...
def _process_event_and_continue(state: _ServerState, event: cygrpc.BaseEvent) -> bool:
    ...
    if event.tag is _SHUTDOWN_TAG:
    ...
    elif event.tag is _REQUEST_CALL_TAG or event.tag in state.registered_method_handlers:
        ...
        with state.lock:
            ...
            concurrency_exceeded = (
                state.maximum_concurrent_rpcs is not None and state.active_rpc_count >= state.maximum_concurrent_rpcs
            )
...
def _serve(state: _ServerState) -> None:
    while True:
        ...
        is_timeout = event.completion_type == cygrpc.CompletionType.queue_timeout
...
def _augment_options(
    ...
    maybe_server_call_tracer_factory_option = _observability.create_server_call_tracer_factory_option(xds)
...
class _Server(grpc.Server):
    ...
    def add_insecure_port(self, address: str) -> int:
        return _common.validate_port_binding_result(address, _add_insecure_port(self._state, _common.encode(address)))
