...
def _event_handler(state: _RPCState, response_deserializer: Optional[DeserializingFunction]) -> UserTag:
...
class _InactiveRpcError(grpc.RpcError, grpc.Call, grpc.Future):
    ...
    def traceback(self, timeout: Optional[float] = None) -> Optional[types.TracebackType]:  # pylint: disable=unused-argument
        ...
        with self._state.condition:
            ...
            def _response_ready():
                return self._state.response is not None or (
                    cygrpc.OperationType.receive_message not in self._state.due and self._state.code is not None
                )
...
class _StreamUnaryMultiCallable(grpc.StreamUnaryMultiCallable):
    ...
    def _blocking(
        ...
        call = self._channel.segregated_call(
            ...
            augmented_metadata,
            None if credentials is None else credentials._credentials,
            _stream_unary_invocation_operations_and_tags(augmented_metadata, initial_metadata_flags),
            self._context,
            self._registered_call_handle,
        )
...
def _unsubscribe(
    ...
    with state.lock:
        for index, (subscribed_callback, _unused_connectivity) in enumerate(state.callbacks_and_connectivities):
