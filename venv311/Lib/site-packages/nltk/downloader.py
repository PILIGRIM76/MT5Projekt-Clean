    ...
    def __init__(
        ...
        **kw,
        ...
        if os.path.isabs(subdir) or ".." in subdir.replace("\\", "/").split("/"):
            raise ValueError(
                f"Invalid package subdir {subdir!r}: must be a relative path " f"without parent directory references"
            )
        self.subdir = subdir
        ...
        if os.sep in id or "/" in id or "\\" in id or ".." in id:
            raise ValueError(f"Invalid package id {id!r}: must not contain path separators")
