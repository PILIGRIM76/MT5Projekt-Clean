    ...
    if hasattr(f, "read"):
        ...
        if allowed_dir is not None:
            ...
            if not target_path.is_relative_to(safe_root):
                raise PermissionError(f"Security Violation: Path {target_path} is outside allowed_dir {safe_root}")
            ...
            try:
                for child in children(node):
                    if child not in traversed:
                        ...
                        warnings.warn(
                            "Discarded redundant search for {} at depth {}".format(child, depth + 1),
                            stacklevel=2,
                        )
            except TypeError:
                pass
...
def acyclic_branches_depth_first(tree, children=iter, depth=-1, cut_mark=None, traversed=None, verbose=False):
    """
    :param tree: the tree root
    :param children: a function taking as argument a tree node
    :param depth: the maximum depth of the search
