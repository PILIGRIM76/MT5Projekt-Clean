...
def _pretty_lu(lu):
    ...
    if "semTypes" in lukeys:
        ...
        outstr += (
            "  " * (len(lu.semTypes) > 0) + ", ".join(f"{x.name}({x.ID})" for x in lu.semTypes) + "\n" * (len(lu.semTypes) > 0)
        )
    if "URL" in lukeys:
        outstr += f"\n[URL] {lu.URL}\n"
...
def _annotation_ascii_frames(sent):
    ...
    for j, k, fname, asetIndex in overt:
        if not j >= i:
            assert j >= i, (
                "Overlapping targets?" + (" UNANN" if any(aset.status == "UNANN" for aset in sent.annotationSet[1:]) else ""),
                (j, k, asetIndex),
            )
        s1 += " " * (j - i) + "*" * (k - j)
        short = fname[: k - j]
        ...
        def _matching_exs():
            for f in frames:
                ...
                if fe is not None:
                    ...
                    if fe2 is not None:
                        fes2 = (
                            {ffe for ffe in f.FE.keys() if re.search(fe2, ffe, re.I)} if isinstance(fe2, str) else {fe2.name}
                        )
