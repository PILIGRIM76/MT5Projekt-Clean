...
class ReviewsCorpusReader(CorpusReader):
    ...
    def features(self, fileids=None):
        ...
        if fileids is None:
            ...
            fileids = [fileids]
        return concat(
            [self.CorpusView(fileid, self._read_features, encoding=enc) for (fileid, enc) in self.abspaths(fileids, True)]
        )
    ...
    def reviews(self, fileids=None):
        ...
        if fileids is None:
            fileids = self._fileids
        return concat(
            [self.CorpusView(fileid, self._read_review_block, encoding=enc) for (fileid, enc) in self.abspaths(fileids, True)]
        )
