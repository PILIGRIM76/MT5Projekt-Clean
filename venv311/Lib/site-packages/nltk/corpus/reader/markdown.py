...
class MarkdownBlock:
    ...
    def __str__(self):
        return f"{self.content[:self.truncate_at]}" f"{'...' if len(self.content) > self.truncate_at else ''}"
    ...
    @property
    def paras(self):
        return [[word_tokenize(sent) for sent in sent_tokenize(para)] for para in blankline_tokenize(self.content)]
...
class CodeBlock(MarkdownBlock):
    ...
    @property
    def paras(self):
        return [[word_tokenize(line) for line in para.splitlines()] for para in blankline_tokenize(self.content)]
...
class CategorizedMarkdownCorpusReader(CategorizedCorpusReader, MarkdownCorpusReader):
    ...
    def concatenated_view(self, reader, fileids, categories):
        return concat(
            [
                ...
                for (path, enc) in self.abspaths(self._resolve(fileids, categories), include_encoding=True)
            ]
        )
    ...
    def image_reader(self, stream):
        return [
            ...
            for inline_token in filter(lambda t: t.type == "inline", self.parser.parse(stream.read()))
