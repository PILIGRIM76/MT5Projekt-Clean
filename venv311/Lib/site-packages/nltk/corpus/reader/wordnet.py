...
class Lemma(_WordNetObject):
    ...
    def _related(self, relation_symbol):
        ...
        return [
            ...
            for pos, offset, lemma_index in self._synset._lemma_pointers[self._name, relation_symbol]
        ]
 ...
 Synset('physical_entity.n.01'), Synset('entity.n.01')]
    ...
    def common_hypernyms(self, other):
        ...
        if not other._all_hypernyms:
            other._all_hypernyms = {
                other_synset for other_synsets in other._iter_hypernym_lists() for other_synset in other_synsets
            }
    ...
    def lch_similarity(self, other, verbose=False, simulate_root=True):
        ...
        if self._pos != other._pos:
            raise WordNetError(
                "Computing the lch similarity requires " "%s and %s to have the same part of speech." % (self, other)
            )
...
class WordNetCorpusReader(CorpusReader):
    ...
    def synset(self, name):
        ...
        try:
            ...
            raise WordNetError(
                f"Lemma {lemma!r} with part of speech {pos!r} only " f"has {n_senses} {'sense' if n_senses == 1 else 'senses'}"
            ) from e
...
def _lcs_ic(synset1, synset2, ic, verbose=False):
    ...
    if synset1._pos != synset2._pos:
        raise WordNetError(
            "Computing the least common subsumer requires " "%s and %s to have the same part of speech." % (synset1, synset2)
        )
