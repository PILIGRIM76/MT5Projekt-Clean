...
class MyServerHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if unquote_plus(sp) == "SHUTDOWN THE SERVER":
            ...
            word = html.escape([p.split("=")[1].replace("+", " ") for p in parts if p.startswith("nextWord")][0])
...
def _synset_relations(word, synset, synset_relations):
    ...
    html = (
        "<ul>"
        + "\n".join(
            "<li>%s</li>" % make_synset_html(*rel_data) for rel_data in get_relations_data(word, synset) if rel_data[2] != []
        )
        + "</ul>"
    )
        ...
        if pos in pos_forms:
            ...
            for w in pos_forms[pos]:
                ...
                try:
                    ...
                    pass
    if not body:
        body = "The word or words '%s' were not found in the dictionary." % html.escape(word)
    return body, word
