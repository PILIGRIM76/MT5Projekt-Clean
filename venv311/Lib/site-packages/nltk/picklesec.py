...
class WarningUnpickler(pickle.Unpickler):
    ...
    def load(self) -> Any:
        if not self._warned:
            msg = PICKLE_WARNING if self._context is None else f"{PICKLE_WARNING} ({self._context})"
            warnings.warn(msg, RuntimeWarning, stacklevel=3)
            self._warned = True
        return super().load()


def pickle_load(file: BinaryIO, *, context: str | None = None, restricted: bool = False) -> Any:
    """
