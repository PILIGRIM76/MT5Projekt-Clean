    ...
    def expand(cls, featurelists, combinations=None, skipintersecting=True):
        ...
        def nonempty_powerset(xs):  # xs is a list
            ...
            combrange = (
                ...
                if k is None
                    ...
                    (k, k + 1) if isinstance(k, int) else (k[0], k[1] + 1)  # n over 1 .. n over n (all non-empty combinations)
                )  # n over k (only
        ...
        for picks in nonempty_powerset(featurelists):
            for pick in it.product(*picks):
                if any(i != j and x.issuperset(y) for (i, x) in enumerate(pick) for (j, y) in enumerate(pick)):
                    continue
                if skipintersecting and any(
                    i != j and x.intersects(y) for (i, x) in enumerate(pick) for (j, y) in enumerate(pick)
                ):
                    continue
