...
def py_where(program: str, path: Optional[PathLike] = None) -> List[str]:
    ...
    def is_exec(fpath: str) -> bool:
        return (
            ...
            and (sys.platform != "win32" or not winprog_exts or any(fpath.upper().endswith(ext) for ext in winprog_exts))
        )
...
class LockFile:
    ...
    def _obtain_lock_or_raise(self) -> None:
        ...
        if osp.isfile(lock_file):
            raise IOError(
                "Lock for file %r did already exist, delete %r in case the lock is illegal" % (self._file_path, lock_file)
            )
