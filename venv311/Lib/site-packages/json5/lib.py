...
def parse(
    ...
    *,
    ...
    ast, err, pos = parser.parse(global_vars={"_strict": strict, "_consume_trailing": consume_trailing})
    if err:
        return None, err, pos
...
class JSON5Encoder:
    ...
    def _encode_array(self, obj: Any, seen: Set, level: int) -> str:
        ...
        indent_str, end_str = self._spacers(level)
        item_sep = self.item_separator + indent_str
        return "[" + indent_str + item_sep.join(self.encode(el, seen, level, as_key=False) for el in obj) + end_str + "]"
