...
@jobs_cli.command("logs", examples=["hf jobs logs <job_id>", "hf jobs logs -f <job_id>", "hf jobs logs --tail 20 <job_id>"])
def jobs_logs(
    job_id: JobIdArg,
    follow: Annotated[
        bool,
    ...
    if follow and tail is not None:
        raise CLIError("Cannot use --follow and --tail together. Use --follow to stream logs or --tail to show recent logs.")
    ...
    for f in filter or []:
        if f.startswith("label!=") or f.startswith("label="):
            if f.startswith("label!="):
                ...
                if "=" in label_part:
                    print(f"Warning: Ignoring invalid label filter format 'label!={label_part}'. Use label!=key format.")
                    continue
...
def _tabulate(rows: list[list[str | int]], headers: list[str]) -> str:
    ...
    for row in rows:
        row_format_args = [
            str(x)[: col_width - 3] + "..." if len(str(x)) > col_width else str(x) for x, col_width in zip(row, col_widths)
        ]
