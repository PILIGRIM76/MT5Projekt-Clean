...
class HfFileSystem(fsspec.AbstractFileSystem, metaclass=_Cached):
    ...
    * Buckets (generic storage):
    ...
    def _repo_and_revision_exist(self, repo_type: str, repo_id: str, revision: str | None) -> tuple[bool, Exception | None]:
    ...
    def resolve_path(
        ...
        return HfFileSystemResolvedRepositoryPath(repo_type, repo_id, revision, path_in_repo, _raw_revision=revision_in_path)
    ...
    def _ls_tree(
        ...
        expand_info = expand_info if expand_info is not None else (self.expand_info if self.expand_info is not None else False)
        resolved_path = self.resolve_path(path, revision=revision)
        path = resolved_path.unresolve()
                    ...
                    if maxdepth is None or depth <= maxdepth:
                        if dir_info["name"] not in self.dircache:
                            ...
                            dirs_to_visit.extend(
                                [(depth + 1, path_info) for path_info in cached_path_infos if path_info["type"] == "directory"]
                            )
        ...
        for bucket_entry in bucket_files:
            ...
            for parent_bucket_folder_str in list(PurePosixPath(bucket_entry.path).parents)[: -min_depth - 1]:
                ...
                if parent_bucket_folder.uploaded_at is not None:
                    ...
                    if bucket_folder.uploaded_at is None or (bucket_folder.uploaded_at < parent_bucket_folder.uploaded_at):
