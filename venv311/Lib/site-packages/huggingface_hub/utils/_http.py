...
def _httpx_follow_relative_redirects_with_backoff(
    ...
    no_retry_kwargs: dict[str, Any] = {} if retry_on_errors else {"retry_on_exceptions": (), "retry_on_status_codes": ()}
...
def hf_raise_for_status(response: httpx.Response, endpoint_name: str | None = None) -> None:
    ...
    try:
        ...
        request_url = str(response.request.url) if response.request is not None and response.request.url is not None else None
        ...
        if error_code == "RevisionNotFound":
        ...
        elif error_code == "RepoNotFound" and request_url is not None and BUCKET_API_REGEX.search(request_url) is not None:
            message = (
                f"{response.status_code} Client Error."
                + "\n\n"
            ...
            raise _format(BucketNotFoundError, message, response, bucket_id=_parse_bucket_id_from_url(request_url)) from e
