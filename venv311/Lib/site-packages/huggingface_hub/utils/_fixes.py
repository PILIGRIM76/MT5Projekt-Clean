...
def WeakFileLock(lock_file: str | Path, *, timeout: float | None = None) -> Generator[BaseFileLock, None, None]:
    ...
    while True:
        ...
        try:
            ...
            logger.info(f"Still waiting to acquire lock on {lock_file} (elapsed: {time.time() - start_time:.1f} seconds)")
        except NotImplementedError as e:
            if "use SoftFileLock instead" in str(e):
                logger.warning("FileSystem does not appear to support flock. Falling back to SoftFileLock for %s", lock_file)
