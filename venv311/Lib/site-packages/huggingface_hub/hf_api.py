...
class ModelInfo:
    ...
    def __init__(self, **kwargs):
        ...
        if isinstance(mapping, list):
            ...
            raise ValueError(f"Unexpected type for `inferenceProviderMapping`. Expecting `dict` or `list`. Got {mapping}.")
...
class HfApi:
    ...
    def list_repo_files(
        ...
        *,
        ...
        return [
            f.rfilename
            for f in self.list_repo_tree(repo_id=repo_id, recursive=True, revision=revision, repo_type=repo_type, token=token)
    ...
    def list_repo_refs(
        ...
        *,
        ...
        return GitRefs(
            ...
            pull_requests=[_format_as_git_ref_info(item) for item in data["pullRequests"]] if include_pull_requests else None,
        )
    ...
    def list_inference_endpoints(
        ...
        return [
            InferenceEndpoint.from_raw(endpoint, namespace=namespace, token=token) for endpoint in response.json()["items"]
        ]
    ...
    def _get_namespace(self, token: bool | str | None = None) -> str:
        ...
        if me["type"] == "user":
            ...
            raise ValueError(
                "Cannot determine default namespace. You must provide a 'namespace' as input or be logged in as a" " user."
            )
    ...
    def _prepare_upload_folder_additions(
        ...
        filtered_repo_objects = list(
            filter_repo_objects(relpath_to_abspath.keys(), allow_patterns=allow_patterns, ignore_patterns=ignore_patterns)
        )
    ...
    def copy_files(self, source: str, destination: str, *, token: str | bool | None = None) -> None:
        ...
        if isinstance(source_handle, _BucketCopyHandle):
            ...
            if source_path_info:
                ...
                all_copies.append(
                    _build_copy_op(target_path, source_file.xet_hash, source_file.size, "bucket", source_handle.bucket_id)
                )
            else:
                ...
                for item in self.list_bucket_tree(
                    ...
                    all_copies.append(_build_copy_op(target_path, item.xet_hash, item.size, "bucket", source_handle.bucket_id))
        ...
        *,
        ...
        if len(add_operations_to_upload) > 0:
            ...
            try:
                ...
                for upload_info, op in zip(xet_upload_infos, [op for op in add_path_operations if op.xet_hash is None]):
                ...
                for upload_info, op in zip(xet_upload_infos, [op for op in add_bytes_operations if op.xet_hash is None]):
                    op.xet_hash = upload_info.hash
                    op.size = upload_info.filesize
    ...
    def download_bucket_files(
        ...
        *,
        ...
        if str_paths:
            bucket_files_by_path = {info.path: info for info in self.get_bucket_paths_info(bucket_id, str_paths, token=token)}
