...
if TYPE_CHECKING:
    from lightgbm.basic import _LGBM_BoosterEvalMethodResultType, _LGBM_BoosterEvalMethodResultWithStandardDeviationType
...
if _imports.is_successful():
    ...
    from optuna_integration._lightgbm_tuner import LightGBMTuner, LightGBMTunerCV
...
class LightGBMPruningCallback:
    ...
    def _find_evaluation_result(
    ...
    ) -> _LGBM_BoosterEvalMethodResultType | _LGBM_BoosterEvalMethodResultWithStandardDeviationType | None:
        ...
        for evaluation_result in evaluation_result_list:
            ...
            if valid_name != target_valid_name or (metric != "valid " + self._metric and metric != self._metric):
                continue
    ...
    def __call__(self, env: CallbackEnv) -> None:
        if (env.iteration + 1) % self._report_interval == 0:
            ...
            evaluation_result_list = env.evaluation_result_list
            is_cv = (
                evaluation_result_list is not None and len(evaluation_result_list) > 0 and len(evaluation_result_list[0]) == 5
            )
            if is_cv:
