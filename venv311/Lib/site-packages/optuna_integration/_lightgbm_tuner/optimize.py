...
from optuna_integration._lightgbm_tuner.alias import _handling_alias_metrics, _handling_alias_parameters
...
class _BaseTuner:
    ...
    def _get_booster_best_score(self, booster: "lgb.Booster") -> float:
        ...
        valid_sets: list["lgb.Dataset"] | tuple["lgb.Dataset", ...] | "lgb.Dataset" | None = self.lgbm_kwargs.get("valid_sets")
    ...
    def _metric_with_eval_at(self, metric: str) -> str:
        ...
        raise ValueError(f"The value of eval_at is expected to be int or a list/tuple of int. '{eval_at}' is " "specified.")
...
class _OptunaObjective(_BaseTuner):
    ...
    def _check_target_names_supported(self) -> None:
        for target_param_name in self.target_param_names:
            if target_param_name in _DEFAULT_LIGHTGBM_PARAMETERS:
                continue
            raise NotImplementedError(f"Parameter `{target_param_name}` is not supported for tuning.")
    ...
    def _postprocess(self, trial: optuna.trial.Trial, elapsed_secs: float, average_iteration_time: float) -> None:
        if self.pbar is not None:
            self.pbar.set_description(self.pbar_fmt.format(self.step_name, self.best_score))
            self.pbar.update(1)

        trial.storage.set_trial_system_attr(trial._trial_id, _ELAPSED_SECS_KEY, elapsed_secs)
        trial.storage.set_trial_system_attr(trial._trial_id, _AVERAGE_ITERATION_TIME_KEY, average_iteration_time)
        trial.storage.set_trial_system_attr(trial._trial_id, _STEP_NAME_KEY, self.step_name)
        trial.storage.set_trial_system_attr(trial._trial_id, _LGBM_PARAMS_KEY, json.dumps(self.lgbm_params))
...
class _OptunaObjectiveCV(_OptunaObjective):
    ...
    def _get_cv_scores(self, cv_results: dict[str, list[float] | "lgb.CVBooster"]) -> list[float]:
        ...
        val_scores = cv_results[metric_key] if metric_key in cv_results else cv_results["valid " + metric_key]
...
class _LightGBMBaseTuner(_BaseTuner):
    ...
    def __init__(
        ...
        *,
        ...
        if study is None:
            self.study = optuna.create_study(direction="maximize" if self.higher_is_better() else "minimize")
        else:
            self.study = study
    ...
    def tune_feature_fraction_stage2(self, n_trials: int = 6) -> None:
        ...
        param_values = np.linspace(best_feature_fraction - 0.08, best_feature_fraction + 0.08, n_trials).tolist()
    ...
    def _tune_params(
        ...
        pbar = tqdm.tqdm(total=n_trials, ascii=True) if self.auto_options["show_progress_bar"] else None
    ...
    def _create_stepwise_study(self, study: optuna.study.Study, step_name: str) -> optuna.study.Study:
...
class LightGBMTunerCV(_LightGBMBaseTuner):
    ...
    def __init__(
        ...
        train_set: "lgb.Dataset",
        num_boost_round: int = 1000,
        folds: Generator[tuple[int, int], None, None] | Iterator[tuple[int, int]] | "BaseCrossValidator" | None = None,
        nfold: int = 5,
    ...
    def get_best_booster(self) -> "lgb.CVBooster":
        ...
        if self.lgbm_kwargs.get("return_cvbooster") is not True:
            raise ValueError("LightGBMTunerCV requires `return_cvbooster=True` for method `get_best_booster()`.")
        if self._best_booster_with_trial_number is not None:
            if self._best_booster_with_trial_number[1] == self.study.best_trial.number:
