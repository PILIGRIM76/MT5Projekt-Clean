...
def _validate_scope(scope: str | None, option_name: str) -> None:
    ...
    if scope not in valid_scopes:
        raise pytest.UsageError(f"{scope!r} is not a valid {option_name}. " f"Valid scopes are: {', '.join(valid_scopes)}.")
    ...
    if pyfuncitem.get_closest_marker("asyncio") is not None:
        if is_async_test(pyfuncitem):
            ...
            for fixname, fixtures in pyfuncitem._fixtureinfo.name2fixturedefs.items():
                ...
                if asyncio_mode == Mode.STRICT and _is_coroutine_or_asyncgen(func) and not _is_asyncio_fixture_function(func):
                    warnings.warn(
                        PytestDeprecationWarning(
