...
class Chat:
    def __init__(self, params, router_obj: Optional[Any]):
        ...
        if self.params.get("acompletion", False) is True:
            ...
            self.completions: Union[AsyncCompletions, Completions] = AsyncCompletions(self.params, router_obj=router_obj)
        else:
...
async def acompletion(  # noqa: PLR0915
    ...
    **kwargs,
    ...
    if fallbacks is not None:
        ...
        if response is None:
            raise Exception("No response from fallbacks. Got none. Turn on `litellm.set_verbose=True` to see more details.")
        return response
    ...
    **kwargs,
    ...
    try:
        ...
        if stream is True:
            ...
            return CustomStreamWrapper(
                completion_stream=mock_completion_streaming_obj(model_response, mock_response=mock_response, model=model, n=n),
                model=model,
                custom_llm_provider="openai",
                logging_obj=logging,
            )
...
def completion(  # type: ignore # noqa: PLR0915
    ...
    **kwargs,
    ...
    assistant_continue_message: Optional[ChatCompletionAssistantMessage] = kwargs.get("assistant_continue_message", None)
    if headers is None:
        headers = {}
    ...
    num_retries = kwargs.get("num_retries", None)  ## alt. param for 'max_retries'. Use this to pass retries w/ instructor.
    ...
    if isinstance(litellm_logging_obj, LiteLLMLoggingObj) and (
        litellm_logging_obj.should_run_prompt_management_hooks(prompt_id=prompt_id, non_default_params=non_default_params)
    ):
        (
            model,
            messages,
    ...
    try:
        ...
        if kwargs.get("azure", False) is True:  # don't remove flag check, to remain backwards compatible for repos like Codium
        ...
        if custom_llm_provider == "azure":
            ...
            if cohere_route == "v2":
                api_base = (
                    api_base or litellm.api_base or get_secret_str("COHERE_API_BASE") or "https://api.cohere.com/v2/chat"
                )
...
def embedding(  # noqa: PLR0915
    ...
    **kwargs,
    ...
    try:
        ...
        if azure is True or custom_llm_provider == "azure":
        ...
        elif custom_llm_provider == "openai_like" or custom_llm_provider == "llamafile" or custom_llm_provider == "lm_studio":
            ...
            api_key = (
                api_key or litellm.huggingface_key or get_secret("HUGGINGFACE_API_KEY") or litellm.api_key
            )  # type: ignore
            ...
            api_base = api_base or litellm.api_base or get_secret_str("VERTEXAI_API_BASE") or get_secret_str("VERTEX_API_BASE")
            ...
            api_base = (
                litellm.api_base or api_base or get_secret_str("OLLAMA_API_BASE") or "http://localhost:11434"
            )  # type: ignore
            ...
            api_base = api_base or litellm.api_base or get_secret_str("WANDB_API_BASE") or "https://api.inference.wandb.ai/v1"
            ...
            api_base = api_base or litellm.api_base or get_secret_str("SAMBANOVA_API_BASE") or "https://api.sambanova.ai/v1"
            response = base_llm_http_handler.embedding(
                model=model,
                input=input,
                custom_llm_provider=custom_llm_provider,
        ...
        if isinstance(prompt, list):
            ...
            if len(prompt) > 0 and isinstance(prompt[0], list):
                ...
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    completed_futures = [
                        executor.submit(process_prompt, i, individual_prompt) for i, individual_prompt in enumerate(prompt)
                    ]
...
async def aadapter_completion(*, adapter_id: str, **kwargs) -> Optional[Union[BaseModel, AdapterCompletionStreamWrapper]]:
    ...
    try:
        ...
        if translation_obj is None:
            raise ValueError(
                "No matching adapter given. Received 'adapter_id'={}, litellm.adapters={}".format(adapter_id, litellm.adapters)
            )
...
def moderation(input: str, model: Optional[str] = None, api_key: Optional[str] = None, **kwargs) -> OpenAIModerationResponse:
...
def stream_chunk_builder(  # noqa: PLR0915
    ...
    try:
        ...
        if len(thinking_blocks) > 0:
            response["choices"][0]["message"]["thinking_blocks"] = processor.get_combined_thinking_content(thinking_blocks)
