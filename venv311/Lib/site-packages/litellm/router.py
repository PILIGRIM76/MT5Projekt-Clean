...
class Router:
    ...
    def __init__(  # noqa: PLR0915
        ...
        if retry_policy is not None:
            ...
            verbose_router_logger.info(
                "\033[32mRouter Custom Retry Policy Set:\n{}\033[0m".format(self.retry_policy.model_dump(exclude_none=True))
            )
    ...
    def routing_strategy_init(self, routing_strategy: Union[RoutingStrategy, str], routing_strategy_args: dict):
        ...
        elif routing_strategy == RoutingStrategy.LATENCY_BASED.value or routing_strategy == RoutingStrategy.LATENCY_BASED:
    ...
    def _completion(self, model: str, messages: List[Dict[str, str]], **kwargs) -> Union[ModelResponse, CustomStreamWrapper]:
        model_name = None
        deployment = None
        try:
    ...
    async def _acompletion(self, model: str, messages: List[Dict[str, str]], **kwargs) -> Union[  # noqa: PLR0915
        ModelResponse,
        CustomStreamWrapper,
    ]:
        """
    ...
    def _handle_clientside_credential(self, deployment: dict, kwargs: dict, function_name: Optional[str] = None) -> Deployment:
        """
    ...
    async def abatch_completion_fastest_response(
        ...
        **kwargs,
        ...
        while pending_tasks:
            done, pending_tasks = await asyncio.wait(pending_tasks, return_when=asyncio.FIRST_COMPLETED)  # type: ignore
    ...
    def _image_generation(self, prompt: str, model: str, **kwargs):
        ...
        try:
            ...
            verbose_router_logger.info(f"litellm.image_generation(model={model_name})\033[31m Exception {str(e)}\033[0m")
            if model_name is not None:
                self.fail_calls[model_name] += 1
            raise e
    ...
    async def _aimage_generation(self, prompt: str, model: str, **kwargs):
        ...
        try:
            ...
            verbose_router_logger.info(f"litellm.aimage_generation(model={model_name})\033[31m Exception {str(e)}\033[0m")
            if model_name is not None:
                self.fail_calls[model_name] += 1
            raise e
    ...
    async def _ageneric_api_call_with_fallbacks_helper(self, model: str, original_generic_function: Callable, **kwargs):
        ...
        try:
            ...
            verbose_router_logger.info(f"ageneric_api_call_with_fallbacks(model={model})\033[31m Exception {str(e)}\033[0m")
            if model is not None:
                self.fail_calls[model] += 1
            raise e
    ...
    async def _acreate_file(  # noqa: PLR0915
        ...
        **kwargs,
        ...
        try:
            ...
            verbose_router_logger.exception(f"litellm.acreate_file(model={model}, {kwargs})\033[31m Exception {str(e)}\033[0m")
            if model is not None:
                self.fail_calls[model] += 1
            raise e
    ...
    async def aretrieve_batch(
        ...
        **kwargs,
        ...
        try:
            parent_otel_span = _get_parent_otel_span_from_kwargs(kwargs)
            if model is not None:
                filtered_model_list: Optional[Union[List[DeploymentTypedDict], List[Dict], Dict]] = (
    ...
    async def _pass_through_assistants_endpoint_factory(
        ...
        **kwargs,
        ...
        return await original_function(custom_llm_provider=custom_llm_provider, client=client, **kwargs)  # type: ignore
        ...
        try:
            ...
            if isinstance(e, litellm.ContextWindowExceededError):
                if context_window_fallbacks is not None:
                    context_window_fallback_model_group: Optional[List[str]] = self._get_fallback_model_group_from_fallbacks(
                ...
                if content_policy_fallbacks is not None:
                    content_policy_fallback_model_group: Optional[List[str]] = self._get_fallback_model_group_from_fallbacks(
    ...
    async def async_deployment_callback_on_failure(self, kwargs, completion_response: Optional[Any], start_time, end_time):
        """
    ...
    @overload
    def get_router_model_info(self, deployment: dict, received_model_name: str, id: None = None) -> ModelMapInfo:
        pass
            ...
            if "*" in model:  # only call pattern_router for wildcard models
                ...
                if potential_models is not None:
                    for potential_model in potential_models:
                        try:
                            if potential_model.get("model_info", {}).get("id") == deployment.get("model_info", {}).get("id"):
        ...
        for idx, deployment in enumerate(_returned_deployments):
            ...
            try:
                ...
                if isinstance(model_info, dict) and model_info.get("max_input_tokens", None) is not None:
                    if isinstance(model_info["max_input_tokens"], int) and input_tokens > model_info["max_input_tokens"]:
        ...
        if len(healthy_deployments) == 0:
            ...
            if len(healthy_deployments) == 0:
                if self.get_model_list(model_name=model) is None:
                    ...
                    message = f"You passed in model={model}. There are no healthy deployments for this model".format(model)
        ...
        try:
            ...
            if request_kwargs is not None:
                ...
                if logging_obj is not None:
                    ...
                    asyncio.create_task(logging_obj.async_failure_handler(e, traceback_exception))  # type: ignore
            raise e
