...
def get_supports_system_message(model: str, custom_llm_provider: Literal["vertex_ai", "vertex_ai_beta", "gemini"]) -> bool:
    try:
...
def get_supports_response_schema(model: str, custom_llm_provider: Literal["vertex_ai", "vertex_ai_beta", "gemini"]) -> bool:
    _custom_llm_provider = custom_llm_provider
    if custom_llm_provider == "vertex_ai_beta":
        _custom_llm_provider = "vertex_ai"
...
def _get_embedding_url(
    ...
    if model.isdigit():
        ...
        url = (
            f"{base_url}/v1/projects/{vertex_project}/locations/{vertex_location}/publishers/google/models/{model}:{endpoint}"
        )
...
def filter_schema_fields(schema_dict: Dict[str, Any], valid_fields: Set[str], processed=None) -> Dict[str, Any]:
    ...
    for key, value in schema_dict.items():
        ...
        if key == "properties" and isinstance(value, dict):
            ...
            result[key] = [filter_schema_fields(item, valid_fields, processed) for item in value]  # type: ignore
        else:
            result[key] = value
...
class VertexAITokenCounter(BaseTokenCounter):
    ...
    async def count_tokens(
        ...
        if VertexAIPartnerModels.is_vertex_partner_model(model_to_use):
            ...
            vertex_credentials = count_tokens_params_request.get("vertex_credentials") or count_tokens_params_request.get(
