...
from litellm.types.rerank import RerankBilledUnits, RerankResponse, RerankResponseMeta, RerankResponseResult
    ...
    def transform_rerank_response(
        ...
        for record in records:
            ...
            if "score" in record:
                ...
                results.append({"index": int(record["id"]), "relevance_score": record.get("score", 0.0)})
            else:
                # Response with only IDs (when ignoreRecordDetailsInResponse=true)
                # We can't provide a relevance score, so we'll use a default
                results.append({"index": int(record["id"]), "relevance_score": 1.0})  # Default score when details are ignored
        ...
        for result in results:
            rerank_results.append(RerankResponseResult(index=result["index"], relevance_score=result["relevance_score"]))
