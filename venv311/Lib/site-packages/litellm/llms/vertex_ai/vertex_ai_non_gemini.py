...
def completion(  # noqa: PLR0915
    ...
    try:
        ...
        prompt = " ".join([message.get("content") for message in messages if isinstance(message.get("content", None), str)])
        ...
        if mode == "chat":
            ...
            request_str += (
                f"llm_model = aiplatform.gapic.PredictionServiceClient(client_options={client_options}, credentials=...)\n"
            )
        ...
        if model in litellm.vertex_language_models and response_obj is not None:
            ...
            if response_obj is not None:
                if hasattr(response_obj, "usage_metadata") and hasattr(response_obj.usage_metadata, "prompt_token_count"):
                    prompt_tokens = response_obj.usage_metadata.prompt_token_count
                    completion_tokens = response_obj.usage_metadata.candidates_token_count
            else:
...
async def async_streaming(  # noqa: PLR0915
    ...
    **optional_params,
    ...
    if mode == "chat":
        ...
        request_str += (
            f"llm_model = aiplatform.gapic.PredictionServiceAsyncClient(client_options={client_options}, credentials=...)\n"
        )
        endpoint_path = llm_model.endpoint_path(project=vertex_project, location=vertex_location, endpoint=model)
