...
class VertexAIImagenImageEditConfig(BaseImageEditConfig, VertexLLM):
    ...
    def map_openai_params(
        ...
        if "size" in filtered_params:
            mapped_params["aspectRatio"] = self._map_size_to_aspect_ratio(filtered_params["size"])  # type: ignore[arg-type]
    ...
    def transform_image_edit_request(  # type: ignore[override]
        ...
        parameters = {"sampleCount": sample_count, "editMode": edit_mode, "editConfig": {"baseSteps": base_steps}}
    ...
    def _prepare_reference_images(
        self, image: Union[FileTypes, List[FileTypes]], image_edit_optional_request_params: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _read_all_bytes(self, image: Any, depth: int = 0, max_depth: int = DEFAULT_MAX_RECURSE_DEPTH) -> bytes:
        ...
        if isinstance(image, (str, Path)):
            ...
            if not path_obj.exists():
                raise ValueError(f"Mask/image path does not exist for Vertex AI Imagen image edit: {path_obj}")
