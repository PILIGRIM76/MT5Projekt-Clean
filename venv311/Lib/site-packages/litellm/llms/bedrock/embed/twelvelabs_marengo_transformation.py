...
class TwelveLabsMarengoEmbeddingConfig:
    ...
    def _transform_request(
        ...
        input_type = cast(
            TWELVELABS_EMBEDDING_INPUT_TYPES, inference_params.get("inputType") or inference_params.get("input_type") or "text"
        )

        # Validate that async-invoke is used for video/audio
        if input_type in ["video", "audio"] and not async_invoke_route:
            raise ValueError(
                f"Input type '{input_type}' requires async_invoke route. " f"Use model format: 'bedrock/async_invoke/model_id'"
            )
    ...
    def _wrap_async_invoke_request(
        ...
        return TwelveLabsAsyncInvokeRequest(
            ...
            outputDataConfig=TwelveLabsOutputDataConfig(s3OutputDataConfig=TwelveLabsS3OutputDataConfig(s3Uri=output_s3_uri)),
        )
