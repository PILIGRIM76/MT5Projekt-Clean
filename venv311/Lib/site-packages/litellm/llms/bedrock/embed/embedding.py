...
class BedrockEmbedding(BaseAWSLLM):
    ...
    def _transform_response(
        ...
        if is_async_invoke and len(response_list) == 1 and "invocationArn" in response_list[0]:
            ...
            if model == "amazon.titan-embed-image-v1":
                ...
                returned_response = AmazonNovaEmbeddingConfig()._transform_response(response_list=response_list, model=model)
    ...
    def embeddings(  # noqa: PLR0915
        ...
        inference_params = {k: v for k, v in inference_params.items() if k.lower() not in self.aws_authentication_params}
        ...
        if provider == "cohere":
            ...
            for i in input:
                if model == "amazon.titan-embed-image-v1":
                    transformed_request: AmazonEmbeddingRequest = AmazonTitanMultimodalEmbeddingG1Config()._transform_request(
                        input=i, inference_params=inference_params
                    )
                elif model == "amazon.titan-embed-text-v1":
                    transformed_request = AmazonTitanG1Config()._transform_request(input=i, inference_params=inference_params)
                elif model == "amazon.titan-embed-text-v2:0":
                    transformed_request = AmazonTitanV2Config()._transform_request(input=i, inference_params=inference_params)
                else:
                    raise Exception(
    ...
    async def _get_async_invoke_status(self, invocation_arn: str, aws_region_name: str, logging_obj=None, **kwargs) -> dict:
        """
