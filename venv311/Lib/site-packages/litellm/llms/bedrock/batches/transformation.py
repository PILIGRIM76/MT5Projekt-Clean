...
class BedrockBatchesConfig(BaseAWSLLM, BaseBatchesConfig):
    ...
    def transform_create_batch_request(
        ...
        s3_output_config: BedrockS3OutputDataConfig = BedrockS3OutputDataConfig(s3Uri=f"s3://{output_bucket}/{output_key}")
        ...
        return {"method": "POST", "url": endpoint_url, "headers": signed_headers, "data": signed_data.decode("utf-8")}
    ...
    def _parse_timestamps_and_status(self, response_data, status_str: str):
        ...
        in_progress_at = (
            parse_timestamp(
                str(response_data.get("lastModifiedTime")) if response_data.get("lastModifiedTime") is not None else None
            )
    ...
    def _extract_errors_and_metadata(self, response_data, raw_response):
        ...
        if message:
            ...
            errors = Errors(data=[BatchError(message=message, code=str(raw_response.status_code))], object="list")
    ...
    def transform_retrieve_batch_response(
        ...
        # Parse timestamps
        created_at, in_progress_at, completed_at, failed_at, cancelled_at, expires_at = self._parse_timestamps_and_status(
