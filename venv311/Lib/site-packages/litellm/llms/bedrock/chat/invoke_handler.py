    ...
    def process_response(  # noqa: PLR0915
                ...
                if self.is_claude_messages_api_model(model):
                    ...
                    if _is_function_call is True and stream is not None and stream is True:
                        ...
                        if isinstance(model_response.choices[0], litellm.Choices):
                            if getattr(model_response.choices[0].message, "tool_calls", None) is not None and isinstance(
                            ...
                            print_verbose("Returns anthropic CustomStreamWrapper with 'cached_response' streaming object")
                    ...
                    model_response.choices[0].finish_reason = map_finish_reason(completion_response.get("stop_reason", ""))
        ...
        try:
            if (
                ...
                and getattr(model_response.choices[0].message, "tool_calls", None) is None  # type: ignore
            ):
                model_response.choices[0].message.content = outputText  # type: ignore
            elif (
                hasattr(model_response.choices[0], "message")
                and getattr(model_response.choices[0].message, "tool_calls", None) is not None  # type: ignore
            ):
                pass
            else:
                raise Exception()
        ...
        if stream and provider == "ai21":
            streaming_model_response = ModelResponse(stream=True)
            streaming_model_response.choices[0].finish_reason = model_response.choices[0].finish_reason  # type: ignore
            # streaming_model_response.choices = [litellm.utils.StreamingChoices()]
            streaming_choice = litellm.utils.StreamingChoices()
            streaming_choice.index = model_response.choices[0].index
...
class AWSEventStreamDecoder:
    ...
    def _chunk_parser(self, chunk_data: dict) -> Union[GChunk, ModelResponseStream, dict]:
        ...
        ######## /bedrock/converse mappings ###############
            ...
            "contentBlockIndex" in chunk_data or "stopReason" in chunk_data or "metrics" in chunk_data or "trace" in chunk_data
        ):
    ...
    async def aiter_bytes(self, iterator: AsyncIterator[bytes]) -> AsyncIterator[Union[GChunk, ModelResponseStream, dict]]:
