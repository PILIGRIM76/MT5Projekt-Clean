...
class BaseAWSLLM:
    ...
    def get_credentials(
        ...
        elif aws_access_key_id is not None and aws_secret_access_key is not None and aws_session_token is not None:
            credentials, _cache_ttl = self._auth_with_aws_session_token(
                aws_access_key_id=aws_access_key_id,
                aws_secret_access_key=aws_secret_access_key,
                aws_session_token=aws_session_token,
            )
        elif aws_access_key_id is not None and aws_secret_access_key is not None and aws_region_name is not None:
    ...
    def _get_aws_region_name(
        ...
        if aws_region_name is None:
            ...
            if aws_region_name is None and litellm_aws_region_name is not None and isinstance(litellm_aws_region_name, str):
                aws_region_name = litellm_aws_region_name

            standard_aws_region_name = get_secret("AWS_REGION", None)
            if aws_region_name is None and standard_aws_region_name is not None and isinstance(standard_aws_region_name, str):
    ...
    def _handle_irsa_cross_account(
        ...
        with tracer.trace("boto3.client(sts) for manual IRSA"):
            sts_client = boto3.client("sts", region_name=region, verify=self._get_ssl_verify(ssl_verify))
