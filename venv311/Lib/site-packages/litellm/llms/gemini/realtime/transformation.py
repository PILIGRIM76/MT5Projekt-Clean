...
class GeminiRealtimeConfig(BaseRealtimeConfig):
    ...
    def map_openai_params(self, optional_params: dict, non_default_params: dict) -> dict:
        ...
        for key, value in non_default_params.items():
            if key == "instructions":
                ...
                if len(transformed_audio_activity_config) > 0:  # if the config is not empty, add it to the optional params
    ...
    def transform_realtime_request(
        ...
        if "type" in json_message and json_message["type"] == "input_audio_buffer.append":
            realtime_input_dict["audio"] = HttpxBlobType(mimeType=self.get_audio_mime_type(), data=json_message["audio"])
        else:
    ...
    def transform_session_created_event(
        ...
        if _model is not None and isinstance(_model, str):
            session["model"] = _model.strip("models/")  # keep it consistent with how openai returns the model name
    ...
    def update_current_delta_chunks(
        ...
        try:
            ...
            raise ValueError(f"Error updating current delta chunks: {e}, got transformed_message: {transformed_message}")
    ...
    def map_openai_event(
        ...
        if model_turn_event:  # check if model turn event
            ...
            for map_key, openai_event in MAP_GEMINI_FIELD_TO_OPENAI_EVENT.items():
                if map_key == key or (
                    "." in map_key and GeminiRealtimeConfig.get_nested_value(json_message, map_key) is not None
                ):
