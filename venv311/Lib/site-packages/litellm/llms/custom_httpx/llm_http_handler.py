...
class BaseLLMHTTPHandler:
    ...
    async def async_completion(
        ...
        if client is None:
            verbose_logger.debug(f"Creating HTTP client with shared_session: {id(shared_session) if shared_session else None}")
    ...
    async def async_anthropic_messages_handler(
        ...
        request_url = anthropic_messages_provider_config.get_complete_url(
            ...
            api_key=api_key,
            model=model,
            optional_params=dict(litellm_params),  # this uses the invoke config, which expects aws_* params in optional_params
            litellm_params=dict(litellm_params),
            stream=stream,
        )
    ...
    async def async_delete_response_api_handler(
        ...
        try:
            response = await async_httpx_client.delete(url=url, headers=headers, json=data, timeout=timeout)
    ...
    def delete_response_api_handler(
        ...
        try:
            response = sync_httpx_client.delete(url=url, headers=headers, json=data, timeout=timeout)
    ...
    async def _call_agentic_completion_hooks(
        ...
        for callback in callbacks:
            try:
                ...
                verbose_logger.exception(f"LiteLLM.AgenticHookError: Exception in agentic completion hooks: {str(e)}")
        ...
        if websearch_converted_stream:
            ...
            verbose_logger.debug("WebSearchInterception: No tool call made, converting non-streaming response to fake stream")
    ...
    async def _call_agentic_chat_completion_hooks(
        ...
        for callback in callbacks:
            try:
                ...
                verbose_logger.exception(f"LiteLLM.AgenticHookError: Exception in chat completion agentic hooks: {str(e)}")
    ...
    async def async_vector_store_search_handler(
        ...
        headers = vector_store_provider_config.validate_environment(headers=extra_headers or {}, litellm_params=litellm_params)
    ...
    async def async_vector_store_file_retrieve_handler(
        ...
        *,
        ...
        url, request_params = vector_store_files_provider_config.transform_retrieve_vector_store_file_request(
            vector_store_id=vector_store_id,
            file_id=file_id,
            api_base=api_base,
        )
    ...
    async def async_vector_store_file_content_handler(
        ...
        *,
        ...
        url, request_params = vector_store_files_provider_config.transform_retrieve_vector_store_file_content_request(
            vector_store_id=vector_store_id,
            file_id=file_id,
            api_base=api_base,
        )
        ...
        return vector_store_files_provider_config.transform_retrieve_vector_store_file_content_response(response=response)
    ...
    async def async_vector_store_file_delete_handler(
        ...
        *,
        ...
        url, request_params = vector_store_files_provider_config.transform_delete_vector_store_file_request(
            vector_store_id=vector_store_id,
            file_id=file_id,
            api_base=api_base,
        )
    ...
    async def async_create_skill_handler(
        ...
        try:
            ...
            if files is not None:
                response = await async_httpx_client.post(url=url, headers=headers, data=data, files=files, timeout=timeout)
            else:
                # No files - send as JSON
                response = await async_httpx_client.post(url=url, headers=headers, json=request_body, timeout=timeout)
        except Exception as e:
