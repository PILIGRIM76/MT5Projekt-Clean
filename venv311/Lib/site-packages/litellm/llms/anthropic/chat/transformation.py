...
class AnthropicConfig(AnthropicModelInfo, BaseConfig):
        ...
        if returned_tool is not None:
            ...
            if tool_type not in (
                ...
                if _allowed_callers is not None:
                    if not isinstance(_allowed_callers, list) or not all(isinstance(item, str) for item in _allowed_callers):
    ...
    def _map_tools(self, tools: List) -> Tuple[List[AllAnthropicToolsValues], List[AnthropicMcpServerTool]]:
        anthropic_tools = []
        mcp_servers = []
        for tool in tools:
    ...
    def map_openai_params(  # noqa: PLR0915
        ...
        for param, value in non_default_params.items():
            ...
            if param == "tools":
                ...
                optional_params = self._add_tools_to_optional_params(optional_params=optional_params, tools=anthropic_tools)
                if mcp_servers:
            ...
            if param == "thinking":
                ...
                optional_params["thinking"] = AnthropicConfig._map_reasoning_effort(reasoning_effort=value, model=model)
    ...
    def _ensure_context_management_beta_header(self, headers: dict, context_management: dict) -> None:
        """
        ...
        if has_other:
            self._ensure_beta_header(headers, ANTHROPIC_BETA_HEADER_VALUES.CONTEXT_MANAGEMENT_2025_06_27.value)
        ...
        if "output_config" in optional_params:
            ...
            if output_config and isinstance(output_config, dict):
                ...
                if effort and effort not in ["high", "medium", "low"]:
                    raise ValueError(f"Invalid effort value: {effort}. Must be one of: 'high', 'medium', 'low'")
    ...
    def _transform_response_for_json_mode(
        ...
        if json_mode is True and len(tool_calls) == 1:
            ...
            if "name" in tool_calls[0]["function"] and tool_calls[0]["function"]["name"] == RESPONSE_FORMAT_TOOL_NAME:
