...
class SagemakerLLM(BaseAWSLLM):
    ...
    def _prepare_request(
        ...
        if optional_params.get("stream") is True:
            api_base = (
                f"https://runtime.sagemaker.{aws_region_name}.amazonaws.com/endpoints/{model}/invocations-response-stream"
            )
        else:
        ...
        if extra_headers is not None and "Authorization" in extra_headers:  # prevent sigv4 from overwriting the auth header
    ...
    def embedding(
        ...
        mock_response = HttpxResponse(
            status_code=200, content=json.dumps(response).encode("utf-8"), headers={"content-type": "application/json"}
        )
