...
if TYPE_CHECKING:
    from litellm.proxy._types import ManagementEndpointLoggingPayload as _ManagementEndpointLoggingPayload
...
class OpenTelemetry(CustomLogger):
    ...
    def _init_logs(self, logger_provider):
        ...
        def create_logger_provider():
            ...
            provider.add_log_record_processor(BatchLogRecordProcessor(log_exporter))  # type: ignore[arg-type]
            return provider
    ...
    def set_attributes(self, span: Span, kwargs, response_obj: Optional[Any]):  # noqa: PLR0915
        try:
            ...
            # get hidden params
            hidden_params = getattr(standard_logging_payload, "hidden_params", None) or (standard_logging_payload or {}).get(
                "hidden_params", {}
            )
            if hidden_params:
                self.safe_set_attribute(
            ...
            if response_obj is not None:
                if response_obj.get("choices"):
                    transformed_choices = self._transform_choices_to_otel_semantic_conventions(response_obj.get("choices"))
                    self.safe_set_attribute(
                        span=span,
