    ...
    def __init__(self):
        try:
            ...
            if packaging.version.Version(version) < packaging.version.Version("0.1.43"):  # type: ignore
                print("Lunary version outdated. Required: >= 0.1.43. Upgrade via 'pip install lunary --upgrade'")  # noqa
                raise ImportError

            self.lunary_client = lunary
        except ImportError:
            print("Lunary not installed. Please install it using 'pip install lunary'")  # noqa  # noqa
            raise ImportError
