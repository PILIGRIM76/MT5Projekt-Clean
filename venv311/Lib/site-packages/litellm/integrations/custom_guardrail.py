...
class CustomGuardrail(CustomLogger):
    ...
    def _validate_event_hook(
        ...
        if isinstance(event_hook, list):
            ...
            _validate_event_hook_list_is_in_supported_event_hooks(list(event_hook.tags.values()), supported_event_hooks)
            if event_hook.default:
                _validate_event_hook_list_is_in_supported_event_hooks([event_hook.default], supported_event_hooks)
    ...
    async def async_pre_call_deployment_hook(self, kwargs: Dict[str, Any], call_type: Optional[CallTypes]) -> Optional[dict]:
        ...
        if self.default_on is True and disable_global_guardrail is not True:
            if self._event_hook_is_event_type(event_type):
                if isinstance(self.event_hook, Mode):
                    ...
                    result = EnterpriseCustomGuardrailHelper._should_run_if_mode_by_tag(data, self.event_hook)
    ...
    def add_standard_logging_guardrail_information_to_request_data(
        ...
        if "metadata" in request_data:
            ...
            verbose_logger.warning("unable to log guardrail information. No metadata found in request_data")
