...
"""
    ...
    async def region_outage_alerts(
        ...
        outage_value: Optional[ProviderRegionOutageModel] = await self.internal_usage_cache.async_get_cache(key=cache_key)
...
"""
    ...
    async def send_monthly_spend_report(self):
        ...
        try:
            ...
            _event_cache_key = (
                f"monthly_spend_report_sent_{first_day_of_month.strftime('%Y-%m-%d')}_{last_day_of_month.strftime('%Y-%m-%d')}"
            )
            if await self.internal_usage_cache.async_get_cache(key=_event_cache_key):
                return
