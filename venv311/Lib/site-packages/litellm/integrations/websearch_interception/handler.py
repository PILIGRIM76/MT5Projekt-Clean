...
class WebSearchInterceptionLogger(CustomLogger):
    ...
    async def async_pre_request_hook(self, model: str, messages: List[Dict], kwargs: Dict) -> Optional[Dict]:
        ...
        verbose_logger.debug(f"WebSearchInterception: Tools after conversion: {[t.get('name') for t in converted_tools]}")
    ...
    async def async_run_chat_completion_agentic_loop(
        ...
        verbose_logger.debug(f"WebSearchInterception: Executing chat completion agentic loop for {len(tool_calls)} search(es)")
    ...
    async def _execute_agentic_loop(
        ...
        try:
            ...
            max_tokens = anthropic_messages_optional_request_params.get(
                "max_tokens", kwargs.get("max_tokens", 1024)  # Default to 1024 if not found
            )
            ...
            kwargs_for_followup = {k: v for k, v in kwargs.items() if not k.startswith("_websearch_interception")}
            ...
            verbose_logger.debug(f"WebSearchInterception: Follow-up request completed, response type: {type(final_response)}")
        ...
        try:
            ...
            if llm_router is not None and hasattr(llm_router, "search_tools"):
                if self.search_tool_name:
                    ...
                    matching_tools = [
                        tool for tool in llm_router.search_tools if tool.get("search_tool_name") == self.search_tool_name
                    ]
                    if matching_tools:
            ...
            verbose_logger.debug(f"WebSearchInterception: Executing search for '{query}' using provider '{search_provider}'")
            ...
            verbose_logger.debug(f"WebSearchInterception: Search completed for '{query}', got {len(search_result_text)} chars")
