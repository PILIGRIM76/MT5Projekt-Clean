...
async def _mock_async_handler_get(self, url, params=None, headers=None, follow_redirects=None):
    ...
    if isinstance(url, str) and "storage.googleapis.com" in url:
        ...
        return MockResponse(status_code=200, json_data=mock_payload, url=url, elapsed_seconds=_MOCK_LATENCY_SECONDS)
    if _original_async_handler_get is not None:
        return await _original_async_handler_get(
            self, url=url, params=params, headers=headers, follow_redirects=follow_redirects
        )
    raise RuntimeError("Original AsyncHTTPHandler.get not available")


async def _mock_async_handler_delete(
    self, url, data=None, json=None, params=None, headers=None, timeout=None, stream=False, content=None
):
    ...
    if isinstance(url, str) and "storage.googleapis.com" in url:
        ...
        return MockResponse(
            status_code=204, json_data=None, url=url, elapsed_seconds=_MOCK_LATENCY_SECONDS  # Empty body for DELETE
        )
