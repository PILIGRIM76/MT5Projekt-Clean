...
class GenerateContentHelper:
    ...
    def setup_generate_content_call(
        ...
        **kwargs,
        ...
        if not kwargs.get("stream", False) and litellm_params.mock_response and isinstance(litellm_params.mock_response, str):
...
async def agenerate_content_stream(
    ...
    **kwargs,
    ...
    try:
        ...
        # get custom llm provider so we can use this for mapping exceptions
        if custom_llm_provider is None:
            _, custom_llm_provider, _, _ = litellm.get_llm_provider(model=model, api_base=local_vars.get("base_url", None))
