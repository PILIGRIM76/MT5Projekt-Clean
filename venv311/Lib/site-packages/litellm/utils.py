...
def _custom_logger_class_exists_in_success_callbacks(
    ...
    """
    return any(isinstance(cb, type(callback_class)) for cb in litellm.success_callback + litellm._async_success_callback)
...
def _custom_logger_class_exists_in_failure_callbacks(
    ...
    Prevents double adding a custom logger callback to the litellm callbacks
    """
    return any(isinstance(cb, type(callback_class)) for cb in litellm.failure_callback + litellm._async_failure_callback)
...
def _process_tool_message_id(msg_copy: dict, thought_signature_separator: str) -> dict:
    ...
    if msg_copy.get("role") == "tool" and isinstance(msg_copy.get("tool_call_id"), str):
        if thought_signature_separator in msg_copy["tool_call_id"]:
            msg_copy["tool_call_id"] = _remove_thought_signature_from_id(msg_copy["tool_call_id"], thought_signature_separator)
...
def function_setup(  # noqa: PLR0915
    ...
    try:
        ...
        if (len(litellm.input_callback) > 0 or len(litellm.success_callback) > 0 or len(litellm.failure_callback) > 0) and len(
        ...
        if kwargs.get("success_callback", None) is not None and isinstance(kwargs["success_callback"], list):
            ...
            for index, callback in enumerate(kwargs["success_callback"]):
                if coroutine_checker.is_async_callable(callback) or callback == "dynamodb" or callback == "s3":
                    if dynamic_async_success_callbacks is not None and isinstance(dynamic_async_success_callbacks, list):
            ...
            if isinstance(messages, list) and len(messages) > 0:
                try:
                    ...
                    if not _is_gemini_model(model, custom_llm_provider):
                        ...
                        processed_messages = _remove_thought_signatures_from_messages(messages, THOUGHT_SIGNATURE_SEPARATOR)
...
def _supports_provider_info_factory(model: str, custom_llm_provider: Optional[str], key: str) -> Optional[Literal[True]]:
    """
...
def register_model(model_cost: Union[str, dict]):  # noqa: PLR0915
    ...
    for key, value in loaded_model_cost.items():
        ...
        if provider in _skip_get_model_info_providers or any(key.startswith(f"{p}/") for p in _skip_get_model_info_providers):
    ...
    **kwargs,
    ...
    if custom_llm_provider == "anthropic":
        ...
        if litellm.AzureOpenAIO1Config().is_o_series_model(model=model):
            ...
            api_version = (
                api_version or litellm.api_version or get_secret("AZURE_API_VERSION") or litellm.AZURE_DEFAULT_API_VERSION
            )
...
def _infer_model_region(litellm_params: LiteLLM_Params) -> Optional[AllowedModelRegion]:
    ...
    """
    model, custom_llm_provider, _, _ = litellm.get_llm_provider(model=litellm_params.model, litellm_params=litellm_params)
...
def get_api_key(llm_provider: str, dynamic_api_key: Optional[str]):
    ...
    # together_ai
        ...
        api_key = api_key or litellm.togetherai_api_key or get_secret("TOGETHERAI_API_KEY") or get_secret("TOGETHER_AI_TOKEN")
    # nebius
...
def _check_provider_match(model_info: dict, custom_llm_provider: Optional[str]) -> bool:
    ...
    """
    if custom_llm_provider and ("litellm_provider" in model_info and model_info["litellm_provider"] != custom_llm_provider):
...
def _is_potential_model_name_in_model_cost(
    ...
    """
    return any(
        _get_model_cost_key(str(potential_model_name)) is not None for potential_model_name in potential_model_names.values()
    )
    ...
    try:
        ...
        if custom_llm_provider == "huggingface":
            ...
            return ModelInfoBase(
                ...
                cache_creation_input_token_cost_above_1hr=_model_info.get("cache_creation_input_token_cost_above_1hr", None),
                ...
                output_cost_per_token_above_128k_tokens=_model_info.get("output_cost_per_token_above_128k_tokens", None),
                output_cost_per_character_above_128k_tokens=_model_info.get(
                    "output_cost_per_character_above_128k_tokens", None
                ),
                output_cost_per_token_above_200k_tokens=_model_info.get("output_cost_per_token_above_200k_tokens", None),
...
def process_messages(messages, max_tokens, model):
    ...
    messages = messages[::-1]
    final_messages = []
    verbose_logger.debug(f"calling process_messages with messages: {messages}, max_tokens: {max_tokens}, model: {model}")
...
def is_base64_encoded(s: str) -> bool:
    try:
        ...
        if not s.startswith("data:"):  # require `data:` for base64 str, like openai. Prevents false positives like s='Dog'
            return False
...
def validate_openai_optional_params(stop: Optional[Union[str, List[str]]] = None, **kwargs) -> Optional[Union[str, List[str]]]:
    """
...
class ProviderConfigManager:
    ...
    def get_provider_embedding_config(
        ...
        if litellm.LlmProviders.VOYAGE == provider and litellm.VoyageContextualEmbeddingConfig.is_contextualized_embeddings(
            model
        ):
    ...
    def get_provider_model_info(
        ...
        if LlmProviders.FIREWORKS_AI == provider:
            ...
            from litellm.llms.vllm.common_utils import VLLMModelInfo  # experimental approach, to reduce bloat on __init__.py
...
def add_openai_metadata(
    ...
    visible_metadata: Dict[str, str] = {str(k): v for k, v in metadata.items() if k != "hidden_params" and isinstance(v, str)}
