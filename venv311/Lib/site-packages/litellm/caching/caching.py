...
class Cache:
    ...
    def _add_namespace_to_cache_key(self, hash_hex: str, **kwargs) -> str:
        ...
        namespace = (
            dynamic_cache_control.get("namespace") or kwargs.get("metadata", {}).get("redis_namespace") or self.namespace
        )
        if namespace:
        ...
        if cached_result is not None and isinstance(cached_result, dict) and "timestamp" in cached_result:
            ...
            try:
                if isinstance(cached_response, dict):
                    ...
                    cached_response = json.loads(cached_response)  # type: ignore  # Convert string to dictionary
            except Exception:
