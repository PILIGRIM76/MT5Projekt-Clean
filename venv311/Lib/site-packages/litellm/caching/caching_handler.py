...
class LLMCachingHandler:
    ...
    async def _async_get_cache(
        ...
        if ((kwargs.get("caching", None) is None and litellm.cache is not None) or kwargs.get("caching", False) is True) and (
            ...
            if litellm.cache is not None and self._is_call_type_supported_by_cache(original_function=original_function):
                ...
                if cached_result is not None and not isinstance(cached_result, list):
                    ...
                    if (isinstance(cached_result, BaseModel) or isinstance(cached_result, CustomStreamWrapper)) and hasattr(
                        cached_result, "_hidden_params"
                    ):
    ...
    def _combine_cached_embedding_response_with_api_result(
        ...
        _caching_handler_response.final_embedding_cached_response._response_ms = (end_time - start_time).total_seconds() * 1000
    ...
    async def _retrieve_from_cache(self, call_type: str, kwargs: Dict[str, Any], args: Tuple[Any, ...]) -> Optional[Any]:
        """
        Internal method to
        - get cache key
        ...
        if call_type == CallTypes.aembedding.value:
            ...
            if litellm.cache._supports_async() is True:
                ...
                cached_result = await litellm.cache.async_get_cache(dynamic_cache_object=self.dual_cache, **new_kwargs)
            else:  # fallback for caches that don't support async
                cached_result = litellm.cache.get_cache(dynamic_cache_object=self.dual_cache, **new_kwargs)
    ...
    def _convert_cached_result_to_model_response(
        ...
        if (call_type == CallTypes.atext_completion.value or call_type == CallTypes.text_completion.value) and isinstance(
            cached_result, dict
        ):
            if kwargs.get("stream", False) is True:
                cached_result = self._convert_cached_stream_response(
        ...
        elif (call_type == CallTypes.arerank.value or call_type == CallTypes.rerank.value) and isinstance(cached_result, dict):
            cached_result = convert_to_model_response_object(
                response_object=cached_result,
        ...
        if self._should_store_result_in_cache(original_function=original_function, kwargs=new_kwargs):
            if (
                ...
                if (
                    ...
                    asyncio.create_task(
                        litellm.cache.async_add_cache_pipeline(result, dynamic_cache_object=self.dual_cache, **new_kwargs)
                    )
                else:
                    asyncio.create_task(
