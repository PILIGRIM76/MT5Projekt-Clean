    ...
    def log_success_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            verbose_router_logger.error(
                "litellm.router_strategy.lowest_tpm_rpm.py::async_log_success_event(): Exception occured - {}".format(str(e))
            )
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            if kwargs["litellm_params"].get("metadata") is None:
                ...
                await self.router_cache.async_set_cache(key=tpm_key, value=request_count_dict, ttl=self.routing_args.ttl)

                ## RPM
                request_count_dict = await self.router_cache.async_get_cache(key=rpm_key) or {}
                request_count_dict[id] = request_count_dict.get(id, 0) + 1

                await self.router_cache.async_set_cache(key=rpm_key, value=request_count_dict, ttl=self.routing_args.ttl)
