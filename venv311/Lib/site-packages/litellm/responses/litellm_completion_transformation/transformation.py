...
class LiteLLMCompletionResponsesConfig:
    ...
    def transform_responses_api_request_to_chat_completion_request(
        ...
        **kwargs,
        ...
        litellm_completion_request: dict = {
            ...
            "model": model,
            "tool_choice": LiteLLMCompletionResponsesConfig._transform_tool_choice(responses_api_request.get("tool_choice")),
            "tools": tools,
            ...
            for _input in input:
                ...
                if LiteLLMCompletionResponsesConfig._is_input_item_tool_call_output(input_item=_input):
                    ...
                    for m in chat_completion_messages:
                        ...
                        if role == "assistant":
                            tool_calls: Any = m.get("tool_calls") if isinstance(m, dict) else getattr(m, "tool_calls", None)
                            call_id = ""
                            if (
                                ...
                                call_id_raw = (
                                    first_call.get("id") if isinstance(first_call, dict) else getattr(first_call, "id", None)
                                )
                                if call_id_raw:
    ...
    def _deduplicate_tool_call_output_messages(
        ...
        for tool_call_message in tool_call_output_messages:
            ...
            if role == "assistant":
                ...
                if isinstance(tool_calls, Sequence) and not isinstance(tool_calls, (str, bytes)) and len(tool_calls) > 0:
                    first_call = tool_calls[0]
                    call_id_raw = None
    ...
    def _ensure_tool_results_have_corresponding_tool_calls(
        ...
        for i, message in enumerate(fixed_messages):
            ...
            if not tool_call_id and prev_assistant_idx is not None:
                ...
                tool_call_id = LiteLLMCompletionResponsesConfig._recover_tool_call_id_from_assistant(prev_assistant, message)
                if tool_call_id:
    ...
    def _transform_input_image_item_to_image_item(
        ...
        """
        image_url_obj = ChatCompletionImageUrlObject(url=item.get("image_url") or "", detail=item.get("detail") or "auto")
            ...
            for item in content:
                if isinstance(item, str):
                    ...
                    if item.get("type") == "input_file":
                        content_list.append(LiteLLMCompletionResponsesConfig._transform_input_file_item_to_file_item(item))
        ...
        if provider_specific_fields and not isinstance(provider_specific_fields, dict):
            ...
            if provider_fields:
                provider_specific_fields = (
                    ...
                    else (dict(provider_fields) if hasattr(provider_fields, "__dict__") else {})  # type: ignore
                )
    ...
    def transform_chat_completion_response_to_responses_api_response(
        ...
        responses_api_response: ResponsesAPIResponse = ResponsesAPIResponse(
            ...
            reasoning=Reasoning(),
            status=LiteLLMCompletionResponsesConfig._map_chat_completion_finish_reason_to_responses_status(finish_reason),
            text={},
