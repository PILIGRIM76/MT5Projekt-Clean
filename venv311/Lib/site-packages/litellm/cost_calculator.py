...
from litellm.llms.azure.cost_calculation import cost_per_token as azure_openai_cost_per_token
from litellm.llms.azure_ai.cost_calculator import cost_per_token as azure_ai_cost_per_token
from litellm.llms.base_llm.search.transformation import SearchResponse
from litellm.llms.bedrock.cost_calculation import cost_per_token as bedrock_cost_per_token
from litellm.llms.databricks.cost_calculator import cost_per_token as databricks_cost_per_token
from litellm.llms.deepseek.cost_calculator import cost_per_token as deepseek_cost_per_token
from litellm.llms.fireworks_ai.cost_calculator import cost_per_token as fireworks_ai_cost_per_token
from litellm.llms.gemini.cost_calculator import cost_per_token as gemini_cost_per_token
from litellm.llms.lemonade.cost_calculator import cost_per_token as lemonade_cost_per_token
from litellm.llms.openai.cost_calculation import cost_per_second as openai_cost_per_second
...
from litellm.llms.vertex_ai.cost_calculator import cost_per_token as google_cost_per_token
...
def cost_per_token(  # noqa: PLR0915
    ...
    if call_type == "speech" or call_type == "aspeech":
        ...
        return batch_cost_calculator(usage=usage_block, model=model, custom_llm_provider=custom_llm_provider)
        ...
        return azure_openai_cost_per_token(model=model, usage=usage_block, response_time_ms=response_time_ms)
        ...
        from litellm.llms.dashscope.cost_calculator import cost_per_token as dashscope_cost_per_token

        return dashscope_cost_per_token(model=model, usage=usage_block)
    elif custom_llm_provider == "azure_ai":
        return azure_ai_cost_per_token(model=model, usage=usage_block, response_time_ms=response_time_ms)
    else:
        model_info = _cached_get_model_info_helper(model=model, custom_llm_provider=custom_llm_provider)

        if model_info.get("input_cost_per_token", 0) > 0 or model_info.get("output_cost_per_token", 0) > 0:
...
def _select_model_name_for_cost_calc(
    ...
    if (
        return_model is not None and custom_llm_provider is not None and not _model_contains_known_llm_provider(return_model)
            ...
            try:
                ...
                if completion_response is not None and (
                    ...
                    if ResponseAPILoggingUtils._is_response_api_usage(_usage):
                        _usage = ResponseAPILoggingUtils._transform_response_api_usage_to_chat_usage(_usage).model_dump()
                ...
                if CostCalculatorUtils._call_type_has_image_response(call_type) and isinstance(
                    ...
                    audio_transcription_file_duration = getattr(completion_response, "duration", 0.0)
