...
async def pass_through_request(  # noqa: PLR0915
    ...
    try:
        ...
        verbose_proxy_logger.debug(
            "Pass through endpoint sending request to \nURL {}\nheaders: {}\nbody: {}\n".format(url, headers, _parsed_body)
        )
...
def create_pass_through_route(
    ...
    try:
        ...
        async def endpoint_func(  # type: ignore
            ...
            passthrough_params = InitPassThroughEndpointHelpers.get_registered_pass_through_route(route=path)
            target_params = {
                "target": target,
                "custom_headers": custom_headers,
            ...
            final_custom_body = custom_body_data if isinstance(custom_body_data, dict) or custom_body_data is None else None
...
async def websocket_passthrough_request(  # noqa: PLR0915
    ...
    try:
        ...
        async with connect(
            ...
            verbose_proxy_logger.info(f"WebSocket passthrough ({endpoint}): Upstream connection established successfully")
            ...
            async def forward_client_to_upstream() -> None:
                ...
                try:
                    ...
                    verbose_proxy_logger.exception(f"WebSocket passthrough ({endpoint}): error forwarding client message")
                    await upstream_ws.close()
            ...
            async def forward_upstream_to_client() -> None:
                ...
                try:
                    ...
                    verbose_proxy_logger.exception(f"WebSocket passthrough ({endpoint}): error forwarding upstream message")
                    raise
    ...
    try:
        ...
        if isinstance(setup_response, dict):
            if "model" in setup_response:
                ...
                "setup" in setup_response and isinstance(setup_response["setup"], dict) and "model" in setup_response["setup"]
            ):
...
class InitPassThroughEndpointHelpers:
    ...
    def remove_endpoint_routes(endpoint_id: str):
        ...
        keys_to_remove = [key for key, value in _registered_pass_through_routes.items() if value["endpoint_id"] == endpoint_id]
    ...
    def is_registered_pass_through_route(route: str) -> bool:
        ...
        for key in _registered_pass_through_routes.keys():
            ...
            if len(parts) == 3:
                route_type = parts[1]
                registered_path = InitPassThroughEndpointHelpers._build_full_path_with_root(parts[2])
...
async def _filter_endpoints_by_team_allowed_routes(
    ...
    if team_metadata is not None and team_metadata.get("allowed_passthrough_routes") is not None:
        ...
        pass_through_endpoints = [
            endpoint for endpoint in pass_through_endpoints if endpoint.path in team_metadata.get("allowed_passthrough_routes")
        ]
