...
async def milvus_proxy_route(
    ...
    is_vector_store_index = litellm.vector_store_index_registry.is_vector_store_index(vector_store_index_name=collection_name)
    ...
    base_target_url = provider_config.get_complete_url(api_base=litellm_params.get("api_base"), litellm_params=litellm_params)
...
async def handle_bedrock_passthrough_router_model(
    ...
    verbose_proxy_logger.debug(f"Bedrock router passthrough: model='{model}', endpoint='{endpoint}', streaming={is_streaming}")
...
async def azure_proxy_route(
    ...
    if len(parts) > 1 and llm_router:
        for part in parts:
            # check if LLM MODEL
            is_router_model = is_passthrough_request_using_router_model(request_body={"model": part}, llm_router=llm_router)
            # check if vector store index
...
async def vertex_discovery_proxy_route(
    ...
    if vector_store_id_match:
        ...
        vector_store_credentials = passthrough_endpoint_router.get_vector_store_credentials(vector_store_id=vector_store_id)
