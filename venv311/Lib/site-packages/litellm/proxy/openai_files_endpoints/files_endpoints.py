...
async def create_file(  # noqa: PLR0915
    ...
    try:
        ...
        _create_file_request = CreateFileRequest(
            file=file_data, purpose=cast(CREATE_FILE_REQUESTS_PURPOSE, purpose), expires_after=expires_after, **data
        )
        ...
        verbose_proxy_logger.exception("litellm.proxy.proxy_server.create_file(): Exception occured - {}".format(str(e)))
    ...
    try:
        ...
        if is_base64_unified_file_id:
            ...
            if hasattr(managed_files_obj, "prisma_client") and getattr(managed_files_obj, "prisma_client", None):
                ...
                db_file = await prisma_client.db.litellm_managedfiletable.find_first(where={"unified_file_id": file_id})
...
async def delete_file(
    ...
    try:
        ...
        if should_route:
            ...
            verbose_proxy_logger.debug(
                f"Deleted file using model: {model_used}" + (f", original_id: {original_file_id}" if original_file_id else "")
            )
        ...
        verbose_proxy_logger.exception("litellm.proxy.proxy_server.delete_file(): Exception occured - {}".format(str(e)))
        if isinstance(e, HTTPException):
            raise ProxyException(
                message=getattr(e, "message", str(e.detail)),
                type=getattr(e, "type", "None"),
