...
async def get_batch_from_database(
    ...
    try:
        ...
        db_batch_object = await prisma_client.db.litellm_managedobjecttable.find_first(where={"unified_object_id": batch_id})
...
async def update_batch_in_database(
    ...
    try:
        ...
        if db_batch_object:
            verbose_proxy_logger.info(f"Updating batch {batch_id} status from {db_batch_object.status} to {response.status}")
        else:
