...
async def get_next_version_for_prompt(prisma_client, prompt_id: str) -> int:
    ...
    """
    existing_prompts = await prisma_client.db.litellm_prompttable.find_many(where={"prompt_id": prompt_id})
...
async def get_prompt_versions(
    ...
    all_prompts = list(IN_MEMORY_PROMPT_REGISTRY.IN_MEMORY_PROMPTS.values())
    prompt_versions = [prompt for prompt in all_prompts if get_base_prompt_id(prompt_id=prompt.prompt_id) == base_prompt_id]
...
async def create_prompt(
    ...
    try:
        ...
        new_version = await get_next_version_for_prompt(prisma_client=prisma_client, prompt_id=request.prompt_id)
...
async def update_prompt(
    ...
    try:
        ...
        existing_prompts = await prisma_client.db.litellm_prompttable.find_many(where={"prompt_id": base_prompt_id})

        if not existing_prompts:
            raise HTTPException(status_code=404, detail=f"Prompt with ID {base_prompt_id} not found")
        ...
        new_version = await get_next_version_for_prompt(prisma_client=prisma_client, prompt_id=base_prompt_id)
...
async def delete_prompt(
    ...
    try:
        ...
        await prisma_client.db.litellm_prompttable.delete_many(where={"prompt_id": base_prompt_id})
...
async def patch_prompt(
    ...
    try:
        ...
        updated_litellm_params = (
            request.litellm_params if request.litellm_params is not None else existing_prompt.litellm_params
        )

        updated_prompt_info = request.prompt_info if request.prompt_info is not None else existing_prompt.prompt_info
        ...
        initialized_prompt = IN_MEMORY_PROMPT_REGISTRY.initialize_prompt(prompt=updated_prompt_spec, config_file_path=None)
