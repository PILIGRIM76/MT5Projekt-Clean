...
async def get_per_user_analytics(
    ...
    try:
        ...
        api_key_records = await prisma_client.db.litellm_verificationtoken.find_many(where={"token": {"in": list(api_keys)}})
        ...
        user_records = await prisma_client.db.litellm_usertable.find_many(where={"user_id": {"in": user_ids}})
        ...
        results = sorted(list(user_metrics.values()), key=lambda x: x.successful_requests, reverse=True)
