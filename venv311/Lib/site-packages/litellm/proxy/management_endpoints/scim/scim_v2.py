...
class UserProvisionerHelpers:
    ...
    @staticmethod
    async def handle_existing_user_by_email(prisma_client, new_user_request: NewUserRequest) -> Optional[SCIMUser]:
        """
        ...
        existing_user = await prisma_client.db.litellm_usertable.find_first(where={"user_email": new_user_request.user_email})
        ...
        for member in group.members:
            ...
            if user:
                ...
                if scim_upsert_user:
                    ...
                    created_user = await _create_user_if_not_exists(user_id=user_id, created_via="scim_group_membership")
                    if created_user:
...
async def create_user(
    ...
    try:
        ...
        if user.userName:
            existing_user = await prisma_client.db.litellm_usertable.find_unique(where={"user_id": user.userName})
            if existing_user:
                raise HTTPException(
                    status_code=409,
...
async def update_user(
    ...
    try:
        ...
        metadata = _build_scim_metadata(user_data["given_name"], user_data["family_name"], user_data["active"])
    ...
    try:
        ...
        for team in teams:
            ...
            if user_id in current_members:
                ...
                await prisma_client.db.litellm_teamtable.update(where={"team_id": team.team_id}, data={"members": new_members})
...
async def create_group(
    ...
    try:
        ...
        existing_team = await prisma_client.db.litellm_teamtable.find_unique(where={"team_id": team_id})
        ...
        for member_id in existing_team.members or []:
            ...
            if user:
                ...
                if group_id in current_teams:
                    ...
                    await prisma_client.db.litellm_usertable.update(where={"user_id": member_id}, data={"teams": new_teams})
        ...
        if path == "displayname":
            ...
            for member_id in member_values:
                ...
                if user:
                    ...
                    if scim_upsert_user:
                        # Create the user if they don't exist (backward compatible behavior)
                        created_user = await _create_user_if_not_exists(user_id=member_id, created_via="scim_group_patch")
                        if created_user:
                            valid_members.append(member_id)
                        # If creation failed, user is skipped (logged in helper)
                    else:
...
async def _apply_group_patch_updates(group_id: str, update_data: Dict[str, Any], final_members: Set[str], prisma_client):
...
async def patch_group(
    ...
    try:
        ...
        refreshed_team = await prisma_client.db.litellm_teamtable.find_unique(where={"team_id": group_id})
        if refreshed_team:
        ...
        final_team = await prisma_client.db.litellm_teamtable.find_unique(where={"team_id": group_id})
        if final_team:
