    ...
    for role in role_hierarchy:
        if role in role_mappings.roles:
            ...
            if isinstance(role_groups, list) and user_groups_set.intersection(set(role_groups)):
                verbose_proxy_logger.debug(f"User groups {user_groups} matched role '{role.value}' via groups: {role_groups}")
                return role
...
def generic_response_convertor(
    ...
    if role_mappings is not None and role_mappings.provider.lower() in [
        ...
        if user_groups:
            ...
            user_role = role_mappings.default_role
            verbose_proxy_logger.debug(f"No groups found in '{group_claim}', using default_role: {role_mappings.default_role}")
...
async def _setup_role_mappings() -> Optional["RoleMappings"]:
    ...
    try:
        ...
        verbose_proxy_logger.debug(f"Could not load role_mappings from database: {e}. Continuing with existing role logic.")
    ...
    if generic_role_mappings is not None:
        ...
        try:
            ...
            if isinstance(generic_user_role_mappings_data, dict):
                ...
                role_mappings = RoleMappings(**role_mappings_data)
                verbose_proxy_logger.debug(f"Loaded role_mappings from environments for provider '{role_mappings.provider}'.")
...
async def insert_sso_user(
    ...
    try:
        ...
        verbose_proxy_logger.debug(f"Could not check role_mappings configuration: {e}. Using default behavior.")
    ...
    if litellm.default_internal_user_params:
        ...
        if role_mappings_configured and user_defined_values.get("user_role") is not None:
            ...
            verbose_proxy_logger.debug(f"Preserved SSO-extracted role '{preserved_role}' (role_mappings configured)")
        else:
...
async def get_ui_settings(request: Request):
    ...
    disable_expensive_db_queries = proxy_state.get_proxy_state_variable("spend_logs_row_count") > MAX_SPENDLOG_ROWS_TO_QUERY
...
class SSOAuthenticationHandler:
    ...
    def _get_generic_sso_redirect_params(
        ...
        if use_pkce:
            ...
            verbose_proxy_logger.debug("PKCE enabled - code_challenge added to authorization request")
        ...
        if generic_client_id is not None and result is not None:
            ...
            if user_role is None:
                ...
                if _role_from_attr is not None:
                    # Convert enum to string if needed
                    user_role = _role_from_attr.value if isinstance(_role_from_attr, LitellmUserRoles) else _role_from_attr
    ...
    def prepare_token_exchange_parameters(
        ...
        if state:
            ...
            if code_verifier:
                ...
                verbose_proxy_logger.debug("PKCE code_verifier retrieved and will be included in token exchange")
...
class MicrosoftSSOHandler:
    ...
    async def get_microsoft_callback_response(
        ...
        user_team_ids = await MicrosoftSSOHandler.get_user_groups_from_graph_api(access_token=microsoft_sso.access_token)
