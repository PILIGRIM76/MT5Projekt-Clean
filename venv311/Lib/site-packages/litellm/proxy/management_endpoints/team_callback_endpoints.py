...
async def add_team_callbacks(
    ...
    try:
        ...
        for callback in team_callback_settings:
            if callback.get("callback_name") == data.callback_name and callback.get("callback_type") == data.callback_type:
...
async def get_team_callbacks(
    ...
    try:
        ...
        verbose_proxy_logger.error("litellm.proxy.proxy_server.get_team_callbacks(): Exception occurred - {}".format(str(e)))
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
                message=getattr(e, "detail", f"Internal Server Error({str(e)})"),
