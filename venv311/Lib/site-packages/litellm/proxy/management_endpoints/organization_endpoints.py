...
async def new_organization(
    ...
    if data.max_budget is not None and data.max_budget < 0:
        raise HTTPException(status_code=400, detail={"error": f"max_budget cannot be negative. Received: {data.max_budget}"})
    if data.soft_budget is not None and data.soft_budget < 0:
        raise HTTPException(status_code=400, detail={"error": f"soft_budget cannot be negative. Received: {data.soft_budget}"})

    user_object_correct_type: Optional[LiteLLM_UserTable] = None

    if user_api_key_dict.user_id is not None:
        try:
            user_object = await prisma_client.db.litellm_usertable.find_unique(where={"user_id": user_api_key_dict.user_id})
...
async def update_organization(
    ...
    if user_api_key_dict.user_id is None:
        raise HTTPException(
            status_code=400,
            detail={"error": "Cannot associate a user_id to this action. Check `/key/info` to validate if 'user_id' is set."},
        )
...
async def delete_organization(
    ...
    for organization_id in data.organization_ids:
        ...
        await prisma_client.db.litellm_teamtable.delete_many(where={"organization_id": organization_id})
        # delete all members in the organization
        await prisma_client.db.litellm_organizationmembership.delete_many(where={"organization_id": organization_id})
        # delete all keys in the organization
        await prisma_client.db.litellm_verificationtoken.delete_many(where={"organization_id": organization_id})
...
async def list_organization(
    org_id: Optional[str] = fastapi.Query(default=None, description="Filter organizations by exact organization_id match"),
...
async def info_organization(organization_id: str):
    ...
    response: Optional[LiteLLM_OrganizationTableWithMembers] = await prisma_client.db.litellm_organizationtable.find_unique(
        where={"organization_id": organization_id},
        include={
            "litellm_budget_table": True,
            "members": True,
...
async def organization_member_add(
    ...
    try:
        ...
        if existing_organization_row is None:
            raise HTTPException(
                status_code=404,
                detail={"error": f"Organization not found for organization_id={getattr(data, 'organization_id', None)}"},
            )
...
async def organization_member_update(
    ...
    try:
        ...
        try:
            existing_organization_membership = await prisma_client.db.litellm_organizationmembership.find_unique(
                where={
        ...
        final_organization_membership: Optional[BaseModel] = await prisma_client.db.litellm_organizationmembership.find_unique(
            where={
                "user_id_organization_id": {
                    "user_id": data.user_id,
                    "organization_id": data.organization_id,
...
async def add_member_to_organization(
    ...
    try:
        ...
        if member.user_id is not None:
            existing_user_id_row = await prisma_client.db.litellm_usertable.find_unique(where={"user_id": member.user_id})
