...
async def update_cost_discount_config(
    ...
    for provider, discount in cost_discount_config.items():
        ...
        if not (0 <= discount <= 1):
            raise HTTPException(status_code=400, detail=f"Discount for {provider} must be between 0 and 1 (0% to 100%)")
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail={"error": f"Failed to update cost discount config: {str(e)}"})
...
async def update_cost_margin_config(
    ...
    for provider, margin_value in cost_margin_config.items():
        if isinstance(margin_value, (int, float)):
            ...
            if not (0 <= margin_value <= 10):  # Allow up to 1000% margin
                raise HTTPException(
                    status_code=400, detail=f"Margin percentage for {provider} must be between 0 and 10 (0% to 1000%)"
                )
            ...
            if "percentage" in margin_value:
                ...
                if not isinstance(percentage, (int, float)):
                    raise HTTPException(status_code=400, detail=f"Margin percentage for {provider} must be a number")
            ...
            if "fixed_amount" in margin_value:
                ...
                if not isinstance(fixed_amount, (int, float)):
                    raise HTTPException(status_code=400, detail=f"Fixed margin amount for {provider} must be a number")
                if fixed_amount < 0:
                    raise HTTPException(status_code=400, detail=f"Fixed margin amount for {provider} must be non-negative")
    ...
    try:
        ...
        return {"message": "Cost margin configuration updated successfully", "status": "success", "values": cost_margin_config}
    except Exception as e:
        verbose_proxy_logger.error(f"Error updating cost margin config: {str(e)}")
        raise HTTPException(status_code=500, detail={"error": f"Failed to update cost margin config: {str(e)}"})
...
async def estimate_cost(
    ...
    daily_cost, daily_input_cost, daily_output_cost, daily_margin_cost = _calculate_period_costs(
    ...
    monthly_cost, monthly_input_cost, monthly_output_cost, monthly_margin_cost = _calculate_period_costs(
