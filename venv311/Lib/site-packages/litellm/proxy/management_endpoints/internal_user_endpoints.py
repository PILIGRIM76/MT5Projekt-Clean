...
async def _check_duplicate_user_field(
    ...
    *,
    ...
    if field_value:
        ...
        existing_user = await prisma_client.db.litellm_usertable.find_first(where=where_clause)
...
async def _add_user_to_team(
    ...
    try:
        ...
        if e.status_code == 400 and ("already exists" in str(e) or "doesn't exist" in str(e)):
            ...
            verbose_proxy_logger.debug(
                "litellm.proxy.management_endpoints.internal_user_endpoints.new_user(): Exception occured - {}".format(str(e))
            )
...
async def user_info(
    ...
    try:
        ...
        if user_info is not None:
            ...
            teams_2 = await prisma_client.get_data(team_id_list=user_info.teams, table_name="team", query_type="find_all")
        ...
        response_data = UserInfoResponse(user_id=user_id, user_info=_user_info, keys=returned_keys, teams=team_list)
...
def _update_internal_user_params(data_json: dict, data: Union[UpdateUserRequest, UpdateUserRequestNoUserIDorEmail]) -> dict:
    non_default_values = {}
    ...
    if "budget_duration" in non_default_values:
        ...
        non_default_values["budget_reset_at"] = get_budget_reset_time(budget_duration=non_default_values["budget_duration"])
...
async def _update_single_user_helper(
    ...
    if user_request.user_id:
        existing_user_row = await prisma_client.db.litellm_usertable.find_first(where={"user_id": user_request.user_id})
    elif user_request.user_email:
        existing_user_row = await prisma_client.db.litellm_usertable.find_first(where={"user_email": user_request.user_email})
    ...
    existing_metadata = cast(Dict, getattr(existing_user_row, "metadata", {}) or {}) if existing_user_row is not None else {}
    ...
    if response is not None:
        try:
            updated_user_row = await prisma_client.db.litellm_usertable.find_first(where={"user_id": response["user_id"]})
...
async def user_update(
    ...
    try:
        ...
        verbose_proxy_logger.exception("litellm.proxy.proxy_server.user_update(): Exception occured - {}".format(str(e)))
    ...
    try:
        for user_request in users_to_update:
            try:
                ...
                verbose_proxy_logger.exception(f"Failed to update user {user_request.user_id or user_request.user_email}: {e}")
                # Record failure
                error_message = str(e)
                verbose_proxy_logger.error(
...
async def bulk_user_update(
    ...
    if data.all_users and data.user_updates:
        ...
        try:
            ...
            await prisma_client.db.litellm_usertable.update_many(where={}, data=non_default_values)  # Update all users
    ...
    if len(users_to_update) > MAX_BATCH_SIZE:
        raise HTTPException(
            status_code=400,
            detail={"error": f"Maximum {MAX_BATCH_SIZE} users can be updated at once. Found {len(users_to_update)} users."},
        )
...
async def get_users(
    ...
    if users is not None:
        for user in users:
            user_list.append(
                LiteLLM_UserTableWithKeyCount(**user.model_dump(), key_count=user_key_counts.get(user.user_id, 0))
            )
    else:
        user_list = []
...
async def delete_user(
    ...
    for user_id in data.user_ids:
        ...
        fetch_all_teams = await prisma_client.db.litellm_teamtable.find_many(where={"team_id": {"in": user_row.teams}})
        teams_to_update = []
    ...
    await prisma_client.db.litellm_verificationtoken.delete_many(where={"user_id": {"in": data.user_ids}})

    ## DELETE ASSOCIATED INVITATION LINKS
    await prisma_client.db.litellm_invitationlink.delete_many(where={"user_id": {"in": data.user_ids}})

    ## DELETE ASSOCIATED ORGANIZATION MEMBERSHIPS
    await prisma_client.db.litellm_organizationmembership.delete_many(where={"user_id": {"in": data.user_ids}})

    ## DELETE ASSOCIATED TEAM MEMBERSHIPS
    await prisma_client.db.litellm_teammembership.delete_many(where={"user_id": {"in": data.user_ids}})

    ## DELETE USERS
    deleted_users = await prisma_client.db.litellm_usertable.delete_many(where={"user_id": {"in": data.user_ids}})
