...
def update_db_model(db_model: Deployment, updated_patch: updateDeployment) -> PrismaCompatibleUpdateDBModel:
    merged_deployment_dict = DeploymentTypedDict(
        ...
        litellm_params=LiteLLMParamsTypedDict(**db_model.litellm_params.model_dump(exclude_none=True)),  # type: ignore
...
async def _add_model_to_db(
    ...
    if should_create_model_in_db:
        model_response = await prisma_client.db.litellm_proxymodeltable.create(data=_data)  # type: ignore
    else:
...
async def delete_model(
    ...
    try:
        ...
        if prisma_client is None:
            raise HTTPException(
                status_code=500,
                detail={"error": "No DB Connected. Here's how to do it - https://docs.litellm.ai/docs/proxy/virtual_keys"},
            )
        ...
        if model_params.model_info.team_id is not None:
            ...
            valid_team_model_aliases = [
                model for team_id, model in removed_model_aliases if team_id == model_params.model_info.team_id
            ]
            ...
            if existing_team_row is not None:
                existing_team_row.models = [
                    model for model in existing_team_row.models if model not in valid_team_model_aliases
                ]
...
async def add_new_model(
    ...
    try:
        ...
        verbose_proxy_logger.exception("litellm.proxy.proxy_server.add_new_model(): Exception occured - {}".format(str(e)))
        if isinstance(e, HTTPException):
            raise ProxyException(
                message=getattr(e, "detail", f"Authentication Error({str(e)})"),
                type=ProxyErrorTypes.auth_error,
...
async def update_model(
    ...
    try:
        ...
        _existing_litellm_params = await prisma_client.db.litellm_proxymodeltable.find_unique(where={"model_id": _model_id})
        ...
        verbose_proxy_logger.exception("litellm.proxy.proxy_server.update_model(): Exception occured - {}".format(str(e)))
        if isinstance(e, HTTPException):
            raise ProxyException(
                message=getattr(e, "detail", f"Authentication Error({str(e)})"),
                type=ProxyErrorTypes.auth_error,
...
async def update_useful_links(
    ...
    try:
        ...
        verbose_proxy_logger.debug(f"Updated useful links to: {request.useful_links} by user: {user_api_key_dict.user_id}")
