...
def _check_team_rpm_tpm_limits(
    ...
    if data.tpm_limit is not None and entity_tpm_limit is not None and data.tpm_limit + allocated_tpm > entity_tpm_limit:
        raise HTTPException(
            status_code=400,
    ...
    if data.rpm_limit is not None and entity_rpm_limit is not None and data.rpm_limit + allocated_rpm > entity_rpm_limit:
        raise HTTPException(
            status_code=400,
...
async def _check_user_team_limits(
    ...
    if data.tpm_limit is not None and user_api_key_dict.tpm_limit is not None and data.tpm_limit > user_api_key_dict.tpm_limit:
        raise HTTPException(
            status_code=400,
            detail={
    ...
    if data.rpm_limit is not None and user_api_key_dict.rpm_limit is not None and data.rpm_limit > user_api_key_dict.rpm_limit:
        raise HTTPException(
            status_code=400,
            detail={
...
async def new_team(  # noqa: PLR0915
    ...
    try:
        ...
        if data.max_budget is not None and data.max_budget < 0:
            raise HTTPException(
                status_code=400, detail={"error": f"max_budget cannot be negative. Received: {data.max_budget}"}
            )
        ...
        if data.soft_budget is not None and data.soft_budget < 0:
            raise HTTPException(
                status_code=400, detail={"error": f"soft_budget cannot be negative. Received: {data.soft_budget}"}
            )
        ...
        if data.team_id is None:
            ...
            _existing_team_id = await prisma_client.get_data(team_id=data.team_id, table_name="team", query_type="find_unique")
...
def validate_team_org_change(
    ...
    not_in_org = [m for m in team_members if m not in org_members and m != SpecialProxyStrings.default_user_id.value]
    if len(not_in_org) > 0:
        raise HTTPException(
            status_code=403,
            detail={
...
async def update_team(  # noqa: PLR0915
    ...
    try:
        ...
        existing_team_row = await prisma_client.db.litellm_teamtable.find_unique(where={"team_id": data.team_id})
        ...
        org_id_to_check = data.organization_id if data.organization_id is not None else existing_team_row.organization_id
    ...
    if isinstance(data.member, Member):
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail={
                    "error": "Unable to add user - {}, to team - {}, for reason - {}".format(data.member, data.team_id, str(e))
                },
            )
        ...
        for m in data.member:
            try:
                ...
                raise HTTPException(
                    status_code=500,
                    detail={"error": "Unable to add user - {}, to team - {}, for reason - {}".format(m, data.team_id, str(e))},
                )
            updated_users.append(updated_user)
            if updated_tm is not None:
                updated_team_memberships.append(updated_tm)
...
async def _validate_and_populate_member_user_info(
    ...
    if member.user_email is not None and member.user_id is not None:
        ...
        if isinstance(users_by_email, list) and len(users_by_email) > 1:
            raise HTTPException(
                status_code=400,
                detail={"error": f"Multiple users found with email '{member.user_email}'. Please use 'user_id' instead."},
            )
    ...
    if member.user_id is not None and member.user_email is None:
        user_by_id = await prisma_client.db.litellm_usertable.find_unique(where={"user_id": member.user_id})
...
async def team_member_delete(
    ...
    if user_api_key_dict.user_role != LitellmUserRoles.PROXY_ADMIN.value and not _is_user_team_admin(
    ...
    existing_user_rows = await prisma_client.db.litellm_usertable.find_many(where=key_val)  # type: ignore
    ...
    for _uid in user_ids_to_delete:
        await prisma_client.db.litellm_teammembership.delete_many(where={"team_id": data.team_id, "user_id": _uid})
    ...
    if user_ids_to_delete:
        ...
        keys_to_delete: List[LiteLLM_VerificationToken] = await prisma_client.db.litellm_verificationtoken.find_many(
            where={
...
async def bulk_team_member_add(
    ...
    if data.all_users:
        ...
        all_users_in_db = await prisma_client.db.litellm_usertable.find_many(order={"created_at": "desc"})
        data.members = [
            Member(
...
async def delete_team(
    ...
    for team_id in data.team_ids:
        try:
            team_row_base: Optional[BaseModel] = await prisma_client.db.litellm_teamtable.find_unique(
...
def validate_membership(user_api_key_dict: UserAPIKeyAuth, team_table: LiteLLM_TeamTable):
    ...
    if user_api_key_dict.user_id not in [m.user_id for m in team_table.members_with_roles]:
        raise HTTPException(
            status_code=403,
            detail={
                "error": "User={} not authorized to access this team={}".format(user_api_key_dict.user_id, team_table.team_id)
            },
        )
...
async def _add_team_member_budget_table(
    ...
    try:
        team_budget = await prisma_client.db.litellm_budgettable.find_unique(where={"budget_id": team_member_budget_id})
        team_info_response_object.team_member_budget_table = team_budget
    except Exception:
        verbose_proxy_logger.info(f"Team member budget table not found, passed team_member_budget_id={team_member_budget_id}")
...
async def team_info(
    ...
    try:
        ...
        team_member_budget_id = _team_info.metadata.get("team_member_budget_id") if _team_info.metadata is not None else None
...
async def list_available_teams(
    ...
    user_info = await prisma_client.db.litellm_usertable.find_unique(where={"user_id": user_api_key_dict.user_id})
    ...
    available_teams_db = await prisma_client.db.litellm_teamtable.find_many(where={"team_id": {"in": available_teams}})
...
async def _build_team_list_where_conditions(
    ...
    if user_id:
        try:
            user_object = await prisma_client.db.litellm_usertable.find_unique(where={"user_id": user_id})
        except Exception:
            raise HTTPException(
                status_code=404,
...
async def list_team_v2(
    http_request: Request,
    user_id: Optional[str] = fastapi.Query(default=None, description="Only return teams which this 'user_id' belongs to"),
    organization_id: Optional[str] = fastapi.Query(
        default=None,
        description="Only return teams which this 'organization_id' belongs to",
    ),
    team_id: Optional[str] = fastapi.Query(default=None, description="Only return teams which this 'team_id' belongs to"),
    ...
    if use_deleted_table:
        ...
        total_count = await prisma_client.db.litellm_deletedteamtable.count(where=where_conditions)
    else:
        teams = await prisma_client.db.litellm_teamtable.find_many(
            where=where_conditions,
            skip=skip,
...
async def list_team(
        ...
        for team in response:
            if team.members_with_roles:
                for member in team.members_with_roles:
                    if "user_id" in member and member["user_id"] is not None and member["user_id"] == user_id:
    ...
    for team in filtered_response:
        ...
        keys = await prisma_client.db.litellm_verificationtoken.find_many(where={"team_id": team.team_id})
            ...
            for team_id in team_ids_list:
                if team_id not in user_info.teams:
                    raise HTTPException(
                        status_code=404,
                        detail={
                            "error": "User does not belong to Team= {}. Call `/user/info` to see user's teams".format(team_id)
                        },
                    )
