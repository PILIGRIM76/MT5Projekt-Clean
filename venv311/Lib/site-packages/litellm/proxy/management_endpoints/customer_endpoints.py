...
async def unblock_user(data: BlockUsers):
    ...
    if not any(isinstance(x, _ENTERPRISE_BlockedUserList) for x in litellm.callbacks) or litellm.blocked_user_list is None:
        raise HTTPException(
            status_code=400,
...
async def new_end_user(
    ...
    try:
        ...
        verbose_proxy_logger.exception(
            "litellm.proxy.management_endpoints.customer_endpoints.new_end_user(): Exception occured - {}".format(str(e))
        )
...
async def end_user_info(
    ...
    try:
        ...
        verbose_proxy_logger.exception(
            "litellm.proxy.management_endpoints.customer_endpoints.end_user_info(): Exception occured - {}".format(str(e))
        )
        raise handle_exception_on_proxy(e)
...
async def update_end_user(
    ...
    try:
        ...
        verbose_proxy_logger.exception("litellm.proxy.proxy_server.update_end_user(): Exception occured - {}".format(str(e)))
        raise handle_exception_on_proxy(e)
...
async def delete_end_user(
    ...
    try:
        ...
        if data.user_ids is not None and isinstance(data.user_ids, list) and len(data.user_ids) > 0:
            ...
            existing_users = await prisma_client.db.litellm_endusertable.find_many(where={"user_id": {"in": data.user_ids}})
        ...
        verbose_proxy_logger.error("litellm.proxy.proxy_server.delete_end_user(): Exception occured - {}".format(str(e)))
        raise handle_exception_on_proxy(e)
...
async def list_end_user(
    ...
    try:
        ...
        response = await prisma_client.db.litellm_endusertable.find_many(include={"litellm_budget_table": True})
        ...
        verbose_proxy_logger.exception(
            "litellm.proxy.management_endpoints.customer_endpoints.list_end_user(): Exception occured - {}".format(str(e))
        )
        raise handle_exception_on_proxy(e)
