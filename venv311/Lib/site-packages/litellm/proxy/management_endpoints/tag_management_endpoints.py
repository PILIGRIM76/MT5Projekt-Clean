...
async def _get_model_names(prisma_client, model_ids: list) -> Dict[str, str]:
    ...
    try:
        models = await prisma_client.db.litellm_proxymodeltable.find_many(where={"model_id": {"in": model_ids}})
...
async def new_tag(
    ...
    try:
        ...
        existing_tag = await prisma_client.db.litellm_tagtable.find_unique(where={"tag_name": tag.name})
...
async def _add_tag_to_deployment(deployment: "Deployment", tag: str):
    ...
    try:
        ...
        db_model = await prisma_client.db.litellm_proxymodeltable.find_unique(where={"model_id": deployment.model_info.id})

        if db_model is None:
            raise HTTPException(status_code=404, detail=f"Model {deployment.model_info.id} not found in database")
...
async def list_tags(
    ...
    try:
        ## QUERY STORED TAGS ##
        tag_records = await prisma_client.db.litellm_tagtable.find_many(include={"litellm_budget_table": True})
        ...
        dynamic_tags_list = [LiteLLM_DailyTagSpendTable(**dynamic_tag.model_dump()) for dynamic_tag in dynamic_tags]
...
async def delete_tag(
    ...
    try:
        # Check if tag exists
        existing_tag = await prisma_client.db.litellm_tagtable.find_unique(where={"tag_name": data.name})
        if existing_tag is None:
