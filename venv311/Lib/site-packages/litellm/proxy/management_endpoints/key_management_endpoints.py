...
def _is_allowed_to_make_key_request(
    ...
    if user_id is not None:
        ...
        ), "User can only create keys for themselves. Got user_id={}, Your ID={}".format(user_id, user_api_key_dict.user_id)
...
def _team_key_operation_team_member_check(
    ...
    is_admin = user_api_key_dict.user_role is not None and user_api_key_dict.user_role == LitellmUserRoles.PROXY_ADMIN.value
...
def _personal_key_generation_check(user_api_key_dict: UserAPIKeyAuth, data: GenerateKeyRequest):
    if litellm.key_generation_settings is None or litellm.key_generation_settings.get("personal_key_generation") is None:
        return True
...
async def _common_key_generation_helper(  # noqa: PLR0915
    ...
    try:
        ...
        verbose_proxy_logger.debug(
            "litellm.proxy.proxy_server.generate_key_fn(): Enterprise key management params not applied - {}".format(str(e))
        )
    ...
    if data.key is not None and not data.key.startswith("sk-"):
        raise HTTPException(
            status_code=400,
            detail={"error": f"Invalid key format. LiteLLM Virtual Key must start with 'sk-'. Received: {data.key}"},
        )
...
async def generate_key_fn(
    ...
    try:
        ...
        verbose_proxy_logger.exception("litellm.proxy.proxy_server.generate_key_fn(): Exception occured - {}".format(str(e)))
...
async def prepare_key_update_data(
    ...
    non_default_values = prepare_metadata_fields(data=data, non_default_values=non_default_values, existing_metadata=_metadata)
...
async def _get_and_validate_existing_key(token: str, prisma_client: Optional[PrismaClient]) -> LiteLLM_VerificationToken:
    """
...
async def update_key_fn(
    ...
    try:
        ...
        verbose_proxy_logger.exception("litellm.proxy.proxy_server.update_key_fn(): Exception occured - {}".format(str(e)))
...
async def delete_key_fn(
    ...
    try:
        ...
        verbose_proxy_logger.debug(f"/keys/delete - cache after delete: {user_api_key_cache.in_memory_cache.cache_dict}")
        ...
        verbose_proxy_logger.exception("litellm.proxy.proxy_server.delete_key_fn(): Exception occured - {}".format(str(e)))
        raise handle_exception_on_proxy(e)
    ...
    try:
        ...
        if (
            ...
            raise HTTPException(
                ...
                detail="You are not allowed to access this key's info. Your role={}".format(user_api_key_dict.user_role),
            )
...
def _check_model_access_group(models: Optional[List[str]], llm_router: Optional[Router], premium_user: bool) -> Literal[True]:
    """
...
async def delete_verification_tokens(
    ...
    try:
        if prisma_client:
            ...
            _keys_being_deleted: List[LiteLLM_VerificationToken] = await prisma_client.db.litellm_verificationtoken.find_many(
            ...
            if user_api_key_dict.user_role == LitellmUserRoles.PROXY_ADMIN.value:
                ...
                if len(deleted_tokens) != len(tokens):
                    ...
                    raise Exception("Failed to delete all tokens. Failed to delete tokens: " + str(failed_tokens))
        else:
...
async def delete_key_aliases(
    ...
    _keys_being_deleted = await prisma_client.db.litellm_verificationtoken.find_many(where={"key_alias": {"in": key_aliases}})
...
async def regenerate_key_fn(
    ...
    try:
        ...
        if premium_user is not True and not is_master_key_regeneration:  # allow master key regeneration for non-premium users
            raise ValueError(f"Regenerating Virtual Keys is an Enterprise feature, {CommonProxyErrors.not_premium_user.value}")
        ...
        if data is not None:
            ...
            non_default_values = await prepare_key_update_data(data=data, existing_key_row=_key_in_db)
...
async def validate_key_list_check(
    ...
    complete_user_info_db_obj: Optional[BaseModel] = await prisma_client.db.litellm_usertable.find_unique(
...
async def get_admin_team_ids(
    ...
    admin_team_ids = [
        team.team_id for team in teams_pydantic_obj if _is_user_team_admin(user_api_key_dict=user_api_key_dict, team_obj=team)
    ]
...
async def _list_key_helper(
    ...
    if use_deleted_table:
        total_count = await prisma_client.db.litellm_deletedverificationtoken.count(where=where)  # type: ignore
    else:
        total_count = await prisma_client.db.litellm_verificationtoken.count(where=where)  # type: ignore
...
async def block_key(
    ...
    if litellm.store_audit_logs is True:
        ...
        record = await prisma_client.db.litellm_verificationtoken.find_unique(where={"token": hashed_token})
        if record is None:
            raise ProxyException(
...
async def _enforce_unique_key_alias(
    ...
    if key_alias is not None and prisma_client is not None:
        ...
        existing_key = await prisma_client.db.litellm_verificationtoken.find_first(where=where_clause)
