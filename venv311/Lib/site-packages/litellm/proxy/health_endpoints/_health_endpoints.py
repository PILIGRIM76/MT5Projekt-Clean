            ...
            if "slack" in general_settings.get("alerting", []):
                ...
                if proxy_logging_obj.slack_alerting_instance.alert_to_webhook_url is not None:
                    for alert_type in proxy_logging_obj.slack_alerting_instance.alert_to_webhook_url:
                        ...
                        await proxy_logging_obj.alerting_handler(message=test_message, level="Low", alert_type=alert_type)
                else:
                ...
                raise HTTPException(
                    status_code=422,
                    detail={"error": '"{}" not in proxy config: general_settings. Unable to test this.'.format(service)},
                )
        if service == "email":
            ...
            await proxy_logging_obj.slack_alerting_instance.send_key_created_or_user_invited_email(webhook_event=webhook_event)
...
async def _save_health_check_to_db(
    ...
    try:
        ...
        error_message = (
            str(unhealthy_endpoints[0]["error"])[:500] if unhealthy_endpoints and unhealthy_endpoints[0].get("error") else None
        )
...
async def health_endpoint(
    ...
    try:
        ...
        verbose_proxy_logger.error("litellm.proxy.proxy_server.py::health_endpoint(): Exception occured - {}".format(str(e)))
        verbose_proxy_logger.debug(traceback.format_exc())
        raise e
...
def _read_license_data() -> Optional[Dict[str, Any]]:
    ...
    if license_data is None and getattr(_license_check, "license_str", None) and getattr(_license_check, "public_key", None):
        try:
...
async def test_model_connection(
    ...
    try:
        ...
        verbose_proxy_logger.debug(f"litellm.proxy.health_endpoints.test_model_connection(): Exception occurred - {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
