...
async def _resolve_embedding_config_from_db(embedding_model: str, prisma_client) -> Optional[Dict[str, Any]]:
    ...
    for model_name in model_name_candidates:
        try:
            db_model = await prisma_client.db.litellm_proxymodeltable.find_first(where={"model_name": model_name})
...
async def _resolve_embedding_config(embedding_model: str, prisma_client, llm_router=None) -> Optional[Dict[str, Any]]:
    ...
    if prisma_client is not None:
        db_config = await _resolve_embedding_config_from_db(embedding_model=embedding_model, prisma_client=prisma_client)
        if db_config:
            verbose_proxy_logger.debug(f"Resolved embedding config from database for model {embedding_model}")
            return db_config

    verbose_proxy_logger.debug(f"Could not resolve embedding config for model {embedding_model} from router or database")
    return None
...
async def create_vector_store_in_db(
    ...
    if litellm_params:
        ...
        if embedding_model and not litellm_params.get("litellm_embedding_config"):
            resolved_config = await _resolve_embedding_config(embedding_model=embedding_model, prisma_client=prisma_client)
            if resolved_config:
                litellm_params["litellm_embedding_config"] = resolved_config
                verbose_proxy_logger.info(f"Auto-resolved embedding config for model {embedding_model}")
    ...
    # Create in database
    _new_vector_store = await prisma_client.db.litellm_managedvectorstorestable.create(data=data_to_create)
...
async def new_vector_store(
    ...
    try:
        ...
        if not vector_store_id or not custom_llm_provider:
            raise HTTPException(status_code=400, detail="vector_store_id and custom_llm_provider are required")
...
async def list_vector_stores(
    ...
    try:
        ...
        accessible_vector_stores = [
            vs for vs in vector_store_map.values() if _check_vector_store_access(vs, user_api_key_dict)
        ]
...
async def delete_vector_store(
    ...
    try:
        ...
        if vector_store_to_check and not _check_vector_store_access(vector_store_to_check, user_api_key_dict):
            raise HTTPException(
                status_code=403,
        ...
        return {"status": "success", "message": f"Vector store {data.vector_store_id} deleted successfully"}
...
async def update_vector_store(
    ...
    try:
        ...
        if litellm.vector_store_registry is not None:
            ...
            verbose_proxy_logger.debug(f"Updated vector store {vector_store_id} in both database and in-memory registry")
