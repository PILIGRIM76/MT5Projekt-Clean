...
class JWTAuthManager:
    ...
    async def find_and_validate_specific_team_id(
        ...
        if team_alias:
            ...
            if team_object:
                ...
                verbose_proxy_logger.info(f"JWT Auth: Resolved team_alias='{team_alias}' to team_id='{individual_team_id}'")
                return individual_team_id, team_object
    ...
    async def map_user_to_teams(
        ...
        try:
            ...
            if e.type == ProxyErrorTypes.team_member_already_in_team:
                verbose_proxy_logger.debug(f"User {user_object.user_id} is already a member of team {team_object.team_id}")
                return None
            else:
                raise e
        return None
    ...
    async def auth_builder(
        ...
        # Check admin access
        admin_result = await JWTAuthManager.check_admin_access(jwt_handler, scopes, route, user_id, org_id, api_key)
        if admin_result:
            return admin_result
