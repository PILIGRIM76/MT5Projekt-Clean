...
def check_complete_credentials(request_body: dict) -> bool:
    ...
    if "sagemaker" in given_model or "bedrock" in given_model or "vertex_ai" in given_model or "vertex_ai_beta" in given_model:
...
def get_request_route(request: Request) -> str:
    ...
    try:
        ...
        verbose_proxy_logger.debug(f"error on get_request_route: {str(e)}, defaulting to request.url.path={request.url.path}")
        return request.url.path
