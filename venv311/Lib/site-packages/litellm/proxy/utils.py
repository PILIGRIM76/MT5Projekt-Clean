            ...
            try:
                ...
                if _callback is not None and isinstance(_callback, CustomLogger):
                    try:
                        ...
                        verbose_proxy_logger.exception(f"[Non-Blocking] Error in async_post_call_failure_hook callback: {e}")
            except Exception as e:
...
class PrismaClient:
    ...
    async def get_generic_data(
        ...
        try:
            if table_name == "users":
                response = await self.db.litellm_usertable.find_first(where={key: value})  # type: ignore
        ...
        try:
            ...
            if table_name == "key":
                ...
                try:
                    ...
                    if "Foreign key constraint failed on the field: `LiteLLM_UserTable_organization_id_fkey (index)`" in str(
                        e
                    ):
                        raise HTTPException(
                            status_code=400,
    ...
    async def update_data(  # noqa: PLR0915
        ...
        try:
            ...
            if token is not None:
                ...
                verbose_proxy_logger.info("\033[91m" + f"DB User Table - update succeeded {update_user_row}" + "\033[0m")
                ...
                if "members_with_roles" in db_data and isinstance(db_data["members_with_roles"], list):
                    db_data["members_with_roles"] = json.dumps(db_data["members_with_roles"])
                if "members_with_roles" in update_key_values and isinstance(update_key_values["members_with_roles"], list):
                ...
                verbose_proxy_logger.info("\033[91m" + f"DB Team Table - update succeeded {update_team_row}" + "\033[0m")
                ...
                for idx, user in enumerate(data_list):
                    ...
                    batcher.litellm_usertable.upsert(
                        ...
                        data={
                            ...
                            "update": {**data_json},  # type: ignore  # just update user-specified values, if it already exists
                        },
                    )
                await batcher.commit()
    ...
    async def delete_data(
        ...
        try:
            if tokens is not None and isinstance(tokens, List):
                ...
                deleted_tokens = await self.db.litellm_verificationtoken.delete_many(where=filter_query)  # type: ignore
...
async def _monitor_spend_logs_queue(
    ...
    verbose_proxy_logger.info(f"Starting spend logs queue monitor (threshold: {threshold}, poll_interval: {base_interval}s)")
...
def validate_model_access(
    ...
    if "," in model_id:
        ...
        if inaccessible_models:
            raise HTTPException(
                status_code=404,
                detail="The following model(s) do not exist or are not accessible: {}".format(", ".join(inaccessible_models)),
            )
    else:
