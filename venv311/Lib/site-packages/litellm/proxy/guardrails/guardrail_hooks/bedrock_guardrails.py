...
class BedrockGuardrail(CustomGuardrail, BaseAWSLLM):
    ...
    def _create_bedrock_input_content_request(self, messages: Optional[List[AllMessageValues]]) -> BedrockRequest:
        ...
        for message in messages:
            message_text_content: Optional[List[str]] = self.get_content_for_message(message=message)
            if message_text_content is None:
                continue
            for text_content in message_text_content:
                bedrock_content_item = BedrockContentItem(text=BedrockTextContent(text=text_content))
            ...
            for choice in response.choices:
                if isinstance(choice, litellm.Choices):
                    if choice.message.content and isinstance(choice.message.content, str):
                        bedrock_content_item = BedrockContentItem(text=BedrockTextContent(text=choice.message.content))
                        bedrock_request_content.append(bedrock_content_item)
            bedrock_request["content"] = bedrock_request_content
        return bedrock_request
    ...
    async def make_bedrock_api_request(
        ...
        bedrock_request_data: dict = dict(self.convert_to_bedrock_format(source=source, messages=messages, response=response))
        bedrock_guardrail_response: BedrockGuardrailResponse = BedrockGuardrailResponse()
        api_key: Optional[str] = None
        if request_data:
            bedrock_request_data.update(self.get_guardrail_dynamic_request_body_params(request_data=request_data))
        ...
        try:
            ...
            if isinstance(response, httpx.Response):
                try:
                    status_code, detail_message = self._parse_bedrock_guardrail_error_response(response)
    ...
    async def async_post_call_success_hook(
        ...
        if should_validate_input:
            ...
            output_task = self.make_bedrock_api_request(source="OUTPUT", response=response, request_data=data)
    ...
    def _update_messages_with_updated_bedrock_guardrail_response(
        ...
        if self.mask_request_content or self.mask_response_content:
            verbose_proxy_logger.debug("Masking enabled but no masked output from guardrail, returning original messages")
    ...
    async def async_post_call_streaming_iterator_hook(
        ...
        if isinstance(assembled_model_response, ModelResponse):
            ...
            if should_validate_input:
                ...
                input_filter = self._prepare_guardrail_messages_for_role(messages=request_data.get("messages"))
    ...
    def _apply_masking_to_messages(self, messages: List[AllMessageValues], masked_texts: List[str]) -> List[AllMessageValues]:
        """
