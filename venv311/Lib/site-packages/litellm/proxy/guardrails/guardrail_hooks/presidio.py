...
class _OPTIONAL_PresidioPIIMasking(CustomGuardrail):
    ...
    def validate_environment(
        ...
        if not (
            self.presidio_analyzer_api_base.startswith("http://") or self.presidio_analyzer_api_base.startswith("https://")
        ):
        ...
        if not (
            self.presidio_anonymizer_api_base.startswith("http://") or self.presidio_anonymizer_api_base.startswith("https://")
        ):
            # add http:// if unset, assume communicating over private network - e.g. render
        ...
        try:
            ...
            async with self._get_session_iterator() as session:
                ...
                if isinstance(analyze_results, dict):
                    ...
                    verbose_proxy_logger.debug("Presidio returned dict (not list), attempting to process as single item")
                    try:
    ...
    def raise_exception_if_blocked_entities_detected(self, analyze_results: Union[List[PresidioAnalyzeResponseItem], Dict]):
        """
    ...
    def logging_hook(self, kwargs: dict, result: Any, call_type: str) -> Tuple[dict, Any]:
        ...
        def run_in_new_loop():
            ...
            try:
                ...
                return new_loop.run_until_complete(self.async_logging_hook(kwargs=kwargs, result=result, call_type=call_type))
            finally:
                new_loop.close()
