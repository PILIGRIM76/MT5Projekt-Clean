...
class ContentFilterGuardrail(CustomGuardrail):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        verbose_proxy_logger.debug(
            f"Loaded {len(self.loaded_categories)} categories with " f"{len(self.category_keywords)} keywords"
        )
    ...
    def _load_categories(self, categories: List[ContentFilterCategoryConfig]) -> None:
        ...
        for cat_config in categories:
            ...
            severity_threshold = cat_config.get("severity_threshold", self.severity_threshold) or self.severity_threshold
            ...
            try:
                ...
                verbose_proxy_logger.info(f"Loaded category {category_name}: " f"{len(category_config_obj.keywords)} keywords")
    ...
    def _find_pattern_spans(self, text: str, pattern_entry: Dict[str, Any]) -> List[Tuple[int, int]]:
        ...
        for match in regex.finditer(text):
            if keyword_matches is not None and not self._match_near_keyword(match.start(), match.end(), keyword_matches, text):
                continue
    ...
    def _filter_single_text(self, text: str, detections: Optional[List[ContentFilterDetection]] = None) -> str:
        ...
        if category_keyword_match:
            ...
            if action == ContentFilterAction.BLOCK:
                error_msg = (
                    f"Content blocked: {category_name} category keyword '{keyword}' detected " f"(severity: {severity})"
                )
                ...
                verbose_proxy_logger.info(f"Masked category keyword '{keyword}' from {category_name} (severity: {severity})")
    ...
    async def _process_images(self, images: List[str], detections: List[ContentFilterDetection]) -> None:
        ...
        verbose_proxy_logger.debug(f"ContentFilterGuardrail: Applying guardrail to {len(descriptions)} image description(s)")
        ...
        async for item in response:
            if isinstance(item, ModelResponseStream) and item.choices:
                ...
                if safe_to_yield_len > yielded_masked_text_len:
                    ...
                    if item.choices and hasattr(item.choices[0], "delta") and item.choices[0].delta:
