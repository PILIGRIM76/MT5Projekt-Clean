...
class PanwPrismaAirsHandler(CustomGuardrail):
    ...
    async def _call_panw_api(  # noqa: PLR0915
        ...
        panw_metadata = {
            "app_user": (metadata.get("app_user") or metadata.get("user") or "litellm_user") if metadata else "litellm_user",
        ...
        try:
            ...
            if result.get("action") == "block" and "error" in result:
                ...
                if "profile" in error_msg and ("not found" in error_msg or "required" in error_msg or "invalid" in error_msg):
            ...
            if status in (401, 403) or is_profile_error:
                ...
                verbose_proxy_logger.error(f"PANW Prisma AIRS: API error (HTTP {status}): {error_body}")
                return {
                    "action": "block",
    ...
    def _handle_api_error_with_logging(
        ...
        if scan_result.get("_is_transient") and self.fallback_on_error == "allow":
            ...
            add_guardrail_to_applied_guardrails_header(request_data=data, guardrail_name=f"{self.guardrail_name}:unscanned")
            return None
        ...
        try:
            ...
            if masked_text and self.mask_request_content:
                if messages:
                    ...
                    data["prompt"] = masked_text
                verbose_proxy_logger.warning("PANW Prisma AIRS: Prompt blocked but masked instead (mask_request_content=True)")
                add_guardrail_to_applied_guardrails_header(request_data=data, guardrail_name=self.guardrail_name)
                return None
    ...
    async def _scan_and_process_streaming_response(
        ...
        if action == "allow":
            if masked_text:
                ...
                verbose_proxy_logger.info(f"PANW Prisma AIRS: Streaming response allowed with masking (Category: {category})")
            else:
