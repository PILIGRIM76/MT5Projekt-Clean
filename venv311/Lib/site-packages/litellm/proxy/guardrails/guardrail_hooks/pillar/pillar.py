...
def build_pillar_response_headers(metadata_store: Dict[str, Any]) -> Dict[str, str]:
    ...
    if "pillar_evidence" in metadata_store:
        truncated_evidence, encoded_value, truncated_flag = _truncate_evidence_payload(metadata_store["pillar_evidence"])
...
class PillarGuardrail(CustomGuardrail):
    ...
    async def async_post_call_success_hook(
        ...
        response_messages = [choice.get("message") for choice in response_dict.get("choices", []) if choice.get("message")]
    ...
    def _handle_api_error(self, error: Exception, data: dict) -> dict:
        ...
        if self.fallback_on_error == "allow":
            ...
            verbose_proxy_logger.warning("Pillar Guardrail: API unavailable, blocking request (fallback_on_error=block)")
            raise HTTPException(
                status_code=503,
                detail={
    ...
    def _prepare_payload(self, data: dict) -> Dict[str, Any]:
        ...
        verbose_proxy_logger.debug(
            f"Pillar Guardrail: Request context - user={user_id}, session={session_id}, " f"model={model}, provider={provider}"
        )
        return payload
