...
async def delete_guardrail(guardrail_id: str):
    ...
    try:
        ...
        result = await GUARDRAIL_REGISTRY.delete_guardrail_from_db(guardrail_id=guardrail_id, prisma_client=prisma_client)
...
async def patch_guardrail(guardrail_id: str, request: PatchGuardrailRequest):
    ...
    try:
        ...
        guardrail_info = (
            request.guardrail_info if request.guardrail_info is not None else existing_guardrail.get("guardrail_info", {})
        )
...
async def get_guardrail_info(guardrail_id: str):
    ...
    try:
        ...
        result = await GUARDRAIL_REGISTRY.get_guardrail_by_id_from_db(guardrail_id=guardrail_id, prisma_client=prisma_client)
        if result is None:
        ...
        result_litellm_params_dict = (
            litellm_params.model_dump(exclude_none=True) if isinstance(litellm_params, LitellmParams) else litellm_params
        ) or {}
...
async def get_category_yaml(category_name: str):
    ...
    if not os.path.exists(category_file_path):
        raise HTTPException(status_code=404, detail=f"Category file not found: {category_name}")
...
def _extract_fields_recursive(
    ...
    for field_name, field in model.model_fields.items():
        ...
        is_basemodel_subclass = (
            inspect.isclass(field_annotation) and issubclass(field_annotation, BaseModel) and field_annotation is not BaseModel
        )
...
async def apply_guardrail(
    ...
    try:
        ...
        return ApplyGuardrailResponse(response_text=response_text[0] if response_text else request.text)
    except Exception as e:
