...
from litellm.proxy.guardrails.guardrail_hooks.grayswan import initialize_guardrail as initialize_grayswan
...
guardrail_class_registry: Dict[str, Type[CustomGuardrail]] = {SupportedGuardrailIntegrations.GRAYSWAN.value: GraySwanGuardrail}
...
class GuardrailRegistry:
    ...
    async def get_guardrail_by_name_from_db(self, guardrail_name: str, prisma_client: PrismaClient) -> Optional[Guardrail]:
        """
        Get a guardrail by its name from the database
        """
        try:
            guardrail = await prisma_client.db.litellm_guardrailstable.find_unique(where={"guardrail_name": guardrail_name})
...
class InMemoryGuardrailHandler:
    ...
    def initialize_guardrail(
        ...
        if initializer:
            ...
            if "llm_router" in sig.parameters:
                custom_guardrail_callback = initializer(litellm_params, guardrail, llm_router)  # type: ignore
            else:
    ...
    def _has_guardrail_params_changed(self, guardrail_id: str, new_guardrail: Guardrail) -> bool:
        ...
        existing_dict = existing_params.model_dump() if isinstance(existing_params, LitellmParams) else existing_params
    ...
    def reinitialize_guardrail(self, guardrail: Guardrail, config_file_path: Optional[str] = None) -> Optional[Guardrail]:
        """
    ...
    def sync_guardrail_from_db(self, guardrail: Guardrail, config_file_path: Optional[str] = None) -> Optional[Guardrail]:
        """
        ...
        if self._has_guardrail_params_changed(guardrail_id, guardrail):
            ...
            verbose_proxy_logger.info(f"Guardrail '{guardrail_name}' (ID: {guardrail_id}) params changed, re-initializing...")
            return self.reinitialize_guardrail(guardrail=guardrail, config_file_path=config_file_path)
