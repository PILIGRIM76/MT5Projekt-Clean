    ...
    try:
        ...
        if model_from_file_id is not None and input_file_id:
            ...
            response = await litellm.acreate_batch(
                custom_llm_provider=credentials["custom_llm_provider"], **_create_batch_data  # type: ignore
            )
            ...
            if response and hasattr(response, "id") and response.id:
                ...
                encoded_batch_id = encode_file_id_with_model(file_id=original_batch_id, model=model_from_file_id)
            ...
            model_param = data.get("model") or request.query_params.get("model") or request.headers.get("x-litellm-model")
        ...
        verbose_proxy_logger.exception("litellm.proxy.proxy_server.create_batch(): Exception occured - {}".format(str(e)))
...
async def retrieve_batch(
    ...
    try:
        ...
        if response is not None:
            verbose_proxy_logger.debug(f"Batch {batch_id} is in non-terminal state {response.status}, syncing with provider")
        ...
        if model_from_id is not None:
            ...
            verbose_proxy_logger.debug(f"Retrieved batch using model: {model_from_id}, original_id: {original_batch_id}")
        ...
        # SCENARIO 3: Fallback to custom_llm_provider (uses env variables)
            ...
            response = await litellm.aretrieve_batch(custom_llm_provider=custom_llm_provider, **data)  # type: ignore
        ...
        verbose_proxy_logger.exception("litellm.proxy.proxy_server.retrieve_batch(): Exception occured - {}".format(str(e)))
        raise handle_exception_on_proxy(e)
...
async def list_batches(
    ...
    try:
        ...
        elif model_param := (data.get("model") or request.query_params.get("model") or request.headers.get("x-litellm-model")):
            # SCENARIO 2: Use model-based routing from header/query/body
            credentials = get_credentials_for_model(
                llm_router=llm_router,
                model_id=model_param,
...
async def cancel_batch(
    ...
    try:
        ...
        if model_from_id is not None:
            ...
            verbose_proxy_logger.debug(f"Cancelled batch using model: {model_from_id}, original_id: {original_batch_id}")
