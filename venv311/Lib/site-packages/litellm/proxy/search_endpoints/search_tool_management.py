...
async def list_search_tools():
    ...
    try:
        ...
        search_tool_configs = [tool for tool in search_tool_configs if tool.get("search_tool_name") not in db_tool_names]
...
async def create_search_tool(request: CreateSearchToolRequest):
    ...
    try:
        result = await SEARCH_TOOL_REGISTRY.add_search_tool_to_db(search_tool=request.search_tool, prisma_client=prisma_client)
