...
async def get_global_spend_report(
    ...
    try:
        ...
        if api_key is not None:
            ...
            return await get_spend_by_team_and_customer(start_date_obj, end_date_obj, team_id, customer_id, prisma_client)
        if group_by == "team":
...
async def ui_view_spend_logs(  # noqa: PLR0915
    ...
    user_api_key_dict: UserAPIKeyAuth = Depends(user_api_key_auth),
    status_filter: Optional[str] = fastapi.Query(default=None, description="Filter logs by status (e.g., success, failure)"),
    ...
    error_code: Optional[str] = fastapi.Query(default=None, description="Filter logs by error code (e.g., '404', '500')"),
...
-H "Authorization: Bearer sk-1234"
    ...
    try:
        ...
        spend_logs = []
        if start_date is not None and isinstance(start_date, str) and end_date is not None and isinstance(end_date, str):
    ...
    try:
        ...
        for _provider, _budget_info in provider_budget_config.items():
            router_budget_logger = next(
                (cb for cb in (llm_router.optional_callbacks or []) if isinstance(cb, RouterBudgetLimiting)),
                None,
            )
