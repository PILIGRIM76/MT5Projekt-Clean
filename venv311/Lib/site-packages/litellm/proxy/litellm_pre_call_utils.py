...
def convert_key_logging_metadata_to_callback(
    ...
    for var, value in data.callback_vars.items():
        ...
        team_callback_settings_obj.callback_vars[var] = str(litellm.utils.get_secret(value, default_value=value) or value)
...
class LiteLLMProxyRequestSetup:
    ...
    def _get_forwardable_headers(
        ...
        forwarded_headers = {}
        for header, value in headers.items():
            if header.lower().startswith("x-") and not header.lower().startswith("x-stainless"):  # causes openai sdk to fail
    ...
    def add_user_api_key_auth_to_request_metadata(
        ...
        data[_metadata_variable_name]["user_api_end_user_max_budget"] = getattr(user_api_key_dict, "end_user_max_budget", None)
...
async def add_litellm_data_to_request(  # noqa: PLR0915
    ...
    _headers = clean_headers(
        request.headers,
        litellm_key_header_name=(general_settings.get("litellm_key_header_name") if general_settings is not None else None),
    )
    ...
    callback_settings_obj = _get_dynamic_logging_metadata(user_api_key_dict=user_api_key_dict, proxy_config=proxy_config)
...
def _get_enforced_params(general_settings: Optional[dict], user_api_key_dict: UserAPIKeyAuth) -> Optional[list]:
    ...
    if general_settings is not None:
        ...
        if "service_account_settings" in general_settings and check_if_token_is_service_account(user_api_key_dict) is True:
...
def add_guardrails_from_policy_engine(
    ...
    registry = get_policy_registry()
    verbose_proxy_logger.debug(
        f"Policy engine: registry initialized={registry.is_initialized()}, " f"policy_count={len(registry.get_all_policies())}"
    )
    if not registry.is_initialized():
