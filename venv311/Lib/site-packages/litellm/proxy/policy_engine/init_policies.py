...
def _print_policies_on_startup(
    ...
    for policy_name, policy_data in policies_config.items():
        ...
        print(f"{_blue_color_code}  - {policy_name}{inherit_str}{_reset_color_code}")  # noqa: T201
        if description:
    ...
    if validation_result.errors:
        for error in validation_result.errors:
            verbose_proxy_logger.error(
                f"Policy validation error in '{error.policy_name}': " f"[{error.error_type}] {error.message}"
            )

    if validation_result.warnings:
        for warning in validation_result.warnings:
            verbose_proxy_logger.warning(
                f"Policy validation warning in '{warning.policy_name}': " f"[{warning.error_type}] {warning.message}"
            )
