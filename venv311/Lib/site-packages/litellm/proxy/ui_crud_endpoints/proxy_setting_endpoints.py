...
class UISettings(BaseModel):
    ...
    require_auth_for_public_ai_hub: bool = Field(
        default=False, description="If true, requires authentication for accessing the public AI Hub."
    )
...
async def update_internal_user_settings(
    ...
    if settings.teams is not None and all(isinstance(team, NewUserRequestTeam) for team in settings.teams):
        await update_default_team_member_budget(settings.teams, user_api_key_dict=user_api_key_dict)  # type: ignore
...
async def update_sso_settings(sso_config: SSOConfig):
    ...
    try:
        env_var_entry = await prisma_client.db.litellm_config.find_unique(where={"param_name": "environment_variables"})
        ...
        if env_var_entry is not None:
            ...
            filtered_env_vars = {key: value for key, value in environment_variables.items() if key not in env_vars_to_remove}
...
async def get_in_product_nudges():
    ...
    db_record = await prisma_client.db.litellm_dailytagspend.find_first(where={"tag": "User-Agent: claude-cli"})
...
async def update_ui_settings(settings: UISettings, user_api_key_dict: UserAPIKeyAuth = Depends(user_api_key_auth)):
    """
