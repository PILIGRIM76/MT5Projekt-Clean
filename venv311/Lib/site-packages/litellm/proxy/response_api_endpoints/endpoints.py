...
async def get_response(
    ...
    if ResponsePollingHandler.is_polling_id(response_id):
        ...
        if not redis_usage_cache:
            raise HTTPException(status_code=500, detail="Redis cache not configured. Polling requires Redis.")
        ...
        if not state:
            raise HTTPException(status_code=404, detail=f"Polling response {response_id} not found or expired")
