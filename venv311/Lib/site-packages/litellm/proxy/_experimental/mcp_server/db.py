...
async def get_all_mcp_servers(
    ...
    try:
        ...
        verbose_proxy_logger.debug("litellm.proxy._experimental.mcp_server.db.py::get_all_mcp_servers - {}".format(str(e)))
        return []
...
async def get_mcp_servers(prisma_client: PrismaClient, server_ids: Iterable[str]) -> List[LiteLLM_MCPServerTable]:
    ...
    """
    _mcp_servers: List[LiteLLM_MCPServerTable] = await prisma_client.db.litellm_mcpservertable.find_many(
        where={
...
async def create_mcp_server(prisma_client: PrismaClient, data: NewMCPServerRequest, touched_by: str) -> LiteLLM_MCPServerTable:
    ...
    new_mcp_server = await prisma_client.db.litellm_mcpservertable.create(data=data_dict)  # type: ignore
