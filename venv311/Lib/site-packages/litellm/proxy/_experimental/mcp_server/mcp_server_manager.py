...
try:
    from mcp.shared.tool_name_validation import validate_tool_name  # pyright: ignore[reportAssignmentType]
...
class MCPServerManager:
    ...
    async def load_servers_from_config(
        ...
        for server_name, server_config in mcp_servers_config.items():
            ...
            new_server = MCPServer(
                ...
                available_on_public_internet=bool(server_config.get("available_on_public_internet", False)),
            )
    ...
    async def _register_openapi_tools(self, spec_path: str, server: MCPServer, base_url: str):
        ...
        from litellm.proxy._experimental.mcp_server.openapi_to_mcp_generator import get_base_url as get_openapi_base_url
        from litellm.proxy._experimental.mcp_server.openapi_to_mcp_generator import (
        ...
        try:
            ...
            verbose_logger.debug(f"Using headers for OpenAPI tools (excluding sensitive values): " f"{list(headers.keys())}")
    ...
    async def build_mcp_server_from_table(
        ...
        *,
        ...
        new_server = MCPServer(
            ...
            available_on_public_internet=bool(getattr(mcp_server, "available_on_public_internet", False)),
    ...
    def filter_server_ids_by_ip(self, server_ids: List[str], client_ip: Optional[str]) -> List[str]:
        ...
        return [
            ...
            if (s := self.get_mcp_server_by_id(sid)) is not None and self._is_server_accessible_from_ip(s, client_ip)
        ]
    ...
    async def get_resource_templates_from_server(
        ...
        try:
            ...
            prefixed_templates = self._create_prefixed_resource_templates(resource_templates, server, add_prefix=add_prefix)
    ...
    async def _fetch_oauth_metadata_from_resource(self, resource_metadata_url: str) -> Tuple[List[str], Optional[List[str]]]:
    ...
    async def _fetch_authorization_server_metadata(self, authorization_servers: List[str]) -> Optional[MCPOAuthMetadata]:
    ...
    def _create_prefixed_prompts(self, prompts: List[Prompt], server: MCPServer, add_prefix: bool = True) -> List[Prompt]:
        """
    ...
    def _create_prefixed_resource_templates(
        ...
        for resource_template in resource_templates:
            name_to_use = add_server_prefix_to_name(resource_template.name, prefix) if add_prefix else resource_template.name
            resource_template.name = name_to_use
            prefixed_templates.append(resource_template)

        verbose_logger.info(f"Successfully fetched {len(prefixed_templates)} resource templates from server {server.name}")
        return prefixed_templates
    ...
    def check_allowed_or_banned_tools(self, tool_name: str, server: MCPServer) -> bool:
        ...
        if server.disallowed_tools:
            return tool_name not in server.disallowed_tools and f"{server.name}-{tool_name}" not in server.disallowed_tools
        return True
    ...
    async def health_check_server(self, server_id: str, mcp_auth_header: Optional[str] = None) -> LiteLLM_MCPServerTable:
        """
