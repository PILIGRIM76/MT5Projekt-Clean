...
class MCPOAuth2TokenCache(InMemoryCache):
    ...
    async def _fetch_token(self, server: "MCPServer") -> Tuple[str, int]:
        ...
        try:
            ...
            raise ValueError(
                f"OAuth2 token request for MCP server '{server.server_id}' " f"failed with status {exc.response.status_code}"
            ) from exc
        ...
        if not access_token:
            raise ValueError(f"OAuth2 token response for MCP server '{server.server_id}' " f"missing 'access_token'")
