...
class MCPRequestHandler:
            ...
            try:
                ...
                if e.status_code in (401, 403):
                    verbose_logger.debug(
                        "MCP OAuth2: Authorization header is not a valid LiteLLM key, " "treating as OAuth2 token passthrough"
                    )
                    validated_user_api_key_auth = UserAPIKeyAuth()
                else:
                    raise
    ...
    async def _get_team_object_permission(
        ...
        verbose_logger.debug(f"MCP team permission lookup: team_id={user_api_key_auth.team_id if user_api_key_auth else None}")
    ...
    async def _get_allowed_mcp_servers_for_key(
        ...
        try:
            key_object_permission = await MCPRequestHandler._get_key_object_permission(user_api_key_auth)
