...
if MCP_AVAILABLE:
                    ...
                    async def forward_progress(progress: float, total: float | None):
                        ...
                        try:
                            await host_session.send_progress_notification(
                                progress_token=host_token, progress=progress, total=total
                            )
    ...
    async def list_resource_templates() -> List[ResourceTemplate]:
        ...
        try:
            ...
            verbose_logger.info(
                "MCP list_resource_templates - Successfully returned " f"{len(resource_templates)} resource templates"
            )
    ...
    async def _get_allowed_mcp_servers(
        ...
        allowed_mcp_server_ids = global_mcp_server_manager.filter_server_ids_by_ip(allowed_mcp_server_ids, client_ip)
        verbose_logger.debug(
    ...
    async def _get_tools_from_mcp_servers(  # noqa: PLR0915
        ...
        if log_list_tools_to_spendlogs:
            ...
            if user_api_key_auth is not None:
                ...
                if user_api_key:
                    cast(dict, list_tools_request_data["metadata"])["user_api_key"] = user_api_key
    ...
    def _strip_stale_mcp_session_header(
        ...
        if known_sessions is not None and _session_id not in known_sessions:
            verbose_logger.warning(
                "MCP session ID '%s' not found in active sessions. " "Stripping stale header to force new session creation.",
                _session_id,
            )
            scope["headers"] = [(k, v) for k, v in scope["headers"] if k != _mcp_session_header]
        ...
        try:
            ...
            for server_name in mcp_servers or []:
                ...
                if server and server.auth_type == MCPAuth.oauth2 and not oauth2_headers:
                    ...
                    authorization_uri = (
                        f"Bearer authorization_uri=" f"{base_url}/.well-known/oauth-authorization-server/{server_name}"
                    )
