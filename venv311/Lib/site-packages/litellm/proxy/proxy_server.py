...
def generate_feedback_box():
    ...
    print("\033[1;37m" + "# {:^59} #\033[0m".format("https://github.com/BerriAI/litellm/issues/new"))  # noqa  # noqa
    print("\033[1;37m" + "#" + " " * box_width + "#\033[0m")  # noqa
    print("\033[1;37m" + "#" + "-" * box_width + "#\033[0m")  # noqa
    print()  # noqa
    ...
    print()  # noqa
    print()  # noqa
    print("\033[1;31mGive Feedback / Get Help: https://github.com/BerriAI/litellm/issues/new\033[0m")  # noqa  # noqa
    print()  # noqa
    print()  # noqa
...
from litellm.proxy._experimental.mcp_server.discoverable_endpoints import router as mcp_discoverable_endpoints_router
from litellm.proxy._experimental.mcp_server.rest_endpoints import router as mcp_rest_endpoints_router
...
from litellm.proxy.anthropic_endpoints.skills_endpoints import router as anthropic_skills_router
...
from litellm.proxy.management_endpoints.callback_management_endpoints import router as callback_management_endpoints_router
...
from litellm.proxy.search_endpoints.search_tool_management import router as search_tool_management_router
...
from litellm.proxy.vector_store_endpoints.management_endpoints import router as vector_store_management_router
from litellm.proxy.vector_store_files_endpoints.endpoints import router as vector_store_files_router
from litellm.proxy.vertex_ai_endpoints.langfuse_endpoints import router as langfuse_router
...
global_max_parallel_request_retry_timeout_env: Optional[str] = os.getenv("LITELLM_GLOBAL_MAX_PARALLEL_REQUEST_RETRY_TIMEOUT")
...
async def proxy_startup_event(app: FastAPI):  # noqa: PLR0915
    ...
    ## CHECK PREMIUM USER
    verbose_proxy_logger.debug("litellm.proxy.proxy_server.py::startup() - CHECKING PREMIUM USER - {}".format(premium_user))
...
try:
    ...
    if is_non_root:
        ...
        if _dir_has_content(runtime_ui_path):
            ...
            verbose_proxy_logger.error(f"UI not found at {runtime_ui_path}. Attempting to populate it from packaged UI.")
        ...
        if shared_health_manager is not None:
            try:
                (
                    healthy_endpoints,
                    unhealthy_endpoints,
                ) = await shared_health_manager.perform_shared_health_check(model_list=_llm_model_list, details=details_bool)
            except Exception as e:
                verbose_proxy_logger.error(
...
class ProxyConfig:
    ...
    async def save_config(self, new_config: dict):
        ...
        if prisma_client is not None and (general_settings.get("store_model_in_db", False) is True or store_model_in_db):
            ...
            if "environment_variables" in config_to_save and config_to_save["environment_variables"]:
                ...
                config_to_save["environment_variables"] = self._encrypt_env_variables(environment_variables=decrypted_env_vars)
    ...
    async def load_config(self, router: Optional[litellm.Router], config_file_path: str):  # noqa: PLR0915
        """
            ...
            for key, value in litellm_settings.items():
                if key == "cache" and value is True:
                    ...
                    for callback in value:
                        ...
                        if "." in callback:
                            litellm.logging_callback_manager.add_litellm_success_callback(get_instance_fn(value=callback))
                    ...
                    for callback in value:
                        # user passed custom_callbacks.async_on_succes_logger. They need us to import a function
                        if "." in callback:
                            litellm.logging_callback_manager.add_litellm_failure_callback(get_instance_fn(value=callback))
                        # these are litellm callbacks - "langfuse", "sentry", "wandb"
                        else:
        ...
        if model_list:
            ...
            print("\033[32mLiteLLM: Proxy initialized with Config, Set models:\033[0m")  # noqa  # noqa
        ...
        router = litellm.Router(
            **router_params,
            ...
            router_general_settings=RouterGeneralSettings(async_only_mode=True),  # only init async clients
    ...
    def initialize_secret_manager(
        ...
        if key_management_system is not None:
            ...
            elif key_management_system == KeyManagementSystem.AWS_SECRET_MANAGER.value:  # noqa: F405
    ...
    async def _add_router_settings_from_db_config(
        ...
        if llm_router is not None and prisma_client is not None:
            db_router_settings = await prisma_client.db.litellm_config.find_first(where={"param_name": "router_settings"})
    ...
    async def add_deployment(
        ...
        try:
            ...
            verbose_proxy_logger.exception("litellm.proxy.proxy_server.py::ProxyConfig:add_deployment - {}".format(str(e)))
    ...
    async def _init_prompts_in_db(self, prisma_client: PrismaClient):
        ...
        try:
            ...
            verbose_proxy_logger.debug("litellm.proxy.proxy_server.py::ProxyConfig:_init_prompts_in_db - {}".format(str(e)))
    ...
    async def _init_agents_in_db(self, prisma_client: PrismaClient):
        ...
        try:
            ...
            verbose_proxy_logger.exception("litellm.proxy.proxy_server.py::ProxyConfig:_init_agents_in_db - {}".format(str(e)))
    ...
    async def _init_search_tools_in_db(self, prisma_client: PrismaClient):
        ...
        try:
            ...
            if len(search_tools) > 0:
                ...
                verbose_proxy_logger.debug("No search tools found in database, keeping config-loaded search tools (if any)")
...
class ProxyStartupEvent:
    ...
    async def _initialize_semantic_tool_filter(
        ...
        verbose_proxy_logger.debug(
            f"Initializing semantic tool filter: llm_router={llm_router is not None}, " f"config={mcp_semantic_filter_config}"
        )
...
@router.get("/models", dependencies=[Depends(user_api_key_auth)], tags=["model management"])  # if project requires model list
...
async def moderations(
    ...
    try:
        ...
        verbose_proxy_logger.exception("litellm.proxy.proxy_server.moderations(): Exception occured - {}".format(str(e)))
        if isinstance(e, HTTPException):
            raise ProxyException(
...
async def create_assistant(
    ...
    try:
        ...
        verbose_proxy_logger.error("litellm.proxy.proxy_server.create_assistant(): Exception occured - {}".format(str(e)))
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
                message=getattr(e, "message", str(e.detail)),
...
async def delete_assistant(
    ...
    try:
        ...
        verbose_proxy_logger.error("litellm.proxy.proxy_server.delete_assistant(): Exception occured - {}".format(str(e)))
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
                message=getattr(e, "message", str(e.detail)),
...
async def non_admin_all_models(
    ...
    if user_api_key_dict.user_id:
        try:
            user_row = await prisma_client.db.litellm_usertable.find_unique(where={"user_id": user_api_key_dict.user_id})
        except Exception:
...
async def get_all_team_and_direct_access_models(
    ...
    if user_api_key_dict.user_role == LitellmUserRoles.PROXY_ADMIN:
        ...
        user_db_object = await prisma_client.db.litellm_usertable.find_unique(where={"user_id": user_api_key_dict.user_id})
        ...
        try:
            ...
            if sort_by:
                ...
                if router_models_count < models_needed_for_page:
                    models_to_fetch = min(models_needed_for_page - router_models_count, db_models_total_count)
...
async def model_info_v2(
    ...
    include_team_models: Optional[bool] = fastapi.Query(False, description="Return all models across all teams user is in."),
...
async def get_model_cost_map_reload_status(
    ...
    try:
        ...
        config_record = await prisma_client.db.litellm_config.find_unique(where={"param_name": "model_cost_map_reload_config"})
