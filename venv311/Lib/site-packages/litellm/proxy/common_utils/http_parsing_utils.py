...
def check_file_size_under_limit(
    ...
    if llm_router is not None and request_data["model"] in router_model_names:
        try:
            ...
            if deployment and deployment.litellm_params is not None and deployment.litellm_params.max_file_size_mb is not None:
        ...
        try:
            ...
            for part in parts[:-1]:
                if not isinstance(current, dict):
                    verbose_proxy_logger.warning(f"Cannot create nested path - intermediate value is not a dict at: {part}")
                    break
