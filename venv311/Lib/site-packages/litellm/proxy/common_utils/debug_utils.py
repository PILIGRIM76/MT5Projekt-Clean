...
def _get_uncollectable_objects_info() -> Dict[str, Any]:
    ...
    return {
        ...
        "warning": (
            "If count > 0, you may have reference cycles preventing garbage collection" if len(uncollectable) > 0 else None
        ),
    }
...
def _get_process_memory_info(worker_pid: int, include_process_info: bool) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        return {
            ...
            "ram_usage": {"megabytes": ram_usage_mb, "description": "Actual physical RAM used by this process"},
            "virtual_memory": {
                "megabytes": virtual_memory_mb,
                "description": "Total virtual memory allocated (includes swapped memory)",
            },
            "system_memory_percent": {"percent": memory_percent, "description": "Percentage of total system RAM being used"},
            "open_file_handles": {
                "count": process.num_fds() if hasattr(process, "num_fds") else "N/A (Windows)",
                "description": "Number of open file descriptors/handles",
            },
            "threads": {"count": process.num_threads(), "description": "Number of active threads in this process"},
        }
    except ImportError:
        return {"pid": worker_pid, "error": "psutil not installed. Install with: pip install psutil"}
    except Exception as e:
...
async def configure_gc_thresholds_endpoint(
    ...
    try:
        ...
        verbose_proxy_logger.info(
            f"GC thresholds updated from {old_thresholds} to " f"({generation_0}, {generation_1}, {generation_2})"
        )
    except Exception as e:
