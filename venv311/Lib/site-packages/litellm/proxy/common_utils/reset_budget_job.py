...
class ResetBudgetJob:
    ...
    async def reset_budget_for_litellm_team_members(self, budgets_to_reset: List[LiteLLM_BudgetTableFull]):
        ...
        return await self.prisma_client.db.litellm_teammembership.update_many(
            where={"budget_id": {"in": [budget.budget_id for budget in budgets_to_reset if budget.budget_id is not None]}},
            data={
                "spend": 0,
            },
        )
    ...
    async def reset_budget_for_litellm_budget_table(self):
        ...
        failed_endusers = []
        try:
            budgets_to_reset = await self.prisma_client.get_data(table_name="budget", query_type="find_all", reset_at=now)
            ...
            if budgets_to_reset is not None and len(budgets_to_reset) > 0:
                ...
                endusers_to_reset = await self.prisma_client.get_data(
                    table_name="enduser",
                    query_type="find_all",
                    budget_id_list=[budget.budget_id for budget in budgets_to_reset if budget.budget_id is not None],
                )
            ...
            if len(failed_endusers) > 0:  # If any endusers failed to reset
                raise Exception(f"Failed to reset {len(failed_endusers)} endusers: {json.dumps(failed_endusers, default=str)}")
    ...
    @staticmethod
    async def _reset_budget_reset_at_date(budget: LiteLLM_BudgetTableFull, current_time: datetime) -> LiteLLM_BudgetTableFull:
        try:
            if budget.budget_duration is not None:
                ...
                if budget.budget_reset_at is None and budget.created_at + timedelta(seconds=duration_s) > current_time:
