...
class DBSpendUpdateWriter:
    ...
    async def _update_team_db(
        ...
        try:
            ...
            verbose_proxy_logger.debug(f"Update Team DB failed to execute - {str(e)}\n{traceback.format_exc()}")
            raise e
    ...
    async def _update_org_db(
        ...
        try:
            ...
            verbose_proxy_logger.debug(f"Update Org DB failed to execute - {str(e)}\n{traceback.format_exc()}")
            raise e
    ...
    async def _update_tag_db(
        ...
        try:
            ...
            verbose_proxy_logger.debug(f"Update Tag DB failed to execute - {str(e)}\n{traceback.format_exc()}")
            raise e
    ...
    async def _insert_spend_log_to_db(
        ...
        verbose_proxy_logger.debug(
            "Writing spend log to db - request_id: {}, spend: {}".format(payload.get("request_id"), payload.get("spend"))
        )
    ...
    async def _commit_spend_updates_to_db_with_redis(
        ...
        if await self.pod_lock_manager.acquire_lock(
            ...
            try:
                db_spend_update_transactions = await self.redis_update_buffer.get_all_update_transactions_from_redis_buffer()
    ...
    async def _update_daily_spend(
        ...
        verbose_proxy_logger.debug(f"Daily {entity_type.capitalize()} Spend transactions: {len(daily_spend_transactions)}")
        BATCH_SIZE = 100
        start_time = time.time()
            ...
            for i in range(n_retry_times + 1):
                try:
                    ...
                    if len(transactions_to_process) == 0:
                        verbose_proxy_logger.debug(f"No new transactions to process for daily {entity_type} spend update")
                        break
    ...
    async def add_spend_log_transaction_to_daily_agent_transaction(
        ...
        if payload["agent_id"] is None:
            verbose_proxy_logger.debug("agent_id is None for request. Skipping incrementing agent spend.")
            return
    ...
    async def add_spend_log_transaction_to_daily_tag_transaction(
        ...
        for tag in request_tags:
            ...
            daily_transaction = DailyTagSpendTransaction(tag=tag, **base_daily_transaction, request_id=payload["request_id"])
