...
class PrismaWrapper:
    ...
    def _calculate_seconds_until_refresh(self) -> float:
        ...
        if expiration_time is None:
            ...
            verbose_proxy_logger.debug(
                f"Could not parse token expiration, using fallback interval of " f"{self.FALLBACK_REFRESH_INTERVAL_SECONDS}s"
            )
    ...
    async def start_token_refresh_task(self) -> None:
        ...
        verbose_proxy_logger.info("Started RDS IAM token proactive refresh background task")
        ...
        while True:
            try:
                ...
                verbose_proxy_logger.error(
                    f"Error in RDS IAM token refresh loop: {e}. " f"Retrying in {self.FALLBACK_REFRESH_INTERVAL_SECONDS}s..."
                )
    ...
    async def _safe_refresh_token(self) -> None:
        ...
        async with self._reconnection_lock:
            ...
            if new_db_url:
                ...
                verbose_proxy_logger.info("RDS IAM token refreshed successfully. New token valid for ~15 minutes.")
            else:
                verbose_proxy_logger.error("Failed to generate new RDS IAM token during proactive refresh")
            ...
            if self.is_token_expired(db_url):
                ...
                if new_db_url:
                    ...
                    if loop.is_running():
                        ...
                        future = asyncio.run_coroutine_threadsafe(self.recreate_prisma_client(new_db_url), loop)
                        try:
                            # Wait up to 30 seconds for reconnection
                            future.result(timeout=30)
                            verbose_proxy_logger.info("Synchronous token refresh completed successfully")
                        except Exception as e:
                            verbose_proxy_logger.error(f"Failed to refresh token synchronously: {e}")
                            raise
                    else:
