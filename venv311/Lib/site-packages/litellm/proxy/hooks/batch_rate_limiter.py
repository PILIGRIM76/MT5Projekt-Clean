...
if TYPE_CHECKING:
    from litellm.proxy.hooks.parallel_request_limiter_v3 import RateLimitDescriptor as _RateLimitDescriptor
    from litellm.proxy.hooks.parallel_request_limiter_v3 import RateLimitStatus as _RateLimitStatus
    from litellm.proxy.hooks.parallel_request_limiter_v3 import _PROXY_MaxParallelRequestsHandler_v3 as _ParallelRequestLimiter
...
class _PROXY_BatchRateLimiter(CustomLogger):
    ...
    def _raise_rate_limit_error(
        ...
        descriptor_index = next((i for i, d in enumerate(descriptors) if d.get("key") == status.get("descriptor_key")), 0)
    ...
    async def _check_and_increment_batch_counters(
        ...
        for descriptor in descriptors:
            ...
            if rate_limit.get("tokens_per_unit") is not None:
                tpm_key = self.parallel_request_limiter.create_rate_limit_keys(key=key, value=value, rate_limit_type="tokens")
                pipeline_operations.append(
                    RedisPipelineIncrementOperation(
                        key=tpm_key,
    ...
    async def async_pre_call_hook(
        ...
        try:
            ...
            verbose_proxy_logger.debug(
                f"Batch input file usage - Tokens: {batch_usage.total_tokens}, " f"Requests: {batch_usage.request_count}"
            )
