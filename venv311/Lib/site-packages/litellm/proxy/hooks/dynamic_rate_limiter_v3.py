...
class _PROXY_DynamicRateLimitHandlerV3(CustomLogger):
    ...
    def _get_priority_allocation(
        ...
        has_explicit_priority = (
            priority is not None and litellm.priority_reservation is not None and priority in litellm.priority_reservation
        )
        ...
        try:
            ...
            if model_group_info.rpm is not None and model_group_info.rpm > 0:
                ...
                if counter_value is not None:
                    ...
                    verbose_proxy_logger.debug(
                        f"Model {model} RPM: {current_requests}/{model_group_info.rpm} " f"({rpm_saturation:.1%})"
                    )
            ...
            if model_group_info.tpm is not None and model_group_info.tpm > 0:
                ...
                if counter_value is not None:
                    ...
                    verbose_proxy_logger.debug(
                        f"Model {model} TPM: {current_tokens}/{model_group_info.tpm} " f"({tpm_saturation:.1%})"
                    )
            ...
            for status in check_response["statuses"]:
                if status["code"] == "OVER_LIMIT":
                    ...
                    # Priority limit exceeded (ONLY enforce when saturated)
                        ...
                        verbose_proxy_logger.debug(
                            f"Enforcing priority limits for {model}, saturation: {saturation:.1%}, " f"priority: {priority}"
                        )
                        raise HTTPException(
                            status_code=429,
                            detail={
