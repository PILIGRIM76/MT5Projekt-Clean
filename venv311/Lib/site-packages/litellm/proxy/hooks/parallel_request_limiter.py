...
class _PROXY_MaxParallelRequestsHandler(CustomLogger):
    ...
    async def async_pre_call_hook(  # noqa: PLR0915
        ...
        cache_objects: CacheObject = await self.get_all_cache_objects(
            current_global_requests=("global_max_parallel_requests" if global_max_parallel_requests is not None else None),
            request_count_api_key=(f"{api_key}::{precise_minute}::request_count" if api_key is not None else None),
            request_count_api_key_model=(
                f"{api_key}::{_model}::{precise_minute}::request_count" if api_key is not None and _model is not None else None
            ),
            request_count_user_id=(
        ...
        if get_key_model_tpm_limit(user_api_key_dict) is not None or get_key_model_rpm_limit(user_api_key_dict) is not None:
            _model = data.get("model", None)
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):  # noqa: PLR0915
        ...
        try:
            ...
            global_max_parallel_requests = kwargs["litellm_params"]["metadata"].get("global_max_parallel_requests", None)
            ...
            user_api_key_model_max_budget = kwargs["litellm_params"]["metadata"].get("user_api_key_model_max_budget", None)
