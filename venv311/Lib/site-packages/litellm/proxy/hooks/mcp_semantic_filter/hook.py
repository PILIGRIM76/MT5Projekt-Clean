    ...
    async def async_pre_call_hook(
        ...
        if self._should_expand_mcp_tools(tools):
            ...
            try:
                ...
                verbose_proxy_logger.info(f"Expanded {len(tools)} MCP reference(s) to {len(expanded_tools)} tools")
        ...
        try:
            ...
            verbose_proxy_logger.debug(
                f"Applying semantic filter to {len(tools)} tools " f"with query: '{user_query[:50]}...'"
            )
    ...
    async def initialize_from_config(
        ...
        try:
            ...
            verbose_proxy_logger.warning(
                f"semantic-router not installed. Install with: " f"pip install 'litellm[semantic-router]'. Error: {e}"
            )
            return None
        except Exception as e:
