...
class CustomStreamWrapper:
    ...
    def is_chunk_non_empty(
        ...
        if (
            ...
            or ("annotations" in model_response.choices[0].delta and model_response.choices[0].delta.annotations is not None)
        ):
            return True
        else:
            return False
    ...
    def chunk_creator(self, chunk: Any):  # type: ignore  # noqa: PLR0915
        ...
        try:
                    ...
                    try:
                        try:
                            ...
                            if "Part has no text." in str(e):
                                ...
                                function_call = chunk.candidates[0].content.parts[0].function_call  # type: ignore
                        ...
                        if (
                            ...
                            and chunk.candidates[0].finish_reason.name != "FINISH_REASON_UNSPECIFIED"  # type: ignore
                        ):  # every non-final chunk in vertex ai has this
                            self.received_finish_reason = chunk.candidates[0].finish_reason.name  # type: ignore
                    except Exception:
            ...
            if original_chunk is not None:  # function / tool calling branch - only set for openai/azure compatible endpoints
                ...
                if hasattr(original_chunk, "provider_specific_fields"):
                    model_response = self.copy_model_response_level_provider_specific_fields(original_chunk, model_response)
                if original_chunk.choices and len(original_chunk.choices) > 0:
                    ...
                    if delta is not None and (delta.function_call is not None or delta.tool_calls is not None):
                        ...
                        try:
                            delta = (
                                dict() if original_chunk.choices[0].delta is None else dict(original_chunk.choices[0].delta)
                            )
    ...
    async def __anext__(self):  # noqa: PLR0915
        ...
        try:
            ...
            if is_async_iterable(self.completion_stream):
                async for chunk in self.completion_stream:
                    ...
                    if hasattr(processed_chunk, "usage"):  # remove usage from chunk, only send on final chunk
                        ...
                        is_empty = is_model_response_stream_empty(model_response=cast(ModelResponseStream, processed_chunk))
            ...
            if self.logging_obj is not None:
                ...
                # Handle any exceptions that might occur during streaming
                asyncio.create_task(self.logging_obj.async_failure_handler(e, traceback_exception))  # type: ignore
