...
class LoggingCallbackManager:
    ...
    def add_litellm_callback(self, callback: Union[CustomLogger, str, Callable]):
        ...
        """
        self._safe_add_callback_to_list(callback=callback, parent_list=litellm.callbacks)  # type: ignore
    ...
    def _add_callback_function_to_list(self, callback: Callable, parent_list: List[Union[CustomLogger, Callable, str]]):
        ...
        if callback not in parent_list:
            ...
            verbose_logger.debug(f"Callback function {callback.__name__} already exists in {parent_list}, not adding again..")
    ...
    def _add_custom_logger_to_list(
        ...
        for existing_logger in parent_list:
            if isinstance(existing_logger, CustomLogger) and self._get_custom_logger_key(existing_logger) == custom_logger_key:
