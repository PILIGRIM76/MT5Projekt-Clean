...
def convert_to_anthropic_tool_invoke_xml(tool_calls: list) -> str:
    ...
    for tool in tool_calls:
        ...
        parsed_args = parse_tool_call_arguments(tool_arguments, tool_name=tool_name, context="Anthropic XML tool invoke")
    ...
    while msg_i < len(messages):
        ...
        while msg_i < len(messages) and messages[msg_i]["role"] == "assistant":
            ...
            if messages[msg_i].get("tool_calls", []):  # support assistant tool invoke conversion
                assistant_text += convert_to_anthropic_tool_invoke_xml(messages[msg_i]["tool_calls"])  # type: ignore
    ...
    while msg_i < len(messages):
        ...
        while msg_i < len(messages) and messages[msg_i]["role"] in user_message_types:
            ...
            if user_message_types_block["role"] == "user":
                ...
                user_content.append(convert_to_anthropic_tool_result(user_message_types_block, force_base64=force_base64))
...
from litellm.types.llms.bedrock import ToolInputSchemaBlock as BedrockToolInputSchemaBlock
...
from litellm.types.llms.bedrock import ToolResultContentBlock as BedrockToolResultContentBlock
...
class BedrockImageProcessor:
    ...
    def _validate_format(mime_type: str, image_format: str) -> str:
        ...
        if is_document:
            return BedrockImageProcessor._get_document_format(mime_type=mime_type, supported_doc_formats=supported_doc_formats)
    ...
    def _get_document_format(mime_type: str, supported_doc_formats: List[str]) -> str:
        ...
        if not valid_extensions:
            raise ValueError(f"No supported extensions for MIME type: {mime_type}. Supported formats: {supported_doc_formats}")
...
def _bedrock_converse_messages_pt(  # noqa: PLR0915
    ...
    messages = BedrockConverseMessagesProcessor._initial_message_setup(messages, model, llm_provider, user_continue_message)
                    ...
                    if isinstance(element, dict):
                        if element["type"] == "thinking":
                            ...
                            if isinstance(element["image_url"], dict):
                                ...
                                image_url = element["image_url"]
                            assistants_part = BedrockImageProcessor.process_image_sync(image_url=image_url)  # type: ignore
                            assistants_parts.append(assistants_part)
                        # Add cache point block for assistant content elements
                        _cache_point_block = litellm.AmazonConverseConfig()._get_cache_point_block(
                            message_block=cast(OpenAIMessageContentListBlock, element),
