...
def infer_content_type_from_url_and_content(
    ...
    raise ValueError(f"Unable to determine content type from URL: {url}. " f"Response content-type: {current_content_type}")
...
def convert_prefix_message_to_non_prefix_messages(
    ...
    for message in messages:
        if message.get("prefix"):
            ...
            new_messages.append({**{k: v for k, v in message.items() if k != "prefix"}})  # type: ignore
        else:
...
def parse_tool_call_arguments(
    ...
    try:
        ...
        error_message = " ".join(error_parts) + f". Error: {str(e)}. Arguments: {arguments}"
