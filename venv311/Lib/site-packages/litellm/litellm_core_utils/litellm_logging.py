...
class Logging(LiteLLMLoggingBaseClass):
    ...
    def __init__(
        ...
        self.standard_built_in_tools_params: StandardBuiltInToolsParams = self.initialize_standard_built_in_tools_params(
            kwargs
        )
    ...
    def initialize_standard_callback_dynamic_params(self, kwargs: Optional[Dict] = None) -> StandardCallbackDynamicParams:
        """
    ...
    def _should_run_prompt_management_hooks_without_prompt_id(
        ...
        if litellm.vector_store_registry is not None:
            if litellm.vector_store_registry.get_vector_store_to_run(non_default_params=non_default_params, tools=tools):
                return True
        return False
    ...
    def _pre_call(self, input, api_key, model=None, additional_args={}):
        ...
        self.model_call_details["litellm_params"]["api_base"] = self._get_masked_api_base(additional_args.get("api_base", ""))
        ...
        try:
            ...
            for callback in callbacks:
                try:
                    ...
                    verbose_logger.debug(f"LiteLLM.Logging: is sentry capture exception initialized {capture_exception}")
                    if capture_exception:  # log this error to sentry for debugging
                        capture_exception(e)
        except Exception as e:
            verbose_logger.exception("LiteLLM.LoggingError: [Non-Blocking] Exception occurred while logging {}".format(str(e)))
    ...
    def _get_request_curl_command(self, api_base: str, headers: Optional[dict], additional_args: dict, data: dict) -> str:
    ...
    def should_run_callback(self, callback: litellm.CALLBACK_TYPES, litellm_params: dict, event_hook: str) -> bool:
        ...
        if EnterpriseCallbackControls is not None and EnterpriseCallbackControls.is_callback_disabled_dynamically(
            ...
            verbose_logger.debug(f"Callback {callback} disabled via x-litellm-disable-callbacks header for {event_hook} event")
            return False
    ...
    def normalize_logging_result(self, result: Any) -> Any:
        ...
        if self.call_type == CallTypes.arealtime.value and isinstance(result, list):
            ...
            self.call_type == CallTypes.llm_passthrough_route.value or self.call_type == CallTypes.allm_passthrough_route.value
    ...
    def success_handler(self, result=None, start_time=None, end_time=None, cache_hit=None, **kwargs):  # noqa: PLR0915
        ...
        try:
            ...
            complete_streaming_response: Optional[Union[ModelResponse, TextCompletionResponse, ResponsesAPIResponse]] = None
            ...
            if complete_streaming_response is not None:
                ...
                self.model_call_details["response_cost"] = self._response_cost_calculator(result=complete_streaming_response)
            ...
            for callback in callbacks:
                try:
                    ...
                    if callable(callback) is True and is_sync_request and customLogger is not None:  # custom logger functions
    ...
    async def async_success_handler(  # noqa: PLR0915
        ...
        if complete_streaming_response is not None:
            ...
            try:
                ...
                verbose_logger.warning(f"Model={self.model} not found in completion cost map. Setting 'response_cost' to None")
                self.model_call_details["response_cost"] = None
    ...
    def failure_handler(self, exception, traceback_exception, start_time=None, end_time=None):  # noqa: PLR0915
        ...
        try:
            ...
            for callback in callbacks:
                try:
                    ...
                    if isinstance(callback, CustomLogger) and is_sync_request:  # custom logger class
    ...
    async def async_failure_handler(self, exception, traceback_exception, start_time=None, end_time=None):
        ...
        for callback in callbacks:
            try:
                ...
                verbose_logger.exception("LiteLLM.LoggingError: [Non-Blocking] Exception occurred while failure \
    ...
    def _get_trace_id(self, service_name: Literal["langfuse"]) -> Optional[str]:
        ...
        if service_name == "langfuse":
            trace_id = in_memory_trace_id_cache.get_cache(litellm_call_id=self.litellm_call_id, service_name=service_name)
    ...
    def _get_assembled_streaming_response(
        ...
        if isinstance(result, ModelResponse):
            ...
            if isinstance(result.response.usage, ResponseAPIUsage):
                transformed_usage = ResponseAPILoggingUtils._transform_response_api_usage_to_chat_usage(result.response.usage)
...
def _get_masked_values(
    ...
    return {
        k: (
            ...
            v
            if ignore_sensitive_values or not any(sensitive_keyword in k.lower() for sensitive_keyword in sensitive_keywords)
            else (
        ...
        if logging_integration == "agentops":  # Add AgentOps initialization
            ...
            if arize_phoenix_config.project_name:
                ...
                if existing_attrs:
                    ...
                    os.environ["OTEL_RESOURCE_ATTRIBUTES"] = f"openinference.project.name={arize_phoenix_config.project_name}"
            ...
            for callback in _in_memory_loggers:
                if isinstance(callback, OpenTelemetry):
                    return callback  # type: ignore
            otel_logger = OpenTelemetry(**_get_custom_logger_settings_from_proxy_server(callback_name=logging_integration))
            ...
            if internal_usage_cache is None:
                raise Exception("Internal Error: Cache cannot be empty - internal_usage_cache={}".format(internal_usage_cache))
...
class StandardLoggingPayloadSetup:
    ...
    @staticmethod
    def get_usage_from_response_obj(response_obj: Optional[dict], combined_usage_object: Optional[Usage] = None) -> Usage:
        ## BASE CASE ##
    ...
    def get_model_cost_information(
        ...
        if model_cost_name is None:
            ...
            try:
                ...
                model_cost_information = StandardLoggingModelInformation(model_map_key=model_cost_name, model_map_value=None)
...
def get_standard_logging_object_payload(
    ...
    try:
        ...
        if cache_hit is True:
            ...
            saved_cache_cost = (
                logging_obj._response_cost_calculator(result=init_response_obj, cache_hit=False) or 0.0  # type: ignore
            )
        ...
        payload: StandardLoggingPayload = StandardLoggingPayload(
            ...
            messages=StandardLoggingPayloadSetup.append_system_prompt_messages(kwargs=kwargs, messages=kwargs.get("messages")),
            response=final_response_obj,
            model_parameters=ModelParamHelper.get_standard_logging_model_parameters(kwargs.get("optional_params", None) or {}),
