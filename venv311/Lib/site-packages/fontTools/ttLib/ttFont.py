    ...
    def __init__(
        ...
        if not hasattr(file, "read"):
            if not isinstance(file, (str, os.PathLike)):
                raise TypeError("fileOrPath must be a file path (str or PathLike) if it isn't an object with a `read` method.")
            closeStream = True
        ...
        if not (reorderTables is None or writer_reordersTables or (reorderTables is False and self.reader is None)):
            if reorderTables is False:
                ...
                if self.reader is None:
                    raise TTLibError("The original table order is unavailable because there isn't a font to read it from.")
