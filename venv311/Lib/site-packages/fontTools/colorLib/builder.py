...
def buildPaletteLabels(labels: Iterable[_OptionalLocalizedString], nameTable: _n_a_m_e.table__n_a_m_e) -> List[Optional[int]]:
    return [
        (
            ...
            else (C_P_A_L_.table_C_P_A_L_.NO_NAME_ID if l is None else nameTable.addMultilingualName({"en": l}, mac=False))
        )
        for l in labels
    ]
    ...
    if any(v is not None for v in (paletteTypes, paletteLabels, paletteEntryLabels)):
        ...
        if paletteEntryLabels is not None:
            if len(paletteEntryLabels) != cpal.numPaletteEntries:
                raise ColorLibError(f"Expected {cpal.numPaletteEntries} paletteEntryLabels, " f"got {len(paletteEntryLabels)}")
...
def _check_base_glyphs_exist(colorGlyphs, glyphMap, where):
    ...
    if missing:
        ...
        raise ColorLibError(f"{where}: base glyph(s) not found in glyphMap: {preview}{extra}")
