...
def intersect(self, glyphs):
    ...
    return _uniq_sort(
        ([0] if any(g not in self.classDefs for g in glyphs) else []) + [v for g, v in self.classDefs.items() if g in glyphs]
    )
...
def subset(self, glyphs, remap=False, useClass0=True):
    ...
    indices = _uniq_sort(
        ([0] if ((not useClass0) or any(g not in self.classDefs for g in glyphs)) else []) + list(self.classDefs.values())
    )
    if remap:
        self.remap(indices)
    return indices
...
def subset_glyphs(self, s):
    if self.Format == 1:
        ...
        class1_map = [
            c for c in self.ClassDef1.subset(s.glyphs.intersection(self.Coverage.glyphs), remap=True) if c < self.Class1Count
        ]
    ...
    if self.Format == 1:
        ...
        for i in indices:
            ...
            for r in getattr(rss[i], c.Rule):
                if not r:
                    continue
                if not all(all(c.Intersect(s.glyphs, cd, k) for k in klist) for cd, klist in zip(ContextData, c.RuleData(r))):
                    continue
                chaos = set()
...
def subset_lookups(self, lookup_indices):
    ...
    return [
        i for i, f in enumerate(self.FeatureRecord) if (f.Feature.subset_lookups(lookup_indices) or f.FeatureTag == "pref")
    ]
...
def subset_features(self, feature_indices):
    ...
    while self.FeatureVariationRecord and not self.FeatureVariationRecord[-1].FeatureTableSubstitution.SubstitutionCount:
...
def prune_features(self, feature_index_map, keepEmptyDefaultLangSys=False):
    if self.DefaultLangSys and not self.DefaultLangSys.prune_features(feature_index_map) and not keepEmptyDefaultLangSys:
        self.DefaultLangSys = None
...
def prune_features(self, feature_index_map, retain_empty):
    # https://bugzilla.mozilla.org/show_bug.cgi?id=1331737#c32
    self.ScriptRecord = [
        s for s in self.ScriptRecord if s.Script.prune_features(feature_index_map, s.ScriptTag == "DFLT") or retain_empty
    ]
    self.ScriptCount = len(self.ScriptRecord)
    return bool(self.ScriptCount)
...
def subset_glyphs(self, s):
    ...
    self.table.LayerList, self.table.BaseGlyphList = buildColrV1({g: colorGlyphsV1[g] for g in colorGlyphsV1 if g in s.glyphs})
...
def subset_glyphs(self, s):
    ...
    self.table.VarCompositeGlyphs.VarCompositeGlyph = _list_subset(self.table.VarCompositeGlyphs.VarCompositeGlyph, indices)
