...
class Builder(object):
    ...
    def build(self, tables=None, debug=False):
        ...
        if tables is None:
            ...
            if unsupported:
                ...
                raise NotImplementedError("The following tables were requested but are unsupported: " f"{unsupported_string}.")
        if "GSUB" in tables:
        ...
        for tag in ("GPOS", "GSUB"):
            ...
            if table.ScriptList.ScriptCount > 0 or table.FeatureList.FeatureCount > 0 or table.LookupList.LookupCount > 0:
    ...
    def buildFeatureParams(self, tag):
        ...
        if tag == "size":
            ...
            params.Format = 0
            params.FeatUILabelNameID = self.cv_parameters_ids_.get((tag, "FeatUILabelNameID"), NO_NAME_ID)
            params.FeatUITooltipTextNameID = self.cv_parameters_ids_.get((tag, "FeatUITooltipTextNameID"), NO_NAME_ID)
            params.SampleTextNameID = self.cv_parameters_ids_.get((tag, "SampleTextNameID"), NO_NAME_ID)
            params.NumNamedParameters = self.cv_num_named_params_.get(tag, 0)
            params.FirstParamUILabelNameID = self.cv_parameters_ids_.get((tag, "ParamUILabelNameID_0"), NO_NAME_ID)
    ...
    def _add_contextual_rule(chain, prefix, glyphs, suffix, lookups):
        ...
        if len(glyphs) == 1 and chain.rules and not chain.rules[-1].is_subtable_break:
            last = chain.rules[-1]
            if len(last.glyphs) == 1 and last.prefix == prefix and last.suffix == suffix and last.lookups == lookups:
    ...
    def makeVariablePos(self, location, varscalar: VariableScalar) -> tuple[int, int | None]:
        ...
        try:
            default, index = self.scalar_builder.add_to_variation_store(varscalar, self.varstorebuilder)
