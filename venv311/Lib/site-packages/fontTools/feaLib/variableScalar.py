...
class VariableScalar:
    ...
    def __init__(self, location_value=None):
        self.values = {Location(location): value for location, value in (location_value or {}).items()}
    ...
    def add_to_variation_store(self, store_builder, model_cache=None, avar=None):
        ...
        if not self.axes:
            raise ValueError(".axes must be defined on variable scalar before calling " "add_to_variation_store()")
        builder = VariableScalarBuilder(
            axis_triples={ax.axisTag: (ax.minValue, ax.defaultValue, ax.maxValue) for ax in self.axes},
...
class VariableScalarBuilder:
    ...
    def from_ttf(cls, ttf: TTFont) -> Self:
        return cls(
            axis_triples={axis.axisTag: (axis.minValue, axis.defaultValue, axis.maxValue) for axis in ttf["fvar"].axes},
            axis_mappings=(
                {} if (avar := ttf.get("avar")) is None else {axis: segments for axis, segments in avar.segments.items()}
            ),
            model_cache={},
        )
    ...
    def from_designspace(cls, doc: DesignSpaceDocument) -> Self:
        return cls(
            axis_triples={axis.tag: (axis.minimum, axis.default, axis.maximum) for axis in doc.axes},
            axis_mappings={
                axis.tag: {
                    normalizeValue(user, (axis.minimum, axis.default, axis.maximum)): normalizeValue(
                        design,
                        (
    ...
    def model(self, scalar: VariableScalar) -> VariationModel:
        ...
        model = self.model_cache[cache_key] = VariationModel(normalized_locations, axisOrder=axisOrder)
