...
class Parser(object):
    ...
    def __init__(self, featurefile, glyphNames=(), followIncludes=True, includeDir=None, **kwargs):
        ...
        if kwargs:
            raise TypeError(
                "unsupported keyword argument%s: %s" % ("" if len(kwargs) == 1 else "s", ", ".join(repr(k) for k in kwargs))
            )
    ...
    def parse(self):
        ...
        if self.missing:
            ...
            raise FeatureLibError(
                "The following glyph names are referenced but are missing from the " "glyph set:\n" + ("\n".join(error)),
                None,
            )
        return self.doc_
    ...
    def parse_glyph_pattern_(self, vertical):
        ...
        while self.next_token_ not in {"by", "from", ";", ","}:
            ...
            if marked:
                if suffix:
                    ...
                    raise FeatureLibError(
                        "Unsupported contextual target sequence: at most " "one run of marked (') glyph/class names allowed",
                        self.cur_token_location_,
                    )
                glyphs.append(gc)
            elif glyphs:
        ...
        if not glyphs and not suffix:  # eg., "sub f f i by"
            ...
            if any(values[: len(prefix)]):
                raise FeatureLibError(
                    "Positioning cannot be applied in the bactrack glyph sequence, " "before the marked glyph sequence.",
                    self.cur_token_location_,
                )
