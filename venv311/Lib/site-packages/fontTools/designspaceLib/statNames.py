...
def _getAxisLabelsForUserLocation(
    ...
    if allAxisLabels.keys() != userLocation.keys():
        LOGGER.warning(
            f"Mismatch between user location '{userLocation.keys()}' and available " f"labels for '{allAxisLabels.keys()}'."
        )
    ...
    for axisName, axisLabels in allAxisLabels.items():
        ...
        label: Optional[AxisLabelDescriptor] = next(
            (
                ...
                or (l.userMinimum is not None and l.userMaximum is not None and l.userMinimum <= userValue <= l.userMaximum)
            ),
            None,
        )
        if label is None:
...
def _getRibbiStyle(self: DesignSpaceDocument, userLocation: SimpleLocationDict) -> Tuple[RibbiStyleName, SimpleLocationDict]:
