...
class UFOReader(_UFOBaseIO):
    ...
    def _readMetaInfo(self, validate: Optional[bool] = None) -> dict[str, Any]:
        ...
        try:
            ...
            unsupportedMsg = (
                f"Unsupported UFO format ({formatVersionMajor}.{formatVersionMinor}) " f"in '{METAINFO_FILENAME}' on {self.fs}"
            )
            if validate:
    ...
    def getCharacterMapping(self, layerName: Optional[str] = None, validate: Optional[bool] = None) -> dict[int, list[str]]:
        """
...
class UFOWriter(UFOReader):
    ...
    def writeLayerContents(self, layerOrder: LayerOrderList = None, validate: Optional[bool] = None) -> None:
        ...
        layerContents = [(layerName, self.layerContents[layerName]) for layerName in layerOrder if layerName is not None]
