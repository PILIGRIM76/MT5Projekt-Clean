...
def run(argv: List[Text]):
    ...
    parser.add_argument("--diff", type=str, help="Run external diff tool command (default: diff)")
    parser.add_argument(
        "--diff-arg",
        type=str,
        default=None,
    ...
    if not file_exists(args.FILE1):
        if not args.quiet:
            sys.stderr.write(f"[*] ERROR: The file path '{args.FILE1}' can not be found.{os.linesep}")
        return 2
    if not file_exists(args.FILE2):
        if not args.quiet:
            sys.stderr.write(f"[*] ERROR: The file path '{args.FILE2}' can not be found.{os.linesep}")
        return 2
    ...
    if args.binary:
        ...
        if include_list is not None:
            include_list = [tag for tag in include_list if tag not in excluded_binary_tables]
        else:
    ...
    diff_tool = args.diff
    color_output = args.color == "always" or (args.color == "auto" and sys.stdout.isatty)
    ...
    if diff_tool is None:
        ...
        if diff_tool is None:
            if not args.quiet:
                sys.stderr.write(f"[*] ERROR: The external diff tool executable " f"'{args.diff}' was not found.{os.linesep}")
            return 2
