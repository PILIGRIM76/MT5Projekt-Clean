...
def _add_fvar(font, axes, instances: List[InstanceDescriptor]):
    ...
    for instance in instances:
        ...
        if subfamilyNameID in {2, 17} and inst.coordinates == default_coordinates:
            ...
            inst.subfamilyNameID = nameTable.addMultilingualName(localisedStyleName, windows=True, mac=macNames, minNameID=256)
...
def _has_inconsistent_use_my_metrics_flag(master_glyf, glyph_name, flagged_components, expected_num_components) -> bool:
    ...
    if master_glyph is not None and master_glyph.isComposite() and len(master_glyph.components) == expected_num_components:
...
def _unset_inconsistent_use_my_metrics_flags(vf, master_fonts):
    ...
    for glyph_name in glyf.keys():
        ...
        flagged_components = [(i, comp.glyphName) for i, comp in enumerate(glyph.components) if (comp.flags & USE_MY_METRICS)]
