...
class Cluster:
    ...
    def get_status(self):
        ...
        process = subprocess.run(
            [self._pg_ctl, "status", "-D", self._data_dir], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        stdout, stderr = process.stdout, process.stderr

        if process.returncode == 4 or not os.path.exists(self._data_dir) or not os.listdir(self._data_dir):
            ...
            if not r:
                raise ClusterError("could not parse pg_ctl status output: {}".format(stdout.decode()))
            self._daemon_pid = int(r.group(1))
            return self._test_connection(timeout=0)
        else:
            raise ClusterError("pg_ctl status exited with status {:d}: {}".format(process.returncode, stderr))
    ...
    def init(self, **settings):
        ...
        if self.get_status() != "not-initialized":
            raise ClusterError("cluster in {!r} has already been initialized".format(self._data_dir))
        ...
        if process.returncode != 0:
            raise ClusterError("pg_ctl init exited with status {:d}:\n{}".format(process.returncode, output.decode()))
        ...
        if _system == "Windows":
            ...
            if os.getenv("ASYNCPG_DEBUG_SERVER"):
                ...
                print(
                    ...
                    " ".join([self._pg_ctl, "start", "-D", self._data_dir, "-o", " ".join(extra_args)]),
                    file=sys.stderr,
                )
            else:
    ...
    def reload(self):
        ...
        if process.returncode != 0:
            raise ClusterError("pg_ctl stop exited with status {:d}: {}".format(process.returncode, stderr.decode()))
    ...
    def reset_wal(self, *, oid=None, xid=None):
        ...
        if process.returncode != 0:
            raise ClusterError("pg_resetwal exited with status {:d}: {}".format(process.returncode, stderr.decode()))
    ...
    def add_hba_entry(self, *, type="host", database, user, address=None, auth_method, auth_options=None):
    ...
    def trust_local_connections(self):
        ...
        self.add_hba_entry(type="host", address="127.0.0.1/32", database="all", user="all", auth_method="trust")
        self.add_hba_entry(type="host", address="::1/128", database="all", user="all", auth_method="trust")
        status = self.get_status()
        if status == "running":
            self.reload()
    ...
    def trust_local_replication_by(self, user):
        ...
        self.add_hba_entry(type="host", address="127.0.0.1/32", database="replication", user=user, auth_method="trust")
        self.add_hba_entry(type="host", address="::1/128", database="replication", user=user, auth_method="trust")
        status = self.get_status()
        if status == "running":
            self.reload()
            ...
            for i in range(timeout):
                ...
                try:
                    con = loop.run_until_complete(
                        asyncpg.connect(database="postgres", user="postgres", timeout=5, loop=loop, **self._connection_addr)
                    )
                except (OSError, asyncio.TimeoutError, asyncpg.CannotConnectNowError, asyncpg.PostgresConnectionError):
                    time.sleep(1)
                    continue
    ...
    def _run_pg_config(self, pg_config_path):
        ...
        if process.returncode != 0:
            raise ClusterError("pg_config exited with status {:d}: {}".format(process.returncode, stderr))
        else:
            config = {}
    ...
    def _find_pg_binary(self, binary):
        ...
        if not os.path.isfile(bpath):
            raise ClusterError(
                "could not find {} executable: ".format(binary) + "{!r} does not exist or is not a file".format(bpath)
            )

        return bpath

    def _get_pg_version(self):
        process = subprocess.run([self._postgres, "--version"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        stdout, stderr = process.stdout, process.stderr

        if process.returncode != 0:
            raise ClusterError("postgres --version exited with status {:d}: {}".format(process.returncode, stderr))

        version_string = stdout.decode("utf-8").strip(" \n")
        prefix = "postgres (PostgreSQL) "
        if not version_string.startswith(prefix):
            raise ClusterError("could not determine server version from {!r}".format(version_string))
...
class TempCluster(Cluster):
    def __init__(self, *, data_dir_suffix=None, data_dir_prefix=None, data_dir_parent=None, pg_config_path=None):
        self._data_dir = _mkdtemp(suffix=data_dir_suffix, prefix=data_dir_prefix, dir=data_dir_parent)
...
class HotStandbyCluster(TempCluster):
    ...
    def init(self, **settings):
        ...
        if process.returncode != 0:
            raise ClusterError("pg_basebackup init exited with status {:d}:\n{}".format(process.returncode, output.decode()))
        ...
        if self._pg_version < (12, 0):
            with open(os.path.join(self._data_dir, "recovery.conf"), "w") as f:
                ...
                """.format(host=self._master["host"], port=self._master["port"], user=self._repl_user)))
        else:
