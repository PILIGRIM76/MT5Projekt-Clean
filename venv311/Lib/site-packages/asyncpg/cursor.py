...
class CursorFactory(connresource.ConnectionResource):
    ...
    def __await__(self):
        if self._prefetch is not None:
            raise exceptions.InterfaceError("prefetch argument can only be specified for iterable cursor")
        cursor = Cursor(
            self._connection,
            self._query,
            self._state,
...
class BaseCursor(connresource.ConnectionResource):
    ...
    def _check_ready(self):
        ...
        if not self._connection._top_xact:
            raise exceptions.NoActiveSQLTransactionError("cursor cannot be created outside of a transaction")
    ...
    async def _bind_exec(self, n, timeout):
        ...
        buffer, _, self._exhausted = await protocol.bind_execute(self._state, self._args, self._portal_name, n, True, timeout)
        return buffer
    ...
    async def _exec(self, n, timeout):
        ...
        buffer, _, self._exhausted = await protocol.execute(self._state, self._portal_name, n, True, timeout)
        return buffer
    ...
    def __repr__(self):
        ...
        return '<{}.{} "{!s:.30}" {}{:#x}>'.format(mod, self.__class__.__name__, self._state.query, " ".join(attrs), id(self))
