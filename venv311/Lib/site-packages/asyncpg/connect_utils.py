...
def _read_password_from_pgpass(
    *, passfile: typing.Optional[pathlib.Path], hosts: typing.List[str], ports: typing.List[int], database: str, user: str
):
    ...
    *,
    ...
    if dsn:
        ...
        if parsed.scheme not in {"postgresql", "postgres"}:
            raise exceptions.ClientConfigurationError(
                "invalid DSN: scheme is expected to be either " '"postgresql" or "postgres", got {!r}'.format(parsed.scheme)
            )
        ...
        if connection_service_file is not None and service is not None:
            ...
            if service in pg_service.sections():
                ...
                if "target_session_attrs" in service_params:
                    dsn_target_session_attrs = service_params.pop("target_session_attrs")
                    if target_session_attrs is None:
                        target_session_attrs = dsn_target_session_attrs
    ...
    if direct_tls is not None:
        ...
        if sslnegotiation is not None:
            try:
                ...
                raise exceptions.ClientConfigurationError(f"`sslnegotiation` parameter must be one of: {modes}") from None
        else:
    ...
    if isinstance(ssl, (str, SSLMode)):
        try:
            ...
            raise exceptions.ClientConfigurationError("`sslmode` parameter must be one of: {}".format(modes)) from None
                ...
                if sslrootcert:
                    ...
                    try:
                        ...
                        if sslrootcert is not None:
                            ...
                            raise exceptions.ClientConfigurationError(
                                "cannot determine location of user " "PostgreSQL configuration directory"
                            )
                    except (
                        ...
                        if sslmode > SSLMode.require:
                            if sslrootcert is None:
                                ...
                                detail = None
                            raise exceptions.ClientConfigurationError(
                                f'root certificate file "{sslrootcert}" does ' f"not exist or cannot be accessed",
    ...
    if gsslib not in {"gssapi", "sspi"}:
        raise exceptions.ClientConfigurationError(
            "gsslib parameter must be either 'gssapi' or 'sspi'" ", got {!r}".format(gsslib)
        )
    ...
    *,
    ...
    if command_timeout is not None:
        try:
            ...
            raise ValueError(
                "invalid command_timeout value: " "expected greater than 0 float (got {!r})".format(command_timeout)
            ) from None
    ...
    def data_received(self, data: bytes) -> None:
        if data == b"S":
            ...
            self.on_data.set_exception(
                ConnectionError(
                    'PostgreSQL server at "{host}:{port}" ' "rejected SSL upgrade".format(host=self.host, port=self.port)
                )
            )
...
async def __connect_addr(
    ...
    if isinstance(addr, str):
        ...
        connector = _create_ssl_connection(
            proto_factory, *addr, loop=loop, ssl_context=params.ssl, ssl_is_advisory=params.sslmode == SSLMode.prefer
        )
    else:
...
async def _connect(*, loop, connection_class, record_class, **kwargs):
    ...
    try:
        ...
        async def _close_candidates(conns, chosen):
            await asyncio.gather(*(c.close() for c in conns if c is not chosen), return_exceptions=True)
    ...
    if isinstance(addr, str):
        ...
        if params.ssl and params.sslmode != SSLMode.allow:
            tr, pr = await _create_ssl_connection(
                CancelProto, *addr, loop=loop, ssl_context=params.ssl, ssl_is_advisory=params.sslmode == SSLMode.prefer
            )
        else:
