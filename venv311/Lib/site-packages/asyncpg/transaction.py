...
class Transaction(connresource.ConnectionResource):
    ...
    __slots__ = ("_connection", "_isolation", "_readonly", "_deferrable", "_state", "_nested", "_id", "_managed")

    def __init__(self, connection, isolation, readonly, deferrable):
        super().__init__(connection)

        if isolation and isolation not in ISOLATION_LEVELS:
            raise ValueError("isolation is expected to be either of {}, " "got {!r}".format(ISOLATION_LEVELS, isolation))
    ...
    async def __aenter__(self):
        if self._managed:
            raise apg_errors.InterfaceError("cannot enter context: already in an `async with` block")
        self._managed = True
        await self.start()
    ...
    def __check_state_base(self, opname):
        if self._state is TransactionState.COMMITTED:
            raise apg_errors.InterfaceError("cannot {}; the transaction is already committed".format(opname))
        if self._state is TransactionState.ROLLEDBACK:
            raise apg_errors.InterfaceError("cannot {}; the transaction is already rolled back".format(opname))
        if self._state is TransactionState.FAILED:
            raise apg_errors.InterfaceError("cannot {}; the transaction is in error state".format(opname))

    def __check_state(self, opname):
        if self._state is not TransactionState.STARTED:
            if self._state is TransactionState.NEW:
                raise apg_errors.InterfaceError("cannot {}; the transaction is not yet started".format(opname))
    ...
    async def commit(self):
        ...
        if self._managed:
            raise apg_errors.InterfaceError("cannot manually commit from within an `async with` block")
        await self.__commit()
    ...
    async def rollback(self):
        ...
        if self._managed:
            raise apg_errors.InterfaceError("cannot manually rollback from within an `async with` block")
        await self.__rollback()
