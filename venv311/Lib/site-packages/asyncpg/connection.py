...
class Connection(metaclass=ConnectionMeta):
        ...
        *,
        ...
        if use_cache:
            ...
            use_cache = self._stmt_cache_enabled and (
                not self._config.max_cacheable_statement_size or len(query) <= self._config.max_cacheable_statement_size
            )
        ...
        while types_with_missing_codecs:
            ...
            if tries > 5:
                ...
                raise exceptions.InternalClientError(
                    "could not resolve query result and/or argument types " "in {} attempts".format(tries)
                )
    ...
    async def _prepare(
        self, query, *, name: typing.Union[str, bool, None] = None, timeout=None, use_cache: bool = False, record_class=None
    ):
        self._check_open()
        if name is None:
    ...
    async def fetchmany(
        ...
        *,
        ...
        """
        self._check_open()
        return await self._executemany(query, args, timeout, return_rows=True, record_class=record_class)
    ...
    async def copy_to_table(
        ...
        *,
        ...
        copy_stmt = "COPY {tab}{cols} FROM STDIN {opts} {cond}".format(tab=tabname, cols=cols, opts=opts, cond=cond)

        return await self._copy_in(copy_stmt, source, timeout)

    async def copy_records_to_table(self, table_name, *, records, columns=None, schema_name=None, timeout=None, where=None):
        ...
        return await self._protocol.copy_in(copy_stmt, None, None, records, intro_ps._state, timeout)

    def _format_copy_where(self, where):
        if where and not self._server_caps.sql_copy_from_where:
            raise exceptions.UnsupportedServerFeatureError("the `where` parameter requires PostgreSQL 12 or later")
    ...
    async def set_type_codec(self, typename, *, schema="public", encoder, decoder, format="text"):
        ...
        if introspection.is_scalar_type(typeinfo):
            ...
            raise exceptions.InterfaceError(
                f"cannot use custom codec on type {schema}.{typename}: " f"it is neither a scalar type nor a composite type"
            )
        if introspection.is_domain_type(typeinfo):
            raise exceptions.UnsupportedClientFeatureError(
                ...
                detail=(
                    "PostgreSQL does not distinguish domains from " "their base types in query results at the protocol level."
                ),
            )

        oid = typeinfo["oid"]
        settings.add_python_codec(oid, typename, schema, full_typeinfos, kind, encoder, decoder, format)
    ...
    async def set_builtin_type_codec(self, typename, *, schema="public", codec_name, format=None):
        ...
        if not introspection.is_scalar_type(typeinfo):
            raise exceptions.InterfaceError("cannot alias non-scalar type {}.{}".format(schema, typename))

        oid = typeinfo["oid"]

        self._protocol.get_settings().set_builtin_type_codec(oid, typename, schema, "scalar", codec_name, format)
        ...
        if self._protocol.is_in_transaction() or self._top_xact is not None:
            if self._top_xact is None or not self._top_xact._managed:
                ...
                self._loop.call_exception_handler(
                    {"message": "Resetting connection with an " "active transaction {!r}".format(self)}
                )
    ...
    def _check_listeners(self, listeners, listener_type):
        if listeners:
            ...
            w = exceptions.InterfaceWarning(
                ...
                "{type} listener{s}".format(conn=self, c=count, type=listener_type, s="s" if count > 1 else "")
            )
    ...
    def _on_release(self, stacklevel=1):
        ...
        self._check_listeners(list(itertools.chain.from_iterable(self._listeners.values())), "notification")
    ...
    async def _execute(
        self, query, args, limit, timeout, *, return_status=False, ignore_custom_codec=False, record_class=None
    ):
    ...
    async def _executemany(
        ...
        with self._stmt_exclusive_section:
            with self._time_and_log(query, args, timeout):
                result, _ = await self._do_execute(query, executor, timeout, record_class=record_class)
        return result

    async def _do_execute(self, query, executor, timeout, retry=True, *, ignore_custom_codec=False, record_class=None):
        if timeout is None:
            stmt = await self._get_statement(
                query,
                None,
...
async def connect(
    ...
    *,
    ...
    if not issubclass(connection_class, Connection):
        raise exceptions.InterfaceError(
            "connection_class is expected to be a subclass of " "asyncpg.Connection, got {!r}".format(connection_class)
        )
...
class _StatementCache:
    ...
    def _set_entry_timeout(self, entry):
        ...
        if self._max_lifetime:
            entry._cleanup_cb = self._loop.call_later(self._max_lifetime, self._on_entry_expired, entry)
...
class _Callback(typing.NamedTuple):
    ...
    def from_callable(cls, cb: typing.Callable[..., None]) -> "_Callback":
        if inspect.iscoroutinefunction(cb):
            ...
            raise exceptions.InterfaceError("expected a callable or an `async def` function," "got {!r}".format(cb))
...
class _Atomic:
    ...
    def __enter__(self):
        if self._acquired:
            raise exceptions.InterfaceError("cannot perform operation: another operation is in progress")
        self._acquired = 1
...
ServerCapabilities = collections.namedtuple(
    "ServerCapabilities",
    ["advisory_locks", "notifications", "plpgsql", "sql_reset", "sql_close_all", "sql_copy_from_where", "jit"],
)
...
def _check_record_class(record_class):
    if record_class is protocol.Record:
        ...
        if record_class.__new__ is not protocol.Record.__new__ or record_class.__init__ is not protocol.Record.__init__:
            raise exceptions.InterfaceError("record_class must not redefine __new__ or __init__")
    else:
        raise exceptions.InterfaceError(
            "record_class is expected to be a subclass of " "asyncpg.Record, got {!r}".format(record_class)
        )
