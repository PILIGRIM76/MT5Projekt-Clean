            ...
            def wrapper(self, *args, __meth__=meth, **kwargs):
                ...
                if timeout:
                    ...
                    try:
                        ...
                        raise self.failureException("test timed out after {} seconds".format(timeout)) from None
                else:
                    self.loop.run_until_complete(coro)
...
class TestCase(unittest.TestCase, metaclass=TestCaseMeta):
    ...
    def assertLoopErrorHandlerCalled(self, msg_re: str):
        ...
        try:
            ...
            raise AssertionError("no message matching {!r} was logged with " "loop.call_exception_handler()".format(msg_re))
        ...
        for key in sorted(context):
            ...
            if key == "source_traceback":
                ...
                try:
                    ...
                    value = "Exception in __repr__ {!r}; " "value type: {!r}".format(ex, type(value))
            lines.append("[{}]: {}\n\n".format(key, value))
...
class ClusterTestCase(TestCase):
    ...
    def create_pool(self, pool_class=pg_pool.Pool, connection_class=pg_connection.Connection, **kwargs):
        conn_spec = self.get_connection_spec(kwargs)
        pool = create_pool(loop=self.loop, pool_class=pool_class, connection_class=connection_class, **conn_spec)
...
class HotStandbyTestCase(ClusterTestCase):
    ...
    def setup_cluster(cls):
        ...
        cls.start_cluster(cls.master_cluster, server_settings={"max_wal_senders": 10, "wal_level": "hot_standby"})

        con = None

        try:
            con = cls.loop.run_until_complete(cls.master_cluster.connect(database="postgres", user="postgres", loop=cls.loop))

            cls.loop.run_until_complete(con.execute("""
            ...
            cls.standby_cluster = cls.new_cluster(
                pg_cluster.HotStandbyCluster, cluster_kwargs={"master": conn_spec, "replication_user": "replication"}
            )
