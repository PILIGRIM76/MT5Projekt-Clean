...
class PoolConnectionProxyMeta(type):
    ...
    @staticmethod
    def _wrap_connection_method(meth_name: str, iscoroutine: bool) -> Callable[..., Any]:
        def call_con_method(self: Any, *args: Any, **kwargs: Any) -> Any:
            # This method will be owned by PoolConnectionProxy class.
            if self._con is None:
                raise exceptions.InterfaceError(
                    "cannot call Connection.{}(): " "connection has been released back to the pool".format(meth_name)
                )
...
class PoolConnectionProxy(connection._ConnectionProxy, metaclass=PoolConnectionProxyMeta, wrap=True):
    ...
    def __repr__(self) -> str:
        if self._con is None:
            return "<{classname} [released] {id:#x}>".format(classname=self.__class__.__name__, id=id(self))
        else:
            return "<{classname} {con!r} {id:#x}>".format(classname=self.__class__.__name__, con=self._con, id=id(self))
...
class PoolConnectionHolder:
    ...
    async def release(self, timeout: Optional[float]) -> None:
        if self._in_use is None:
            raise exceptions.InternalClientError("PoolConnectionHolder.release() called on " "a free connection holder")
...
class Pool:
    ...
    def __init__(
        ...
        **connect_kwargs,
        ...
        if max_inactive_connection_lifetime < 0:
            raise ValueError("max_inactive_connection_lifetime is expected to be greater " "or equal to zero")
        ...
        if not issubclass(record_class, protocol.Record):
            raise TypeError("record_class is expected to be a subclass of " "asyncpg.Record, got {!r}".format(record_class))
    ...
    async def _get_new_connection(self):
        ...
        if not isinstance(con, self._connection_class):
            ...
            raise exceptions.InterfaceError(
                "expected pool connect callback to return an instance of " f"'{good_n}', got " f"'{bad_n}'"
            )
    ...
    async def copy_records_to_table(self, table_name, *, records, columns=None, schema_name=None, timeout=None, where=None):
        ...
        async with self.acquire() as con:
            return await con.copy_records_to_table(
                table_name, records=records, columns=columns, schema_name=schema_name, timeout=timeout, where=where
            )
