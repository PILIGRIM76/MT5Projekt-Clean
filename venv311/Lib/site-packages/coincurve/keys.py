...
class PrivateKey:
    ...
    def sign_recoverable(self, message: bytes, hasher: Hasher = sha256, custom_nonce: Nonce = DEFAULT_NONCE) -> bytes:
        ...
        signed = lib.secp256k1_ecdsa_sign_recoverable(self.context.ctx, signature, msg_hash, self.secret, nonce_fn, nonce_data)
...
class PublicKey:
    ...
    def from_signature_and_message(
        ...
        """
        return PublicKey(recover(message, deserialize_recoverable(signature, context=context), hasher=hasher, context=context))
...
class PublicKeyXOnly:
    def __init__(
        self, data: bytes | ffi.CData, parity: bool = False, context: Context = GLOBAL_CONTEXT
    ):  # noqa: FBT001, FBT002
        """
