    ...
    def __add__(self, other):
        ...
        if isinstance(other, StateSpace):
            ...
            if self.D.shape == other.shape:
                ...
                raise ValueError("Cannot add systems with incompatible " f"dimensions ({self.D.shape} and {other.shape})")
...
def _YT_real(ker_pole, Q, transfer_matrix, i, j):
    ...
    transfer_matrix_j_mo_transfer_matrix_j = np.vstack((transfer_matrix[:, i, np.newaxis], transfer_matrix[:, j, np.newaxis]))
    ...
    if not np.allclose(sm[0], sm[1]):
        ...
        ker_pole_ij = np.vstack(
            (np.hstack((ker_pole[i], np.zeros(ker_pole[i].shape))), np.hstack((np.zeros(ker_pole[j].shape), ker_pole[j])))
        )
