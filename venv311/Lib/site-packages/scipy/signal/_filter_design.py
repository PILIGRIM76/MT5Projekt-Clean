...
def freqz(b, a=1, worN=512, whole=False, plot=None, fs=2 * pi, include_nyquist=False):
    ...
    if _is_int_type(worN):
        ...
        if (
            a.size == 1 and (b.ndim == 1 or (b.shape[-1] == 1)) and n_fft >= b.shape[0] and n_fft > 0
...
def group_delay(system, w=512, whole=False, fs=2 * pi):
    ...
    if np.any(singular):
        gd[singular] = 0
        warnings.warn(
            "The group delay is singular at frequencies " f"[{', '.join(f'{ws:.3f}' for ws in w[singular])}], setting to 0",
            stacklevel=2,
        )
...
def iirfilter(N, Wn, rp=None, rs=None, btype="band", analog=False, ftype="butter", output="ba", fs=None):
    ...
    if not analog:
        if np.any(Wn <= 0) or np.any(Wn >= 1):
            if fs is not None:
                raise ValueError("Digital filter critical frequencies must " f"be 0 < Wn < fs/2 (fs={fs} -> fs/2={fs/2})")
