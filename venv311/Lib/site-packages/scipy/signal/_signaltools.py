...
def _reject_objects(arr, name):
    ...
    if not (
        np.issubdtype(dt, np.integer) or dt in [np.bool_, np.float16, np.float32, np.float64, np.complex64, np.complex128]
    ):
        msg = (
...
def oaconvolve(in1, in2, mode="full", axes=None):
    ...
    shape_ret = [
        ret.shape[i] if i not in fft_axes else ret.shape[i] * ret.shape[i - 1] for i in range(ret.ndim) if i not in split_axes
    ]
...
def envelope(
    ...
    * The frequency spacing ``1 / (n*T)`` for corner frequencies of the bandpass filter
    ...
    if not (-n // 2 <= bp.start < bp.stop <= (n + 1) // 2):
        raise ValueError("`-n//2 <= bp_in[0] < bp_in[1] <= (n+1)//2` does not hold " + f"for n={z.shape[axis]=} and {bp_in=}!")
...
def _validate_pad(padtype, padlen, x, axis, ntaps):
    ...
    if padtype not in ["even", "odd", "constant", None]:
        raise ValueError(f"Unknown value '{padtype}' given to padtype. " "padtype must be 'even', 'odd', 'constant', or None.")
