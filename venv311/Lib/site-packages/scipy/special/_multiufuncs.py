    ...
    def __call__(self, *args, **kwargs):
        ...
        if self._resolve_out_shapes is not None:
            ...
            ufunc_arg_dtypes = tuple(
                ufunc_arg.dtype if hasattr(ufunc_arg, "dtype") else np.dtype(type(ufunc_arg)) for ufunc_arg in ufunc_args
            )
...
def _(diff_n):
    if (not isinstance(diff_n, numbers.Integral)) or (diff_n < 0):
        raise ValueError(f"diff_n must be a non-negative integer, received: {diff_n}.")
    if not 0 <= diff_n <= 2:
        raise NotImplementedError("diff_n is currently only implemented for orders 0, 1, and 2," f" received: {diff_n}.")
    return diff_n
