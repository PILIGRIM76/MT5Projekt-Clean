...
def comb(N, k, *, exact=False, repetition=False):
    ...
    if exact:
        ...
        msg = "`exact=True` is deprecated for non-integer `N` and `k` and will raise " "an error in SciPy 1.16.0"
...
def perm(N, k, exact=False):
    ...
    if exact:
        ...
        if (k > N) or (N < 0) or (k < 0):
            if non_integral:
                msg = "Non-integer `N` and `k` with `exact=True` is deprecated and " "will raise an error in SciPy 1.16.0."
                warnings.warn(msg, DeprecationWarning, stacklevel=2)
            return 0
        if non_integral:
...
def _factorialx_wrapper(fname, n, k, exact, extend):
    ...
    if fname == "factorial2":
        msg_needs_complex += (
            " Additionally, it will rescale the values of the double" " factorial at even integers by a factor of sqrt(2/pi)."
        )
