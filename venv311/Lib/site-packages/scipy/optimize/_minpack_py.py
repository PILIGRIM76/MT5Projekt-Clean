...
def leastsq(
    ...
    if n > m:
        raise TypeError(f"Improper input: func input vector length N={n} must" f" not exceed func output vector length M={m}")
    ...
    errors = {
        ...
        7: [f"xtol={xtol:f} is too small, no further improvement in " "the approximate\n  solution is possible.", ValueError],
        8: [
            f"gtol={gtol:f} is too small, func(x) is orthogonal to the " "columns of\n  the Jacobian to machine precision.",
            ValueError,
        ],
    }
    ...
    **kwargs
    ...
    if method == "lm":
        ...
        if ydata.size != 1 and n > ydata.size:
            raise TypeError(f"The number of func parameters={n} must not" f" exceed the number of data points={ydata.size}")
