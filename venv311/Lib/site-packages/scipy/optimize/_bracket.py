...
def _bracket_root_iv(func, xl0, xr0, xmin, xmax, factor, args, maxiter):
    ...
    if not xp.isdtype(maxiter.dtype, "numeric") or maxiter.shape != tuple() or xp.isdtype(maxiter.dtype, "complex floating"):
...
def _bracket_minimum_iv(func, xm0, xl0, xr0, xmin, xmax, factor, args, maxiter):
    ...
    xl0, xm0, xr0, xmin, xmax, factor = xp.broadcast_arrays(
        xp.asarray(xl0), xm0, xp.asarray(xr0), xp.asarray(xmin), xp.asarray(xmax), xp.asarray(factor)
    )
