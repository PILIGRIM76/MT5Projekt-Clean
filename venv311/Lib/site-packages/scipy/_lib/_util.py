...
if np.lib.NumpyVersion(np.__version__) >= "1.25.0":
    from numpy.exceptions import AxisError, ComplexWarning, VisibleDeprecationWarning, DTypePromotionError
else:
...
def _dict_formatter(d, n=0, mplus=1, sorter=None):
    ...
    if isinstance(d, dict):
        ...
        s = "\n".join(
            [
                k.rjust(m) + ": " + _indenter(_dict_formatter(v, m + n + 2, 0, sorter), m + 2)  # right justified, width m
                for k, v in sorter(d)
            ]
        )  # +2 for ': '
    else:
