...
class Quadratic:
    ...
    def __init__(self, interpolation, values, debug):
        ...
        if interpolation.npt < interpolation.n + 1:
            raise ValueError(f"The number of interpolation points must be at least " f"{interpolation.n + 1}.")
    ...
    def __call__(self, x, interpolation):
        ...
        return (
            self._const
            + self._grad @ x_diff
            + 0.5 * (self._i_hess @ (interpolation.xpt.T @ x_diff) ** 2.0 + x_diff @ self._e_hess @ x_diff)
        )
    ...
    def solve_systems(interpolation, rhs):
        ...
        left_scaled_solutions = eig_vectors @ ((eig_vectors.T @ rhs_scaled) * inv_eig_values[:, np.newaxis])
        return (
...
class Models:
    ...
    def _check_interpolation_conditions(self):
        ...
        if error_cub > tol * np.max(np.abs(self.cub_val), initial=1.0):
            warnings.warn(
                "The interpolation conditions for the inequality constraint " "function are not satisfied.",
                RuntimeWarning,
                2,
            )
        if error_ceq > tol * np.max(np.abs(self.ceq_val), initial=1.0):
            warnings.warn(
                "The interpolation conditions for the equality constraint " "function are not satisfied.",
                RuntimeWarning,
                2,
            )
