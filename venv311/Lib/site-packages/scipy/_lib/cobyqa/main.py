        ...
        if s_norm <= constants[Constants.SHORT_STEP_THRESHOLD] * framework.resolution:
            ...
            if same_best_point:
                ...
                if (
                    ...
                    and np.linalg.norm(normal_step) > constants[Constants.BYRD_OMOJOKUN_FACTOR] ** 2.0 * framework.radius
                ):
                ...
                enhance_resolution = (
                    radius_save <= framework.resolution and ratio <= constants[Constants.LOW_RATIO] and not improve_geometry
                )
            else:
...
def _set_default_options(options, n):
    ...
    if Options.RHOBEG in options and Options.RHOEND in options:
        if options[Options.RHOBEG] < options[Options.RHOEND]:
            raise ValueError(
                "The initial trust-region radius must be greater " "than or equal to the final trust-region radius."
            )
    ...
    if Options.NPT in options and options[Options.NPT] > ((n + 1) * (n + 2)) // 2:
        raise ValueError(f"The number of interpolation points must be at most " f"{((n + 1) * (n + 2)) // 2}.")
...
def _set_default_constants(**kwargs):
    ...
    if Constants.PENALTY_INCREASE_THRESHOLD in constants and Constants.PENALTY_INCREASE_FACTOR in constants:
        if constants[Constants.PENALTY_INCREASE_FACTOR] < constants[Constants.PENALTY_INCREASE_THRESHOLD]:
            raise ValueError(
                "The constant penalty_increase_factor must be " "greater than or equal to " "penalty_increase_threshold."
            )
