...
def warn_extraneous(extraneous):
    ...
    if extraneous:
        warn(
            "The following arguments have no effect for a chosen solver: " f"{', '.join(f'`{x}`' for x in extraneous)}.",
            stacklevel=3,
        )
...
def validate_tol(rtol, atol, n):
    ...
    if np.any(rtol < 100 * EPS):
        warn("At least one element of `rtol` is too small. " f"Setting `rtol = np.maximum(rtol, {100 * EPS})`.", stacklevel=3)
