...
def _get_pairs(k, h0, inclusive, dtype, xp, work):
    ...
    if len(work.pair_cache.indices) <= k + 2 or not isinstance(h0, type(work.pair_cache.h0)) or h0 != work.pair_cache.h0:
...
def _tanhsinh_iv(f, a, b, log, maxfun, maxlevel, minlevel, atol, rtol, args, preserve_shape, callback):
    ...
    if not (
        np.issubdtype(params.dtype, np.number) and np.all(np.isreal(params)) and np.all(params.astype(np.int64) == params)
    ):
        raise ValueError(message)
