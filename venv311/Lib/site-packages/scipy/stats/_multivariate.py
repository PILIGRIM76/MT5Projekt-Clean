...
class multivariate_normal_gen(multi_rv_generic):
    ...
    def logcdf(self, x, mean=None, cov=1, allow_singular=False, maxpts=None, abseps=1e-5, releps=1e-5, *, lower_limit=None):
    ...
    def cdf(self, x, mean=None, cov=1, allow_singular=False, maxpts=None, abseps=1e-5, releps=1e-5, *, lower_limit=None):
        """Multivariate normal cumulative distribution function.
...
def _dirichlet_check_input(alpha, x):
    ...
    if (np.abs(np.sum(x, 0) - 1.0) > 10e-10).any():
        raise ValueError("The input vector 'x' must lie within the normal " f"simplex. but np.sum(x, 0) = {np.sum(x, 0)}.")
    ...
    def _process_quantiles(self, x, dim):
        ...
        if x.ndim == 1:
            ...
            if not x.shape[0] == x.shape[1]:
                raise ValueError("Quantiles must be square if they are two dimensional," f" but x.shape = {str(x.shape)}.")
            x = x[:, :, np.newaxis]
        elif x.ndim == 3:
            if not x.shape[0] == x.shape[1]:
                raise ValueError(
...
class multinomial_gen(multi_rv_generic):
    ...
    def _process_quantiles(self, x, n, p):
        ...
        if xx.size != 0 and not xx.shape[-1] == p.shape[-1]:
            raise ValueError(
                "Size of each quantile should be size of p: " "received %d, but expected %d." % (xx.shape[-1], p.shape[-1])
            )
...
class multivariate_hypergeom_gen(multi_rv_generic):
    ...
    def _process_quantiles(self, x, M, m, n):
        ...
        if not x.shape[-1] == m.shape[-1]:
            raise ValueError(
                f"Size of each quantile must be size of 'm': " f"received {x.shape[-1]}, " f"but expected {m.shape[-1]}."
            )
    ...
    **Fitting the distribution parameters**
    ...
    def _rejection_sampling(self, dim, kappa, size, random_state):
        ...
        while n_accepted < n_samples:
            ...
            criterion = (
                kappa * coord_x
                + dim_minus_one * (np.log((1 + envelop_param - coord_x + coord_x * envelop_param) / (1 + envelop_param)))
                - correction
            ) > np.log(accept_tol)
