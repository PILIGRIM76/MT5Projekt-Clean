...
def fit(dist, data, bounds=None, *, guess=None, method="mle", optimizer=optimize.differential_evolution):
    ...
    if isinstance(user_bounds, dict):
        ...
        if user_bounds:
            message = "Bounds provided for the following unrecognized " f"parameters will be ignored: {set(user_bounds)}"
        ...
        if user_bounds.ndim != 2 or user_bounds.shape[1] != 2:
            message = "Each element of `bounds` must be a tuple specifying " "the lower and upper bounds of a shape parameter"
            raise ValueError(message)
    ...
    if guess_array is not None:
        ...
        for i in rounded:
            message = f"Guess for parameter `{param_info[i].name}` " f"rounded from {guess_array[i]} to {guess_rounded[i]}."
            warnings.warn(message, RuntimeWarning, stacklevel=2)
        ...
        for i in clipped:
            message = (
                f"Guess for parameter `{param_info[i].name}` " f"clipped from {guess_rounded[i]} to " f"{guess_clipped[i]}."
            )
            warnings.warn(message, RuntimeWarning, stacklevel=2)
