...
class ContinuousDistribution(_ProbabilityDistribution):
    ...
    def _identify_parameterization(self, parameters):
        ...
        for parameterization in self._parameterizations:
            ...
            if not parameter_names_set:
                message = (
                    f"The `{self.__class__.__name__}` distribution " "family requires parameters, but none were " "provided."
                )
            else:
    ...
    def cache_policy(self, cache_policy):
        ...
        if cache_policy not in cache_policies:
            message = (
                f"Attribute `cache_policy` of `{self.__class__.__name__}` " f"must be one of {cache_policies}, if specified."
            )
            raise ValueError(message)
    ...
    def validation_policy(self, validation_policy):
        ...
        if validation_policy not in iv_policies:
            message = (
                f"Attribute `validation_policy` of `{self.__class__.__name__}` " f"must be one of {iv_policies}, if specified."
            )
            raise ValueError(message)
    ...
    def _mode_optimization(self, **params):
        ...
        f, args = _kwargs2args(lambda x, **params: -self._pdf_dispatch(x, **params), args=(), kwargs=params)
    ...
    def sample(self, shape=(), *, method=None, rng=None):
        ...
        res = self._sample_dispatch(sample_shape, full_shape, method=method, rng=rng, **self._parameters)
...
def make_distribution(dist):
    ...
    def _overrides(method_name):
        return getattr(dist.__class__, method_name, None) is not getattr(stats.rv_continuous, method_name, None)
    ...
    docs = [
        f"This class represents `scipy.stats.{dist.name}` as a subclass of " "`ContinuousDistribution`.",
...
class TruncatedDistribution(TransformedDistribution):
    ...
    _parameterizations = [_Parameterization(_lb_param, _ub_param), _Parameterization(_lb_param), _Parameterization(_ub_param)]
...
class ShiftedScaledDistribution(TransformedDistribution):
    ...
    def _sample_dispatch(self, sample_shape, full_shape, *, rng, loc, scale, sign, method, **params):
        rvs = self._dist._sample_dispatch(sample_shape, full_shape, method=method, rng=rng, **params)
...
class MonotonicTransformedDistribution(TransformedDistribution):
    ...
    def __init__(self, X, /, *args, g, h, dh, logdh=None, increasing=True, repr_pattern=None, str_pattern=None, **kwargs):
