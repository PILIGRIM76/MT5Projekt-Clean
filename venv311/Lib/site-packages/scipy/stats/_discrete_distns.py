...
class skellam_gen(rv_discrete):
    ...
    def _pmf(self, x, mu1, mu2):
        with np.errstate(over="ignore"):  # see gh-17432
            px = np.where(
                x < 0, scu._ncx2_pdf(2 * mu2, 2 * (1 - x), 2 * mu1) * 2, scu._ncx2_pdf(2 * mu1, 2 * (1 + x), 2 * mu2) * 2
            )
    ...
    def _cdf(self, x, mu1, mu2):
        x = floor(x)
        with np.errstate(over="ignore"):  # see gh-17432
            px = np.where(x < 0, scu._ncx2_cdf(2 * mu2, -2 * x, 2 * mu1), 1 - scu._ncx2_cdf(2 * mu1, 2 * (x + 1), 2 * mu2))
        return px
...
class yulesimon_gen(rv_discrete):
    ...
    def _stats(self, alpha):
        ...
        g2 = np.where(alpha > 4, alpha + 3 + ((11 * alpha**3 - 49 * alpha - 22) / (alpha * (alpha - 4) * (alpha - 3))), np.inf)
