...
def sobol_indices(*, func, n, dists=None, method="saltelli_2010", rng=None):
    ...
    if not callable(method):
        ...
        try:
            ...
            message = f"{method!r} is not a valid 'method'. It must be one of" f" {set(indices_methods)!r} or a callable."
            raise ValueError(message) from exc
    else:
        ...
        if set(sig.parameters) != {"f_A", "f_B", "f_AB"}:
            message = "If 'method' is a callable, it must have the following" f" signature: {inspect.signature(saltelli_2010)}"
