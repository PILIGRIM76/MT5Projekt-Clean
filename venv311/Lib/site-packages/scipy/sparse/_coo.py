...
class _coo_base(_data_matrix, _minmax_mixin):
    ...
    def _check(self):
        ...
        if self.ndim != len(self.coords):
            raise ValueError("mismatching number of index arrays for shape; " f"got {len(self.coords)}, expected {self.ndim}")
    ...
    def dot(self, other):
        ...
        if isdense(other):
            ...
            if self.shape[0] != other.shape[0]:
                raise ValueError(f"shapes {self.shape} and {other.shape}" " are not aligned for inner product")
            return self @ other
        elif self.ndim == 2 and other.ndim == 2:
            # Handle matrix multiplication (2-D arrays)
            if self.shape[1] != other.shape[0]:
                raise ValueError(f"shapes {self.shape} and {other.shape}" " are not aligned for matmul")
            return self @ other
        else:
    ...
    def _sparse_tensordot(self, other, axes_self, axes_other):
        ...
        self_reduced_coords = np.ravel_multi_index([self.coords[ax] for ax in axes_self], [self.shape[ax] for ax in axes_self])
        ...
        # Get the shape of the non-reduced axes
        self_nonreduced_shape = tuple(self.shape[ax] for ax in range(ndim_self) if ax not in axes_self)
        other_nonreduced_shape = tuple(other.shape[ax] for ax in range(ndim_other) if ax not in axes_other)

        # Create 2D coords arrays
        ravel_coords_shape_self = (math.prod(self_nonreduced_shape), math.prod([self.shape[ax] for ax in axes_self]))
        ravel_coords_shape_other = (math.prod([other.shape[ax] for ax in axes_other]), math.prod(other_nonreduced_shape))
    ...
    def _dense_tensordot(self, other, axes_self, axes_other):
        ...
        reshape_other = (*non_reduced_shape_other[:-1], math.prod(reduced_shape_other), *non_reduced_shape_other[-1:])
