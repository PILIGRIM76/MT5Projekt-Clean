...
from scipy.sparse._sputils import is_pydata_spmatrix, convert_pydata_sparse_to_scipy, get_index_dtype, safely_cast_index_arrays
...
def spilu(
    ...
    return _superlu.gstrf(N, A.nnz, A.data, indices, indptr, csc_construct_func=csc_construct_func, ilu=True, options=_options)
...
def is_sptriangular(A):
    ...
    if not (issparse(A) and A.format in ("csc", "csr", "coo", "dia", "dok", "lil")):
        warn("is_sptriangular needs sparse and not BSR format. Converting to CSR.", SparseEfficiencyWarning, stacklevel=2)
        A = csr_array(A)
