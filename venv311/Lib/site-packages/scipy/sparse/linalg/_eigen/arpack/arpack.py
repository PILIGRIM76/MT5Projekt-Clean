                ...
                while i <= k:
                    if abs(d[i].imag) == 0:
                        ...
                        if i < k:
                            ...
                            z[:, i + 1] = z[:, i].conjugate()
                            d[i] = (
                                np.dot(zr[:, i], self.matvec(zr[:, i])) + np.dot(zr[:, i + 1], self.matvec(zr[:, i + 1]))
                            ) + 1j * (
                                np.dot(zr[:, i], self.matvec(zr[:, i + 1])) - np.dot(zr[:, i + 1], self.matvec(zr[:, i]))
                            )
                            d[i + 1] = d[i].conj()
                            i += 1
                        else:
...
class IterOpInv(LinearOperator):
    ...
    def _matvec(self, x):
        ...
        if info != 0:
            raise ValueError(
                "Error in inverting [A-sigma*M]: function " "%s did not converge (info = %i)." % (self.ifunc.__name__, info)
            )
        return b
...
def eigs(
    ...
    if M is not None:
        ...
        if np.dtype(M.dtype).char.lower() != np.dtype(A.dtype).char.lower():
            warnings.warn(
                "M does not have the same type precision as A. " "This may adversely affect ARPACK convergence", stacklevel=2
            )
    ...
    if k >= n - 1:
        warnings.warn(
            "k >= N - 1 for N * N square matrix. " "Attempting to use scipy.linalg.eig instead.", RuntimeWarning, stacklevel=2
        )
...
def eigsh(
    ...
    if k >= n:
        warnings.warn(
            "k >= N for N * N square matrix. " "Attempting to use scipy.linalg.eigh instead.", RuntimeWarning, stacklevel=2
        )
