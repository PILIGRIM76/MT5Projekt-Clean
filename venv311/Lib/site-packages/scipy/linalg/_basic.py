...
def det(a, overwrite_a=False, check_finite=True):
    ...
    if a1.shape[-1] != a1.shape[-2]:
        raise ValueError("Last 2 dimensions of the array must be square" f" but received shape {a1.shape}.")

    # Also check if dtype is LAPACK compatible
    if a1.dtype.char not in "fdFD":
        dtype_char = lapack_cast_dict[a1.dtype.char]
        if not dtype_char:  # No casting possible
            raise TypeError(f'The dtype "{a1.dtype.name}" cannot be cast ' "to float(32, 64) or complex(64, 128).")
...
def lstsq(a, b, cond=None, overwrite_a=False, overwrite_b=False, check_finite=True, lapack_driver=None):
    ...
    if len(b1.shape) == 2:
        ...
        nrhs = 1
    if m != b1.shape[0]:
        raise ValueError("Shape mismatch: a and b should have the same number" f" of rows ({m} != {b1.shape[0]}).")
    ...
    if driver in ("gelss", "gelsd"):
        if driver == "gelss":
            ...
            v, x, s, rank, work, info = lapack_func(a1, b1, cond, lwork, overwrite_a=overwrite_a, overwrite_b=overwrite_b)
