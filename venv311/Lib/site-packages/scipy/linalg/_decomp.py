    ...
    if select != 0:  # (non-all)
        ...
        if select == 1:  # (value)
            ...
            if sr.dtype.char.lower() not in "hilqp":
                raise ValueError(
                    f'when using select="i", select_range must ' f"contain integers, got dtype {sr.dtype} ({sr.dtype.char})"
                )
...
def eig_banded(
    a_band, lower=False, eigvals_only=False, overwrite_a_band=False, select="a", select_range=None, max_ev=0, check_finite=True
):
    """
...
def eigvals(a, b=None, overwrite_a=False, check_finite=True, homogeneous_eigvals=False):
    ...
    """
    return eig(
        a, b=b, left=0, right=0, overwrite_a=overwrite_a, check_finite=check_finite, homogeneous_eigvals=homogeneous_eigvals
    )
