    ...
    * `decode` definition must be ``def decode(value: str) -> bytes:``.
    ...
    def __init__(self, string: str = None):
        ...
        if string:
            ...
            self._dc_id, ip, self._port, key = struct.unpack(_STRUCT_PREFORMAT.format(ip_len), StringSession.decode(string))
    ...
    def save(self: Session):
        ...
        return CURRENT_VERSION + StringSession.encode(
            struct.pack(_STRUCT_PREFORMAT.format(len(ip)), self.dc_id, ip, self.port, self.auth_key.key)
        )
