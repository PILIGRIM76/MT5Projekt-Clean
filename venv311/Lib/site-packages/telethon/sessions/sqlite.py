...
from ..tl.types import InputDocument, InputPhoto, PeerChannel, PeerChat, PeerUser
...
class SQLiteSession(MemorySession):
    ...
    def __init__(self, session_id=None, store_tmp_auth_key_on_disk: bool = False):
        ...
        if c.fetchone():
            ...
            tuple_ = c.fetchone()
            if tuple_:
                self._dc_id, self._server_address, self._port, key, tmp_key, self._takeout_id = tuple_
    ...
    def get_update_state(self, entity_id):
        row = self._execute("select pts, qts, date, seq from update_state " "where id = ?", entity_id)
        if row:
    ...
    def list_sessions(cls):
        ...
        """
        return [os.path.splitext(os.path.basename(f))[0] for f in os.listdir(".") if f.endswith(EXTENSION)]
    ...
    def get_entity_rows_by_username(self, username):
        c = self._cursor()
        try:
            results = c.execute("select id, hash, date from entities where username = ?", (username,)).fetchall()
            ...
            if len(results) > 1:
                ...
                c.executemany("update entities set username = null where id = ?", [(t[0],) for t in results[:-1]])
    ...
    def get_file(self, md5_digest, file_size, cls):
        row = self._execute(
            "select id, hash from sent_files " "where md5_digest = ? and file_size = ? and type = ?",
            md5_digest,
            file_size,
