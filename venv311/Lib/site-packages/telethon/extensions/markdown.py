...
from ..helpers import add_surrogate, del_surrogate, strip_text, within_surrogate
    ...
    while i < len(message):
        ...
        if m:
            ...
            if end != -1:
                ...
                message = "".join((message[:i], message[i + len(delim) : end], message[end + len(delim) :]))
            ...
            if m:
                ...
                result.append(MessageEntityTextUrl(offset=m.start(), length=len(m.group(1)), url=del_surrogate(m.group(2))))
                i += len(m.group(1))
                continue
