    ...
    async def get(self):
        ...
        while self._deque and len(batch) <= MessageContainer.MAXIMUM_LENGTH:
            ...
            if size <= MessageContainer.MAXIMUM_SIZE:
                ...
                batch.append(state)
                self._log.debug(
                    "Assigned msg_id = %d to %s (%x)", state.msg_id, state.request.__class__.__name__, id(state.request)
                )
                continue
            ...
            self._log.warning(
                "Message payload for %s is too long (%d) and cannot be sent", state.request.__class__.__name__, len(state.data)
            )
        ...
        if len(batch) > 1:
            ...
            data = struct.pack("<Ii", MessageContainer.CONSTRUCTOR_ID, len(batch)) + buffer.getvalue()
            buffer = io.BytesIO()
