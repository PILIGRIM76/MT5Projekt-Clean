...
def get_input_document(document):
    ...
    if isinstance(document, types.Document):
        return types.InputDocument(id=document.id, access_hash=document.access_hash, file_reference=document.file_reference)
    ...
    *,
    ...
    if isinstance(media, types.MessageMediaDocument):
        return types.InputMediaDocument(
            id=get_input_document(media.document), spoiler=media.spoiler, ttl_seconds=ttl or media.ttl_seconds
        )
    ...
    if isinstance(media, (types.InputFile, types.InputFileBig)):
        if is_photo:
            ...
            return types.InputMediaUploadedDocument(
                file=media, mime_type=mime, attributes=attrs, force_file=force_document, ttl_seconds=ttl
            )

    if isinstance(media, types.MessageMediaGame):
        return types.InputMediaGame(id=types.InputGameID(id=media.game.id, access_hash=media.game.access_hash))

    if isinstance(media, types.MessageMediaContact):
        return types.InputMediaContact(
            phone_number=media.phone_number, first_name=media.first_name, last_name=media.last_name, vcard=""
        )
...
def _get_metadata(file):
    ...
    try:
        ...
        parser = hachoir.parser.guess.guessParser(
            hachoir.stream.InputIOStream(stream, source="file:" + filename, tags=[], filename=filename)
        )
...
def sanitize_parse_mode(mode):
    ...
    if all(hasattr(mode, x) for x in ("parse", "unparse")) and all(callable(x) for x in (mode.parse, mode.unparse)):
        return mode
    elif callable(mode):
...
def get_peer(peer):
    try:
        if isinstance(peer, int):
            ...
            peer, (types.contacts.ResolvedPeer, types.InputNotifyPeer, types.TopPeer, types.Dialog, types.DialogPeer)
        ):
            return peer.peer
...
def resolve_bot_file_id(file_id):
    ...
    if (version == 2 and len(data) == 24) or (version == 4 and len(data) == 25):
        ...
        if file_type == 3 or file_type == 9:
            ...
            attributes.append(types.DocumentAttributeVideo(duration=0, w=0, h=0, round_message=file_type == 13))
        # elif file_type == 5:  # other, cannot know which
        elif file_type == 8:
            attributes.append(types.DocumentAttributeSticker(alt="", stickerset=types.InputStickerSetEmpty()))
        elif file_type == 10:
        ...
        # else version == 4:
            ...
            file_type, dc_id, media_id, access_hash, volume_id, secret, local_id, _ = struct.unpack("<iiqqqqi5s", data)
        elif len(data) == 77:
            # See #1613.
...
def pack_bot_file_id(file):
    ...
    if isinstance(file, types.Document):
        ...
        return _encode_telegram_base64(_rle_encode(struct.pack("<iiqqb", file_type, file.dc_id, file.id, file.access_hash, 2)))
