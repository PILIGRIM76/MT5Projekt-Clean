...
class MessageParseMethods:
        ...
        for i in reversed(range(len(msg_entities))):
            ...
            if not e.length:
                ...
                if m:
                    ...
                    if not is_mention:
                        del msg_entities[i]
            elif isinstance(e, (types.MessageEntityMentionName, types.InputMessageEntityMentionName)):
    ...
    def _get_response_message(self: "TelegramClient", request, result, input_chat):
        ...
        if isinstance(result, types.UpdateShort):
            ...
            entities = {utils.get_peer_id(x): x for x in itertools.chain(result.users, result.chats)}
        else:
            return None
        ...
        for update in updates:
            ...
            elif isinstance(update, types.UpdateEditChannelMessage) and utils.get_peer_id(request.peer) == utils.get_peer_id(
        ...
        if random_id is None:
            ...
            self._log[__name__].warning("No random_id in %s to map to, returning None message for %s", request, result)
            return None
        ...
        if not utils.is_list_like(random_id):
            ...
            if not msg:
                self._log[__name__].warning("Request %s had missing message mapping %s", request, result)
        ...
        try:
            ...
            self._log[__name__].warning("Request %s had missing message mappings %s", request, result)

        return [id_to_message.get(random_to_id[rnd]) if rnd in random_to_id else None for rnd in random_id]
