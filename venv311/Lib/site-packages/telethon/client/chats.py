...
from ..tl import custom, functions, types
    ...
    async def _update(self):
        try:
            ...
            if self._auto_cancel:
                await self._client(functions.messages.SetTypingRequest(self._chat, types.SendMessageCancelAction()))
    ...
    async def _init(self, entity, filter, search):
        ...
        if search and (filter or ty != helpers._EntityType.CHANNEL):
            ...
            self.filter_entity = lambda ent: (
                search in utils.get_display_name(ent).casefold() or search in (getattr(ent, "username", None) or "").casefold()
            )
        else:
        ...
        if ty == helpers._EntityType.CHANNEL:
            if self.limit <= 0:
                ...
                self.total = (await self.client(functions.channels.GetFullChannelRequest(entity))).full_chat.participants_count
                ...
                self.total = (
                    await self.client(
                        functions.channels.GetParticipantsRequest(
                            channel=self.requests.channel, filter=types.ChannelParticipantsRecent(), offset=0, limit=1, hash=0
                        )
                    )
                ).count
...
class _ProfilePhotoIter(RequestIter):
    async def _init(self, entity, offset, max_id):
        ...
        if ty == helpers._EntityType.USER:
            self.request = functions.photos.GetUserPhotosRequest(entity, offset=offset, max_id=max_id, limit=1)
        else:
    ...
    async def _load_next_chunk(self):
        ...
        if isinstance(result, types.photos.Photos):
            ...
            self.buffer = [x.action.photo for x in result.messages if isinstance(x.action, types.MessageActionChatEditPhoto)]
            ...
            self.buffer.extend(
                ...
                if isinstance(x.action, types.MessageActionChatEditPhoto) and x.action.photo.id != seen_id
            )
...
class ChatMethods:
    ...
    def iter_profile_photos(
        self: "TelegramClient", entity: "hints.EntityLike", limit: int = None, *, offset: int = 0, max_id: int = 0
    ) -> _ProfilePhotoIter:
        """
    ...
    def action(
        ...
        *,
        ...
        if isinstance(action, types.SendMessageCancelAction):
            ...
            return self(functions.messages.SetTypingRequest(entity, types.SendMessageCancelAction()))
    ...
    async def edit_admin(
        ...
        *,
        ...
        if ty == helpers._EntityType.CHANNEL:
            ...
            return await self(functions.messages.EditChatAdminRequest(entity.chat_id, user, is_admin=is_admin))
    ...
    async def edit_permissions(
        ...
        *,
        ...
        if user is None:
            return await self(functions.messages.EditChatDefaultBannedRightsRequest(peer=entity, banned_rights=rights))

        user = await self.get_input_entity(user)

        return await self(functions.channels.EditBannedRequest(channel=entity, participant=user, banned_rights=rights))

    async def kick_participant(self: "TelegramClient", entity: "hints.EntityLike", user: "typing.Optional[hints.EntityLike]"):
        """
        ...
        if ty == helpers._EntityType.CHAT:
            ...
            if isinstance(user, types.InputPeerSelf):
                ...
                await self(
                    functions.channels.EditBannedRequest(
                        channel=entity, participant=user, banned_rights=types.ChatBannedRights(until_date=None)
                    )
                )
        else:
