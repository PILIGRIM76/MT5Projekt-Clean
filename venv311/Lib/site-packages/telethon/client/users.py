...
from ..tl import TLRequest, functions, types
...
class UserMethods:
    ...
    async def _call(self: "TelegramClient", sender, request, ordered=False, flood_sleep_threshold=None):
        ...
        for attempt in retry_range(self._request_retries):
            try:
                ...
                last_error = e
                self._log[__name__].warning("Telegram is having internal issues %s: %s", e.__class__.__name__, e)
                ...
                if e.seconds <= self.flood_sleep_threshold:
                    ...
                    raise
            except (errors.PhoneMigrateError, errors.NetworkMigrateError, errors.UserMigrateError) as e:
                last_error = e
    ...
    async def get_entity(
        ...
        if chats:  # TODO Handle chats slice?
            chats = (await self(functions.messages.GetChatsRequest([x.chat_id for x in chats]))).chats
        if channels:
        ...
        for x in inputs:
            if isinstance(x, str):
                ...
                result.append(next(u for u in id_entity.values() if isinstance(u, types.User) and u.is_self))
    ...
    async def get_input_entity(self: "TelegramClient", peer: "hints.EntityLike") -> "types.TypeInputPeer":
        ...
        if isinstance(peer, types.PeerUser):
            users = await self(functions.users.GetUsersRequest([types.InputUser(peer.user_id, access_hash=0)]))
            ...
            try:
                channels = await self(
                    functions.channels.GetChannelsRequest([types.InputChannel(peer.channel_id, access_hash=0)])
                )
    ...
    async def get_peer_id(self: "TelegramClient", peer: "hints.EntityLike", add_mark: bool = True) -> int:
        """
    ...
    async def _get_entity_from_string(self: "TelegramClient", string):
        ...
        if phone:
            try:
                ...
                raise ValueError("Cannot get entity by phone number as a " "bot (try using integer IDs, not strings)")
