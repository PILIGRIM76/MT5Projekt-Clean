...
class AuthMethods:
    ...
    async def _start(
        self: "TelegramClient", phone, password, bot_token, force_sms, code_callback, first_name, last_name, max_attempts
    ):
    ...
    async def sign_in(
        ...
        *,
        ...
        if phone and not code and not password:
            ...
            raise ValueError(
                "You must provide a phone and a code the first time, " "and a password only if an RPCError was raised before."
            )
        ...
        if force_sms:
            try:
                ...
                if _retry_count > 2:
                    raise
                self._phone_code_hash.pop(phone, None)
                self._log[__name__].info("Phone code expired in ResendCodeRequest, requesting a new code")
                return await self.send_code_request(phone, force_sms=False, _retry_count=_retry_count + 1)
