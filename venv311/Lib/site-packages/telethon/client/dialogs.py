...
class _DialogsIter(RequestIter):
    async def _init(self, offset_date, offset_id, offset_peer, ignore_pinned, ignore_migrated, folder):
    ...
    async def _load_next_chunk(self):
        ...
        for d in r.dialogs:
            ...
            if peer_id not in self.seen:
                ...
                if cd.dialog.pts:
                    self.client._message_box.try_set_channel_state(utils.get_peer_id(d.peer, add_mark=False), cd.dialog.pts)
        ...
        last_message = next(
            filter(None, (messages.get(_dialog_message_key(d.peer, d.top_message)) for d in reversed(r.dialogs))), None
        )
...
class _DraftsIter(RequestIter):
    async def _init(self, entities, **kwargs):
        ...
        self.buffer.extend(custom.Draft(self.client, entities[utils.get_peer_id(d.peer)], d.draft) for d in items)
...
class DialogMethods:
    ...
    async def get_drafts(self: "TelegramClient", entity: "hints.EntitiesLike" = None) -> "hints.TotalList":
        """
    ...
    async def edit_folder(
        ...
        *,
        ...
        return await self(
            functions.folders.EditPeerFoldersRequest([types.InputFolderPeer(x, folder_id=y) for x, y in zip(entities, folder)])
        )

    async def delete_dialog(self: "TelegramClient", entity: "hints.EntityLike", *, revoke: bool = False):
        """
        ...
        if ty == helpers._EntityType.CHAT and not deactivated:
            try:
                result = await self(
                    functions.messages.DeleteChatUserRequest(entity.chat_id, types.InputUserSelf(), revoke_history=revoke)
                )
