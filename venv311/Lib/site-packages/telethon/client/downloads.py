...
class _DirectDownloadIter(RequestIter):
    async def _init(self, file, dc_id, offset, stride, chunk_size, request_size, file_size, msg_data, cdn_redirect=None):
            ...
            try:
                ...
                for option in config.dc_options:
                    if option.ip_address == self.client.session.server_address:
                        await utils.maybe_async(self.client.session.set_dc(option.id, option.ip_address, option.port))
...
class DownloadMethods:
    ...
    async def download_profile_photo(
        self: "TelegramClient", entity: "hints.EntityLike", file: "hints.FileLike" = None, *, download_big: bool = True
    ) -> typing.Optional[str]:
        """
        ...
        if entity.SUBCLASS_OF_ID not in ENTITIES:
            ...
            if not hasattr(entity, "photo"):
                ...
                return await self._download_photo(entity.chat_photo, file, date=None, thumb=thumb, progress_callback=None)
        ...
        file = self._get_proper_filename(file, "profile_photo", ".jpg", possible_names=possible_names)
    ...
    async def download_media(
        ...
        *,
        ...
        if isinstance(media, (types.MessageMediaPhoto, types.Photo)):
            ...
            return await self._download_document(media, file, date, thumb, progress_callback, msg_data)
        ...
        *,
        ...
        if (
            ...
            self._log[__name__].info(
                "Starting direct file download in chunks of " "%d at %d, stride %d", request_size, offset, stride
            )
        else:
            cls = _GenericDownloadIter
            self._log[__name__].info(
                "Starting indirect file download in chunks of " "%d at %d, stride %d", request_size, offset, stride
            )
    ...
    def _get_thumb(thumbs, thumb):
        ...
        elif isinstance(thumb, (types.PhotoSize, types.PhotoCachedSize, types.PhotoStrippedSize, types.VideoSize)):
            return thumb
        else:
            return None
    ...
    async def _download_photo(self: "TelegramClient", photo, file, date, thumb, progress_callback):
        ...
        result = await self.download_file(
            types.InputPhotoFileLocation(
                id=photo.id, access_hash=photo.access_hash, file_reference=photo.file_reference, thumb_size=size.type
            ),
            file,
            file_size=file_size,
            progress_callback=progress_callback,
    ...
    def _download_contact(cls, mm_contact, file):
        ...
        file = cls._get_proper_filename(file, "contact", ".vcard", possible_names=[first_name, phone_number, last_name])
        if file is bytes:
            return result
    ...
    async def _download_web_document(cls, web, file, progress_callback):
        ...
        if not aiohttp:
            raise ValueError(
                "Cannot download web documents without the aiohttp " "dependency install it (pip install aiohttp)"
            )

        # TODO Better way to get opened handles of files and auto-close
        kind, possible_names = cls._get_kind_and_names(web.attributes)
        file = cls._get_proper_filename(file, kind, utils.get_extension(web), possible_names=possible_names)
        if file is bytes:
            f = io.BytesIO()
