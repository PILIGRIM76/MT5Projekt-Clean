...
from .. import errors, events, utils
...
class UpdateMethods:
    ...
    def remove_event_handler(self: "TelegramClient", callback: Callback, event: EventBuilder = None) -> int:
        """
        ...
        try:
            ...
            while self.is_connected():
                ...
                if get_diff:
                    ...
                    try:
                    ...
                    except (errors.ServerError, errors.TimedOutError, errors.FloodWaitError, ValueError) as e:
                        ...
                        self._log[__name__].info(
                            "Cannot get difference since the account is not logged in: %s", type(e).__name__
                        )
                        self._message_box.end_difference()
                        ...
                        self._log[__name__].warning(
                            "Cannot get difference since the account is likely misusing the session: %s", e
                        )
                        self._message_box.end_difference()
                        self._updates_error = e
                        await self.disconnect()
                        ...
                        self._log[__name__].info(
                            "Cannot get difference since the network is down: %s: %s", type(e).__name__, e
                        )
                ...
                if get_diff:
                    ...
                    try:
                        ...
                        self._message_box.end_channel_difference(
                            get_diff, PrematureEndReason.TEMPORARY_SERVER_ISSUES, self._mb_entity_cache
                        )
                        if was_once_logged_in:
                            self._updates_error = e
                            await self.disconnect()
                        ...
                        self._message_box.end_channel_difference(get_diff, PrematureEndReason.BANNED, self._mb_entity_cache)
                        continue
                    except errors.RPCError as e:
                        # Fallback; treat as transient error (the amount of "fatal errors" reported seem to indicate this is most likely what we need to do)
                        self._log[__name__].warning(
    ...
    async def _dispatch_update(self: "TelegramClient", update):
        ...
        for builder, callback in self._event_builders:
            ...
            try:
                ...
                self._log[__name__].debug('Event handler "%s" already has an open conversation, ' "ignoring new one", name)
            except events.StopPropagation:
                name = getattr(callback, "__name__", repr(callback))
                self._log[__name__].debug(
                    'Event handler "%s" stopped chain of propagation ' "for event %s.", name, type(event).__name__
                )
                break
            except Exception as e:
    ...
    async def _handle_auto_reconnect(self: "TelegramClient"):
        ...
        try:
            ...
            self._log[__name__].warning("Error executing high-level request " "after reconnect: %s: %s", type(e), e)
        ...
        try:
            ...
            self._log[__name__].exception("Unhandled exception while getting update difference after reconnect")
...
class EventBuilderDict:
    ...
    def __getitem__(self, builder):
        try:
            ...
            event = self.__dict__[builder] = builder.build(self.update, self.others, self.client._self_id)
