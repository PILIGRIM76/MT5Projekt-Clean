...
from .. import __name__ as __base_name__
from .. import helpers, utils, version
from .._updates import ChannelState, Entity
from .._updates import EntityCache as MbEntityCache
from .._updates import EntityType, MessageBox, SessionState
from ..crypto import rsa
from ..extensions import markdown
from ..network import Connection, ConnectionTcpFull, MTProtoSender, TcpMTProxy
from ..sessions import MemorySession, Session, SQLiteSession
...
class _ExportState:
    ...
    def should_disconnect(self):
        return self._n == 0 and self._connected and (time.time() - self._zero_ts) > _DISCONNECT_EXPORTED_AFTER
...
class TelegramBaseClient(abc.ABC):
    ...
    def __init__(
        ...
        *,
        ...
        if not api_id or not api_hash:
            raise ValueError("Your API ID or Hash cannot be empty or None. " "Refer to telethon.rtfd.io for more information.")
        ...
        init_proxy = (
            None if not issubclass(connection, TcpMTProxy) else types.InputClientProxy(*connection.address_info(proxy))
        )
    ...
    async def connect(self: "TelegramClient") -> None:
        ...
        if not callable(getattr(self._loop, "sock_connect", None)):
            raise TypeError(
                ...
                "import asyncio\n"
                "asyncio.set_event_loop(asyncio.SelectorEventLoop())".format(self._loop.__class__.__name__)
            )

        # ':' in session.server_address is True if it's an IPv6 address
        if not self.session.server_address or (":" in self.session.server_address) != self._use_ipv6:
            await utils.maybe_async(
                self.session.set_dc(DEFAULT_DC_ID, DEFAULT_IPV6_IP if self._use_ipv6 else DEFAULT_IPV4_IP, DEFAULT_PORT)
            )
        ...
        if self._catch_up:
            ...
            for state in cs:
                try:
                    ...
                    self._log[__name__].warning("No access_hash in cache for channel %s, will not catch up", state.channel_id)
                else:
    ...
    async def _disconnect(self: "TelegramClient"):
        ...
        await helpers._cancel(
            self._log[__name__], updates_handle=self._updates_handle, keepalive_handle=self._keepalive_handle
        )
    ...
    async def _get_dc(self: "TelegramClient", dc_id, cdn=False):
        ...
        try:
            ...
            self._log[__name__].warning(
                "Failed to get DC %s (cdn = %s) with use_ipv6 = %s; retrying ignoring IPv6 check", dc_id, cdn, self._use_ipv6
            )
            try:
                return next(dc for dc in cls._config.dc_options if dc.id == dc_id and bool(dc.cdn) == cdn)
            except StopIteration:
    ...
    async def _create_exported_sender(self: "TelegramClient", dc_id):
        ...
        await sender.connect(
            self._connection(dc.ip_address, dc.port, dc.id, loggers=self._log, proxy=self._proxy, local_addr=self._local_addr)
        )
    ...
    async def _borrow_exported_sender(self: "TelegramClient", dc_id):
        ...
        async with self._borrow_sender_lock:
            ...
            if state is None:
                ...
                await sender.connect(
                    self._connection(
                        dc.ip_address, dc.port, dc.id, loggers=self._log, proxy=self._proxy, local_addr=self._local_addr
                    )
                )
    ...
    async def _get_cdn_client(self: "TelegramClient", cdn_redirect):
        ...
        client = self.__class__(session, self.api_id, self.api_hash, proxy=self._proxy, timeout=self._timeout, loop=self.loop)
