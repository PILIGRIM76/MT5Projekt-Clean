...
class _MessagesIter(RequestIter):
    ...
    async def _init(
        self, entity, offset_id, min_id, max_id, from_user, offset_date, add_offset, filter, search, reply_to, scheduled
    ):
    ...
    async def _load_next_chunk(self):
        ...
        for message in messages:
            if isinstance(message, types.MessageEmpty) or self.from_id and message.sender_id != self.from_id:
                continue
...
class MessageMethods:
    ...
    def iter_messages(
        ...
        *,
        ...
        if ids is not None:
            ...
            return _IDsIter(client=self, reverse=reverse, wait_time=wait_time, limit=len(ids), entity=entity, ids=ids)
    ...
    async def _get_comment_data(
        self: "TelegramClient", entity: "hints.EntityLike", message: "typing.Union[int, types.Message]"
    ):
        r = await self(functions.messages.GetDiscussionMessageRequest(peer=entity, msg_id=utils.get_message_id(message)))
    ...
    async def edit_message(
        ...
        *,
        ...
        file_handle, media, image = await self._file_to_media(
            file, supports_streaming=supports_streaming, thumb=thumb, attributes=attributes, force_document=force_document
        )
    ...
    async def delete_messages(
        ...
        *,
        ...
        message_ids = (
            m.id if isinstance(m, (types.Message, types.MessageService, types.MessageEmpty)) else int(m) for m in message_ids
        )
        ...
        if ty == helpers._EntityType.CHANNEL:
            return await self([functions.channels.DeleteMessagesRequest(entity, list(c)) for c in utils.chunks(message_ids)])
        else:
            return await self([functions.messages.DeleteMessagesRequest(list(c), revoke) for c in utils.chunks(message_ids)])
        ...
        *,
        ...
        if max_id is not None:
            if helpers._entity_type(entity) == helpers._EntityType.CHANNEL:
                return await self(functions.channels.ReadHistoryRequest(utils.get_input_channel(entity), max_id=max_id))
            else:
