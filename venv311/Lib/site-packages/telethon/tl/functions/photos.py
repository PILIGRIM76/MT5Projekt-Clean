    ...
    def _bytes(self):
        return b"".join(
            (
                ...
                struct.pack(
                    "<I",
                    (0 if self.fallback is None or self.fallback is False else 1)
                    | (0 if self.bot is None or self.bot is False else 2),
                ),
...
class UploadContactProfilePhotoRequest(TLRequest):
    ...
    def to_dict(self):
        return {
            ...
            "video_emoji_markup": (
                self.video_emoji_markup.to_dict() if isinstance(self.video_emoji_markup, TLObject) else self.video_emoji_markup
            ),
        }
    ...
    def _bytes(self):
        return b"".join(
            (
                ...
                struct.pack(
                    "<I",
                    (0 if self.suggest is None or self.suggest is False else 8)
                    | (0 if self.save is None or self.save is False else 16)
                    | (0 if self.file is None or self.file is False else 1)
                    | (0 if self.video is None or self.video is False else 2)
                    | (0 if self.video_start_ts is None or self.video_start_ts is False else 4)
                    | (0 if self.video_emoji_markup is None or self.video_emoji_markup is False else 32),
                ),
                self.user_id._bytes(),
                ...
                (
                    b""
                    if self.video_emoji_markup is None or self.video_emoji_markup is False
                    else (self.video_emoji_markup._bytes())
                ),
            )
        )
    ...
    def from_reader(cls, reader):
        ...
        return cls(
            ...
            file=_file,
            video=_video,
            video_start_ts=_video_start_ts,
            video_emoji_markup=_video_emoji_markup,
        )
    ...
    def _bytes(self):
        return b"".join(
            (
                ...
                struct.pack(
                    "<I",
                    (0 if self.fallback is None or self.fallback is False else 8)
                    | (0 if self.bot is None or self.bot is False else 32)
                    | (0 if self.file is None or self.file is False else 1)
                    | (0 if self.video is None or self.video is False else 2)
                    | (0 if self.video_start_ts is None or self.video_start_ts is False else 4)
                    | (0 if self.video_emoji_markup is None or self.video_emoji_markup is False else 16),
                ),
                b"" if self.bot is None or self.bot is False else (self.bot._bytes()),
                b"" if self.file is None or self.file is False else (self.file._bytes()),
                b"" if self.video is None or self.video is False else (self.video._bytes()),
