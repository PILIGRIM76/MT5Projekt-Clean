...
class GetDifferenceRequest(TLRequest):
    ...
    def _bytes(self):
        return b"".join(
            (
                ...
                struct.pack(
                    "<I",
                    (0 if self.pts_limit is None or self.pts_limit is False else 2)
                    | (0 if self.pts_total_limit is None or self.pts_total_limit is False else 1)
                    | (0 if self.qts_limit is None or self.qts_limit is False else 4),
                ),
                struct.pack("<i", self.pts),
    ...
    def from_reader(cls, reader):
        ...
        if flags & 4:
            ...
            _qts_limit = None
        return cls(
            pts=_pts, date=_date, qts=_qts, pts_limit=_pts_limit, pts_total_limit=_pts_total_limit, qts_limit=_qts_limit
        )
