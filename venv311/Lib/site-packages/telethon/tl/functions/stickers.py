    ...
    def _bytes(self):
        return b"".join(
            (
                ...
                struct.pack(
                    "<I",
                    (0 if self.emoji is None or self.emoji is False else 1)
                    | (0 if self.mask_coords is None or self.mask_coords is False else 2)
                    | (0 if self.keywords is None or self.keywords is False else 4),
                ),
                self.sticker._bytes(),
...
class CheckShortNameRequest(TLRequest):
    ...
    def to_dict(self):
        return {"_": "CheckShortNameRequest", "short_name": self.short_name}
        ...
        return b"".join(
            (
                ...
                struct.pack(
                    ...
                    | (0 if self.emojis is None or self.emojis is False else 32)
                    | (0 if self.text_color is None or self.text_color is False else 64)
                    | (0 if self.thumb is None or self.thumb is False else 4)
                    | (0 if self.software is None or self.software is False else 8),
                ),
                self.user_id._bytes(),
    ...
    def _bytes(self):
        return b"".join(
            (
                ...
                struct.pack(
                    "<I",
                    (0 if self.thumb is None or self.thumb is False else 1)
                    | (0 if self.thumb_document_id is None or self.thumb_document_id is False else 2),
                ),
                self.stickerset._bytes(),
                b"" if self.thumb is None or self.thumb is False else (self.thumb._bytes()),
                (
                    b""
                    if self.thumb_document_id is None or self.thumb_document_id is False
                    else (struct.pack("<q", self.thumb_document_id))
                ),
            )
        )
...
class SuggestShortNameRequest(TLRequest):
    ...
    def to_dict(self):
        return {"_": "SuggestShortNameRequest", "title": self.title}
