    ...
    def _bytes(self):
        return b"".join(
            (
                ...
                (
                    b""
                    if self.slots is None or self.slots is False
                    else b"".join(
                        (
                            b"\x15\xc4\xb5\x1c",
                            struct.pack("<i", len(self.slots)),
                            b"".join(struct.pack("<i", x) for x in self.slots),
                        )
                    )
                ),
                self.peer._bytes(),
...
class GetBoostsStatusRequest(TLRequest):
    ...
    def to_dict(self):
        return {"_": "GetBoostsStatusRequest", "peer": self.peer.to_dict() if isinstance(self.peer, TLObject) else self.peer}
...
class GetMyBoostsRequest(TLRequest):
    ...
    def to_dict(self):
        return {"_": "GetMyBoostsRequest"}

    def _bytes(self):
        return b"".join((b"J{\xe7\x0b",))
