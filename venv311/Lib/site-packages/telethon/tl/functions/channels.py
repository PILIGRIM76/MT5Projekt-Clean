...
class CheckSearchPostsFloodRequest(TLRequest):
    ...
    def to_dict(self):
        return {"_": "CheckSearchPostsFloodRequest", "query": self.query}
...
class CreateChannelRequest(TLRequest):
    ...
    def _bytes(self):
        assert ((self.geo_point or self.geo_point is not None) and (self.address or self.address is not None)) or (
            (self.geo_point is None or self.geo_point is False) and (self.address is None or self.address is False)
        ), "geo_point, address parameters must all be False-y (like None) or all be True-y"
        return b"".join(
            (
                b"\x07g\x00\x91",
                struct.pack(
                    ...
                    | (0 if self.forum is None or self.forum is False else 32)
                    | (0 if self.geo_point is None or self.geo_point is False else 4)
                    | (0 if self.address is None or self.address is False else 4)
                    | (0 if self.ttl_period is None or self.ttl_period is False else 16),
                ),
...
class GetAdminLogRequest(TLRequest):
    ...
    def _bytes(self):
        return b"".join(
            (
                ...
                struct.pack(
                    "<I",
                    (0 if self.events_filter is None or self.events_filter is False else 1)
    ...
    def from_reader(cls, reader):
        ...
        return cls(
            channel=_channel, q=_q, max_id=_max_id, min_id=_min_id, limit=_limit, events_filter=_events_filter, admins=_admins
        )
...
class GetAdminedPublicChannelsRequest(TLRequest):
    ...
    def __init__(
        self, by_location: Optional[bool] = None, check_limit: Optional[bool] = None, for_personal: Optional[bool] = None
    ):
        """
        ...
        return b"".join(
            (
                ...
                struct.pack(
                    ...
                    | (0 if self.check_limit is None or self.check_limit is False else 2)
                    | (0 if self.for_personal is None or self.for_personal is False else 4),
                ),
            )
        )
...
class GetParticipantsRequest(TLRequest):
    ...
    def __init__(
        self, channel: "TypeInputChannel", filter: "TypeChannelParticipantsFilter", offset: int, limit: int, hash: int
    ):
        """
...
class GetSendAsRequest(TLRequest):
    ...
    def __init__(
        self, peer: "TypeInputPeer", for_paid_reactions: Optional[bool] = None, for_live_stories: Optional[bool] = None
    ):
        """
    ...
    def _bytes(self):
        return b"".join(
            (
                ...
                struct.pack(
                    "<I",
                    (0 if self.for_paid_reactions is None or self.for_paid_reactions is False else 1)
                    | (0 if self.for_live_stories is None or self.for_live_stories is False else 2),
                ),
                self.peer._bytes(),
            )
        )
        ...
        return b"".join(
            (
                ...
                struct.pack(
                    ...
                    | (0 if self.query is None or self.query is False else 2)
                    | (0 if self.allow_paid_stars is None or self.allow_paid_stars is False else 4),
                ),
...
class ToggleSignaturesRequest(TLRequest):
    ...
    def __init__(
        self, channel: "TypeInputChannel", signatures_enabled: Optional[bool] = None, profiles_enabled: Optional[bool] = None
    ):
        """
        :returns Updates: Instance of either UpdatesTooLong, UpdateShortMessage, UpdateShortChatMessage, UpdateShort, UpdatesCombined, Updates, UpdateShortSentMessage.
        """
    ...
    def _bytes(self):
        return b"".join(
            (
                ...
                struct.pack(
                    "<I",
                    (0 if self.signatures_enabled is None or self.signatures_enabled is False else 1)
                    | (0 if self.profiles_enabled is None or self.profiles_enabled is False else 2),
                ),
                self.channel._bytes(),
            )
        )
        ...
        return b"".join(
            (
                ...
                struct.pack(
                    ...
                    | (0 if self.background_emoji_id is None or self.background_emoji_id is False else 1),
                ),
                self.channel._bytes(),
                b"" if self.color is None or self.color is False else (struct.pack("<i", self.color)),
                (
                    b""
                    if self.background_emoji_id is None or self.background_emoji_id is False
                    else (struct.pack("<q", self.background_emoji_id))
                ),
            )
        )
...
class UpdatePaidMessagesPriceRequest(TLRequest):
    ...
    def __init__(
        self, channel: "TypeInputChannel", send_paid_messages_stars: int, broadcast_messages_allowed: Optional[bool] = None
    ):
        """
        :returns Updates: Instance of either UpdatesTooLong, UpdateShortMessage, UpdateShortChatMessage, UpdateShort, UpdatesCombined, Updates, UpdateShortSentMessage.
        """
    ...
    def _bytes(self):
        return b"".join(
            (
                b"{2\x12K",
                struct.pack(
                    "<I", (0 if self.broadcast_messages_allowed is None or self.broadcast_messages_allowed is False else 1)
                ),
                self.channel._bytes(),
