...
class EligibleToJoin(TLObject):
    ...
    def to_dict(self):
        return {"_": "EligibleToJoin", "terms_url": self.terms_url, "monthly_sent_sms": self.monthly_sent_sms}
...
class Status(TLObject):
    ...
    def __init__(
        self,
        recent_sent: int,
        recent_since: Optional[datetime],
        recent_remains: int,
        total_sent: int,
        total_since: Optional[datetime],
        terms_url: str,
    ...
    def _bytes(self):
        return b"".join(
            (
                ...
                struct.pack(
                    "<I",
                    (0 if self.allow_international is None or self.allow_international is False else 1)
                    | (0 if self.last_gift_slug is None or self.last_gift_slug is False else 2),
                ),
                ...
                (
                    b""
                    if self.last_gift_slug is None or self.last_gift_slug is False
                    else (self.serialize_bytes(self.last_gift_slug))
                ),
                self.serialize_bytes(self.terms_url),
            )
        )
    ...
    def from_reader(cls, reader):
        ...
        _terms_url = reader.tgread_string()
        return cls(
            recent_sent=_recent_sent,
            recent_since=_recent_since,
            recent_remains=_recent_remains,
            total_sent=_total_sent,
            total_since=_total_since,
            terms_url=_terms_url,
            allow_international=_allow_international,
            last_gift_slug=_last_gift_slug,
        )
