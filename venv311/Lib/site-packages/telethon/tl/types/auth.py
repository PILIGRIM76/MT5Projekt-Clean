...
class Authorization(TLObject):
    ...
    def __init__(
        self,
        user: "TypeUser",
        setup_password_required: Optional[bool] = None,
    ...
    def _bytes(self):
        assert (
            (self.setup_password_required or self.setup_password_required is not None)
            and (self.otherwise_relogin_days or self.otherwise_relogin_days is not None)
        ) or (
            (self.setup_password_required is None or self.setup_password_required is False)
            and (self.otherwise_relogin_days is None or self.otherwise_relogin_days is False)
        ), "setup_password_required, otherwise_relogin_days parameters must all be False-y (like None) or all be True-y"
        return b"".join(
            (
                b"\xd4\xc0\xa2.",
                struct.pack(
                    "<I",
                    (0 if self.setup_password_required is None or self.setup_password_required is False else 2)
                    | (0 if self.otherwise_relogin_days is None or self.otherwise_relogin_days is False else 2)
                    | (0 if self.tmp_sessions is None or self.tmp_sessions is False else 1)
                    | (0 if self.future_auth_token is None or self.future_auth_token is False else 4),
                ),
                (
                    b""
                    if self.otherwise_relogin_days is None or self.otherwise_relogin_days is False
                    else (struct.pack("<i", self.otherwise_relogin_days))
                ),
                b"" if self.tmp_sessions is None or self.tmp_sessions is False else (struct.pack("<i", self.tmp_sessions)),
                (
                    b""
                    if self.future_auth_token is None or self.future_auth_token is False
                    else (self.serialize_bytes(self.future_auth_token))
                ),
                self.user._bytes(),
            )
        )
    ...
    def from_reader(cls, reader):
        ...
        return cls(
            user=_user,
            setup_password_required=_setup_password_required,
            otherwise_relogin_days=_otherwise_relogin_days,
            tmp_sessions=_tmp_sessions,
            future_auth_token=_future_auth_token,
        )
...
class AuthorizationSignUpRequired(TLObject):
    ...
    def to_dict(self):
        return {
            ...
            "terms_of_service": (
                self.terms_of_service.to_dict() if isinstance(self.terms_of_service, TLObject) else self.terms_of_service
            ),
        }
...
class CodeTypeFragmentSms(TLObject):
    ...
    def to_dict(self):
        return {"_": "CodeTypeFragmentSms"}

    def _bytes(self):
        return b"".join((b"\x8c\x99\xed\x06",))
...
class CodeTypeMissedCall(TLObject):
    ...
    def to_dict(self):
        return {"_": "CodeTypeMissedCall"}

    def _bytes(self):
        return b"".join((b"\xee\xd6\x1a\xd6",))
...
class LoggedOut(TLObject):
    ...
    def to_dict(self):
        return {"_": "LoggedOut", "future_auth_token": self.future_auth_token}
...
class SentCodePaymentRequired(TLObject):
    ...
    def from_reader(cls, reader):
        ...
        return cls(
            store_product=_store_product,
            phone_code_hash=_phone_code_hash,
            support_email_address=_support_email_address,
            support_email_subject=_support_email_subject,
            currency=_currency,
            amount=_amount,
        )
...
class SentCodeTypeEmailCode(TLObject):
    ...
    def __init__(
        ...
        email_pattern: str,
        length: int,
        apple_signin_allowed: Optional[bool] = None,
        google_signin_allowed: Optional[bool] = None,
        reset_available_period: Optional[int] = None,
        reset_pending_date: Optional[datetime] = None,
    ):
        """
        Constructor for auth.SentCodeType: Instance of either SentCodeTypeApp, SentCodeTypeSms, SentCodeTypeCall, SentCodeTypeFlashCall, SentCodeTypeMissedCall, SentCodeTypeEmailCode, SentCodeTypeSetUpEmailRequired, SentCodeTypeFragmentSms, SentCodeTypeFirebaseSms, SentCodeTypeSmsWord, SentCodeTypeSmsPhrase.
        """
    ...
    def _bytes(self):
        return b"".join(
            (
                ...
                struct.pack(
                    "<I",
                    (0 if self.apple_signin_allowed is None or self.apple_signin_allowed is False else 1)
                    | (0 if self.google_signin_allowed is None or self.google_signin_allowed is False else 2)
                    | (0 if self.reset_available_period is None or self.reset_available_period is False else 8)
                    | (0 if self.reset_pending_date is None or self.reset_pending_date is False else 16),
                ),
                self.serialize_bytes(self.email_pattern),
                struct.pack("<i", self.length),
                (
                    b""
                    if self.reset_available_period is None or self.reset_available_period is False
                    else (struct.pack("<i", self.reset_available_period))
                ),
                (
                    b""
                    if self.reset_pending_date is None or self.reset_pending_date is False
                    else (self.serialize_datetime(self.reset_pending_date))
                ),
            )
        )
    ...
    def from_reader(cls, reader):
        ...
        return cls(
            ...
            length=_length,
            apple_signin_allowed=_apple_signin_allowed,
            google_signin_allowed=_google_signin_allowed,
            reset_available_period=_reset_available_period,
            reset_pending_date=_reset_pending_date,
        )
...
class SentCodeTypeFirebaseSms(TLObject):
    ...
    def _bytes(self):
        assert (
            (self.play_integrity_project_id or self.play_integrity_project_id is not None)
            and (self.play_integrity_nonce or self.play_integrity_nonce is not None)
        ) or (
            (self.play_integrity_project_id is None or self.play_integrity_project_id is False)
            and (self.play_integrity_nonce is None or self.play_integrity_nonce is False)
        ), "play_integrity_project_id, play_integrity_nonce parameters must all be False-y (like None) or all be True-y"
        assert ((self.receipt or self.receipt is not None) and (self.push_timeout or self.push_timeout is not None)) or (
            (self.receipt is None or self.receipt is False) and (self.push_timeout is None or self.push_timeout is False)
        ), "receipt, push_timeout parameters must all be False-y (like None) or all be True-y"
        return b"".join(
            (
                b"6\xd7\x9f\x00",
                struct.pack(
                    "<I",
                    (0 if self.nonce is None or self.nonce is False else 1)
                    | (0 if self.play_integrity_project_id is None or self.play_integrity_project_id is False else 4)
                    | (0 if self.play_integrity_nonce is None or self.play_integrity_nonce is False else 4)
                    | (0 if self.receipt is None or self.receipt is False else 2)
                    | (0 if self.push_timeout is None or self.push_timeout is False else 2),
                ),
                b"" if self.nonce is None or self.nonce is False else (self.serialize_bytes(self.nonce)),
                (
                    b""
                    if self.play_integrity_project_id is None or self.play_integrity_project_id is False
                    else (struct.pack("<q", self.play_integrity_project_id))
                ),
                (
                    b""
                    if self.play_integrity_nonce is None or self.play_integrity_nonce is False
                    else (self.serialize_bytes(self.play_integrity_nonce))
                ),
    ...
    def from_reader(cls, reader):
        ...
        return cls(
            length=_length,
            nonce=_nonce,
            play_integrity_project_id=_play_integrity_project_id,
            play_integrity_nonce=_play_integrity_nonce,
            receipt=_receipt,
...
class SentCodeTypeFragmentSms(TLObject):
    ...
    def to_dict(self):
        return {"_": "SentCodeTypeFragmentSms", "url": self.url, "length": self.length}
...
class SentCodeTypeMissedCall(TLObject):
    ...
    def to_dict(self):
        return {"_": "SentCodeTypeMissedCall", "prefix": self.prefix, "length": self.length}
...
class SentCodeTypeSmsPhrase(TLObject):
    ...
    def to_dict(self):
        return {"_": "SentCodeTypeSmsPhrase", "beginning": self.beginning}
...
class SentCodeTypeSmsWord(TLObject):
    ...
    def to_dict(self):
        return {"_": "SentCodeTypeSmsWord", "beginning": self.beginning}
