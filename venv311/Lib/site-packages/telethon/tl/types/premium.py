    ...
    def _bytes(self):
        return b"".join(
            (
                ...
                b"\x15\xc4\xb5\x1c",
                struct.pack("<i", len(self.boosts)),
                b"".join(x._bytes() for x in self.boosts),
...
class BoostsStatus(TLObject):
    ...
    def to_dict(self):
        return {
            ...
            "premium_audience": (
                self.premium_audience.to_dict() if isinstance(self.premium_audience, TLObject) else self.premium_audience
            ),
            "prepaid_giveaways": (
                []
                if self.prepaid_giveaways is None
                else [x.to_dict() if isinstance(x, TLObject) else x for x in self.prepaid_giveaways]
            ),
            "my_boost_slots": [] if self.my_boost_slots is None else self.my_boost_slots[:],
        }

    def _bytes(self):
        assert ((self.my_boost or self.my_boost is not None) and (self.my_boost_slots or self.my_boost_slots is not None)) or (
            (self.my_boost is None or self.my_boost is False) and (self.my_boost_slots is None or self.my_boost_slots is False)
        ), "my_boost, my_boost_slots parameters must all be False-y (like None) or all be True-y"
        return b"".join(
            (
                b"zBYI",
                struct.pack(
                    "<I",
                    (0 if self.my_boost is None or self.my_boost is False else 4)
                    | (0 if self.gift_boosts is None or self.gift_boosts is False else 16)
                    | (0 if self.next_level_boosts is None or self.next_level_boosts is False else 1)
                    | (0 if self.premium_audience is None or self.premium_audience is False else 2)
                    | (0 if self.prepaid_giveaways is None or self.prepaid_giveaways is False else 8)
                    | (0 if self.my_boost_slots is None or self.my_boost_slots is False else 4),
                ),
                ...
                (
                    b""
                    if self.next_level_boosts is None or self.next_level_boosts is False
                    else (struct.pack("<i", self.next_level_boosts))
                ),
                ...
                (
                    b""
                    if self.prepaid_giveaways is None or self.prepaid_giveaways is False
                    else b"".join(
                        (
                            b"\x15\xc4\xb5\x1c",
                            struct.pack("<i", len(self.prepaid_giveaways)),
                            b"".join(x._bytes() for x in self.prepaid_giveaways),
                        )
                    )
                ),
                (
                    b""
                    if self.my_boost_slots is None or self.my_boost_slots is False
                    else b"".join(
                        (
                            b"\x15\xc4\xb5\x1c",
                            struct.pack("<i", len(self.my_boost_slots)),
                            b"".join(struct.pack("<i", x) for x in self.my_boost_slots),
                        )
                    )
                ),
            )
    ...
    def from_reader(cls, reader):
        ...
        return cls(
            level=_level,
            current_level_boosts=_current_level_boosts,
            boosts=_boosts,
            boost_url=_boost_url,
            my_boost=_my_boost,
            gift_boosts=_gift_boosts,
            next_level_boosts=_next_level_boosts,
            premium_audience=_premium_audience,
            prepaid_giveaways=_prepaid_giveaways,
            my_boost_slots=_my_boost_slots,
        )
...
class MyBoosts(TLObject):
    ...
    def to_dict(self):
        return {
            "_": "MyBoosts",
            "my_boosts": (
                [] if self.my_boosts is None else [x.to_dict() if isinstance(x, TLObject) else x for x in self.my_boosts]
            ),
    ...
    def _bytes(self):
        return b"".join(
            (
                b"\xe2(\xe2\x9a",
                b"\x15\xc4\xb5\x1c",
                struct.pack("<i", len(self.my_boosts)),
                b"".join(x._bytes() for x in self.my_boosts),
                b"\x15\xc4\xb5\x1c",
