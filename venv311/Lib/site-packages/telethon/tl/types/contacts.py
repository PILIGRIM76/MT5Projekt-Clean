...
class Found(TLObject):
    ...
    def __init__(
        self, my_results: List["TypePeer"], results: List["TypePeer"], chats: List["TypeChat"], users: List["TypeUser"]
    ):
        """
    ...
    def to_dict(self):
        return {
            "_": "Found",
            "my_results": (
                [] if self.my_results is None else [x.to_dict() if isinstance(x, TLObject) else x for x in self.my_results]
            ),
...
class ImportedContacts(TLObject):
    ...
    def to_dict(self):
        return {
            "_": "ImportedContacts",
            "imported": (
                [] if self.imported is None else [x.to_dict() if isinstance(x, TLObject) else x for x in self.imported]
            ),
            "popular_invites": (
                []
                if self.popular_invites is None
                else [x.to_dict() if isinstance(x, TLObject) else x for x in self.popular_invites]
            ),
...
class SponsoredPeersEmpty(TLObject):
    ...
    def to_dict(self):
        return {"_": "SponsoredPeersEmpty"}

    def _bytes(self):
        return b"".join((b"\xb1\xb42\xea",))
...
class TopPeers(TLObject):
    ...
    def to_dict(self):
        return {
            "_": "TopPeers",
            "categories": (
                [] if self.categories is None else [x.to_dict() if isinstance(x, TLObject) else x for x in self.categories]
            ),
            "chats": [] if self.chats is None else [x.to_dict() if isinstance(x, TLObject) else x for x in self.chats],
            "users": [] if self.users is None else [x.to_dict() if isinstance(x, TLObject) else x for x in self.users],
        }
