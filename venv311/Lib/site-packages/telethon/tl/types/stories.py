    ...
    def _bytes(self):
        return b"".join(
            (
                ...
                b"".join(x._bytes() for x in self.albums),
            )
        )
...
class AlbumsNotModified(TLObject):
    ...
    def to_dict(self):
        return {"_": "AlbumsNotModified"}

    def _bytes(self):
        return b"".join((b"\xeb\xdaNV",))
...
class AllStories(TLObject):
    ...
    def to_dict(self):
        return {
            ...
            "state": self.state,
            "peer_stories": (
                [] if self.peer_stories is None else [x.to_dict() if isinstance(x, TLObject) else x for x in self.peer_stories]
            ),
    ...
    def _bytes(self):
        return b"".join(
            (
                ...
                b"\x15\xc4\xb5\x1c",
                struct.pack("<i", len(self.peer_stories)),
                b"".join(x._bytes() for x in self.peer_stories),
                b"\x15\xc4\xb5\x1c",
...
class CanSendStoryCount(TLObject):
    ...
    def to_dict(self):
        return {"_": "CanSendStoryCount", "count_remains": self.count_remains}
    ...
    def _bytes(self):
        return b"".join(
            (
                ...
                b"".join(x._bytes() for x in self.stories),
    ...
    def _bytes(self):
        return b"".join(
            (
                ...
                (
                    b""
                    if self.pinned_to_top is None or self.pinned_to_top is False
                    else b"".join(
                        (
                            b"\x15\xc4\xb5\x1c",
                            struct.pack("<i", len(self.pinned_to_top)),
                            b"".join(struct.pack("<i", x) for x in self.pinned_to_top),
                        )
                    )
                ),
                b"\x15\xc4\xb5\x1c",
...
class StoryViewsList(TLObject):
    ...
    def from_reader(cls, reader):
        ...
        return cls(
            count=_count,
            views_count=_views_count,
            forwards_count=_forwards_count,
            reactions_count=_reactions_count,
            views=_views,
            chats=_chats,
            users=_users,
