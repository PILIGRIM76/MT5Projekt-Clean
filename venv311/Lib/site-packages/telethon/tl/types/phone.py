...
class GroupCall(TLObject):
    ...
    def to_dict(self):
        return {
            ...
            "participants": (
                [] if self.participants is None else [x.to_dict() if isinstance(x, TLObject) else x for x in self.participants]
            ),
...
class GroupCallStars(TLObject):
    ...
    def __init__(
        self, total_stars: int, top_donors: List["TypeGroupCallDonor"], chats: List["TypeChat"], users: List["TypeUser"]
    ):
        """
    ...
    def to_dict(self):
        return {
            ...
            "top_donors": (
                [] if self.top_donors is None else [x.to_dict() if isinstance(x, TLObject) else x for x in self.top_donors]
            ),
            "chats": [] if self.chats is None else [x.to_dict() if isinstance(x, TLObject) else x for x in self.chats],
            "users": [] if self.users is None else [x.to_dict() if isinstance(x, TLObject) else x for x in self.users],
        }
    ...
    def _bytes(self):
        return b"".join(
            (
                ...
                b"\x15\xc4\xb5\x1c",
                struct.pack("<i", len(self.top_donors)),
                b"".join(x._bytes() for x in self.top_donors),
                b"\x15\xc4\xb5\x1c",
                struct.pack("<i", len(self.chats)),
                b"".join(x._bytes() for x in self.chats),
                b"\x15\xc4\xb5\x1c",
...
class GroupCallStreamChannels(TLObject):
    ...
    def to_dict(self):
        return {
            ...
            "channels": (
                [] if self.channels is None else [x.to_dict() if isinstance(x, TLObject) else x for x in self.channels]
            ),
        }
    ...
    def _bytes(self):
        return b"".join(
            (
                b"\xb2\x82\xe4\xd0",
                b"\x15\xc4\xb5\x1c",
                struct.pack("<i", len(self.channels)),
                b"".join(x._bytes() for x in self.channels),
            )
        )
...
class GroupCallStreamRtmpUrl(TLObject):
    ...
    def to_dict(self):
        return {"_": "GroupCallStreamRtmpUrl", "url": self.url, "key": self.key}
...
class GroupParticipants(TLObject):
    ...
    def from_reader(cls, reader):
        ...
        return cls(
            count=_count, participants=_participants, next_offset=_next_offset, chats=_chats, users=_users, version=_version
        )
