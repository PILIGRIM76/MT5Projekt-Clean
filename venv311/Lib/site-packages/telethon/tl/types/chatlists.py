...
class ChatlistInvite(TLObject):
    ...
    def __init__(
        ...
        chats: List["TypeChat"],
        users: List["TypeUser"],
        title_noanimate: Optional[bool] = None,
    ...
    def _bytes(self):
        return b"".join(
            (
                ...
                struct.pack(
                    "<I",
                    (0 if self.title_noanimate is None or self.title_noanimate is False else 2)
                    | (0 if self.emoticon is None or self.emoticon is False else 1),
                ),
                self.title._bytes(),
    ...
    def from_reader(cls, reader):
        ...
        return cls(
            title=_title, peers=_peers, chats=_chats, users=_users, title_noanimate=_title_noanimate, emoticon=_emoticon
        )
...
class ChatlistInviteAlready(TLObject):
    ...
    def to_dict(self):
        return {
            ...
            "missing_peers": (
                []
                if self.missing_peers is None
                else [x.to_dict() if isinstance(x, TLObject) else x for x in self.missing_peers]
            ),
            "already_peers": (
                []
                if self.already_peers is None
                else [x.to_dict() if isinstance(x, TLObject) else x for x in self.already_peers]
            ),
            "chats": [] if self.chats is None else [x.to_dict() if isinstance(x, TLObject) else x for x in self.chats],
            "users": [] if self.users is None else [x.to_dict() if isinstance(x, TLObject) else x for x in self.users],
        }
    ...
    def _bytes(self):
        return b"".join(
            (
                ...
                b"\x15\xc4\xb5\x1c",
                struct.pack("<i", len(self.missing_peers)),
                b"".join(x._bytes() for x in self.missing_peers),
                b"\x15\xc4\xb5\x1c",
                struct.pack("<i", len(self.already_peers)),
                b"".join(x._bytes() for x in self.already_peers),
                b"\x15\xc4\xb5\x1c",
                struct.pack("<i", len(self.chats)),
                b"".join(x._bytes() for x in self.chats),
                b"\x15\xc4\xb5\x1c",
    ...
    def from_reader(cls, reader):
        ...
        return cls(
            filter_id=_filter_id, missing_peers=_missing_peers, already_peers=_already_peers, chats=_chats, users=_users
        )
