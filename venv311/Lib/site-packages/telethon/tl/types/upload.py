...
class FileCdnRedirect(TLObject):
    ...
    def __init__(
        self, dc_id: int, file_token: bytes, encryption_key: bytes, encryption_iv: bytes, file_hashes: List["TypeFileHash"]
    ):
        """
        Constructor for upload.File: Instance of either File, FileCdnRedirect.
        """
    ...
    def to_dict(self):
        return {
            ...
            "file_hashes": (
                [] if self.file_hashes is None else [x.to_dict() if isinstance(x, TLObject) else x for x in self.file_hashes]
            ),
        }
