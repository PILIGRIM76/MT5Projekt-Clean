...
from ... import errors, helpers, utils
...
class Conversation(ChatGetter):
    ...
    def __init__(self, client, input_chat, *, timeout, total_timeout, max_messages, exclusive, replies_are_responses):
    ...
    def get_response(self, message=None, *, timeout=None):
        ...
        """
        return self._get_message(message, self._response_indices, self._pending_responses, timeout, lambda x, y: True)
    ...
    def get_edit(self, message=None, *, timeout=None):
        ...
        earliest_edit = min(
            (x for x in self._incoming if x.edit_date and x.id > target_id and x.edit_date.timestamp() > target_date),
    ...
    def _cancel_all(self, exception=None):
        ...
        for pending in itertools.chain(
            self._pending_responses.values(), self._pending_replies.values(), self._pending_edits.values()
        ):
            if exception:
