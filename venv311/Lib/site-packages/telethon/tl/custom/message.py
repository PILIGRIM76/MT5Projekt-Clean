...
class Message(ChatGetter, SenderGetter, TLObject):
    ...
    def _finish_init(self, client, entities, input_chat):
        ...
        if self.via_bot_id:
            self._via_bot, self._via_input_bot = utils._get_entity_pair(self.via_bot_id, entities, cache)

        if self.fwd_from:
            self._forward = Forward(self._client, self.fwd_from, entities)

        if self.action:
            if isinstance(self.action, (types.MessageActionChatAddUser, types.MessageActionChatCreate)):
                ...
                self._action_entities = [entities.get(utils.get_peer_id(types.PeerChannel(self.action.channel_id)))]
            elif isinstance(self.action, types.MessageActionChannelMigrateFrom):
                self._action_entities = [entities.get(utils.get_peer_id(types.PeerChat(self.action.chat_id)))]

        if self.replies and self.replies.channel_id:
            self._linked_chat = entities.get(utils.get_peer_id(types.PeerChannel(self.replies.channel_id)))
    ...
    def video_note(self):
        ...
        """
        return self._document_by_attribute(types.DocumentAttributeVideo, lambda attr: attr.round_message)
    ...
    def geo(self):
        ...
        """
        if isinstance(self.media, (types.MessageMediaGeo, types.MessageMediaGeoLive, types.MessageMediaVenue)):
    ...
    def reply_to_msg_id(self):
        ...
        """
        return self.reply_to.reply_to_msg_id if isinstance(self.reply_to, types.MessageReplyHeader) else None
    ...
    async def get_reply_message(self):
        ...
        if self._reply_message is None and self._client:
            ...
            self._reply_message = await self._client.get_messages(
                await self.get_input_chat() if self.is_channel else None, ids=types.InputMessageReplyTo(self.id)
            )
        ...
        *,
        ...
        if self.poll is not None:
            ...
            return await self._client(
                functions.messages.SendVoteRequest(peer=self._input_chat, msg_id=self.id, options=options)
            )
        ...
        button = find_button()
        if button:
            return await button.click(share_phone=share_phone, share_geo=share_geo, password=password, open_url=open_url)
    ...
    async def pin(self, *, notify=False, pm_oneside=False):
        ...
        if self._client:
            return await self._client.pin_message(await self.get_input_chat(), self.id, notify=notify, pm_oneside=pm_oneside)
    ...
    def _set_buttons(self, chat, bot):
        ...
        """
        if self._client and isinstance(self.reply_markup, (types.ReplyInlineMarkup, types.ReplyKeyboardMarkup)):
            self._buttons = [
    ...
    def _needed_markup_bot(self):
        ...
        """
        if self._client and not isinstance(self.reply_markup, (types.ReplyInlineMarkup, types.ReplyKeyboardMarkup)):
            return None
            ...
            for button in row.buttons:
                if isinstance(button, types.KeyboardButtonSwitchInline):
                    ...
                    if button.same_peer or not self.via_bot_id:
                        ...
                        try:
                            return self._client._mb_entity_cache.get(utils.resolve_id(self.via_bot_id)[0])._as_input_peer()
