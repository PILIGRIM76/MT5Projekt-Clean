        ...
        *,
        ...
        try:
            ...
            if isinstance(media, types.InputPhoto):
                ...
                r = await self._client(functions.messages.UploadMediaRequest(types.InputPeerSelf(), media=media))
        ...
        *,
        ...
        if sum(1 for x in args if x is not None and x is not False) != 1:
            raise ValueError(
                "Must set exactly one of text, geo, contact or game (set {})".format(
                    ", ".join(x[0] for x in zip("text geo contact game".split(), args) if x[1]) or "none"
                )
            )
        ...
        if text is not None:
            ...
            if media:
                ...
                return types.InputBotInlineMessageMediaAuto(message=text, entities=msg_entities, reply_markup=markup)
            else:
                return types.InputBotInlineMessageText(
                    message=text, no_webpage=not link_preview, entities=msg_entities, reply_markup=markup
                )
        elif isinstance(geo, (types.InputGeoPoint, types.GeoPoint)):
            return types.InputBotInlineMessageMediaGeo(geo_point=utils.get_input_geo(geo), period=period, reply_markup=markup)
