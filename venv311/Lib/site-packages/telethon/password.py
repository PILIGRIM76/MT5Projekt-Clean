...
def compute_digest(algo: types.PasswordKdfAlgoSHA256SHA256PBKDF2HMACSHA512iter100000SHA256ModPow, password: str):
    ...
    value = pow(algo.g, int.from_bytes(compute_hash(algo, password), "big"), int.from_bytes(algo.p, "big"))
...
def compute_check(request: types.account.Password, password: str):
    ...
    M1 = sha256(xor(sha256(p_for_hash), sha256(g_for_hash)), sha256(algo.salt1), sha256(algo.salt2), a_for_hash, b_for_hash, K)
