    ...
    * Whenever a user is removed or leaves a group if it has
    ...
    def build(cls, update, others=None, self_id=None):
        ...
        if isinstance(update, types.UpdatePinnedChannelMessages) and not update.pinned:
            return cls.Event(types.PeerChannel(update.channel_id), pin_ids=update.messages, pin=update.pinned)
            ...
            return cls.Event(types.PeerChat(update.chat_id), added_by=update.inviter_id or True, users=update.user_id)
        ...
        # better not to rely on this. Rely only in MessageActionChatDeleteUser.
        ...
        elif isinstance(update, (types.UpdateNewMessage, types.UpdateNewChannelMessage)) and isinstance(
            update.message, types.MessageService
        ):
            msg = update.message
            ...
            if isinstance(action, types.MessageActionChatJoinedByLink):
                ...
                return cls.Event(msg, kicked_by=utils.get_peer_id(msg.from_id) if msg.from_id else True, users=action.user_id)
        ...
        elif isinstance(update, types.UpdateChannelParticipant) and bool(update.new_participant) != bool(
    ...
    class Event(EventCommon):
        ...
        async def delete(self, *args, **kwargs):
            ...
            return await self._client.delete_messages(await self.get_input_chat(), [self.action_message], *args, **kwargs)
        ...
        async def get_pinned_messages(self):
            ...
            if chat:
                self._pinned_messages = await self._client.get_messages(self._input_chat, ids=self._pin_ids)
        ...
        def users(self):
            ...
            if self._users is None:
                self._users = [self._entities[user_id] for user_id in self._user_ids if user_id in self._entities]
        ...
        async def get_users(self):
            ...
            if (self.users is None or len(self._users) != len(self._user_ids)) and self.action_message:
                ...
                self._users = [u for u in self.action_message.action_entities if isinstance(u, (types.User, types.UserEmpty))]
                ...
                for user_id in self._user_ids:
                    ...
                    try:
                        self._input_users.append(
                            self._client._mb_entity_cache.get(utils.resolve_id(user_id)[0])._as_input_peer()
                        )
                        continue
