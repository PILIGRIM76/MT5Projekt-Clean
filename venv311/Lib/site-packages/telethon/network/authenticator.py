...
from ..tl.functions import ReqDHParamsRequest, ReqPqMultiRequest, SetClientDHParamsRequest
...
async def do_authentication(sender):
    ...
    pq_inner_data = bytes(
        PQInnerData(
            pq=rsa.get_byte_array(pq), p=p, q=q, nonce=res_pq.nonce, server_nonce=res_pq.server_nonce, new_nonce=new_nonce
        )
    )
    ...
    assert isinstance(server_dh_params, (ServerDHParamsOk, ServerDHParamsFail)), "Step 2.1 answer was %s" % server_dh_params
    ...
    if isinstance(server_dh_params, ServerDHParamsFail):
        nnh = int.from_bytes(sha1(new_nonce.to_bytes(32, "little", signed=True)).digest()[4:20], "little", signed=True)
