...
from ... import helpers
from ...errors import InvalidBufferError, InvalidChecksumError
...
class Connection(abc.ABC):
    ...
    def _wrap_socket_ssl(sock):
        if ssl_mod is None:
            raise RuntimeError("Cannot use proxy that requires SSL " "without the SSL module being available")

        return ssl_mod.wrap_socket(
            sock, do_handshake_on_connect=True, ssl_version=ssl_mod.PROTOCOL_SSLv23, ciphers="ADH-AES256-SHA"
        )
    ...
    async def _proxy_connect(self, timeout=None, local_addr=None):
        ...
        if python_socks:
            ...
            if local_addr is None:
                sock = await proxy.connect(dest_host=self._ip, dest_port=self._port, timeout=timeout)
            else:
                ...
                await asyncio.wait_for(helpers.get_running_loop().sock_connect(sock=sock, address=address), timeout=timeout)
                ...
                sock = await proxy.connect(dest_host=self._ip, dest_port=self._port, timeout=timeout, _socket=sock)
    ...
    async def _connect(self, timeout=None, ssl=None):
        ...
        if not self._proxy:
            self._reader, self._writer = await asyncio.wait_for(
                asyncio.open_connection(host=self._ip, port=self._port, ssl=ssl, local_addr=local_addr), timeout=timeout
            )
        else:
    ...
    def __str__(self):
        return "{}:{}/{}".format(self._ip, self._port, self.__class__.__name__.replace("Connection", ""))
