...
class MTProtoState:
    ...
    def write_data_as_message(self, buffer, data, content_related, *, after_id=None):
        ...
        if after_id is None:
            ...
            body = GzipPacked.gzip_if_smaller(content_related, bytes(InvokeAfterMsgRequest(after_id, _OpaqueRequest(data))))
    ...
    def update_time_offset(self, correct_msg_id):
        ...
        if self.time_offset != old:
            self._last_msg_id = 0
            self._log.debug(
                "Updated time offset (old offset %d, bad %d, good %d, new %d)", old, bad, correct_msg_id, self.time_offset
            )
