...
from ..tl.functions import DestroyAuthKeyRequest, DestroySessionRequest, PingRequest
...
class MTProtoSender:
    ...
    def _transport_connected(self):
        return not self._reconnecting and self._connection is not None and self._connection._connected
        ...
        for attempt in retry_range(self._retries):
            ...
            if not self.auth_key:
                try:
                    ...
                    # connect again.
                    self._log.warning("Connection error %d during auth_key gen: %s: %s", attempt, type(e).__name__, e)
    ...
    async def _try_connect(self, attempt):
        try:
            ...
            self._log.warning("Attempt %d at connecting failed: %s: %s", attempt, type(e).__name__, e)
            await asyncio.sleep(self._delay)
            return False
    ...
    async def _reconnect(self, last_error):
        ...
        for attempt in retry_range(retries, force_retry=False):
            try:
                ...
                last_error = e
                self._log.info("Failed reconnection attempt %d with %s", attempt, e.__class__.__name__)
                await asyncio.sleep(self._delay)
    ...
    async def _send_loop(self):
        ...
        while self._user_connected and not self._reconnecting:
            ...
            self._log.debug("Encrypting %d message(s) in %d bytes for sending", len(batch), len(data))
    ...
    async def _recv_loop(self):
        ...
        while self._user_connected and not self._reconnecting:
            ...
            try:
                ...
                self._log.info("Type %08x not found, remaining data %r", e.invalid_constructor_id, e.remaining)
                continue
    ...
    async def _handle_update(self, message):
        try:
            ...
            self._log.warning("Note: %s is not an update, not dispatching it %s", message.obj.__class__.__name__, message.obj)
            return
    ...
    async def _handle_state_forgotten(self, message):
        ...
        """
        self._send_queue.append(RequestState(MsgsStateInfo(req_msg_id=message.msg_id, info=chr(1) * len(message.obj.msg_ids))))
