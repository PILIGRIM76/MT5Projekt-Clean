    ...
    for name, task in tasks.items():
        ...
        try:
            ...
            if e.args != ("yield from wasn't used with future",):
                log.exception("Unhandled exception from %s after cancelling " "%s (%s)", name, type(task), task)
        except Exception:
            log.exception("Unhandled exception from %s after cancelling " "%s (%s)", name, type(task), task)
    ...
    async def __aenter__(self):
        ...
        if self._size is None:
            ...
            if seekable:
                ...
                _log.warning("Could not determine file size beforehand so the entire " "file will be read in-memory")
