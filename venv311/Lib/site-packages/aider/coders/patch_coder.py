...
class PatchCoder(Coder):
    ...
    def get_edits(self) -> List[EditResult]:
        ...
        if (
            ...
            is_patch_like = any(
                _norm(line).startswith(("@@", "*** Update File:", "*** Add File:", "*** Delete File:")) for line in lines
            )
    ...
    def _parse_patch_text(self, lines: List[str], start_index: int, current_files: Dict[str, str]) -> Patch:
        """
        ...
        while index < len(lines):
            ...
            if norm_line.startswith("*** Update File: "):
                ...
                if existing_action is not None:
                    ...
                    new_action, index, fuzz = self._parse_update_file_sections(lines, index, file_content)
    ...
    def _parse_update_file_sections(self, lines: List[str], index: int, file_content: str) -> Tuple[PatchAction, int, int]:
        ...
        while index < len(lines):
            ...
            if found_index == -1:
                ...
                raise DiffError(
                    f"Could not find patch context {marker} starting near line" f" {current_file_index}:\n{ctx_txt}"
                )
                    ...
                    if action.move_path:
                        ...
                        if target_path_obj.exists() and full_path != target_full_path:
                            self.io.tool_warning(
                                "UPDATE Warning: Target file for move already exists, overwriting:" f" {action.move_path}"
                            )
                    else:
