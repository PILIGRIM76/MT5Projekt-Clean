...
class AutoCompleter(Completer):
    ...
    def tokenize(self):
        ...
        for fname in self.all_fnames:
            ...
            self.words.update((token[1], f"`{token[1]}`") for token in tokens if token[0] in Token.Name)
...
class InputOutput:
    ...
    def __init__(
        ...
        if line_endings not in valid_line_endings:
            raise ValueError(
                f"Invalid line_endings value: {line_endings}. " f"Must be one of: {', '.join(valid_line_endings)}"
            )
    ...
    def get_input(
        ...
        show = ""
        if rel_fnames:
            rel_read_only_fnames = [get_rel_fname(fname, root) for fname in (abs_read_only_fnames or [])]
        ...
        def _(event):
            ...
            if self.multiline_mode and not (
                self.editingmode == EditingMode.VI and event.app.vi_state.input_mode == InputMode.NAVIGATION
            ):
