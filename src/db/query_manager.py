...
from .database_manager import TradeHistory, StrategyPerformance, TradeAudit
...
class QueryManager:
    ...
    def get_all_strategy_performance(self) -> pd.DataFrame:
        ...
        with self.session_factory() as session:
            query = select(StrategyPerformance).order_by(
                StrategyPerformance.profit_factor.desc()
            )
    ...
    def get_portfolio_metrics(self) -> Dict[str, Any]:
        ...
        with self.session_factory() as session:
            ...
            ).group_by(TradeHistory.strategy)
            strategy_profit = {
                r.strategy: r.profit 
                for r in session.execute(strategy_profit_query)
            }
            ...
            symbol_profit = {
                r.symbol: r.profit 
                for r in session.execute(symbol_profit_query)
            }
            
            # Сегодняшняя прибыль
            today = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)
            today_profit_query = select(
                func.sum(TradeHistory.profit)
            ).where(TradeHistory.time_close >= today)
    ...
    def get_audit_statistics(self) -> Dict[str, Any]:
        ...
        with self.session_factory() as session:
            ...
            executed = session.query(TradeAudit).filter(
                TradeAudit.execution_status == "EXECUTED"
            ).count()
            rejected = session.query(TradeAudit).filter(
                TradeAudit.execution_status == "REJECTED"
            ).count()
            failed = session.query(TradeAudit).filter(
                TradeAudit.execution_status == "FAILED"
            ).count()
    ...
    def get_monthly_performance(self, year: int) -> pd.DataFrame:
        ...
        with self.session_factory() as session:
            query = select(
                ...
                func.strftime('%Y', TradeHistory.time_close) == str(year)
            ).group_by(
                func.strftime('%m', TradeHistory.time_close)
            ).order_by('month')
    ...
    def get_drawdown_periods(self, threshold: float = -5.0) -> pd.DataFrame:
        ...
        with self.session_factory() as session:
            query = select(
                ...
                TradeHistory.profit < threshold
            ).order_by(TradeHistory.time_close.desc()).limit(50)
