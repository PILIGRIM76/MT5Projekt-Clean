...
class CommandManager:
    ...
    def update_trade_close(
        self, ticket: int, exit_price: float, profit: float, close_time: Optional[datetime] = None, close_reason: str = "TP/SL"
    ) -> bool:
        """
    ...
    def upsert_strategy_performance(
        ...
        with self.session_factory() as session:
            try:
                ...
                logger.info(
                    f"{'Обновлена' if perf else 'Создана'} запись производительности " f"для {strategy_name} на {symbol}"
                )
                return perf.id
    ...
    def update_strategy_status(self, strategy_name: str, symbol: str, market_regime: str, status: str) -> bool:
        """
    ...
    def bulk_update_strategy_status(self, updates: List[Dict[str, str]]) -> int:
        """
