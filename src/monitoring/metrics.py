...
TRADES_TOTAL = Counter("trades_total", "Total number of trades executed", ["symbol", "strategy", "type"])
...
TRADES_DURATION = Summary("trade_duration_seconds", "Trade duration in seconds", ["symbol", "strategy"])
...
ACCOUNT_EQUITY = Gauge("account_equity", "Current account equity")

ACCOUNT_MARGIN_USED = Gauge("account_margin_used", "Current margin used")

ACCOUNT_MARGIN_FREE = Gauge("account_margin_free", "Current free margin")

ACCOUNT_MARGIN_LEVEL = Gauge("account_margin_level", "Current margin level percentage")
...
PORTFOLIO_VAR = Gauge("portfolio_var", "Portfolio Value at Risk (99%)")

DAILY_DRAWDOWN = Gauge("daily_drawdown", "Current daily drawdown percentage")

OPEN_POSITIONS = Gauge("open_positions", "Number of open positions", ["symbol"])

RISK_EXPOSURE = Gauge("risk_exposure", "Current risk exposure", ["symbol", "type"])
...
MARKET_REGIME = Gauge("market_regime", "Current market regime", ["regime"])

VOLATILITY_INDEX = Gauge("volatility_index", "Current volatility index", ["symbol"])
...
PREDICTION_CONFIDENCE = Gauge("prediction_confidence", "Prediction confidence score", ["model_name", "symbol"])

CONCEPT_DRIFT_SCORE = Gauge("concept_drift_score", "Concept drift detection score", ["model_name", "symbol"])
...
SYSTEM_HEALTH = Gauge("system_health", "System health status (1=healthy, 0=unhealthy)", ["component"])

MEMORY_USAGE = Gauge("memory_usage_bytes", "Memory usage in bytes")

CPU_USAGE = Gauge("cpu_usage_percent", "CPU usage percentage")

EVENT_BUS_EVENTS = Counter("event_bus_events_total", "Total events published", ["event_type"])
...
STRATEGY_SIGNALS = Counter("strategy_signals_total", "Total signals generated", ["strategy", "signal_type"])
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                TRADES_TOTAL.labels(symbol=symbol, strategy=strategy, type=trade_type).inc()
                ...
                TRADES_DURATION.labels(symbol=symbol, strategy=strategy).observe(duration)
...
def update_account_metrics(balance: float, equity: float, margin_used: float, margin_free: float, margin_level: float) -> None:
    """
...
def update_strategy_metrics(strategy: str, win_rate: float, profit_factor: float, total_trades: int, total_pnl: float) -> None:
    """
