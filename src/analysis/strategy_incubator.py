...
class StrategyIncubator:
    ...
    def get_graduated_strategies(self) -> List[str]:
        ...
        return [name for name, data in self.strategies.items() if data["status"] == IncubationStatus.GRADUATED]

    def get_incubating_strategies(self) -> List[str]:
        """Возвращает список стратегий в инкубаторе."""
        return [name for name, data in self.strategies.items() if data["status"] == IncubationStatus.INCUBATING]

    def _generate_recommendation(self, criteria_passed: Dict[str, bool], stats: Dict[str, Any], days_trading: int) -> str:
