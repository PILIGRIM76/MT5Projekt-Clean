...
class FakeTradingSystemForBacktest:
    ...
    def get_rl_orchestrator_state(self) -> Dict[str, float]:
        ...
        if len(trade_series) < 20:
            return {
                "portfolio_var": 0.0, "weekly_pnl": 0.0,
                "sharpe_ratio": 0.0, "win_rate": 0.0,
                "news_sentiment": 0.0, "market_volatility": 0.5
            }
...
class SystemBacktester:
    ...
    def __init__(self, historical_data: pd.DataFrame, config: Settings, initial_balance: float = 10000.0):
        ...
        self.risk_engine = RiskEngine(config, trading_system_ref=self.fake_trading_system, querier=None,
                                      mt5_lock=threading.Lock(), is_simulation=True)
    ...
    def run(self) -> Dict[str, Any]:
        ...
        for i in range(start_bar, len(self.full_data)):
            ...
            regime_allocations = self.risk_engine.capital_allocation.get(current_regime,
                                                                         self.risk_engine.default_capital_allocation)
        ...
        report_generator = StrategyBacktester(strategy=None, data=pd.DataFrame(), timeframe=0, config=self.config,
    ...
    def _open_trade(self, signal: TradeSignal, candle: pd.Series, df_slice: pd.DataFrame, strategy_name: str):
        ...
        if stop_loss_in_price <= 0: return
    ...
    def _close_trade(self, position: Dict, exit_price: float):
        pnl_points = exit_price - position['entry_price'] if position['type'] == SignalType.BUY else position[
