...
class MonitoringService(BaseService):
    ...
    def _on_start(self) -> None:
        ...
        self._thread = threading.Thread(target=self._monitoring_loop, daemon=True, name="MonitoringService-Thread")
        self._thread.start()
    ...
    def _monitoring_loop(self) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            try:
                ...
                if (
                    current_time - self._last_graph_update_time > self._graph_update_interval
                ...
                if self._iteration_count % 300 == 0:  # 300 секунд = 5 минут
                    ...
                    if mt5_initialized:
                        self._logger.info(f"[Monitoring] ✓ MT5 активен (итерация #{self._iteration_count})")
                    else:
                        self._logger.warning(f"[Monitoring] ⚠ MT5 не активен (итерация #{self._iteration_count})")
                ...
                if not mt5.initialize(
                    ...
                    if isinstance(err_code, tuple) and err_code[0] == -6:
                        ...
                        if self._restart_mt5():
                            ...
                            self._logger.error("[Monitoring] ✗ Не удалось перезапустить MT5")
                            return

            # Если соединение восстановлено, сбросим флаг
            if self.trading_system.mt5_connection_failed:
                self._logger.info(f"[Monitoring] ✓ MT5 соединение восстановлено! Возврат в NORMAL режим торговли")
                self.trading_system.mt5_connection_failed = False
    ...
    def _update_balance(self, account_info) -> None:
        ...
        self.trading_system._safe_gui_update("update_balance", account_info.balance, account_info.equity)
    ...
    def _update_positions_light(self) -> None:
        ...
        if hasattr(self.trading_system, "_last_positions_cache"):
            ...
            if positions and self.trading_system._last_positions_cache:
                ...
                for p in positions:
                    cached = next(
                        (pos for pos in self.trading_system._last_positions_cache if pos.get("ticket") == p.ticket), None
                    )
                    if cached:
                ...
                if updated_positions:
                    self.trading_system._safe_gui_update("update_positions_view", updated_positions)
    ...
    def _restart_mt5(self) -> bool:
        ...
        try:
            ...
            if mt5.initialize(
                ...
                self._logger.info("[MT5-Restart] ✓ MT5 успешно подключен!")
                return True
            else:
                err_code = mt5.last_error()
