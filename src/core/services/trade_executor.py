...
class TradeExecutor:
    def __init__(
        self, config: Settings, risk_engine: RiskEngine, portfolio_service: PortfolioService, mt5_lock: threading.Lock
    ):
        self.config = config
    ...
    def _is_market_open(self, symbol: str, symbol_info: Any) -> bool:
        ...
        if hasattr(self.risk_engine, "trading_system") and self.risk_engine.trading_system:
            session_mgr = getattr(self.risk_engine.trading_system, "session_manager", None)
            if session_mgr and not session_mgr.is_trading_hours(symbol):
                logger.debug(f"[{symbol}] Трейдинг вне торгового времени (SessionManager).")
                return False
        ...
        if not is_24_7_instrument and not self.risk_engine.trading_system.config.ALLOW_WEEKEND_TRADING:
            ...
            if is_24_7_instrument:
                ...
                logger.debug(f"[{symbol}] Weekend trading разрешен конфигом. Проверка выходных пропущена.")
    ...
    async def _execute_twap(
        ...
        if stop_level_points <= 0:
            is_crypto = any(x in symbol.upper() for x in ["BTC", "ETH", "BIT", "COIN"])
            stop_level_points = 10 if is_crypto else 1
            logger.debug(f"[{symbol}] TWAP: Используем динамический stop_level={stop_level_points} (крипто={is_crypto})")

        if point <= 0:
            logger.error(f"[{symbol}] TWAP: Invalid point={point}. Пропуск.")
            return None
        ...
        if volume_step > 0:
            ...
            part_lot = round(round(lot_size / num_parts / volume_step) * volume_step, decimals)
        else:
        ...
        for i in range(num_parts):
            ...
            if not self.mt5_lock.acquire(timeout=0.5):
                logger.debug(f"[{symbol}] TWAP: MT5 Lock недоступен (timeout 0.5s) на части {i + 1}.")
                continue
            try:
                if not connector.initialize(path=self.config.MT5_PATH):
                    logger.error(f"[{symbol}] TWAP: Ошибка инициализации MT5 на части {i + 1}.")
                    break
                try:
                    tick = connector.symbol_info_tick(symbol)
                    if not tick:
                        logger.error(f"[{symbol}] TWAP: Нет тика на части {i + 1}.")
                        break
                    ...
                    tp = (
                        price + (sl_distance * self.config.RISK_REWARD_RATIO)
                        if signal_type == SignalType.BUY
                        else price - (sl_distance * self.config.RISK_REWARD_RATIO)
                    )
                    ...
                    request = self._build_request(
                        symbol, part_lot, signal_type, price, sl, tp, timeframe, strategy_name, digits
                    )
                    ...
                    if result and result.retcode == mt5.TRADE_RETCODE_DONE:
                        ...
                        if position_ticket is None or position_ticket == 0:
                            logger.error(f"TWAP PART {i + 1}/{num_parts} ОШИБКА: MT5 вернул DONE, но deal=0. Пропуск.")
                            break

                        if first_ticket is None:
                            first_ticket = position_ticket
                        logger.critical(f"TWAP PART {i + 1}/{num_parts} ИСПОЛНЕН. Ticket: {position_ticket}")
    ...
    async def _execute_market_order(
        ...
        if not self.mt5_lock.acquire(timeout=0.5):
            logger.debug(f"[{symbol}] _execute_market_order: MT5 Lock недоступен (timeout 0.5s). Пропуск.")
            return None
        try:
            if not connector.initialize(path=self.config.MT5_PATH):
                return None
            try:
                ...
                if stop_level_points <= 0:
                    is_crypto = any(x in symbol.upper() for x in ["BTC", "ETH", "BIT", "COIN"])
                    stop_level_points = 10 if is_crypto else 1
                    logger.debug(f"[{symbol}] Используем динамический stop_level={stop_level_points} (крипто={is_crypto})")

                if point <= 0:
                    logger.error(f"[{symbol}] MARKET ORDER: Invalid point={point}. Пропуск.")
                    return None
                ...
                if stop_loss_in_price is None or stop_loss_in_price <= 0:
                    logger.error(f"MARKET ОРДЕР ОШИБКА: stop_loss_in_price is None or <= 0. Пропуск ордера.")
                    return None
                if stop_loss_in_price < min_distance_price * 1.1:
                    logger.error(
                        f"MARKET ОРДЕР ОШИБКА: Рассчитанный SL ({stop_loss_in_price:.5f}) "
                ...
                sl = price - stop_loss_in_price if signal_type == SignalType.BUY else price + stop_loss_in_price
                tp = (
                    price + (stop_loss_in_price * self.config.RISK_REWARD_RATIO)
                    if signal_type == SignalType.BUY
                    else price - (stop_loss_in_price * self.config.RISK_REWARD_RATIO)
                )
                ...
                request = self._build_request(
                    symbol, lot_size, signal_type, price, sl, tp, timeframe, strategy_name, symbol_info.digits
                )
    ...
    def _calculate_adaptive_offset(self, df: pd.DataFrame, tick: Any, symbol_info: Any) -> float:
        ...
        dynamic_offset_pips = max(2.0, min(20.0, volatility_component + liquidity_component))

        logger.info(
            f"[{symbol_info.name}] Адаптивный офсет: {dynamic_offset_pips:.2f} пипс. "
    ...
    async def execute_trade(
        ...
        stop_loss_in_price: float,
        observer_mode: bool,
        prediction_input: Optional[Any] = None,
        entry_price_for_learning: Optional[float] = None,
    ) -> Optional[int]:
        """
        ...
        tick = None
        if not self.mt5_lock.acquire(timeout=0.5):
            logger.debug(f"[{symbol}] execute_trade: MT5 Lock недоступен (timeout 0.5s). Пропуск.")
            return None
        try:
            if not connector.initialize(path=self.config.MT5_PATH):
                return None
        ...
        if fair_spread is None:
            logger.warning(f"[{symbol}] fair_spread is None. Пропуск спред-фильтра.")
            return None
        ...
        normalized_atr_percent = (
            (df["ATR_14"].iloc[-1] / df["close"].iloc[-1]) * 100
            if "ATR_14" in df.columns and df["close"].iloc[-1] > 0
            else 1.0
        )
        ...
        if is_low_volatility and is_tight_spread:
            ...
            if position_ticket:
                ...
                logger.warning(f"[{symbol}] Limit-to-Market не сработал. Переход к Market Order.")
    ...
    async def _try_limit_entry_async(
        ...
        # Расчет SL/TP
        sl = limit_price - stop_loss_in_price if signal_type == SignalType.BUY else limit_price + stop_loss_in_price
        tp = (
            limit_price + (stop_loss_in_price * self.config.RISK_REWARD_RATIO)
            if signal_type == SignalType.BUY
            else limit_price - (stop_loss_in_price * self.config.RISK_REWARD_RATIO)
        )
                    ...
                    try:
                        ...
                        for pos in positions:
                            if pos.order == order_ticket:
                                logger.critical(f"[{symbol}] Limit ордер #{order_ticket} ИСПОЛНЕН! Position: {pos.ticket}")
                                return pos.ticket
    ...
    def _try_limit_entry(
        ...
        if signal_type == SignalType.BUY:
            ...
            tp = limit_price + (stop_loss_in_price * self.config.RISK_REWARD_RATIO)
        else:
        ...
        if not result or result.retcode != mt5.TRADE_RETCODE_DONE:
            logger.warning(f"[{symbol}] Не удалось выставить Limit ордер: {result.comment if result else 'Unknown'}")
            return None
            ...
            if not orders:
                ...
                if hist_orders:
                    ...
                    if state == mt5.ORDER_STATE_FILLED or state == mt5.ORDER_STATE_PARTIAL:
                        logger.info(f"[{symbol}] Limit ордер #{order_ticket} ИСПОЛНЕН!")
                        deal_id = hist_orders[0].deal
                        return self._wait_for_position_id(connector, deal_id)
                    elif state == mt5.ORDER_STATE_CANCELED:
                        logger.warning(f"[{symbol}] Limit ордер #{order_ticket} был отменен.")
                        return None
                return None
        ...
        logger.info(f"[{symbol}] Тайм-аут Limit ордера #{order_ticket}. Отмена...")
        cancel_request = {
    ...
    def _wait_for_position_id(self, connector, deal_id: int) -> int:
        ...
        for i in range(20):
            ...
            try:
                deals = connector.get_history_deals(date_from=None, date_to=None, ticket=deal_id)
    ...
    def _send_order_with_retry(self, connector, request):
        ...
        if check_result and check_result.retcode != mt5.TRADE_RETCODE_DONE:
            logger.info(f"Результат order_check для {request['symbol']} не 'DONE': {check_result.comment}")
    ...
    def _track_trade_outcome(self, symbol: str, profit: float):
        ...
        # Определяем исход
        if profit is None:
            logger.warning(f"[{symbol}] profit is None. Пропуск записи исхода.")
            return
        if profit > 0:
            outcome = "profit"
        elif profit < 0:
    ...
    def emergency_close_position(self, ticket: int):
        ...
        with self.mt5_lock:
            if not connector.initialize(path=self.config.MT5_PATH):
                logger.error("Не удалось подключиться к MT5 для закрытия позиции.")
                return
            try:
    ...
    def emergency_close_all_positions(self):
        ...
        logger.info(f"Обнаружено {len(positions_to_close)} позиций для закрытия. Начинаю процесс...")
    ...
    def modify_position_sltp_to_be(self, ticket: int, entry_price: float):
        ...
        with self.mt5_lock:
            ...
            try:
                positions = connector.get_positions(ticket=ticket)
                if not positions:
                    logger.warning(f"Не удалось найти позицию #{ticket} для перевода в БУ.")
                    return
