...
class RiskService(BaseService):
    ...
    def check_and_apply_hedging(self, positions: List, data_dict: Dict[str, pd.DataFrame], account_info) -> Optional[tuple]:
        """
        ...
        """
        try:
            result = self.risk_engine.check_and_apply_hedging(positions, data_dict, account_info)
            ...
            if result:
                ...
                self._logger.critical(f"ХЕДЖИРОВАНИЕ: {signal_type_name} {lot_size:.2f} по {symbol}")
    ...
    def calculate_position_size(self, symbol: str, df: pd.DataFrame, account_info, trade_type, strategy_name: str) -> tuple:
        """
        ...
        try:
            lot_size, sl_price = self.risk_engine.calculate_position_size(
                symbol=symbol, df=df, account_info=account_info, trade_type=trade_type, strategy_name=strategy_name
            )
