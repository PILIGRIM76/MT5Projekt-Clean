        ...
        try:
            with self.trading_system.mt5_lock:
                ...
                try:
                    ...
                    drawdown_from_peak = ((self.peak_equity - current_equity) / self.peak_equity) * 100 if self.peak_equity > 0 else 0
                    ...
                    if now.minute % 5 == 0 and now.second < 15:
                        logger.info(f"[SAFETY] Status OK: Daily Loss={daily_loss_percent:.2f}%, DD from Peak={drawdown_from_peak:.2f}%, Consecutive Losses={self.consecutive_losses}")
