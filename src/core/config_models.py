...
class MeanReversionStrategyParams(BaseModel):
    ...
    confirmation_buffer_std_dev_fraction: float = Field(default=0.05,
                                                        description="Буфер для подтверждения возврата к среднему.")
...
class MarketRegimeSettings(BaseModel):
    ...
    volatility_high_percentile: float = Field(default=0.80,
                                              description="Верхний процентиль для определения высокой волатильности.")
    volatility_low_percentile: float = Field(default=0.20,
                                             description="Нижний процентиль для определения низкой волатильности.")
...
class PreMortemSettings(BaseModel):
    ...
    tail_risk_multiplier: float = Field(default=2.0,
                                        description="Множитель для определения 'катастрофического' убытка (X * SL).")
    tail_risk_probability_threshold: float = Field(default=0.05,
                                                   description="Макс. допустимая вероятность катастрофического убытка.")
...
class TrailingProfitSettings(BaseModel):
    ...
    activation_threshold_percent: float = Field(default=0.3,
                                                description="Минимальная прибыль (%) для активации трейлинга.")
    pullback_percent: float = Field(default=0.15,
                                    description="Допустимый откат прибыли (%). Если цена откатит на этот % от пика, сделка закроется.")


class RiskEngineSettings(BaseModel):
    confidence_risk_map: Dict[str, float] = Field(default={"low": 0.5, "medium": 1.0, "high": 1.5},
                                                  description="Множители риска в зависимости от уверенности сигнала.")

    recent_trades_for_dynamic_risk: int = Field(default=20,
                                                description="Кол-во последних сделок для оценки динамического риска.")
    drawdown_sensitivity_threshold: float = Field(default=2.0,
                                                  description="Порог текущей просадки в %, при котором риск снижается до минимума.")
    portfolio_var_confidence_level: float = Field(default=0.99, description="Уровень доверия для расчета VaR портфеля.")
    toxic_regime_update_interval_sec: int = Field(default=3600,
                                                  description="Как часто обновлять кэш 'токсичных' режимов (в секундах).")
    toxic_regime_risk_multiplier: float = Field(default=0.5,
...
class RLManagerRewardSettings(BaseModel):
    ...
    partial_close_multiplier: float = Field(default=0.5,
                                            description="Награда за частичное закрытие прибыльной позиции.")
...
class RLManagerSettings(BaseModel):
    training_timesteps_per_trade: int = Field(default=2000,
                                              description="Кол-во шагов обучения на одну реальную сделку.")
    rewards: RLManagerRewardSettings = Field(default_factory=RLManagerRewardSettings)


class OrchestratorSettings(BaseModel):
    decision_interval_seconds: int = Field(default=14400,
                                           description="Интервал принятия решений Оркестратором (перераспределение капитала).")
    training_interval_seconds: int = Field(default=604800, description="Интервал дообучения RL-модели Оркестратора.")


class SystemSettings(BaseModel):
    initial_history_sync_days: int = Field(default=90,
                                           description="Глубина первоначальной синхронизации истории сделок (в днях).")
    history_sync_margin_minutes: int = Field(default=5,
                                             description="Запас времени для повторного запроса истории сделок.")
...
class Settings(BaseModel):
    ...
    INTER_MARKET_SYMBOLS: List[str] = Field(default=[],
                                            description="Символы для межрыночного анализа (напр. DXY, VIX).")
    INTER_MARKET_SYMBOL_ALIASES: Dict[str, List[str]] = Field(default_factory=dict,
                                                              description="Псевдонимы для межрыночных символов.")
    IGNORE_HISTORICAL_DRAWDOWN_ON_START: bool = Field(default=True,
                                                      description="Игнорировать просадку до запуска системы при расчете дневного лимита.")
    ...
    # --- ML Settings ---
    INPUT_LAYER_SIZE: int = Field(default=60,
                                  description="Размер входной последовательности (кол-во баров) для нейросетей.")
    TRAINING_DATA_POINTS: int = Field(default=2000, description="Кол-во баров для набора данных при обучении моделей.")
    PREDICTION_DATA_POINTS: int = Field(default=300,
                                        description="Кол-во баров, запрашиваемых для формирования прогноза.")
    ...
    ENTRY_THRESHOLD: float = Field(
        description="Минимальное прогнозируемое изменение цены (в %) для открытия сделки AI.")
    ...
    MAX_PORTFOLIO_VAR_PERCENT: float = Field(default=5.0,
    ...
    # --- News & Data Sources ---
    IMPORTANT_NEWS_ENTITIES: List[str] = Field(default_factory=list,
                                               description="Ключевые слова для определения важных новостей.")
    ...
    source_weights: Dict[str, float] = Field(default_factory=dict,
                                             description="Веса для различных источников данных в ConsensusEngine.")
    economic_calendar: Dict[str, Any] = Field(default_factory=dict,
                                              description="Настройки для экономического календаря.")
    ...
    ENABLE_KNOWLEDGE_GRAPH_VISUALIZATION: bool = Field(default=False,
                                                       description="Включает/отключает отрисовку Графа Знаний в GUI.")
