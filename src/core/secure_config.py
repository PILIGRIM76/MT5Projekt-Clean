...
class SecureConfigLoader:
        ...
        if self.encryption_key:
            try:
                ...
                if not self.encryption_key.startswith("ENCRYPTED:"):
                    self.cipher = Fernet(
                        self.encryption_key.encode() if isinstance(self.encryption_key, str) else self.encryption_key
                    )
    ...
    def load_mt5_credentials(self) -> dict:
        ...
        return {"login": int(login), "password": password, "server": server, "path": mt5_path}
    ...
    def load_database_credentials(self) -> dict:
        ...
        return {"neo4j": {"uri": neo4j_uri, "user": neo4j_user, "password": neo4j_password}}
