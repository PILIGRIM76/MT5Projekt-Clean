...
class AutoUpdater:
    ...
    def __init__(self, trading_system, bridge: QObject, check_interval_hours: int = 24):
        ...
        logger.info(f"AutoUpdater инициализирован. Режим: {'DEV (Git)' if self.is_dev_mode else 'PRODUCTION (GitHub Releases)'}")
    ...
    def _apply_release_update(self):
        ...
        try:
            ...
            if sys.platform.startswith('win'):
                ...
                subprocess.Popen([str(installer_path), '/SILENT', '/CLOSEAPPLICATIONS'], 
                               shell=False)
            else:
