...
class ConfigValidator:
        ...
        for field, expected_type in type_checks.items():
            if field in config:
                ...
                if not isinstance(value, expected_type):
                    self.errors.append(
                        f"Поле '{field}' должно быть типа {expected_type}, "
            ...
            if field in config:
                ...
                if not (min_val <= value <= max_val):
                    self.warnings.append(
                        f"{message}. Текущее значение: {value}. "
                        f"Используется безопасное значение по умолчанию."
                    )
        ...
        for field in path_fields:
            if field in config:
                ...
                if field == "MT5_PATH":
                    if not path.exists():
                        self.errors.append(
                            f"MT5 терминал не найден по пути: {path}"
                        )
                    elif not path.is_file():
                        self.errors.append(
                            f"MT5_PATH должен указывать на файл, а не директорию"
                        )
                ...
                # Для директорий создаем если не существуют
                    ...
                    if not path.exists():
                        self.warnings.append(
                            f"Директория БД не найдена: {path}. Будет создана."
                        )
                        try:
                            path.mkdir(parents=True, exist_ok=True)
                        except Exception as e:
                            self.errors.append(
                                f"Не удалось создать директорию БД: {e}"
                            )
    ...
    def get_safe_defaults(self) -> Dict[str, Any]:
        ...
        return {
            ...
            # Признаки
            "FEATURES_TO_USE": [
                "close", "tick_volume", "ATR_14", "RSI_14",
                "BB_WIDTH", "STOCHk_14_3_3", "MACD_12_26_9"
            ],
