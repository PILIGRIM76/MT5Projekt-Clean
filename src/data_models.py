...
class TradeSignalBase(BaseModel):
    ...
    confidence: float = Field(
        ...
        ge=0.0, 
        le=1.0,
        description="Уверенность сигнала от 0.0 до 1.0"
    )
    ...
    def validate_confidence(cls, v):
        ...
        if v < 0.3:
            raise ValueError(
                f'Уверенность сигнала слишком низкая: {v:.3f}. '
                f'Минимальный порог: 0.3'
            )
        return v
...
class TradeSignal(TradeSignalBase):
    ...
    predicted_price: Optional[float] = Field(
        None,
        gt=0,
        description="Прогнозируемая цена (должна быть положительной)"
    )
    stop_loss: Optional[float] = Field(
        None,
        gt=0,
        description="Стоп-лосс (должен быть положительным)"
    )
    take_profit: Optional[float] = Field(
        None,
        gt=0,
        description="Тейк-профит (должен быть положительным)"
    )
    strategy_name: Optional[str] = Field(
        None,
        min_length=1,
        max_length=50,
        description="Название стратегии, сгенерировавшей сигнал"
    )
    ...
    def validate_take_profit_vs_stop_loss(cls, v, values):
        ...
        if v is not None and 'stop_loss' in values and values['stop_loss'] is not None:
            ...
            if v <= sl:
                raise ValueError(
                    f'Take-Profit ({v}) должен быть больше Stop-Loss ({sl})'
                )
        return v
...
class TradeRequest(BaseModel):
    ...
    symbol: str = Field(
        ...
        min_length=4,
        max_length=10,
        description="Торговый инструмент (4-10 символов)"
    )
    lot: float = Field(
        ...,
        gt=0.0,
        le=100.0,
        description="Объем сделки (0 < lot <= 100)"
    )
    order_type: OrderType = Field(
        ...,
        description="Тип ордера: BUY или SELL"
    )
    stop_loss: Optional[float] = Field(
        None,
        gt=0,
        description="Стоп-лосс (опционально)"
    )
    take_profit: Optional[float] = Field(
        None,
        gt=0,
        description="Тейк-профит (опционально)"
    )
    strategy_name: Optional[str] = Field(
        None,
        min_length=1,
        max_length=50,
        description="Название стратегии для аудита"
    )
    ...
    def validate_symbol(cls, v):
        ...
        if not re.match(r'^[A-Z]{4,10}$', v) and v not in ['BITCOIN', 'GOLD', 'SILVER']:
            raise ValueError(
                f'Неверный формат символа: {v}. '
                f'Ожидается 4-10 заглавных букв (например, EURUSD)'
            )
        return v
...
class ClosePositionRequest(BaseModel):
    ...
    ticket: int = Field(
        ...,
        gt=0,
        description="Номер тикета позиции для закрытия"
    )
    partial_lot: Optional[float] = Field(
        None,
        gt=0,
        le=100.0,
        description="Объем для частичного закрытия (опционально)"
    )
    ...
    def validate_partial_lot(cls, v, values):
        ...
        if v is not None and v > 50.0:
            raise ValueError(
                f'Объем частичного закрытия слишком большой: {v}. '
                f'Максимум: 50.0'
            )
        return v
...
class NewsItemPydantic(BaseModel):
    ...
    sentiment: Optional[float] = Field(
        ...
        ge=-1.0,
        le=1.0,
        description="Сентимент новости от -1.0 до 1.0"
    )
