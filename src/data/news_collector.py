...
class NewsCollector:
    ...
    def __init__(self, config: Settings, db_manager: DatabaseManager):
        ...
        self.important_entities = getattr(
            config, "IMPORTANT_NEWS_ENTITIES", ["USD", "EUR", "GBP", "JPY", "Fed", "ECB", "NFP", "CPI"]
        )
        ...
        # Статистика
        self.stats = {"news_collected": 0, "last_collection_time": None, "sources_used": 0, "errors": 0}
    ...
    async def fetch_finnhub_news(
        self, symbol: str = "forex", start_date: Optional[datetime] = None, end_date: Optional[datetime] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            params = {"category": symbol, "token": self.finnhub_api_key}
    ...
    async def fetch_newsapi_news(self, query: str = "forex OR currency OR Fed OR ECB", days: int = 7) -> List[Dict[str, Any]]:
        """
    ...
    async def fetch_all_news(self) -> List[Dict[str, Any]]:
        ...
        logger.info(f"Сбор новостей завершён за {elapsed:.1f} сек. " f"Найдено {len(all_news)} уникальных новостей")
    ...
    def get_recent_news(self, symbol: Optional[str] = None, hours: int = 24, limit: int = 100) -> List[Dict[str, Any]]:
        """
    ...
    def get_news_sentiment(self, symbol: str, hours: int = 24) -> Dict[str, Any]:
        """
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {**self.stats, "symbols_monitored": len(self.symbols)}
