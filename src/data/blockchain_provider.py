    ...
    async def get_onchain_metrics(self, symbol: str) -> Optional[pd.DataFrame]:
        ...
        try:
            response = await self.client.post(
                self.base_url, json={"query": query}, headers={"Authorization": f"Apikey {self.api_key}"}, timeout=20.0
            )
            response.raise_for_status()
            data = response.json()
            ...
            df_mvrv = pd.DataFrame(data_root.get("mvrv", {}).get("timeseriesData", [])).rename(columns={"value": "mvrv_ratio"})
