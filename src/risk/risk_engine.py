...
class RiskEngine:
    ...
    def __init__(self, config: Settings, trading_system_ref=None, querier: Optional[KnowledgeGraphQuerier] = None,
                 mt5_lock: threading.Lock = None, is_simulation: bool = False):
        self.config = config
    ...
    def update_capital_allocation(self, new_allocation: Dict[str, float]):
        logger.warning(
            "RiskEngine: Использован устаревший метод update_capital_allocation. Обновлен только 'Default' режим.")
    ...
    def is_trade_safe_from_events(self, symbol: str) -> bool:
        ...
        affecting_events = self.knowledge_graph_querier.find_events_affecting_entities(
            ...
            source_types=important_source_types,
            time_window=block_window
        )
    ...
    def check_daily_drawdown(self, account_info) -> bool:
        ...
        if daily_profit < 0:
            ...
            if drawdown_percent >= self.max_daily_drawdown_percent:
                logger.critical(
                    f"!!! ТОРГОВЛЯ ОСТАНОВЛЕНА !!! Дневная просадка ({drawdown_percent:.2f}%) превысила лимит.")
                return False
        return True
    ...
    def update_correlation_matrix(self, data_dict: Dict[str, pd.DataFrame]):
        try:
            ...
            returns = closes.interpolate(method='linear', limit_direction='forward', axis=0).pct_change().dropna(
                how='all')
        ...
        with self.mt5_lock:
            ...
            try:
                ...
                if quote_currency != account_currency and self.trading_system.data_provider:
                    conversion_rate = self.trading_system.data_provider.get_conversion_rate(
    ...
    def check_and_apply_hedging(self, open_positions: List, data_dict: Dict[str, pd.DataFrame], account_info,
                                portfolio_var=None) -> \
