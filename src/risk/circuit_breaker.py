...
class CircuitBreakerTrip:
    ...
    def __init__(self, reason: CircuitBreakerReason, timestamp: datetime, context: Optional[Dict[str, Any]] = None):
        self.reason = reason
...
class CircuitBreaker:
    ...
    def _check_spread_normal(self) -> bool:
        ...
        try:
            ...
            if avg_spread > 0 and max_current_spread > avg_spread * self.spread_multiplier_threshold:
                logger.warning(f"Аномальный спред обнаружен! " f"Текущий: {max_current_spread:.2f}, Средний: {avg_spread:.2f}")
                return False
    ...
    def _check_volatility_normal(self) -> bool:
        ...
        try:
            ...
            for symbol in symbols:
                ...
                with self._mt5_lock():
                    rates = mt5.copy_rates_from_pos(symbol, mt5.TIMEFRAME_M1, 0, self.volatility_window_minutes)
    ...
    def trip(self, reason: CircuitBreakerReason, context: Optional[Dict[str, Any]] = None) -> None:
        ...
        with self._state_lock:
            ...
            trip = CircuitBreakerTrip(reason=reason, timestamp=self.last_trip_time, context=context)
            self.trip_history.append(trip)

            logger.critical(
                f"🚨 CIRCUIT BREAKER СРАБОТАЛ! " f"Причина: {reason.value}, " f"Срабатываний за сессию: {self.trip_count}"
            )
            ...
            if hasattr(self.trading_system, "alert_manager"):
                self.trading_system.alert_manager.send_alert(
                    level="CRITICAL", message=f"🚨 Circuit Breaker сработал: {reason.value}", context=context
                )
    ...
    def reset(self) -> None:
        ...
        with self._state_lock:
            ...
            if hasattr(self.trading_system, "alert_manager"):
                self.trading_system.alert_manager.send_alert(
                    level="INFO", message="✅ Circuit Breaker сброшен, торговля возобновлена"
                )
    ...
    def record_error(self) -> None:
        ...
        if self.consecutive_errors >= self.max_consecutive_errors:
            self.trip(CircuitBreakerReason.CONSECUTIVE_ERRORS, context={"error_count": self.consecutive_errors})
