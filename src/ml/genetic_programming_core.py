...
TERMINALS = {
    ...
    "AI_PREDICT_CHANGE": lambda df: (
        df["AI_PREDICT_VALUE"].pct_change() if "AI_PREDICT_VALUE" in df.columns else pd.Series(0.0, index=df.index)
    ),
...
class GeneticProgrammingCore:
    ...
    def _fitness(self, individual: dict) -> float:
        ...
        for i, segment_data in enumerate(segments):
            ...
            backtester = StrategyBacktester(strategy=temp_strategy, data=segment_data, timeframe=timeframe, config=self.config)
    ...
    def evolve(self):
        ...
        population = [
            {"buy_tree": self._create_random_tree(), "sell_tree": self._create_random_tree()}
        ...
        final_fitness_scores = sorted([(self._fitness(ind), ind) for ind in population], key=lambda x: x[0], reverse=True)
