...
class FeatureEngineer:
    ...
    def __init__(self, config: Settings, querier: Optional[KnowledgeGraphQuerier] = None):
        ...
        self.inflation_map = {"HIGHER_THAN_FORECAST": 1.0, "LOWER_THAN_FORECAST": -1.0, "RISES": 0.5, "FALLS": -0.5}
    ...
    def _get_related_entities(self, symbol: str) -> List[str]:
        ...
        if len(symbol) == 6:
            ...
            if "USD" in entities:
                entities.extend(["FED", "POWELL", "FOMC"])
            if "EUR" in entities:
                entities.extend(["ECB", "LAGARDE"])
            if "GBP" in entities:
                entities.extend(["BOE"])
            if "JPY" in entities:
                entities.extend(["BOJ"])
    ...
    def generate_graph_features(self, df: pd.DataFrame, symbol: str) -> pd.DataFrame:
        ...
        try:
            ...
            def get_inf_score(row):
                ...
                is_inflation_event = any(x in row["source_name"] or x in row["target_name"] for x in ["INFLATION", "CPI"])
    ...
    def generate_features(
        ...
        try:
            ...
            if onchain_data is not None and not onchain_data.empty:
                ...
                onchain_data = onchain_data.rename(
                    columns={"mvrv_ratio": "ONCHAIN_MVRV", "funding_rate": "ONCHAIN_FUNDING_RATE"}
                )

                if "ONCHAIN_MVRV" in onchain_data.columns:
                    onchain_data["ONCHAIN_MVRV_ZSCORE"] = (
                        onchain_data["ONCHAIN_MVRV"] - onchain_data["ONCHAIN_MVRV"].mean()
                    ) / onchain_data["ONCHAIN_MVRV"].std()
                ...
                df_out = pd.merge_asof(
                    df_out.sort_index(), onchain_data.sort_index(), left_index=True, right_index=True, direction="backward"
                )
            ...
            if lunarcrush_data is not None and not lunarcrush_data.empty:
                ...
                if "LC_SOCIAL_VOLUME" in lunarcrush_data.columns:
                    ...
                    lunarcrush_data["LC_SOCIAL_VOLUME_Z"] = (
                        lunarcrush_data["LC_SOCIAL_VOLUME"] - lunarcrush_data["LC_SOCIAL_VOLUME"].mean()
                    ) / lunarcrush_data["LC_SOCIAL_VOLUME"].std()

                # Объединяем LunarCrush данные
                df_out = pd.merge_asof(
                    df_out.sort_index(), lunarcrush_data.sort_index(), left_index=True, right_index=True, direction="backward"
                )
