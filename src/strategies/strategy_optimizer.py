...
class ParameterRange:
    ...
    def get_values(self) -> List[int | float]:
        ...
        if self.log_scale:
            ...
            values = np.logspace(
                np.log10(self.min_value), np.log10(self.max_value), num=int((self.max_value - self.min_value) / self.step) + 1
            )
        else:
            # Линейная шкала
...
class WalkForwardOptimizer:
    ...
    def optimize(self, data: pd.DataFrame, strategy_backtest_func: Callable) -> WalkForwardResult:
        """
        ...
        if len(data) < self.n_folds * 20:
            raise ValueError(f"Недостаточно данных: {len(data)} баров, " f"минимум {self.n_folds * 20} требуется")
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(folds):
            ...
            fold_result = self._optimize_fold(train_data, test_data, strategy_backtest_func, fold_idx)
        ...
        logger.info(f"Оптимизация завершена. " f"Рекомендуемые параметры: {result.optimal_parameters}")

        return result

    def _create_walk_forward_folds(self, data: pd.DataFrame) -> List[Tuple[List[int], List[int]]]:
        """
    ...
    def _optimize_fold(
        self, train_data: pd.DataFrame, test_data: pd.DataFrame, backtest_func: Callable, fold_idx: int
    ) -> Dict[str, Any]:
        """
        ...
        for params in param_combinations:
            ...
            is_metrics = self._evaluate_parameters(params, train_data, backtest_func)

            # Out-of-sample производительность
            os_metrics = self._evaluate_parameters(params, test_data, backtest_func)

            results.append({"parameters": params, "is_metrics": is_metrics, "os_metrics": os_metrics})
    ...
    def _generate_param_combinations(self) -> List[Dict[str, float]]:
        ...
        param_values = {param.name: param.get_values() for param in self.parameter_ranges}
    ...
    def _evaluate_parameters(
        self, params: Dict[str, float], data: pd.DataFrame, backtest_func: Callable
    ) -> StrategyPerformance:
        """
        ...
        try:
            ...
            if len(trades) < self.min_trades:
                # Недостаточно сделок
                return StrategyPerformance(sharpe_ratio=-999, total_trades=len(trades))
    ...
    def _calculate_composite_score(self, metrics: StrategyPerformance) -> float:
        ...
        composite = sharpe_normalized * 0.4 + pf_normalized * 0.3 + winrate_normalized * 0.2 + dd_normalized * 0.1

        return composite

    def _aggregate_results(self, fold_results: List[Dict[str, Any]], data: pd.DataFrame) -> WalkForwardResult:
        """
        ...
        # Рекомендация
        recommended, reason = self._make_recommendation(os_metrics, overfitting_coeff, robustness)
    ...
    def _aggregate_metrics_across_folds(self, fold_results: List[Dict[str, Any]], metrics_key: str) -> Dict[str, float]:
    ...
    def _calculate_overfitting_coefficient(self, fold_results: List[Dict[str, Any]]) -> float:
        """
        Расчет коэффициента переобучения.
    ...
    def _calculate_robustness_score(self, fold_results: List[Dict[str, Any]]) -> float:
        """
    ...
    def _make_recommendation(
        self, os_metrics: Dict[str, float], overfitting_coeff: float, robustness: float
    ) -> Tuple[bool, str]:
        """
...
def save_optimized_params(params: Dict[str, float], strategy_name: str, output_file: str = "configs/optimized_params.json"):
    """
    ...
    existing_params[strategy_name] = {k: int(v) if isinstance(v, float) and v.is_integer() else v for k, v in params.items()}
    ...
    logger.info(f"Оптимизированные параметры для {strategy_name} сохранены в {output_file}")


def load_optimized_params(strategy_name: str, input_file: str = "configs/optimized_params.json") -> Optional[Dict[str, float]]:
    """
