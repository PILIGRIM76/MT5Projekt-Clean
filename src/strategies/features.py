...
class FeatureStore:
    ...
    def _validate_data(self, df: pd.DataFrame, symbol: str) -> pd.DataFrame:
        ...
        if missing_cols:
            raise ValueError(f"Отсутствуют колонки для {symbol}: {missing_cols}")

        # Проверка на NaN
        nan_count = df[required_cols].isna().sum().sum()
        if nan_count > 0:
            logger.warning(f"{symbol}: найдено {nan_count} NaN значений, заполняем forward-fill")
            df = df.ffill().bfill()
    ...
    def _add_volatility_metrics(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "atr" in df.columns:
            df["atr_ratio"] = df["atr"] / df["atr"].rolling(window=window).mean()
    ...
    def _add_volume_profile(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["volume_zscore"] = (df["volume"] - avg_volume) / (volume_std + 1e-10)
    ...
    def _add_support_resistance(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["dist_to_resistance"] = (df["local_max"] - df["close"]) / df["close"] * 100
        df["dist_to_support"] = (df["close"] - df["local_min"]) / df["close"] * 100
    ...
    def _add_price_position(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["price_position"] = (df["close"] - rolling_min) / (range_size + 1e-10)
...
class BreakoutFeatureEngine:
    ...
    def calculate_breakout_features(self, df: pd.DataFrame, window: int, current_index: int) -> BreakoutFeatures:
        """
        ...
        if current_index < window + 1:
            self.logger.warning(f"Недостаточно данных для расчёта (index={current_index}, window={window})")
            return BreakoutFeatures()

        # Канал
        rolling_high = df["high"].iloc[current_index - window : current_index].max()
        rolling_low = df["low"].iloc[current_index - window : current_index].min()

        current_price = df["close"].iloc[current_index]
        prev_price = df["close"].iloc[current_index - 1]

        channel_width = rolling_high - rolling_low
        price_position = (current_price - rolling_low) / (channel_width + 1e-10)
        ...
        false_prob = self._estimate_false_breakout_probability(df, current_index, rolling_high, rolling_low)
    ...
    def _estimate_false_breakout_probability(
        self, df: pd.DataFrame, current_index: int, resistance: float, support: float
    ) -> float:
        """
