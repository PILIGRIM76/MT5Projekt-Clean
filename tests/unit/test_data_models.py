...
from src.data_models import (
    TradeSignal, TradeRequest, ClosePositionRequest,
    SignalType, OrderType, NewsItemPydantic
)
...
class TestTradeSignal:
    ...
    def test_confidence_validation_low(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            TradeSignal(
                type=SignalType.BUY,
                confidence=0.2,  # Ниже порога 0.3
                symbol="EURUSD"
            )
...
class TestNewsItemPydantic:
    ...
    def test_sentiment_validation(self):
        ...
        for sentiment in [-1.0, -0.5, 0.0, 0.5, 1.0]:
            news = NewsItemPydantic(
                source="TestSource",
                text="Тестовая новость достаточной длины",
                timestamp=datetime.now(),
                sentiment=sentiment
            )
