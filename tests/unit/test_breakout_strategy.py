...
from src.strategies.features import FeatureStore, BreakoutFeatureEngine, FeatureConfig
...
def default_settings() -> Settings:
    ...
    return Settings(
        ...
        STRATEGY_WEIGHTS={"breakout": 0.4,
                          "mean_reversion": 0.3, "ma_crossover": 0.3},
        ...
        EXCLUDED_SYMBOLS=[],
        rd_cycle_config={"sharpe_ratio_threshold": 1.2, "max_drawdown_threshold": 15.0,
                         "performance_check_trades_min": 20, "profit_factor_threshold": 1.1, "model_candidates": []},
        online_learning={"enabled": False, "learning_rate": 0.0001,
                         "adjustment_factor": 0.1, "max_expected_profit": 100.0},
        anomaly_detector={"enabled": True, "training_data_bars": 5000, "features": [
        ], "threshold_std_multiplier": 3.0, "risk_off_duration_hours": 4, "epochs": 50, "batch_size": 32},
...
class TestBreakoutFeatureEngine:
    ...
    def test_calculate_breakout_features(
        self,
        dataframe_with_features: pd.DataFrame
    ):
    ...
    def test_insufficient_data(self, dataframe_with_features: pd.DataFrame):
        ...
        features = engine.calculate_breakout_features(
            dataframe_with_features,
            window=15,
            current_index=10  # Меньше window + 1
        )

        assert features.channel_high == 0.0
        assert features.channel_low == 0.0

    def test_false_breakout_probability_range(
        self,
        dataframe_with_features: pd.DataFrame
    ):
...
class TestCheckEntryConditions:
    ...
    def test_signal_has_stop_loss_take_profit(
        self,
    ...
    def _create_breakout_dataframe(
        self,
        direction: str = 'up',
        n_bars: int = 50
    ) -> pd.DataFrame:
    ...
    def _create_channel_dataframe(self, n_bars: int = 50) -> pd.DataFrame:
        ...
        close = np.random.uniform(
            channel_low + 0.001, channel_high - 0.001, n_bars)
        high = close * 1.0002
        low = close * 0.9998
...
class TestDynamicConfidence:
    ...
    def test_confidence_range(self, breakout_strategy: BreakoutStrategy):
        ...
        df = breakout_strategy.feature_store.calculate_all_features(
            df, 'EURUSD')
        features = breakout_strategy.breakout_engine.calculate_breakout_features(
            df, breakout_strategy.window, 30
        )

        confidence = breakout_strategy._calculate_dynamic_confidence(
            df, 30, features, BreakoutType.UPPER_BREAKOUT, 60
        )

        assert 0 <= confidence <= 1

    def test_confidence_with_high_adx(
        self,
        breakout_strategy: BreakoutStrategy
    ):
        ...
        current_index = 100
        features = breakout_strategy.breakout_engine.calculate_breakout_features(
            df, breakout_strategy.window, current_index
        )
    ...
    def test_confidence_with_low_adx(
        self,
        breakout_strategy: BreakoutStrategy
    ):
    ...
    def test_timeframe_impact(self, breakout_strategy: BreakoutStrategy):
        ...
        confidence_m1 = breakout_strategy._calculate_dynamic_confidence(
            df, 30, features, BreakoutType.UPPER_BREAKOUT, 1
        )
        confidence_h4 = breakout_strategy._calculate_dynamic_confidence(
            df, 30, features, BreakoutType.UPPER_BREAKOUT, 240
        )
...
class TestExitConditions:
    ...
    def test_stop_loss_exit(
        ...
        exit_signal = breakout_strategy.check_exit_conditions(
            sample_dataframe,
            len(sample_dataframe) - 1,
            position
        )
...
class TestIntegration:
    ...
    def test_full_cycle_breakout(
        ...
        # Расчет признаков
        df = strategy.feature_store.calculate_all_features(
            sample_dataframe, 'EURUSD')
    ...
    def test_multiple_symbols(
        ...
        for symbol in symbols:
            ...
            signal = breakout_strategy.check_entry_conditions(
                df, len(df) - 1, 60)
...
class TestParametrized:
    ...
    @pytest.mark.parametrize("window,expected_warning", [
        (3, "ValueError"),
        (8, "WARNING"),
        (15, None),
        (60, "WARNING"),
    ...
    @pytest.mark.parametrize("timeframe,min_confidence", [
        (1, 0.0),    # M1
        (5, 0.0),    # M5
        (15, 0.0),   # M15
        (60, 0.0),   # H1
