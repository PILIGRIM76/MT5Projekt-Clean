...
@pytest.fixture
def sample_config():
    """Фикстура с тестовой конфигурацией (Mock)."""
    config = MagicMock(spec=Settings)

    # Создаем мок для position_sizing с правильным .get() и изменяемыми атрибутами
    position_sizing_mock = MagicMock()
    position_sizing_mock.method = "fixed_fractional"
    position_sizing_mock.fixed_risk_percent = 0.01
    position_sizing_mock.kelly_use_half = True
    position_sizing_mock.kelly_max_percent = 0.25
    position_sizing_mock.volatility_atr_multiplier = 2.0
    position_sizing_mock.risk_parity_total_risk = 0.05
    position_sizing_mock.risk_parity_max_positions = 5
    position_sizing_mock.min_lot = 0.01
    position_sizing_mock.max_lot = 100.0
    position_sizing_mock.lot_step = 0.01
    # Настройка .get() для возврата значений
    position_sizing_mock.get.side_effect = lambda key, default=None: getattr(position_sizing_mock, key, default)

    config.position_sizing = position_sizing_mock
...
class TestFixedFractionalSizer:
    ...
    def test_basic_calculation(self):
        ...
        result = sizer.calculate(
            symbol="EURUSD", signal_type=SignalType.BUY, account_equity=100000, entry_price=1.1000, stop_loss_price=1.0950
        )
        ...
        # Стоп-лосс 50 pips (с плавающей точкой)
        assert pytest.approx(result.stop_loss_pips, rel=1e-5) == 50.0
    ...
    def test_zero_stop_loss(self):
        ...
        # Валидация проходит, но lot = 0
        assert result.validation_passed is True
        assert result.risk_usd == 1000.0  # Риск всё равно рассчитывается
...
class TestKellyCriterionSizer:
    ...
    def test_kelly_with_stats(self):
        ...
        strategy_stats = {"win_rate": 0.60, "profit_factor": 2.0}  # 60% побед  # Прибыль/Убыток = 2
    ...
    def test_kelly_max_limit(self):
        ...
        strategy_stats = {"win_rate": 0.80, "profit_factor": 5.0}
...
class TestVolatilityAdjustedSizer:
    ...
    def test_basic_calculation(self):
        """Тест базового расчёта."""
        sizer = VolatilityAdjustedSizer(target_risk_percent=0.01, atr_multiplier=2.0)
...
class TestRiskParitySizer:
    ...
    def test_equal_risk_distribution(self):
        ...
        sizer = RiskParitySizer(total_risk_percent=0.05, max_positions=5)
...
class TestPositionSizer:
    ...
    def test_validation_max_lot(self, sample_config):
        ...
        # Лот должен быть больше максимума (валидация не проходит)
        # Валидация только помечает ошибку но не ограничивает
        assert result.validation_passed is False
        assert "max_lot" in str(result.validation_errors) or result.lot > 1.0
...
class TestPositionSizeResult:
    ...
    def test_to_dict(self):
        ...
        result = PositionSizeResult(
            lot=1.5, method=SizingMethod.FIXED_FRACTIONAL, risk_usd=150.0, risk_percent=1.0, stop_loss_pips=50.0
        )
