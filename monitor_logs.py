...
class LogMonitor:
    ...
    def __init__(self, log_file: str, hours: int = 4):
        ...
        # Статистика
        self.stats = {"INFO": 0, "WARNING": 0, "ERROR": 0, "CRITICAL": 0, "total_lines": 0}
    ...
    def process_line(self, line: str):
        ...
        if level == "ERROR" or level == "CRITICAL":
            ...
            self.errors_list.append({"timestamp": parsed["timestamp"], "component": component, "message": message})
        elif level == "WARNING":
            self.components[component]["warnings"] += 1
            self.warnings_list.append({"timestamp": parsed["timestamp"], "component": component, "message": message})
    ...
    def print_final_report(self):
        ...
        if self.components:
            print("\n🔧 КОМПОНЕНТЫ С ОШИБКАМИ:")
            sorted_components = sorted(self.components.items(), key=lambda x: x[1]["errors"], reverse=True)
...
def main():
    parser = argparse.ArgumentParser(description="Монитор логов Genesis Trading System")
    parser.add_argument("--hours", type=int, default=4, help="Длительность мониторинга в часах (по умолчанию 4)")
    parser.add_argument(
        "--log-file", type=str, default="logs/genesis.log", help="Путь к файлу логов (по умолчанию logs/genesis.log)"
    )
    parser.add_argument("--errors-only", action="store_true", help="Показывать только ошибки (genesis_errors.log)")
